...
5. **同じスクリプトを条件別に回すとき、`env` へ渡す変数を1つの変数にまとめない**。**このシェルは zsh で、引用符なしのパラメータ展開を語分割しない**。

   ```bash
   # ❌ PLAN に "free W=375 H=812" という文字列全体が入り、W・EXEMPT は未設定のまま
   for V in "PLAN=free W=375 H=812" "PLAN=premium W=1400 H=950"; do env $V node sweep.js; done
   # ✅ 変数ごとに渡す（関数にして引数で受けるのがいちばん安全）
   run(){ env PLAN="$1" W="$2" H="$3" EXEMPT="$4" node sweep.js; }
   ```

   条件を変えたつもりで**全条件が同じ数字を返す**が、スクリプトは正常終了し、pageerror も0件なので
   「どの条件でも異常なし」という**もっともらしいPASS**になる。
   2026-09-01（#102）実測: 管理者タブ巡回を free / premium / billingExempt で回したつもりが、
   3条件とも同一の出力になった。`plan=free` と `plan=premium` で数字が同じはずがないと気づいて初めて発覚した。

   **対策は、条件を変える検証では「受け取った値」をスクリプト側に印字させること**（`sweep.js` が
   `{plan, viewport, exempt}` を結果に含める）。渡したつもりの値ではなく、届いた値を見る。

6. **`h.clickByText` は `button` しか探さない**（mount-component.js:155 が `page.$$("button")` を回す）。
   Shifty には**クリック導線が `div` の `onClick` になっている画面がいくつもある**——スタッフ提出画面の
   名前カード（app-staff.js:318・:326「お名前を入力してください（必須）」）が代表例。`clickByText` は
   見つからないと**例外を投げずに `false` を返す**ので、戻り値を捨てていると「クリックしたのに
   何も起きない」＝アプリのバグに見える。`div` を押すときは自分で拾って `.click()` する
   （React の onClick は合成イベントなので、要素の `.click()` で正しく発火する）:

   ```js
   await h.evaluate(() => {
     const d=[...document.querySelectorAll("div")].filter(x=>x.innerText&&x.innerText.includes("お名前を入力してください")).pop();
     if(d) d.click();            // .pop() で最も内側＝実際に onClick を持つ div を取る
   });
   ```
...
7. **タブ側が「初期表示の絞り込み」を勝手に決めることがある**。提出一覧タブは
   ...
   検証したい提出が最新期間のものでないと、**その行はそもそも描画されない**。
   ハーネス側で明示的に戻すこと:

   ```js
   await h.evaluate(() => {
     const sel=document.querySelector("select");
     Object.getOwnPropertyDescriptor(window.HTMLSelectElement.prototype,"value").set.call(sel,"all");
     sel.dispatchEvent(new Event("change",{bubbles:true}));
   });
   ```

   **見分け方**: 提出一覧には「件数：N」が出ている（app-admin.js）。**渡した subs の件数と画面の
   「件数」が合っているかを最初に照合する**。合っていなければ、アプリではなく絞り込みを疑う。
   2026-09-04 実測: subs 3件を渡したのに `件数：1`。渡した配列を `window.__diag` に出して3件届いて
   いることを確認してから、初めて絞り込みに当たりが付いた。

**3〜7 の教訓は同じ**: 結果が期待と違ったとき、**アプリを疑う前に測り方を疑う**。#101 の「全件が同じ異常値を見たら、まず測定手段の故障を疑う」の小さい版で、ここでは「1件だけ想定外」でも同じことが起きる。#102 はさらにその裏返しで、**「複数条件が同じ値を返したら疑う」**——変わるべきものが変わっていない測定は、異常値より静かに嘘をつく。判定に使う値は、**修正前の実装に対して流して期待どおり落ちるか**（下記の `SHIFTY_ROOT`）で必ず裏を取る——素通りしている測定はここで露見する。
