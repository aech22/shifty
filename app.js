...
const CK_SHOP="ots_shopId";   // 現在のアクティブ店舗ID（後方互換用）
const CK_SHOPS="ots_shopIds"; // 複数店舗IDリスト（JSON配列）
const ckStaffKey=(shopId,periodId)=>`ots_staff_${shopId}_${periodId}`; // スタッフ名Cookie

// Cookie版の複数店舗リスト操作ヘルパー
function getCookieShops(){
  try{
    const v=getCookie(CK_SHOPS);
    if(v){const arr=JSON.parse(v);if(Array.isArray(arr)&&arr.length>0)return arr;}
  }catch{}
  // フォールバック: 旧形式（CK_SHOP）を1件として返す（"default"は除外）
  const single=getCookie(CK_SHOP);
  return (single&&single!=="default")?[single]:[];
}
function saveCookieShops(ids){
  setCookie(CK_SHOPS,JSON.stringify(ids),365);
  // CK_SHOPも現在のアクティブ店舗に合わせて更新
  if(ids.length>0)setCookie(CK_SHOP,ids[0],365);
}
...
function App(){
    ...
    firebaseDB.ref("global/shops").once("value").then(snap=>{
      ...
      if(parsed&&parsed.token&&parsed.type==="staff"){
        ...
        if(authUser){
          ...
          // 2) Auth なし → Cookie チェック（複数店舗対応）
          const ckIds=getCookieShops();
          if(ckIds.length>0){
            // Cookieに登録された全店舗を読み込む
            const cookieShops=ckIds.map(id=>sh.find(s=>s.id===id)).filter(Boolean);
            if(cookieShops.length>0){
              console.log("Cookie店舗:", cookieShops.map(s=>s.name));
              setShops(cookieShops);
              ls("shift_shops_v6",cookieShops);
              // セッション復元 or 最初の店舗
              const ssId=ssGet(SS_SHOP,null);
              const targetId=cookieShops.find(s=>s.id===ssId)?ssId:cookieShops[0].id;
              currentShopIdRef.current=targetId;
              setCurrentShopId(targetId);
              startSubscriptions(targetId,cookieShops);
              setReady(true);
            } else {
  ...
  useEffect(()=>{
    ...
    if(!_hasUrlToken){
      ...
      if(sid&&sid!=="default"){
        setCookie(CK_SHOP,sid,365);
        // CK_SHOPSにsidが含まれていなければ追加（初回起動時等）
        const ckIds=getCookieShops();
        if(!ckIds.includes(sid)){
          saveCookieShops([...ckIds,sid]);
        }
      }
    }
  },[sid]);
  ...
  const applyInviteCode=()=>{
    ...
    firebaseDB.ref("global/shops").once("value").then(snap=>{
      ...
      if(found){
        ...
        // Cookie: 既存リストに追加（重複なし）
        const ckIds=getCookieShops();
        if(!ckIds.includes(code)){
          const newIds=[...ckIds,code];
          saveCookieShops(newIds);
        } else {
          setCookie(CK_SHOP,code,365);
        }
        const existingShops=shops.filter(s=>s.id!==code);
        const newShops=[...existingShops,found];
  ...
  const createNewShop=()=>{
    ...
    firebaseDB.ref("global/shops").once("value").then(snap=>{
      ...
      // Cookie: 既存リストに追加
      const ckIds=getCookieShops();
      saveCookieShops([...ckIds,newShop.id]);
    ...
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:"#0F172A",padding:"20px"}}>
      <div style={{background:"#1E293B",borderRadius:24,padding:"36px 28px",width:"100%",maxWidth:420,boxShadow:"0 12px 40px rgba(0,0,0,.5)"}}>
          ...
          :emailMode
            ...
            :<>
              ...
              <button onClick={signInWithApple} disabled={authLoading}
                style={{width:"100%",padding:"14px",background:"#1C1C1E",border:"1px solid #334155",borderRadius:14,color:"white",fontSize:15,fontWeight:700,cursor:"pointer",marginBottom:10,display:"flex",alignItems:"center",justifyContent:"center",gap:10}}>
                <svg width="18" height="20" viewBox="0 0 814 1000" fill="white"><path d="M788.1 340.9c-5.8 4.5-108.2 62.2-108.2 190.5 0 148.4 130.3 200.9 134.2 202.2-.6 3.2-20.7 71.9-68.7 141.9-42.8 61.6-87.5 123.1-155.5 123.1s-85.5-39.5-164-39.5c-76.5 0-103.7 40.8-165.9 40.8s-105-43.4-150.3-109.3C89.3 743.9 53.4 668.8 53.4 534.8 53.4 355.5 171.2 261 287.1 261c64 0 117.6 42.2 158.1 42.2 38.2 0 98.9-44.7 166.1-44.7 28.1 0 108.2 2.7 175.9 105.1zM553.4 71.6c31.1-36.9 53.1-88.1 53.1-139.3 0-7.1-.6-14.3-1.9-20.1-50.6 1.9-110.8 33.7-147.1 75.8-28.5 32.4-55.1 83.6-55.1 135.5 0 7.8 1.3 15.6 1.9 18.1 3.2.6 8.4 1.3 13.6 1.3 45.4 0 102.5-30.4 135.5-71.3z"/></svg>
                Appleでログイン
              </button>
        ...
        <div style={{marginTop:20,fontSize:11,color:"#475569",textAlign:"center",lineHeight:1.7}}>
          複数端末でデータを同期するにはGoogle/Apple/メール認証をご利用ください。<br/>
  ...
  return(
    <div style={{fontFamily:"'Hiragino Sans','Yu Gothic',sans-serif",minHeight:"100vh",background:"var(--c-bg)"}}>
      ...
      {syncStatus!=="online"&&<div style={{background:syncStatus==="offline"?"#F59E0B":"#6B7280",color:"white",fontSize:11,fontWeight:700,textAlign:"center",padding:"4px 8px"}}>
        {syncStatus==="offline"?"🟡 オフライン（再接続中...）":syncStatus==="no_config"?"⚙️ Firebase未設定":"⏳ 接続中..."}
      </div>}
              ...
              :<>
                <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12}}>
                  ...
                  <div style={{display:"flex",gap:5,flexShrink:0,flexWrap:"wrap",justifyContent:"flex-end"}}>
                    <button onClick={e=>{e.stopPropagation();setEid(p.id);}} style={{padding:"5px 9px",background:"var(--c-input)",border:"1px solid #E5E7EB",borderRadius:6,color:"var(--c-text2)",fontSize:11,cursor:"pointer"}}>✏️ 編集</button>
                    <button onClick={e=>{e.stopPropagation();expXl(p,subs,staffList,tt,settings.xlShopName||shopName,{staffColors:settings.staffColors||{}});}} style={{padding:"5px 9px",background:"linear-gradient(135deg,#c45e1f,#a34d19)",border:"none",borderRadius:6,color:"white",fontSize:11,fontWeight:700,cursor:"pointer"}}>📊 Excel</button>
...
function PEF({period,onSave,onCancel}){
  ...
  return(<div onClick={e=>e.stopPropagation()}>
    <div style={{fontSize:13,fontWeight:700,color:"var(--c-text2)",marginBottom:10}}>✏️ 編集中</div>
...
function StaffTab({staffList,onSave,tt,plan="free",onUpgrade,onRenameStaff,settings={},onSaveSettings,subs=[],periods=[]}){
  ...
  const confirmEdit=i=>{
    ...
    if(!trimmed){tt("⚠️ 名前を入力してください");return;}
    if(staffList.includes(trimmed)&&trimmed!==staffList[i]){tt("⚠️ 既に登録されている名前です");return;}
