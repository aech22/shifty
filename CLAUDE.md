...
## Shifty バグチェックレポート（2026-08-18 自動実行 #82）

### 修正済み

- **🟡 勤務時間の上限判定が、期間をまたいだ勤務を数えていなかった**（[app-admin.js:3026](app-admin.js)）→ `cac2b21`
  提出一覧の行バッジ（週超過・月超過・連勤超過）は `ds=Object.keys(sub.shifts).sort()` ＝**その sub 自身の日付だけ**で集計していた。**1つの sub は1期間ぶんの日付しか持たない**ため、期間の境界にかかる週と、`periodUnit:"2week"` のときの月は**構造的に必ず過少**になる。
  **配信物の判定ブロックを切り出して Node で実行した実測**: 8月を2週×2期間に分け **20日×8時間＝160時間**働いても、**月上限100時間の判定は両期間とも `false`**。その裏で**同じ画面の詳細モーダルは月計 `160:00` と表示している**（:3084）。週も同様で、週(月)8/10〜8/16 に期間Aの16時間＋期間Bの16時間＝32時間あっても、**週上限25時間が両期間とも `false`**。
  **断定できた根拠は、同じ「その週/月の勤務時間」を出す実装が他に2つあり、どちらも既に期間をまたいでいたこと**——シフト作成タブの `getWeekMin`（:1026・`_getWorkShift` 経由で全 sub の name|date マップを引く）と、詳細モーダルの週間勤務時間/月計（:3084・`wSS` フォールバック）。**3箇所のうちこの1箇所だけが期間内に閉じていた。**
  さらに**仕様側にも決着がついていた**: 完了済みBACKLOG「週間勤務時間の表示（前期間跨ぎ対応）」（2026-06-18）の受け入れ条件が「**週が前期間・次期間にまたがる場合、他の提出データも合算**」と明記している。一方で上限判定を入れた「スタッフ属性設定と勤務時間制限」は**その2日前（2026-06-16）**の実装で、期間跨ぎの意味が決まる前に書かれ、以後更新されていなかった。**仕様判断ではなく実装の取り残し**と確定できた。
  多数派（期間跨ぎ）へ揃え、別名フォールバックも合わせた（`registerAlias` は `sub.staffName` を書き換えないため別名提出は別名のまま残る）。日次上限は1日が1つの sub に属するので従来どおり sub 自身の日付で判定する。**修正後のブロックを再度切り出して15ケース実行**（期間跨ぎ4・非回帰6・別名2・境界3）し全パス。

- **🟡 他店舗の勤務判定が、管理者の休み希望マークを無視していた**（[app-admin.js:397](app-admin.js)・[:861](app-admin.js)）→ `2e59fe4`
  `effShiftStart`/`effShiftEnd`（app-utils.js:113-122）は「**管理者が休み希望(`y`/`休`)を入れたセルは `""` を返す**」という判定の正本で、#50 で追加されたときコメントに「戻り値の `""` は呼び出し側の `st&&en` 判定で値なしとして扱われる」と**呼び出し規約まで書いてある**。ところが **app-admin.js での採用は0箇所**で、生の `adjustedStart??sh.start` を**6箇所に手で展開**していた。うち1箇所（:526 `getStoredTime`）だけは `adminRest` ガードを手写ししており、**規約を知っていた形跡がありながら他へ展開されなかった**ことが分かる。
  実害のある2箇所を揃えた。**:397 `hasBoth`** は同一 `name|date` に複数 sub があるとき `workMap` に残す方を選ぶ判定で、休みマーク済みのセルが「両方入っている」と見なされ**実際に勤務している別 sub より優先されうる**。**:861 の他店舗シフト重複エラー**は、他店舗側が休み希望マークを付けたセルでも提出時刻で重なりを見るため、**休みの日に「○○店と重複」の赤エラーが出る**。
  **修正後の2式を配信物から切り出して7ケース実行**（休みマーク2・非回帰5）し全パス。`timeToMin` が falsy で `null` を返すため :861 は直後の `if(os===null||oe===null)continue` で正しく除外されることも実測で確認した。
  **`effShiftStart`/`effShiftEnd` が eslint globals に未登録だった**ことも判明し追加した（app-admin.js からヘルパーが一度も使われてこなかったのは、使うと `no-undef` で落ちたためと思われる）。**ヘルパーを作っても呼び出し側の設定を通していなければ採用されない**という形の負債。

### 要確認（未修正）

- **🔴（継続・プロセス）このループから実機E2Eが5回続けて実行できない**（環境要因・コード変更なし）
  今回は `preview_list`（稼働0件）と `preview_start` を実際に実行して確認し、**「Dev servers can't be started from unattended sessions」と明示的に拒否された**。Bashでのサーバー起動は規約で禁止のため代替なし。**#78〜#81 に続き今回も1件も実行できなかった。ユーザーが対話セッションで `/bug-check` を手動実行すれば解消する。**
- **🟢 `effShiftStart`/`effShiftEnd` の未採用が app-admin.js になお3箇所残る**（`:994` ヒートマップの時間軸／`:3027` 提出一覧の出勤日数／`:3085` 詳細モーダルの出勤日数）。**今回あえて触らなかった**——この3つは生の値で `(st&&en)` を見たあと `shiftBandInfo(sh)` に**生の shift を渡し直しており、内側では `effShiftStart` が効く**。つまり1つの式の中で2つの「実効開始時刻」が混在するが、**分岐の結果は「片側セルの日を何日と数えるか」に合流する**ため、BACKLOG の判断待ち項目（下記）が決まらないと正しい直し方が定まらない。
- **🟡 #77 の宿題（Free 上限の実機検証）は静的監査完了・実測のみ未達**（上記🔴が理由）。**残るは実機で21人目・2件目を押す1回だけ**
- **🟢 `index.html` の `?v=` と `build:` が現在の配信物を指していない**（`20260814-53f610f` のまま。**今回さらに app-admin.js を2回変更したので差は広がった**）。申し送りのみ——`/release-to-main` の手順63がバンプする
...
- 以下は #46〜#81 からの継続（変化なし）: 未claim店舗の取り込み／`findActiveSubscription` の `|| live[0]`／`planExpiry` のUTC計算／**片側セルだけの日が0分**／`purgeOldPeriods` のUTC日付／`generateInviteCode` のデッドコード／ほか

### 今回の対象と見つけ方

**#81 以降のコミットは0件。10回続けて「差分ゼロ」の回である。**

**(1) 申し送りどおり「日付・期間・時刻で引いている述語」のレンズを回し、今回は当たりがレンズの真ん中にあった。** #81 は「トーストのレンズを回したが当たりは確認文だった」＝**外れたレンズの途中で拾った**形だったが、今回は指定どおりの場所から2件とも出た。**申し送りの精度が上がっている。**

**(2) 「同じ操作を正しく書いている箇所が他にあるか」が5回連続で決め手だった。** 1件目は**3箇所中2箇所が既に期間跨ぎ**、2件目は**ヘルパー本体＋手写し1箇所が既に休みマークを見ていた**。どちらも多数派の式へ揃えるだけで済み、設計判断が要らなかった。

**(3) 今回は「多数派」に加えて『仕様側の決着が完了済みBACKLOGに書いてあるか』を確認した。** 1件目は「期間跨ぎで合算する」が**完了済みタスクの受け入れ条件として明文化**されており、しかも上限判定の実装が**その2日前**だと分かった。**コードの多数決だけなら仕様判断の疑いが残るが、日付の前後関係まで取れたので「取り残し」と断定できた。** #81 の仕分け基準（検証手段があるか）に、**「仕様がいつ決まり、その実装がいつ書かれたか」という時系列**を足せた。

**(4) 「ヘルパーが呼ばれていない」ことの原因が lint 設定側にあった。** `effShiftStart` の採用が0箇所である理由を探して eslint globals の未登録に行き着いた。**「規約はコメントに書かれ、手写しまでされているのに、機械が使わせてくれない」**という形の負債で、コードだけを読んでいると「なぜか誰も使わない関数」に見える。

**(5) 配信物をNodeで実行する手法が5回目の定着。** 今回は**修正前の判定ブロックで欠陥を実証し、修正後の同じブロックを切り出し直して15ケース＋7ケースを走らせた**。ブラウザ無しで「バッジが出るか出ないか」を真偽値で読めている。

### 検証したこと

- `DEV_MODE`（app-core.js:12・式のまま）／`DEV_PLAN_OVERRIDE`（:125）正常
- `subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／CF `.delete()` **0件**／CF secrets **6宣言**
- 読み込み順維持・SRI **11本**／`database.rules.json` の `".read": true` **0件**
- 未定義CSS変数 **0件**／ダーク二重管理の乖離**なし**／SVG属性内 `var()` は `app-staff.js` の1件で増えていない
- `npm test` **168件パス**／`npx eslint app-*.js` **0 errors 98 warnings**（修正前後で増減なし）／`node --check functions/index.js` OK
- **日付・期間・時刻レンズ**: `periodId` で引く**16箇所**／`gd(` の**12箇所**／週起点計算の**3箇所**／締切判定（`idp` と `subHasRealUpdate`・**両者とも `T23:59:59` のローカル時刻で一致**）／`effShiftStart` 相当の式**7箇所**を列挙して突き合わせ
- **`source:"grid"` の除外レンズ**: `periodId` で引く7箇所のうち4箇所が除外・3箇所が非除外であることを確認。**非除外3箇所はいずれも誤りではない**（:2012 Excel と :1291 PDF はグリッド入力を出力に載せるのが正しく、:2290 は後段の `!staffList.includes(n)` が実質的に打ち消す）ため**報告のみ**
- **修正の検証（配信物の実評価）**: 上限判定15ケース／`effShift` 系7ケース
- **未検証（今回は手段が無い）**: ブラウザでの実描画すべて／**新しく出るようになった上限バッジの実表示**／**2店舗を連携した重複エラーの実表示**／**Free 上限に達した店舗での実ブロック**／実機 iOS Safari

### 総括

**測れない回が5回続き、それでも 🟡 が2件出て2件とも直った。** どちらも「同じ意味の値を2つの式で出している」形で、**#81 が名指しした場所（日付・期間・時刻の述語）から出た。**

**1件目は影響が大きい。** 週40時間・月上限は労基法に紐づく機能で、`periodUnit:"2week"` の店舗では**月上限が構造的に一度も発火しない**状態だった。しかも**同じ画面の詳細モーダルは正しい月計を表示していた**ので、管理者は「160時間と表示されているのに警告が出ない」を見ていたことになる。

**2件目は「負債の形」が新しい。** ヘルパーも規約コメントも手写しの前例もあったのに採用0だった原因が**lint設定の未登録**で、コードを読むだけでは辿り着けない。**「使われていない関数」を見つけたら、使えない理由が設定側にないかを見る**——次回以降の手として残す。

**次回の手を1つに絞る。** 実測が復活したなら**#77 の宿題（実機で21人目・2件目を押す1回）をそのまま実行する**。使えないままなら、推奨は**「集計の入力になっている日付集合」を横断で洗う**こと——今回の当たりは2件とも「どの日を数えるか」が式ごとに違った例で、`ds`（sub自身）・`dates`（期間）・`allD`（前期間込み）・`perDs`・`allWork` と**5種類の日付集合が名前も範囲もバラバラに共存している**。`consecCounts`（:1128）は「期間内」とコメントで明示しているので正しいが、**明示のないものが残っていないか**を次に見る。
...
**影響範囲**: app-utils.js（`carryAdminShiftFields`・`calcNetWorkMinutes`・`resolveAlias`）、app-staff.js（`buildShift`）、app-admin.js（集計・上限判定・ポジション設定UI・`del`:2314・`gridStaff`:1060・`heatData`:762・`expXl`:2015・`buildPdfCols`:1289）、tests/core.test.js
**備考**: バグチェック #46・#52・#57・#64・#65・**#79** で継続検出・**条件B（仕様判断）に該当**。各項目とも「実装は難しくないが、正解が仕様側にしかない」。**#79 の2項目はどちらも「スタッフを削除する」という1つの操作から出ている**が、片方は設定データの後始末、もう片方は未登録名の表示規則で、決め方が独立するため分けて記載した。なお #79 では**確認ダイアログの不在だけ**（他の破壊的操作4箇所は全て confirm を持つ＝正解がコードベース内にあった）を判断待ちにせず `69f62bf` で修正済み。
