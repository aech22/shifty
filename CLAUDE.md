...
## Shifty バグチェックレポート（2026-09-19 自動実行 #136）

> 着手時の HEAD は `14348df`。#135 以降 app-*.js・functions/index.js・database.rules.json は変わっていなかった。
> #135 の申し送り「入力の側」から、期間の開始日を編集したときに日付を鍵に持つ設定がどう読まれるかを当て、
> **🟡を1件見つけて直した**。

### 修正済み

- **[🟡] 期間の開始日を編集すると、非表示スタッフの範囲がずれる**（app-admin.js:2096・`fa4c982`）
  `staffHidden` の範囲は期間の `startDate` を値で持つが、期間編集（`PEF`）の保存は `periods` しか書いていなかった。
  非表示にした期間の開始日を前へずらすと、その期間のシフト表・Excel・PDFに名前が戻る。
  解除した期間の開始日を前へずらすと、表示に戻したはずの人が消える。どちらも警告は出ない。
  `moveStaffHiddenBoundaries`（app-utils.js）を追加し、旧開始日と同じ境界を新しい開始日へ移すようにした。

  | 操作（田中を9月前半だけ非表示にした状態から） | 修正前 | 修正後 |
  |---|---|---|
  | 9月前半の開始日を 09-01 → 08-31 | 9月前半が**表示に戻る** | 9月前半は非表示のまま |
  | 続けて9月後半の開始日を 09-16 → 09-15 | 9月後半が**非表示になる** | 9月後半は表示のまま |
  | 8月後半の開始日を編集（境界と無関係） | 設定を書かない | 設定を書かない |

  実測は実ブラウザで `PeriodsTab` だけをマウントし、`onSaveSettings` をスパイに差し替えて行った（Firebase へは出していない）。
  修正前の数値は、同じスクリプトを HEAD の配信物に向けて取った。コンソールエラーは0件。

### 要確認（未修正）

- **[🟢] 同じ開始日の期間が2つあると境界を動かさない**（app-utils.js の `moveStaffHiddenBoundaries`）。
  境界がどちらの期間を指して書かれたか区別できないためで、意図的に残した。重なる期間の作成には既に確認ダイアログが出る。
- **#135 から継続（変化なし）**: `dup`・`posErr` が画面限定である理由がPDFのコメントに無い（🟢）／
  部門別PDFで未登録名の列が落ちる理由のコメントが無い（🟢）／休み提出の日に斜線と黄色が両方乗る（🟢）／
  「締」が無効な店舗でもメモから「締」が消える（🟢）／「締」入りの略称（🟢）／店舗切替で `plan` が同期リセットされない（🟢）／
  `purgeInactiveShops` の招待コード後始末（🟢）／企業ログインの試行制限（🟡 → BACKLOG化済み）／`recentPeriodIds` がテスト専用（🟢）／
...
- **`keepAttrs`・`keepStaff` も開始日の編集でずれる → 否定**。どちらも期間IDをキーに持つので日付を動かしても外れない。
- **改名で `staffHidden` が取り残される → 否定**。`STAFF_KEYED_SETTING_MAPS` に入っており、`renameStaffInSettings` で範囲ごと新しい名前へ移る（実測）。

### 検証したこと

- `npm test` **264件パス**（+2）。追加した2件は修正前の app-utils.js で落ちることを確認した。
- `npx eslint app-*.js` **0 errors / 95 warnings**（増減なし）。
- 配信版数は `20260916-c20a092` のままで、#134 と今回の app-*.js 修正を指していない。リリース時にバンプすれば足りる。
- Firebase・Stripe には一切アクセスしていない。`.cursorrules` の未コミット変更は着手前からあったもので触っていない。

**申し送り（次回の観点）**: 値で他のデータを指している箇所は、今回の `staffHidden` と同じ壊れ方をする。
期間の開始日のほかに、店舗略称（`shopAbbrs`）を変えたときのセル値や、候補時間を消したときの `dateCandidatePosTypes` の参照を当てるとよい。
