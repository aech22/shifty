...
## Shifty バグチェックレポート（2026-08-04 自動実行 #53）

### 修正済み

- **[🟡] 片側セルのみ入力のスタッフがポジション不足判定から丸ごと脱落し、埋まっている枠を「不足」と誤報していた**（app-admin.js:869 `positionErrors`／:919 `staffSectionOn`／コミット `1fbc44c`）
  シフト作成グリッドは出勤セルと退勤セルが独立していて、**片側だけ埋まっている状態が正規の中間状態**として扱われている。ヒートマップ（`heatData`・:766-768）はこれを明示的にサポートし、「出勤のみ→ランチ終わり(`HEAT_LUNCH_END_MIN`)まで」「退勤のみ→ディナー始まり(`HEAT_DINNER_START_MIN`)から」出勤扱いに補完してカウントする。ところが同じ「その日そのスタッフは出ているか」を判定する `positionErrors` は `if(s===null||e===null||s>=e)return;` で**補完せず早期returnし、片側入力のスタッフを attendees から丸ごと落としていた**。結果、そのスタッフが持っているポジションが未充足とみなされ、**実際には埋まっている枠に対して不足エラー（グリッド上部のエラーパネル＋セル赤ハイライト）が出る**。
  **非対称の構図**: `positionErrors` のコメント自身が「section判定は heatData と同じ入口(`bandSectionsOf`)を使い」と**heatData との整合を明示的に主張している**のに、その手前の在席判定だけが揃っていなかった。ヘルプ帯クリップ（`resolveBandValues`）は両者で丁寧に揃えてあり、**揃える意図があったうえで null ガードだけが取り残された**形。
  **実測検証**（`app-utils.js` の実関数 `HEAT_BAND_SPLIT_MIN`／`matchPositionSlots` を require し、両者の判定規則を同一入力に通した Node 再現）: ①出勤`9:00`のみ → heatData は**在席(lunch)**、positionErrors は**attendeeから除外** → ランチ「揚げ場」1枠に対し、揚げ場を持つ本人が出ているのに**不足エラーあり**。②退勤`22:00`のみ → heatData は**在席(dinner)**、positionErrors は**除外**。③対照として両セル入力`9:00-16:00` は両者とも在席(lunch)・不足なしで一致、④両セル空欄も両者とも非在席で一致（＝①②だけが壊れている）。修正後は①②とも heatData と**帯（lunch/dinner）まで含めて完全一致**し、①の不足エラーが消えることを再実行で確認した。
  **`staffSectionOn` 側の同型ズレも同時に修正**: セル赤ハイライトの帯所属を決める `staffSectionOn` は `bandSectionsOf(name,date,s==null?0:s,e==null?0:e)` と **null を 0 で埋めており**、退勤セルのみのシフトが `stM=0`（＝深夜0時から在席）と解釈されて**ランチ帯所属**と誤判定されうる。`positionErrors` と同じ補完を入れて、不足の計算側とハイライト側の帯を揃えた。両セル空欄のケースは従来どおり `0` フォールバックに落ちる（今回の対象外）。
  **影響**: ポジション不足判定は Premium 限定機能。管理者がシフトを組んでいる最中（出勤だけ入れて退勤は後回し、という普通の作り方）に、**赤いセルと「◯◯ 1名不足」のエラーパネルが出続ける**。実際には足りているので、管理者は存在しない穴を埋めようとして余計な人を入れるか、エラー表示自体を信用しなくなる。後者になると、**本当の不足も見逃す**。

### 今回の対象と見つけ方

**前回（#52）以降のコード変更は #52 の修正コミット自身のみ**（`9ca18d3` と同期コミット `334f885`）。実質的に新規差分ゼロの回が **4回連続**。今回は #52 の総括が明示的に残した再現可能な手法をそのまま実行した — **「後から追加されたフィールド名を grep して、それを消費すべき経路の数と、実際に対応している経路の数を数える」**。

`adminRest`／`extraStart`/`extraEnd`／`adjustedStartFixed`/`adjustedEndFixed`／`adjustedStart`/`adjustedEnd`／`origStatus` の8フィールドをファイル別に数え上げたところ、`adjustedStart` が app-staff.js に1件あるのに `adjustedEnd` は0件、という非対称がまず目に付いた（→ 追跡した結果 `SmModal.applyCellEdit` のスプレッドマージで、既存フィールドは全部保持されており健全。空振り）。

そこで**フィールド名ではなく「同じ問いに答えている関数」を数える**方向に切り替えた。「その日そのスタッフは出勤しているか」を独立に判定している箇所を grep（`getEffHHMM` の呼び出し元）で洗い出すと **:732 `getHeatShift`／:755 `heatData`／:833 店舗間重複／:869 `positionErrors`／:914 `staffSectionOn`** の5箇所あり、**片側セル補完を実装しているのは heatData ただ1つ**だった。決め手は #52 と同じく**コメント**で、`positionErrors` に書かれた「heatData と同じ入口を使い」という自己申告と、実際のコードが**その手前で食い違っていた**。

### 検証したこと

- **「出勤しているか」を判定する関数の全数え上げ**: `getEffHHMM` の呼び出し元5箇所。片側補完ありは `heatData` のみ、`positionErrors`・`staffSectionOn`・店舗間重複は補完なし
- **修正対象の絞り込み**: 店舗間シフト重複（:833）の早期returnは**据え置いた**。他店舗との時間帯重複は両端が確定しないと重なりを判定できず、片側補完は「実際には入力されていない時間」を根拠に他店舗との重複エラーを出すことになる。#47以降「店舗跨ぎは対象外方針」でもあり、ここは補完しないほうが安全側
- **`getHeatShift`（:732）の非対称は意図的**と確認: `if(!st||!en)return null;` は休憩計算用の一時シフト生成で、両端が揃わないと `getBreaksFor` に渡せない。heatData 側は `hsh` が null なら休憩なし・OT加算なしで素通しする作りになっており、片側入力でもカウント自体は成立する
- **スコープ変数の確認**: `HEAT_LUNCH_END_MIN`／`HEAT_DINNER_START_MIN` は :740-745 で定義されており、:869・:919 の両方から参照可能（eslint 0 errors で `no-undef` なしを確認）。`positionErrors` の useMemo 依存配列には `settings` が既に入っているため、候補時間の変更で両定数が変わっても再計算される
- **偽陰性リスクの評価**: 片側入力を attendee に含めることで「本当は不足なのに不足と出なくなる」方向に倒れないか検討した。グリッド全体（ヒートマップ・純勤務時間・休みカウント）が既に片側入力を実シフトとして扱っており、ポジション判定だけが別基準を採る理由がない。heatData 側に揃えるのが一貫した扱い
- **帯（lunch/dinner）の一致まで確認**: 在席するかどうかだけでなく、`s<17:00→lunch`／`e>17:00→dinner` の振り分けが heatData と一致することを再現スクリプトで突き合わせた（出勤のみ→lunch、退勤のみ→dinner）
...
- **🟡（継続・次リリースで必須）index.html のキャッシュ版数を必ずバンプすること**。現在 `20260802-94ffc47`（index.html:211-215）。**#51・#52 から3回連続で引き継いでいる項目**で、develop の本番未反映コミットは**計5件**（うち fix 3件）に増えた。app-utils.js・app-staff.js・app-admin.js の実挙動が変わっているため、版数を上げずに配信するとブラウザキャッシュで旧ファイルが使われ、**ポジション不足の誤報も Excel の締め欠落もスタッフ再提出の消失も直っていない状態のまま**になる。
- **🟢（継続）SubsTab の「出勤」日数が、管理者が両セルを空欄化した日を1日として数える**（app-admin.js:2933・:2980 の2箇所）。`att` の計算は `(st&&en)||(extraStart&&extraEnd)` が偽のとき `shiftBandInfo` を通さず **`:1`（＝1日）にフォールバック**する。管理者がグリッドで出退勤の両セルを空欄にすると `adjustedStart=""`／`adjustedEnd=""` が保存され、スタッフ提出の `start`/`end` が残っているため `origStatus` 復元も走らず `status:"work"` のまま。結果、実効的には勤務のない日（`calcNetWorkMinutes` は0分）が出勤1日として数えられる。フォールバックの `:1` は「時刻が欠けた提出日は1日とみなす」意図と読めるので、**仕様判断が要る**（#52からの継続）。
- **🟢（継続）属性を削除しても休憩のタグ（`breakTimes[].tags`）に残る**（app-admin.js `deleteType`）。`deleteType` は `staffTypeLimits` と `staffAttributes` はカスケード削除するが `breakTimes` のタグは掃除しない（grep で `breakTimes` 参照0件を再確認）。残ったタグはどのスタッフの属性とも一致しないため、その休憩は**誰にも適用されなくなる**。ポジション削除が同名カスケードを実装しているのと非対称。
- **🟢（継続）Excel出力とシフト作成タブで別名提出の解決順が違う**（app-admin.js:2107 付近）。`expXl` は `ss.find(s=>登録名一致||別名一致)` で**配列順の先頭**を採る一方、`_getSubForPeriod` は**完全一致を優先**してから別名を探す。登録名と別名の両方で提出が存在する場合にだけ食い違う。
- **🟢（継続）期間管理タブの Excel 出力だけが管理者調整値を反映しない**（app-admin.js:1911 付近の resolver なし呼び出し）。ファイル名が `_修正前` になる仕様であり意図的な使い分けの可能性が高いため未修正。
- **🟢（継続）ヒートマップの時間列収集が `adminRest` を無視**（app-admin.js:968）。列見出しの範囲決めのみで人数カウントは正しく除外。実害は余分な列が1本出る程度。
- **🟢（継続）店舗間シフト重複エラーが片側入力と他店舗の `adminRest` を見ない**（app-admin.js:833）。今回**意図的に据え置いた**（上記「検証したこと」に理由を記載）。店舗跨ぎは #47 以降 対象外方針。
...
- **🟢（継続・期限接近）`PURGE_OLD_PERIODS_DRY_RUN` の本有効化**（functions/index.js:470）。着手予定 **2026-08-12 以降**＝あと **8日**。
- **🟢（継続）実機E2E未実施**: 「曜日別から選ぶ」UI／#46 削除操作／#47 改名修正／#51 のスタッフ再提出／#52 の Excel 締めセル。**今回の修正も実機E2Eは未実施**（両判定規則の Node 再現で在席・帯・不足の一致を確認したが、実際にグリッドで片側入力して赤ハイライトとエラーパネルが消えることの目視確認は未実施）。
- **🟢群（継続）**: 変更マークの締切ゲート対象外（app-staff.js:166）／`globalTemplates` の命名／`VISION.md` 不在

### 総括

**4回連続で、実質コード変更ゼロの回に構造から 🟡 を1件見つけた**。#50「フラグの解釈がレイヤーをまたいで配られていなかった」→ #51「引き継ぐべきフィールドの一覧がハードコードされ後発分が載らなかった」→ #52「同じ表示を組み立てる経路が3つあり、後発フィールドが1つにだけ配線されていなかった」→ #53「**同じ問いに答える判定が5つあり、片側入力の扱いが1つにしか実装されていなかった**」。同じ根から生えた4本目の枝で、いずれも「**ある扱いが、それを必要とする全箇所に届いていない**」に還元できる。

今回は #52 が残した手法（後発フィールドの消費経路を数える）を**そのまま実行して空振りし、そこから一段抽象化して当てた**。フィールド名で数えると `adjustedStart`/`adjustedEnd` の件数差という**偽の非対称**に引っかかる（実体は健全なスプレッドマージだった）。効いたのは、**フィールドではなく「同じ問いに答えている関数」を数える**という捉え直しで、`getEffHHMM` の呼び出し元を列挙したら5箇所あり、うち1箇所だけが片側補完を持っていた。次回以降はこちらを使う — **「この判定は何箇所で独立に行われているか」を先に数え、その全箇所が同じ入力の端ケース（null・空文字・片側のみ）を同じように扱っているかを突き合わせる**。

#50 以来3回連続で決め手になった**コメント読み**が今回も効いた。ただし今回効いたのは #52 の「バグを直した痕跡のコメント」ではなく、**「他とこう揃えてある」と整合性を自己申告しているコメント**だった。`positionErrors` の「heatData と同じ入口を使い」がまさにそれで、**整合を主張するコメントは、その主張が破れている箇所を探すための検証可能な仮説になる**。コードが自分について語る主張は、常に突き合わせる価値がある。

壊れ方の性質は今回だけ毛色が違った。#51 は「一部が勝手に戻る」、#52 は「Excel でだけ一部が消える」で、どちらも**データが欠ける**方向だった。今回は逆に**存在しない問題が報告される**方向で、しかも**赤いセルとエラーパネルという、最も目に付く形で毎日出続ける**。データ欠損バグは気づかれにくいのが害だったが、誤報バグは**気づかれるが正しいと信じられてしまう**のが害で、放置するとエラー表示そのものが信用を失い、**本物の不足を見逃す土台になる**。警告を出す機能は、出ないことより**間違って出ること**のほうが最終的に高くつく。
