...
## Shifty バグチェックレポート（2026-09-19 自動実行 #135）

> 着手時の HEAD は `bfe888f`。**#134 以降 app-*.js・functions/index.js・database.rules.json は1バイトも変わっていない**。
> #134 の申し送り「画面に出る情報のうち、Excel・PDFに出ないものを全部並べる」を起点に、
> 同じデータを複数の入口が読む箇所を4つ当てた。**いずれも揃っていて、🔴🟡は見つからなかった**。
...
- **#134 から継続（変化なし）**: `dup`・`posErr` が画面限定である理由がPDFのコメントに無い（🟢）／
  部門別PDFで未登録名の列が落ちる理由のコメントが無い（🟢）／休み提出の日に斜線と黄色が両方乗る（🟢）。
- **#133 から継続（変化なし）**: 「締」が有効でない店舗でもメモから「締」だけが無言で消える（🟢）／
  既に登録されている「締」入りの略称（🟢・本番データ未確認）。
- **#132・#131 から継続（変化なし）**: 店舗切替で `plan` だけが同期リセットされない（🟢）／
  `purgeInactiveShops` が削除済みの招待コード機能の後始末を毎日実行している（🟢）／
  企業ログインにサーバー側の試行回数制限が無い（🟡 → BACKLOG化済み）／`companyCodes/${code}` の形の未検証（🟢）／
  `recentPeriodIds` がテスト専用（🟢）。
...
- **変化なし（BACKLOG化済み）**: Cloud Functions の本番反映（🟡・3件たまったまま）／二重課金の根治／
  ...
  PDFの実物確認／`purgeOldPeriods` の本有効化（着手日 2026-09-11 を過ぎている）／
...
1. **「PDFの値解決（`pdfResolve`）だけが休み希望(y)のメモを抑制していない」→ 否定**。
   1388行の分岐は `getStoredNote` と違って `adminRest` を見ないが、関数の先頭で `fieldRest` を先に判定して
   空欄を返しているので、その分岐には到達しない。シフト作成タブの `adjResolver` も期間管理タブの `storedRv`
   （#134 で追加）も、同じく休み希望を先に返している。**3つの解決規則は揃っている**。
2. **「PDFのセル描画が、画面と違う優先順位で色を塗る」→ 否定**。
   変更マーク（緑）は休み希望・休み提出・空白のすべてのセルに乗り、黄色（メモ）より優先される。
   #134 に記録した画面の優先順位と同じ。
3. **「クライアントにUTCで日付へ落とす箇所が残っている」→ 否定**。
   CF側では `planExpiry`（2026-08-25）と `purgeOldPeriodsCutoff`（#111）の2回見つかった形。
   app-*.js の `toISOString()` 11箇所はすべて時刻の記録（`submittedAt`・`createdAt` 等）で、
   日付への変換は `fd()`（ローカル時刻）に統一されている。
4. **「期間の日付検証が、作成と編集で違う」→ 否定**。
   作成（`create`）と編集（`PEF` の `onSave`）は同じ `validatePeriodDates` を通す。
   締切の判定もスタッフ画面・期間管理タブ・`subHasRealUpdate` の3箇所で同じ規則（締切日の終わりまで）を使っている。
...
- RULES.md のスキャン項目はすべてクリア（`DEV_MODE` は式のまま・`subs` 全体 `set()` 0件・
  `.delete()` 0件・`secrets:` 7件・読み込み順 utils→core→staff→admin→main）。
- **配信版数は #133 から引き続き遅れている**: `?v=` 5箇所と `build:` 1箇所は `20260916-c20a092` のままで、
  #134 の app-admin.js 修正（`7e20591`・`c58eb12`）を指していない。リリース時にバンプすれば足りる。
- **今回はコードを読んで判定しただけで、実行による実測はしていない**（仮説がすべてコード上で否定できたため）。
  Firebase・Stripe には一切アクセスしていない。

**申し送り（次回の観点）**: 出力（画面・PDF・Excel）どうしの食い違いは、#134 の2件を直した時点で
コードから読める範囲では出尽くしたように見える。残る穴は **PDFを機械的に測る手段が無い**ことで、
`buildShiftTableHtml` が `ShiftEditTab` のクロージャの中にある限り変わらない。
次回は出力ではなく **入力の側**を見るとよい——同じ値を書く入口が複数ある箇所
（例: 期間の編集で `startDate` を動かしたとき、その日付を鍵に持つ `staffHidden` の範囲や
`keepAttrs` がどう読まれるか）は、まだ誰も当てていない。
