...
## Shifty バグチェックレポート（2026-08-28 自動実行 #100）

> 着手時の HEAD は `27793a5`（リリース直後）。#99 以降の実コード変更は **`1807b5b` の配色巻き戻し1件**だけなので、
> 下記「テーマ・CSS変数の一括置換が入った回のみ」の検査ブロックが発火する回だった。あわせて #99 が残した宿題
> （`CellEditPanel` の書き込みを1.7節で測る）を実行し、そこから1件見つけた。

### 修正済み

なし（下記の🟡は**条件B＝仕様判断**に当たるためループでは直せない）。

### 要確認（未修正）

- **🟡 既存スタッフの名前で提出するだけで、その人の提出の「削除権」を取得できる**（[app-staff.js:206](app-staff.js)）→ **BACKLOG #68 に追記済み**

  `submitterUid` は提出のたびに**無条件で現在の端末の匿名uidへ書き換わる**。`existSub` は名前＋期間で既存subを探して**再利用**するため、
  他人の名前で提出すると新規subは作られず、**その人のsubの `submitterUid` が自分のuidになる**。ルールは sub の削除を
  「`submitterUid` の持ち主かオーナー」に限るので、**削除権ごと移る**。
  ...
  **「✎ 修正」は必須ではない。最小再現は「他人の名前を打ち込んで提出する」だけ**で③④が同じ結果になることを別スクリプトで確認した。
  **スタッフ名は提出状況一覧に「未提出（3名）／佐藤／鈴木／高橋」と表示されている**ので、名前を知らない訪問者でも読める。

  **ルール側も止めない**: `submitterUid` の `.validate` は `newData.val() === auth.uid || owners…` ＝ **自分のuidを書くことを明示的に許可**しており、
  乗っ取りの書き込みは正規の書き込みとして通り、通った結果として `.write` の削除条件（`data.child('submitterUid').val() === auth.uid`）を満たす。
  標準テスト店舗の田中のsubは元々 `submitterUid` が **null**（＝オーナーしか消せない）だったものが、2タップで「訪問者だけが消せる」状態に変わる。
  `8952f8f` の「提出者とオーナーの2者のみ削除可」と `2c1c9c7` のUI修正が、**この経路では両方とも成立しない**。

  **ループで直さなかった理由（条件B）**: 「同一人物の機種変更」と「なりすまし」は**区別できない**。スタッフに本人性を置かないのが設計原則1で、
  `submitterUid` の上書きもコード内に「別端末から出し直した場合は最後に提出した端末が提出者になる」と明記された意図的な仕様。どう締めるかは**ユーザー判断**。

- **🟢 `CellEditPanel`（#99 の宿題）は正常**。他人の提出のセルを編集して「✓ 確定」すると **`update shops/{sid}/subs` が1回だけ**発行され、
  パスは `{subId}/shifts/{日付}` と `{subId}/updatedAt` の**2本のみ**。管理者フィールド（`adjustedEnd`・`adjustedEndFixed`・`extraStart`・`extraEnd`・`origStatus`）は**すべて保持**。
- **🟢 何も変更せずに再提出すると日ごとの `changed:true` が消える**。`buildShift` の `delete nw.changed`（「過去のchangedは作り直す」）による**意図的な挙動**でバグではない。
- 変化なし・**BACKLOG化済み**: 片側セルのヒートマップ／スタッフ削除の後始末／別名提出の重複（#81）／二重課金・特商法（🔴）／解約時のプラン判定／企業連携の解除／解約がアプリに伝わらない。

### 配色巻き戻し（`1807b5b`）の検証 — 異常なし

- **未定義のCSS変数の参照: 0件**／**削除された13変数の残存参照: 0件**／**ダーク定義の乖離: なし**
- **SVG属性内の `var()`: 1件**（`app-staff.js:9` の `ShiftyIcon`。#71 で Chromium・WebKit とも解決を確認済みの既知の1件）
- **index.html の `<style>` ブロックは色変更前のベースライン `8384467` と完全一致**（変数セット・値とも1バイトの差もなし）
- 色トークンを `§` に正規化した差分で「色だけか」を判定。残った3箇所（`app-staff.js:651` の `isEditing` 条件・`app-admin.js:3001` の `mode===id?"white"` 条件・`app-admin.js:1208` の `hBg` の `calc()`）は
  **いずれも `8384467` と一致＝正しい復元**であることを照合済み。
- **タップ領域の是正は意図どおり残っている**（タブバーの `gap:8`・`AD` 定数の `marginLeft:10`）。

> **記録の訂正**: 色の作業は `d22653c` の1コミットではなく **`488d7e7`（Auto-commit）＋ `d22653c` の2コミット**にまたがっていた。
> `git show d22653c~1` を基準にすると巻き戻しが余計なものまで消したように見える。**真のベースラインは `8384467`**（`488d7e7` の親）。
> 自動コミットフックが挟まる環境では「そのコミットの親」＝「作業前の状態」とは限らない。

### 検証したこと

- RULES.md: `DEV_MODE` は式のまま／`subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／読み込み順維持／SRI **11本**／今回の差分に input 追加 **0件**。
- 配信版数 `20260827-c4065a7`（6箇所一致）。**版数が指すコミット以降に app-*.js は変わっていない**。**本番配信も同じ版数**（#99 の6コミット分のずれは `27793a5` で解消）。
- プラン境界: Premium機能を `isPro` で守っている箇所 **0件**。
- CF: `secrets:` **6箇所**／`.delete()` 誤用 **0件**／`node --check` OK。
- 削除系: `filter(s=>s.id!==…)` の7箇所すべてが `deletedId` を渡すか `remove()` を直接呼ぶ。
- `npm test` **197件パス**／`npx eslint app-*.js` **0 errors 95 warnings**（増減なし）／実ブラウザ4本すべてで **pageerror・console.error 0件**。
- **devのデータは1バイトも変えていない**（`set`/`update`/`remove` を横取りし記録のみ）。
- **未検証**: Excel・PDF の実物確認（**8回連続**・ユーザー操作が要る）／iOS Safari 実機／本番データ。

### 総括

**昨日塞いだ穴の、隣に扉があった。** `2c1c9c7`（#99）が直した削除ボタンのゲートは `myName`——**ユーザーが自己申告した名前**でしかなく、
申告を検証する仕組みが無い。「他人の名前で1回提出する」だけでゲートの内側に入れる。**UIのゲートを直しても、その下の権限が自己申告で動くなら迂回される。**

**今回いちばん危なかったのは、コードではなく記録の方だった。** CLAUDE.md が色の作業を1コミットと書いていたため、
`git show d22653c~1` を基準にすると3箇所が「余計に消えた」ように見えた。**正しい巻き戻しを誤りと報告する寸前だった。**

**次回の手**: ①Excel・PDF の実物確認（8回連続・ユーザー操作が要る）②#68 の判断が出たらスタッフ画面側（`✎ 修正`・名前の再利用・`submitterUid` の上書き）も同時に扱う ③まだ実ブラウザで通していないのは**企業連携タブ**と**マイページ（プラン変更導線）**。
