...
## Shifty バグチェックレポート（2026-08-06 自動実行 #58）

### 修正済み

- **[🟡] 管理者がシフト作成タブで下書きした日にスタッフが初回提出すると、その提出が「再提出（更新）」として記録され、提出日時が管理者の入力時刻のまま残っていた**（app-staff.js:158 `submit`／コミット `a2e38cb`）
  **#56 が導入した印（`source:"grid"`）が、それを必要とする最後の1箇所に届いていなかった**。#56 は「シフト作成タブのセルに管理者が直接入力して生まれた sub は**スタッフの提出ではない**」と定め、`source:"grid"` を付けて件数から除外した。除外側は4箇所（app-staff.js:439 バッジ／:514 提出済みN名／app-admin.js:1925 期間管理タブ／:2928 提出一覧）すべてに行き渡っている。**届いていなかったのは「除外する側」ではなく「前回の提出として読む側」**で、`submit()` の `existSub` は `source` を見ずに拾っていた。
  **実測（app-staff.js:145-212 の submit/buildShift と、app-utils.js の実関数 `carryAdminShiftFields`・`subHasRealUpdate`・`subLastActionTime` を逐語コピーした Node 再現）**: 管理者が 8/10・8/11 のセルに下書き（`source:"grid"`・`adjustedStart`/`adjustedEnd`）した状態で、スタッフ「山田」が初回提出すると、修正前は **A 提出日時＝2026-08-01T02:00Z（＝管理者がセルに打ち込んだ時刻）／ B `isUpdated:true`（再提出扱い）／ C 管理者が下書きした全日に `changed:true`（`["2026-08-10","2026-08-11"]`）** になった。修正後は **提出日時＝スタッフが提出した時刻／`isUpdated` なし／`changed` なし** に揃うことを再実行で確認した。
  **壊れ方は3経路に出る**。①提出一覧の「提出日時」（app-admin.js:2957）が管理者自身がグリッドを打った時刻を表示する。②締切後の提出だと `subHasRealUpdate` が `true` を返し、初回提出なのに「変更あり」バッジが出る（再現で締切8/5 のとき修正前 `true`→修正後 `false` を確認）。③**日ごとの `changed` マークは締切ゲートの対象外**（app-staff.js:166 の既知の仕様保留項目）のため、**シフト作成グリッド（app-admin.js:1143）と PDF（:1371）が、管理者が下書きした日をすべて緑で塗る**。
  **修正方針**: `isFirstSubmission=!existSub||existSub.source==="grid"` を導入し、「提出としては初回」と「レコードとしては既存」を分離した。`id` の再利用と `carryAdminShiftFields` による管理者フィールドの引き継ぎ（#51）は**従来どおり維持**し、`submittedAt`・`isUpdated`/`updatedAt`・`changed` の3つだけを初回提出として扱う。PostHog の `shift_submitted.is_update`（:212）も同じ判定に揃えた（同じ「再提出である」という主張を別の場所で二重に持たせないため）。`source` を新 sub に引き継がないのは従来どおり意図的で、`onSub`→`fbSet`（app-main.js:1469）が sub 全体 set のため提出後は印が消え、正しく提出として数えられる。
  **影響**: 誤るのは**管理者が提出前にグリッドで下書きする**という通常運用の動線そのもの。`changed` の緑マークは「スタッフが後から変えた日」を管理者に見せるための信号なので、**下書きした日が全部緑になると、本物の変更が信号として機能しなくなる**（ノイズが信号を埋める型の壊れ方で、#55・#57 の「意思が消える」型とは別種）。提出日時のほうは、締切遵守を後から確認する唯一の記録が管理者自身の作業時刻にすり替わる。

### 今回の対象と見つけ方

**前回（#57）以降のコード変更はゼロ**（差分は `62e99b5` の CLAUDE.md 同期のみ）。実質的に新規差分ゼロの回が **9回連続**。今回は #57 の総括が指定した手法をそのまま採った — 「過去レポートで直した関数のコメントを順に読み、そこに書かれた『ズレの形』ごとに、他にこの形を作れる経路は何本あるかを数える」。

最初に読んだのは `effShiftStart`（app-utils.js:101-109）のコメントで、そこから「status と実効時刻が食い違う shift を作れる経路」を数え直したが、**#57 で全数え上げ済みだったため新しい当たりは出なかった**（`adminRest` の読み手6経路・`origStatus` の3経路をいずれも再確認し、`getStoredTime`/`getHeatVal` 経由で一貫していることを確認）。**当たったのは2本目のコメント**で、#56 が `source:"grid"` の除外箇所（app-admin.js:1921-1925）に残した「この印で除外する」という導入コメントだった。

そこで #57 の総括のもう一方の指示 —「**フィールドを消す・引き継ぐ・除外する修正をしたときは、そのフィールドと同じ役割を持つ他の箇所を必ず数える**」— を `source` に適用した。ここで手法が一段進んだ点がある。**#56 のときに数えるべきだったのは「この印を見て除外する箇所」だけではなく、「この印が付いた sub を読む箇所」全部だった**。実際、除外側4箇所は完備していて、抜けていたのは「読む側」（`subs.find` で前回提出として拾う経路・app-staff.js:158）だった。`grep -n "subs.filter\|subs.find" app-admin.js app-staff.js` で読み手を全数え上げしたところ、`source` を見ていない読み手は3つ（app-staff.js:61 `prevSub`／:158 `existSub`／app-admin.js:1971 の analytics）で、うち意味論的に壊れるのは :158 だけだった。

### 検証したこと

- **`source:"grid"` を読む/数える全経路の数え上げ**: 除外している4箇所（app-staff.js:439・:514、app-admin.js:1925・:2928）＝いずれも正しい。`source` を見ない読み手3箇所（app-staff.js:61 `prevSub`＝Cookie名の下書き復元・害なし／:158 `existSub`＝**今回のバグ・修正済み**／app-admin.js:1971 `excel_exported.submission_count`＝analyticsのみ・下記 要確認）
- **grid sub が「未登録名」経由で他の集計に混じらないことの確認**: app-admin.js:1263 `buildPdfCols` と :2251 `unregisteredNames` はどちらも `!staffList.includes(n)` で未登録名だけを拾う。`applyEditToSubs` が grid sub を作るのは grid 行＝`realStaff`（＝登録済み名）に対してのみのため、この2経路には混じらない
- **修正の非回帰（Node再現）**: ①`source` なしの本物の再提出は従来どおり `isUpdated:true`・`submittedAt` 維持・`changed` 算出（修正前後で完全一致）②`existSub` が無い純粋な新規提出は修正前と同一の結果 ③grid sub 上に提出した後の2回目の提出は、`source` が消えているため正しく「再提出」として扱われる ④管理者の下書き（`adjustedStart`/`adjustedEnd`）は初回提出でも `carryAdminShiftFields` で保持される（`13:00`/`10:00` が残ることを確認）
- **`adminRest`・`origStatus` の一貫性再確認（今回は当たりなし）**: `adminRest` の読み手は `effShiftStart`/`effShiftEnd`（app-utils.js:110/115）・`getStoredTime`（app-admin.js:514）・`getStoredNote`（:515）・`fieldRest`（:513）・`holidayCellDash`（:1137）・PDF（:1364）で、`isWorkDay`/`restCounts`/`consecCounts` はいずれも `getHeatVal`→`getStoredTime` 経由のため抑制が効いている。`applyEditToSubs` の休み希望トグル（:574-580）が `adjustedXxxFixed` を消した後、:607-616 の追加出勤トグルが `extraStart`/`extraEnd` を再評価して落とすことも確認した
...
- **🟡（#57からの継続・仕様判断が要る）スタッフが再提出でその日を「休み」にすると、管理者の調整値が残ったまま status だけ holiday になる**（app-staff.js:161 `buildShift` → app-utils.js:190 `carryAdminShiftFields`）。`carryAdminShiftFields({status:"holiday"},{status:"work",adjustedStart:"10:00",adjustedEnd:"20:00"})` が調整値を残したまま返す。`adjustedStart`/`adjustedEnd` にも「締」と同じ status 復元を広げれば「管理者の調整が勝つ」、逆に落とせば「スタッフの新しい休み申告が勝つ」。**どちらが正しいかは仕様判断**（#51 は「管理者が作り込んだものを黙って消さない」方針だった）
- **🟡（次リリースで必須）index.html のキャッシュ版数を必ずバンプすること**。本番（origin/main）は `20260805-78e8888`、develop は `20260802-94ffc47`（release-to-main が main 側でのみ版数を上げる運用どおりで異常ではない）。**現時点の本番未反映は `origin/main..develop` の8コミット（うち fix 4件＝#55・#56・#57・#58）**。版数を上げ忘れると app-staff.js・app-admin.js の実挙動が変わっているためブラウザキャッシュで旧ファイルが使われ、4件とも直っていない状態のまま配信される
- **🟢（新規）Excel出力の analytics が管理者のグリッド入力を提出として数える**（app-admin.js:1971 `ph("excel_exported",{submission_count:subs.filter(s=>s.periodId===p.id).length})`）。同じ「提出件数」を答える他の4箇所はすべて `source!=="grid"` で除外している。PostHog の集計値のみで画面表示には出ないため実害はないが、**提出数の推移をプロダクト判断に使うと管理者の下書き分だけ水増しされる**
- **🟢（#57からの継続）`ADMIN_SHIFT_FIELDS` の完全性テストが登録漏れを検出できない形になっている**（tests/core.test.js:1124）。期待値をテストファイル内にハードコードして `deepStrictEqual` するだけで、実装から導出されていない。`changed`（app-admin.js:1187 `toggleChanged`）が未登録
- **🟢（#56からの継続）マイページの「スタッフ数」使用量バーが空白列（スペーサー）を人数に数える**（app-admin.js:3957）。上限判定側3箇所は `filter(n=>!isSpacer(n))` で正しく、実施は正しい側のため機能的実害なし
- **🟢（#52からの継続）SubsTab の「出勤」日数が、管理者が両セルを空欄化した日を1日として数える**（app-admin.js:2957・:3004）。フォールバック `:1` の意味を決めないと直せない。あわせてこの判定が生値 `adjustedStart??start` を読み `effShiftStart` を通していない
...
- **🟢（継続）期間管理タブの Excel 出力だけが管理者調整値を反映しない**（app-admin.js:1929）。ファイル名が `_修正前` になる意図的な使い分け
- **🟢（継続）ヒートマップの時間列収集が `adminRest` を無視**（app-admin.js:980）。列見出しの範囲決めのみで人数カウントは正しく除外
- **🟢（継続）店舗間シフト重複エラーが片側入力と他店舗の `adminRest` を見ない**（app-admin.js:825 `dupErrors`／:385 `hasBoth` も生値読み）。店舗跨ぎは #47 以降 対象外方針
...
- **🟢（継続）同名ポジション登録の仕様**（#46・判断待ち）／**ポジション不足サマリーがセクション非区別**／**`isSpecialRedDate` のテスト不在**（BACKLOG登録済み）／**`VISION.md` が不在**（CLAUDE.md が参照しているが実体なし・BACKLOG登録済み）
- **🟢（継続・期限到来）`PURGE_OLD_PERIODS_DRY_RUN` の本有効化**（functions/index.js:470 付近）。着手予定 **2026-08-12 以降**＝あと **6日**
- **🟢（継続）実機E2E未実施**: #46 削除操作／#47 改名修正／#51 のスタッフ再提出／#52 の Excel 締めセル／#53 の片側入力／#54 の休み提出日メモ／#55・#57 の提出状況一覧セル／**#58 の「管理者が下書き→スタッフ初回提出」も実機E2Eは未実施**（submit/buildShift の式を逐語コピーした Node 再現で確認したが、実際にグリッドへ下書きしてからスタッフURLで提出し、グリッドとPDFが緑にならないこと・提出日時が提出時刻になることの目視確認は未実施）。`submit()` は StaffView 内のローカル関数で export されていないため**ユニットテストが無い**。#55・#57 の `applyCellEdit` に続き、**同じファイルの export されていない関数で3回続けてバグが出ている**ので、切り出してテスト対象にする価値がある

### 総括

**9回連続で、実質コード変更ゼロの回に構造から 🟡 を1件見つけた**。#50〜#57 の8本と同じ根 —「**ある扱いが、それを必要とする全箇所に届いていない**」— だが、今回は**届いていなかった先の種類が違う**。#57 までは「同じ処理をする箇所が複数あり、1つだけが漏れていた」（＝**同種の並び**の取りこぼし）だった。今回漏れていたのは、`source:"grid"` を**除外する**側ではなく**前回の提出として読む**側で、**役割の異なる経路**だった。除外側4箇所は #56 の時点で完備していたのに、`subs.find` で拾う経路だけが印を知らなかった。

ここから得られる次回の手法は、#57 の「同じ役割を持つ他のフィールドを数える」を一段一般化したものになる。**印（フラグ）を導入する修正をしたら、「その印を見る箇所」ではなく「その印が付いたデータを読む箇所」を全部数える**こと。前者は自分が今書いている箇所なので自然に数えられるが、後者は**印を知らないまま既に存在している**ので、印を追加した側からは見えない。今回 `grep -n "subs.filter\|subs.find"` で読み手を機械的に全列挙したのが決め手で、**「除外の網羅」ではなく「読み手の網羅」を検索の単位にする**と抜けが出る側を直接叩ける。

**コメントを検索クエリとして読む手法は9回目も有効**だったが、今回は**当たりが2本目のコメントだった**という点が新しい。1本目（`effShiftStart` の「この読み方をしないとこうズレる」）は #57 で汲み尽くされていて空振りし、2本目（#56 の「この印で除外する」という導入コメント）が当たった。**汲み尽くされたコメントは空振りする**ので、次回は「前回のレポートがどのコメントを起点にしたか」を先に確認し、**まだ起点にしていないコメントから読む**のが効率的である。起点候補として未使用なのは、`ADMIN_SHIFT_FIELDS`（app-utils.js:177-183 の「ここに載っていないフィールドは黙って消える」）、`diffSubForFlatWrite`（:565-571 の「sub全体をset/updateすると他端末の変更を巻き戻す」）、`applyEditToSubs` の「締」トグル（app-admin.js:604-606）あたり。

害の質は #55〜#57 の3本（「意思が消える／休みなのに出勤に見える」）とは**別種**で、今回は**ノイズが信号を埋める**型である。`changed` の緑マークは「スタッフが後から変えた日」だけを光らせるための信号で、管理者はそこだけを見て差分を追う。管理者が提出前にグリッドで下書きするのは通常運用の動線なので、**下書きした日が全部緑になると、本物の変更がその中に紛れる**。#56 が「作業が進むほど問題が見えなくなる」、#57 が「最後に直したはずのものだけが直っていない」だったのに対し、今回は「**正しく動いている警告が、鳴りすぎることで無視されるようになる**」という誤り方で、壊れているという自覚が最も生まれにくい。
