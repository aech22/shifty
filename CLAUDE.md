...
## Shifty バグチェックレポート（2026-08-14 自動実行 #74）

### 修正済み

- **🟡 13.2×14px の × ボタンが、確認なしにポジション設定を連鎖削除していた**（app-admin.js:3618）— `3199f7a`
  設定タブ「ポジション設定」の削除ボタンは **13.2×14px＝面積185px²**で、Apple HIG の最小タップ領域 44×44（1936px²）の**約1割**しかない。**アプリ内で最小の操作要素**でありながら、`delPos` は1タップで `settings.positions` から消すだけでなく、**`requiredPositions`（全日付区分×ランチ/ディナー×kitchen/hall/all）と `staffPositions`（全スタッフ）から同名を巻き添えで消す**。しかも確認ダイアログが無く、取り消し手段も無い。
  **実測（標準テスト店舗・375px・`?plan=premium`）**: 1タップで **必要ポジション枠 5→2（-3件）・スタッフのポジション 3→2（-1件）＝計4件が無言で消えた**（`田中` のランチ枠が丸ごと消滅）。テスト店舗はポジション2件・スタッフ3名という最小構成なので、**実店舗では日付区分数とスタッフ数に比例して被害が増える**。
  **なぜ「実装の誤り」と言えるか**: 期間削除（:1964）・提出削除（:3030）・店舗削除（:132）は既に `confirm()` を挟んでいる。**カスケード範囲が最も広いこの削除だけが素通りだった**——寸法の好みではなく、アプリ自身の規約からの逸脱である。規約に揃えたうえで、何件が道連れになるかを提示するようにした（例:「ポジション「調理長」を削除しますか？／必要ポジション設定の枠 3件・スタッフのポジション 1件も一緒に削除されます。」）。
  **検証**: キャンセルで `positions`/`requiredPositions`/`staffPositions` が完全に無傷・OKで従来どおり -3/-1（修正前と同一＝非回帰）・コンソールエラー0件。**Chromium と WebKit 26.5 の両方**でダイアログ発火と実測寸法（13.2×14 / 13.3×14）を確認。テスト店舗は実測後に完全復元した。

### 要確認（未修正）

- **🟡 管理者画面のタップ可能要素が、ほぼ全て 44×44px を下回っている**（アプリ全体）→ **BACKLOG化済み・デザイン判断待ち**
  375px の全DOM実測で、8タブ＋スタッフ画面＋ログイン画面の**タップ要素351件中309件が44px未満**。ただしこれは事故ではなく**一貫した設計**（ボタンは軒並み32〜38px高・`fontSize`12〜13px）なので、直すなら全体の寸法方針の決定になる。決めること3点（タブバー／グリッドセル／行内ボタン）を案つきでBACKLOGに起票した
- **🟢 `index.html` の `?v=` と `build:` が今回の修正を指していない**（`20260814-53f610f` のまま・app-admin.js の最終変更は `3199f7a`）。**申し送りのみ**——`/release-to-main` の手順63が標準工程としてバンプするため BACKLOG 化しない
- **🟢（新規）スタッフ削除に確認ダイアログが無い**（app-admin.js:2312 `del`）。`staffList` から1件 splice するだけで `confirm()` を通らない。ボタンは48×32px と極端に小さくはなく、375px では **x=581＝横スクロールコンテナの画面外**にあるため誤タップの確率は低い。またコード上 `onSave(a)` は `staffList` しか書き換えず提出データ（`staffName` キー）には触れないため、同じ名前を入れ直せば復旧できる。**今回は「1コミット=1論点」を守って手を出さず、次回の判断材料として記録する**
...
- 以下は #46〜#73 からの継続（状態変化なし）: 未claim店舗を誰でも自分の企業に取り込める根／`findActiveSubscription` の `|| live[0]`／`subscription_schedule.*` の4分岐が未購読／`planExpiry` のUTC基準計算／決済失敗フラグの誤解除／片側セルだけの日が0分／`purgeOldPeriods` のUTC日付／`getBreaksFor` の休憩判定／`generateInviteCode` のデッドコード／`urlLocked` が StaffView で未使用／`staffSectionOn` と `isCountExcluded`／SubsTab「月計」の別名重複／`sub.shopId` が死にフィールド／店舗削除が `global/shops/{id}` を消さない／`companyCodes` の TOCTOU／略称の先勝ち／期間作成の日付検証なし／提出状況ビューのセル編集／Excel analytics の数え方／スタッフ数バーのスペーサー／`breakTimes[].tags` の残留／別名解決順の不一致／`heatHours` が `adminRest` を無視／スタッフ削除時のキー残留／`onSub` のローカルstate更新キーの不一致

### 今回の対象と見つけ方

**#73 以降のコミットは3件で、コードの変更は1件も無い**（`c338cf3` と `91d276c` は記録、`0a502e0` はリリース）。したがって今回も**検査すべき新しい差分は存在せず**、中身は #73 が指名した宿題そのものになった。#73 と同じ形の回である。

**(1) 宿題の実行 — 375px でタップ可能要素の実寸を全DOM測定した**。`button` / `select` / `input` / `textarea` / `a[href]` / `[role=button]` を列挙し、`getBoundingClientRect()` の実寸・`fontSize`・祖先チェーンを記録して、**44×44px 未満**と**互いに8px未満まで近接している組**を出す検査を書いた。#73 の教訓に従い、**タブごとに新しいブラウザコンテキストを作ってリロードから始める**構成にして、8タブ全部＋スタッフ画面＋ログイン画面の**10画面すべてを実測できたことを確認してから**結果を読んだ。オーナー復帰（管理コード投入）も各コンテキストで実行し、10画面すべてで `claim=ok` を確認している（閲覧のみ状態では出ないボタンを取りこぼさないため）。

**(2) 結果は「ほぼ全部が44px未満」——つまりこの検査は、そのままでは欠陥を指さない**。351件中309件が該当した。ここで「309件のバグを検出」と書くのは誤りで、実際には**32〜38px高・`fontSize`12〜13px という一貫した設計**が写っているだけである。#73 が「はみ出し」検査で SEO用の非表示ブロック12件を偽陽性として掴んだのと同じ形で、**閾値を当てるだけでは意図した設計と事故を区別できない**。

**(3) そこで判定軸を「寸法」から「寸法×取り返しのつかなさ」へ変えたら、1件だけが残った**。誤タップの結果で分類し直すと、タブバーは別タブが開くだけ、グリッドセルはフォーカスが移るだけ（入力＋blur するまでデータは変わらない）、行内の削除ボタンは**全て `confirm()` を持つ**（期間・提出・店舗の3経路を実際に読んで確認した）。**確認も取り消しも無いまま設定が消えるのは `delPos` の1件だけ**で、しかもそれが**全351件中で最小の要素**だった。18×18px のスタッフ色トグル（:2387）が次に小さいが、こちらは可逆なトグルである。

**(4) 「小さい」だけでは弱いので、1タップで何が失われるかを実データで数えた**。設定を読む→× をクリック→設定を読み直す、を標準テスト店舗で実行し、**必要ポジション枠 -3件・スタッフのポジション -1件**を実測した。同時に `page.on("dialog")` を仕掛けて**ダイアログが1つも発火しないこと**も確認している。この2つが揃って初めて「13×14px の誤タップで設定が黙って消える」と言い切れる。測定後はテスト店舗を修正前の値へ完全復元した。

**(5) 修正は「寸法」ではなく「確認」を選んだ**。寸法を変えるとアプリ全体の設計判断に踏み込むうえ、グローバル指示「UIデザインの基準」がユーザーの合意を求めている。一方、**確認ダイアログの追加はアプリ自身の既存規約に揃える行為**で、見た目を一切変えずにデータ喪失だけを止められる。寸法そのものはBACKLOGへ回した。
...
- 未定義のCSS変数参照 **0件**／ダーク定義の二重管理は**乖離なし**／SVG属性内 `var()` は `app-staff.js:9` の1件で増えていない（色置換コミットが今回の窓に無いため、ロジック混入の差分検査は**発火条件を満たさず未実施**）
- `npm test` **168件パス**／`npx eslint app-*.js` **0 errors 98 warnings**／`node --check functions/index.js` OK（修正前後で同じ）
- **`delPos` の全呼び出し元を横断 grep で確認**（`app-admin.js:3619` の1箇所のみ・シグネチャ不変）
- **実機実測（標準テスト店舗・`?plan=premium`・375×812）**: 管理者8タブ＋スタッフ画面＋ログイン画面の**10画面すべてで `documentElement.scrollWidth == 375`**（#73 の `flexWrap` 修正の非回帰を確認）。修正後の確認ダイアログはキャンセル／OK の両方を通し、コンソールエラー0件
- **WebKit 26.5** でも × の実寸 13.3×14px・確認ダイアログの発火・設定タブ `scrollWidth 371 ≤ 375` を確認
- **未検証**: 実機のiOS Safari（WebKitエンジンでの確認に留まり、実際の指での誤タップ率は測っていない）／375px 以外の狭い幅（320px 等）／ダークテーマでの375px巡回／🟡の実機E2E（スタッフURLでの他人の提出削除）は今回も未実施

### 総括

**「44px未満を数える」検査は、そのままでは1件も欠陥を指さなかった。** 351件中309件が閾値を割ったが、その中身は一貫した設計であり、件数を報告しても何も起きない。**#72 のコントラスト・#73 の横幅では閾値を割った件がそのまま欠陥だったので、同じ気分で書けば「309件の指摘」という無意味な報告になっていた**。今回の収穫は、**閾値検査には「閾値を割ること自体が異常な軸」と「割るのが常態で、割り方に意味がある軸」の2種類がある**と分かったことである。前者は数えれば終わる。後者は数えたあとに**第2の軸**が要る。

**第2の軸は「取り返しがつくか」だった。** 誤タップの結果で分類し直すと、309件のうち308件は「別タブが開く」「フォーカスが移る」「確認ダイアログが出る」のいずれかで止まり、止まらないのは1件だけだった。**そしてその1件が偶然にも全体で最小の要素だった**——13.2×14px は次に小さい18×18px の半分以下の面積で、他のどの操作要素とも桁が違う。**設計から外れた寸法と、規約から外れた無確認が、同じ1つのボタンに重なっていた**わけで、これは「設計」ではなく単に見落とされていた箇所だと判断できる。

**アプリ自身の規約を物差しにできたのが決め手だった。** 「44pxはAppleの基準だから従うべき」という外部の物差しだけなら、それは好みの押し付けになりユーザー判断を待つしかない。しかし**「期間削除・提出削除・店舗削除は confirm を持つのに、最も広く波及する削除だけが持っていない」**という内部の不整合は、誰の好みにも依存しない。**外部基準は BACKLOG へ、内部の不整合はループが直す**——この線引きは #72（既定の黒24件は直す・色の選択はBACKLOG）と同じ形で、2回続けて同じ判断基準が機能したことになる。

**「小さい」を主張するために被害を数えたのは正しかった。** 13×14px という数字だけでは「押しにくい」という体験の話に留まり、優先度をつけようがない。**1タップで4件の設定が無言で消えることを実データで示した**ことで初めて、これがVISION 設計原則5「データを勝手に失わない」に触れる問題だと言える。しかも数えた結果は確認ダイアログの文面にそのまま使えた——**測定が、指摘の根拠と修正の中身を兼ねた**。

**次回の手を1つに絞る。** 3回続けて「指定ではなく描画結果を測る」で成果が出たので、**軸を変えずに対象を変える**。**次は 375px の「ダークテーマ」で #72 のコントラスト検査と #73 のはみ出し検査を再実行する**。理由は3つ: ①#72 のコントラスト実測は**1400px 幅で行われており**、375px では折り返し・縦積みで**要素の重なり方が変わる**ため、ダークで沈む帯が別の場所に出うる ②#73 の横幅検査と今回のタップ検査は**どちらもライトテーマだけ**で回しており、ダーク時に色が変わることで生じるレイアウト差（border の有無による寸法差は #72 の box-shadow 追試で実際に観測されている）を一度も測っていない ③#73 の `flexWrap` 修正で375pxの縦積みが増えたので、**修正後のレイアウトはまだダークで一度も見られていない**。**コードと実機だけで閉じられ、ユーザー判断を要しない。**
