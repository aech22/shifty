...
## Shifty バグチェックレポート（2026-08-26 自動実行 #97）

> 着手時の HEAD は `fb6d255`。#96 の直後の1コミット（`14194e9`）と、そこから広げた「同じ式の写し」「拒否が伝わらない経路」が対象。**#96 の次回の手①（閲覧専用端末の実測）を実施し、BACKLOG #68 の判断材料が揃った。**

### 修正済み

- **🟢→修正 候補時間を平坦化する式の null ガードが、3つの写しのうち1つにしか無い**（[app-admin.js:798](app-admin.js)・[app-admin.js:1052](app-admin.js)）→ `4d1fad1`
  BACKLOG の申し送りは「同じ式が**2箇所**にあり、現時点で出力は一致」としていたが、**実際は3箇所あり、しかも等価ではなかった**。`app-utils.js` の `oneSidedFillBounds` だけが `.filter(c=>c&&…)` を持ち、`ShiftEditTab` 内の2箇所（境界算出 :798・ヒートマップの時間列 :1052）には無い。
  **差は「degrade か throw か」**: 配列に穴があるとスプレッドで `undefined` 要素になり、ガードの無い側は `c.closed` で **TypeError を投げてタブ全体が描画不能**になる。Firebase は null 要素をキーごと削除するため候補配列は `{0:..,2:..}` として保存され、SDK の配列変換規則（全キーが整数 && `maxKey < 2*numKeys`）で**疎な配列として戻りうる**。
  **実測（dev標準テスト店舗の実 settings で修正前後の式を実行）**: 正常データでは候補**24件**・境界 `{lunchEnd:1020,dinnerStart:1020}`・時間列 `[10..23]` が**旧・新で完全一致**（挙動は変わらない）。疎な配列では **旧＝TypeError／新＝例外なしで20件に縮めて継続**。
  **穴を作る経路は特定できていない**（`delG`/`delW`/`delD` は splice で穴を作らない）＝**再現する不具合ではなく防御的ガード**。重大度も 🟢 のまま。

### 実測して確定させたこと（BACKLOG #68 の判断材料）

- **🟡 「閲覧専用」端末でも提出のセル編集は実際に通る** → **BACKLOG化済み・材料を追記**
  dev で新しい匿名uid（＝`owners` 未登録＝「閲覧のみ」バナーが出る端末と同じ状態。`owners/{自分}` の読みは **401**）を作り、標準テスト店舗の実データで測定:

  | 操作 | 結果 |
  |---|---|
  | `subs/{id}/shifts/{date}/adjustedStartNote` の書き込み（＝セル編集と同じ形） | **HTTP 200・書けて読み返せる** |
  | `settings/xlShopName` の書き込み | **401 Permission denied** |
  | `subs/{id}` 全体の削除 | **401 Permission denied**（sub は現存） |

  ルール側の根拠も一致する: `subs/$subId/.write` は `auth != null && $shopId !== 'demo-toriMatsu-v1' && (newData.exists() || owners… || submitterUid === auth.uid)` で、**編集（`newData.exists()` が真）は認証済みなら誰でも通る**。**#68 の前提は推測ではなく実データで確定した**（案A/案Bの選択はユーザー判断のまま）。書き換えた1フィールドは着手前の値 `""` へ復元済み。

### 要確認（未修正）

- **🟢 subs の書き込みで、削除以外の拒否は今も無言**（[app-main.js:1204](app-main.js) `saveSubs` の `if(!deletedId)return;`）。`b2220fe`→`14194e9` で潰してきた形の最後の隣。ただし上の実測どおり **subs の編集は認証済みなら誰でも通る**ため権限拒否は実質起こらない（`.validate` 違反・デモ店舗のみ）＝**実害が出ないので修正しない**。#68 で案A（編集を締める）を選ぶ場合は**この行も同時に直さないと同じ無言の失敗が再発する**。
- **🟢 BACKLOG に、完了済みの作業を指す残骸が2箇所**（[BACKLOG.md:227](BACKLOG.md)・[BACKLOG.md:322](BACKLOG.md)）。参照先の「未デプロイの変更」タスクは存在せず、`c51b62e`・`d6c826a` は 2026-08-25 の CF デプロイに含まれている（`git diff d3dfb3f HEAD -- functions/` が**空**）→ 同時に訂正した。
- **🟢 配信版数 `20260825-92a9e27` の後に app-admin.js（今回）・app-main.js（`14194e9`）が動いた**ため次のリリースでバンプが要る（`/release-to-main` 手順63が持つ）。
- 変化なし・**BACKLOG化済み**: 片側セルのヒートマップ／スタッフ削除の後始末／別名提出の重複（#81）／二重課金・特商法（🔴）／解約時のプラン判定／企業連携の解除／解約がアプリに伝わらない。

### 検証したこと

- **今回はじめて CSS変数の整合検査4点を通した**（`d22653c` が色トークンを大量に足した回で、#96 のレポートに実行記録が無かった）。①未定義変数の参照 **0件** ②`:root[data-theme="dark"]` と `@media prefers-color-scheme` の**定義集合が完全一致**（両方とも非空を確認＝空同士の偽の一致ではない） ③SVG属性内 `var()` は `app-staff.js:9` の1件のみで #71 から増えていない。
- RULES.md: `DEV_MODE` は式のまま／`subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／`".read": true` **0件**／読み込み順維持／SRI **11本**／今回の差分に input 追加 **0件**。
- CF: `secrets:` **6箇所**（SMTPは2箇所ともUSER/PASS揃い）／`.delete()` 誤用 **0件**／`node --check` OK。
- `npm test` **197件パス**／`npx eslint app-*.js` **0 errors 95 warnings**（増減なし）。dev のテストデータは着手前の状態へ復元済み。
- **未検証**: Excel・PDF の実物確認（**5回連続**の申し送り）／iOS Safari 実機／候補・設定・マイページの3タブのタップ測定（2回連続で未実施）／本番データ。

### 総括

**「申し送りに書いてある事実そのものを数え直す」のが効いた。** BACKLOG は「同じ式が2箇所・出力は一致」と書いていたが、数えたら**3箇所あり、一致どころか片方は例外を投げる側**だった。#96 の教訓は「同じ形が隣の経路に残っていないか」だったが、今回は一段手前——**申し送りが数えた件数を信じない**——で当たった。申し送りは前回の観測であって、現在の実測ではない。
**もう1つは、実測が推測を追い越したこと。** #68 は「バナーの文言と実権限が食い違っている（はず）」という読みのまま 🟡 で止まっていたが、匿名uidを1つ作るだけで**編集200・設定401・削除401**の表が採れた。**「ユーザー判断が要る」ことと「材料が採れない」ことは別**で、後者を前者の一部と思い込んで材料の取得ごと止めていた。
**次回の手**: ①Excel・PDF の実物確認（5回連続・ユーザー操作が要る）②候補・設定・マイページの3タブのタップ測定 ③今回の「新しい匿名uid＋REST で権限表を採る」手順を `shifty-e2e-verify` に残すか検討する。
