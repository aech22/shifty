...
- **🔴（継続・プロセス）このループから実機E2Eが11回続けて実行できない**（環境要因・コード変更なし）
  ...
  **⚠️ 2026-08-22 解消・および上記の診断が誤りだったことの訂正**: 同じセッションでユーザーが会話を始めたあと `preview_start` を再試行したところ**そのまま起動した**。`1c5272b`（期間確定）と `c889660`（別名の正規化）の実機E2Eを完走し、全項目パス・コンソールエラー0件（詳細はBACKLOG該当タスク）。**この拒否はセッション単位で固定されるものではなく「無人の間だけ」効く**ので、#78〜#88 の11回ぶんの「このセッションはスケジュール起点だから不可」という申し送りは**誤り**だった——再試行していなかっただけである。以後、ユーザーからのメッセージが来たターンでE2Eの宿題が溜まっているなら、必ず `preview_start` を1回試すこと。
