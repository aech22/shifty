...
## Shifty バグチェックレポート（2026-08-16 自動実行 #78）

### 修正済み

- **🟡 期間削除の確認が「削除しますか？」だけで、提出データの連鎖削除を伝えていなかった**（[app-admin.js:1966](app-admin.js)）→ `ad1fadb`
  `savePeriods`（[app-main.js:1132-1145](app-main.js)）は期間の削除時に**その期間の subs と `tokens/{urlToken}` まで連鎖削除する**。押した瞬間に「提出済みのシフトが全部消える」「スタッフに配ったURLが無効になる」が同時に起きるのに、確認は `confirm("削除しますか？")` で**対象名も結果も示していなかった**。
  **Free はこの導線に構造的に誘導される**——期間1件が上限なので、翌月を集めるには唯一の期間を消すしかない。**毎月必ず通る道が、警告のない連鎖削除だった。**
  修正後: `「2026年7月前半」を削除しますか？ / 提出済みのシフト2件も一緒に削除されます。 / スタッフ用URLも無効になります。この操作は取り消せません。`（0件なら中央行を省く）
  **実装の取りこぼしと断定できた根拠**: ①**必要な件数はボタンの2行上で既に計算・表示されている**（:1959 の「提出：N件」）②同ファイルの他の破壊的確認は全て対象名か連鎖結果を示す（`:132`・`:2596`・`:3188`・`:3618`）——**ここだけが例外**③`savePeriods` 側には連鎖削除を説明するコメントが**書かれている**（作者は認識済みで、ダイアログだけが黙っていた）。
  件数は表示側と**同じ式**（`source!=="grid"` 除外）にした。食い違わせると**バグチェック#56 と同じ混乱**を新造してしまう。VISION 設計原則5「データを勝手に失わない」に該当し、#74 の `3199f7a` と同型の修正。

### 要確認（未修正）

- **🔴（新規・プロセス）このループから実機E2Eが実行できなくなった**（環境要因・コード変更なし）
  `preview_start` が **「Dev servers can't be started from unattended sessions」** で拒否され、稼働中サーバーも0。Bash でのサーバー起動は規約で禁止のため代替なし。**#72〜#77 の6回が収穫の柱にしてきたブラウザ実測が1件も実行できなかった。** 次回以降のスケジュール実行も同じ制約を受ける。
  **ユーザーが対話セッションで `/bug-check` を手動実行すれば解消する**（サーバー起動を承認できるため）。
- **🟡 #77 の宿題（Free 上限の実機検証）は静的監査まで完了・実測のみ未達**（上記🔴が理由）
  **コード側は「正しい」と確認済み**: スタッフ追加は `add()`（:2309）で上限ブロック、期間は**手動 :1844・プリセット :1911 の両経路とも** `checkPeriodLimit()` を通る。**追加経路の取りこぼしも無い**（`:181` はリネームで件数不変／未登録名ボタンは別名登録で件数を増やさない／空白列は Pro 限定かつ `isSpacer` 除外）。UpgradeModal も両 type を保持。**残るは実機で21人目・2件目を押す1回だけ。**
- **🟢（新規）提出削除の確認も対象名を示さない**（app-admin.js:3032）。ただし詳細モーダル内＝対象が見えている単一削除で連鎖も無いため申し送りのみ
- **🟢 `index.html` の `?v=` と `build:` が今回の変更を指していない**（`20260814-53f610f` のまま）。**申し送りのみ**——`/release-to-main` の手順63がバンプする
- **🟡 プラン制限の告知アンバーがライトで2.15:1**（#77）／**淡い帯にダークで沈む文字**（#72）／**アクセント塗り＋白文字 2.85:1**（#72）／**土日祝の日付色**（#75）／**固定明色チップ**（#76）／**タップ領域44px未満**（#74）→ BACKLOG化済み・デザイン判断待ち（**今回は実測不能のため状態変化なし**）
- **🟡 スタッフURLを開いた誰でも他人の提出を削除できる**（#70）／**解約イベントだけが metadata でプラン判定**（functions/index.js:319）→ BACKLOG化済み
- 以下は #46〜#77 からの継続（状態変化なし）: 未claim店舗の取り込み／`findActiveSubscription` の `|| live[0]`／`planExpiry` のUTC計算／片側セルだけの日が0分／`purgeOldPeriods` のUTC日付／`generateInviteCode` のデッドコード／スタッフ削除に確認ダイアログが無い／ほか

### 今回の対象と見つけ方

**#77 以降のコミットは0件。6回続けて「差分ゼロ」の回**である。

**(1) 主力の手法が使えなくなった回だった。** ブラウザ実測が1つも実行できず、「測れないので異常なし」と書きかけた。回避できたのは、**#77 の宿題を実測の前段である静的監査として先にやり切った**から。

**(2) 宿題を「測る」から「読む」に翻訳したら、隣でバグが落ちてきた。** Free の上限を読むうちに `PLAN_LIMITS.free.periods = 1` の**運用上の意味**に行き当たった——**Free は翌月を作るために唯一の期間を消すしかない**。そこで「その削除は何を消すのか」を追い、`savePeriods` の連鎖削除に到達した。**上限そのものではなく、上限が強制する行動の先にバグがあった。**

**(3) 判定は今回も「答えがコードベースにあるか」。** 6回連続で同じ基準が機能。決め手は**件数がボタンの2行上で既に表示されていた**ことと、**他4箇所の破壊的確認が全て対象名か連鎖を示していた**こと。色も寸法も新変数も要らない。

**(4) 新しい検証手段を確立した——「配信されるコードそのものを Node で評価する」。** 該当行をファイルから正規表現で抽出し、`confirm` の引数を Node で eval して実文面を出力した。**手で書き写した式ではなく配信物そのもの**を評価する点が重要で、ブラウザが無い回の部分的な代替になる。

**(5) 「正しいことの確認」も成果として数えた。** 上限は全書き込み経路を洗い出して取りこぼしゼロを確認。**バグが出なかったこと自体が宿題の答えの半分**で、残り半分（実機ブロック）だけが未達だと切り分けられた。
...
- **プラン上限の全書き込み経路の洗い出し（静的）**: 上記のとおり取りこぼしゼロ
- **修正の検証**: 配信される `app-admin.js:1966` から件数式と `confirm` 引数を抽出し Node で評価。提出3件（うち grid 1件）→「**2件**」＝**カード表示と一致**。0件では中央行が出ないことを確認
- **未検証（今回は手段が無い）**: ブラウザでの実描画すべて／**Free 上限に達した店舗での実ブロック**／今回追加した confirm の実表示／実機 iOS Safari

### 総括

**測れない回に、測らずに出せる成果を出せるかが試された。** 🟡 が1件出たのは、宿題を「実測」ではなく「宿題が指す対象を読む」に翻訳したからである。

**今回の発見は6回で最も「Freeユーザーの実害」に近い。** コントラストは読みにくさ、タップ領域は押しにくさだが、これは**毎月必ず通る道で提出済みシフトが警告なく消える**、しかも Free は**回避できない**。**Free の制約が利用者に何を強いるかを追うと、まだ出てくる。**

**ループの主力手段が失われたことは 🔴 として扱う。** #72〜#77 の主要な収穫はどれも実測由来だった。**ユーザーが対話セッションで `/bug-check` を1回回せば解消する**ので、その一手を最優先で提案する。

**次回の手を1つに絞る。** 実測が復活したなら**#77 の宿題をそのまま実行する**（コードは確認済みなので実機で1回押すだけ）。実測が使えないままなら**今回と同じ翻訳を続ける**: 次の候補は**スタッフ削除**（`del`・app-admin.js:2312）——確認ダイアログが無く、スタッフ名をキーに持つ設定マップが残留する既知の🟢があり、**「消える側」ではなく「残る側」の害**という点で今回と対になる。

**CF側の宿題（#68 の2つ目）は今回も持ち越し。** 実購入テストのダウングレード発火直後に subscription の `metadata.plan` を1回読むだけで閉じられる。
