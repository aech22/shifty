...
## Shifty バグチェックレポート（2026-08-03 自動実行 #52）

### 修正済み

- **[🟡] Excel出力だけが「締」の追加出勤とメモのみセルを空欄に落としていた**（app-admin.js:1511 `adjResolver`／:2135 `expXl`／コミット `9ca18d3`）
  シフト作成タブの Excel 出力に渡す `adjResolver` が `{time, note}` しか返さず、「締」（`kind:"fixed"`・23:00〜25:00の追加出勤）の永続化フラグ `adjustedStartFixed`/`adjustedEndFixed` を読んでいなかった。締めフラグは h/k/x・店舗略称などの「素の」note を汚さないために **note とは別枠で保存する設計**（app-admin.js:516-518 に明記）だが、その別枠を Excel 経路だけが拾っていなかった。あわせて `expXl` のセル表示が**時刻の有無だけ**で表示可否を判定しており（`startT?…:null`）、時刻を持たないセルは note・締めがあっても空欄に落ちていた。
  **非対称の構図**: 同じ「時刻＋note＋締め」を組み立てる経路は3つあり、他の2つは正しい。グリッド `getVal`（:519）は `t?t+note+fx:(note+fx)||""`、PDF `pdfResolve`（:1230）は `(dec||fx||note)?dec+note+fx:""`。PDF には「**従来はdecのみでfx脱落=空欄化していた**」というコメントが残っており、**同じバグが PDF では既に修正され、Excel だけが取り残されていた**。
  **実測検証**（実データ形状のNode再現・3経路に同一 shift を通して比較）: ①出勤`13`/退勤`17締` → グリッド`17締`・PDF`17締` に対し **Excel は `17`**（締めが脱落）。②休み提出の日に単独`締`（集計上は純勤務**120分**・出勤**0.5日**として成立している日） → グリッド`締`・PDF`締` に対し **Excel は完全な空欄**。しかも `status` が `"work"` に固定されるため休みの斜線も引かれず、**「休み」とも「出勤」とも読めない白セル**になる。③対照として出勤`16k` は3経路とも `16k` で一致（修正前後とも不変＝非回帰）。修正後は①②ともグリッド／PDFと完全一致することを再実行で確認。
  **影響**: Excel は印刷して掲示する最終成果物であり、②のケースでは **23:00〜25:00 の閉店シフトが入っている本人に、その日が空欄のシフト表が配られる**。アプリ内の集計・PDF とは食い違うため、Excel だけを見ている限り誤りに気づけない。
  **修正**: `adjResolver` に `fixed` を追加し（保存済みは `getStoredFixed`、未保存の編集中は `extractNote().hasFixed`）、`expXl` 側で `sFx`/`eFx` として表示に合成。表示可否の判定を `(startT||sNote||sFx)` に変えてグリッド `getVal`・PDF `pdfResolve` と同じ真偽判定に揃えた。**黄色塗り（`startFill`/`endFill`）は従来どおり note のみで判定し締めは対象外**にした（PDF のセル背景 `cbg` が `r.note` だけで決めているのと揃えるため。ここを `sFx` 込みにすると PDF と Excel で塗りが食い違う）。resolver なしの経路（期間管理タブの「_修正前」出力）は `rv=null` で `sFx=eFx=""` となり従来と同一。

### 今回の対象と見つけ方

**前回（#51）以降のコード変更は #51 の修正コミット自身のみ**（`60bfafd` と同期コミット `b11b5d5`）。実質的に新規差分ゼロの回が **3回連続**。今回は #51 の教訓「**あとから追加されるものの集合を、増えることを前提にしない書き方で持っている**」をそのまま検索条件に転用した。#51 は「引き継ぎフィールドのハードコード配列」だったので、今回は同じ形の**「同じデータを組み立てる経路が複数あり、片方だけ後から拡張された」箇所**を探した。`adminRest`（#50追加）と `extraStart`/`adjustedStartFixed`（2026-07-12追加）という**後発フィールドが、どのレンダラーまで配られているか**を grep で突き合わせたところ、`adminRest` はグリッド・PDF・Excel の3経路すべてに配線済み（:1352・:1512 に明示的な対応コードがある）なのに対し、**締めフラグだけが Excel に配線されていない**ことが分かった。決め手は PDF 側の「従来はdecのみでfx脱落=空欄化していた」というコメントで、**同じバグを一度踏んで直した痕跡が片方の経路にだけ残っている** = もう片方は未修正、と読めた。

### 検証したこと

- **「時刻＋note＋締め」を組み立てる経路の全数え上げ**: グリッド `getVal`（:519）／PDF `pdfResolve`（:1230）／Excel `adjResolver`+`expXl`（:1511・:2135）の3つ。うち **Excel の1つだけが締めフラグ未対応**だった
- **締めフラグが note と別枠である理由の確認**: :516-518 のコメントどおり、note を「素の」値（h/k/x・他店舗略称）に保つことで h/k 判定と `abbrToShop` の完全一致 lookup を壊さないための設計。**したがって「note に締めを混ぜる」形の修正は不可**で、別枠で受け渡す必要があることを確認した
- **黄色塗りの非回帰**: PDF の `cbg`（:1359）が `r.note` のみで黄色を決めていることを確認し、Excel も `sNote` のみのままにした。締め単独セルが Excel でだけ黄色くなる差異を作らない
- **resolver なし経路（期間管理タブ「_修正前」）への影響**: `rv=null` のとき `sFx=eFx=""` となるため締めの合成は発生しない。表示判定の `(startT||sNote)` 化のみが理論上の差分だが、スタッフ提出値は時刻とセットでしか `startNote` を持たないため実質不変（かつ発生しても他経路と揃う方向）
- **`adminRest` との相互作用**: `fieldRest` が真のフィールドは早期 return で `fixed:false` を返すため、休み希望セルに締めが混ざって表示されることはない。片フィールドのみ休み希望＋他方に締め、というケースは他方のセルで正しく `締` が出る（PDF と同挙動）
- **3経路の一致を修正後に再実測**: 上記①②③すべてでグリッド／PDF／Excel の出力文字列が一致することを確認
...
- **🟡（継続・次リリースで必須）index.html のキャッシュ版数を必ずバンプすること**。現在 `20260802-94ffc47`（index.html:211-215）。**#51 から引き継いだ項目で、今回さらに app-admin.js の変更が積まれた**（develop の未リリースコミットは計3件）。app-utils.js・app-staff.js・app-admin.js の実挙動が変わっているため、版数を上げずに配信するとブラウザキャッシュで旧ファイルが使われ、Excel の締め欠落もスタッフ再提出の消失も**直っていない状態のまま**になる。
- **🟢（新規）SubsTab の「出勤」日数が、管理者が両セルを空欄化した日を1日として数える**（app-admin.js:2926）。`att` の計算は `(st&&en)||(extraStart&&extraEnd)` が偽のとき `shiftBandInfo` を通さず **`:1`（＝1日）にフォールバック**する。管理者がグリッドで出退勤の両セルを空欄にすると `adjustedStart=""`／`adjustedEnd=""` が保存され（:593）、スタッフ提出の `start`/`end` が残っているため `origStatus` 復元も走らず `status:"work"` のまま。結果、実効的には勤務のない日（`calcNetWorkMinutes` は0分）が出勤1日として数えられる。同じ経路で作られる `source:"grid"` の休み希望のみ sub は SubsTab が `s.source!=="grid"` で除外する（:2897）ため無関係。フォールバックの `:1` は「時刻が欠けた提出日は1日とみなす」意図と読めるので、仕様判断が要る。
- **🟢（新規）属性を削除しても休憩のタグ（`breakTimes[].tags`）に残る**（app-admin.js:3392 `deleteType`）。※#51からの継続。`deleteType` は `staffTypeLimits` と `staffAttributes` はカスケード削除するが `breakTimes` のタグは掃除しない。残ったタグはどのスタッフの属性とも一致しないため、その休憩は**誰にも適用されなくなる**。ポジション削除（:3497）が同名カスケードを実装しているのと非対称。
- **🟢（継続）Excel出力とシフト作成タブで別名提出の解決順が違う**（app-admin.js:2107）。`expXl` は `ss.find(s=>登録名一致||別名一致)` で**配列順の先頭**を採る一方、`_getSubForPeriod`（:489）は**完全一致を優先**してから別名を探す。登録名と別名の両方で提出が存在する場合にだけ食い違う。
- **🟢（継続）期間管理タブの Excel 出力だけが管理者調整値を反映しない**（app-admin.js:1911 の resolver なし呼び出し）。ファイル名が `_修正前` になる仕様であり「提出そのまま」と「完成シフト」の意図的な使い分けの可能性が高いため未修正（今回の修正でもこの経路は意図的に据え置いた）。
- **🟢（継続）ヒートマップの時間列収集が `adminRest` を無視**（app-admin.js:968）。列見出しの範囲決めのみで人数カウントは正しく除外。実害は余分な列が1本出る程度。
- **🟢（継続）店舗間シフト重複エラーが他店舗の `adminRest` を見ない**（app-admin.js:847）。店舗跨ぎは #47 以降 対象外方針のため踏襲。
...
- **🟢（継続）スタッフ削除時に設定マップのキーが残る**（app-admin.js:2247）。改名側（:176 `renameKey`）は7マップ全て移し替える実装になっているのと非対称。
- **🟢（継続）同名ポジション登録の仕様**（#46・判断待ち）／**ポジション不足サマリーがセクション非区別**／**`isSpecialRedDate` のテスト不在**（BACKLOG登録済み）
- **🟢（継続・期限接近）`PURGE_OLD_PERIODS_DRY_RUN` の本有効化**（functions/index.js:470）。着手予定 **2026-08-12 以降**＝あと **9日**。
- **🟢（継続）実機E2E未実施**: 「曜日別から選ぶ」UI／#46 削除操作／#47 改名修正／#51 のスタッフ再提出。**今回の修正も実機E2Eは未実施**（3経路のNode再現で出力文字列の一致を確認したが、実際に Excel ファイルを生成して締めセルを目視する確認は未実施）。
- **🟢群（継続）**: 変更マークの締切ゲート対象外（app-staff.js:166）／`globalTemplates` の命名／`VISION.md` 不在

### 総括

**3回連続で、実質コード変更ゼロの回に構造から 🟡 を1件見つけた**。#50「フラグの解釈がレイヤーをまたいで配られていなかった」→ #51「引き継ぐべきフィールドの一覧がハードコードされ後発分が載らなかった」→ #52「同じ表示を組み立てる経路が3つあり、後発フィールドが1つにだけ配線されていなかった」。**同じ根から生えた3本の枝**で、いずれも「**あとから追加されたものが、それを必要とする全箇所に届いていない**」という一点に還元できる。

今回の発見手法は明確に再現可能で、次回以降も使える。**「後から追加されたフィールド名を grep して、それを消費すべき経路の数と、実際に対応している経路の数を数える」**。`adminRest` は3経路すべてに対応コードがあった（だから健全）。`adjustedStartFixed` は2経路にしかなかった（だからバグ）。**対応箇所の数を数えるだけで、コードを読み込まずに非対称を検出できる**。

もう一つの収穫は、**「一度踏んで直したバグのコメント」が、同型バグの在処を指す標識になる**と分かったこと。PDF の `pdfResolve` に残っていた「従来はdecのみでfx脱落=空欄化していた」という一文が決め手だった。修正時に「なぜこう書いたか」を書き残す習慣が、**別経路の同じバグを見つける検索キーとして機能した**。今後は「バグを直したときのコメント」を grep の起点にする価値がある（#50 で有効だった「検証可能な主張を書いたコメントを辿る」手法の変種）。

壊れ方の性質も #51 と共通していた。#51 は「シフト表の一部だけが勝手に戻る」、今回は「**Excel でだけ一部のセルが消える**」。どちらも**全体が壊れるのではなく一部だけが非対称に欠ける**ため、原因を推測しにくい。特に今回は、画面（グリッド）と PDF では正しく見えているので、**管理者が自分の画面で確認する限り絶対に気づけない**。Excel を印刷して配ったあと、現場で「その日シフト入ってないですよね？」と言われて初めて発覚する種類のバグだった。**出力経路が複数ある機能は、経路ごとに実際の出力を突き合わせないと検証したことにならない**。
