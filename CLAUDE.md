...
## Shifty バグチェックレポート（2026-09-23 自動実行 #141）

> 着手時の HEAD は `ae64245`。#140 以降に入った唯一の新しいコードは、**並行セッションが実装した
> 全表示（`fullView`）レイアウト**（app-admin.js・+101/-50 行）で、#140 が「触らず報告に留めた」
> あの作業がそのまま自動コミットで入っていた。そこで今回は、BACKLOG の受け入れ条件で唯一
> 「未検証」のまま残っていた**色の再現**を実測した。答えは**半分だけ再現される**（🟡）。
>
> **⚠️ 並行セッションは今も動いている**。作業中（03:00 時点）に app-admin.js が再び書き換わり
> （+24/-13・`fvTableW` による中央寄せと行高の小数化）、コミット前の状態にある。
> **本回は app-admin.js を1バイトも触っていない**。下の実測は `ae64245` に対するもので、
> その後の未コミット変更は反映していない（ただし2件とも原因の行はその差分でも変わっていない）。

### 修正済み

なし（下記2件はどちらも app-admin.js の中にあり、並行セッションが編集中のため触っていない）。

### 要確認（未修正）

- **[🟡] 全表示では、セルの `input` が `td` を隙間なく覆うため、土日祝の行色とポジション不足の黄色が消える**（app-admin.js の `AI2` の `fullView` 分岐）

  グリッドのセルは**2系統の色を場所で分けて**見せている。セルコマンドの色（`cellBgStyle`）は
  **input の背景**、土日祝の行色（`baseRb`）とポジション不足の黄色（`rbS`/`rbE`）は **td の背景**。
  通常表示は td の `padding:"1px 1px"` と input の `width:colW-3` で td の色を帯として覗かせている。
  全表示は td が `padding:0`・input が `width:"100%"`／`height:fvRowH` なので**帯が消える**。

  **実測**（`ShiftEditTab` だけを実ブラウザ 1400×900 にマウントし、index.html の `:root` と同じ
  テーマ変数を注入。**Firebase へは1バイトも出していない**）:

  | | 通常表示 | 全表示 |
  |---|---|---|
  | td の色が見える帯（横／縦） | **7px / 4.5px** | **0px / 0.5px** |
  | スタッフ名の赤・日付文字の土曜色 | 出る | **出る** |
  | セルの font（16日×8名 / 31日×25名） | 16 / 16px | 16 / **10px** |

  受け入れ条件の4系統のうち**2つは再現され、2つ（土日祝・ポジション不足）は消える**。
  土日祝は両端の日付の文字色で判別できるが、**ポジション不足の黄色は「誰のセルが足りないか」を
  示す唯一のセル単位の信号**で、代わりはグリッド下の文字一覧だけになる。
  回帰スクリプト `.claude/skills/shifty-e2e-verify/scripts/example-fullview-cell-colors.js`（**EXIT=1**・
  通常表示側が対照として通る）。**直さなかった理由（条件B＋D）**: セルに余白が無い以上、
  2系統の色を**どう同居させるか**の判断が要る。加えて同じ関数を並行セッションが編集中。

- **[🟡] 高さだけウィンドウをリサイズすると、全表示が古い高さのまま作り直されない**（`fvAvailH` と `gridTop` の useEffect）

  `fvAvailH` は `window.innerHeight` と `gridTop` から毎レンダー算出されるのに、再レンダーの
  引き金は `containerW`（＝`window.innerWidth-16`）と `gridTop` の変化だけで、**どちらも高さでは動かない**。

  | 操作 | 可視高 | グリッド下端とビューポート下端の差 |
  |---|---|---|
  | 1400×900（初期） | 824px | −6px |
  | **1400×600（高さだけ）** | **824px のまま** | **+294px（画面外へはみ出す）** |
  | 1399×600（幅も1px・対照） | 530px | −6px（直る） |

  `maxHeight` が古いままなのでコンテナは自分でスクロールせず、**ページごとスクロールしないと
  最終日に到達できない**。**幅を1px動かせば直る**ことが「高さでは再レンダーが起きていない」の証拠。
  **直さなかった理由（条件D）**: 修正は小さいが、その useEffect は並行セッションが今まさに
  書き換えている関数の中にある。

- **[🟢] E2Eハーネス（`mount-component.js`）はテーマのCSS変数を注入しないので `var(--c-*)` が全部無効になる**
  `td` の下枠が **0px**（本番相当のCSSを入れると 1px）、スクロールコンテナの枠も 0px。
  色の計測は**そもそも成り立たない**（input の背景が透明になり、覆っていても下の色が見える＝偽陰性）。
  **幾何の結論への影響は確かめた**——`example-fitall-geometry.js` に同じCSSを入れて回し直しても
  6水準すべて余り0のままで、変わったのは31日のセルfontが 10px→**9px** になった点だけ。

- **#140 から継続（変化なし）**: 詳細モーダルの「勤務計」等が `det.shifts[d]` を直に読む（🟢）／
  `_windowVio` の窓の起点（🟢）——どちらも同じ1つの仕様判断（→ #81 に追記済み）。
  確定済み期間のタブ間の食い違い（🟡・BACKLOG化済み）／変更マークとバッジ（🟡・BACKLOG化済み）／
  属性削除で休憩のタグが残る（🟢）／企業ログインの試行制限（🟡・BACKLOG化済み）／
  退勤が出勤より前（🟡・BACKLOG化済み）／その他の🟢群。
...
- **全表示の幾何が本番相当のCSSでは破れる → 否定**（テーマ変数を入れて6水準を2回、余りは全水準0）。
- **`gridContentW` が全表示で日付列2本を数え落とす → 否定**（90 は 45px×2 と一致し、使う `fitsCentered` は
  `!fitAll` 前提なので全表示では評価されない）。
- **全表示の変更が通常表示・本番の「全員表示」に漏れている → 否定**（分岐はすべて `fullView` 三項か
  `BOXS`＝非全表示では `undefined`）。
- **`isPro` で Premium 機能 → 否定**。**CF の `secrets` 抜け → 否定**（7つの `runWith` すべてに宣言・`.delete()` 0件）。

### 検証したこと

- `npm test` **270件パス**・`npx eslint app-*.js` **0 errors / 95 warnings**（#140 から増減なし）。
...
- **配信版数は `20260920-40a88de` のままで、9/21 と 9/23 の app-admin.js 変更を指していない**。
  リリース時に `/release-to-main` の手順63でバンプすること。
- **Firebase・Stripe には一切アクセスしていない**。

> **⚠️ 並行セッションが稼働中**: app-admin.js は 02:48（`ae64245`）と 02:59（未コミット）の2回書き換わった。
> **本回のコミットは新規スクリプト1本だけ**。**main へのマージ・本番デプロイは、
> 並行セッションの作業が終わってからまとめて1回にすること。**

**申し送り（次回の観点）**: 今回当たったのは「**同じ意味に surface が2つある所**」で、全表示はその片方
（td の背景）を潰していた。#139・#140 の「同じ数字を出す式が2本ある」と形が似ている——
**surface が2つある所は、レイアウトが変わると片方が黙って消える**。次は Excel・PDF に同じ問いを
当てるとよい（画面で td と input に分かれている色が `expXl`／`buildShiftTableHtml` では何に対応するか）。
あわせて**E2Eハーネスにテーマ変数を既定で注入するかを決める**と、色・枠線の計測が今後そのまま信用できる。
...
- [x] 全表示でも土日祝の色（`dc`/`baseRb`）・ポジション不足の黄色（`rbS`/`rbE`）・セルコマンドの背景色（`cellBgStyle`）・スタッフ名色（`nameColor`）が通常表示と同一規則で再現される。`fill` 方式（input を透明にして td の色を全面に透かす）で実測（`example-fullview-cell-colors.js` → EXIT=0・`fullShowsTdColor:true`・6水準ともコンソールエラー0件）
