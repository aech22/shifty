...
## Shifty バグチェックレポート（2026-08-25 自動実行 #95）

> 対象は **2026-08-25 の一括リリース（`d3dfb3f`・BACKLOG10件超を1日で実装して本番へ出した回）**。着手時の HEAD は `5167ea2`、`git diff b8242b6..HEAD` で app-*.js が 605行、functions/index.js が 69行、database.rules が 24行動いていた。並行セッションの気配はなし。

### 修正済み

- **🟡 期間プリセットからの作成だけが、日付の重なり検証を通っていない**（[app-admin.js:2002](app-admin.js)）→ `93c1022`
  同日の `a082d5d` は `validatePeriodDates` を新設し、**「日付の検証は作成・編集で同じ関数を通す（片方にしか無いと、編集だけ素通りする）」**とコミットメッセージに書いている。ところが**作成の入口は3つ**あり、通したのは手入力の `create()` と編集の `PEF` の2つだけで、**プリセットのボタン（:2001）が漏れていた**。しかもプリセットは `usePreset` の初期値が `true` ＝ **既定の作成手段**で、UIも「選択するとすぐに作成されます」と即時作成をうたっている。そこにあるのは開始日と終了日の**完全一致**チェックだけなので、部分的に重なる期間は無警告で作れる。
  **実際に踏む経路**: ①手入力で 2026-09-05〜09-20 を作ったあとプリセットの「2026年9月前半」(09-01〜09-15) ②期間の単位を 2week→1month に切り替えたあと、前半/後半が既にある月の「2026年9月」。
  **実ブラウザでの実測（`PeriodsTab` を単体マウント・`onSave` はスパイでFirebaseへ書かない）**: **修正前の版**（`SHIFTY_ROOT` を `HEAD` の app-admin.js へ向けて同手順）は **confirm 0件のまま 09-01〜09-15 を作成**。**修正後**は確認が出て、キャンセルすると作成されない／OKなら作成できる（決定済みの案B通り）。重ならない「2026年10月前半」は確認なし＝誤検出なし。console error 0件。

- **🟡 管理者画面からの提出削除が拒否されても「削除しました」としか出ない**（[app-main.js:1161](app-main.js)）→ `b2220fe`
  同日の `8952c8f` は `subs` の削除をルールで「提出した端末（`submitterUid`）か店舗オーナー」に限定したが、**拒否の手当てを足したのはスタッフ画面側（`onDeleteSub`・:1508）だけ**で、**管理者側（`saveSubs` の `deletedId`）は `.catch(e=>console.warn(...))` のまま**だった。管理者側の削除ボタンは2箇所（`SmModal`・:1961／提出一覧・:3414）とも**呼び出し直後に無条件で「削除しました」**を出すので、**管理キーを持たない端末（`ownerReadOnly`）では成功表示だけが残る**。あわせて `pendingSubWritesRef` を外して `reconcileSubs` を呼び直し、サーバー内容で確実に描き直すようにした（拒否時は `value` イベントが来る保証がないため）。`deletedId` があるときだけ扱う＝通常の編集の失敗は挙動不変。
  **未検証**: ルールに拒否させる実機E2E（Firebaseの拒否応答が要り、`firebaseDB` が null の単体マウントでは再現不可）。

### 要確認（未修正）

- **🟢 片側セルのみの日に、勤務時間は休憩を引くのにヒートマップは引かない**（[app-admin.js:828](app-admin.js) `getHeatShift` の `if(!st||!en)return null;`）→ **BACKLOG化済み**。実測: 候補09:00〜15:00・休憩12:00〜13:00・出勤セルだけ09:00 で `getBreaksFor` は1件・`calcNetWorkMinutes` は300分になるが、ヒートマップ12時台は在席のまま。同じ1行が退勤延長も落とす。**直すとヒートマップの人数表示が変わる**ため表示上の判断。
- **🟢 スタッフ削除の設定後始末が、期間に名前を残した場合は永久に走らない**（[app-admin.js:2570](app-admin.js) の `keepIds.size===0` 条件）→ **BACKLOG化済み**。残した期間の最終日を過ぎると一覧の行は自動で消えるが、そのとき後始末を走らせる経路が無く、#79の②「死んだ別名が `staffAliases` に残る」がこの経路でだけ生き残る。
- **🟢 片側セル補完の境界が2箇所に同じ式で書かれている**（[app-admin.js:798](app-admin.js) と [app-utils.js:146](app-utils.js) `oneSidedFillBounds`）。現時点で出力は一致するが、片方だけ直すと集計とヒートマップで補完位置が食い違う。判断不要の純粋な重複なので申し送りのみ。
- **🟢 スタッフ画面で他人の提出を「✎ 修正」すると、その提出の削除権限が編集者へ移る**（[app-staff.js:206](app-staff.js)）。`submitterUid` は提出のたびに上書きされる。書き換える能力自体は従来からあり、`8952c8f` も「最後に提出した端末が提出者になる」と明記＝新しい穴ではない。
- **🟢 配信版数は6箇所とも `20260825-92a9e27` で一致していたが、今回 app-admin.js・app-main.js が変わった**ため次のリリースでバンプが要る（`/release-to-main` 手順63が持つ）。
- 変化なし・**BACKLOG化済み**: 「閲覧専用」端末に設定・シフトの書き換えを許すか（#68）／`keepStaff` の位置が近似になる（#94）／Excelに変更マークが無い／休憩タグの編集がindex／`selDates` の破壊的ソート／`PLAN_LIMITS` フォールバックの不一致／`name|date` の分解が2箇所で食い違う／CFの `linkStoreToCompany` の文字種検証／写しによる `periods` 肥大（案A適用後の残り）／二重課金の実購入検証・特商法表記（🔴）／解約イベントの metadata 判定。

### 検証したこと

- `DEV_MODE`（app-core.js:12・式のまま）／`DEV_PLAN_OVERRIDE`（:125）正常／`subs` の `set()` 全体上書き **0件**／`accounts`・`global/shops` 全件読み **0件**／`".read": true` **0件**／読み込み順維持・SRI **11本**
- **今回の差分で新規追加された `input`/`select`/`textarea` は 0件**。既存も含め `fontSize` 16未満は **0件**
- `npm test` **197件パス**／`npx eslint app-*.js` **0 errors 95 warnings**（増減なし）／`node --check functions/index.js` OK
- **色の一括置換の回の4点チェック**（`d22653c` が対象）: 未定義CSS変数 **0件**／`:root[data-theme="dark"]` と `prefers-color-scheme: dark` の**差分なし**／SVG属性内 `var()` **1件**（`ShiftyIcon`）→ **chromium・webkit × light・dark の4条件すべてで `rgb(248,112,54)` に解決**（#91以降ずっとChromiumのみだった条件を実測で追加。既定の黒に落ちていない）
- CF: `secrets:` **6箇所**／`.delete()` 誤用 **0件**／`resolveShopMeta` の新しい早期returnは Session・Invoice では `planOfSubscription` が null を返し `md.plan` にフォールバックすることを確認（誤ったプラン解決は起きない）
- **期間の凍結の完全性**: `ShiftEditTab` が読む `settings.*` を全数列挙して `PERIOD_SNAPSHOT_SETTING_KEYS` と突き合わせ、差は `dateCandidates`（`3464eaa` の案Aで意図的に除外）と `xlShopName`（元から対象外）**の2つだけ**＝取りこぼしなし
- `isCountExcluded`（`x`）が `heatData`/`dupErrors`/`positionErrors` の**3箇所すべて**に入っていること、`calcNetWorkMinutes` の呼び出し**全件**が第4引数 `settings` を渡していることを grep で確認
- **未検証**: ルール拒否下での提出削除／iOS Safari 実機（WebKitはPlaywrightビルドで測定）／Excel・PDF の実物確認（#93から継続）／タップ領域44pxの再実測（`d22653c` 適用後・#74から継続）／コントラストの追試／本番データ（ユーザー操作）

### 総括

**1日で10件以上を本番へ出した回に対して、同じ形の取りこぼしが2件出た。** どちらも機能の誤りではなく、**修正を入れた入口の数が、実際の入口の数より1つ少なかった**。片方はコミットメッセージ自身が「作成・編集で同じ関数を通す」と入口を2つと数えており実際は3つ、もう片方は削除の呼び出し元2系統のうち1つにしか手当てが入っていなかった。

**一括実装の回に効いたのは、コードの正しさを読むことではなく、著者の数え上げを grep で検算することだった。** どちらも grep 1回で出る。CLAUDE.md「実装依頼の受け方」手順3（全呼び出し元の洗い出し）がまさにこれを要求しているが、**10件を1コミットに畳むと手順3が10回ぶん必要になる**。あわせて「既定値がどれか」も見ること——プリセットの漏れは、**大多数のユーザーが通る道が検証を通らない側だった**ために重大度が上がった。

**据え置き項目の減らし方**も1つ分かった。色の4点チェックは全部空振りだったが、そこで指された唯一の箇所（`ShiftyIcon` の属性内 `var()`）を **WebKit でも測る**という追加だけで、#91 から5回続いた「WebKit未測定」をこの箇所については閉じられた。**全面再測定は重いが、チェックが指した1箇所なら安い。** 同じやり方が使える残りは、タップ領域（`d22653c` が変えたタブバーの `gap` だけ）とコントラスト（`d22653c` が触った変数を使う要素だけ）。

**次回の手**: ①**リリース直後の回では、各コミットメッセージが「何箇所に入れた」と主張しているかを抜き出し、対応する grep を全部走らせる**（今回2件ともこれで出た）②`d22653c` が変えた寸法**だけ**を375pxで実測して #74 の据え置きを部分的に閉じる ③Excel・PDF の実物確認（#93から3回連続の申し送り）。
