...
## Shifty バグチェックレポート（2026-09-06 自動実行 #112）

> 着手時の HEAD は `50bd4e3`。app-*.js に新しい変更は無く、#111 以降はドキュメントのコミットだけ。
> 観点は #111 が申し送った「**日付・期限・"猶予"・"暫定" を含む記述を機械的に洗い出して期限切れを確かめる**」と、
> #111 自身が名指しした形——**「修正が報告された場所で止まり、同じ問いに答えている残りへ広がらない」**——の2つ。
> 前者は当たりが無く、後者で 🟡 を1件見つけた。

### 修正済み

- **🟡 Cookieから復元した名前が別名解決を通らず、提出が重複していた**
  （[app-staff.js:67・:167](app-staff.js)・`c9a9d14`）

  #81 の「同一人物・同一期間に2つのsubができる」は `c889660` が**入力経路**を直したが、
  **Cookieから名前を復元する経路（:48・:59）は `resolveAlias` を通っていなかった**。
  :156 のコメントは「name は :292/:281 で resolveAlias 済み＝常に登録名」と、
  **遠くの2箇所に依存して**不変条件を主張していたが、Cookie経路はその2箇所を通らない。

  **別名フォールバックが効かない理由**: `staffAliases` は**登録名をキーに持つ**（`{"田中":["たなか"]}`）ため、
  別名を起点にすると `staffAliases["たなか"]` が `undefined` になり、`resolveSubByAlias` は
  完全一致に失敗した後の**フォールバック先を1つも持てない**。#105・#106 の「解決規則を1関数へ一本化」は
  正しかったが、**渡す名前が別名のままなら一本化は効かない**。

  再現（配信物の関数を Node で実行）: 別名「たなか」で提出 → 管理者が別名登録 →
  別端末からの再提出で `staffName` が「田中」へ正規化 → 端末A（Cookie＝「たなか」）で開く。
  ...
  **被害はスタッフ側で終わらない**。読み手（グリッド・週集計・提出一覧・Excel）は `resolveSubByAlias` で
  「田中」のsubを引くため、**この経路で作られた2つ目のsubは管理者の画面に一度も出ない**（#106 と同じ形）。

  修正: :67 で Cookie名を `resolveAlias` で寄せ探索を `resolveSubByAlias` に統一。
  :167 で `staffName` を `resolveAlias` に通し、**:156 の不変条件を構成で保証**する（冪等）。

### 要確認（未修正）

- **変化なし（BACKLOG化済み・条件A/Bでループの権限外）**: 二重課金の根治／特商法表記（🔴 2件）／解約通知／
  解約時のプラン判定／企業連携の解除と `verifyShopOwner` の移行猶予／別名提出の重複の**根**（#81）／
  PDF の実物確認／`purgeOldPeriodsCutoff` の UTC 切り（**本有効化は 2026-09-11 以降＝今日はまだ着手日前**）。
- **引き受け済みのトレードオフ（再検出しても直さない）**: `subs/$subId/.write` は認証済みなら通り、
  提出を触れるのを本人に絞っているのは UI（`canTouch`）のみ。2026-08-31 決定1。

### 検証したこと

**#111 の申し送りは当たりが無かった**。`猶予|暫定|当面|一時的|移行期|TODO|FIXME` と日付形状リテラルを
全コードに当てて1件ずつ確認した結果、期限切れは**既にBACKLOG化済みの `verifyShopOwner` の「移行猶予」だけ**。
`index.html:107` の `defaults:'2026-01-30'` は PostHog 自身の設定バージョン指定で日付ではない。

**UTC/JST のズレに兄弟がいないかを全経路で確認**。クライアントの `fd()` はローカル時刻（＝JST）で正しく、
`toISOString()` の用途はすべて**日付ではなくタイムスタンプ**。`planExpiry` はJSTへ寄せてから切っており、
`purgeInactiveShops` は**ミリ秒の引き算だけ**で時差非依存。**ズレは `purgeOldPeriodsCutoff` の1箇所に閉じている**。

**SRI ハッシュ11本を実際のCDNと照合した（今回が初めて）**。崩れるとアプリが起動しないのに配信物からは分からない項目。
`curl | openssl dgst -sha384` で**11本一致・不一致0・取得失敗0**。

**価格の記述19箇所を機械照合**。500円/2,980円が `terms.html`・`TERMS_TEXT`・各導線・Stripe price 定数コメントで
全一致（Stripe の price オブジェクトの実額はダッシュボード確認が要る＝未検証）。

**「subs を名前で引く」全経路を洗い直した**。自前探索を全部拾って判定し、`app-admin.js:588-595` は
完全一致→別名を登録順で `resolveSubByAlias` と同順、`:207`・`:1355`・`:2412` は完全一致が正しい別の問い、
`:548`・`:2494`・`:3540` は別名を明示的に見ている。**残っていたのは Cookie 経路の1つだけ**。

**実ブラウザでも確認**（`StaffView` だけをマウントし `app-main.js` を読まない＝**Firebase へは1バイトも出していない**）。
4ケース（本件／Cookieもsubも登録名／別名なしの店舗／他人の名前）すべて期待どおりで `pageerror`・`console.error` 0件。
**修正前の配信物（`50bd4e3`）へ向けると本件だけが落ちる**ことを確認済み＝素通りするテストではない。
スクリプトは `.claude/skills/shifty-e2e-verify/scripts/example-staff-cookie-alias.js` に保全（`8a96acb`）。

**RULES.md のスキャン項目は全項目クリア**: `DEV_MODE` は式のまま／`subs` の `set()` 全体上書き **0件**／
全件読み **0件**／読み込み順維持／SRI **11本**／フォーム部品56件の `fontSize<16` **0件**／
`.delete()` 誤用 **0件**／`secrets` 7箇所。

`npm test` **235件パス**／`npx eslint app-*.js` **0 errors 96 warnings**／`node --check functions/index.js` OK。
**Firebase・Stripe には読み書きとも一切アクセスしていない。**

**未検証**: PDF の実物（jsPDFがラスタ化するため列テキストが残らない）／iOS実機／本番データ（claim状況・
Stripe実データ・Cloud Logging）／Stripe の price オブジェクトの実際の金額。

### 別セッションとの並行（報告事項）

作業中に別セッションが「スタッフ非表示」を実装し**本番リリースまで行った**（版数 `20260906-d3006ae`）。
その結果、**今回の修正 `c9a9d14` は自分では main に触っていないのにそのリリースに乗って本番へ出ている**
（`git branch --contains` が develop と main の両方を返す）。版数はリリース側でバンプ済みで今回の修正も含まれる。
同じ `app-staff.js` を両セッションが触ったが**今回の2行は上書きされずに生きている**ことを確認し、
**ツリーが入れ替わったのでテスト・eslint・回帰テストは現ツリーで測り直した**（テストが226→235なのは別セッションの追加分）。

### 測り方の記録

**今回の収穫は「一本化した関数に、正しい引数が渡っているかまでは誰も見ていなかった」こと。**
#105・#106 は「解決規則を `resolveSubByAlias` へ集約する」正しい処置を2回続けたが、
**集約先の関数は渡された名前が別名だと何もできない**。
**一本化は「呼び出し側を全部通したか」だけでなく「入口で値が正規化されているか」まで見て初めて完成する。**

**コメントが遠くの行を根拠に不変条件を主張していたら疑う。** この形は**書かれた時点では正しく、
あとから経路が増えると黙って偽になる**。次は「`X は～済み` と別の行を根拠に書いたコメント」を集め、
その state の**全書き込み元**を数えるとよい（今回なら `setName` の呼び出し元を数えれば即座に分かった）。

**#111 の申し送りは外れたが、外し方が有用だった。** #110・#111 が2回続けて掘った結果**鉱脈が尽きた**と読め、
申し送りは外れたときに**その観点を閉じてよい合図**として使える。

### 総括

**🔴 ゼロ。🟡 は1件検出し、その場で修正・検証・コミットまで完了した。**
これで **#81 の根に6つ目の経路**があったことになる（#105 が4つ、#106 が5つ目、今回が6つ目＝Cookie経路）。
3回とも「一本化した規則そのもの」は正しく、**壊れていたのは常にその規則へ入る手前**だった。
今回は入口を `resolveAlias` に通したので、**7つ目の経路が将来増えても同じ形では再発しない**。
あわせて未実測だった **SRI 11本の実物照合**と**価格19箇所の照合**を通し、
どちらも異常なしだが**静かに壊れる**種類なので今回の実測値を基準として残す。
