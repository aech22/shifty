...
## Shifty バグチェックレポート（2026-08-13 自動実行 #71）

### 修正済み

**なし。今回はコードを1行も変更していない。** 🔴・🟡の実行時バグを検出しなかったため。新規に挙げた3件は🟡1件＝リリース工程のゲート、🟢2件＝申し送り。

### 要確認（未修正）

- **🟡（新規・リリースゲート）`?v=` と `build:` が develop の app-*.js に追随していない**（index.html:214-218 / app-core.js:2）
  6箇所すべて `20260813-64d7a4b` で**相互には一致している**が、この版数が指す `64d7a4b` 以降に **app-admin.js・app-core.js・app-main.js・app-staff.js と index.html が変更されている**（`9427359` テーマ修正＋`a3376a4` の CSS変数追加）。本番も `?v=20260813-64d7a4b` を配信中（curl実測）。**バンプせずにリリースするとキャッシュに旧 app-*.js が残りテーマ修正が届かない**。ただし**壊れた中間状態にはならない**（`?v=` は index.html の中にあるので旧index→旧JS・新index→新JSで必ず整合する）。起きるのは「修正が黙って届かない」だけ。
  **未修正の理由＝ループの権限外**（版数はリリース時のSHAで決まる／main へのマージはユーザー確認後）。`/release-to-main` 手順63が `?v=` と `build:` の更新を標準工程に持つため**その工程を通せば閉じる**。BACKLOG化はしない（二重管理になるため）。**直前の `64d7a4b` の「app-*.js は無変更のためバンプしない」は当時は正しいが、続く2コミットで失効している。リリース時にそのメッセージだけを読まないこと。**
- **🟢（新規）`--c-danger` が定義のみで使用ゼロ**（index.html:127）。`a3376a4` が `--c-accent` と同時に追加したが app-*.js からの参照0件。赤系は `#FF4757`・`#EF4444` が直書きのまま残り、`--c-danger`(`#DC2626`) はどちらとも違う値。デザイン是正 Phase 2 で決めればよい。
- **🟢（新規）`onSub` のローカルstate更新キーと Firebase 書き込みキーが違う**（app-main.js:1513-1521）。書き込みは `diffSubForFlatWrite(sub.id,...)`＝**id基準**、`setSubs` は **staffName+periodId基準**。同名同期間のsubが2件あると別レコードを指す。**通常フローでは重複は生まれない**（提出は `existSub` の id を再利用・app-staff.js:189／`applyEditToSubs` も別名まで見てから新規作成・app-admin.js:550-560）。到達は**同時提出の競合**のみで、影響もローカルstate/localStorageの一時的なズレ（Firebaseへは正しいidに飛び、リスナー着信で自己修復）。
- **🟢 マイページに `ownerReadOnly` が渡っていない**（app-admin.js:208）。#68 から変化なし
- **🟡 Pro/Premium 店舗ではスタッフURLを開いた誰でも他人の提出を削除できる**（#70）→ BACKLOG化済み・仕様判断待ち。**今回の prop 突き合わせで「この1件が構造クラスの全部」と確定**（下記）
- **🟡 解約イベントだけが metadata でプランを判定している**（functions/index.js:319）→ BACKLOG化済み（#68起票）。Stripe実データが要るため決着不可・状態変化なし
- 以下は #46〜#70 からの継続（状態変化なし）: 未claim店舗を誰でも自分の企業に取り込める根／`findActiveSubscription` の `|| live[0]`／`subscription_schedule.*` の4分岐が未購読／`planExpiry` のUTC基準計算／決済失敗フラグの誤解除／片側セルだけの日が0分／`purgeOldPeriods` のUTC日付／`getBreaksFor` の休憩判定／`generateInviteCode` のデッドコード／`urlLocked` が StaffView で未使用／`staffSectionOn` と `isCountExcluded`／SubsTab「月計」の別名重複／`sub.shopId` が死にフィールド／店舗削除が `global/shops/{id}` を消さない／`companyCodes` の TOCTOU／略称の先勝ち／期間作成の日付検証なし／提出状況ビューのセル編集／Excel analytics の数え方／スタッフ数バーのスペーサー／`breakTimes[].tags` の残留／別名解決順の不一致／`heatHours` が `adminRest` を無視／スタッフ削除時のキー残留

### 今回の対象と見つけ方

**#70 以来はじめて「動いたコード」がある回**（develop は origin/main の2コミット先＝`9427359` Auto-commit 4ファイル158行 と `a3376a4` テーマ修正＝**デザイン是正 Phase 1**）。対象を2本立てにした: **(1) 新規コードのレビュー**、**(2) #70 が次の手に絞った「`StaffView` の prop を1件ずつ突き合わせる」の実行**。

**(1) 新規コード — 「色の置換で静かに壊れる型」を4つ挙げて全部潰した（すべて否定的結果）**

色の置換は**壊れてもコンソールに何も出ない**（無効値は黙って捨てられ、testもlintも通る）ため、失敗の型ごとに機械的な検査を当てた。

- **仮説A: `var()` が SVG のプレゼンテーション属性で解決されない。** `ShiftyIcon`（app-staff.js:9）の `<rect fill="var(--c-accent)"/>` は **app-*.js で唯一の「属性に直接 var() を書いた箇所」**。失敗すれば fill が既定の黒に落ち、**全画面に出るアプリアイコンが黒い四角になる**。歴史的に WebKit の穴だった箇所。**実測: Chromium・WebKit 26.5 の両方、ライト/ダーク両テーマで `rgb(248,112,54)`。CSSルール経由の対照実験も同値。→ 問題なし**
- **仮説B: 未定義の変数名を参照している。** **実測: app-*.js が使う12変数と index.html の13定義を突き合わせ、未定義0件**
- **仮説C: ダークの2ブロックが食い違っている。** テーマは `:root[data-theme="dark"]` と `@media (prefers-color-scheme: dark)` の**二重管理**。**実測: 両ブロックとも同一の11変数で完全一致。`--c-accent`/`--c-danger` はブランド色として意図的に `:root` のみ＝両ダークでも継承されることを実測**
- **仮説D: 「色だけ」と称するコミットにロジックが混ざっている**（コミット名が `Auto-commit: app-*.js changes` で意図の記載なし）。**検査: 差分の追加/削除行から色リテラル（`#xxxxxx`・`rgba()`・`var()`・data URI 内 `%23xxxxxx`）と空白を除去して正規化し残差を見た。4ファイルとも残ったのは色の値・`border` プロパティ追加・説明コメント2行だけで、ロジック変更0件**

**(2) #70 の宿題 — prop 突き合わせは「穴が広がる」ではなく「閉じている」を示した**

- `StaffView` の受け口（app-staff.js:18）は13個。app-main.js:1503-1540 が渡すのも**同じ13個で過不足なし**
- 再転送先は `SmModal`（:236・257）と、その中の `CellEditPanel`（:649）だけ。`CellEditPanel` の5 prop は**すべて `SmModal` のローカル値**で App からの素通しは0
- スタッフ側とAdmin側（app-admin.js:1867）の `SmModal` prop の差は **`onEditByName` がスタッフ側だけ**。共通で渡る危険な prop は **`onDeleteSub` 1個だけ**
- app-staff.js と app-admin.js の共有コンポーネントは `ShiftyIcon`（表示のみ）と `SmModal`（＋`CellEditPanel`）**の2つで全部**

→ **#70 の `onDeleteSub` はこの構造クラスの"サンプル"ではなく"全部"だった。** 「他にもありうる」にコードで上限を与えて閉じた。

**(3) 副産物として追った仮説（これも否定）**: `onSub` のキー不一致から `source:"grid"` を疑った。app-staff.js:163 は「**source を引き継がないのは意図的で、提出後は実際の提出として数えられるようにする**」と宣言するが、**`onSub` は 2026-08-10 に `set()` から差分 `update()` へ変更されている**。`update()` は渡さなかったキーを消さないため、差分生成が「消えたキー」を扱っていなければ **`source:"grid"` が残留し、提出したのに提出一覧・件数バッジから永久に消えたままになる**という筋。**実測: `diffSubForFlatWrite`（app-utils.js:604-607）は `!(key in newSub)` のキーに明示的に `null` を書いており、`source` は正しく削除される。→ 意図は保たれている**

### 検証したこと

- `DEV_MODE`（app-core.js:12・式のまま）正常。`DEV_PLAN_OVERRIDE`（:125）正常
- `subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／`global/shops` 全件読み **0件**／CF `.delete()` 誤用 **0件**
- 読み込み順（utils→core→staff→admin→main）維持・SRI **11本**・版数6箇所すべて `20260813-64d7a4b` で相互一致（版数自体が古い件は上記🟡）
- `npm test` **168件パス**／`npx eslint app-*.js` **0 errors 98 warnings**／`node --check functions/index.js` OK
- **実機実測（localhost:3000・Chromium と WebKit 26.5）**: `fill="var(--c-accent)"` は両ブラウザ・両テーマで `rgb(248,112,54)`／ライト bg `#F0F2F5`・ダーク bg `#0F172A`・accent は両テーマ `#f87036`（意図どおりテーマ非依存）／**コンソールエラー0件**
- 本番配信版数を `curl` で実測 → `app-core.js?v=20260813-64d7a4b`
- **未検証**: テーマ修正の見た目そのもの（コントラスト比）は `a3376a4` の検証記録に依拠し、今回は「変数が解決されること・未定義がないこと」だけを対象にした／🟡の実機E2E（スタッフURLでの他人の提出削除）は今回も未実施／新規🟢のキー不一致は重複subを実際に作って再現させてはいない

### 総括

**新しいコードが入った回の正しい使い方は、変更の種類から失敗の型を先に決めて、型ごとに潰すことだった。** 入ったのは色の置換というレビューで最も流し読みされやすい変更で、実際ロジックは1行も変わっていなかった。だがこの種の変更の危険は**壊れてもコンソールに何も出ない**点にある。4つの型を挙げて機械的検査を当て、4つとも空振り＝コードは健全と確定した。

**空振りを空振りとして記録できたのは、検査を目視ではなく実測と正規化差分で組んだから。** 特に `fill="var(--c-accent)"` は Chromium だけで終えると**ユーザーの主戦場である Safari／iOS Safari を見ないまま「問題なし」と書くことになっていた**。「色だけのコミット」の判定も、148行を目で追う代わりに色トークンを `§` に正規化して差分を取ることで**見落としの余地なく "ロジック変更0件" を言えた**。

**#70 の宿題からは否定的だが確定的な収穫。** #70 は「同じ構造の prop が他にもありうる」と締めたが、答えは「これで全部」だった。**"広がっているかもしれない" を "ここまでで閉じている" に変えるのは、穴を1つ見つけるのと同じくらい判断に効く**——BACKLOG の案の選択（案D／`onDeleteSub` を外す）を、未発見の穴を巻き込む心配なしに決められる。

**唯一の🟡はコードでなくリリース工程にある。** `?v=` が2コミット分古い。**壊れた状態にはならず、ただ修正が届かない**ため、リリース後に「直したはずが直っていない」という形でしか表面化しない。しかも直前の `64d7a4b` が「app-*.js は無変更なのでバンプしない」と**正しい理由を明記している**ため、リリース時にそれだけを読むと前提の失効に気づけない。#70 の教訓（誤った記述は周辺の見積もり全体を歪める）の変種で、今回は**記述が誤っていたのではなく、正しい記述が古くなっていた**。

**次回の手を1つに絞る。** #70 の構造クラス（共用コンポーネント経由の権限漏れ）は閉じたので、**次は同じ「二重用途」を prop ではなくデータの側で見る**。`SmModal` が `source!=="grid"` で除外している**管理者由来の sub**（`applyEditToSubs`・app-admin.js:564・573 が作る）について、**除外を忘れている場所がないかを1件ずつ突き合わせる**。今回 `source` の削除経路を追う過程で、この印が「提出一覧」「バッジ件数」「SubsTab」「期間カードの件数」の**4箇所で個別に `s.source!=="grid"` と書かれている＝共通関数を通さず複製された条件**だと分かった。**漏れが1箇所あれば管理者の下書きが提出として数えられる**（プラン上限のスタッフ数カウント・Excel出力・PDFが候補）。コードだけで閉じられる。
