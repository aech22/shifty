...
## Shifty バグチェックレポート（2026-09-08 自動実行 #116）

> 着手時の HEAD は `243b7b6`。観点は **#115 の申し送りをそのまま実行**した——
> 「今回と同じ手を、コードではなくドキュメント側へ向ける。`.md` の `ファイル:行` 引用を
> 機械的に解決し、引用先に名指しされたシンボルが実在するかを当たる」。
> 予想どおり同じ形が出た。**そのうえで、位置ではなく「存在」を当てたら別の層が出た。**

### 修正済み

- **🟡 ドキュメントが「削除済みのコード」を今もあると書いていた**
  （[CLAUDE.md](CLAUDE.md)・`2142be5` / `9a9046d`）

  旧・招待コード方式は **2026-08-24 の `8384467` で完全に削除**されている
  （`generateInviteCode` の定義約60行・state 2件・`database.rules.json` の
  `inviteCodes` と `accounts/{uid}/members` のルール）。ところが CLAUDE.md は
  **4箇所でそれが今も在るものとして記述していた**。

  | 箇所 | 書かれていたこと | 実態 |
  |---|---|---|
  | 関数一覧 | 「`generateInviteCode()` は **デッドコード**（app-main.js:819 に定義はあるが呼び出し元ゼロ）」 | **定義ごと存在しない**（grep 0件） |
  | 企業アカウント節 | 「`database.rules.json` の `inviteCodes` ルールも**維持されている**」 | ルールの最上位キーは `global/tokens/shops/accounts/companies/companyCodes` の6つで **`inviteCodes` は無い**。`accounts` 配下も `$uid` のみ |
  | state 一覧 | 「`inviteCodeDisplay` \| string\|null \| 企業招待コード表示用」 | **同じコミットで消えた state**（grep 0件） |
  | Firebase データ構造 | ツリーに `inviteCodes/` と `accounts/{uid}/inviteCode`・`members/` を掲載 | どれも無い |

  **これは行番号のずれより悪い。** 位置がずれた参照は「その行に別のものがある」で済むが、
  **存在の断言が偽だと、読み手は無い物を探しに行く。** 実際この記述は
  「デッドコードが残っている＝掃除の余地がある」と読めるよう書かれており、
  次のループがそれを拾えば **既に済んだ掃除をもう一度やろうとして空振りする**。

  **矛盾は同じファイルの中にあった。** CLAUDE.md 末尾の完了済みタスクには
  「招待コード方式の残骸を削除（2026-08-25・`8384467`）」がちゃんと載っている。
  **同じ文書が、前半で「残っている」と言い、後半で「消した」と言っていた。**

- **🟡 `.md` の `ファイル:行` 引用 42箇所が別のコードを指していた**
  （[BACKLOG.md](BACKLOG.md)・`c789cdd`）

  5つの文書（CLAUDE.md・BACKLOG.md・RULES.md・VISION.md・サブスク_プラン設計書.md）から
  `ファイル:行` 形式の引用を機械的に集めると **93件**あり、
  引用と同じ行に出てくるシンボル名が引用先の±3行に無いものが **49件**だった。

  実測（引用された番号 → 実際の定義行）:

  | シンボル | 引用 | 実際 | ずれ | 引用先に実在したもの |
  |---|---|---|---|---|
  | `UpgradeModal` | app-admin.js:3810-3811 | 4951 | **1141行** | 店舗切り替えボタン |
  | `UpgradeModal` | app-admin.js:4171 | 4951 | 780行 | **空行** |
  | `registerAlias` | app-admin.js:2969 | 3560 | 591行 | StaffTab の「編集」ボタン |
  | `unlinkStoreFromCompany` | functions/index.js:815 | 1223 | 408行 | `db.ref("shops").once("value"),` |
  | `linkStoreToCompany` | functions/index.js:815 | 1179 | 364行 | 同上 |
  | `saveSubs` | app-main.js:1464 | 1186 | -278行 | 「Googleでログイン」 |
  | `isSpecialRedDate` | app-utils.js:646 | 807 | 161行 | `extractNote` の return |
  | `diffSubForFlatWrite` | app-utils.js:572 | 733 | 161行 | `resolveSubByAlias` 内の1行 |
  | `resolveShopMeta` | functions/index.js:319 | 381 | 62行 | 空行 |
  | `onSub` の `findIndex` | app-main.js:1517 | 1559 | 42行 | フッターの div |

  **「存在しないコードを指す引用」も混じっていた**: BACKLOG.md の完了済みタスクにある
  `inviteCodes` の `expiresAtMs` 書き込み（app-main.js:847-853）は、
  **その行どころか `expiresAtMs` 自体がコードから消えている**（上の `8384467`）。

  **直し方は #115 と同じ「寿命の短いほうを捨てる」**。番号を振り直すのではなく、
  **番号を落としてシンボル名・コンポーネント名に置き換えた**（`grep` できる側を正本にする）。
  番号しか無かった箇所（`（:4628）`・`（:557）`・`` `createCheckoutSession`:127 `` 等）は、
  指していたものの名前にした。**42箇所すべて、置換前に実際の定義位置を grep で確認している。**

  同期経路も揃えた。BACKLOG.md は Obsidian 側が正本で `scripts/obsidian-sync.js` が
  CLAUDE.md と `.cursorrules` へ写すため、**リポジトリ・Obsidian・写し2つの4箇所**を
  一致させてある（`diff` でバイト一致を確認・`--once` で写しを再生成）。

### 要確認（未修正）

- **🟢 #115 レポート本文の引用4件は直していない**（CLAUDE.md 末尾の「最新レポート」ブロック内）。
  **意図的**。あれはその日に測った値の記録であり、後から番号を書き換えると**記録が偽になる**。
  なお残り4件は検査器の偽陽性で、いずれも**コメント見出し行を正しく指している**
  （`app-admin.js:4594` は `// ===== プランの購入・変更の唯一の入口 =====`）。
  ±3行にバッククォート付きシンボルが無いという理由だけで拾われたもの。
- **🟢 配信版数が直近6コミットを含んでいない**。`index.html` の `?v=` と `app-core.js` の
  `build:` は6箇所とも `20260906-d3006ae` で相互に一致しているが、app-*.js の最終変更は `4e274a4`。
  **`/release-to-main` の手順63 がバンプするので申し送りのみ**（#115 から持ち越し・変化なし）。
...
**今回 BACKLOG へ起こした項目は無い。** 検出した🟡2件はその場で閉じた。

### 検証したこと

**今回の変更はドキュメントだけで、コードは1バイトも触っていない。**
3コミットの差分は `CLAUDE.md`・`BACKLOG.md`・`.cursorrules` の3ファイルのみで、
`git diff 243b7b6..HEAD -- '*.js' '*.html' '*.json'` は**空**。

- **引用の検査器**（`ファイル:行` を解決し、同じ行のバッククォート識別子が±3行に在るかを見る）:
  修正前 **93件中49 MISS** → 修正後 **33件中4 MISS**（残り4件は上記の偽陽性）。
  BACKLOG.md の MISS は **0件**、裸の `:NNN` も **0件**。
- **存在の検査器**（文書中のバッククォート識別子が配信物のどこかに在るか）: 205件中34件が不在。
  **33件は偽陽性**（コミットSHA `dbdd9d9`・`ca2caa8` 等、ツール名 `preview_start`・`python3`、
  「廃止済み」と明記済みの `applyFixedShiftToSubs`・`globalTemplates`）で、
  **本物は `inviteCodeDisplay` の1件**だった＝上の修正で 0件。
- 置換の安全性: 42件の置換は**すべて出現回数がちょうど1であることを確認**してから適用し
  （1件でも0回/2回以上ならスクリプトを中断する形にした）、差分は35行の増減で、
  各行が参照トークンの置き換えだけであることを目視で確認した。

**RULES.md のスキャン項目は全項目クリア**: `DEV_MODE` は式のまま（app-core.js）／
`subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／`global/shops` の全件読み **0件**／
読み込み順（utils→core→staff→admin→main）維持／SRI **11本**／
フォーム部品57件の `fontSize<16` **0件**／`functions/index.js` の `.delete()` 誤用 **0件**／`secrets` 7箇所。
提出削除の3経路はいずれも `deletedId` を渡すか `.remove()` を直接呼んでおり、
「filter しただけで Firebase に残る」形は**0件**。

`npm test` **236件パス**／`npx eslint app-*.js` **0 errors 96 warnings**（増減なし）／
`node --check functions/index.js` OK。
**Firebase・Stripe には読み書きとも一切アクセスしていない。実ブラウザ検証は行っていない**
（変更が文書のみで、画面に現れる差が原理的に存在しないため）。
**未検証**: iOS 実機／PDF の実物／本番データ／Stripe 実データ。

### 測り方の記録

**引用は2つの別々のことを主張していて、壊れ方も別だった。**
`app-admin.js:2969` という引用は「**そこに在る**」と「**在るのはこれだ**」を同時に言っている。
#115 は前者（位置）を測り、27件中25件の破損を見つけた。今回**後者（存在）を測ったら、
位置の検査を全部すり抜けるものが出た**——`inviteCodes` のルールは、
どの行を指してもよいのではなく、**指す先そのものが無い**。
位置の検査器は「引用先に別のコードが在る」としか言えないので、これを拾えない。
**壊れ方が2種類あるなら、検査器も2つ要る。**

**「消した」という記録は、「消した対象を説明していた記述」を直さない。**
`8384467` は削除をきちんとコミットメッセージに書き、BACKLOG の完了済みにも残した。
それでも CLAUDE.md 前半の4箇所は生き残った。**削除の記録を増やしても、
削除前提で書かれた説明文は自動的には消えない。** #115 が
「コメントで列挙することも書き写しに数える」と結論したのと同じ形で、
**「機能を消したら、その機能を説明していた文も消す対象に数える」**必要がある。

**一番安い検査は「その名前は今もあるか」だった。** 205件のバッククォート識別子を
配信物へ grep するだけで、判定に主観が入らず、行番号のように毎日腐らない。
偽陽性33件はコミットSHAとツール名で、**形で機械的に除ける**。
位置の検査（±3行）より弱い代わりに、**位置がどう動いても壊れない**。

**申し送り（次回の観点）**: **今度は「数」と「一覧」を当たる。**
#115 は「7つ」という注記が `staffHidden` を数え落としていたのを見つけ、今回は
「4エンドポイントすべて」「呼び出し元3箇所」「最上位キー6つ」のような**数を伴う断言**を
引用の巻き添えで何度も検算することになった。文書とコメントには
**「N件」「N箇所」「すべて」「〜だけ」という数量の断言**が大量にあり、
これは位置とも存在とも別の壊れ方をする（**要素が1つ増えた瞬間に静かに偽になる**）。
`(\d+)(件|箇所|本|つ|経路|マップ|エンドポイント)` を機械的に集め、
その数を実際に数え直せば同じ形が拾えるはずである。**数は、位置と違って
「ずれた」ことが読んで分からない**ぶん、行番号より発見が遅れる。

### 総括

**🔴 ゼロ。🟡 は2件検出し、どちらもその場で修正・検証・コミットまで完了した。**
#115 の申し送り（同じ手をドキュメントへ）をそのまま実行したところ、予想どおり
**93件の引用のうち49件が別のコードを指していた**（最悪 1141行のずれで、引用先には
無関係な店舗切り替えボタンが実在した）。**予想していなかったのはもう一層あったこと**で、
位置を測る検査器を全部すり抜けたまま、**CLAUDE.md が 2026-08-24 に削除済みの
招待コード方式を4箇所で「今もある」と書き続けていた**——同じ文書の後半には
「削除した」という完了記録が載っていたので、**文書が自分自身と矛盾していた**。
壊れ方が「位置」と「存在」の2種類あり、片方の検査器ではもう片方が見えない。
コードは1バイトも触っていない。
