...
## Shifty バグチェックレポート（2026-08-17 自動実行 #79）

### 修正済み

- **🟡 スタッフ削除だけが確認なしで、最も誤タップしやすいリストから対象を消していた**（[app-admin.js:2314](app-admin.js)）→ `69f62bf`
  `del` は `const a=[...staffList];a.splice(i,1);onSave(a);` の1行で、**確認ダイアログが無かった**。一方 app-admin.js の他の破壊的操作は**4箇所すべて**が対象名か連鎖結果を示す——店舗削除（`:132`）・期間削除（`:1966`・#78で修正）・提出削除（`:3032`）・ポジション削除（`:3618`）。**ここだけが例外だった。**
  **しかも最も誤タップしやすい場所である**。#74 の375px実測で**スタッフタブは44px未満のタップ要素が最多**（43要素中39個）。Premiumの1行は 色/番号/属性/別名/ポジション/編集/削除 が `gap:8` で並び、**削除の隣は「編集」**。押し間違えると対象が**シフト作成の列**（`gridStaff`・:1060）と**ヒートマップ**（`heatData`・:762）から即座に消える。
  **Free はこの導線に構造的に誘導される**——スタッフ20名が上限で `add()` が弾く（:2310）ため、**21人目を入れるには誰かを消すしかない**。#78 の期間削除と**同じ形**。
  修正後: `「田中」を削除しますか？ / 提出済みのシフト2件は削除されません（提出一覧とExcelには残ります）。`（0件なら2行目を省く／空白列は従来どおり確認なし）
  **断定できた根拠**: ①他の破壊的操作4箇所が全て確認を持つ中でここだけが素通り ②**同ファイルの `onRenameStaff`（:180）はスタッフ名をキーに持つ設定マップ7つを漏れなく移し替え、「漏れると属性・ポジション等が黙って初期値に戻る」とコメントまで書いている**＝作者は認識済みで**リネーム側だけが手当てされていた** ③削除ボタンと同じ行に「別名 (2)」「ポジション (3)」と**巻き添えデータの件数が既に表示されている**。
  **意図的に書かなかったこと**: 「この操作は取り消せません」。`del` は `staffList` しか触らず、同名で追加し直せば設定も subs も復帰し**実際に失われるのは並び順だけ**。**過大な警告も食い違いの一種**として扱った。件数は `source!=="grid"` を除外し提出一覧・SmModal と同一式に揃えた。

### 要確認（未修正）

- **🔴（継続・プロセス）このループから実機E2Eが2回続けて実行できない**（環境要因・コード変更なし）
  `preview_start` が **「Dev servers can't be started from unattended sessions」** で拒否され、稼働中サーバーも0件（今回も実行して確認）。Bash でのサーバー起動は規約で禁止のため代替なし。**#72〜#77 の6回が柱にしてきたブラウザ実測が、#78 に続き今回も1件も実行できなかった。**
  **ユーザーが対話セッションで `/bug-check` を手動実行すれば解消する。**
- **🟡 #77 の宿題（Free 上限の実機検証）は静的監査完了・実測のみ未達**（上記🔴が理由・#78から変化なし）。**残るは実機で21人目・2件目を押す1回だけ**
- **🟡（新規）スタッフ削除の後始末が、リネームと正反対**（`del`:2314 / `onRenameStaff`:180）→ **BACKLOG化済み**
  `onRenameStaff` は7つの設定マップを移し替えるのに `del` は**1つも触らない**。①同名で追加し直すと前任者の番号・属性・ポジションを黙って継承する（が誤削除からの復帰手段でもある）②`resolveAlias`（app-utils.js:429）は `staffList` を参照しないため**死んだ別名が生き続ける**。**条件B**
- **🟡（新規）未登録名の提出が、画面に出ずExcel/PDFにだけ出る**→ **BACKLOG化済み**
  グリッド（:1060）とヒートマップ（:762）は未登録名を**表示しない**が、Excel（:2015）と PDF（:1289）は**末尾に列を追加する**。管理者は「その人がいない画面」で組み、**その人の列が入ったExcelを配る**。**条件B**
- **🟢 提出削除の確認が対象名を示さない**（app-admin.js:3032）。詳細モーダル内＝対象が見えている単一削除で連鎖も無いため申し送りのみ
...
- **🟡 スタッフURLを開いた誰でも他人の提出を削除できる**（#70）／**解約イベントだけが metadata でプラン判定**（functions/index.js:319）→ BACKLOG化済み
- 以下は #46〜#78 からの継続（変化なし）: 未claim店舗の取り込み／`findActiveSubscription` の `|| live[0]`／`planExpiry` のUTC計算／片側セルだけの日が0分／`purgeOldPeriods` のUTC日付／`generateInviteCode` のデッドコード／ほか

### 今回の対象と見つけ方

**#78 以降のコミットは0件。7回続けて「差分ゼロ」の回**である。

**(1) 前回の申し送りをそのまま実行した回。** #78 が「次はスタッフ削除（`del`）を見よ」と**対象を1つに絞って**終わっていたため、探索フェーズが実質ゼロで着地した。**申し送りの粒度が次回の収穫量をほぼ決める。**

**(2) #78 の予想は半分外れ、外れ方が収穫だった。** #78 の本命は「設定マップが**残留する**害」だったが、残留は**誤削除からの復帰手段としても働く**ため仕様判断（BACKLOG行き）。代わりに確定したのは**確認ダイアログの不在**という予想外の側。**「残る側の害」を探しに行って「そもそも押せてしまう」を見つけた。**

**(3) 判定基準は8回連続で「答えがコードベースにあるか」。** 決め手は**リネーム側の存在**。同じ作者が同じデータについてリネームでは手当てしデリートでは何もしていない——**片側だけが実装されている非対称は、設計ではなく取りこぼしである。**

**(4) 「Free の制約が強いる行動」レンズが2回連続で当たった。** #78「期間1件だから唯一の期間を消すしかない」→今回「20名上限だから誰かを消すしかない」。**プラン上限は "使えない" だけでなく "破壊的操作を強制する" 形で効く。**

**(5) 修正の「書かなさ」も設計した。** 「取り消せません」は警告として強いが**実際には並び順しか失われない**ので嘘になる。**過大な警告も食い違いの一種**として書かなかった。

**(6) #78 の「配信物を Node で評価する」を関数まるごとに拡張した。** `del` の定義を正規表現で切り出し、`confirm`/`onSave`/`tt` を差し替えた `new Function` に流して**4ケースを実行**。**途中1ケースが落ちた**が原因は実装ではなく**テスト側のスペーサー表現の誤り**（実物は `"__spacer__"+token`）。**ブラウザ無しでも偽陽性を出せる精度はある**と分かった。
...
- 読み込み順維持・SRI **11本**／`database.rules.json` の `".read": true` **0件**（全て `auth != null` 以上の条件付き）
...
- **修正の検証（配信物の実評価）**: 提出3件（うち grid 1件）→「**2件**」／grid のみ→**2行目を省く**／キャンセルで `onSave` もトーストも**発火しない**／空白列は**確認なしで従来どおり削除**、の4ケース
- **呼び出し元の網羅**: `del` の呼び出しは `:2390`（空白列）と `:2416`（実スタッフ）の2箇所のみで、どちらも分岐が効くことを確認
- **未検証（今回は手段が無い）**: ブラウザでの実描画すべて／**Free 上限に達した店舗での実ブロック**／今回追加した confirm の実表示／実機 iOS Safari

### 総括

**測れない回が2回続き、それでも 🟡 が1件出た。** #78 が次の一手を1つに絞って終わっていたためで、**申し送りの粒度が次回の収穫量をほぼ決める。**

**今回の発見は #78 と対になる。** #78 は「押すと**消える**」、今回は「**押せてしまう**」。どちらも Free が構造的に通らされる道で、**プラン上限は機能制限としてだけでなく破壊的操作の強制として効く。**

**主力手段が失われた状態は 🔴 のまま2回目。** **ユーザーが対話セッションで `/bug-check` を1回回せば解消する**ので、その一手を最優先で提案する。

**次回の手を1つに絞る。** 実測が復活したなら**#77 の宿題をそのまま実行する**（実機で21人目・2件目を1回押すだけ）。実測が使えないままなら**同じレンズを3回目まで回す**: 残る候補は**店舗削除**（`:132`）——確認はあるが `「○○」を削除しますか？` だけで、**Authユーザーなら `onUnlinkShop`（紐付け解除）、非Authなら `saveShops` によるローカル削除と、同じボタンが2つの異なる操作に分岐する**（コードで確認済み）。どちらが起きるかを文面が示しておらず、#78・#79 と同じ「確認が結果を語っていない」形である。
