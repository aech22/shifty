...
## Shifty バグチェックレポート（2026-08-27 自動実行 #99）

> 着手時の HEAD は `4b3d446`（#98 の直後）。コード差分は #98 自身の修正1件だけで、静的スキャンで新しく出るものは無い状態。#98 が `shifty-e2e-verify` 1.7節に残した「配信物を起動して書き込みを数える」手順を使い、**これまで一度も実ブラウザで触っていなかったスタッフ提出画面**に入って、そこで1件見つけた。

### 修正済み

- **🟡→修正 名前を入力していないスタッフに、他人の提出の「削除」ボタンが出る**（[app-staff.js:605](app-staff.js)）→ `2c1c9c7`
  提出状況一覧（`SmModal`）は**スタッフ画面と管理者画面の両方から使う共通部品**で、`myName` を渡すかどうかで削除ボタンの範囲を切り替える設計になっている（コード内のコメントが「myName を渡すと『その名前の行だけ』出す（スタッフ画面用）。渡さない＝管理者画面からの利用で全行に出す」と明記）。ところが判定が **`!myName`** だったため、スタッフ画面が渡す `""`（＝まだ名前を入力していない初回訪問者）が**管理者と同じ分岐に落ち、他人の行にも削除ボタンが出ていた**。
  **実測（375px・実ブラウザ・書き込みは `firebaseDB.ref()` をフックして全て横取り＝devには1バイトも出していない）**:

  | 状態 | 修正前 | 修正後 |
  |---|---|---|
  | 名前未入力（初回訪問者） | 削除ボタン **1個**（他人＝田中の行） | **0個** |
  | 他人の名前「佐藤」を入力 | 0個 | 0個 |
  | 本人の名前「田中」を入力 | 1個 | **1個**（非回帰） |
  | 管理者利用（`myName` 省略・sub 2件） | 2個 | **2個**（非回帰） |

  管理者側の非回帰は 1.6節のハーネスで `SmModal` だけを直接マウントして測った。修正は `!myName` → **`myName===null`**。
  **実害の範囲**: Firebaseのルールは sub の削除を「提出した端末（`submitterUid`）か店舗オーナー」だけに許すので、赤の他人の提出は押しても拒否され `b2220fe` の「△ この提出は削除できません」が出る。ただし**店頭の共用タブレットから全員が提出している店舗では submitterUid が全員同じ端末uidになる**ため、そこでは実際に消せてしまう。名前を入れさせるゲートはこの場合にだけ効く安全弁で、初回訪問時に外れていた。

### 実測して確定させたこと

- **スタッフ提出画面をこの一連のバグチェックで初めて実ブラウザで通した**（#95〜#98 は管理者タブとデモURLに寄っていた）。375px で「名前入力 → 全日程『ランチ』一括反映 → 提出」を最後まで通し、**pageerror・console.error ともに0件**、書き込みは **`update shops/{sid}/subs` の1回だけ**（既存subを再利用した日付単位の差分書き込み）。
- **一括反映（通し／ランチ／ディナー）は休業日を正しく除外している**（app-staff.js:129）。
- **デモ（`#/demo`）の書き込み遮断に穴は無い**。`.remove()` は3箇所しかなく（app-main.js:891・:1138・:1560）、2箇所は `!DEMO_MODE` で直接ガード、残る1箇所は `authUser` 必須でデモでは到達しない。企業連携CFは `_callCF` がまとめて弾く（:927）。
- **クライアントの書き込みパス15箇所を全数で洗い出し**、拒否時に黙るものが残っていないか確認した。`revertAdminWrite` を通さない直書きは app-admin.js:3530 のみで、**自前で失敗トーストを出している**＝穴ではない。

### 要確認（未修正）

- **🟢 配信版数 `20260825-92a9e27` が古い。差は 6コミット・3ファイル**（app-admin.js／app-main.js／app-staff.js が `origin/main` と不一致）。**#98 の「3コミット」は数え落としで、正しくは `93c1022`・`b2220fe`・`14194e9`・`4d1fad1`・`f56dabc`・`2c1c9c7` の6件**。次のリリースでバンプが要る（`/release-to-main` 手順63が持つため BACKLOG化はしない）。**今回を含む6件の修正はすべて本番へ出るまで効かない。**
- **🟢 スタッフ画面の提出状況一覧では、他人の提出の「編集」は今も誰でもできる** → **BACKLOG #68 に追記済み**。削除は本人・オーナー限定だが、**セル編集（`handleCellClick`）と名前クリックの「✎ 修正」（`onEditByName`）には myName の絞り込みが無い**。根は #68 と同じ「`subs` の書き込みルールが認証済みなら誰でも編集を許す」で、**案A（編集UIを止める）を選ぶならスタッフ画面まで射程に入る**。
- 変化なし・**BACKLOG化済み**: 片側セルのヒートマップ／スタッフ削除の後始末／別名提出の重複（#81）／二重課金・特商法（🔴）／解約時のプラン判定／企業連携の解除／解約がアプリに伝わらない。

### 検証したこと

- RULES.md: `DEV_MODE` は式のまま／`subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／読み込み順維持／SRI **11本**／今回の差分に input 追加 **0件**・色リテラルの置換 **0件**。
- テーマ: 未定義CSS変数の参照 **0件**／`:root[data-theme="dark"]` と `prefers-color-scheme: dark` の**乖離なし**。
...
- **devのデータは1バイトも変えていない**（全スクリプトで `writes` を出力し0件または想定どおりの1件であることを確認）。
- **未検証**: Excel・PDF の実物確認（**7回連続**・ユーザー操作が要る）／iOS Safari 実機／本番データ。

### 総括

**共通部品を2つの画面で使い回すときは、「渡さない」と「空で渡す」を同じ扱いにしてはいけない、という話だった。** `SmModal` は `myName` の有無で削除ボタンの範囲を切り替え、コメントもそう宣言していたが、`!myName` は「渡していない」と「渡したが空」を区別しない。スタッフ画面は常に渡すので、名前を入力するまでの間だけ管理者と同じ画面になっていた。

**#98 が「実測が読解を追い越した」なら、今回は「道具が使い回せた」。** #98 の最後に書いた煙テストを `4b3d446` で 1.7節に残し、それを最初に使った先が**未踏の画面（スタッフ提出）**で、そこにバグがあった。**残した道具は、残した回ではなく次の回に効く。**

**次回の手**: ①Excel・PDF の実物確認（7回連続・ユーザー操作が要る）②スタッフ画面はまだ `CellEditPanel`（提出状況一覧のセル編集）を通していないので、次はそこを1.7節で測る（今回「編集は誰でもできる」をコードで確認したが、**実際に書き込みが発行されるところまでは見ていない**）。
...
> ルール側でも「提出した端末かオーナー」に限られる。**つまり削除は二重に守られていて、編集だけが素通し**という
> 非対称が、管理者画面とスタッフ画面の両方で同じ形で出ている。
