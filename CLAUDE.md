...
## Shifty バグチェックレポート（2026-09-20 自動実行 #137）

> 着手時の HEAD は `dc824e7`。#136 以降 app-*.js・functions/index.js・database.rules.json は変わっていなかった。
> #136 の申し送り（「値で他のデータを指している箇所」）を当てたが**2件とも既に塞がれていた**ため、
> 同じ「1つの事実を描く surface が複数あり、片方だけが別の判定を持つ」形を別の場所で探し、**🟡を1件見つけて直した**。

### 修正済み

- **[🟡] 提出エントリの無い日が、配布するExcelにだけ「休み」の斜線で出る**（app-admin.js の `expXl`・`630ade7`）
  `expXl` のセル分岐は `sub` の有無しか見ておらず、**sub はあるのにその日の `shifts` エントリが無い**日が
  `else`（休み＝斜線）へ落ちていた。この状態は例外ではなく**常用経路で生まれる**。

  - 管理者がシフト作成グリッドで**未提出スタッフ**のセルに入力すると、`applyEditToSubs` が
    **その日だけ**を持つ sub（`source:"grid"`）を作る＝期間の残り全日がこの状態になり、
    **1日入力しただけで列の大半が「休み希望」として配布Excelに出る**
  - スタッフの提出後に**期間の終了日を延ばす**と、増えた日は提出時の `shifts` に無い
    （`buildShift` は提出した時点の `dates` ぶんしか作らない）

  画面（`holidayCellDash` の `if(!sh)return false`）と PDF（`if(sh&&sh.status==="holiday")`）は
  どちらも空白にしており、**Excel だけが違っていた**。

  | | 修正前 | 修正後 |
  |---|---|---|
  | 田中 09-02（本人提出の休み）／09-03（管理者の `y`） | 斜線 | 斜線（非回帰） |
  | 田中 09-04・09-05（期間の延長で増えた日） | **斜線** | **空白** |
  | 佐藤（グリッドで1日だけ入力）の残り4日 | **斜線** | **空白** |
  | 鈴木（sub 自体が無い） | 空白 | 空白（非回帰） |
  | 佐藤 09-05 の未保存編集「13」 | 13 | 13（非回帰） |

  `!sh` の分岐は work／管理者表示の判定より**後ろ**に置く（前に置くとシフト作成タブの
  `adjResolver` が `localEdits` から解決する未保存の編集まで握り潰す）。
  実測はハーネスに ExcelJS を載せて**生成された .xlsx のバイト列**を読み直し `border.diagonal` を見た
  （Firebase へは1バイトも出していない）。PDF は `capturePdf()` で **70td中2セルだけが斜線**＝
  田中09-02 のみであることを確認。2つのExcel入口（`resolver` あり／なし）の両方で同じ結果。
  回帰スクリプトは `.claude/skills/shifty-e2e-verify/scripts/example-excel-missing-day.js`
  （修正前の配信物へ向けると6項目中3項目が落ちる）。

### 要確認（未修正）

- **[🟢] 属性を削除すると、その属性でタグ付けした休憩が「誰にも当たらない行」として残り、UIから外せない**
  （app-admin.js の `deleteType`）。`staffTypeLimits` と `staffAttributes` は掃除するが
  **`breakTimes[区分][i].tags` は掃除しない**。残ったタグは誰の属性にも一致しないので、その休憩行は
  **一覧には出るのに誰の勤務時間からも引かれない**。タグのトグルボタンは生きている属性しか並べないため
  **外すボタンが存在せず**、チップには内部ID（`custom_xxxxxxxx`）が出る。
  **条件B（仕様判断）**: タグを外すと**その休憩が全員に当たる**ようになり、休憩行ごと消すと**誰も引かれない**まま。
  「属性を消す」がどちらを意味するかはユーザーが決める必要がある。`keepAttrs` は同じ問題を
  `attrIdExists`（#119）で既に塞いでおり、**休憩タグだけが取り残されている**。
- **#136 から継続（変化なし）**: 同じ開始日の期間が2つあると `moveStaffHiddenBoundaries` が境界を動かさない（🟢・意図的）／
...
- **#136 の申し送り①「候補時間を消したときの `dateCandidatePosTypes`」→ 否定**。`delD` が最後の1件を消すときに
  `dateCandidatePosTypes[日付]` も同じ保存で落としている。`positionDayTypeFor` も未知の値を無視する二重の守り。
- **#136 の申し送り②「店舗略称を変えたときのセル値」→ 否定（削除の意図どおり）**。
- **属性削除で `period.keepAttrs` が死んだIDを指す → 否定**（`attrIdExists`・#119 で対処済み）。
- **改名で `keepStaff`／`keepAttrs` が取り残される → 否定**（`renameStaffInPeriods` が写しも含め3つとも移す）。
- **ポジション削除で `requiredPositions`／`staffPositions` に死んだ名前が残る → 否定**（`delPos` のカスケード）。

### 検証したこと

- `npm test` **264件パス**（増減なし。修正箇所は `expXl` 内の分岐でユニットテストの対象外のため、
  代わりに上記E2E回帰スクリプトで担保した）。`npx eslint app-*.js` **0 errors / 95 warnings**（増減なし）。
  `node --check functions/index.js` 通過。
- RULES.md スキャン: `DEV_MODE` は式のまま／`subs` の全体 `set()` 0件／`accounts` 全件読み 0件／SRI 全件付与／
  読み込み順 utils→core→staff→admin→main／フォーム部品 **58件・`fontSize<16` は0件**。
- 配信版数は `20260916-c20a092` のままで、**#134・#136・#137 の app-*.js 修正を指していない**。リリース時にバンプすれば足りる。
- Firebase・Stripe には一切アクセスしていない。

**申し送り（次回の観点）**: 今回効いたのは「**同じ事実を描く surface が3つあり、1つだけが別の判定を持つ**」を探す形。
`expXl`・`buildShiftTableHtml`（PDF）・グリッドは**セルの分岐条件を共有せず各自が書き写している**（#52・#54・#134・#137 は全部ここ）。
次は分岐ではなく**列の集合**を突き合わせるとよい——部門フィルタ（`deptFilter`）を掛けた状態で Excel と PDF が同じ列を出すか、
`showSpacerDate`（35名以上）の境界で両者が揃うかは、まだ誰も測っていない。
