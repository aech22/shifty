...
## Shifty バグチェックレポート（2026-07-07 自動実行 #4）

### 修正済み

- **🟡 doLogout が実ログインセッションを次回リロードで強制サインアウトするバグ**（app-main.js:665、commit dd2a41e）
  `doLogout()` は「店舗セッションのみログアウト（企業連携・Firebase Auth は維持）」設計だが、`doFullSignOut()` と同じく明示ログアウトフラグ `AUTH_LOGGED_OUT_LS` を `true` にしていたため、doLogout後にページをリロードするとGoogle/メール/企業ログインセッションが強制サインアウトされ匿名に戻ってしまっていた。`doFullSignOut()` 側のみでフラグを立てるよう修正。
  発見経緯: セッション開始時点で develop に未コミットの作業ツリー変更（実ログインの永続化を`NONE`→`LOCAL`に変更する企業ログイン関連の修正）があり、作業中に別セッションがそれをcommit 96e5fa0として確定させた。そのコミットに混入していた副作用バグを本レポートで発見・修正。

### 要確認（未修正）

- **🟢 詳細モーダルの「時間」列ヘッダーがnon-Premiumでも常に表示される**（app-admin.js:2056）
  non-Premiumユーザーは全行「-」表示。機能上の問題はないが視覚的に不要な列が表示される軽微なUX問題。継続監視中。

### 前回から改善された点（今回の巡回で確認・過去レポートから解消済み）
- `signInWithApple` / `signInAndLinkApple` デッドコード → 削除済み
- `AI`定数・「コードで追加」入力欄・出退勤調整select・スタッフ属性selectの`fontSize:12/14` → すべて`fontSize:16`に修正済み（iOS Safariズーム対策完了）
- `onJoinByInviteCode` デッドプロップ → 招待コード入力UIがログイン画面に実装され使用されるようになった
...
- `npx eslint app-*.js` 0 errors / 96 warnings（すべてno-unused-vars誤検知）。`npm test` 22件全パス
