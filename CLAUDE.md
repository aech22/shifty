...
## Shifty バグチェックレポート（2026-08-03 自動実行 #51）

### 修正済み

- **[🟡] スタッフが再提出すると管理者の休み希望（y／休）と「締」の追加出勤が黙って消えていた**（app-staff.js:161 `buildShift`／コミット `60bfafd`）
  スタッフが提出フォームから再提出すると、`buildShift` が日ごとのシフトオブジェクトを**作り直す**。そのとき既存提出から引き継ぐ管理者フィールドが `["adjustedStart","adjustedEnd","adjustedStartNote","adjustedEndNote"]` の**4つにハードコード**されていた。このリストは管理者調整値しか無かった頃に書かれたもので、その後に追加された `adminRest`（休み希望 y／休・#50で集計に反映させたばかり）・`extraStart`/`extraEnd`・`adjustedStartFixed`/`adjustedEndFixed`（「締」の追加出勤・2026-07-12追加）・`origStatus` は載っておらず、再提出のたびに失われていた。
  **発火条件**: 提出フォームの状態 `sd` は、端末にCookieがある場合だけ既存提出から復元される（app-staff.js:59-68）。Cookieは**提出時にしか書かれない**（:195）ため、**別端末からの再提出**（PCで出して後からスマホで直す等）・**「リセット」後の再提出**・Cookie削除後の再提出では `sd` が初期値（全日 `{status:"holiday"}`）のままとなり、管理者フィールドが上記4つを除いて全て消える。書き込みはフラットパス `subId/shifts/{日付}` で日オブジェクトごと置換されるため、消えた状態がそのままFirebaseに永続する。
  **実測検証**（実データ形状のNode再現）: 管理者が `y` を入れた日は **0分／0日 → 480分／1日**（休みが勤務8時間に戻る）、管理者が「締」を入れた日は **120分／0.5日 → 0分／0日**（追加出勤が消える）。修正後はどちらも再提出前と完全に一致。
  **修正**: 引き継ぎ対象を app-utils.js の `ADMIN_SHIFT_FIELDS` レジストリ（10フィールド）に集約し、`carryAdminShiftFields()` を経由させた。あわせて、追加出勤フラグを引き継いだ日は `status="work"` も復元する（`applyEditToSubs` と同じ不変条件。**フィールドを引き継ぐだけでは不十分**で、status を戻さないと `extraStart`/`extraEnd` だけが残り `calcNetWorkMinutes`／`shiftBandInfo` の `status!=="work"` 早期returnで0分に落ちることを再現で確認した）。`changed` 判定は status 復元後の最終形で行い、引き継ぎで元と同じ状態に戻った日を「変更あり」と誤表示しないようにした。
  **再発防止**: レジストリの完全性テストを追加（`ADMIN_SHIFT_FIELDS` が管理者フィールド全量と一致することを検証）。新しい管理者フィールドを追加してレジストリに登録し忘れると `npm test` が落ちる。テスト6件追加・**149件 → 155件パス**。

### 今回の対象と見つけ方

**前回（#50）以降のコード変更はゼロ**（新規コミットは `94ffc47` マージと `ec012b8` リリースのみで、いずれも本番反映とキャッシュ版数バンプ）。差分レビューが使えないため #50 と同様に構造から探した。今回の入口は**#50で有効だった「検証可能な主張を書いたコメントを辿る」手法**で、app-staff.js:503 の「提出一覧タブ/Excel出力と同じ照合」を検証している最中に、その近くの `buildShift`（:161）が**引き継ぎフィールドをハードコードした配列**で持っていることに気づいた。「管理者フィールドの一覧」という*増える性質のもの*が1箇所にベタ書きされていれば、後から追加された分が漏れているはず、という筋で `adminRest`・`extraStart` を突き合わせて確定した。

### 検証したこと

- **管理者が日ごとに書き込むフィールドの全数え上げ**: `adjustedStart`/`adjustedEnd`/`adjustedStartNote`/`adjustedEndNote`/`adminRest`/`extraStart`/`extraEnd`/`adjustedStartFixed`/`adjustedEndFixed`/`origStatus` の10個。うち**6個が引き継ぎ漏れ**だった
- **`sd` が既存提出から復元される経路の特定**: Cookie経由（:59-68）のみ。Cookieの書き込みは提出時（:195）の1箇所だけであることを grep で確認（＝別端末では復元されない）
- **同じ「作り直しで落ちる」パターンが他にないか横断確認**: シフトオブジェクトを組み立てる全箇所（app-staff.js:515 `applyCellEdit`／app-admin.js:555 新規sub作成／app-admin.js:2900 `saveAdj`／app-admin.js:573 `applyEditToSubs`）を確認。**いずれも既存オブジェクトをspreadしており安全**。作り直し方式は `buildShift` だけだった
- **`origStatus` を引き継ぐ安全性**: 古い `origStatus` が残っても、復元は `applyEditToSubs`:599 の `!sd.start&&!sd.end` ガードの内側でしか起きないため、スタッフが時間つきで再提出した日を巻き戻すことはない
- **`adminRest` が引き継がれてスタッフの新提出を上書きする件は仕様どおり**: 管理者が「その日は休み」と決めた判断がスタッフの希望より優先されるのが `adminRest` の目的。ただし `carryAdminShiftFields` は `nw[k]==null` のときだけ引き継ぐため、スタッフ側に既に値があるフィールドは上書きしない
- **フラットパス書き込みでの削除の永続性**: `diffSubForFlatWrite`（app-utils.js:549）は日オブジェクトを丸ごと `subId/shifts/{日付}` に書くため、キーの消失がそのままDBに反映されることを確認（＝画面上だけの問題ではない）
...
- `database.rules.json` の `".read": true` **0件**（締めルール反映済みを維持）
- Cloud Functions: secrets 5箇所抜け漏れなし・`.delete()` 誤用 **0件**・`purgeInactiveShops` の Invalid Date スキップと `archived/shops` 二段削除は健在
- 読み込み順 utils→core→staff→admin→main 維持・SRI 11本・今回の差分に `fontSize<16` の新規混入 **0件**
- `npm test` **155件パス**（新規6件）、`npx eslint app-*.js` **0 errors 98 warnings**（修正前と同数）

### 要確認（未修正）

- **🟡（新規・次リリースで必須）index.html のキャッシュ版数を必ずバンプすること**。現在 `20260802-94ffc47`（index.html:211-215）。今回の修正は **app-utils.js と app-staff.js の実挙動を変える**ため、版数を上げずに配信するとブラウザキャッシュで旧ファイルが使われ修正が反映されない。
- **🟢（新規）属性を削除しても休憩のタグ（`breakTimes[].tags`）に残る**（app-admin.js:3392 `deleteType`）。`deleteType` は `staffTypeLimits` と `staffAttributes` はカスケード削除するが `breakTimes` のタグは掃除しない。残ったタグはどのスタッフの属性とも一致しないため、その休憩は**誰にも適用されなくなる**（タグなし休憩の抑制にも効かないので他の休憩は正常）。UIではタグチップが `attrName()` の解決に失敗して空表示になる。実害は限定的だが、ポジション削除（:3497）が同名カスケードを実装しているのと非対称。
- **🟢（新規）Excel出力とシフト作成タブで別名提出の解決順が違う**（app-admin.js:2107）。expXl は `ss.find(s=>登録名一致||別名一致)` で**配列順の先頭**を採る一方、ShiftEditTab の `_getSubForPeriod`（:489）は**完全一致を優先**してから別名を探す。登録名と別名の両方で提出が存在する場合にだけ結果が食い違う。
- **🟢（継続）期間管理タブの Excel 出力だけが管理者調整値を反映しない**（app-admin.js:1952 `expXl` を resolver なしで呼ぶ経路）。「提出そのまま」と「完成シフト」の意図的な使い分けの可能性があり仕様判断が必要なため未修正。
...
- **🟢（継続・期限接近）`PURGE_OLD_PERIODS_DRY_RUN` の本有効化**（functions/index.js:470）。着手予定 **2026-08-12 以降**＝あと9日。
- **🟢（継続）実機E2E未実施**: 「曜日別から選ぶ」UI／#46 削除操作／#47 改名修正。**今回の修正も実機E2Eは未実施**（純粋関数のNode再現で挙動を固定したが、実際に2端末でスタッフ再提出して管理者の y／締 が残ることの画面確認は未実施）。
- **🟢群（継続）**: 変更マークの締切ゲート対象外（app-staff.js:166）／`globalTemplates` の命名／`VISION.md` 不在

### 総括

**2回連続で、コード変更ゼロの回に構造から 🟡 を1件見つけた**。#50 は「フラグの解釈がレイヤーをまたいで配られていなかった」バグで、今回は「**引き継ぐべきフィールドの一覧が1箇所にハードコードされ、後から増えたフィールドが載らなかった**」バグ。どちらも根は同じで、**あとから追加されるものの集合を、増えることを前提にしない書き方で持っている**という一点に尽きる。

今回の教訓は「引き継ぎリストのハードコードは腐る」ことそのものよりも、**壊れ方が非対称だったこと**にある。`adjustedStart`（管理者調整値）は生き残り、`adminRest`（休み希望）と `extraStart`（締）だけが消える。つまり管理者から見ると「シフト表の一部だけが勝手に戻る」という、原因を推測しにくい形で現れる。しかも発火条件がスタッフ側の端末のCookie有無なので、管理者の操作では再現できない。**「たまに戻る」系の報告は、こういう非対称な部分消失を疑うべき**という具体的な手がかりが得られた。

もう一つ、修正の途中で**フィールドを引き継ぐだけでは直らない**ことが再現で分かったのが収穫だった。`extraStart`/`extraEnd` は `status==="work"` とセットで初めて意味を持つのに、`status` はスタッフ提出値なので引き継ぎ対象ではない。**データの一部だけを復元すると、そのデータが依存している不変条件が壊れたまま復元される**。#50 の `getOT` を直さなかった判断（直すと別のバグが出る）と同じで、「同じように見える修正を機械的に当てない」という点は今回も効いた。**引き継ぎ・コピー・マージを実装するときは、フィールド単位ではなく「そのフィールドが成立するのに必要な条件」の単位で考える**必要がある。

なお #50 の申し送りだった 🟡 2件（develop のキャッシュ版数が古い／本番未反映の7コミット）は、`ec012b8` のリリースで**いずれも解消済み**であることを確認した。ただし今回の修正でまた develop に未リリースのコミットが1件積まれたため、キャッシュ版数のバンプを新しい 🟡 として引き継ぐ。
