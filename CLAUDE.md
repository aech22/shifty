...
## Shifty バグチェックレポート（2026-09-07 自動実行 #114）

> 着手時の HEAD は `d5cc886`（#113 以降はドキュメントのコミットだけで、コードは動いていない）。
> 観点は **#113 の申し送りをそのまま実行**した——「**UIが文章で約束していることを集め、
> その約束をコードが守れるか1つずつ当たる**」。confirm・モーダルの注記・トーストから
> 検証可能な約束を集め、配信物の関数を実際に走らせて突き合わせた。

### 修正済み

- **🟡 休憩の適用条件の注記が、6週間前に変わった仕様のまま残っていた**
  （[app-admin.js:3425](app-admin.js)・`621b315`）

  休憩時間設定タブの注記は「休憩は出退勤時間が**実際に休憩時間帯と重なる**スタッフにのみ
  適用されます」と書いてある。しかし `getBreaksFor`（app-utils.js:325）は 2026-08-25〜08-31 の
  決定3以降、**勤務が休憩を丸ごと含む日**にだけ適用し（`ws>=bs` と `we<=be` で弾く）、
  **片側セル（出勤だけ・退勤だけ入力した日）には一切適用しない**。
  「重なる」は「丸ごと含む」より**明確に広い**ので、注記は実装より多くを約束していた。

  実測（配信物の `getBreaksFor` を Node で実行・休憩12:00〜13:00・平日）:

  | 勤務 | 注記の予告 | 実際 |
  |---|---|---|
  | 09:00〜18:00（丸ごと含む） | 適用 | 適用 |
  | **12:30〜20:00（休憩の内側から出勤）** | 適用 | **非適用** |
  | **09:00〜12:30（休憩の途中まで）** | 適用 | **非適用** |
  | **12:00〜20:00（出勤=休憩開始）** | 適用 | **非適用** |
  | **09:00〜13:00（退勤=休憩終了）** | 適用 | **非適用** |
  | **09:00〜(空)（片側）** | 適用? | **非適用** |
  | **(空)〜20:00（片側）** | 適用? | **非適用** |
  | 14:00〜20:00（重ならない・対照） | 非適用 | 非適用 |

  **8ケース中6ケースで食い違う。** しかも外れるのは半日勤務・遅番という飲食店で最も普通の形で、
  合致するのは「終日勤務」だけだった。

  **直したのは注記のほうで、実装は動かしていない**。包含条件は決定3で意図して選んだ仕様
  （2026-08-25 の案C を撤回した結果）であり、実装が正しい。壊れていたのは説明だけ。

  **見つけ方は日付の比較だった**。注記の最終更新は `b5e23c1`（2026-07-10）、包含条件が入ったのは
  `c59707b`（2026-08-25）。**注記のほうが6週間古い**。純勤務時間は属性別の勤務時間上限の判定に
  そのまま入るため、管理者はこの注記を読んで「休憩が引かれたはず」と誤って読む。

  再発防止として、**正本が `getBreaksFor` であること**と「条件を変えるときは同じコミットで
  この文も直す」ことをコメントに残した。

### 要確認（未修正）

- **🟢「残す」を選んだときのポップアップの括弧書きが実態とずれる**（[app-admin.js:2822](app-admin.js)）。
  「シフト表に名前を残す」を選ぶと「（**スタッフ一覧からは消えます**）」と書かれるが、実際には
  その人はスタッフ一覧に**元の位置のまま残り**、取り消し線と「削除済み ／ シフト表に表示中（…） ／
  YYYY/MM/DD を過ぎるとこの一覧から消えます」が付く（`1e2b1cd` の仕様）。
  **すぐ次の画面で本人が見えるので誤解は自己修正される**うえ、「登録スタッフとしては消える」という
  読み方も成り立つため 🟢 に留めた。直すなら「一覧では削除済みとして表示されます」が実態に近い。
- **🟢 プラン上限モーダルの「店舗」文言に到達経路が無い**（[app-admin.js:4954](app-admin.js)）。
  `PLAN_LIMITS` の `shops` は free/pro/premium いずれも `Infinity` で、判定側（:46・:126）も
  `??Infinity` に倒すため `onUpgrade({type:"shops"})` は発火しえない。**表示されないだけで害はない**。
- **🟢 略称バリデーションのコメントが、トースト文言については偽**（[app-admin.js:3763](app-admin.js)）。
  「予約語は CELL_COMMANDS レジストリ駆動。新規コマンド追加時にここを個別更新する必要がない」と
  書いてあるが、**判定はレジストリ駆動でも、エラー文言は `h・k・x・y・休・締` と直書き**なので、
  コマンドを足すなら文言は手で直す必要がある。**現時点の文言はレジストリと一致している**
  （キーは h/k/x/y/締、`isRestCommand` が y/ｙ/休）ので、いま実害は無い。
- **🟢 配信版数が今回と前回の修正を含んでいない**。`index.html` の `?v=` と `app-core.js` の `build:`
  は6箇所とも `20260906-d3006ae` で相互に一致しているが、その後 `084d7cc`（#113 の修正）と
  `621b315`（今回）が app-*.js に入っている。**リリース時に `/release-to-main` の手順63 が
  バンプするので申し送りのみ**（BACKLOG化はしない＝二重管理になる）。
...
**今回 BACKLOG へ起こした項目は無い。** 検出した🟡はその場で閉じ、🟢4件はいずれも
条件A〜D（ユーザーにしか出来ない操作・仕様判断・ユーザーしか持たない情報・1ループで完結しない規模）に
当たらないため、次回ループが自力で拾える。

### 検証したこと

**約束を集めて、1つずつ実際に走らせた。** 合致を確認できたものは次のとおり。

- **タグ付き休憩の割り当て**（:3426）「タグを設定した休憩はその属性のスタッフにのみ。タグなしは、
  タグ付き休憩がない属性のスタッフに」→ 社員(タグ有)=12:00-13:00 のみ／バイト=15:00-15:30 のみ。**一致**
- **祝日の優先**（:3225）「祝日は平日・土日より優先適用されます」→ 候補の解決（app-staff.js:102-110）は
  holSat→キー7、holSun→キー8→7 を**曜日キーより先に**見る。祝日が土曜でも祝日側が勝つ。**一致**
- **必要ポジションの重複追加**（:4274）「同じポジションを複数回追加すると、その人数分が必要になります」→
  `matchPositionSlots(["ホール","ホール"], 2人)` は充足、1人だと `{ホール:1}` の不足。**一致**
- **退勤延長**（:4170）「勤務時間合計に加算され、提出一覧の退勤欄に表示されます」→ `calcNetWorkMinutes` に
  `getOT` が渡り（:3670）、退勤欄に `→HH:MM` が出る。**一致**
- **属性別制限**（:4116）「制限を超えたスタッフは提出一覧で赤くハイライトされます」→ :3617 が
  「1日超過」「週超過」を `#FF4757` で描く。**一致**
- **期間削除**（:2077）「提出済みのシフト◯件も一緒に削除されます。スタッフ用URLも無効になります」→
  `savePeriods`（app-main.js:1155）が `tokens/{urlToken}` を `remove()` し、subs ノードを読んで
  該当 `periodId` を `update()` で null にする。**購読していない古い期間ぶんも消える**（ローカルの
  `subs` ではなくサーバーの全件を見ているため）。**一致**
- **非表示**（:2883）「シフト作成タブ・Excel・PDF から名前が出なくなるだけ」→ `buildPdfCols` は
  絞り込み後の `staffList` を使い、未登録名の判定だけは名簿側（`rosterStaffList`）を渡している。
  #113 が Excel・グリッド・ヒートマップを確認済みなので、**PDF を足して4経路すべて**が揃った。**一致**
- **プラン上限の数字**（:4955-4956）「Free は最大20名」「期間を1件まで」→ `PLAN_LIMITS.free` は
  `{staff:20, periods:1}`。**一致**
- **略称の予約語**（:3769）→ 判定は `CELL_COMMANDS`＋`isRestCommand`＋先頭 `[\d.:]`。文言と**一致**
- **非表示の範囲の約束**（:2860-2897）「これより古い期間は選べません（表示は変わりません）」→
  `isStaffHiddenInPeriod` は `startDate` が範囲に入るかだけで決まり、`from` より古い期間は
  どの選択肢を選んでも判定が変わらない。解除時も同様。**一致**

**RULES.md のスキャン項目は全項目クリア**: `DEV_MODE` は式のまま／`subs` の `set()` 全体上書き **0件**／
`accounts` 全件読み **0件**／読み込み順（utils→core→staff→admin→main）維持／SRI **11本**／
...
**実ブラウザ**: 375px に `CandTab` だけをマウント（`shifty-e2e-verify` 1.6節のハーネス。
`onSave` を差し替えているので Firebase へは1バイトも出ない）。新しい注記が出て「重なる」が
消えたことを確認（pageerror・console.error **0件**）。**同じスクリプトを修正前の配信物へ向けると
「重なる」が出る**ことも確認済み＝素通りする検証ではない。

**未検証**: iOS 実機／PDF の実物／本番データ／Stripe 実データ。

### 測り方の記録

**今回いちばん効いたのは「文とコードの、どちらが新しいか」を git で比べたこと。**
注記の最終更新（`b5e23c1`・2026-07-10）と条件が変わったコミット（`c59707b`・2026-08-25）を
`git log -S` で突き合わせると、**説明が実装より6週間古い**ことが一目で出た。
文言そのものを読んで違和感を探すより速く、判定に主観が入らない。

**「AでもBでも」型の語は、実装より広いことが多い。** 「重なる」「含む」「以降」「〜だけ」は
境界の扱いが必ずどこかに寄っており、**広いほうの語で書かれた説明は、狭い実装を約束してしまう**。
今回のずれもちょうどこの形だった（重なる ⊃ 丸ごと含む）。

**約束は、通った経路の数で強くなる。** 非表示機能の「Excel・PDF から出なくなる」は
#113 が3経路、今回 PDF を足して4経路すべてで確認できた。1つの文が複数の出力を名指ししているときは、
**名指しされた出力の数だけ確認先がある**と読むのが正しい。

**申し送り（次回の観点）**: **同じ観点をコード内のコメントへ向ける。**
今回 🟢 に落とした :3763 のコメント（「新規コマンド追加時にここを個別更新する必要がない」）は、
判定については真だがトースト文言については偽という**半分だけ正しい**記述だった。
#112 も「`X は～済み` と別の行を根拠に書いたコメント」を集めている。
**「〜する必要がない」「〜は済んでいる」「〜だけを見ればよい」と断言しているコメント**を機械的に集め、
その断言が今も成り立つかを1つずつ当たると、同じ形が拾えるはずである。

### 総括

**🔴 ゼロ。🟡 は1件検出し、その場で修正・検証・コミットまで完了した。**
#113 の申し送り（UIの約束を集めて突き合わせる）をそのまま実行したところ、
**11個の約束のうち10個は実装と一致し、1個だけが6週間前の仕様のまま残っていた**。
壊れていたのはコードではなく説明で、**直し方も注記を実装へ合わせるだけ**だった。
ただし説明は管理者が勤務時間を読むときの唯一の手がかりであり、
半日勤務という最も普通の形で外れていたので、無害ではない。
