...
- **労務の要修正に当たる日はセルを塗る**（`CELL_COLOR_LEGEND` の `laborErr`）。塗るのは
  `LABOR_DAY_FIX_KEYS`＝12h超・4h未満・1日の残業が上限超・休憩不足の4つだけで、
  **8h超（B制の残業）と週40h超は塗らない**——要修正ではないので、塗ると直すべき日が埋もれる。
  週・月に帰属する判定（月の残業・目安・年の36協定）は日を特定できないので、
  **パネルに名前が出ていてもセルが塗られないことがある**。理由はセルの `title` に出る。
