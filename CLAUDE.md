...
## Shifty バグチェックレポート（2026-08-13 自動実行 #72）

### 修正済み

- **🟡 ダークで色指定のない文字が既定の黒になり読めなかった**（index.html の `body` / app-staff.js:240）— `3490707`
  `body` が `color` を持たないため、`color` を明示しない要素がブラウザ既定の黒になっていた。ライトでは問題ないが**ダークでは暗い背景に黒文字が乗って沈む**。実測で該当24要素で、**すべて Premium の主機能であるシフト作成タブに集中**していた（「シフト作成」見出し **1.18:1**／ヒートマップの時刻見出し・「日付」・集計表の行見出し **1.44:1**。AA は4.5:1）。`body` に `color:var(--c-text)` を置いて既定をテーマの文字色にした（ライトの `--c-text` は `#1A1A2E` ＝既定の黒とほぼ同じなのでライトの見た目は不変）。**同時に app-staff.js:240（提出完了パネル）に `color:"#1A1A2E"` を明示**——ここだけは背景が固定色 `#FEF0E8` で、body の色を継承すると逆に読めなくなるため。PDF出力は描画コンテナが `background:#fff;color:#000` を自前で持つので影響なし。

### 要確認（未修正）

- **🟡（新規）固定の淡い色の帯にダークで沈む文字色が直書きされている** → **BACKLOG化済み**。閲覧のみバナー本文 `#92400E` on 合成後 `rgb(38,37,39)` = **2.16:1**。DEMOバナー・決済失敗バナーも同型。**inline style では `@media`／`data-theme` を書けない**ためテーマ切り替えには CSS変数の新設が要る。ライトの見た目を保ったまま直す色の選択が**デザイン判断（条件B）**
- **🟡（新規）アクセント塗り＋白文字が 2.85:1 で AA 未達** → **BACKLOG化済み**。白 on `#f87036` = 2.85:1 で、タブバー・主要ボタン・選択状態・バッジの全アクセント塗りが該当。**Phase 3（`7357531`）は「出勤=薄オレンジ地に茶文字(3.88:1)で同化している」ことを理由に塗りへ変えたが、変更後の 2.85:1 は挙げた問題より悪い**（休み側の `--c-danger`+白=4.83:1 は記述どおり改善。両数値は再計算で commit と一致）。ブランド色の扱いに関わるため条件B
- **🟢（新規）`firebase.json` の hosting ignore に削除済みの `ogp.html` が残っている**（firebase.json:11・無害）
- **🟢（新規）OGP画像の「7名提出済み」と表示行数4名が不一致**（ogp.png）。カードが切れている読み方も成立するが、具体的な数字を出すなら合わせたほうが強い
- **🟢 マイページに `ownerReadOnly` が渡っていない**（app-admin.js:208）。#68 から変化なし
- **🟡 Pro/Premium 店舗ではスタッフURLを開いた誰でも他人の提出を削除できる**（#70）→ BACKLOG化済み・仕様判断待ち
- **🟡 解約イベントだけが metadata でプランを判定している**（functions/index.js:319）→ BACKLOG化済み（#68起票）。Stripe実データが要るため状態変化なし
- 以下は #46〜#71 からの継続（状態変化なし）: 未claim店舗を誰でも自分の企業に取り込める根／`findActiveSubscription` の `|| live[0]`／`subscription_schedule.*` の4分岐が未購読／`planExpiry` のUTC基準計算／決済失敗フラグの誤解除／片側セルだけの日が0分／`purgeOldPeriods` のUTC日付／`getBreaksFor` の休憩判定／`generateInviteCode` のデッドコード／`urlLocked` が StaffView で未使用／`staffSectionOn` と `isCountExcluded`／SubsTab「月計」の別名重複／`sub.shopId` が死にフィールド／店舗削除が `global/shops/{id}` を消さない／`companyCodes` の TOCTOU／略称の先勝ち／期間作成の日付検証なし／提出状況ビューのセル編集／Excel analytics の数え方／スタッフ数バーのスペーサー／`breakTimes[].tags` の残留／別名解決順の不一致／`heatHours` が `adminRest` を無視／スタッフ削除時のキー残留／`onSub` のローカルstate更新キーの不一致
  （`--c-danger` の未使用は Phase 3 がスタッフ画面の休み選択に使ったため解消）

### 今回の対象と見つけ方

**#71 以降に8コミット入り、5つがデザイン是正 Phase 1〜5 の一括色置換だった**（Phase2 `42684ef`・Phase3 `7357531`・Phase5 `8fb47b8`・Phase4 `405bbeb`・Auto-commit `f3a83cb`/`f4a7276`、加えて `e785d22` ログアウト導線の一本化と release 2件）。**#71 が確立した4検査をそのまま適用**し、そのうえで #71 が指名した宿題を実行した。

**(1) #71 の4検査 — 全部空振り（＝コードは健全）**: 未定義のCSS変数参照 **0件**／ダーク定義の二重管理は同一11変数で**乖離なし**／「色だけ」と称するコミットへの**ロジック混入0件**（色トークンを `§` に正規化した差分の残差416行は角丸の統一・fontSize・border・コメントのみ）／SVG属性内 `var()` は `app-staff.js:9` の1件で増えていない。

**(2) #71 唯一の🟡（版数）は release コミットで閉じた**: `?v=`5箇所と `build:` が `20260813-e785d22` で相互一致し、**この版数が app-*.js の最終変更コミットを指している**ことを確認（`fe985fe` は版数のみの変更）。

**(3) #71 の宿題「4箇所に複製された `s.source!=="grid"` に漏れがないか」→ 漏れなし・閉じた**。#71 が候補に挙げた3箇所はいずれも**除外が不要**と確定: プラン上限は `staffList` を数えており subs を見ない（:2285）／Excel・PDF は grid 由来を**含めるのが仕様**（:2004・:1288）／StaffTab の「未登録の提出名」（:2282）は、grid が sub を作るときの `name` が**グリッド行名＝`staffList` か既存の未登録提出名に限られる**（:549-575）ため新しい未登録名を持ち込めない。フィルタが無いのは `ph("excel_exported")`（:2002）だけで、これは既知の🟢と同一。

**(4) 新しい検査を1つ足した — コントラストの全DOM実測。ここで唯一の実害が出た**。一括色置換で本当に怖いのは「変数が解決されるか」ではなく「**解決された結果が読めるか**」で、#71 の4検査はそこを見ていなかった。`getComputedStyle` から前景色と実効背景色（祖先を辿って透明を合成）を取り WCAG比を計算する検査を書き、管理者8タブ＋候補の内部3タブを巡回して実測。ダーク118件・ライト59件が閾値未満で、うち**24件は「色指定がなく既定の黒」＝判断の要らない実装バグ**だったので修正した。

**修正の副作用は直す前に測った**: `body` の色は全要素に影響するため、先に「既定の黒に依存している要素」を抽出して修正後の値を計算し**改善24件・悪化0件**を確認。実行時に描画されない要素を取りこぼさないよう「**固定の明るい背景を持ち、かつ `color` を持たない** style オブジェクト」を静的に全走査し、危険な形が app-staff.js:240 の**1件だけ**であることを確定させてから同じコミットで塞いだ。

**(5) 仮説を否定したつもりが、否定自体が誤りだった（同日中に再測定して修正・`7f58443`）**: `<tr>` への inset box-shadow が描画されないのでは、という仮説を「スクリーンショットのバイト比較」で否定したが、**この測り方が間違っていた**。td の border で行の高さが1px変わると画像は必ず不一致になるため、**非描画を描画と誤判定していた**。PNG をデコードして `#FF4757` のピクセル数を直接数え直した結果:

| 変種 | Chromium | WebKit |
|---|---|---|
| `tr` に inset（td に border あり＝当時の実装） | 72px | **0px** |
| `tr` に inset（td に border なし） | 70px | **0px** |
| `td:first-child` に inset（修正後） | 70px | 68px |
| `tr` に背景（旧方式・対照） | ティントは両方描画 | 同左 |

**WebKit は `<tr>` の box-shadow を一切描画しない**（border-collapse も隣接セルの border も無関係。決め手は tr か td か）。つまり **Safari / iOS Safari では超過行の目印が消えていた**。バッジ文字は daily/weekly しか出さないため、隔週・月・カスタム期間の超過は Safari で**画面上に手がかりが皆無**だった。`7f58443` で先頭の td へ移して解消（Chromium 側の見た目は不変）。**教訓: バイト比較は描画の証拠にならない。色のピクセルを数えること**

／`e785d22` で `logout` prop が死んだのでは → app-admin.js:102 でフォールバックとして生存／Phase 2 が消した「空要素4箇所」は本当に空か → **生バイトで `><` を確認**、しかも5ファイル分割時点から空／SmModal のセル要素削除で行の高さがズレるのでは → 両コンテナとも `height:72` 固定／Phase 5 で利用規約の条文が欠けたのでは → `TERMS_TEXT` と `terms.html` の**全20条の見出しが完全一致**。

### 検証したこと

- `DEV_MODE`（app-core.js:12・式のまま）／`DEV_PLAN_OVERRIDE`（:125）正常
- `subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／CF `.delete()` 誤用 **0件**
- 読み込み順維持・SRI **11本**・版数6箇所が相互一致**かつ最終の app-*.js 変更に追随**
- `npm test` **168件パス**／`npx eslint app-*.js` **0 errors 98 warnings**／`node --check functions/index.js` OK（修正後も同じ）
- **実機実測（Chromium・標準テスト店舗・`?plan=premium`）**: ダークのコントラスト違反 **118→94件**、既定の黒に依存する要素 **24→0件**、シフト作成タブのダークのスクリーンショットで時刻見出し・集計行見出しが読めることを確認、ライト/ダークともコンソールエラー **0件**。**WebKit 26.5** で `<tr>` の inset box-shadow の描画を確認。OGP画像を目視（1200×630・日本語グリフ欠落なし）
- **未検証**: 提出完了パネルのダーク実機（完了画面の復元経路に到達できず。ただし inline style は継承に**構造上**負けないため `#1A1A2E` on `#FEF0E8` = 15.3:1 は計算で確定）／ライトの59件は既定の黒が絡まないため今回の対象外／🟡の実機E2E（スタッフURLでの他人の提出削除）は今回も未実施

### 総括

**#71 の検査は正しく働き、正しく働いたからこそ足りない一段が見えた。** 4検査は今回も全部空振りで、5コミット分の一括色置換が健全だと機械的に示せた。だが**それらはすべて「指定が壊れていないか」の検査で、「指定した結果が読めるか」は誰も見ていなかった**。実際の欠陥はそこにあり、Premium の主機能でダークの文字が **1.18:1** ＝事実上不可視だった。

**この欠陥はデザイン是正が作ったのではなく、デザイン是正が見落としたものだった。** `body` に `color` が無いのは5ファイル分割より前からで、Phase 1 は「テーマ非追随の是正」を名目にしながら最大の非追随（画面全体の既定文字色）を通り過ぎていた。**個別要素を1つずつ直す作業では「どの要素にも書かれていないから壊れている」種類の欠陥に届かない。**

**"0件破壊" を主張できるのは、破壊されうる集合を先に列挙したからで、テストが通ったからではない。** `body` の色変更は影響範囲が全要素なので、直してから確かめる順番では壊した箇所に気づけない。抽出→計算→静的走査で補完、の順を守った。

**Phase 3 の commit メッセージに事実の誤りを1件見つけた。** 「3.88:1 で同化していたので塗りにした」の変更後が **2.85:1** ——挙げた問題より悪い。**数値を挙げて改善を主張する記述ほど、その数値の再計算まで含めて読む必要がある**（休み側の 4.83:1 は正しかった。だからこそ片方だけ検算せずに通してしまう）。#71 の「正しい記述が古くなる」の変種で、今回は**同じ commit 内に正しい数値と誤った数値が並んでいた**。

**次回の手を1つに絞る。** 収穫は「**指定ではなく結果を測る**」検査を増やせたことなので、同じ発想を広げる。**次はモバイル幅（375px）での「はみ出し・重なり」を実測する**——`scrollWidth > clientWidth` の要素と `getBoundingClientRect()` が親を越える要素を全DOMから列挙する。理由は3つ: ①VISION の設計原則4がモバイルファーストなのに今回の巡回は全部1400px幅 ②Phase 2〜4 は padding・角丸・fontSize・`marginLeft:"auto"` を広範に触っており**横幅の破綻はコンソールに何も出ない**（コントラストと同じ沈黙する欠陥） ③タブ列の `flexWrap` と Phase 3 が入れた `marginLeft:"auto"` は折り返し時に意図しない空白を作りうる。**コードと実機だけで閉じられ、ユーザー判断を要しない。**
