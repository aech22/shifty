...
## Shifty バグチェックレポート（2026-09-18 自動実行 #134）

> 着手時の HEAD は `b846019`。**#133 以降 app-*.js は1バイトも変わっていない**（#133 の2コミットが最後）。
> そこで #133 の申し送り「**同じレジストリを読む複数の箇所が、同じ照合規則で読んでいるか**」を、
> 名指しされた `CELL_COLOR_LEGEND` から当てた。**照合規則ではなく「読む箇所の数」がずれていた**
> ——レジストリの色を描く出力は画面・PDF・Excel の3つあり、**Excel だけが1色を持っていなかった**。
> そこから「Excel を出す入口が2つある」ほうへ辿って、**2つの入口が別物のシフト表を出す**のを見つけた。
> どちらも配布物（スタッフが受け取るファイル）が黙って壊れる形なので🟡として修正した。

### 修正済み

- **[🟡] 変更マーク（緑）が、配布するExcelにだけ乗らない**（app-admin.js:2231／`7e20591`）

  `CELL_COLOR_LEGEND` の `changed`（セルをトリプルクリックで付ける「確定後に変更した
  シフトの目印」）を描く出力は3つある。画面（`cellBgFor`）は changed を**最優先・無条件**で塗り、
  PDF（`chgBg`）も同じ `#B7EBC6` を乗せる——`984dc54` は「PDF出力で空白セルの変更マークが
  落ちていた」をわざわざ直しており、**PDFは2度手当てされている**。一方 `expXl` には
  `changed` という文字列が1つも無く、意図的に外したと書いたコメントも無い。

  **配布されるのはたいてい Excel** なので、目印を付けた本人の画面には緑が出ているのに、
  スタッフが受け取るファイルからだけ無言で消える。エラーもトーストも出ない。

  **実測**（実ブラウザで配信物の `expXl` を実行し、生成された .xlsx を ExcelJS で読み返した。
  `app-main.js` を読み込まないので **Firebase へは1バイトも出していない**）:

  | セル | 修正前 | 修正後 |
  |---|---|---|
  | 7/1 出勤・変更マークあり | `pattern:"none"` | solid **FFB7EBC6** |
  | 7/2 休み・変更マークあり | `pattern:"none"` | solid **FFB7EBC6** |
  | 7/3 変更マークなし（対照） | `pattern:"none"` | `pattern:"none"` |

  修正前は**マークした日とマークしていない日の塗りが1バイトも違わない**＝素通りするテストではない。
  非回帰も同じ手順で実測した: note のみ=**FFFFFF00**（黄・従来どおり）／note+変更マーク=**FFB7EBC6**
  （画面・PDF と同じく changed が note に勝つ）／土曜=**FFDDEEFF**（従来どおり）。
  出勤セルだけでなく休みセル（斜線）にも塗る——PDF が出勤・休み・空白のすべてに乗せているのに合わせた。

- **[🟡] 期間管理タブのExcelが、管理者の調整値を捨ててスタッフの提出値のまま出力する**
  （app-admin.js:2305／`c58eb12`）

  `expXl` は調整済み値の解決を呼び出し元の `resolver` に任せ、**渡されなければ `sh.start`/`sh.end` を
  そのまま印字する**。`resolver` を渡すのはシフト作成タブの「Excel出力」だけなので、
  **期間管理タブの「Excel」ボタンから出したファイルだけが、管理者調整値・休み希望(y)・「締」を
  すべて落とした提出値そのまま**になる。

  **コード自身が、この失敗を名指しで禁じていた**。期間管理タブのボタンの直上（app-admin.js:2115）には
  「ここだけ現在値のまま出すと、**同じ期間のExcelが出す場所によって中身が変わる**」と書いてあり、
  `1c5272b` は名簿と設定についてはそのとおりに揃えた。**セルの中身だけが揃っていなかった。**

  **実測**（同じ subs を2つの入口から出力し、生成された .xlsx を読み返した。Firebaseへは出ていない）:

  | 7/1 出勤を 9:00→11:00 へ調整した日 | シフト作成タブ | 期間管理タブ |
  |---|---|---|
  | 修正前 | 11 | **9** |
  | 修正後 | 11 | 11 |

  | 7/2 出勤フィールドを休み希望(y)にした日 | シフト作成タブ | 期間管理タブ |
  |---|---|---|
  | 修正前 | 空欄＋斜線 | **9・斜線なし** |
  | 修正後 | 空欄＋斜線 | 空欄＋斜線 |

  調整していない日（対照）は修正前後・両入口とも 9/17 で変化なし。
  修正は `expXl` 内に既定の解決 `storedRv` を置き、`resolver` 未指定のときそれを使う形にした
  （シフト作成タブの `adjResolver` の「保存値」分岐＝`getStoredTime`/`getStoredNote`/`getStoredFixed` と同じ規則）。
  **入口を増やしても既定で揃うよう、呼び出し元ではなく `expXl` 側に入れた。**
  未保存の `localEdits` はシフト作成タブしか持たないので、その差だけは resolver 側に残る。

### 要確認（未修正）

- **[🟢] `dup`（店舗間シフト重複・赤）と `posErr`（ポジション不足・黄）は画面にしか出ない**
  `CELL_COLOR_LEGEND` の5件のうち、PDF は `changed` と `rest` だけ、Excel は `changed`（今回追加）・
  `note`・`rest` だけを描く。**どちらも作業中のエラー表示**なので配布物に出さないのは妥当に見えるが、
  PDFのコメント（app-admin.js:1515）は「画面と同じ優先順位」とだけ書いて赤を省いた理由に触れていない。
  **直すべきものではなく、書き足すべきもの**（次に読む人が「取りこぼし」と読める）。
- **[🟢] 部門別PDF（キッチン／ホール）だけ、未登録名の提出者が列ごと落ちる**（app-admin.js:1400）
  `buildPdfCols("kit")`/`("hall")` は `realStaff` を絞るだけで `unreg` を足さない。全体PDF・Excel は足す。
  部門別は在籍者の名簿を出すものなので意図的に見えるが、こちらもコメントが無い。
- **[🟢] スタッフが休みで提出した日に管理者のメモが残っていると、画面で斜線と黄色が両方乗る**
  斜線の判定（`holidayCellDash`＝adminRest **または** status==="holiday"）と、黄色を抑制する判定
  （`fieldRest`＝adminRest **のみ**）が食い違う。2026-08-25 の決定（再提出で休みにしてもメモは残す）で
  到達しうるようになった。**どちらが正しいかは見た目の好みの問題**で、実害は無い。
- **#133 から継続（変化なし）**: 「締」が有効でない店舗でもメモから「締」だけが無言で消える（🟢）／
  既に登録されている「締」入りの略称は今回の修正では直らない（🟢・本番データ未確認）。
- **#132・#131 から継続（変化なし）**: 店舗切替で `plan` だけが同期リセットされない（🟢）／
...
- **変化なし（BACKLOG化済み）**: Cloud Functions の本番反映（🟡・3件たまったまま）／二重課金の根治／
  特商法表記（🔴）／解約通知／解約時のプラン判定／`verifyShopOwner` の移行猶予／
...
いずれも「**同じレジストリを、違う照合規則で読んでいる箇所が無いか**」を狙った。

1. **「他店舗略称の突き合わせが、登録時と参照時で大文字小文字の扱いが違う」→ 否定**。
   `addAbbr` の重複チェック（`cur.includes(v)`・`abbrsOf(s.id).includes(v)`）も `abbrToShop` の
   lookup も**どちらも完全一致・大文字小文字を区別する**。`isReservedShopAbbr` だけが
   `toLowerCase()` するが、それは**コマンド（h/k/x）との衝突判定専用**で正しい向き。
2. **「`staffAliases` を読む5つの関数で trim の有無が食い違う」→ 実害なしと判定**。
   確かに `resolveAlias`・`aliasOwnerOf`・`buildSuggestList` は**両側を trim** し、
   `resolveSubByAlias`・`isUnregisteredSubName` は**生のまま**比較する。しかし別名を登録する入口は
   `addAlias` 1つだけで、**渡せるのは実在する `sub.staffName` そのもの**（未登録名の一覧から選ぶUIで、
   自由入力欄が無い）。生同士の比較は必ず一致するので、食い違いは表面化しない。
3. **「必要ポジションと休憩で、旧4区分 "hol" の流用条件がずれている」→ 否定**。
   `getBreakList`（app-utils.js:212）と `requiredPositionsFor`（:462）は**どちらも
   `positionDayTypeFor` で区分を解決し、`holSat`/`holSun` で中身が空のときだけ `hol` を流用する**。
   #120 で揃えた状態が保たれている。
4. **「クライアント由来の shopId／companyId を検証せずパスへ入れる入口がまだ残っている」→ 否定**。
   `isValidShopId` は課金系4本・`createCompany`・`linkStoreToCompany`・`claimCompanyShop`・
   `unlinkStoreFromCompany` の**8箇所すべて**、`isValidCompanyId` は企業系**5箇所すべて**で
   呼ばれている（#125・#126・#132・#133 の積み残しなし）。

### 検証したこと

- `npm test` **262件パス**（増減なし）・`npx eslint app-*.js` **0 errors / 95 warnings**（増減なし）・
  `node --check functions/index.js` OK。
- **今回の2件はユニットテストで固定できない**（塗りとセル値は `expXl` の内部で、純粋関数に切り出せない）。
  代わりに**生成された .xlsx を毎回読み返す実測**を証拠にした。修正前の版で走らせると
  両方とも実際に食い違う値を返すことを確認済み＝素通りする検証ではない。
...
- **配信版数は #133 から引き続き遅れている**: `?v=` 5箇所と `build:` 1箇所は `20260916-c20a092` のままで、
  今日の app-admin.js 変更（`7e20591`・`c58eb12`）を指していない。**リリース時にバンプすれば足りる**
  （`/release-to-main` の標準工程。BACKLOG化はしない）。
- **未検証**: 今回の2件は**アプリのUIからボタンを押して確かめてはいない**。`expXl` を直接呼んで
  出力ファイルを読み返す形で測っており、期間管理タブ・シフト作成タブのボタンが
  `expXl` に渡す引数そのものは、コードを読んで確認しただけ（app-admin.js:2117 と :1698）。
- **Firebase・Stripe には一切アクセスしていない。** 上の実測はすべて配信物を実ブラウザに読み込んで
  評価したもので、`app-main.js` を読み込まないため `firebaseDB` は `null` のままである。

**申し送り（次回の観点）**: 今回の2件はどちらも「**画面で見えているものが、配布物になると変わる**」形だった。
画面・PDF・Excel の3つは同じデータから3通りに描くのに、**3つ揃っているかを確かめる仕組みが無い**
（レジストリの完全性テストは「レジストリに登録されているか」しか見ておらず、
「**登録された項目を全部の出力が描いているか**」は誰も見ていない）。
**次回は「画面に出る情報のうち、Excel・PDFに出ないものを全部並べる」と良い**——
今回わかっているだけでも `dup`・`posErr` が画面限定、未登録名の列が部門別PDF限定で欠ける。
あわせて **PDF側の入口が1つしかない**ことにも注意したい（Excelは2つあり、それが今回の2件目の温床だった）。
`buildShiftTableHtml` は `ShiftEditTab` のクロージャの中にあるので**外から呼べず機械的に測れない**
——ここだけ実測の手段が無いまま残っている（BACKLOG「PDFの実物確認」が閉じないのも同じ理由）。
