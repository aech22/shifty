...
## Shifty バグチェックレポート（2026-08-24 自動実行 #93）

### 修正済み

- **🟡 選択中の期間が他端末で削除されると「保存」が消えた期間の日付を別の期間へ書き込む**（[app-admin.js:443](app-admin.js)）→ `f3669d1`
  `ShiftEditTab` の `localEdits`/`heatEdits` は **`名前|日付|フィールド` キーのバッファで、期間を持たない**。したがって `selPid` が変わるときは必ずクリアしなければならず、**期間ドロップダウン（:1579）と店舗切替（:388）はどちらもクリアしている**。ところが「選択中の期間が `periods` から消えたら `periods[0]` へ移す」補正（:443）**だけがクリアしていない**。
  そこへ「保存」を押すと `handleSaveAll` が `localEdits` の全エントリを**現在の `selPid`**（＝移った先の期間）に対して `applyEditToSubs` で再適用するため、**消えた期間の日付が別の期間のsubへ書き込まれる**。グリッドは期間内の日付しか描かないので**画面には出ない**が、提出一覧の詳細モーダルは `Object.keys(det.shifts)` を列挙する（app-admin.js:3192）ので、**出勤日数と日付表に幽霊の1日が増える**（`start`/`end` を持たない `status:"work"` として入るため、勤務時間は0分のまま出勤日だけが+1される）。
  **到達経路は「他端末・他セッションが、こちらがシフト作成タブを開いたまま期間を削除する」**。期間タブでの削除はタブ切替で `ShiftEditTab` がアンマウントされる（app-admin.js:212 は条件レンダー）ため、**単独端末では起きない**。企業アカウント・複数オーナー端末を前提にしているので経路自体は現実にある。
  **実測（配信物の `ShiftEditTab` を実ブラウザで単体マウント。`onSave` をローカルのスパイに差し替えてありFirebaseへは1バイトも書かない）**: 期間A(7/1-7/15)の 7/2 出勤セルを 9→11 に編集（`subA` に `adjustedStart:"11:00"` が正しく入る）→ `periods` から期間Aを外す → `selPid` が **pB(8/1-8/15) へ自動補正** → 保存を押すと **「✓ 1件のシフトを保存しました」**と出て、**期間Bのsubに `2026-07-02: {status:"work", adjustedStart:"11:00"}` が入った**（期間Bの範囲外の日付）。**修正後は同じ手順で「変更はありません」となり、期間Bのsubは `2026-08-03` のみのまま。** いずれも pageerror・console error 0件。
  **修正**: 補正の中で `setLocalEdits({})`／`setHeatEdits({})`／`setFocusKey(null)` を呼ぶ（既存2箇所と同じ形）。**捨ててよいことの根拠**: セルの値は blur 時点で `handleBlur` が既に subs へ適用済みで、`localEdits` は表示と「押せば確実に保存される」導線のための冗長バッファでしかない。しかも消えた期間の subs は `savePeriods`（app-main.js:1147-1157）が連鎖削除しているので、残しても書き戻す先が無い。
  **非回帰（実測）**: ①期間Bで2セル編集→保存が「✓ 2件」で両方反映（未提出スタッフ「佐藤」には `source:"grid"` のsubが**期間内の日付で**正しく新規作成される）②ドロップダウンで期間Aへ切替→保存が従来どおり「変更はありません」③期間Aで1セル編集→保存が「✓ 1件」で `subA` に反映。

### 要確認（未修正）

- **🟢 Excel には変更マーク（緑）が無い**（`expXl`・[app-admin.js:2139-2141](app-admin.js)）。画面（`cellBgFor`:1194）と PDF（`984dc54` で修正済み）は `shift.changed` を緑で塗るが、Excel は `fSat`/`fHol`/`fYel`（土曜・祝日・note の黄）だけを持ち **`changed` を一切参照しない**（`grep changed app-admin.js` が `expXl` の範囲に1件もヒットしない）。ただし Excel は名前色・罫線・斜線という別の表現体系を持つ配布物で、「画面と同じ塗りを持つべき」と決まっているわけではない。**仕様の範囲の選択であって実装の食い違いとは言い切れない**ため、BACKLOG化はせず申し送りにとどめる。
- **🟢 休憩タグの編集が index で対象を持っている**（[app-admin.js:2606](app-admin.js) `editTagKey` = `"dayType_index"`）。#92 から変化なし。今回直した件と同じ「対象がズレる」形だが、休憩リストを変えるのも編集するのも同じパネル内の同じ操作者で、他端末の変更で並びが動く余地が実質ない。
- **🟢 `selDates` を render 中に破壊的ソートしている**（[app-admin.js:2834](app-admin.js) `selDates.sort().map(...)`）。#92 から変化なし。
- **🟢 スタッフ用の `PLAN_LIMITS` フォールバックだけが Free の実値と食い違う**（[app-admin.js:2362](app-admin.js) `PLAN_LIMITS[plan]?.staff??10`。`PLAN_LIMITS.free.staff` は **20**）。#91・#92 から変化なし。`plan` state の初期値が `"free"` なので到達経路が現存せず、**「原因だった証拠」を示せない**ため今回も触っていない。
- **🟢 CFの `linkStoreToCompany` が shopId の文字種を検証していない**（#90 から継続・変化なし）。**修正には CF のデプロイ（ユーザー操作）が要る**ため申し送り。
- **🟢 `name|date` 複合キーの分解が2箇所で食い違う**（[app-admin.js:1647](app-admin.js) `indexOf` / [:3078](app-admin.js) `lastIndexOf`）。#90 から継続・変化なし。
- **🟢 `effShiftStart`/`effShiftEnd` の未採用が app-admin.js になお3箇所**（`:1037` / `:3126` / `:3183`）。**#82〜#92 と同じ理由で今回も触っていない**——直し方が BACKLOG の判断待ち項目（片側セルだけの日を何時間と数えるか）に依存する。
- **🟢 配信版数（`?v=` と `build:`）は6箇所とも `20260824-95990ec` で一致しているが、今回 app-admin.js が変わった**ため次のリリースでバンプが要る。申し送りのみ（`/release-to-main` の手順63が標準工程として持つ。BACKLOG化はしない）。
- 以下は変化なし・すべて BACKLOG化済み: **コントラスト5件**（#72・#75・#76・#77・#91でベースライン取得済み）／**タップ領域44px未満**（#74）／**休憩の適用条件が画面の説明文と食い違う**（#89）／**期間の確定（写し）が `periods` を肥大化させる**（#88）／**期間どうしの日付の重なりと、終了日<開始日・日付の空欄の検証漏れ**（#83・#92）／**別名で提出した人の再提出でsubが2つできる**（#81・コード修正済み・Excel実物の確認と本番データ確認が残）／**スタッフURLを開いた誰でも他人の提出を削除できる**（#70）／**スタッフ削除の後始末がリネームと正反対**・**未登録名が画面に出ずExcel/PDFにだけ出る**（#79）／**`x` セルが店舗間重複の判定から除外されていない**（#85）／**店舗間重複の判定が退勤延長と片側セルを見ていない**（#86）／**解約イベントだけが metadata でプラン判定**（#68）／**企業連携の解除が店舗をオーナー0人に戻す**（#65）／**Pro→Premium の二重課金の根治**・**特商法表記**（BACKLOG 🔴）
- 以下は #46〜#92 からの継続（変化なし）: 未claim店舗の取り込み／`findActiveSubscription` の `|| live[0]`／`planExpiry` のUTC計算／`purgeOldPeriods` のUTC日付／`generateInviteCode` のデッドコード／ほか

### 今回の対象と見つけ方

**#92 が最後に名指ししたレンズ（「操作の開始時に控えた値と、確定時に読む値が別物になっている場所」）をそのまま引き継いだら、名指しされていた当の場所——`localEdits`/`heatEdits`——から出た。** ただし**ズレの軸は #92 の予想と違った**。#92 は「キーが `name|date|field` で**名前**を含むため同種のズレが起きうる」と書いていたが、実際に穴が開いていたのは名前ではなく**期間**の側だった。名前の軸は既に塞がっている（店舗切替 :388 が両バッファをクリアし、コメントに「同名スタッフ（複数店舗在籍者）の別店舗データへ誤って書き込まれる」と理由まで書いてある）。**予想された軸が塞がっていたので同じ場所を諦めかけたが、「このバッファはどのスコープに属するのか」を列挙し直したら、属するスコープが2つ（店舗と期間）あって片方しか守られていなかった。**

**(1) 探し方は「setterの数を数える」だった。** `selPid` を書き換える箇所を全数（`grep -n 'setSelPid' app-admin.js`）出すと3箇所しかない——初期化(:361)・ドロップダウン(:1579)・periods補正(:449)。このうち**ドロップダウンだけが `setLocalEdits({});setHeatEdits({});setFocusKey(null);` を伴っている**。同じ3点セットは `shopId` 変更のuseEffect(:388)にもある。**「同じ後始末が2箇所に書いてあって、3箇所目に無い」という形は、コードを読まなくても grep の出力の形だけで見える。** 今回の発見コストのほとんどはここに集約されていて、残りは「3箇所目に到達する経路が現実にあるか」の確認だった。

**(2) 「単独端末では起きない」ことの確認に時間をかけた。** 期間の削除UIは期間タブにあり、タブは条件レンダー（app-admin.js:185-215）なのでタブを移った時点で `ShiftEditTab` はアンマウントされ、state ごと消える。`purgeOldPeriods` は dry-run。したがって残るのは他端末・他セッションだけ、と確定させてから修正した。**「経路が無い」で終わらせず「経路はこれ1本」まで詰めると、修正の重みも報告の書き方も決まる。**

**(3) 「画面に出ない」ことが、この件を一番見えにくくしていた。** 書き込まれた日付は移った先の期間の範囲外なので、**シフト作成グリッドには絶対に描画されない**（`dates.map` が期間の日付しか回さない）。しかも保存トーストは「✓ 1件のシフトを保存しました」と成功を報告する。気づける場所は提出一覧の詳細モーダルだけで、そこでも「出勤が1日多い」という形でしか出ない。**成功を報告しながら見えない場所を汚す操作は、ユーザーからのバグ報告が原理的に上がってこない。**

**(4) 検証は #92 の「部品だけ実ブラウザでマウントする」をそのまま再利用した。** `route.fulfill` で配信物の app-utils/core/staff/admin を供給し（**app-main.js は読み込まない**ので `firebaseDB` は `null` のまま＝Firebaseへの接続自体が発生しない）、自前の親コンポーネントから `ShiftEditTab` を直接 render して `onSave`/`savePeriods` をスパイに差し替えた。**「他端末が期間を削除する」という本来なら2端末が要る状況を、親の state から `periods` を1行で書き換えるだけで再現できた。** グリッドのセルは `data-sc="日付|start"` / `data-scn="名前"` を持っているので、DOMの構造に依存せず狙ったセルを直接掴める（この属性は Enter でのフォーカス移動のために既に付いていたもので、検証用ではない）。

### 検証したこと

- `DEV_MODE`（app-core.js:12・式のまま）／`DEV_PLAN_OVERRIDE`（:125）正常
- `subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／`global/shops` の全件読み **0件**（ヒットする15箇所はすべて `global/shops/${id}` の直キー読み・書き）／`database.rules.json` の `".read": true` **0件**
- 読み込み順維持（utils→core→staff→admin→main）・SRI **11本**／新規追加した `input` **0件**（今回の差分は useEffect 1つ）
- `npm test` **180件パス**（修正前後で増減なし）／`npx eslint app-*.js` **0 errors 98 warnings**（増減なし）／`node --check functions/index.js` OK
- Cloud Functions: `secrets` の抜け漏れ **なし**（6箇所）／`.delete()` の誤用 **0件**
- **`setSelPid` の全3経路がバッファの後始末を持つことを修正後に再grepで確認**（:361 初期化＝バッファも空／:449 補正＝今回追加／:1579 ドロップダウン＝既存）
- **時刻文字列の辞書順比較の安全性を確認**: `gto()`（app-utils.js:87-96）も `parseTime`（app-admin.js:503-509）も**必ず2桁ゼロ埋め**で返すため、`sd[d].start>=sd[d].end`（app-staff.js:149）等の文字列比較は 25:00〜27:00 の深夜帯を含めて正しく動く
- **別名の一意性の入口を全数確認**: 別名を作れるのは `addAlias`（app-admin.js:2334）と `registerAlias`（:3079）の2つで、**どちらも「既に誰かの別名になっている名前」を選択肢に出さない**（`unregisteredNames`:2360 と `isUnregistered`:3055 がともに `allAliases` を除外している）。`resolveAlias` は最初に一致した登録名を返す実装なので同じ別名が2人に付くと誤解決するが、**単独端末で到達する経路は無い**
- **実ブラウザでの実測2本**（devサーバー無し・`route.fulfill` で配信物を供給。いずれも pageerror・console error **0件**）: ①修正前後の再現（別の期間へ書き込まれる／書き込まれない）②非回帰3項目（同一期間での編集→保存・ドロップダウン切替・未提出スタッフへの新規sub作成）。**どちらも Firebase へ書き込みを行っていない**（`app-main.js` を読み込まないため `firebaseDB` が `null`）
- **未検証**: iOS Safari 実機（WebKitでの再測定は #91・#92 に続き今回も未実施・Chromiumのみ）／実アプリを2端末で起動しての本物の期間削除（今回は親stateの書き換えで再現した）／Excel・PDF ファイルを実際に開いての中身確認／本番データの確認（ユーザー操作）／タップ領域44pxの実測（#74・#91・#92 に続き据え置き）

### 総括

**#92 が残したレンズは当たった。しかも「予想された軸は既に塞がっていて、同じ場所の別の軸が開いていた」という形で当たった。**

**今回の分かれ目は「バッファが属するスコープを列挙し直した」こと。** `localEdits` は最初から「店舗が変わったら捨てる」ことになっていて、その理由もコメントに書いてある。だから「もう守られている」と読める。しかしこのバッファのキーは `名前|日付|フィールド` で、**書き込み先を決めるのはキーに入っていない `selPid`** である。つまり属するスコープは店舗と期間の2つあり、守られていたのは片方だけだった。**「このstateはどのスコープに属するか」を1つだと思い込むと、2つ目のスコープの後始末が抜ける。**

**同じ後始末が複数箇所にコピーされている場所は、抜けを grep だけで見つけられる。** 今回の `setLocalEdits({});setHeatEdits({});setFocusKey(null);` は完全に同じ3行が2箇所にあり、3箇所目に無かった。**コードの意味を読む前に、setter の全呼び出し箇所と、そこに付随する後始末の有無を表にするだけで出る**類のバグだった。#92 の `editIdx`/`aliasIdx`/`posIdx` 全数洗い出しと同じ手口で、対象を「index を持つstate」から「scopeを持つstate」に替えただけとも言える。

**成功トーストを出しながら見えない場所を汚す、という性質に注意が要る。** 「✓ 1件のシフトを保存しました」と出て、グリッドには何も現れない。ユーザーから見れば正常な操作にしか見えず、後日「出勤日数がなぜか1日多い」という形でしか表面化しない。**#92 の「ガードの存在が発見を遅らせる」の親戚で、今回は「成功報告の存在が発見を遅らせる」だった。**

**次回の手。** 実測手段（部品の単体マウント）が3回連続で機能しているので、据え置き項目を片付けるほうが効率的な段階に来ている。優先順は ①**タップ領域44px の実測**（#74・#91・#92 と3回据え置き。手順はコントラストと同じで `getBoundingClientRect` に替えるだけ）②**WebKit での再測定**（ユーザーの主戦場は iOS Safari だが #91〜#93 の3回ともChromiumのみ）③**スタッフ20名超の上限UI**（`staffList` を25名にしたpropsで `StaffTab` を単体マウントすればdevデータを汚さずに測れる）。
コード側の新しいレンズを1つ挙げるなら、**「同じ後始末のコードブロックが2箇所以上にコピーされている箇所を全数grepし、あるべき3箇所目が無いかを数える」**。今回の3行セットがそうだったように、**コピーされた後始末は「コピーし忘れ」という形でしか壊れない**ので、意味を読まずに件数だけで検出できる。候補としては `activeSubsRef` の `off()` 解除（app-main.js）、`pendingSubWritesRef` の掃除、`ssSave`/`ssGet` のセッション保存キーあたりが同じ形をしている。
