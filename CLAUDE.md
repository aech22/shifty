...
## Shifty バグチェックレポート（2026-09-05 自動実行 #110）

> 着手時の HEAD は `b1cbb8f`。作業ツリーは**クリーン**（untracked の `.playwright-mcp/` ログ8件のみ）で、
> #109 で保全が必要だった「他セッションの未コミット変更」は今回は無かった。
> #109 の総括が残した宿題——**「同じ式を探す」ではなく「同じ問いに答えている箇所を探す」**——を
> 出発点にし、名簿・提出・勤務時間の各クラスタを機械照合したうえで、**手で並べたデータそのものが
> 黙って劣化する形**へ観点を移したところ、そちらで当たった。

### 修正済み

- **🟡 振替休日3件が祝日テーブルから欠落し、GW最終日が平日扱いになっていた**（[app-utils.js:12](app-utils.js)）→ `18f35cb`

  `JH_DATES` から **2025-02-24・2025-05-06・2026-05-06** の3件が落ちていた。どれも
  「祝日が日曜に当たり、直後の祝日を飛び越して連休の先へ押し出された振替休日」で、
  **同じ性質の 2025-11-24（勤労感謝の日の振替）は入っている**——つまり方針ではなく記入漏れである。

  | 元の祝日 | 曜日 | 振替日 | 修正前の `isHoliday` |
  |---|---|---|---|
  | 2025-02-23 天皇誕生日 | 日 | 2025-02-24(月) | **false** |
  | 2025-05-04 みどりの日 | 日 | 2025-05-06(火)（5/5を飛ばす） | **false** |
  | 2026-05-03 憲法記念日 | 日 | 2026-05-06(水)（5/4・5/5を飛ばす） | **false** |

  落ちると `isHoliday=false` → `isWeekendOrHoliday=false` → `dayTypeOf="weekday"` と連鎖し、
  **休憩時間・必要ポジション・候補時間の区分が平日設定で適用され、シフト表に「祝」も出ない**。
  飲食店にとって最繁忙日であるゴールデンウィーク最終日が、ちょうどこの形になる。

  **実害の実測**（平日と祝日で休憩を分けている店舗・09:00〜22:00 の勤務）:

  | 日付 | 区分 | 適用された休憩 | 純勤務 |
  |---|---|---|---|
  | 2026-05-06（振替休日） | 修正後 `holSun` | 15:00〜17:00 | **11:00** |
  | 2026-05-07（対照・平日） | `weekday` | 14:00〜15:00 | 12:00 |

  修正前は 5/6 も `weekday` だったので**この2日はまったく同じ結果**になっていた。
  給与に効く1時間が、平日設定を拾ったぶんだけずれていたことになる。

  **本体は「隣の日の区分まで変わる」ことのほう**である。2026-05-06 を足すと 2026年GWの休み日の塊が
  5/2〜5/6 に伸び、**連休最終日が 5/5 → 5/6 へ移り、5/5 は `holSun` から `holSat` に変わる**。
  既存テスト「連休（2日以上の休み日の塊）の最終日は holSun」は誤ったカレンダーを前提に 5/5 を
  最終日と書いていたため、実態に合わせて更新した（**このテストが落ちたこと自体が修正の効いた証拠**）。

  **確定済み（写しを持つ）期間にも遡って効く**。`PERIOD_SNAPSHOT_SETTING_KEYS` は settings を凍結するが
  祝日テーブルは凍結対象外で、`dayTypeOf` は常に現在のテーブルで解決されるためである。
  意図した挙動（正しい値に寄る）だが、過去期間の休憩控除・必要ポジションの表示が変わる点は記しておく。

  **再発防止**: ハッピーマンデー・春分秋分・国民の休日・振替休日を計算した参照カレンダーと、
  `JH_DATES` が覆う全年（2025〜2028）を機械照合するテストを追加した。
  **修正前の `app-utils.js` に向けると実際に落ち、欠落3件を日付で名指しする**ことを確認済み。

### 要確認（未修正）

- **🟢（新規）祝日テーブルが 2028-11-23 で尽き、2029年から19件中9件が祝日でなくなる** → **BACKLOG化済み**

  `JH_FIXED`（日付固定の10件）は毎年効くが、`JH_DATES`（移動祝日・振替）は2028年で終わる。
  **実測**（2029年の実際の祝日19件を参照カレンダーで生成して照合）: 成人の日・春分の日・海の日・
  敬老の日・秋分の日・スポーツの日と振替休日3件の**計9件が `isHoliday=false` になる**。
  上の🟡とまったく同じ連鎖で休憩・必要ポジション・候補時間の区分が崩れる。

  **壊れるのは 2029-01-08（成人の日）から**。直さなかったのは、2029年の春分・秋分が
  **官報での確定公表を待つ性質のもの**（前年2月に暦要項で告示される）で、計算値で埋めてよいかが
  判断になるため（条件B・条件C）。時限で落ちる CI テストは入れなかった——コード変更なしに
  ある日突然ビルドが落ちるのは副作用が大きいので、期限は BACKLOG 側に持たせた。

- **🟢 配信版数が app-*.js に追いついていない**（#103 から継続）。`?v=` と `build:` は
  `20260831-c340d5c` のままで、以後 app-*.js のファイル変更が**19件**（今回の1件を含む）入っている。
  `/release-to-main` の手順63が標準工程としてバンプするため申し送りのみ。
- **提出状況一覧の「未提出」が生の `staffList` を見る件（#109 起票）に、呼び出し元をもう1つ追記した**。
  `SmModal` は**管理者画面（`PeriodsTab`・[app-admin.js:1961](app-admin.js)）からも開かれ、そこも
  生の `staffList` プロップを渡している**。締め切って良いかを判断するのは管理者なので、
  実害はむしろ管理者側にある。BACKLOG の影響範囲を更新した（**新規起票はしていない・同根のため**）。
- **変化なし（#109 から継続・BACKLOG化済み）**: プラン変更予約を取り消せない（🟡）／
  秘密鍵ログ修正の本番反映／二重課金の根治／特商法表記（🔴 2件）／解約通知／解約時のプラン判定／
  企業連携の解除／別名提出の重複（#81・根は未解決）／`database.rules.tightened.json` の残骸
  （今回も `cmp` でバイト同一を再確認）。
- **変化なし（#109 から継続・BACKLOG化しない🟢）**: `resolveShopMeta` の JST 未適用／
  自分の別名へ改名すると別名が自分に残る／ヒートマップの時間目盛りだけが素の `subs` を走査
  （[app-admin.js:1057](app-admin.js)・`adminRest` を見ないぶん軸が広がるだけの表示差）／
  片側セルの実効退勤バッジ／期限切れの後始末の範囲／`purgeInactiveShops` が存在しない `inviteCodes` を掃除。
...
- **RULES.md のスキャン項目は全項目クリア**: `DEV_MODE` は式のまま／`subs` の `set()` 全体上書き **0件**／
  ...
  SRI **11本**／**フォーム部品56件の `fontSize<16` 0件**／CSS変数の未定義参照 **0行**／
  ダーク定義の二重管理（`[data-theme]` と `prefers-color-scheme`）**差分なし**。
- `npm test` **225件パス**（+2。祝日テーブルの参照カレンダー照合と振替休日3件の回帰。どちらも
  修正前のコードでは落ちることを確認済み）／`npx eslint app-*.js` **0 errors 95 warnings**（増減なし）／
  `node --check functions/index.js` OK／`secrets` 指定6箇所／`.delete()` の誤用 **0件**／
  `database.rules.json` と `tightened` は `cmp` でバイト同一。
- **`matchPositionSlots` を総当たりの厳密解と突き合わせた**（ポジション不足エラーの判定＝Premium）。
  スロット1〜5・出勤者0〜5・4ポジションで **20,000ケース／最大マッチングとの不一致 0件**。
  不足件数の合計が `slots.length - matchedCount` と常に一致することも同時に確認。Kuhn法の実装は正しい。
- **「同じ問いに答える箇所」の機械照合**: `calcNetWorkMinutes`／`shiftBandInfo` の呼び出し**10箇所**すべてで
  片側セル補完用の `settings` が渡っている（1箇所でも抜けると同じ日が画面ごとに別の分数になる＝#82 の形）。
  `effShiftStart`/`effShiftEnd` を通さない直読みは、既知の🟢2箇所（ヒートマップの時間目盛り・
  詳細モーダルの実効退勤バッジ）以外に無し。名簿の解決も `mergeKeepStaff`／`resolvePeriodMaster`／
  `isUnregisteredSubName` の3経路に収束しており、新しい直書きは増えていない。
- **手で並べたデータ・レジストリの整合**: `TO` 109件が15分刻みで欠けなし・`TO_START` 55件が `TO` の部分集合／
  `PLAN_LIMITS` が VISION（Free=スタッフ20名・期間1件）と一致／`PLAN_RANK_UI` と
  functions の `PLAN_RANK` が同値／`POSITION_DAY_TYPES` の5区分が `dayTypeOf` の値域と過不足なく一致／
  祝日テーブルの写しは**リポジトリ内に1つだけ**（`app-utils.js`）。
- **Excel の実物を開いて列構成を確認した（17回連続の「未検証」を解消）**。詳細は次節。
- **Firebase には1バイトも書いていない**（`app-main.js` を読まないハーネスのみ使用）。
- **未検証**: PDF の実物（jsPDF がラスタ画像に落とすため列テキストを読めない。列は Excel と同じ
  `isUnregisteredSubName` ＋同じ解決済み名簿から作られるが、**測ってはいない**）／iOS 実機／本番データ／
  プラン変更予約の Stripe 実データ。

### Excel の実物確認（BACKLOG の宿題を1つ閉じた）

`expXl` はグローバル関数なので、1.6節のハーネスに ExcelJS を載せ、`URL.createObjectURL` と
`HTMLAnchorElement.prototype.click` を差し替えて**生成された .xlsx のバイト列をそのまま捕まえ**、
ExcelJS で読み直した。BACKLOG「スタッフ削除時の…残りの確認2件」の受け入れ条件1がこれで閉じる。

| ケース | Excel のヘッダー行（実測） |
|---|---|
| A: 期間管理タブ／佐藤を「**残す**」 | 田中 ／ **佐藤** ／ (空白列) ／ 鈴木 ／ 山田 |
| B: 期間管理タブ／佐藤を「**残さない**」 | 田中 ／ (空白列) ／ 鈴木 ／ **佐藤** ／ 山田 |
| C: シフト作成タブ（`resolver` あり）／「残す」 | 田中 ／ **佐藤** ／ (空白列) ／ 鈴木 ／ 山田 |

- **「残す」を選んだ人は元の位置に出る**（末尾送りではなく、田中の直後・空白列の前）＝受け入れ条件どおり。
- **「残さない」を選んでも提出があれば末尾に未登録名として出る**（佐藤・山田がともに鈴木の後ろ）＝仕様どおり。
- **2つの入口（期間管理タブ／シフト作成タブ）が同じ列構成を出す**ことも同時に確認できた。
- 空白列は `__spacer__`+token が**空のヘッダーとして描かれる**（名前が漏れていない）。
- `pageerror`・`console.error` ともに **0件**。ファイル1つあたり 7,635〜7,695 bytes・12行9列。

副産物として **Excel のファイル名が入口で変わる**ことが分かった。`resolver`（管理者調整値を返す関数）を
渡さない期間管理タブからの出力だけ `_修正前.xlsx` が付く（[app-admin.js:2344](app-admin.js)）。
調整前の生の提出を出すという意味で**設計どおり**であり、バグではない。

### 測り方の記録

**今回いちばん効いたのは、コードではなくデータを疑ったこと。** #105〜#109 は5回連続で
「同じ不変条件に入口が複数あり片方だけが知らない」形を追っており、今回もその線で名簿・提出・
勤務時間の各クラスタを照合したが、**そちらは全部きれいだった**（上の検証節のとおり）。
当たったのは、`JH_DATES` という**手で並べた文字列の集合**のほうである。

この種のデータは、**抜けても何も壊れない**のが厄介である。祝日が1件落ちても例外は出ず、
`npm test` も `eslint` も通り、画面も普通に描画される。ただ「その日が平日になる」だけで、
気づく手がかりは**現場の人が『あれ、この日は祝日設定じゃないの？』と言うこと**しか無い。
コードの不整合と違って、レビューでも grep でも見つからない。

**見つけ方は「実装を読む」ではなく「外部の正解を計算して突き合わせる」**だった。
ハッピーマンデー・春分秋分・振替休日の規則から参照カレンダーを組み、
アプリの `isHoliday` と全日を比較した。この方法は**逆方向（余分に祝日としている日）も同時に検出でき**、
そちらは0件だった。同じ手は `matchPositionSlots`（総当たりの最大マッチングと照合）でも使い、
20,000ケースで不一致0件という**「異常なし」を証拠付きで言える形**にできた。

**Excel については、17回「未検証」と書き続けたことのほうが問題だった。** 実際にやってみると、
ダウンロードの出口（`URL.createObjectURL` と `a.click()`）を差し替えるだけで、バイト列を
その場で読み直せた。**「ユーザー操作が要る」と分類していた項目が、出口を1つ差し替えるだけで
自動化できた**わけで、同じ理由で残っている他の項目（PDF）も、出口の形を確かめてから
「できない」と言うべきである。今回 PDF が閉じられなかったのは、jsPDF が
**HTMLをラスタ画像にしてから埋め込む**ため、そもそも読み取る文字が残らないという別の理由による。

### 総括

**🔴・🟡 ともに未修正はゼロ。** 見つかった🟡1件（振替休日の欠落）はその場で直し、
再発を機械が検出するテストを置いた。

**#107→#108→#109 が3日連続で追っていた「正本の繋ぎ忘れ」は、今回は1件も出なかった。**
名簿の5経路も勤務時間の10経路も揃っている。あの3日で置いたドリフト検出テストが効いていると
考えてよい。そのぶん今回は別の形——**外部世界の事実を写し取ったデータが古くなる**——に当たった。
`JH_DATES` は2028年で尽き、`purgeOldPeriods` は観察期間が本番デプロイ日で決まり、
`STRIPE_PRICES` は Stripe 側の設定に従う。**コードの正しさとは別に、「いつまで正しいか」を
持っているデータがいくつもある**ので、次回以降はそれらの有効期限を1つずつ確かめておきたい。
...
## 🟢 祝日テーブルが 2028-11-23 で尽き、2029年から19件中9件が祝日でなくなる

**目的**: `JH_FIXED`（日付が毎年固定の10件）は年に依らず効くが、`JH_DATES`（ハッピーマンデー・
春分秋分・振替休日）は **2028年で終わっている**（[app-utils.js:12](app-utils.js)）。
2029年に入ると、実際の祝日19件のうち**9件を `isHoliday` が false と答える**。

**実測**（ハッピーマンデー・春分秋分・国民の休日・振替休日の規則から参照カレンダーを生成して照合）:

| 落ちるもの | 日付 |
|---|---|
| 成人の日（第2月曜） | 2029-01-08 |
| 春分の日 | 2029-03-20 |
| 海の日（第3月曜） | 2029-07-16 |
| 敬老の日（第3月曜） | 2029-09-17 |
| 秋分の日 | 2029-09-23 |
| スポーツの日（第2月曜） | 2029-10-08 |
| 振替休日 | 2029-02-12 ／ 2029-04-30 ／ 2029-09-24 |

**壊れ方はバグチェック#110 で直した振替休日の欠落とまったく同じ**で、
`isHoliday=false` → `isWeekendOrHoliday=false` → `dayTypeOf="weekday"` と連鎖し、
**休憩時間・必要ポジション・候補時間の区分が平日設定で適用され、シフト表に「祝」も出ない**。
例外もログも出ないので、気づく手がかりは現場の人の指摘しかない。

**壊れ始めるのは 2029-01-08（成人の日）**。逆算すると、2029年前半のシフトを作り始める
**2028年12月までに入れておく**必要がある。

**受け入れ条件**:
- [ ] 2029年ぶんを計算値で先に入れるか、官報（暦要項）の確定公表を待つかを決める（**ユーザー判断**）
  - 案A: 計算値で先に埋める。春分秋分は1980〜2099で使える近似式があり、**既存の2025〜2028の
    実データと完全一致することを #110 で確認済み**。ハッピーマンデーと振替休日は規則が確定しているので
    ずれる余地は無い
  - 案B: 前年2月の暦要項（春分・秋分の日を正式に告示するもの）を待って手で入れる。確実だが、
    入れ忘れるとそのまま2029年に突入する
  - 案C: `JH_DATES` を廃止し、規則から計算する関数に置き換える（テーブル自体を無くす）。
    根治するが、祝日法の改正・臨時の祝日（即位礼等）を吸収できなくなるので、
    「計算＋例外テーブル」の二段構えにする必要がある
- [ ] 決めた案で 2029年（案Cなら恒久）を反映し、`tests/core.test.js` の
      「祝日テーブルが計算した日本の祝日と一致する」を通す（**このテストは `JH_DATES` が覆う年を
      自動で拾うので、年を足せばその年も自動的に照合対象になる**）
- [ ] 期限切れに気づく仕掛けを置くか決める（例: 覆う最終年が「今年+1」を下回ったら落ちるテスト。
      **コード変更なしにある日CIが落ちる**ので、入れるかどうかは判断が要る。#110 では入れなかった）

**影響範囲**: app-utils.js（`JH_DATES`・案Cなら `isHoliday`）、tests/core.test.js
**備考**: バグチェック#110（2026-09-05）で検出・**条件B（計算値で埋めるかの判断）と
条件C（官報の確定値というユーザー側にしかない情報）に該当**。同じ回で、**現在の対象年に
実際に落ちていた振替休日3件**（2025-02-24・2025-05-06・2026-05-06）は判断が要らないので
修正済み（`18f35cb`）。祝日テーブルの写しはリポジトリ内に `app-utils.js` の1つだけであることも確認済み。

---

## 🟢 期限付き削除で名前を残した人が「未提出」の一覧にだけ数えられない

**目的**: 2026-09-04 に「その期間の名簿＝`staffList` + `period.keepStaff`」という解決を
`isUnregisteredSubName`（app-utils.js）へ一本化し、シフト作成グリッド・Excel・PDF・提出一覧の
「別名を登録」・スタッフ画面の名前サジェストの**5経路がすべてマージ済み名簿を見る**ようになった。
残る1経路が提出状況一覧の「未提出」（[app-staff.js:541](app-staff.js) `notSubmitted`）で、
ここだけが生の `staffList` を見ている。

**実測**（SmModal だけを実ブラウザにマウント。`staffList=["田中","鈴木"]`・9月前半の
`keepStaff` に佐藤・提出は田中のみ。Firebase へは1バイトも出していない）:
...
つまり佐藤は、**その期間のシフト表には列があり、提出画面では自分の名前をサジェストから
選べる**のに、**未提出の一覧には最後まで出てこない**。管理者は「全員出した」と読んで
締め切ってしまえる。

**受け入れ条件**:
- [ ] 期限付き削除で名前を残した人を「未提出」に数えるかを決める（**ユーザー判断**）
  - 案A: 数える（`notSubmitted` を `mergeKeepStaff(staffList,ap)` ベースにする）。
    2026-09-04 の決定「残した人はその期間の名簿の一員」と揃う。退職者が赤いチップで
    並ぶことになるが、提出を待っている人が漏れなく出る
  - 案B: 数えない（現状維持）。退職者を催促しても意味がないという読み。ただし
    **提出画面で本人の名前をサジェストしている**ことと食い違う
- [ ] 決めた案を実装し、`SmModal` に渡す名簿の出どころをコメントで明示する（次に名簿の要素が
      増えたとき、この経路が取り残されないように）

**影響範囲**: app-staff.js（`SmModal` の `notSubmitted`、`StaffView` からの `staffList` 受け渡し2箇所）、
**app-admin.js:1961（`PeriodsTab` から同じ `SmModal` を開く3つ目の呼び出し元）**
**備考**: バグチェック#109（2026-09-04）で検出・**条件B（どちらの挙動が正しいかの仕様判断）に該当**。
同じ回で、Excel と PDF の2経路が正本を通っていなかったぶんは判断が要らないので修正済み（`04c4049`）。

**2026-09-05 追記（#110）— 呼び出し元がもう1つある**: `SmModal` は**管理者画面からも開かれる**
（[app-admin.js:1961](app-admin.js) の `PeriodsTab`）。そこも `staffList` を**生のプロップのまま**
渡している（`resolvePeriodMaster` を通していない＝`ShiftEditTab` の解決済み名簿とは別物）。
起票時は影響範囲をスタッフ画面だけと書いていたが、**「全員出した」と読んで期間を締め切るのは管理者**
なので、実害はむしろこちらにある。案Aを採るなら**2ファイル3箇所**を同時に直すことになる。
...
で作られ、セクション全体が `changeOptions.length>0` でしか描画されない（:4628）。
...
**逃げ道も無い**: 解約バナー（:4604）は「解約を取り消す場合は下の『請求管理』から」と
