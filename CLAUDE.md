...
## Shifty バグチェックレポート（2026-08-22 自動実行 #90）

### 修正済み

- **🟡 shopId に含まれる `=` でCookie名が途中で切られ、期間が変わるとスタッフ名の記憶が失われる**（[app-core.js:187](app-core.js) / [app-utils.js:450](app-utils.js) 追加）→ `2c96ff3`
  `genSecureId`（app-utils.js:417）の文字集合は **Firebaseのキー禁止文字だけ**を避けており `=` を含む。ところが同じIDは `ckStaffKey`（`ots_staff_{shopId}_{periodId}`）として **Cookie名**にも使われる。ブラウザは Cookie を**最初の `=` で名前と値に分割する**（RFC 6265 §5.2）ため、`=` を含む shopId から作った名前は途中で切られ、**同じ店舗の全期間が1つのCookieを奪い合う**。
  **何が起きるか**: 期間ごとに別のCookieができるはずが1つしかできず、新しい期間で提出するたびに前の期間のぶんが上書きされる。結果、**スタッフが翌月のURLを開いたとき名前が復元されない**（毎回打ち直しになる）。これは BACKLOG の「別名で提出した人が別端末から再提出すると2つのsubができる」と地続きで、**打ち直しこそが別名sub・表記ゆれを生む入口**である。
  **どれくらいの店舗が該当するか**: `genSecureId` は 67文字の集合から `crypto.getRandomValues` の `b%67` で選ぶ。256÷67 に余りが出るためモジュロ偏りがあり `=` の重みは 3/256。24文字なら **1-(253/256)^24 ≒ 24.7%** ＝ **およそ4店舗に1つ**。BACKLOG に実在の shopId として記録されている `mKdff4?v88uPN=B=eEsc&WHW` は `=` を2つ持つ。
  **実測（Chromium・WebKit 26.5 の両方／配信物の実バイトを読み込んで実行）**: `=` を含む shopId では **cookies=1・先月ぶんが `null`**、`=` を含まない shopId では **cookies=2・両方読み戻せる**。両エンジンで完全に同値。
  **修正**: `cookieSafeKey`（`=` → `.` 置換）を app-utils.js に追加し `ckStaffKey` を通した。**`.` を選んだのは `genSecureId` の文字集合に `.` が無く**（Firebaseのキー禁止文字として除外済み）、`periodId` も `p_<数字>` なので**置換後に別のキーと衝突しないから**。`=` を持たない shopId では**キーが1バイトも変わらない＝既存のCookieを壊さない**（実測で確認）。`getCookie` の正規表現エスケープは `.` を対象に含んでいるので副作用もない。
  **移行**: `=` を持つ店舗の既存Cookieは1つだけ孤児になるが、そこはもともと「期間をまたぐと消える」状態だったので、打ち直しは1回で済むようになる（毎期間から1回きりへ）。ユニットテスト3件追加（**177→180件パス**）。

- **🟡 店舗コードに記号が混ざると「確認中...」のまま固まり、理由が一切表示されない**（[app-admin.js:30](app-admin.js) / [app-main.js:1260](app-main.js)）→ `aec63cd`
  ログイン画面の「店舗コードを貼り付け」と、店舗メニューの「コードで追加」は、入力を `parseShopCode` に通したあと **`firebaseDB.ref(\`global/shops/${code}\`)` へそのまま渡す**。`ref()` は禁止文字（`# $ [ ]`）に対して**同期に throw する**ため、両経路が持つ `.catch`（promise に付く）では受け取れない。直前に出した「確認中...」が消えず、**エラーも出ないまま固まる**。
  **現実的な入力で起きる**: 共有URLのhash（`#/s/...`）をコードとして貼る、`#` や `[` が混じった文字列を貼る、など。**`/` は throw しない**（パス区切りとして扱われ深い階層を読みに行くだけ）ことも実測で確認した。
  **正解はコードベース内にあった**: #89 で追加した `firebaseKeyForbiddenChars` が、まさに `ref()` が拒む集合（`. # $ / [ ]` と制御文字）の判定器である。**IDを生成する側と名前を受け取る側は既にこれを通していて、コードを受け取る側だけが通っていなかった。** 仕様判断なしにそこへ揃えられた。app-admin.js 側は**プラン上限判定より前**に置いた（不正なコードでアップグレード案内を出さないため・#89 と同じ理由）。
  **実測（実ブラウザでアプリを起動し、実際に入力欄へ打ち込んで確認）**: **修正前**は `#/s/abc12345`・`shop#123`・`shop[1]` の3件が「確認中...」のまま固まり、コンソールに `child failed: path argument was an invalid path ... can't contain ".", "#", "$", "[", or "]"` が**未捕捉の pageerror として3件**出た。**修正後**は同じ3件が「コードが正しくありません。もう一度確認してください。」を表示し、**pageerror 0件**。
  **非回帰**: 「正常な形式だが未登録のコード」は**修正前後で同一の挙動**（Firebaseの読みまで進んで「コードが正しくありません」）。`=` を含む正常な shopId も素通りする。
  **1つ訂正**: 調査の途中で「`addShopByCode` には `.catch` が無い」と読んだが**誤り**で、[app-admin.js:58](app-admin.js) に存在する。問題は catch の有無ではなく、**同期 throw は promise の catch では捕まらない**という一点だった。

### 要確認（未修正）

- **🟢（新規・プロセス）「無人セッションだから実機検証ができない」は誤りだった。#78〜#89 の12回の申し送りを訂正する**
  `preview_start` は今回も明示的に拒否された（「Dev servers can't be started from unattended sessions」）。これ自体は正常な動作である。しかし**devサーバーはブラウザ検証の必要条件ではなかった**。`/Users/hiroshi/.claude/skills/rendered-contrast-check/node_modules/playwright-core` を直接読み込み、`page.route("**/*")` で**ローカルの配信物をディスクから `route.fulfill` で返し、CDN と Firebase だけ実ネットワークへ通す**と、**Shifty が丸ごと起動する**。今回それで `DEV_MODE=true`・`?plan=premium`・`firebaseEnabled=true`・**コンソールエラー0件**でログイン画面まで描画し、入力欄への打鍵とクリックまで実行して上記2件目を検証した（この方法が無ければ「同期throwが `.catch` を素通りする」ことの実機証拠は取れなかった）。
  Playwright **MCP** の方は他セッションに占有されて `Browser is already in use` で落ちたが、playwright-core の直叩きは別プロセスなので通る。**Chromium と WebKit の両方が使える**（`~/Library/Caches/ms-playwright` に既にある）。
  **これで次の宿題が無人のまま進められる**: **コントラスト実測（#72・#75・#76・#77 の「決めること1〜5」）**、**タップ領域44px実測（#74）**、**`?plan=free` での Free 上限UI（#77 の宿題）**。いずれも「実測不能」を理由に据え置かれていたが、その前提は消えた。手順は memory の `reference_headless_browser_without_dev_server` に残した。
- **🟢（新規）`name|date` 複合キーの分解が2箇所で食い違っている**（[app-admin.js:1637](app-admin.js) が `indexOf("|")` ／ [app-admin.js:3063](app-admin.js) が `lastIndexOf("|")`）
  キーは `name|date` で、日付側に `|` は入らないので **`lastIndexOf` が常に正しく、`indexOf` はスタッフ名に `|` が無いときだけ正しい**。`firebaseKeyForbiddenChars` は `|`（Firebaseのキーとしては有効）を弾かないため、名前に `|` を入れることは可能。壊れるのは店舗間重複エラーパネルの**表示だけ**（名前が切れて日付が化ける）で、データは壊れない。飲食店のスタッフ名に `|` が入る見込みは薄く 🟢 とした。**直すなら正解は同ファイル内の `:3063` にある**（1トークンの変更）。
- **🟢（新規）Cloud Functions の `linkStoreToCompany` が shopId の文字種を検証していない**（[functions/index.js](functions/index.js) の `if (!companyId || !shopId)` は空文字しか見ていない）
  クライアント側の3つ目の `parseShopCode` 呼び出し（[app-main.js:1027](app-main.js)）は `ref()` ではなく CF へ渡すため、**今回のクライアント修正の対象外**。CF 側で `db.ref(\`global/shops/${shopId}\`)` が同期 throw しても、`async` ハンドラなので rejected promise になり、クライアントの `try/catch` が「追加に失敗しました」を表示する＝**固まらない**。理由が具体的でないだけなので 🟢。**修正には CF のデプロイ（ユーザー操作）が要る**ため申し送りに留める。
- **🟢 配信版数（`?v=` と `build:`）は6箇所とも `20260822-e9d98a4` で一致しているが、今回 app-utils.js・app-core.js・app-admin.js・app-main.js の4本が変わった**ため、次のリリースでバンプが要る。申し送りのみ（`/release-to-main` の手順63が標準工程として持つ。BACKLOG化はしない）。
- **🟡 #77 の Free 上限の実機検証**は静的監査のみ完了・実測は未達だったが、**上記のとおり手段ができたので次回は実施できる**（`?plan=free` で起動しスタッフタブで21人目・2件目を押す）。
- 以下は変化なし・すべて BACKLOG化済み: **休憩の適用条件が画面の説明文と食い違う**（#89）／**期間の確定（写し）が `periods` を肥大化させる**（#88）／**別名で提出した人の再提出でsubが2つできる**（#81・コード修正済み・Excel実物の確認と本番データ確認が残）／**コントラスト5件**（#72・#75・#76・#77）／**タップ領域44px未満**（#74）／**スタッフURLを開いた誰でも他人の提出を削除できる**（#70）／**スタッフ削除の後始末がリネームと正反対**・**未登録名が画面に出ずExcel/PDFにだけ出る**（#79）／**期間の重複にバリデーションが無い**（#83）／**`x` セルが店舗間重複の判定から除外されていない**（#85）／**店舗間重複の判定が退勤延長と片側セルを見ていない**（#86）／**解約イベントだけが metadata でプラン判定**（#68）／**企業連携の解除が店舗をオーナー0人に戻す**（#65）／**Pro→Premium の二重課金の根治**・**特商法表記**（BACKLOG 🔴）
- **🟢 `effShiftStart`/`effShiftEnd` の未採用が app-admin.js になお3箇所**（`:1009` / `:3055` / `:3113`）。**#82〜#89 と同じ理由で今回も触っていない**——直し方が BACKLOG の判断待ち項目（片側セルだけの日を何時間と数えるか）に依存する。
- 以下は #46〜#89 からの継続（変化なし）: 未claim店舗の取り込み／`findActiveSubscription` の `|| live[0]`／`planExpiry` のUTC計算／`purgeOldPeriods` のUTC日付／`generateInviteCode` のデッドコード／ほか

### 今回の対象と見つけ方

**#89 が名指しで残した宿題から入った。** 「値として扱っているはずのものが、どこかでキー・ID・ファイル名になっていないか」を、**ファイル名**と **localStorage のキー**へ広げるという指示だった。

**(1) 名指しされた2つは、どちらも既に守られていた。** Excel（[app-admin.js:2289](app-admin.js)）と PDF（[:1300](app-admin.js) の `pdfSanitize`）は**同じ文字集合 `[\\/:*?"<>|]` を独立に除去していて**、期間ラベルも `${mo}月${前半|後半}` の生成物で自由入力ではなかった。ExcelJS のシート名も固定リテラル。localStorage のキーも `shopId`・`periodId` という**生成ID**しか含まない。**宿題は空振りだった。**

**(2) 空振りの過程で、レンズが1段ずれた。** localStorage のキーを追ううちに、**同じ生成IDが Cookie 名にも使われている**ことに気づいた。ここで効いたのは #89 が書き残していた事実の方だった——`genSecureId` のコメントは「**Firebase禁止文字**を除外した記号のみ使用」と、**何のために安全なのかを明示している**。つまりこのIDは *Firebaseのキーとしてだけ* 安全だと宣言されている。そこで「Firebase以外の、文字に規則がある場所」を数え直したところ、**Cookie名（`=` が区切り）**・**`shopId.adminKey` の `.` 区切り**・**URL** の3つが出た。後の2つは無事だった（`.` は文字集合から除外済み、URLは `genToken` の英小数字のみ）。**残ったCookieだけが穴だった。**

**(3) 「1箇所だけ守られている」ではなく「守っている理由が限定的だと自分で書いてある」から辿れた。** #89 は「守られている場所が1つだけあった」ことから逆算したが、今回は**守りの適用範囲がコメントで宣言されていた**ので、その範囲の外側を列挙するだけでよかった。仕様判断は一度も要らなかった。

**(4) 2件目は、1件目の道具（`firebaseKeyForbiddenChars`）を持ったまま歩いていたから見えた。** 「この判定器を通していない入口が他に無いか」を探すと、**ユーザーが文字列を打ってFirebaseのパスになる経路がもう1つ**あった。#89 が閉じたのはスタッフ名で、店舗コードは残っていた。しかも**失敗の仕方が正反対**で、スタッフ名は「黙って保存されたように見える」、店舗コードは「黙って固まる」——**どちらも `.catch` が書いてあるのに、同期throwなので発火しない**という同じ構造から出ている。

**(5) 最大の収穫は修正2件ではなく、12回続いた「実機検証できない」という前提が誤りだったと分かったこと。** `preview_start` の拒否は本物だが、それは**devサーバーの話であってブラウザの話ではなかった**。playwright-core はスキルの `node_modules` に、ブラウザ本体はキャッシュに、最初からあった。**「拒否された」を「不可能」と読み替えたまま12回申し送っていた。** #88 で「前回拒否を理由に試さないのが最大の失敗」と学んだはずが、学びが `preview_start` の再試行だけに閉じていて、**別の手段を探すところまで広がっていなかった。**

### 検証したこと

- `DEV_MODE`（app-core.js:12・式のまま）／`DEV_PLAN_OVERRIDE`（:125）正常
- `subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／`global/shops` の全件読み **0件**／`database.rules.json` の `".read": true` **0件**
- 読み込み順維持（utils→core→staff→admin→main）・SRI **11本**／新規追加した `input` **0件**（`fontSize<16` の混入なし）
- `npm test` **180件パス**（新規3件）／`npx eslint app-*.js` **0 errors 98 warnings**（修正前後で増減なし）／`node --check functions/index.js` OK
- Cloud Functions: `secrets` の抜け漏れ **なし**（6箇所とも必要な鍵を宣言）／`.delete()` の誤用 **0件**
- 動的 `RegExp` の生成は `getCookie` の1箇所のみで、名前を正規表現エスケープ済み（今回入れる `.` も対象に含まれる）
- **Cookieの分割規則を実ブラウザで実測**: Chromium・WebKit 26.5 の両方で、`=` を含む名前は最初の `=` で切られ、同一店舗の複数期間が1つのCookieを共有することを確認。修正後は期間ごとに別Cookieになり、`=` を持たない shopId はキーが不変
- **`ref()` の同期throwを実SDK（firebase-admin）で実測**: `#` `$` `[` `]` で同期throw、`/` はthrowせず、`.` 始まり（code が空文字）は `global/shops` 全体の読みになることを確認。修正後は**入口で拒否6件・読みに進む3件・固まる0件**
- **実ブラウザでアプリを起動して実UI検証**（devサーバーなし・`route.fulfill` で配信物を供給）: 店舗コード欄に5パターンを実際に打鍵。**修正前は3件が「確認中...」で停止＋未捕捉 pageerror 3件、修正後は5件とも理由を表示・pageerror 0件**。正常形式のコードは修正前後で同一挙動
- **`parseShopCode` の呼び出し元3箇所を全数確認**（app-admin.js:29／app-main.js:1259／app-main.js:1027）。3つ目は CF 経由でクライアント側の `ref()` を通らないため対象外と判定
- **未検証**: iOS Safari 実機（WebKit エンジンでは確認済みだが実端末ではない）／`=` を含む本番店舗が実在するかの本番データ確認（**本番データの読み取りはユーザー操作**）／Excel・PDF ファイルを実際に開いての中身確認

### 総括

**#89 が残した宿題（ファイル名・localStorageキー）は空振りだったが、その空振りの途中でレンズが1段ずれて 🟡 を2件直せた。**

**今回の分かれ目は「守りが宣言している適用範囲を読んだ」こと。** `genSecureId` のコメントは「Firebase禁止文字を除外した」と書いてある。これは**安全性の主張であると同時に、安全な範囲の限定でもある**。#89 はこのコメントを「禁止文字の一覧」として使ったが、今回は「**この安全性はFirebaseにしか及ばない**」という読み方をして、及ばない先（Cookie名・`.` 区切り・URL）を数えた。**コメントは仕様書としてだけでなく、適用範囲の宣言としても読める。**

**2件とも、失敗が観測できない形をしていた。** Cookieの方はエラーが1つも出ない——ブラウザは切り詰めた名前を黙って受け入れる。店舗コードの方は例外こそ出るが、**それを拾うために書かれた `.catch` が同期throwを取れない**ので画面には何も出ない。#89 の `fbW` とまったく同じ構造で、**このコードベースには「`.catch` を書いたので安心している同期throw」が他にも10箇所ある**（`fbSet(...).catch(...)` 型）。今はどれも生成IDしか渡っていないが、**自由入力がそこへ届いた瞬間に同じ形で黙る**。

**そして、この回で一番効いたのは修正そのものではなく前提の訂正だった。** 「無人だから実機検証できない」は12回申し送られてきたが、**誤りだった**。devサーバーは拒否されるがブラウザは動く。道具（playwright-core とブラウザ本体）は最初から手元にあった。#88 で得た教訓——「前回拒否されたことを理由に試さない」——を、`preview_start` の再試行だけに適用して、**同じ目的を別の手段で達せないかを問わなかった**のがこの12回の失敗である。**「できない」と書くときは、拒否されたのが目的なのか手段なのかを分けること。**

**次回の手。** 手段が戻ったので、**据え置かれてきた実測系を一気に片付ける**のが最も効率がよい。優先順は ①**#77 の Free 上限**（`?plan=free` で起動しスタッフタブで完結）②**コントラスト実測**（#72・#75・#76・#77 の決めること1〜5。ダーク／ライト×375px／1400px を `getComputedStyle` で全DOM測定。**測って件数を出すところまではループの仕事で、色を選ぶのがユーザー判断**）③**タップ領域44px**（#74）。いずれも判断待ちの前に「現状の実測値」が要るので、測っておけばユーザーが決める材料が揃う。コード側の新しいレンズを1つ挙げるなら、**上記の `fbSet(...).catch(...)` 10箇所について「その path に自由入力が届く経路があるか」を1つずつ潰す**——今回はスタッフ名（#89）と店舗コード（#90）で2つ見つかっており、3つ目がある前提で数えるのが妥当。

**CF側の宿題（#68 の2つ目）と、本番データの確認は今回も持ち越し。**
