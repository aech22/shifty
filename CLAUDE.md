...
## Shifty バグチェックレポート（2026-08-23 自動実行 #91）

### 修正済み

- **🟡 スタッフURLの末尾に記号が1文字混ざるだけで、アプリが「データを読み込み中...」のまま永久に固まる**（[app-main.js:175](app-main.js) / [app-main.js:227](app-main.js)）→ `e209e24`
  Phase1 の `loadShops` は URL hash から取り出したトークンを**一切検証せず** `tokens/${token}` として `ref()` に渡す。`ref()` は禁止文字（`. # $ [ ]`）に対して**同期に throw する**ため、すぐ下に書いてある `.catch`（app-main.js:252）は**付く前に投げられていて拾えない**。
  **watchdog も効かない**のがこの件の悪いところ。10秒で初期化不能を検知する `authWatchdog` は用意されているが、`clearTimeout(authWatchdog)` は `proceed()` の中で **`loadShops()` を呼ぶ直前**に走る（app-main.js:148→151）。つまり保険を外した直後に throw するので、`setReady` も `setUnbound` も `setInitError` も走らないまま止まる。**画面には何も起きず、エラーも出ず、再試行ボタンすら現れない。**
  **どういうときに起きるか**: アプリが作るURLは安全（`genToken` は英小文字と数字のみ）。壊れるのは**配布の途中でURLの末尾に記号が付いたとき**で、最も現実的なのは**メールやLINEのリンク化が文末のピリオドまでリンクに含める**ケース（「シフトはこちらです https://shiftyshifty.app/...#/s/abc12345.」）。`/` は throw しない（パス区切りとして解釈されるだけ）ことも実測で確認した。
  **実測（Chromium・配信物をそのまま起動）**: **修正前**は正常トークンが終端画面に到達するのに対し、末尾ピリオドのトークンは **14秒経っても「データを読み込み中...」のまま**で、未捕捉の pageerror が1件（`child failed: path argument was an invalid path = "tokens/abc12345."`）。**修正後**は3パターン（正常・末尾ピリオド・`%23`）とも**同一の終端画面**に到達し **pageerror 0件**。
  **修正**: 禁止文字を含むトークンは `tokens` に存在しえないので、**既に書かれている「一致なし」の分岐**（`setInitError("resolve"); cookieFallback();`）へ落とす。**新しい挙動を発明していない**——仕様判断が要らないのは、正解が同じ関数の12行下にあったから。あわせて `global/shops` の直キー読みの単一窓口である `readShop` にも同じ理由でガードを入れ、`null`（＝見つからなかった）を返すようにした（**呼び出し元5箇所すべてが `null` を処理済みであることを確認済み**）。
  **#90 との関係**: まったく同じ「同期throwが `.catch` を素通りする」構造で、#89（スタッフ名）・#90（店舗コード）に続く**3件目**。#90 が「3つ目がある前提で数えるのが妥当」と書いて終わっていたのが、そのとおりだった。**しかも3件のうちこれが一番深い**——前2件は入力欄が固まるだけだが、これは**アプリ全体が起動しない**。

### 検証して問題なしだったもの（#77 の宿題を1つ閉じた）

- **✅ Free プランの期間上限は正しく効いている**（#72〜#77 で6回申し送られていた「実測未達」の1つ）
  `?plan=free` で標準テスト店舗（期間2件・Free上限1件）を開き、**実際にボタンを押して**確認した。「＋ 新しい期間を作成」を押すとフォーム自体は開くが、**プリセットを選んだ時点でアップグレードモーダルが出て期間は作られない**（実測: 期間は2件のまま）。コード側も `create()`（:1903）とプリセット（:1970）の**両経路が `checkPeriodLimit()` を通っている**ことを確認した。上限表示も `（2/1件）` と、上限を超えた既存店舗の状態を正しく出す。**バグではない。**
  🟢として1点だけ: **上限に達していてもフォームが開く**（止まるのは選択した瞬間）。データは作られないので実害はないが、押してから止められる形ではある。ボタン段階で止めるかはUXの選択なので申し送りに留める。

### 実測して数字を出したもの（→ BACKLOG化済み・追記）

- **コントラストのベースラインを全数測定した**（#72・#75・#76・#77 の「決めること1〜5」が、ようやく「あとは色を選ぶだけ」の状態になった）
  375px・管理者画面の6タブ巡回・実効背景を祖先合成して算出。**閾値未満のユニークな組み合わせ: ライト Free 48件／Premium 75件、ダーク Free 39件／Premium 56件。** 既に BACKLOG に書いてあった代表値（2.15:1 / 2.85:1 / 2.78:1 / 2.15:1）は**すべて今回の実測と一致**した＝記載は正しく、不足していたのは全体量だけだった。BACKLOG のタスクに表として追記済み（実装後に同じ手順で測り直せば効果がそのまま件数差で出る）。
- **測り方で1つハマった（次回のために残す）**: ブラウザの `prefers-color-scheme: dark` を指定しても**ダークにならない**。`applyTheme(lg(THEME_KEY,"light"))`（app-core.js:205）で**既定が明示的に `"light"`** なので `data-theme="light"` が付き、メディアクエリを打ち消す。最初の測定はこれを踏んで**ライトの数字をダークとして出しかけた**（ダークとライトの件数が1件も違わないので気づけた）。ダークを測るには `localStorage.ots_theme='"dark"'` を先に入れる。
  ついでに分かったこと: **OSがダークの利用者も、設定タブで「自動」か「ダーク」を選ぶまではライトで表示される。** テーマ設定UIには3択（自動／ライト／ダーク）が揃っているので、これは既定値の選択であって不具合ではない。
- **決めること4の新しい実例（両テーマで未達）**: 期間タブの**スタッフ配布用URLの表示**が、固定の明るい背景 `rgb(240,240,240)` にテーマ追随の `--c-text3` を載せていて **ライト 2.23:1 ／ ダーク 2.25:1**（11px）。管理者が送信前に目視確認する文字列。BACKLOG に追記済み。

### 要確認（未修正）

- **🟢 スタッフ用の `PLAN_LIMITS` フォールバックだけが Free の実値と食い違う**（[app-admin.js:2351](app-admin.js) の `PLAN_LIMITS[plan]?.staff??10`）
  `PLAN_LIMITS.free.staff` は **20** なのに、フォールバックは **10**。期間側（`??1`）は `free.periods` と一致しているので、**staffだけが揃っていない**。`plan` state の初期値は `"free"`（app-main.js:74）なので現状このフォールバックには到達せず**実害はない**が、Firebase が想定外のプラン名（`trial` 等）を返した瞬間にスタッフ上限が黙って半分になる。1トークンの修正だが、**到達経路が現存しない＝「原因だった証拠」を示せない**ため今回は触っていない。
- **🟢 Free 上限に達していても期間作成フォームは開く**（上記のとおり。止まるのは選択時・データは作られない）
- **🟢 CFの `linkStoreToCompany` が shopId の文字種を検証していない**（#90 から継続・変化なし）。CF側は `async` ハンドラなので rejected promise になり固まらない（理由が具体的でないだけ）。**修正には CF のデプロイ（ユーザー操作）が要る**ため申し送り。
- **🟢 `name|date` 複合キーの分解が2箇所で食い違う**（[app-admin.js:1637](app-admin.js) `indexOf` / [:3063](app-admin.js) `lastIndexOf`）。#90 から継続・変化なし。
- **🟢 `effShiftStart`/`effShiftEnd` の未採用が app-admin.js になお3箇所**（`:1009` / `:3055` / `:3113`）。**#82〜#90 と同じ理由で今回も触っていない**——直し方が BACKLOG の判断待ち項目（片側セルだけの日を何時間と数えるか）に依存する。
- **🟢 配信版数（`?v=` と `build:`）は6箇所とも `20260822-e9d98a4` で一致しているが、今回 app-main.js が変わった**ため次のリリースでバンプが要る。申し送りのみ（`/release-to-main` の手順63が標準工程として持つ。BACKLOG化はしない）。
- 以下は変化なし・すべて BACKLOG化済み: **コントラスト5件**（#72・#75・#76・#77／今回ベースラインを追記）／**タップ領域44px未満**（#74）／**休憩の適用条件が画面の説明文と食い違う**（#89）／**期間の確定（写し）が `periods` を肥大化させる**（#88）／**別名で提出した人の再提出でsubが2つできる**（#81・コード修正済み・Excel実物の確認と本番データ確認が残）／**スタッフURLを開いた誰でも他人の提出を削除できる**（#70）／**スタッフ削除の後始末がリネームと正反対**・**未登録名が画面に出ずExcel/PDFにだけ出る**（#79）／**期間の重複にバリデーションが無い**（#83）／**`x` セルが店舗間重複の判定から除外されていない**（#85）／**店舗間重複の判定が退勤延長と片側セルを見ていない**（#86）／**解約イベントだけが metadata でプラン判定**（#68）／**企業連携の解除が店舗をオーナー0人に戻す**（#65）／**Pro→Premium の二重課金の根治**・**特商法表記**（BACKLOG 🔴）
- 以下は #46〜#90 からの継続（変化なし）: 未claim店舗の取り込み／`findActiveSubscription` の `|| live[0]`／`planExpiry` のUTC計算／`purgeOldPeriods` のUTC日付／`generateInviteCode` のデッドコード／ほか

### 今回の対象と見つけ方

**#90 が名指しで残した宿題から入った。** 「`fbSet(...).catch(...)` の10箇所について、その path に自由入力が届く経路があるか1つずつ潰す。スタッフ名（#89）と店舗コード（#90）で2つ見つかっており、**3つ目がある前提で数えるのが妥当**」という指示だった。

**(1) 宿題そのものは空振りに近かった。** 補間を含む `ref()` を全数（28箇所）洗い出して入力元を辿ったが、`fbSet` 系の書き込みパスに届く自由入力はもう無かった。店舗コードの3経路（app-admin.js:29／app-main.js:1259／app-main.js:1027）は #90 で塞がれており、ポジション名は**配列の要素であってキーではない**、店舗略称も値、`companyId`・`uid`・shopId は生成物かDB由来。**書き込み側はきれいだった。**

**(2) レンズを「書き込み」から「読み取り」へ回したら出た。** #89・#90 はどちらも**ユーザーが打った文字列**が起点だったので、無意識に「入力欄」を探していた。しかし `firebaseKeyForbiddenChars` が守るべきなのは「入力欄」ではなく「**外から来た文字列が Firebase のパスになる場所**」で、**Shifty には打鍵を伴わない入口がもう1つある——URL の hash である。** `parseUrl`（app-core.js:151）は `h.slice(4)` を返すだけで一切検証しない。しかもそれは**プロダクトで最も使われる入口**（スタッフはURLを開くことしかしない）だった。

**(3) 「同じバグの3件目」ではなく「同じバグの一番深いところ」だった。** #89 は保存が黙って失われ、#90 は入力欄が「確認中...」で固まる。今回は**アプリが起動しない**。深さが違うのは、throw する場所が**イベントハンドラの中ではなく初期化のトップレベル**だから。React のイベントハンドラで throw してもその操作が失敗するだけだが、Phase1 で throw すると**後続の初期化が丸ごと失われる**。**同じ構造のバグでも、どのライフサイクルで起きるかで被害が変わる。**

**(4) watchdog があるのに効かない、が最後の一押しだった。** コードには「アプリ内ブラウザで10秒固まったらエラー画面へ」という保険が明示的に書いてある。それがあるのに固まり続けたので、保険が外れる順序（`clearTimeout` → `loadShops`）を疑うことができた。**「保険があるのに効いていない」は、保険を外すタイミングを見に行く合図。**

**(5) 実測系の宿題も1つ閉じ、1つ数字を出した。** #90 が「次回は据え置かれてきた実測系を一気に片付けるのが最も効率がよい」と優先順を①Free上限 ②コントラスト ③タップ領域 と書いていたので、そのとおり①と②をやった。①は**バグではなかった**（正しく効いていた）と確定でき、②は**ベースラインの件数**が出た。**「実測不能」を理由に据え置かれていた項目は、手段が戻った時点で「未検証」ではなく「検証して問題なし」か「数字あり」に変えられる。**
...
- 読み込み順維持（utils→core→staff→admin→main）・SRI **11本**／新規追加した `input` **0件**
- `npm test` **180件パス**（修正前後で増減なし）／`npx eslint app-*.js` **0 errors 98 warnings**（増減なし）／`node --check functions/index.js` OK
- Cloud Functions: `secrets` の抜け漏れ **なし**（6箇所）／`.delete()` の誤用 **0件**／`purgeInactiveShops` の安全装置（`Number.isNaN` スキップ・`archived/shops` 二段削除）維持
- **補間を含む `ref()` を全数（28箇所）棚卸しし、入力元を1つずつ辿った**。自由入力が届くのは店舗コード3経路（#90で対処済み）と**URLトークン1経路（今回対処）**のみで、残りは生成IDかDB由来のキー
- **`ref()` の同期throwを実SDK（firebase-admin）で実測**: `.` `#` `$` `[` `]` で同期throw、`/` と全角句点（`。`）はthrowしない、空文字は親ノードの読みになる（`parseUrl` の `if(token)` で到達しない）
- **実ブラウザでアプリを起動して実測**（devサーバー無し・`route.fulfill` で配信物を供給）: 修正前後の3パターン比較、Free上限の実クリック検証、コントラスト4条件（plan×theme）×6タブの全数測定。**いずれもコンソールエラー0件**
- `readShop` の呼び出し元**5箇所すべて**が `null` を処理済みであることを確認（:207 / :221 / :243 / :267 / :285）
- **未検証**: iOS Safari 実機（WebKitエンジンでの再測定は今回未実施・Chromiumのみ）／スタッフ数20名超での上限UI（テスト店舗は4名。**上限に達した状態を作るとdevデータを汚すため未実施**）／Excel・PDF ファイルを実際に開いての中身確認／本番データの確認（ユーザー操作）

### 総括

**#90 の宿題は書き込み側では空振りだったが、レンズを読み取り側へ回したところ、同じ構造のバグの一番深い個体が出た。**

**今回の分かれ目は「入力欄を探すのをやめた」こと。** #89・#90 が2回続けて「ユーザーが打った文字列」だったので、3件目も入力欄にあると思い込みかけた。実際の条件は「**外から来た文字列がFirebaseのパスになること**」であって、打鍵は必要条件ではない。Shifty の設計原則1（スタッフに登録を求めない＝URLを開くだけ）が、そのまま「**打鍵のない最大の入口**」を作っている。**設計原則は、そのプロダクト特有の攻撃面・故障面の在り処でもある。**

**同じ構造でも、起きる場所で被害が変わる。** `.catch` が同期throwを拾えないという1つの誤解から、#89では保存の黙殺、#90では入力欄の停止、#91ではアプリ全体の起動不能が出た。差はライフサイクル上の位置だけである。**「同種のバグを潰した」と言うときは、種類ではなく深さで数え直したほうがよい。**

**保険が効かなかったことが手がかりになった。** 10秒 watchdog は正しく実装されていたのに救えなかった。理由は `clearTimeout` が `loadShops` の**直前**に置かれていること——保険を外してから危険な処理に入っている。**保険が空振りしたときは、保険の中身ではなく外すタイミングを見る。**

**次回の手。** 実測の手段が定着したので、残る据え置きを続けて片付けるのが効率的。優先順は ①**タップ領域44px の実測**（#74・今回は未実施。手順はコントラストと同じで、`getBoundingClientRect` に替えるだけ）②**WebKit での再測定**（今回はChromiumのみ。ユーザーの主戦場は iOS Safari で、#75・#76 は両エンジン一致を確認しているが今回は取っていない）③**スタッフ20名超の上限UI**（devデータを汚さない作り方——一時店舗を作って測って消す——を決めてから）。
コード側の新しいレンズを1つ挙げるなら、**「保険・タイムアウト・リトライが書かれているのに救えない経路」を数える**。今回の watchdog がそうだったように、**保険の存在は「ここは失敗しうる」という作者の申告**であり、その申告と実際の防御範囲がズレている場所は他にもありうる（`.catch(()=>{})` で握り潰している10箇所、`try{}catch{}` で `console.warn` だけして続行している初期化4箇所が候補）。
