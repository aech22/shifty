...
## Shifty バグチェックレポート（2026-08-25 自動実行 #96）

> 対象は #95 の直後の2コミット（`93c1022`・`b2220fe`）と、そこから広げた管理系の書き込み経路。着手時の HEAD は `2e8e9aa`。

### 修正済み

- **🟡 管理系の書き込みが拒否されても成功としか表示されない**（[app-main.js:1110](app-main.js) `revertAdminWrite`）→ `14194e9`
  前回の `b2220fe` は**提出データの削除だけ**を手当てしたが、**同じ形が管理系の5経路すべてに残っていた**。`settings`／`periods`／`staff`／`templates`／`global-shops` はいずれも `shops/{shopId}/owners/{auth.uid}` 登録端末しか書き込めない一方、クライアントは**ローカルstateを先に書き換え**、呼び出し元は成功前提で「削除しました」を出す。拒否は `.catch(console.warn)` に握られ、画面には成功表示だけが残っていた。
  **実測①（REST・dev）**: 新規の匿名uid（＝閲覧専用端末と同じ状態）から `periods`／`staff`／`settings`／`tokens` へ書くと**すべて HTTP 401**。残骸なし。
  **実測②（実ブラウザA/B・dev標準テスト店舗）**: 配信物をそのまま読み込み、**店舗コードだけで参加＝オーナーにならない**端末で期間を削除。**修正前は「削除しました」だけでエラー0件／修正後は「△ 保存できませんでした（この端末は管理者として登録されていません）」が出る**。dev のデータは periods 2件・subs 7件・staff 5件で着手前と同一。
  あわせて `savePeriods` の subs 連鎖削除と `once("value")` の **unhandled rejection** を塞いだ。権限以外の失敗（`.validate` 等）は文言を分けている。
  **見立ての訂正**: 「拒否されると画面が削除済みのまま残る」と読んでいたが、**実測では RTDB SDK が巻き戻して `value` を流すため行は一瞬消えて自動で戻った**。実害は「取り消せない操作が成功と表示され、失敗が一切伝わらない」ことに限られる。

### 要確認（未修正）

- **🟢 期間タブの「編集」と「Excel」が 375px で 5.0px しか離れていない**（[app-admin.js:2065](app-admin.js) `gap:5`）。#95 の申し送り②を実施して検出。`d22653c` が変えたタブバーの `gap`（4→8px）と削除ボタンの `marginLeft:10` は**8px未満の組0件**＝意図どおり効いている。破壊的操作の隣接ではないため 🟢。寸法はユーザー判断で決めてきた経緯があるため変更していない。
- **🟢 配信版数 `20260825-92a9e27` の後に app-core.js・app-admin.js・app-main.js が動いた**ため次のリリースでバンプが要る（`/release-to-main` 手順63が持つ）。
- 変化なし・**BACKLOG化済み**: 片側セルのヒートマップ／スタッフ削除の後始末／「閲覧専用」端末の可否（#68・**今回「無言で成功」の部分は解消**、残るはUIを止めるか文言かの判断のみ）／別名提出の重複（#81）／二重課金・特商法（🔴）／解約時のプラン判定／企業連携の解除。

### 検証したこと

- **各コミットの「何箇所に入れた」を grep で検算**（#95 の手①）。`93c1022` の「入口は3つ」→ `validatePeriodDates` は :1933・:2007・:2048 の**3箇所**、期間生成は :1937・:2011 の**2箇所**＝取りこぼしなし。`b2220fe` の「管理者側は2箇所」→ 第2引数付きの呼び出しは :1961・:3421 の**2箇所のみ**＝取りこぼしなし。**検算は2件とも空振りで、当たりは「隣の経路に同じ形が残っている」ほうだった。**
- `DEV_MODE` 式のまま／`subs` の `set()` 全体上書き **0件**／`global/shops` 全件読み **0件**／`".read": true` **0件**／読み込み順・SRI **11本**／今回の差分に input 追加**0件**・`fontSize`16未満**0件**。
- CF: `secrets:` **6箇所**（SMTPは2箇所ともUSER/PASS揃い）／`.delete()` 誤用 **0件**／purge の `Number.isNaN` ガードと `archived/shops` 二段削除が健在。
- `npm test` **197件パス**／`npx eslint app-*.js` **0 errors 95 warnings**／`node --check functions/index.js` OK。
- **未検証**: Excel・PDF の実物確認（**4回連続**の申し送り）／iOS Safari 実機／候補・設定・マイページの3タブはタップ測定で開けず未測定／本番データ。

### 総括

**「同じ形が隣の経路に残っていないか」を見るのが今回いちばん効いた。** #95 の手①（コミットの主張を grep で検算）は実行したが**2件とも取りこぼしゼロ**。当たりは一段外側で、`b2220fe` が直した「拒否されたのに成功と表示する」という**形そのもの**が管理系5経路に残っていた。修正の正しさを確かめるだけでは出ず、**形を抽象化して同じ場所を数え直して初めて出た**。
**もう1つは、読んで立てた仮説を実測が否定したこと。** コードからは「画面が削除済みのまま残る」と読めたが、実ブラウザでは行は自動で戻った。仮説のまま報告していれば**実際には起きない症状で重大度を付けていた**。推測で直さない原則は**症状の側にも同じだけ要る**。
**次回の手**: ①今回確立した「閲覧専用端末を実ブラウザで作る手順」を再利用して #68 の判断材料（セル編集が実際に通るか）を採る ②Excel・PDF の実物確認 ③開けなかった3タブをオーナー端末で測り直す。
