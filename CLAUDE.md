...
**(5) 副産物として否定した仮説**: `<tr>` への inset box-shadow が `border-collapse:collapse` で描画されないのでは（Phase 3 が超過行の目印を `background` から変えた。外れると**目印が消えたことに誰も気づけない**）→ **同一クリップのスクリーンショットをバイト比較する方法で Chromium・WebKit 両方で描画を確認**。**ただし後日の追試で条件依存と判明: WebKit は `<td>` に border があるときだけ描画する**（border を外した簡略再現では描画されない。Chromium は両方で描画）。SubsTab の td は `borderBottom` を持つので現状は問題ないが、**セルの境界線を外す変更を入れると Safari でだけ超過行の目印が消える**（app-admin.js:2991 と :2993 の td は一緒に扱うこと）／`e785d22` で `logout` prop が死んだのでは → app-admin.js:102 でフォールバックとして生存／Phase 2 が消した「空要素4箇所」は本当に空か → **生バイトで `><` を確認**、しかも5ファイル分割時点から空／SmModal のセル要素削除で行の高さがズレるのでは → 両コンテナとも `height:72` 固定／Phase 5 で利用規約の条文が欠けたのでは → `TERMS_TEXT` と `terms.html` の**全20条の見出しが完全一致**。
