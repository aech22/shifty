...
## Shifty バグチェックレポート（2026-08-20 自動実行 #85）

### 修正済み

- **🟡 店舗間シフト重複の判定が、別名で提出されたぶんを見落として一度も発火しなかった**（[app-admin.js:395](app-admin.js)）→ `fb91849`
  企業連携している他店舗のシフトを持つ `companyData[].workMap` は、`sub.staffName` を**生のまま**キーにしていた（:405）。`sub.staffName` には別名がそのまま入る（`registerAlias` は `staffAliases` に登録するだけで `staffName` を書き換えない）。一方、参照側の `dupErrors`（:861）は `workMap.get(name+"|"+date)` の `name` に **`realStaff` 由来の登録名**を渡す。したがって**他店舗で別名を使って提出している人は、キーが一致することが原理的にありえず、重複が常に検出されない**。
  **同じ形のインデックスで、自店舗側は全部フォールバックを持っていた**——`_getSubForPeriod`(:513)・`_getWorkShift`(:525)・`_shiftAt`(:3000) はいずれも「登録名で引き、外れたら別名を順に試す」形で揃っており、`getStoredTime`→`_getSub` もこの経路に乗る。**規約から外れていたのは他店舗ぶんの `workMap` 1箇所だけ**で、設計判断は不要だった。
  **配信物の該当ブロックを切り出してNodeで実行して断定した**。他店舗で「たなか」（登録名「田中」の別名）が 10:00〜18:00 に勤務、自店舗は「田中」を 12:00〜20:00 に配置——実際には**6時間重なっている**のに、`workMap` のキーは `たなか|2026-08-20` で、登録名での参照は**null（エラーなし）**。同条件で別名を使わない場合は正しく「二号店」を返すので、**別名の有無だけで検出の有無が変わる**ことを対照付きで確認した。
  **修正**: 他店舗自身の `settings/staffAliases` を追加で読み、`resolveAlias` で登録名へ解決してからキーにする。`settings` の読みは `auth != null` で、既に読んでいる `settings/shopAbbrs` と同じ権限のため新しい露出はない。`resolveAlias` は一致しなければ入力をそのまま返すので、**別名を使っていない店舗の挙動は変わらない**。非回帰8ケース（別名なし・時間が重ならない・休み希望マーク・両方揃ったシフトの優先・status非work・alias未設定(null)・無関係の提出者）を配信物の実評価で全パス。

### 要確認（未修正）

- **🔴（継続・プロセス）このループから実機E2Eが8回続けて実行できない**（環境要因・コード変更なし）
  今回も `preview_list`（稼働0件）と `preview_start` を実際に実行し、**「Dev servers can't be started from unattended sessions」と明示的に拒否された**。Bashでのサーバー起動は規約で禁止のため代替なし。**#78〜#84 に続き今回も1件も実行できなかった。ユーザーが対話セッションで `/bug-check` を手動実行すれば解消する。**
- **🟡 #77 の宿題（Free 上限の実機検証）は静的監査完了・実測のみ未達**（上記🔴が理由）。**残るは実機で21人目・2件目を押す1回だけ**
- **🟡（新規）`x`（ヘルプ・カウント外）のセルが店舗間重複の判定から除外されていない** → BACKLOG化済み（仕様判断待ち）
  同じ `realStaff` を回す3計算のうち、`heatData`(:779) と `positionErrors`(:896) は `isCountExcluded` で `x` を除外するが、**`dupErrors` だけが呼んでいない**。`dupErrors` は**もう一方のヘルプ（店舗略称の明示指定）は既に除外している**（:852）ため、「ヘルプ」を指す2つの入力のうち片方だけが除外される形になっている。ただし `x` の `desc` は「時間帯別出勤人数に数えない」としか書いておらず**「他店舗にいる」とは言っていない**ため、誤検出なのか本物の衝突なのかは運用次第＝ユーザー判断。**今回のコミットとは独立**（`workMap` の修正はキーの一致だけを直しており、`x` の扱いには触れていない）
- **🟢（申し送り）`_dayMin`(:3112) と `_min`(:3050) は乖離していない**（#84の宿題・確認完了）。`:3087` が `setDet({...sub,staffName:resolvedName})` と**解決済みの登録名を詰めてから**モーダルへ渡しているため、`_dayMin` が `det.staffName` で引いても登録名になる。**両者とも `_shiftAt`＋登録名で一致**しており、少数派は生まれていない
- **🟢 `effShiftStart`/`effShiftEnd` の未採用が app-admin.js になお3箇所残る**（`:1000` ヒートマップの時間軸／`:3039` 提出一覧の出勤日数／`:3097` 詳細モーダルの出勤日数）。**#82・#83・#84 と同じ理由で今回も触らなかった**——直し方が BACKLOG の判断待ち項目（片側セルだけの日を何時間と数えるか）に依存する
...
- 以下は #46〜#84 からの継続（変化なし）: 未claim店舗の取り込み／`findActiveSubscription` の `|| live[0]`／`planExpiry` のUTC計算／**片側セルだけの日が0分**／`purgeOldPeriods` のUTC日付／`generateInviteCode` のデッドコード／ほか

### 今回の対象と見つけ方

**#84 以降のコミットは0件。13回続けて「差分ゼロ」の回である。**

**(1) 申し送りの指示をそのまま実行し、5マップを1つずつ潰した。** #84 は「まだ `staffColors`・`staffNumbers`・`staffPositions`・`staffWorkplaces`・`staffAliases` の5つを同じやり方で見ていない」と名指しで残していた。実際に5つとも呼び出しを並べたところ、**5つのうち4つは白だった**——`staffPositions`(:886) と `staffWorkplaces`(:846) は `realStaff`（登録名）を回すだけ、`staffColors`/`staffNumbers` は Excel(:2067) と PDF(:1295) の**両方が `!allAliases.includes(n)` で別名を列から除いており、2つの実装が一致していた**。**申し送りが指した場所そのものは空振りだったが、その過程で「同じ形なのに別名フォールバックを持たないインデックス」が1つだけ視野に入った。**

**(2) 決め手は「自店舗の同型インデックスは全部フォールバックを持っている」という多数派の存在だった。** `_getSubForPeriod`・`_getWorkShift`・`_shiftAt` の3つは、いずれも `staffName` を生のキーにしたうえで**読み出し側に別名フォールバックを置く**という同じ設計になっている。`workMap` だけが同じ「生キー」で作られながら**読み出し側にフォールバックが無い**。#84 と同じく、**多数派へ揃えるだけで済み、設計判断が要らなかった**。

**(3) 「他店舗のデータだから」という理由で調査対象から外していた。** `workMap` は #82 で `hasBoth` の休み希望マーク対応を入れたときに一度触っており、#83・#84 の「同型インデックスの棚卸し」でも `subsByKey`・`workShiftByStaffDate`・`shiftByStaffDate`・`workMap` の4つを並べて**重複解決規則は突き合わせていた**。しかし見ていたのは「重複時にどれを採るか」だけで、**キーそのものの作り方は3回とも素通りしていた**。同じ4つを3回並べても出なかったものが、軸を変えた1回で出た。

**(4) 修正が権限の壁に当たらないかを、直す前に確認した。** 他店舗の `settings/staffAliases` を読む必要があったため、`database.rules.json` の `settings` の `.read` が `auth != null` であること——つまり**既に読んでいる `settings/shopAbbrs` と同じ権限で、新しい露出が発生しないこと**——をルールファイルで確認してから実装した。読めない値を前提にした修正を書くと、動かないうえに原因がクライアント側に見えない。

**(5) 自分のテストが落ちたとき、コードではなくテストを疑って正解だった。** 非回帰8ケースのうち「休み希望マーク付きは検出しない」が最初 FAIL したが、原因は `adminRest` を `"full"` という文字列で書いた**テスト側の誤り**で、実際の形は `{start:true,end:true}`（`effShiftStart` の実装で確認）だった。**配信物の定義を読みに行ってからテストを直した**ので、実装を誤って「直す」ことにはならなかった。
...
- **名前キーのレンズ（今回の主レンズ・#84の申し送りを継続）**: `staffColors`・`staffNumbers`・`staffPositions`・`staffWorkplaces`・`staffAliases` の5マップについて全参照を並べ、引くキーが登録名か生のsub名かを分類。**4マップは白**、`workMap` 経由の1件のみ違反
- **同型インデックスの再棚卸し（キーの作り方の軸）**: `subsByKey`(:491)・`workShiftByStaffDate`(:498)・`shiftByStaffDate`(:3005)・`workMap`(:395) の4つについて、**読み出し側に別名フォールバックがあるか**を確認。自店舗の3つは `_getSubForPeriod`(:513)・`_getWorkShift`(:525)・`_shiftAt`(:3000) で全て有り、`workMap` のみ無し（→修正）
- **#84 の宿題**: `_dayMin`(:3112) と `_min`(:3050) が乖離していないことを確認（`:3087` で `det.staffName` に登録名が入るため両者一致）
- **権限の事前確認**: `database.rules.json` の `shops/$shopId/settings` の `.read` が `auth != null`＝既存の `shopAbbrs` 読みと同権限であることを確認
- **修正の検証（配信物の実評価）**: 別名提出時の重複検出（修正前 null → 修正後「二号店」）／別名なしの対照／非回帰8ケース全パス
- **未検証（今回は手段が無い）**: ブラウザでの実描画すべて／**修正後に実際に赤セルとエラーパネルが出ること**／**2店舗を実際に企業連携した状態での動作**／**Free 上限に達した店舗での実ブロック**／実機 iOS Safari

### 総括

**測れない回が8回続き、それでも 🟡 が1件出て直った。** 根は #81・#84 と同じ——**`sub.staffName` には別名がそのまま入る**——だが、これまで閉じてきたのが「自店舗の集計」だったのに対し、今回は**他店舗から読んだデータ**だった。#81 が別名レンズで9箇所、#84 が設定マップの引き方で3箇所を閉じ、いずれも**自店舗の中だけを見ていた**ため、`companyData` の取得経路は3回とも視野の外にあった。

**実害は「黙って出ない」形なので、気づく手がかりが画面に残らない。** 数字が違うのではなく、**出るべき赤セルとエラーパネルが出ない**。しかも別名を使っていない店舗では正常に動くため、機能自体は「動いている」ように見える。企業連携＋別名という2条件が揃った店舗でだけ、**同じ人を2店舗の同じ時間帯に入れてしまう**——この機能が防ぐために存在する事故が、そのまま通る。

**申し送りが空振りでも、実行したこと自体は無駄ではなかった。** #84 が名指しした5マップは4つが白で、**「まだ見ていない場所」を見ても何も出なかった**。しかしその作業の中で `workMap` が視界に入り、**軸を「引くキーが登録名か」から「読み出し側にフォールバックがあるか」へずらした**ところで出た。#84 が「レンズを尽くすとレンズをずらすは違う」と書いた形が、今回もそのまま再現した。

**次回の手を1つに絞る。** 実測が復活したなら**#77 の宿題（実機で21人目・2件目を押す1回）をそのまま実行する**。使えないままなら、推奨は**「他店舗・他期間から読んだデータを、自店舗の名前・IDで引いている箇所を全部並べる」**——今回の1件は「自店舗の中では揃っているのに、外から来たデータだけ規約から外れていた」形だった。`companyData`（略称 `abbrs` の突き合わせ・`getHelpInfo`(:731) の略称→店舗解決）と `shopMeta`（CompanyTab・:3162 で他店舗の `staff` と `staffWorkplaces` を読む）は**同じ「他店舗から読む」経路なのに今回は重複判定しか追っていない**。あわせて、**今回1つ増えた `otherAliases` が次回の少数派になっていないか**を見ること。
...
**影響範囲**: app-utils.js（`carryAdminShiftFields`・`calcNetWorkMinutes`・`resolveAlias`・`CELL_COMMANDS`）、app-staff.js（`buildShift`）、app-admin.js（集計・上限判定・ポジション設定UI・`del`:2314・`gridStaff`:1060・`heatData`:762・`expXl`:2015・`buildPdfCols`:1289・**`PEF`:1999 と期間プリセット生成**・**`dupErrors`:846**）、tests/core.test.js
**備考**: バグチェック #46・#52・#57・#64・#65・**#79**・**#83**・**#85** で継続検出・**条件B（仕様判断）に該当**。各項目とも「実装は難しくないが、正解が仕様側にしかない」。**#79 の2項目はどちらも「スタッフを削除する」という1つの操作から出ている**が、片方は設定データの後始末、もう片方は未登録名の表示規則で、決め方が独立するため分けて記載した。なお #79 では**確認ダイアログの不在だけ**（他の破壊的操作4箇所は全て confirm を持つ＝正解がコードベース内にあった）を判断待ちにせず `69f62bf` で修正済み。
