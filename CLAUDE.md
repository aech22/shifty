...
## Shifty バグチェックレポート（2026-09-01 自動実行 #102）

> 着手時の HEAD は `bda7681`（リリース直後）。#101 以降に入った実コード変更は多い
> （`0c236ac` 提出権限を名前で判断・`64d5b74`/`59343ca` 休憩の適用範囲・`08a531e` 設定の後始末・
> `9291e1a` billingExempt・`19f7fc5` 削除ポップアップの既定・`90ceea8`/`c340d5c` マイページの出し分け）。
> **どれもコミットメッセージに実測が付いていて、そこは追試しても崩れなかった。**
> 見つかったのは、**それらの変更が届かなかった隣**だった。

### 修正済み

- **🟡 出勤セルだけ入力した日に、ランチではなくディナーの退勤延長が足されていた**（[app-utils.js:346](app-utils.js)）→ `6613c78`

  #82（2026-08-25 案A）で片側セルの日も「補完して数える」ようになったが、`getOT` は補完前の
  生の `adjustedEnd ?? end` で帯を判定したままだった。出勤セルだけの日は `end` が無いので
  `if(!en)return dinner` に落ち、**`calcNetWorkMinutes` が同じ日をランチ帯として補完して数えている
  その値に、ディナー側の延長が足されていた。**

  **穴は既に名指しされていた。** [app-admin.js:856](app-admin.js) のコメントが
  「片側セル（出勤だけ）の日は実効退勤が空文字なので、hsh をそのまま渡すと getOT が一律ディナー扱いになる」
  と書いており、対処として `{...hsh,adjustedEnd:minToHHMM(pEnM)}` を明示的に渡していた。
  ただし**その回避はヒートマップの1箇所だけ**で、勤務時間・週/月の上限判定・店舗間重複判定（:932）には
  届いていなかった。**同じ帯を3箇所が別の式で判定していて、正しいのは1箇所だった。**

  修正は `getOT` 自身を `effShiftRangeMin` 経由にした（`calcNetWorkMinutes`・`shiftBandInfo` と同じ入口）。
  補完できない日は従来の生の値へフォールバックするので、既存の呼び出しの答えは変わらない。

  **実測A（node・候補 09:00-15:00 / 17:00-23:00、田中の延長 ランチ+15分・ディナー+60分）**

  | シフト | 実効レンジ | getOT | 純勤務 |
  |---|---|---|---|
  | 出勤セルだけ 09:00 | 540-900 | 60分 → **15分** | 7:00 → **6:15** |
  | 両側 09:00-15:00（**同じレンジ**） | 540-900 | 15分 | 6:15（変化なし） |
  | 両側 17:00-23:00 | 1020-1380 | 60分 | 7:00（変化なし） |
  | 退勤セルだけ 23:00 | 1020-1380 | 60分 | 7:00（変化なし） |

  **実測B（実ブラウザ・SubsTab のみマウント＝Firebase へは1バイトも出していない・詳細モーダル）**
  9/1 を片側セル（出勤だけ 09:00）、9/2 を両側 09:00-15:00 にした同一人物のsub:

  | | 修正前（`6613c78~1` の配信物） | 修正後 |
  |---|---|---|
  | 「延長」バッジ | **+60分** | **+15分** |
  | 9/1（片側） | **7:00** | **6:15** |
  | 9/2（両側・同じレンジ） | 6:15 | 6:15 |
  | 勤務計 ／ 合計 | **13:15** | **12:30** |

  **補完すると両側そろったランチのシフトと1分も違わないレンジになる日が、延長だけ45分多く
  計上されていた**（週・月の上限超過バッジに効く）。ヒートマップは回避策のおかげで元から正しかったので、
  **画面のヒートマップと勤務時間集計が違う延長を使っていた**。
  ユニットテスト1件追加（205件パス）。修正前の実装に対しては落ちることを確認済み。

- **🟡 Stripe秘密鍵の先頭12文字を Cloud Functions のログへ出していた**（[functions/index.js:66,119](functions/index.js)）→ `87f66ee`・**本番反映は未実施／BACKLOG化済み**

  **この修正は、着手時に作業ツリーへ未コミットで置かれていたものを取り込んだ**（`git status` の
  ` M functions/index.js`・mtime 2026-09-01 02:02。おそらく `diagnose-secret-corruption` 系の別セッション）。
  内容は正しく両方のログ地点が揃って直っていたが、**コミットされていないので `git checkout` 1回で失われる状態**だった。

  デプロイ済みのコードは `getStripe()` と `createCheckoutSession` の入口の2箇所で
  `key.substring(0,12)` を出す。`sk_live_` が8文字なので**実際の秘密の4文字**が Cloud Logging に
  決済のたびに蓄積する。修正後は `loaded(<バイト数>b)` / `EMPTY` だけ——バイト長があれば
  「未設定」と「二重ペースト・末尾改行混入」は判別できるので切り分けの用途は失われない。
  `substring(0` の残りは0件、他に秘密情報をログへ出す箇所も無いことを確認済み。

- **🟢 CLAUDE.md のセキュリティモデルが 2026-07-28 の締めルール切替前のままだった**（CLAUDE.md:286）→ `857041e`

  「移行猶予（現行ルール）: owners未登録（未claim）店舗は従来通り書き込み可」と書いてあったが、
  切り替えは `dbdd9d9`（2026-07-28）で実施済み。実測（`database.rules.json` をパースして確認）:
  settings / periods / staff / templates / lastActivity の `.write` はすべて
  `auth != null && …owners.child(auth.uid).exists()` で、**未claim分岐は残っていない**。
  CLAUDE.md は毎セッションのコンテキストに載るので、これを信じたセッションは
  **もう存在しない前提**で設計判断をしうる。あわせて `0c236ac` のトレードオフ
  （subs の write は認証済みなら誰でも通る・**再検出しても直さない**）も同じ節に明記した。

- **🟢 スタッフ削除の後始末のコメント2行が `08a531e` の書き換えで重複していた**（[app-admin.js:2665](app-admin.js)）→ `857041e`。
  「設定も戻せる状態にしておく」と「ここでは消さない」の二通りに読める状態だった。#101 の `ed70a29` と同じ形。

### 要確認（未修正）

- **🟢 `database.rules.tightened.json` が現行ルールとバイト同一の残骸**（`cmp` で確認）→ **BACKLOG化済み**。
  切り替えは完了しているのに、ルールを変えるたびに両方を手で同期する必要がある
  （`0c236ac`・`8384467` はどちらも両方を編集している）。`firebase.json:7` は
  `database.rules.json` の1本しか参照していないので削除しても配線は壊れないが、
  **消すか残すかはリポジトリ構成の判断**（条件B）。

- **🟢 片側セルの日は「→15:15（+15分）」の実効退勤バッジが出ない**（[app-admin.js:3533](app-admin.js)）。
  `effEnd` の条件が `(s.adjustedEnd??s.end)` を要求するため、退勤の無い日はバッジが省かれる。
  **分の計算は正しい**（合計に +15分 が入っている）ので実害は「なぜ合計が増えたか画面から辿れない」だけ。
  上の 🟡 を直した結果として見えるようになった粗さで、直すなら補完後の時刻を出すことになる。

- **🟢 期限切れの後始末が、より古い期間にしか残っていない人の設定を消しうる**（[app-admin.js:2533](app-admin.js)）。
  `expiredRetained` は最新3期間だけを見るので、4つ目以降の `keepStaff` に載っている人の設定7マップも
  一緒に落ちる。その期間が写し（snapshot）を持っていれば表示は写しから復元されるが、
  **一度もシフト作成タブを開かないまま終わった期間は写しを持たない**ので、従業員番号・退勤延長が消える。
  2026-08-31 決定4・案Aの範囲設定そのままなので**仕様判断**（条件B）。実害は限定的。

- **引き受け済みのトレードオフ（再検出しても直さない）**: `subs/$subId/.write` は
  `auth != null && $shopId !== 'demo-toriMatsu-v1'` だけで、提出を触れるのを本人に絞っているのは
  UI（`canTouch`）のみ。RESTを直接叩けば迂回できる。2026-08-31 決定1で承知のうえ引き受けたもの。

- **変化なし・BACKLOG化済み**: 二重課金の根治／特商法表記（🔴 2件）／解約通知（`customer.subscription.updated`）／
  解約時のプラン判定／企業連携の解除／別名提出の重複（#81）／スタッフ削除の残り確認2件。

### 検証したこと

- **`getOT` の帯判定を3レイヤーで実測**: node（純粋関数）→ ユニットテスト（修正前には落ちる）→
  実ブラウザの SubsTab 詳細モーダル（修正前の配信物との A/B）。**「原因だった証拠」が3つとも同じ数字を指した。**
- **AdminView を単体マウントして8タブを巡回**（`app-main.js` を読み込まない＝Firebase へは1バイトも出ない）。
  `premium/1400×950`・`premium/375×812`・`free/375×812`・`billingExempt=true` の4条件で
  **pageerror・console.error ともに0件**、**`fontSize<16px` の input 0件**。
  `billingExempt=true` ではマイページタブが消え、全タブで文字数が約72字減る（＝決済失敗バナーごと消えている）
  ——`9291e1a`/`90ceea8`/`c340d5c` の意図どおり。free と premium で 設定タブ 409字/1461字・
  スタッフタブ 21要素/42要素と差が出ることも確認（プラン境界が効いている）。
- RULES.md: `DEV_MODE` は式のまま／`subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／
  読み込み順維持／SRI **11本**／`fontSize<16` の input 追加 **0件**。
- CSS変数: 未定義参照 **0行**／ダーク定義の二重管理（`[data-theme]` と `prefers-color-scheme`）**差分なし**／
  SVG属性内 `var()` は既知の `ShiftyIcon` 1件のみ。
- **同じ値を複数の式が数えていないか**を横断確認: スタッフ数（`filter(!isSpacer)` に全箇所統一済み・#101 の修正が保たれている）／
  候補時間の平坦化（3つの写しが同じ規則）／`ShiftEditTab` の `settings`・`staffList` が
  `resolvePeriodMaster` で正しくシャドウされている／**リネーム・削除・期限切れ後始末の3経路が同じ7マップを扱っている**。
- CF: `secrets:` **6箇所**／`.delete()` 誤用 **0件**／`node --check` OK。
- 削除操作は全箇所が `saveSubs(..., deletedId)` の第2引数を渡している（Firebaseから消えないパターン **0件**）。
- `npm test` **205件パス**（+1）／`npx eslint app-*.js` **0 errors 95 warnings**（増減なし）。
- **devのデータは1バイトも変えていない**（実ブラウザ検証はすべて `shifty-e2e-verify` 1.6節の
  コンポーネント単体マウント。`firebaseDB` は `null` のまま）。
- **未検証**: Excel・PDF の実物確認（**10回連続**・ユーザー操作が要る）／iOS Safari 実機／本番データ／
  企業連携タブのログイン後の中身（dev への書き込みが要る）／CF の本番反映後のログ。

### 測り方を間違えた記録

タブ巡回を条件別に回すとき `for V in "PLAN=free W=375 H=812"; do env $V node ...` と書いた。
**zsh は引用符なしのパラメータ展開を語分割しない**ので、`PLAN` に `"free W=375 H=812"` という
文字列**全体**が入り、`W`・`EXEMPT` は未設定のままだった。結果、3条件が**まったく同じ数字**を返した。
気づけたのは「free と premium で同じ」がありえないと分かったからで、**受け取った値をスクリプト側から
印字させて初めて確定**した。#101 の「全件が同じ異常値を見たら、まず測定手段の故障を疑う」の変奏で、
今回は**「複数条件が同じ値を返したら疑う」**。判定に使う値は、条件を変えたときに**変わるべきものが
実際に変わるか**を先に確かめること。

### 総括

**回避策は、それが書かれた1箇所しか直さない。** 穴そのものは #82（2026-08-25 案A）で
片側セルを「補完して数える」ようにしたときに開いた。そして**昨日（2026-08-31 22:14・`59343ca`）**、
決定3の作業中に [app-admin.js:856](app-admin.js) のコメントで**正しく名指しされた**
——「hsh をそのまま渡すと getOT が一律ディナー扱いになる」。書いた人は分かっていた。
分かっていたのに、**直したのは自分が今書いている行だけ**で、`getOT` を呼ぶ**残り11箇所**
（全12箇所中）はそのままだった。
コメントで穴を説明できるということは、**関数側を直せるということ**でもある。
「ここでは回避する」と書いた瞬間が、根に戻る合図だった。

**#101 は「同じ数を2箇所が別の式で数えている」だったが、今回は3箇所だった。**
そして3箇所のうち**正しかったのは、回避策を書いた1箇所だけ**だった。
多数決が使えない——正しさは数ではなく、どの入口を通っているかで決まる。
