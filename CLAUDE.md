...
## Shifty バグチェックレポート（2026-09-07 自動実行 #113）

> 着手時の HEAD は `ca30c03`。観点は「**昨日リリースされたばかりで、まだ一度もバグチェックを通っていないコード**」。
> 別セッションが 2026-09-06 に実装し本番リリースまで行った**スタッフ非表示**（版数 `20260906-d3006ae`）が対象で、
> 機能そのものの設計は堅かったが、**設定を保存できない形が1つ**残っていた。

### 修正済み

- **🟡 期間が1件も無い店舗で非表示にすると、Firebaseの往復で非表示が消える**
  （[app-utils.js:985](app-utils.js) `_writeHiddenRanges`・`084d7cc`）

  非表示は `settings.staffHidden[名前] = [{from,to}]` という**期間の範囲**で持ち、境界には
  「その時点の最新期間の startDate」を入れる。**期間が1件も無いと下限も上限も書けず
  `{from:null,to:null}` になる**が、この範囲は**Firebaseに保存できない**——nullのキーは書かれず、
  キーの残らない空オブジェクトはノードごと消えるため、配列の要素が消え、要素が1つなら
  `staffHidden` 自体が消える。

  **失敗が静かなだけでなく、成功したと言う**。ローカルstateを先に更新するので行は「非表示」に変わり
  トーストも成功を出すが、Firebaseの購読が空の値を返した時点で**黙って元に戻る**。ポップアップは
  「これから作る期間もすべて非表示になります」と明示的に約束しているので、
  **期間を作った瞬間に、隠したはずの人が現れる**。

  実測（配信物の関数を Node で実行し、Firebase の `set()` 意味論＝nullキーの脱落・空ノードの消滅を再現）:

  | | 修正前 | 修正後 |
  |---|---|---|
  | 書く値 | `{"佐藤":[{"from":null,"to":null}]}` | `{"佐藤":true}` |
  | 往復後の `staffHidden` | **消滅** | `{"佐藤":true}` |
  | 往復後に非表示か | **false** | true |

  **正解はすでにデータ形式の中にあった**。読み側の `staffHiddenRanges` は
  「`true` ＝下限も上限もない1本の範囲」として旧形式を受けており、これは
  **保存できない範囲が表したかったものとちょうど同じ**。書く側でも、下限も上限も無い範囲が
  1つでもあれば `true` に潰す（全期間を覆う範囲は他に閉じた範囲があっても結果が同じ＝**近似ではなく同値**）。
  読み・改名・削除の後始末・解除の各経路は `true` をもともと正しく扱うため、変更はこの1関数に閉じた。

  回帰テストを1件追加（**236件パス**）。**修正前の `app-utils.js` に向けると落ちる**ことを確認済み。

### 要確認（未修正）

- **🟢 本修正はまだ本番に出ていない**（`084d7cc` は develop 止まり。リリースはユーザーの操作）。
  **本番の実害は限定的**——到達条件は「期間が1件も無い店舗」だけで、既存の稼働店舗は該当しない。
- **🟢 非表示の人は提出状況一覧（`SmModal`）に「未提出」として出続ける**。これは**仕様どおり**で、
  ポップアップ自身が効果範囲を「シフト作成タブ・Excel・PDF から名前が出なくなるだけ」と明示している。
  ただし休職者が締切まで「未提出」に並び、管理者が「全員提出済み」を読み取れない摩擦は残る。
...
**非表示機能の「絞り込みが行き渡っているか」を全経路で確認した**。コメント自身が
「**`isUnregisteredSubName` には通すな**（通すと非表示の人の提出が未登録名に化けて末尾の列として復活する）」
と警告しており、その呼び出し**4箇所すべてが名簿側（非表示を含む）を渡している**ことを確認
（`app-admin.js:1361`・`:2131`・`:2427`・`:3533`）。`expXl` は呼び出し元2箇所とも名簿を渡し、絞り込みは
関数内の1箇所で完結。ヒートマップ・集計・ポジション判定は `realStaff`（＝絞り込み後）を起点にしており、
**グリッドに出ない人が数字にだけ残ることはない**。

**設定の保存経路を確認した**。`saveSettings` は settings ノード全体の `set()` なので、キーを消した変更は
Firebaseにも伝わる（`update()` なら「削除が永続しない」既知パターンに当たるところだった）。
逆にこの `set()` であるがゆえに、保存できない値は静かに消える——今回の不具合はその裏返し。

**範囲の境界を実測で固定**（8/16から非表示・9/1で解除 → 8/01期間=表示・8/16期間=非表示・9/01期間=表示）。
旧形式 `true`・Firebaseが配列を数値キーObjectで返す形のどちらも正しく読めることも確認。

**#112 の申し送り（「`X は～済み` と別の行を根拠に書いたコメント」を集める）を実行**。該当は6箇所で
**新たに偽になっているものは無かった**。残るのは行番号のずれだけ（🟢）。

**Cloud Functions の secrets 宣言と使用を照合**。7つの `runWith` すべてが使用する環境変数を宣言しており
片落ちなし。秘密鍵のログは `loaded(<バイト数>b)` の形のまま。

**RULES.md のスキャン項目は全項目クリア**: `DEV_MODE` は式のまま／`subs` の `set()` 全体上書き **0件**／
全件読み **0件**／読み込み順維持／SRI **11本**／フォーム部品の `fontSize<16` **0件**／`.delete()` 誤用 **0件**／
`secrets` 7箇所／提出削除は3経路とも `deletedId` を渡すか `.remove()` を直接呼ぶ。
**配信版数 `20260906-d3006ae` は現在の app-*.js と一致**。

`npm test` **236件パス**／`npx eslint app-*.js` **0 errors 96 warnings**／`node --check functions/index.js` OK。
**Firebase・Stripe には読み書きとも一切アクセスしていない。**

**未検証**: 実ブラウザでの操作（今回の不具合は保存値の形の問題で、配信物の関数を Node で実行して判定した）／
PDF の実物／iOS実機／本番データ／Stripe 実データ。

### 測り方の記録

**今回の収穫は「新しい設定を足したら、その値がFirebaseに保存できる形かを確かめる」という観点**。
`staffHidden` は範囲の意味・境界の包含・後方互換・凍結対象外の理由までコメントで説明されており、
**壊れていたのは意味論ではなく、保存できる形かどうか**だった。Firebase RTDB は
「nullのキーは書かない・空ノードは消す」ため、**全フィールドがnullになりうるオブジェクトは保存できない**。
新しい設定マップを足すときは、値が取りうる形のうち**最も情報の少ないもの**を1つ選び、
`sanitizeForSet` を通して往復させてみるとよい。

**「読み側にしかない後方互換の形」は、書き側の逃げ道になることがある**。`true` は旧データを読むための
分岐に見えたが、実際には「下限も上限もない範囲」を**保存できる形で表す唯一の手段**でもあった。

**申し送り（次回の観点）**: **UIが文章で約束していることを集め、その約束をコードが守れるか1つずつ当たる。**
今回の入口はポップアップの一文だった。Shifty はポップアップ・注記・トーストで挙動を細かく説明する作りで、
**説明文は仕様書として読める**。`tt(` の文字列とモーダル内の説明文を機械的に集め、それぞれが成立する条件を
確かめると、今回と同じ形——**言っていることとやっていることのずれ**——が拾える。

### 総括

**🔴 ゼロ。🟡 は1件検出し、その場で修正・検証・コミットまで完了した。**
対象は前日にリリースされたばかりの機能で、**設計・コメント・テストのどれも質が高かったにもかかわらず、
「保存できない値」という別の層で1つ落ちていた**。境界の意味論は何度も確かめられていたが、
その値がデータベースに入るかは誰も確かめていなかった。
修正は読み側が既に持っていた表現（`true`）を書き側でも使うだけで、**新しい表現を増やさずに閉じた**。
