...
## Shifty バグチェックレポート（2026-08-17 自動実行 #80）

### 修正済み

- **🟡 店舗の「削除」が、実際には削除ではなく一覧から外すだけだった**（[app-admin.js:132](app-admin.js)）→ `20728d6`
  ボタンは「削除」と名乗り `「○○」を削除しますか？` と聞くが、**クライアントに店舗を削除する経路は存在しない**。`global/shops/{id}` を消すのはコードベース中ただ1箇所、`functions/index.js:760` の `purgeInactiveShops` だけである。
  **配信物をNodeで実行して3ケース確認**: ①**Authあり**＝`unlinkShopFromAuth`（app-main.js:914）が `accounts/{uid}/shops/{id}` を消すだけで、**確認は「削除」なのにトーストは「✓ 連携を解除しました」**＝1操作の中で自己矛盾 ②**Authなし**＝`saveShops`（app-main.js:1204）は `v.forEach` で**残る店舗しか書かない**＝削除処理を持たないのに、トーストだけが「削除しました」 ③**連携1件のとき**＝「削除しますか？」への回答が `✕ 最後の店舗は解除できません`。
  **断定できた根拠は、同じ関数を呼ぶ別ボタンが正しい語を既に使っていること**。CompanyTab（`:3203`）は**同一の `onUnlinkShop`** を「**解除**」というラベルで `「○○」の連携を解除しますか？` と確認している。**同じ作者が同じ操作について片方だけ正しく書いていた。**
  **実際に失われるのは「戻る手段」**。データは全て残るが復帰には店舗コードが要り、それは**その店舗の設定タブ（`:3481`）の中にしか出ない**。`global/shops` は一覧読み不可なので、控えていなければ**データは生きているのに二度と辿り着けない**。
  修正後: ラベルを「解除」に統一、確認を `「○○」を一覧から外しますか？ / シフトデータは削除されません。戻すには店舗コード（設定タブ）が必要です。` に。**「取り消せません」とは書かなかった**（何も消えないので嘘になる）。赤色も据え置き（色はデザイン判断）。

- **🟡 提出削除の確認が、誰の提出かを示していなかった**（[app-admin.js:3049](app-admin.js)）→ `7ea1a50`
  `confirm("削除しますか？")` だけで対象名も件数も無かった。**#79 の判定を訂正する**——#79 はこれを「詳細モーダル内＝対象が見えている」として🟢に落としたが、実際は**提出一覧の `<tr>` の中**で、`:3048` の「詳細」は**モーダルを開く側**のボタンである。1行=1人なので**行を1つ間違えると別人の提出が消える**。#74 の375px実測はこの2つを「gap 4px」と名指しで記録していた。
  削除は本物: `saveSubs` が `flat[deletedId]=null` で **sub 全体を Firebase から消す**（`archived/` 退避なし＝店舗削除とは逆に本当に消える）。
  **決め手はここでも同じ形の非対称**: スタッフ側の同じ操作（[app-staff.js:589](app-staff.js)）は `「田中」の提出を削除しますか？` と**既に名前を出している**。
  修正後: `「田中」の提出を削除しますか？ / 15日分の希望が消えます（戻すには本人の再提出が必要です）。`（0日なら2行目を省く）。**ここでも「取り消せません」とは書かない**（本人が出し直せば戻る）。

### 要確認（未修正）

- **🔴（継続・プロセス）このループから実機E2Eが3回続けて実行できない**（環境要因・コード変更なし）
  `preview_start` が **「Dev servers can't be started from unattended sessions」** で拒否され、稼働中サーバーも0件（今回も実行して確認）。Bash でのサーバー起動は規約で禁止のため代替なし。**#72〜#77 の6回が柱にしてきたブラウザ実測が、#78・#79 に続き今回も1件も実行できなかった。**
  **ユーザーが対話セッションで `/bug-check` を手動実行すれば解消する。**
...
- 以下は #46〜#79 からの継続（変化なし）: 未claim店舗の取り込み／`findActiveSubscription` の `|| live[0]`／`planExpiry` のUTC計算／片側セルだけの日が0分／`purgeOldPeriods` のUTC日付／`generateInviteCode` のデッドコード／ほか

### 今回の対象と見つけ方

**#79 以降のコミットは0件。8回続けて「差分ゼロ」の回**である。

**(1) 申し送りをそのまま実行し、その場で1つ追加で拾った。** #79 が「次は店舗削除（`:132`）」と対象を1つに絞って終わっていたため、探索ではなく検証に時間を使えた。**申し送りの粒度が次回の収穫量をほぼ決める**が2回連続で再現。

**(2) 「同じ操作を正しく書いている箇所が同一ファイルにあるか」が3回連続で決め手だった。** #79 は `onRenameStaff`、今回は CompanyTab（店舗）と app-staff.js（提出）。**この判定法は探索手順として実用的**——疑わしい操作を見つけたら同じ操作の別実装を探せばよく、仕様書もユーザー判断も要らずに白黒がつく。

**(3) レンズを閉じるところまでやった。** 修正後に `confirm(` を全件列挙し、**8箇所すべてが対象名を出し、連鎖があるものは結果を書いている**ことを確認。#74→#78→#79→#80 で**4回かけてこのレンズは閉じた**。次回回しても新しいものは出ない。

**(4) 前回の判定を1件ひっくり返した。** #79 の🟢は「モーダル内だから対象が見えている」が前提だったが、**実際は `<tr>`** だった。#74 は同じ2つのボタンを「gap 4px」と測っていたのに、#79 はそれと突き合わせずに判定していた。**過去回の測定結果と過去回の判定は別物で、後者は疑ってよい。**

**(5) 2件で症状が正反対だったので修正文も逆向きに書いた。** 店舗は**「削除」と言って何も消さない**、提出は**「削除しますか？」としか言わずに本当に消す**。前者に「消えません」、後者に「消えます」。**どちらも「取り消せません」とは書いていない**（店舗はデータが残り、提出は本人が出し直せる）＝#79 の「過大な警告も食い違いの一種」を2件とも適用。

**(6) 配信物をNodeで実行する手法が3回目の定着。** JSX 内の `onClick` を**波括弧の対応を数えて切り出し**、`confirm`/`onSave`/`tt` を差し替えて実行。修正前後で計6ケースを走らせ、**ブラウザ無しで「確認文とトーストの食い違い」を目視した**。

### 検証したこと

- `DEV_MODE`（app-core.js:12・式のまま）／`DEV_PLAN_OVERRIDE`（:125）正常
- `subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／`global/shops` 全件読み **0件**／`global/templates` **0件**／CF `.delete()` **0件**／CF secrets 抜けなし
...
- **修正の検証（配信物の実評価）**: 店舗＝Auth/非Auth/ガード発火の3ケース／提出＝15日分・0日・キャンセルの3ケース
- **クライアントに `global/shops` 削除経路が無いこと**を全ファイル grep で確認（唯一の削除は functions/index.js:760）
- **レンズの完全性**: `confirm(` 全8箇所がすべて対象名を出していることを確認
- **未検証（今回は手段が無い）**: ブラウザでの実描画すべて／**Free 上限に達した店舗での実ブロック**／今回変更した2つの confirm の実表示／実機 iOS Safari

### 総括

**測れない回が3回続き、それでも 🟡 が2件出た。** #79 の申し送りが対象を絞っていたおかげで、探索ではなく検証に時間を使えたためである。

**4回かけて「確認ダイアログが結果を語らない」レンズを閉じた。** ポジション・期間・スタッフ・店舗・提出の全8箇所が対象名を出す状態になった。**このレンズからの収穫はここで尽きる。**

**手法として残したいのは (2) と (4)。** 「同じ操作の別実装を探す」は仕様判断を挟まずに白黒がつく検査で3回連続で機能した。「過去回の判定は疑ってよい（測定結果は疑わなくてよい）」は #79 の🟢を今回ひっくり返したことで実証された。

**主力手段が失われた状態は 🔴 のまま3回目。** **ユーザーが対話セッションで `/bug-check` を1回回せば解消する**ので、その一手を最優先で提案する。

**次回の手を1つに絞る。** 実測が復活したなら**#77 の宿題をそのまま実行する**（実機で21人目・2件目を1回押すだけ）。実測が使えないままなら、**確認ダイアログのレンズは閉じたので別のレンズへ移る**——推奨は**「トーストが実際の結果と合っているか」**。今回すでに2件で食い違いが出た（`削除しました`／`連携を解除しました`）ため、**同種のズレがトースト側に残っている見込みが高い**。`tt(` の全件を直前の処理と突き合わせて1周する。
