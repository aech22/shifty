...
## Shifty バグチェックレポート（2026-08-16 自動実行 #77）

### 修正済み

- **🟡 非Premiumのシフト作成グリッドで、アップグレード導線が一度も発火しなかった**（[app-admin.js:1665,1684](app-admin.js)）→ `4f8c95b`
  グリッドのセルは `readOnly={!isPremium} disabled={!isPremium}` の**両方**が付いていた。一方でセルには非Premium向けの導線が**4本**書かれている——`onClick`×2 と `onFocus`×2 がいずれも `onUpgrade({type:"edit",plan})` を呼び、さらに `cursor` が `isPremium?"text":"pointer"` で**押せることを明示している**。しかし **`disabled` な `input` は click も focus も発火しない**ため、この4本は全て到達不能だった。Free/Pro のユーザーから見ると「ポインターに変わるセルを押しても何も起きない」。`disabled` を外して `readOnly` のみにして解消（Chromium・WebKit 26.5 の両方で、修正前 UpgradeModal=false → 修正後 true を実測）。
  **実装の取りこぼしだと断定できた根拠**: `handleBlur`（[app-admin.js:633](app-admin.js)）の冒頭に **`if(!isPremium)return;`** がある。非Premiumでハンドラが一切発火しない設計なら、このガードは到達不能な死にコードになる。つまり**作者は非Premiumでもイベントが来る前提で書いており**、`disabled` の付与だけがそれと矛盾していた。色や寸法の選択は一切要らない。
  **非Premiumが編集できないことは維持**: `readOnly` が入力を止め、`handleBlur` は上記ガードで即return、`onChange` は呼び出し側が `isPremium&&` で囲っている。実キーボードで `99` を打って Tab した実測で、free/pro はセル値が `"10"` のまま不変・premium のみ変化することを確認した。

### 要確認（未修正）

- **🟡（新規）プラン制限の告知アンバー `#F59E0B` が、ライトでだけ 2.15:1**（app-admin.js:1885/:2362 他）→ **BACKLOG化済み（決めること5）・デザイン判断待ち**
  **決めること1（ダークで沈む）の鏡像**である。同じ `#F59E0B` が **ライト（白地）2.15:1 ／ ダーク（`--c-card`）6.81:1** で、**ライト側だけが落ちる**。11px。**既にBACKLOGにある「閲覧のみバナー」の 2.16:1 よりわずかに悪い**。該当は「期間追加はProプランで利用できます」「並べ替え・名前色変更はProプラン（500円/月）で利用できます」など、**Free ユーザーに課金理由を伝える文字列そのもの**
- **🟡（新規）マイページのプラン比較表に、Premiumでは出ない違反が集中している** → **BACKLOG化済み（決めること2の実例として追記）**
  「Premium にする」白 on `#f87036` = **2.85:1**／「月 2,980円」= **2.63:1**／**「✕」が Free に11個・Pro に7個で 2.86:1（ダーク）・2.39:1（ライト）**／「無料プランをご利用中です」3.07:1（ダーク）・2.54:1（ライト）。**Premium では1つも描画されない**ため #72〜#76 では計上されていなかった
- **🟢（新規）今回の修正で、グリッドセルが WCAG の適用範囲に入った**（app-admin.js:1676/:1692）→ BACKLOG化済み（決めること5に併記）
  `disabled` な操作部品は WCAG 1.4.3 の適用除外だが `readOnly` は除外されない。非Premium時の `opacity:0.55` により **ライト 3.07:1 ／ ダーク 4.05:1**。**画素は1つも変わっていない**が分類だけが変わった
- **🟡 固定の淡い色の帯にダークで沈む文字色**（#72から継続）／**🟡 アクセント塗り＋白文字 2.85:1**（#72）／**🟡 土日祝の日付色がダークで沈む**（#75・今回も 3.18:1／3.46:1 を再確認）／**🟡 固定の明色チップが両テーマでAA未達**（#76）→ いずれもBACKLOG化済み・デザイン判断待ち
- **🟡 管理者画面のタップ可能要素がほぼ全て 44×44px 未満**（#74から継続）→ BACKLOG化済み。プラン別に測っても傾向は同じで**新しい事実は出なかった**
- **🟢 `index.html` の `?v=` と `build:` が今回の変更を指していない**（`20260814-53f610f` のまま）。**申し送りのみ**——`/release-to-main` の手順63が標準工程としてバンプする
...
- 以下は #46〜#76 からの継続（状態変化なし）: 未claim店舗を誰でも自分の企業に取り込める根／`findActiveSubscription` の `|| live[0]`／`subscription_schedule.*` の4分岐が未購読／`planExpiry` のUTC基準計算／決済失敗フラグの誤解除／片側セルだけの日が0分／`purgeOldPeriods` のUTC日付／`getBreaksFor` の休憩判定／`generateInviteCode` のデッドコード／`urlLocked` が StaffView で未使用／`staffSectionOn` と `isCountExcluded`／SubsTab「月計」の別名重複／`sub.shopId` が死にフィールド／店舗削除が `global/shops/{id}` を消さない／`companyCodes` の TOCTOU／略称の先勝ち／期間作成の日付検証なし／提出状況ビューのセル編集／Excel analytics の数え方／スタッフ数バーのスペーサー／`breakTimes[].tags` の残留／別名解決順の不一致／`heatHours` が `adminRest` を無視／スタッフ削除時のキー残留／`onSub` のローカルstate更新キーの不一致／スタッフ削除に確認ダイアログが無い

### 今回の対象と見つけ方

**#76 以降のコミットは0件**（`ffde4be` が #76 の記録）。**5回続けて「差分ゼロ・宿題実行」の回**である。#76 が指名した宿題は「`?plan=free` と `?plan=pro` で375px×ダーク/ライトを測る」で、#72〜#76 の5回はすべて `?plan=premium` で測っており **Free/Pro でしか描画されないUIは一度も画面に出ていなかった**。

**(1) 今回も計測が壊れていた——ただし壊れ方が前2回と違った**。#75「セレクタが別要素に一致」・#76「テーマが効いていない」に対し、今回は **`hasText` の部分一致で「スタッフ」が「スタッフ画面」に当たり、管理者画面から離脱したまま測り続けていた**。**ログ上はエラーが出ず「スタッフ: el=310 contrast=69」という、それらしい数字が返っていた**。気づけたのは**プランを変えても el=310 が1要素も動かなかった**からである——StaffTab は Premium で番号入力・属性select・ポジションボタンが増えるので、**プランで要素数が変わらないのはおかしい**。修正後は 146/158/178 と正しく差が出た。**3回連続で計測不良を出荷前に捕まえている**が、いずれも別種なので「前回の対策」では防げていない。

**(2) 「プランで差が出るか」を軸にして、5回ぶんの死角を1回で開けた**。2テーマ×3プラン×8タブ＝48面を測り、**premium に無い違反だけを差分で抜いた**。#75 が「ダークにだけ出るか」を判定軸にしたのと同じ手を、**テーマではなくプランの軸に当て替えた**形で、決めること1〜4のどれとも違う**「ライトでだけ沈む」という第3の型**が出た。

**(3) 一番の収穫はコントラストではなく機能バグだった**。`opacity:0.55` を調べる過程で `disabled={!isPremium}` に気づき、**「`disabled` なら onClick は発火しないはずだ」という仮説を立てて実際にクリックして確かめた**。**`cursor:"pointer"` で押せると示しておいて何も起きない**のは、体験としてもコンバージョン導線としても壊れている。拾えたのは**「非Premiumの画面を実際に触った」のが5回で初めてだった**からにすぎない。

**(4) 危うく偽陽性を1件報告するところだった**。特記セル（「研修」）の `#333` が計測上 **premium/dark で 1.05:1**——#76 で直した 1.02:1 より悪い値として出た。だが **`getComputedStyle().backgroundColor` は `linear-gradient` の層を含まない**。`cellBgStyle` は「不透明ベース(`--c-input`)＋レジェンド色を gradient 層で重ねる」設計で、特記セルの実際の背景は**不透明な黄 `#FFF3B0`**。**実画素のヒストグラムで `255,243,176` が 484px で支配的**と確認でき、`#333` on `#FFF3B0` = **11.25:1 で問題なし**と確定した。

**(5) 修正するかどうかは、今回も severity ではなく「答えがコードベースにあるか」で決めた**。5回連続で同じ基準が機能している。今回それを決めたのは **`handleBlur` 冒頭の `if(!isPremium)return;`**＝**非Premiumでもハンドラが発火する前提でしか書かれえないガード**である。一方 `#F59E0B` の 2.15:1 は、白地で読める新しいアンバーを選び、かつダークにも通すならテーマ変数の新設が要る＝選択が2つ入るのでBACKLOGへ回した。

### 検証したこと

- `DEV_MODE`（app-core.js:12・式のまま）／`DEV_PLAN_OVERRIDE`（:125）正常。`?plan=` が実際に解決されることを画面内で実測
- `subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／`global/shops` の全件読み **0件**／CF `.delete()` 誤用 **0件**／CF secrets の抜け漏れなし
- 読み込み順維持（utils→core→staff→admin→main）・SRI **11本**
- 未定義のCSS変数参照 **0件**／ダーク定義の二重管理は**乖離なし**／SVG属性内 `var()` は `app-staff.js:9` の1件で増えていない
- `npm test` **168件パス**／`npx eslint app-*.js` **0 errors 98 warnings**／`node --check functions/index.js` OK
- **375px×（ダーク／ライト）×（free／pro／premium）×8タブ＝48面の実測**:
  - 各面で `data-theme` と `body` の実背景色を毎回出力し確認してから計測（#76 由来の手当て）
  - **はみ出し: 48面すべて 0件**、`documentElement.scrollWidth == 375`（#73 の非回帰）
  - 要素数がプランで正しく変化することを確認（スタッフ 146/158/178・設定 132/144/354・シフト作成 1189/1189/1194）＝**プラン制限UIが実際に描画されている裏取り**
- **修正の検証**: グリッドセルのクリックで UpgradeModal が開くかを Chromium・WebKit 26.5 で実測（修正前 free/pro=false → 修正後 true）。**premium は修正前後とも false でフォーカスがセルに入る**＝非回帰。実キーボード入力で free/pro のセル値が不変であることも確認
- **修正が描画に影響していないことの裏取り**: シフト作成タブの違反件数は free 48→43・pro 48→43・**premium 50→45** と動いたが、**premium は今回の変更対象外（元から `disabled=false`）なのに同じ −5 が出ている**ため、3プラン共通の計測ゆらぎであって修正の影響ではない
- **数値の検算**: `#F59E0B` on 白 = 2.148（実測 2.15）、on `#1E293B` = 6.81、`#333` on `#FFF3B0` = 11.25 を手計算で再現し実測と一致
- **誤検出と判定して報告しなかったもの**: ①特記セルの `#333`（実画素で 11.25:1 と確定）②期間タブの共有URL表示の `scrollWidth > clientWidth`（意図的な省略表示）③`#seo-static` の隠しブロック
- リポジトリは**着手前と同じ**（計測物はすべてスクラッチパッドに置いた）
- **未検証**: 実機のiOS Safari（WebKitエンジンでの確認に留まる）／375px 以外の狭い幅／**スタッフ数・期間数が実際に上限へ達した Free 店舗**（標準テスト店舗はスタッフ4名・期間2件のため、app-admin.js:2475 の「▲ 上限に達しています」と `type:"staff"`/`type:"periods"` の UpgradeModal は**今回も描画されていない**）／🟡の実機E2E（スタッフURLでの他人の提出削除）

### 総括

**5回続いた「差分ゼロの回」で、初めてコントラスト以外の実バグが出た。** 出てきた理由ははっきりしていて、**非Premiumの画面を実際に触ったのが5回で初めてだった**からである。`?plan=` を書き換えるだけで到達できる面を5回見落としていたのは、**「巡回で到達する画面」を測るという習慣が、そのままカバレッジの天井になっていた**ということ。次にどこを測るかを決めるとき、「まだ押していない状態」を数え上げるのが効く。

**判定軸の使い回しが効いた。** #75 の「ダークにだけ出るか」を**プランに当て替えた**（premium に無い違反だけを抜く）。落ちてきたものが**決めること1の鏡像＝「ライトでだけ沈む」**という新しい型だったのは、軸を変えたからこそである。決めること1（ダークで沈む）・4（両テーマ）・5（ライトで沈む）で、**固定色リテラルの壊れ方が3種類そろった**——単一の根から出ている3面なので、決めること1の案Bを採ればまとめて閉じられる可能性が高い。

**計測の壊れ方が3回とも違ったのは、対策の立て方を変えるべきだという合図。** #75「セレクタ」・#76「テーマ」・#77「部分一致でのタブ離脱」。共通するのは**「壊れてもツールは正常終了してそれらしい数字を返す」**こと。今回それを捕まえたのは個別の対策ではなく、**「プランを変えたら要素数が変わるはずだ」という不変条件を先に持っていた**ことだった。次回以降は、測る前に「この操作が効いていれば必ず変わる値」を1つ決めて、それが変わらなければ計測を信じない、という形にする。

**偽陽性を1件、報告直前で止められた。** `#333` の 1.05:1 は数字だけ見れば #76 の 1.02:1 より悪く Premium の主機能なので🔴として書きかけていた。止められたのは `getComputedStyle().backgroundColor` が gradient 層を見ないという事実に思い当たったからで、**最終的には実画素のヒストグラムという、計算を介さない証拠で決着させた**。**怪しい値は必ず画素で殴る**のを定型にする。

**次回の手を1つに絞る。** 未検証に明確な穴が1つ残った——**スタッフ数・期間数が実際に上限へ達した Free 店舗**である。告知文は描画できたが、`app-admin.js:2475` の「▲ 上限に達しています」と `type:"staff"`/`type:"periods"` の UpgradeModal は**今回も一度も出ていない**。標準テスト店舗はスタッフ4名・期間2件で、Free の上限（20名・1件）のうち**期間だけが超過している**状態だった。**次は Free 上限に達したスタッフ構成のテスト店舗を用意し、「上限に達したときに実際に作成をブロックできるか」という機能面まで含めて実測する**。色ではなく「Free ユーザーが上限を超えて登録できてしまわないか」という課金の境界の検証で、コードと dev Firebase だけで閉じられる。
