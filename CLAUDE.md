...
## Shifty バグチェックレポート（2026-09-21 自動実行 #140）

> 着手時の HEAD は `ecc3e8e`。#139 以降 app-*.js は1バイトも変わっていなかったため、**#139 の申し送りを
> そのまま実行した**——「行の『1日超過』だけが `sub.shifts[d]` を直に読み、週・月は `_shiftAt` 経由で
> 引く非対称が、同じ日について別のシフトを読むのではないか」。答えは**読む**（🟡・修正済み）。
> 測っている途中で、**この食い違いは別名を1件も使わない店舗でも到達する**ことが分かった。

### 修正済み

- **[🟡] 提出一覧の「1日超過」バッジと、同じ行の週判定・詳細モーダルの週間勤務時間が、同じ人・同じ日について別のシフトを読む**（app-admin.js の `SubsTab`・`2f758c4`）

  このバッジが出す上限判定は5つ（1日・週・2週・月・任意日数）ある。**そのうち4つは `_min`
  （`_shiftAt` 経由＝同じ名前|日付のシフトの最初の1件を、別名も解決して引く）を通すのに、
  1日だけが `sub.shifts[d]` を直に読んでいた。**

  **到達経路は2つあり、2つ目は別名を要しない**:

  | 経路 | 前提 |
  |---|---|
  | ① 別名で出した提出と登録名の提出が併存 | #81 の根（BACKLOG 🟡・未解決） |
  | ② **期間の重なり** | `validatePeriodDates` は重なりを**警告のみで通す**（2026-08-25 決定・案B）。別名は1件も要らない |

  **実測②**（`SubsTab` だけを実ブラウザにマウント＝**Firebase へは1バイトも出していない**。別名 0 件。
  9/24 を含む2期間があり、片方の提出が 09:00〜13:00、もう片方が 09:00〜18:00。属性の1日上限は 7:00）:

  | | 修正前 | 修正後 |
  |---|---|---|
  | 行の「1日超過」バッジ | **出る**（＝9:00 と判定） | 出ない |
  | 同じモーダルの週間勤務時間 9/21〜9/27 | **4:00** | 4:00 |

  **バッジは「7時間を超えた」と言っているのに、同じ画面の週計は 4:00 ちょうど**という、
  1つの画面の中で完結する矛盾になっていた。修正は `ds.forEach` を `_min` に通すだけで、これは
  同ファイル :3744 のコメントが宣言している「**同じ問いに2つの式がある状態を、多数派へ揃える**」という
  既存の決定（#84 の `bc95af4`・#105 の `e2fe16a` と同じ形）に沿う。**重複が無い通常時は 28 ケース
  （休憩あり／退勤延長／管理者調整／締の追加出勤／深夜跨ぎ／片側セル／休み／別名提出の組み合わせ）
  すべてで現行と同じ分数を返す**ことを事前に確認した。

  回帰テストを `.claude/skills/shifty-e2e-verify/scripts/example-subs-daily-vs-week.js` に保全（`daa39fd`）。
  **修正前の配信物（`ecc3e8e`）に向けると EXIT=1** ＝素通りするテストではない。

### 要確認（未修正）

- **[🟢] 詳細モーダルの「勤務計」「合計」「各日の勤務時間」だけが `det.shifts[d]` を直に読む**
  同じモーダルの週間勤務時間・月計は `_dayMin`（`_shiftAt` 経由）で引く。上と同じ2経路で重複が起きると、
  実測で **勤務計 9:00 / 週間勤務時間 4:00** と食い違う。**今回は直さなかった**——モーダルは
  「その提出の明細」を見せる画面なので、各日の表にその提出自身の時刻を出すのはむしろ正しいとも読める。
  **どちらを表すのかはコードからは決められない**（条件B）。ただし上の修正でバッジ側は揃ったので、
  **残る食い違いは数字の表示だけ**で、上限超過の判定が画面ごとに反転することは無くなった。

- **[🟢] `_windowVio` の窓の起点だけが `sub.shifts[d]` を直に読む**
  窓の中の合算は `_min` × `_allWork()`（マップ由来）で行うのに、**どの日を窓の起点にするか**だけが
  その行の sub を直に読む。影響は 2週・任意日数の上限を設定している店舗に限られる。起点の範囲を変えると
  **検査する窓の集合そのものが変わる**ので、上の「多数派へ揃える」論法がそのまま当てはまらない。

- **#139 から継続（変化なし）**: 確定済み期間でタブ間の勤務時間が食い違う（🟡・BACKLOG化済み）／
  変更マークと「変更あり」バッジの食い違い（🟡・BACKLOG化済み）／属性削除で休憩のタグが残る（🟢）／
  `moveStaffHiddenBoundaries` の同一開始日（🟢・意図的）／`dup`・`posErr` がPDFに無い理由のコメント（🟢）／
  部門別PDFの未登録名（🟢）／休み提出の斜線と黄色（🟢）／「締」が無効な店舗のメモ（🟢）／
  「締」入りの略称（🟢）／店舗切替の `plan`（🟢）／`purgeInactiveShops` の招待コード後始末（🟢）／
  企業ログインの試行制限（🟡・BACKLOG化済み）／`recentPeriodIds`（🟢）／
  `ShiftEditTab` の他店舗 `subs` 全件読み（🟢）／退勤が出勤より前（🟡・BACKLOG化済み）。
- **変化なし（BACKLOG化済み）**: Cloud Functions の本番反映（3件）／二重課金の根治／特商法表記（🔴）／
  解約時のプラン判定／`verifyShopOwner` の移行猶予／別名提出の重複の根（#81）／PDFの実物確認／
  `purgeOldPeriods` の本有効化／非表示スタッフの未提出カウント／「締」の休みの入口2つ／完全削除と `keepAttrs`。
- **引き受け済みのトレードオフ（再検出しても直さない）**: `subs/$subId/.write` は認証済みなら通る。2026-08-31 決定1。

### 当てて否定した仮説

- **モーダルの `_dayMin` と行の `_min` が違う settings を見る → 否定**（#139 の修正で `pAttrSettings`・
  `detAttrSettings` はどちらも `applyKeepAttrs` を通っており、同じ属性を引く）。
- **`_shiftAt` が別名の登録順を取り違える → 否定**（`resolveSubByAlias` に一本化済み・#105/#106/#112）。
- **`isPro` で Premium 機能を出している → 否定**（0件）。**CF の `secrets` 抜け → 否定**（7つの `runWith`
  すべてに宣言あり・`.delete()` 誤用0件）。

### 検証したこと

- `npm test` **270件パス**・`npx eslint app-*.js` **0 errors / 95 warnings**（修正前後で増減なし）。
  `node --check functions/index.js` 通過。
- RULES.md スキャン: `DEV_MODE` は式のまま／`subs` の全体 `set()` 0件／`accounts` 全件読み 0件／
  SRI 11/11／読み込み順 utils→core→staff→admin→main／フォーム部品 **58件・`fontSize<16` は0件**。
- **配信版数は `20260920-40a88de` のままで、今日の app-admin.js 変更を指していない**。
  リリース時に `/release-to-main` の手順63でバンプすること。
- **Firebase・Stripe には一切アクセスしていない**。

> **⚠️ 並行セッションを検出した（この回の作業中）**
> 実行の途中から作業ツリーに**別セッションの未コミット作業**が現れた——`app-admin.js` の
> `SummaryTable`／`ShiftEditTab` に `fullView`・`labelW` を足す変更（+101/-50 行）と、BACKLOG の新規 🔴
> 「シフト作成タブ: 『全員表示』を『全表示』に改める」、および `example-fitall-geometry.js`。
> **触らず報告に留めた**。本回のコミットは `2f758c4`（1日超過の修正）と `daa39fd`（回帰テスト）の2つだけ。
> **main へのマージ・本番デプロイは、並行セッションの作業が終わってからまとめて1回にすること。**

**申し送り（次回の観点）**: 「同じ数字を出す式が2本ある場所」という当たり方は2回続けて当たっている
（#139 の行とモーダル、#140 の1日と週）。**まだ残っているのは上の🟢2件**で、どちらも
「その提出を見せるのか、その人のその日を見せるのか」という1つの問いに帰着する。
**次はこの問いを先に決めてから当たると、2件同時に閉じられる**。あわせて、今回**期間の重なりが
「2つ目のsubができる別経路」だと分かった**ので、`#81`（別名）だけを根だと思って書かれている箇所が
他に無いかを見るとよい。
...
> **🚧 2026-09-23: Dev限定で実装済み（未コミット・本番は未変更）**
> ユーザー指示「一旦Devのみで表示して」により、新レイアウトは `const fullView=fitAll&&DEV_MODE;`（app-admin.js）
> で囲ってある。**本番（shiftyshifty.app）は従来の「全員表示」のまま**で、ラベルも挙動も変わらない
> （通常表示の10指標がベースラインとバイト単位で一致することを実測済み）。本番へ出すときは
> この1行から `&&DEV_MODE` を外す。**そのとき初めて本番の横はみ出し（下記）も直る**——
> 現状は `boxSizing:"border-box"` も `fullView` 限定なので、本番の全員表示は今も右端が切れたまま。
>
> 実測（`node .claude/skills/shifty-e2e-verify/scripts/example-fitall-geometry.js` → **EXIT=0 / allPass**。
> viewport 1400×900・Firebase非接続）:
>
> | 水準 | 横の余り | 縦の余り | 可視高 | 日付列 | スタッフ列 | セルのfont |
> |---|---|---|---|---|---|---|
> | 16日×8/15/25名 | 0px | 0px | 814px | 46px | 162/86/52px | **16px**（規約違反なし） |
> | 31日×8/15/25名 | 0px | 0px | 822px | 46px | 162/86/52px | **10px** |
>
> **2週間期間なら16pxのまま収まる**（＝16px規約に触れるのは1ヶ月期間だけ）。1ヶ月でも10pxで、
> 計画時の試算（5〜7px）より読める。全表示のまま編集できることも実測済み
> （31日×3名で186個のinputが描画され、セル編集→保存が「✓ 2件のシフトを保存しました」で通る）。
>
> **残り**: ①本番解放（`&&DEV_MODE` を外す）②全表示での色の再現（土日祝・ポジション不足・セルコマンド・
> スタッフ名色）の目視確認 ③dev標準テスト店舗での実機E2E ④コミット。
...
**受け入れ条件**:
- [ ] **本番解放**: `const fullView=fitAll&&DEV_MODE;` から `&&DEV_MODE` を外す。**この1行を外すまで本番は一切変わらない**
- [ ] **本番の横はみ出しの解消**: `boxSizing` も `fullView` 限定にしてあるため、本番の全員表示は今も右端が切れたまま（8/15/25名で26/49/89px）。本番解放と同時に解消する。無条件に `border-box` にすると通常表示の列幅も 43px→39px と変わるので、外すのは全表示の解放とセットにする
- [x] ラベルを「全表示」に変更（`fitAll?"通常表示":(DEV_MODE?"全表示":"全員表示")`）し、コメントも更新。**本番は「全員表示」のまま**なので `grep "全員表示"` は0件にならない（Dev限定の間は正しい状態）
- [x] 全表示で、左上の「日付」ヘッダから右下（最終日の行の右端の日付セル）までが縦横スクロールなしで可視。**16日/31日 × 8/15/25名の6水準**で `scrollHeight ≤ clientHeight` かつ `scrollWidth ≤ clientWidth` を実測（`example-fitall-geometry.js` → EXIT=0・全水準で余り0px）
...
- [ ] 全表示でも土日祝の色（`dc`/`baseRb`）・ポジション不足の黄色（`rbS`/`rbE`）・セルコマンドの背景色（`cellBgStyle`）・スタッフ名色（`nameColor`）が通常表示と同一規則で再現される → **未検証**（コード上は分岐を入れていないので同じ規則が通るはずだが、目視・実測ともしていない）
