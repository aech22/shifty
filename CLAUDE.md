...
## Shifty バグチェックレポート（2026-08-15 自動実行 #76）

### 修正済み

- **🟡 ダークで編集中セルの出退勤時刻が見えなくなっていた**（[app-staff.js:628,631](app-staff.js)）→ `17ff0a7`
  提出状況一覧（SmModal）のセルをタップすると、そのセルの背景が固定の明色 `#FEF0E8` になる。一方で時刻の文字色は `var(--c-text)` のままで、**ダークでは `#F1F5F9`（ほぼ白）に解決される**ため、明色の上に明色が乗って **1.02:1 ＝ 実質不可視**になっていた。ライトでは同じ式が `#1A1A2E` に解決されて **15.31:1** なので、**ダークにだけ出る**。`isEditing` のときだけ `#1A1A2E` に固定して解消（修正後 15.31:1、スクリム `rgba(0,0,0,.4)` 越しの実効値でも **6.04:1**）。**非編集セルは従来どおり `var(--c-text)`** でテーマに追随する

### 要確認（未修正）

- **🟡（新規）固定の明色チップ＋固定の色文字が、両テーマとも AA 未達**（app-staff.js:341/627/629/656 他）→ **BACKLOG化済み（決めること4）・デザイン判断待ち**
  モーダル面を初めて実測して見つかった3種類。**ダークとライトで件数が完全に一致する**（9/9・26/26・15/15）＝ テーマ非依存の固定ペアで、決めること1（ダークでだけ沈む）とは別の問題。出勤チップ `#d4601a` on `#FEF0E8` = **3.43:1**／未提出チップ・締切超過バナー・日曜 `#FF4757` on `#FFF0F1` = **3.02:1**／土曜 `#3B82F6` on `#EFF6FF` = **3.38:1**。**決めること3（曜日色）の前提を1つ更新した**——スタッフ画面側の曜日色は淡いチップの上にあり**ライトでも未達**なので、色を決めるときは管理者画面の地色の上とスタッフ画面のチップの上の**2文脈**を同時に満たす必要がある
- **🟡 固定の淡い色の帯にダークで沈む文字色が直書きされている**（#72から継続）→ BACKLOG化済み・**デザイン判断待ち**。今回モーダル5画面すべてで「閲覧のみバナー」`#92400E` on 合成後 `rgb(38,37,39)` = **2.16:1** を再確認（**#76 のダークのみ違反はこれ1種類だけ**）
- **🟡 アクセント塗り＋白文字が 2.85:1 で AA 未達**（#72から継続）→ BACKLOG化済み・**デザイン判断待ち**。モーダルでも主要ボタン（「Premium にする」「✓ 作成する」「保存」「✓ 確定」）が軒並みこれ
- **🟡 土日祝の日付色が固定の色リテラルで、ダークにだけ沈む**（#75から継続）→ BACKLOG化済み・デザイン判断待ち（上記のとおり前提を1つ更新）
- **🟡 管理者画面のタップ可能要素がほぼ全て 44×44px 未満**（#74から継続）→ BACKLOG化済み・デザイン判断待ち
- **🟢 `index.html` の `?v=` と `build:` が今回の変更を指していない**（`20260814-53f610f` のまま。今回 app-staff.js を変更した）。**申し送りのみ**——`/release-to-main` の手順63が標準工程としてバンプするため BACKLOG 化しない
- **🟢 編集中セルの区切り記号「〜」が両テーマで AA 未達**（app-staff.js:629）。`var(--c-text4)` を固定背景 `#FEF0E8` の上に置いており **ダーク 4.27:1 ／ライト 2.28:1**（9px）。片方だけではないので今回の修正対象から外し、決めること4に併記した
- **🟢 スタッフ削除に確認ダイアログが無い**（app-admin.js:2312）。#74 からの申し送り。状態変化なし
- **🟢 `firebase.json` の hosting ignore に削除済みの `ogp.html` が残っている**（firebase.json:11・無害）／**🟢 OGP画像の「7名提出済み」と表示行数4名が不一致**／**🟢 マイページに `ownerReadOnly` が渡っていない**（app-admin.js:208）
...
- 以下は #46〜#75 からの継続（状態変化なし）: 未claim店舗を誰でも自分の企業に取り込める根／`findActiveSubscription` の `|| live[0]`／`subscription_schedule.*` の4分岐が未購読／`planExpiry` のUTC基準計算／決済失敗フラグの誤解除／片側セルだけの日が0分／`purgeOldPeriods` のUTC日付／`getBreaksFor` の休憩判定／`generateInviteCode` のデッドコード／`urlLocked` が StaffView で未使用／`staffSectionOn` と `isCountExcluded`／SubsTab「月計」の別名重複／`sub.shopId` が死にフィールド／店舗削除が `global/shops/{id}` を消さない／`companyCodes` の TOCTOU／略称の先勝ち／期間作成の日付検証なし／提出状況ビューのセル編集／Excel analytics の数え方／スタッフ数バーのスペーサー／`breakTimes[].tags` の残留／別名解決順の不一致／`heatHours` が `adminRest` を無視／スタッフ削除時のキー残留／`onSub` のローカルstate更新キーの不一致

### 今回の対象と見つけ方

**#75 以降のコミットは0件**（`b5d9312` が #75 の記録で、そこからコードは1行も動いていない）。**4回続けて「差分ゼロ・宿題実行」の回**である。#75 が指名した宿題は明確だった——「巡回に出てこない画面（モーダル・空状態・エラー状態）を375px×ダークで測る」。#72〜#75 の4回は**ライト1400px・ライト375px・ダーク375px を埋めたが、いずれも巡回で到達する画面だけ**で、モーダル面は一度も測られていなかった。

**(1) 測る前に、また計測が壊れていた——今度は「ダークで測っていなかった」**。テーマをダークにするつもりで Playwright の `colorScheme:"dark"` を渡し、あわせて `localStorage["ots_theme"]="dark"` を入れた。事前確認スクリプトを流したところ **`theme=light` / `bodyBg=rgb(240,242,245)`** が返り、**ライトのまま測るところだった**。原因は2つ重なっていた: ①`lg()` は `JSON.parse` するので生の `dark` は parse に失敗し**既定の `"light"` へ落ちる**（`JSON.stringify("dark")` が要る）②`applyTheme(lg(THEME_KEY,"light"))` の既定が `"light"` なので、**`prefers-color-scheme` だけではこのアプリは絶対にダークにならない**（`data-theme` が明示的に light で書かれる）。#75 は「セレクタが別要素に一致」「前置きが実行されない」の2つを捕まえたが、**今回は「テーマそのものが効いていない」**で、**3回とも計測ツールは正常終了してそれらしい数字を返していた**。

**(2) 検出器のノイズを先に潰した**。最初の実行では全画面で overflow が12件ずつ出たが、**どの画面でも同じ12件**だった。中身は `index.html:195` の `#seo-static`＝`width:1px;clip:rect(0,0,0,0)` のSEO用隠しブロックで、画面には出ない。**全画面で同数のノイズは「実際のはみ出し」を隠す**ので除外したところ、**モーダル5画面すべてで overflow 0件**になった（#73 の `flexWrap` 修正がモーダル面でも非回帰）。

**(3) 「ダークにだけ出る違反」で39件を2件に落とした**。#75 で最も切れ味のあった軸をそのまま使った。CellEditPanel 画面はダーク39件・ライト92件（**ライトの方が多い**）で、件数だけでは何も指さない。しかし**ライトと突き合わせて「ダークにだけ出る」で絞ると2種類**になり、うち1つは既知の閲覧のみバナー。**残る1つが編集中セルの 1.02:1** だった。

**(4) 「見えている画面」と「計測値」が食い違ったので、食い違いの方を調べた**。スクリーンショットではセルが `#FEF0E8`（クリーム）ではなく**くすんだ土色**に見えた。`elementFromPoint` が CellEditPanel を返したので一度は「パネルがセルを覆っているだけ＝ユーザーには見えない」と疑ったが、**幾何で確かめると `overlaps:false`**（セル y=120〜192／パネル y=321.5〜366.5）で覆っていない。真因は **`rgba(0,0,0,.4)` の全画面スクリム（z-index 400）**だった。そこで**スクリム越しの実効コントラストを計算し直した**ところ、前景と背景が等しく暗転するため **1.02:1 のまま**（`#919395` on `#98908B`）で、報告値は変わらないことを確認した。

**(5) 修正するかどうかは、severity ではなく「直し方が決まるか」で判断した**。#72・#74・#75 で3回連続して機能した線をそのまま当てた。`#FEF0E8` は app-staff.js に4箇所あり、**うち3箇所（:242 の `#1A1A2E`、:341/:627 の `#d4601a`）はいずれも文字色を固定している**。文字色をテーマ変数のままにしているのは**編集中セルだけ**という非対称で、**ファイル内に既に答えが書かれていた**。しかも**ライトの `var(--c-text)` は `#1A1A2E` と同値**なので、固定してもライトは1ピクセルも変わらない。**新しい色を選ぶ必要がない＝ループが直してよい**と判断した。

**(6) 逆に、直さなかったものは「両テーマで未達」だった**。同じ計測で出た出勤チップ（3.43:1）・未提出チップ（3.02:1）・土曜チップ（3.38:1）は、**ダークとライトで件数が完全に一致**（9/9・26/26・15/15）＝ 固定ペアで、直すには色を選ぶ必要がある。決めること4として BACKLOG へ回した。

### 検証したこと

- `DEV_MODE`（app-core.js:12・式のまま）／`DEV_PLAN_OVERRIDE`（:125）正常
- `subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／`global/shops` の全件読み **0件**／CF `.delete()` 誤用 **0件**
- 読み込み順維持（utils→core→staff→admin→main）・SRI **11本**
- 未定義のCSS変数参照 **0件**／ダーク定義の二重管理（`:root[data-theme=dark]` と `@media prefers-color-scheme`）は**乖離なし**／SVG属性内 `var()` は `app-staff.js:9` の1件で増えていない
- `npm test` **168件パス**／`npx eslint app-*.js` **0 errors 98 warnings**／`node --check functions/index.js` OK
- **375px×ダークでのモーダル・空状態・フォーム面の実測（標準テスト店舗・`?plan=premium`／UpgradeModal のみ `?plan=free`）**——#72〜#75 が一度も測っていなかった面:
  - 到達を確認した11面: **SmModal(スタッフ側)・SmModal(管理者側)・CellEditPanel(＋編集中セル)・TermsModal・UpgradeModal・期間作成パネル・期間作成手動入力フォーム・期間編集フォーム(PEF)・提出一覧タブ・シフト作成タブ・未提出チップ**。各面で `data-theme=dark` / `bodyBg=rgb(15,23,42)` と要素数・タップ要素数を毎回出力し、**ライトと DOM要素数が一致すること**（398/632/197/224/250）を突き合わせてから計測した
  - **はみ出し: `#seo-static` を除外して全面 0件**。全面で `documentElement.scrollWidth == 375`（#73 の非回帰）
  - **コントラスト: ダークのみに出る違反は「閲覧のみバナー」1種類だけ**（修正後）。修正前は2種類で、差分が今回直した編集中セル
  - **Chromium と WebKit 26.5 で一致**。修正後の WebKit 実測でも時刻の `color` が `rgb(26,26,46)` = `#1A1A2E` であることを確認
- **数値の検算**: `#F1F5F9` on `#FEF0E8` = 1.02:1、`#1A1A2E` on `#FEF0E8` = 15.31:1、スクリム `rgba(0,0,0,.4)` 合成後がそれぞれ 1.02:1 / 6.04:1 を手計算で再現し、計測値と一致することを確認
- **修正の効果を件数で確認**: CellEditPanel 画面のダーク違反 39→37件、**ダークのみ違反 2→0件**、**ライト違反 92→92件（不変＝ライトの見た目が変わっていないことの裏取り）**
- **誤検出と判定して報告しなかったもの**: 期間タブの共有URL表示が `scrollWidth 338 > clientWidth 194`。`textOverflow:ellipsis` + `overflow:hidden` + `whiteSpace:nowrap` の**意図的な省略表示**で、検出器側の取りこぼし条件（`overflowX:auto|scroll` しか除外していなかった）だった
- リポジトリは**着手前と同じ**（計測物はすべてスクラッチパッドに置き、`git status` の未追跡ファイルが着手前と同一であることを確認）
- **未検証**: 実機のiOS Safari（WebKitエンジンでの確認に留まる）／375px 以外の狭い幅（320px 等）／モーダルのタップ領域実測（#74 の管理者画面と同様に 44px 未満が多いと見込まれるが測っていない）／**提出0件の期間**そのもの（標準テスト店舗には提出済みの期間しかなく、空の期間を作ると店舗データが変わるため作らなかった。提出一覧タブ・シフト作成タブの通常状態は測った）／🟡の実機E2E（スタッフURLでの他人の提出削除）は今回も未実施

### 総括

**今回の収穫は「ダークで測っているつもりでライトを測っていた」を出荷前に捕まえたことと、それでも1件の実バグに届いたことの両方である。** 計測が壊れていたのは #74・#75 に続いて3回連続で、しかも今回は**テーマの取り違え**という、レポートに「ダーク◯件」と書いた瞬間に全部が嘘になる種類だった。原因は `lg()` が `JSON.parse` すること1点で、**アプリ側の既定が `"light"` なので `prefers-color-scheme` を渡すだけでは絶対にダークにならない**という仕様と重なって、静かに落ちていた。**事前確認で `data-theme` と `body` の実背景色を毎回出す**という #75 の手当てが、そのまま今回を救った。

**「ダークにだけ出るか」という軸は3回目で、もう単なる絞り込みではなく判定基準そのものになった。** 今回はこの軸が**修正するかどうかを直接決めた**。編集中セルは「ダーク 1.02:1 ／ライト 15.31:1」で片側だけが壊れており、チップ3種は「ダークとライトで件数が完全一致」で両側が同じだった。**前者は実装の取りこぼし、後者は配色の選択**という切り分けが、数値だけで機械的にできた。#75 が「非対称を2つ持つものを選ぶ」と書いた基準を、今回は**件数の一致/不一致という形で自動化できた**のが進歩である。

**「見た目と計測値が食い違ったとき、計測値を疑うのではなく食い違いを調べる」のが効いた。** スクリーンショットのセルがクリーム色でなく土色に見えたとき、素直に読めば「計測が間違っている」だが、調べたら **`rgba(0,0,0,.4)` のスクリム**という第三の要因だった。ここで大事だったのは、**一度は「パネルがセルを覆っているなら、そもそもユーザーには見えない＝報告してはいけない」と疑って幾何で確かめた**ことである（`overlaps:false` で否定できた）。**ユーザーに見えないものをバグとして報告しない**ための検算で、これがなければ「不可視の要素の不可視性」を報告していた可能性があった。そしてスクリム越しの実効値を計算し直して、報告値が変わらないことまで確かめた。

**修正の線引きは4回連続で同じ基準が機能している。** 「直し方に色や寸法の選択が入るか」だけで判断し、今回は**ファイル内の他3箇所が同じ背景に固定文字色を使っていた**ことと、**ライトの `var(--c-text)` が `#1A1A2E` と同値でライトが不変**であることの2つで「選択不要」と確定できた。#72 の「既定の黒」、#74 の「他は confirm があるのにここだけ無い」、#75 の「色を新しく決めないと直せない→BACKLOG」と並べると、**ループが直せるのは常に『既にコードベースの中に正解が書かれている』ケース**だと言える。これは severity とは無関係で、今回 1.02:1（不可視）を直し 3.02:1（読みにくい）を直さなかったのは、**悪さの度合いではなく答えの所在で決めた**結果である。

**次回の手を1つに絞る。** 「描画結果を測る」軸は #72〜#76 の5回で、ライト1400px・ライト375px・ダーク375px・**モーダル/フォーム面**を埋めた。**残っている最大の未測定は「タップ領域をダークとモーダルで測っていない」ことではなく、`?plan=free` と `?plan=pro` の画面**である。#72〜#76 はすべて `?plan=premium`（UpgradeModal だけ free）で測っており、**Free/Pro でしか出ないUI——上限到達の警告・Premium機能のロック表示・「Proプランで利用できます」の帯——は5回とも一度も描画されていない**。これらは**淡い背景＋注意色という、決めること1でまさに問題になっている配色**を使いやすい場所で、しかも `?plan=` を変えるだけで到達でき**コードと実機だけで閉じられる**。**次は `?plan=free` と `?plan=pro` で375px×ダーク/ライトを測り、プラン制限まわりのUIを初めて実測する。**
...
**影響範囲**: index.html（CSS変数を増やす場合）、app-admin.js（3バナー・タブ・バッジ・ボタン・日付列の曜日色 :246/:1653・ポジションチップ）、app-staff.js（出勤/休みの選択状態・CellEditPanel・曜日色・チップ :341/:627/:629/:656）、app-core.js（`AB` 等のスタイル定数）
**備考**: バグチェック#72（2026-08-13）で検出・**条件B（仕様＝デザイン判断）に該当**。グローバル指示「UIデザインの基準」により、検出結果を一括修正で先行させずユーザーの合意を得てから直す。既定の黒に依存していた24件（最悪1.18:1・シフト作成タブ）は判断不要の実装バグだったため同日 `3490707` で修正済み。**決めること3は バグチェック#75（2026-08-15）で追加**——#72 のコントラスト実測は1400px幅・ライト主体だったため、375px×ダークで初めて出た。#75 の実測では「色指定がなく既定の黒」は**0件**で `3490707` の非回帰を確認済み。**決めること4は バグチェック#76（2026-08-15）で追加**——#72〜#75 の4回はいずれも巡回で到達する画面だけを測っており、モーダル面は一度も測られていなかった。#76 で SmModal・CellEditPanel・UpgradeModal・TermsModal を開いた上で実測して初めて出た。なお #76 で見つかった「編集中セルの時刻がダークで不可視（1.02:1）」は**ライトでは 15.31:1 で、かつ同ファイル内の他3箇所が同じ背景に固定文字色を使っているという慣習があった**＝色の選択が不要な実装バグだったため、判断を待たず `17ff0a7` で修正済み。
