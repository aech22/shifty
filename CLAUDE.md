...
## Shifty バグチェックレポート（2026-08-19 自動実行 #83）

### 修正済み

- **🟡 「2週間」上限の判定が、期間をまたぐ連続2週を数えていなかった**（[app-admin.js:3038](app-admin.js)）→ `4db34ed`
  スタッフ属性別の勤務時間制限のうち、UI表記「2週間」の上限（`biweekly`）は **週を2つずつ組にして合算**していた。ところが**組の起点が `weekMap` のキー順＝その sub がたまたま触れた週**に依存する。`weekMap` のキーは `ds`（sub 自身の shifts の日付）から作られるため、**期間の区切り方がそのまま組の区切りになる**。
  **配信物の判定ブロックを切り出して Node で実行した実測**: 週08-10 に35時間・週08-17 に35時間＝**連続2週で70時間**働いても、**2週間上限60時間の判定は両期間とも `false`**。各行の `weekMap` が1週ぶんしか持たないため、組が `(その週, undefined)` になり単週35時間としか比較されない。**同じ勤務を週境界に揃った1期間（08/10〜08/23）で見ると `true`** になる——つまり**同じ人の同じ勤務が、期間の区切り方だけで判定が反転していた**。
  **断定できた根拠は、同じ問い（N日間の合計が上限を超えたか）を正しく解く実装が同じ式の中にあったこと**——直下の任意日数上限（`customDays`/`customHours`）は「**各出勤日を起点にN日間を前方に合算する**」スライディング窓で書かれており、`allWork`（`_workDatesOf` 経由・期間跨ぎ）を走査している。**同じ 70 時間を `customDays:14`/`customHours:60` で判定すると `true` になる**ことも実測した。
  **さらに時系列でも決着した**: `biweekly` の導入は `8334f8e`（2026-06-28 **23:24:21**）、スライディング窓の `customDays` は `8501732`（同日 **23:29:53**）で、**5分後に書かれた一般形のほうが正しい**。`git merge-base --is-ancestor` で先後も確認済み。**仕様判断ではなく、5分前に書いたものを更新し忘れた取り残し**と確定できた。
  窓の実装を `_windowVio(days, limitHours)` に一本化し、`biweekly` を `days=14` で通す形に揃えた（`customDays` も同じ関数を通すので、以後どちらか片方だけが直る形にならない）。**修正後のブロックを配信物から抽出し直して18ケース実行**（期間跨ぎ3・非回帰15。境界は `diffD>=14` で除外されることを13日差／14日差の2ケースで確認、別名・`adminRest`・週/月/日/任意日数の各上限の非回帰も含む）し全パス。

- **🟡 提出一覧の勤務時間マップだけが、重複キーで「最後の1件」を採っていた**（[app-admin.js:2999](app-admin.js)）→ `f6ede4a`
  「`staffName|date` → 出勤シフト」という**同じ形・同じキーのマップがコードベースに2つ**ある。ShiftEditTab の `workShiftByStaffDate`（:490-500）は `!m.has(k)` で**最初の1件**を採り、さらに `subsByKey`（:485）は「**重複時は find と同じ「最初の1件」を採用**」とコメントで規約まで書いている。ところが SubsTab の `shiftByStaffDate` だけが**無条件 `set` ＝最後の1件**だった。これは **#82 の修正（`cac2b21`・前日）で新設されたマップ**で、既存の規約から外れたのはこの1件だけである。
  **キーの衝突は実際に起こしうる**: 期間の重複には**バリデーションが無い**（`PEF`・:1999 は `onSave(f)` を直接呼ぶだけで、開始日・終了日の重なりを一切見ない。プリセット側も `startDate===p.startDate&&endDate===p.endDate` の**完全一致**しか弾かない）。重なった日に両方の期間へ提出があると `staffName|date` が衝突する。
  **実測**: 重なった5日について、**シフト作成タブ（`getWeekMin`→`_getWorkShift`）が 25:00、提出一覧の行バッジ（`_shiftAt`）が 65:00** と食い違い、**週上限40時間の超過判定が画面ごとに反転**した。修正後は両者とも 25:00 で一致することを、**修正後の配信物から `shiftByStaffDate` を抽出して評価**して確認した。**衝突が無い通常時の挙動は不変**（1件のみ・holiday 除外の非回帰も実測）。

- **（付随）上限判定の勤務日集合を行ごとに1回だけ引くようにした**（[app-admin.js:3038](app-admin.js)）→ `c629332`
  `_windowVio` に `biweekly` と `customDays` の**2つの呼び出し元ができた**ため、両方を設定した店舗では `_workDatesOf`（マップ全走査）が**行ごとに2回**走っていた。遅延1回だけ引く形にした。**自分の変更が生んだコストなので同じ回で閉じた**。判定結果は不変（18ケース再実行して全パス）。

### 要確認（未修正）

- **🔴（継続・プロセス）このループから実機E2Eが6回続けて実行できない**（環境要因・コード変更なし）
  今回も `preview_list`（稼働0件）と `preview_start` を実際に実行し、**「Dev servers can't be started from unattended sessions」と明示的に拒否された**。Bashでのサーバー起動は規約で禁止のため代替なし。**#78〜#82 に続き今回も1件も実行できなかった。ユーザーが対話セッションで `/bug-check` を手動実行すれば解消する。**
- **🟢（新規）期間の重複にバリデーションが無い**（[app-admin.js:1999](app-admin.js) `PEF`）→ **BACKLOG化済み**
  上記2件目の衝突の前提条件。`PEF` は保存時に他期間との重なりを見ず、プリセットも完全一致しか弾かない。**重複そのものを禁止すべきか**（2週と月次を並行運用したい店舗がありうる）は仕様判断のため、既存の「仕様判断が必要な挙動のまとめ」へ項目として追加した。なお**月計の二重計上は起きない**（`_shiftAt` が `name|date` につき1つのシフトしか返さないため）ことは実測で確認済み。
- **🟡 #77 の宿題（Free 上限の実機検証）は静的監査完了・実測のみ未達**（上記🔴が理由）。**残るは実機で21人目・2件目を押す1回だけ**
- **🟢 `effShiftStart`/`effShiftEnd` の未採用が app-admin.js になお3箇所残る**（`:994` ヒートマップの時間軸／`:3033` 提出一覧の出勤日数／`:3091` 詳細モーダルの出勤日数）。**#82 と同じ理由で今回も触らなかった**——直し方が BACKLOG の判断待ち項目（片側セルだけの日を何時間と数えるか）に依存する
- **🟢 `index.html` の `?v=` と `build:` が現在の配信物を指していない**（`20260814-53f610f` のまま。**今回さらに app-admin.js を3回変更したので差は広がった**）。申し送りのみ——`/release-to-main` の手順63がバンプする
- **🟢 `addAllFromWeekday`（`:2601`）と `handleSaveAll`（`:692`）のトーストが件数を多めに言う**（#81から継続・変化なし）
...
- 以下は #46〜#82 からの継続（変化なし）: 未claim店舗の取り込み／`findActiveSubscription` の `|| live[0]`／`planExpiry` のUTC計算／**片側セルだけの日が0分**／`purgeOldPeriods` のUTC日付／`generateInviteCode` のデッドコード／ほか

### 今回の対象と見つけ方

**#82 以降のコミットは0件。11回続けて「差分ゼロ」の回である。**

**(1) 申し送りが名指しした場所から、そのまま2件出た。** #82 は「次は**集計の入力になっている日付集合**を横断で洗え／`ds`・`dates`・`allD`・`perDs`・`allWork` の5種類が名前も範囲もバラバラに共存している」と書いていた。今回の2件はどちらも**その5種類の境目**にあった——1件目は `ds` 由来の週集合で組を作りながら合計だけ `allWork` 由来だった不整合、2件目は `allWork` を作るマップ自体の重複規則の不一致。**2回続けて申し送りが当たっている。**

**(2) 「同じ操作を正しく書いている箇所が他にあるか」が6回連続で決め手だった。** 1件目は**同じ式の中に正しいスライディング窓（`customDays`）があった**、2件目は**同じ形のマップが2つあり、規約をコメントに明記している方が多数派だった**。どちらも多数派へ揃えるだけで済み、設計判断が要らなかった。

**(3) #82 で足した「仕様がいつ決まり、実装がいつ書かれたか」という時系列が、今回も断定に効いた。** 1件目は `biweekly`(23:24:21) と `customDays`(23:29:53) が**同じ日の5分差**で、しかも `merge-base --is-ancestor` で先後が取れた。**5分後に書かれた一般形のほうが正しい**という並びなので、「2週間だけ別の意味にしたかった」という仕様上の説明が成り立たない。**コードの多数決だけなら残る疑いを、コミット時刻が消した。**

**(4) 「直前の回の修正が、新しい少数派を作っていないか」を初めて明示的に見た。** 2件目の `shiftByStaffDate` は **#82 の修正（前日）で新設されたマップ**で、既存の2つの同型マップが持っていた「最初の1件」の規約から外れていた。**バグチェックが自分の前回の修正を点検対象に含める**という手が有効だと分かった。#82 は「多数派へ揃える」ことで直したが、その際に**新しい式を1つ増やしている**ことまでは見ていなかった。

**(5) 自分の修正が生んだコストを同じ回で閉じた。** `_windowVio` の呼び出し元が2つになったことで `_workDatesOf` の全走査が倍になる点に気づき、`c629332` で遅延1回に直した。**「直したが遅くなった」を次回に持ち越さない。**

**(6) 配信物をNodeで実行する手法が6回目の定着。今回は初めて「ファイルから判定ブロックを読み出して評価」した。** これまでは手写しで再現していたが、今回は `app-admin.js` の該当行を読み、`const weekMap={};` から `const hasVio=` までを**実行時に切り出して `new Function` で評価**した。**手写しのズレが原理的に入らない**ため、「実物の判定がこう動く」と言い切れる。修正の前後で同じ抽出器を使い回せる点も効いた。

### 検証したこと

- `DEV_MODE`（app-core.js:12・式のまま）／`DEV_PLAN_OVERRIDE`（:125）正常
- `subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／CF `.delete()` **0件**／CF secrets **6宣言**
- 読み込み順維持（utils→core→staff→admin→main）・SRI **11本**／`database.rules.json` の `".read": true` **0件**
- 未定義CSS変数 **0件**／ダーク二重管理の乖離**なし**／SVG属性内 `var()` は `app-staff.js:9` の1件で増えていない
- `npm test` **168件パス**／`npx eslint app-*.js` **0 errors 98 warnings**（修正前後で増減なし）／`node --check functions/index.js` OK
- **日付集合レンズ**: 上限判定ブロックの `ds`（sub自身）・`wkSet2`/`moSet2`（`ds` 由来）・`weekMap`/`monthMap`（`_min` 経由で期間跨ぎ）・`allWork`（`_workDatesOf`）の対応を全て突き合わせ、**`weekly`/`monthly` は #82 の修正で期間跨ぎ済み・`daily` は1日単位で正しい・`customDays` は元から正しい**ことを確認したうえで、`biweekly` だけが取り残されていることを特定
- **同型マップの棚卸し**: `subsByKey`(:485・最初)／`workShiftByStaffDate`(:490・最初)／`shiftByStaffDate`(:2999・最後→修正)／`companyData` の `workMap`(:400-407・両方揃った方を優先) の**4つの重複解決規則**を列挙して突き合わせ
- **連勤の二重実装は無いことを確認**: `consecCounts`（:1132・「期間内」とコメントで明示）が唯一の実装で、完了済みBACKLOGにあった SubsTab 側の「最長連勤」サマリーは**現在のコードに存在しない**（`grep 連勤` で確認）。#82 が懸念した「明示のない期間内集計」は連勤については該当なし
- **修正の検証（配信物の実評価）**: biweekly 18ケース／`shiftByStaffDate` の一致・非回帰3ケース
- **未検証（今回は手段が無い）**: ブラウザでの実描画すべて／**新しく出るようになった2週間上限バッジの実表示**／**重複期間での画面間の一致**／**Free 上限に達した店舗での実ブロック**／実機 iOS Safari

### 総括

**測れない回が6回続き、それでも 🟡 が2件出て2件とも直った。** どちらも「同じ意味の値を2つの式で出している」形で、**#82 が名指しした場所（集計の入力になっている日付集合）から出た。**

**1件目は実害が大きい。** 「2週間」上限は労基法（変形労働時間制）に紐づく設定で、`periodUnit:"2week"` の店舗では**期間の切れ目がそのまま組の切れ目になる**ため、**期間をまたいで連続2週働いた場合は構造的に一度も発火しない**。しかも**同じ勤務でも期間の区切り方で判定が反転する**ので、「先月は警告が出たのに今月は同じ働き方で出ない」という形で管理者を混乱させる。

**2件目は「バグチェックが自分の前回の修正を点検した」最初の例。** #82 が入れた新しいマップが、既存2つの同型マップの規約から外れていた。**修正は多数派へ揃えることで正しくなるが、その過程で新しい式が1つ増える**——増えた式が規約に従っているかは、次の回が見るしかない。**この点検を今後の定型に入れる価値がある。**

**次回の手を1つに絞る。** 実測が復活したなら**#77 の宿題（実機で21人目・2件目を押す1回）をそのまま実行する**。使えないままなら、推奨は**「同じ形・同じキーのインデックス（Map/オブジェクト）を全部並べて、重複時の解決規則を突き合わせる」**こと——今回4つ並べたら1つだけ規則が違った。同じやり方で、`subsByKey` 系以外（`abbrToShop`・`shiftByStaffDate` 以外の `useMemo` インデックス全般）に広げる余地がある。あわせて、**#82・#83 の修正で新しく増えた式（`_windowVio`・`_allWork`）が次回の少数派になっていないか**を必ず見ること。
...
**影響範囲**: app-utils.js（`carryAdminShiftFields`・`calcNetWorkMinutes`・`resolveAlias`）、app-staff.js（`buildShift`）、app-admin.js（集計・上限判定・ポジション設定UI・`del`:2314・`gridStaff`:1060・`heatData`:762・`expXl`:2015・`buildPdfCols`:1289・**`PEF`:1999 と期間プリセット生成**）、tests/core.test.js
**備考**: バグチェック #46・#52・#57・#64・#65・**#79**・**#83** で継続検出・**条件B（仕様判断）に該当**。各項目とも「実装は難しくないが、正解が仕様側にしかない」。**#79 の2項目はどちらも「スタッフを削除する」という1つの操作から出ている**が、片方は設定データの後始末、もう片方は未登録名の表示規則で、決め方が独立するため分けて記載した。なお #79 では**確認ダイアログの不在だけ**（他の破壊的操作4箇所は全て confirm を持つ＝正解がコードベース内にあった）を判断待ちにせず `69f62bf` で修正済み。
