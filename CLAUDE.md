...
## Shifty バグチェックレポート（2026-09-12 自動実行 #122）

> 着手時の HEAD は `e8cd26c`（スクロールの巻き戻り修正 `fe54a15` を本番反映した直後）。
> #121 の申し送りは「既定値（`makeSettings`）の履歴を当たる」だったのでそれを先にやり切り、
> **クリーンだったので観点を変えた**。効いたのは **ESLint の `react-hooks/exhaustive-deps` を
> このコードベースで初めて回したこと**（`eslint-plugin-react-hooks` は未導入だったので、
> リポジトリを汚さずスクラッチパッドに入れて写しに対して回した）。

### 修正済み（🟡）

- **🟡 シフト作成タブの重い useMemo 6つが、打鍵ごとに毎回フル再計算されていた**
  （app-admin.js:490 `staffList` ／ :503 `dates` ／ :504 `realStaff`・`e8c2980`）

  `staffList`（`visibleStaffList` の `.filter`）・`dates`（`gd`）・`realStaff`（`.filter`）は
  **毎レンダー新しい配列を返す**。この3つが `heatData` / `dupErrors` / `positionErrors` /
  `positionErrorEntries` / `restCounts` / `consecCounts` の依存配列に並んでいるため、
  依存が毎回「変化した」と判定され、**6つのメモ化が1度も効いていなかった**。

  セルに1文字打つと `setLocalEdits` で1レンダー起きるので、**打鍵のたびに**ヒートマップ・
  ポジション不足の最大二部マッチング（Kuhn法・日付×ランチ/ディナー×キッチン/ホール）・
  店舗間重複・休みカウント・連勤カウントを全部やり直していた。

  **実測**（スタッフ31名・31日、メモ本体の実行時間を計測。10打鍵＝10レンダー）:

  | | メモ実行回数 | メモ実行時間の合計 | 1打鍵あたり |
  |---|---|---|---|
  | 修正前（`e8cd26c`） | **60回** | **143.4ms** | **14.3ms** |
  | 修正後 | **0回** | 0.0ms | 0ms |

  **非回帰**（同じ操作を修正前後の配信物へ流して突き合わせ）:
  - **データが変われば従来どおり再計算する**。subs変更・settings変更（休憩を1本追加）・
    名簿変更のいずれでも再計算回数は >0（メモ化が効きすぎて固まる、にはなっていない）。
  - **描画結果は4スナップショットで完全一致**。表6つの全文・グリッドセル160〜180件の値・
    **全 `td` の背景色**・body全文がバイト一致。**ポジション不足を意図的に起こして
    不足行が20件以上出る状態**でも一致した（`1日ランチ幻の役割 -1、…` の並びまで同一）。
  - `pageerror`・`console.error` ともに0件。

  **着手前に依存配列が実際の入力を推移的に覆っていることを確認した**——ここを確かめずに
  メモ化を効かせると、いま「毎回作り直すから正しい」で隠れている古い値が表に出る。
  `heatData` は `settings`・`companyData`・`fixedShiftEnabled` まで持ち、
  `restCounts`/`consecCounts` は `settings` を参照しない（`_getSub`・`getHeatVal`・
  `hasFixedCmd` の3つしか呼ばず、その3つは `subs`/`selPid`/`heatEdits`/`fixedShiftEnabled` だけを
  閉じ込める）。`hallStaff`・`spIdx` は `staffList` からの派生で、`staffList` は各依存配列に居る。

  **報告された Chrome のカクつきがこれで解消するかは未確認**（#121 の `d776c0e`・`fe54a15` と同じ立場）。
  スクロールは `setState` を起こさないので**スクロール中の再レンダーは元々発生しない**＝本件は
  カクつきの原因ではない。**直したのは打鍵の応答性**で、14.3ms/打鍵は
  ヘッドレスChromium・このMacでの値なので、Windows実機ではこれより重いと見るのが妥当。

### 要確認（未修正・すべて🟢）

- **🟢 `settings.staffAliases` が無い店舗では、別名を使う軽いメモ2つも毎回作り直される**
  （app-admin.js:2472 `unregisteredNames` ／ app-staff.js:250 `allSuggests`）。
  `const staffAliases=settings.staffAliases||{}` は、**別名を1つも登録していない店舗で
  毎レンダー新しい `{}` を返す**ので同じ形のメモ破れになる。ただし計算量が
  O(その期間のsubs) と O(スタッフ数) で、上の6つとは桁が違うため修正しない（申し送り）。
- **🟢 `makeSettings` の `templates:[]` は書かれるだけで一度も読まれない**。
  テンプレートは `shops/{shopId}/templates`（店舗単位）へ移っており、`settings.templates` の
  読み手はリポジトリ全体で0件（ヒットは `serverSnapRef.current.templates` と `snap.templates`＝
  別パス）。新規店舗すべてに空配列を1つ書いているだけの残骸。
- **🟢 `syncAxis` の echo 記録（`fe54a15`）は、反射イベントが来なかった場合に要素ごとの
  記録が残り続ける**（WeakMap のエントリを消すのは反射を受け取ったときだけ）。
  **未検証の推測にとどまる**——書き込みは「0.5px以上違う値」にしか行わないので反射が
  来ないケースを実際には作れておらず、再現も取っていない。次回この経路を触るときに確かめる。
- **#121 から継続（変化なし）**: 配信版数の申し送り（今回 app-admin.js を変えたので
  `/release-to-main` の手順63でバンプが要る）／曜日キー7の意味変更の余波／`keepStaff` の
  旧形式の読み手2箇所が未確認／`PLAN_LIMITS` の既定値10／SubsTab が現在の `staffTypeLimits` で
  判定する件／休憩タグ・`staffWorkplaces` の残骸／`oneSidedFillBounds` の式の二重化／
  属性変更の既定の件／略称バリデーションのコメント／「残す」ポップアップの括弧書き／
  プラン上限モーダルの「店舗」文言。
- **変化なし（BACKLOG化済み・条件A/Bでループの権限外）**: 二重課金の根治／特商法表記（🔴 2件）／
  解約通知／解約時のプラン判定／企業連携の解除と `verifyShopOwner` の移行猶予／
  別名提出の重複の根（#81）／PDF の実物確認／`purgeOldPeriods` の本有効化（**2026-09-11 から着手可**）／
  非表示スタッフを「未提出」に数えるか（#113）／「締」の休みの入口2つ（#118）／
  完全削除したスタッフの `keepAttrs`（#118）。
- **引き受け済みのトレードオフ（再検出しても直さない）**: `subs/$subId/.write` は認証済みなら通る。2026-08-31 決定1。

**今回 BACKLOG へ起こしたものは無い**（条件A〜Dに当たる新規項目が出なかった）。

### 検証したこと

- `npm test` は **260件パス**（テストの増減なし＝本件は純粋関数を変えていない）。
- `npx eslint app-*.js` は **0 errors・94 warnings**（増減なし）。
- `node --check functions/index.js` は OK（functions は未変更）。
- **RULES.md のスキャン項目はすべてクリア**。
  - `DEV_MODE` は式のまま（app-core.js:12）。
  - `subs` の全体 `set()` は0件。`filter(s=>s.id!==…)` は7件すべて `deletedId` 付き・
    `.remove()` 直呼び・店舗一覧の操作のいずれか。
  - `accounts` の全件読みは0件。
  - 読み込み順は utils→core→staff→admin→main のまま。SRI は11本。
  - 未定義のCSS変数0件。ダーク定義の二重管理（`[data-theme]` と `prefers-color-scheme`）に差分なし。
  - **フォーム部品58件・`fontSize<16` は0件**（CLAUDE.md の波括弧対応スキャナで判定）。
  - SVGのプレゼンテーション属性の `var()` は `app-staff.js:9`（ShiftyIcon）の1件のみ＝既知。
- `functions/index.js` の `.delete()` 誤用0件・`secrets:` 7件。
- **Firebase・Stripe には読み書きとも一切アクセスしていない**（`app-main.js` を読み込まない
  1.6節のハーネスなので `firebaseDB` は `null` のまま）。

**#121 の申し送り（`makeSettings` の既定値の履歴）の結果——クリーン**:

既定値の変遷を全コミットから機械的に復元した（キーの追加・削除・値の形の変化だけを抽出）。

| 既定値の変化 | 影響を受ける店舗 | 現在の読み手 |
|---|---|---|
| `password:DEFAULT_PW` が消えた（`f8340ca`・7/06） | 〜7/06 作成 | **読み手0件**（設定の `password` を読むコードは無い）✅ |
| `staffHelp` が入って消えた（`dfc23bb`→`e245ea8`・7/08〜7/11） | その3日間 | 読み手0件＝ただの残骸 ✅ |
| `staffTypeLimits` に `biweekly`/`monthly`/`customDays`/`customHours` が追加（`8334f8e`・6/28） | 6/16〜6/28 作成 | SetTab:4276・SubsTab:3762 が**既定を spread して補う**。上限判定4箇所は `l.monthly` 等が undefined＝falsy で「上限なし」に落ちる ✅ |
| `staffTypeLimits` の名前あり化・組み込みの4→2（`8501732`/`83107c8`） | 同上 | #121 で `getAttrOptions` を修正済み ✅ |
| `breakTimes` の `hol` → `holSat`/`holSun`（`52bf52f`・7/27） | 〜7/27 作成 | `getBreakList` と `requiredPositionsFor` が流用。CandTab:3615 と 必要ポジション:4435 は**旧「祝日」行を画面に出して消せるようにしている** ✅ |
| `positions`/`requiredPositions`/`staffPositions` が追加（`7c087cb`・7/10） | 〜7/10 作成 | 全読み手が `||{}` ガード付き ✅ |
| `staffNumbers`（7/03）・`shopAbbrs`/`staffWorkplaces`(7/08)・`overtimeSettings`(6/18)・`staffAttributes`/`staffTypeLimits`(6/16)・`breakTimes`(6/16) の追加 | それ以前 | 20キーを機械走査して**ガード無しの参照は0件** ✅ |

`templates:[]` が読まれないこと（上の🟢）だけが副産物。**古い店舗のデータで壊れる読み手は
1つも見つからなかった**ので、この観点は閉じてよい。

### 測り方の記録

**効いたのは、道具そのものを増やしたこと。** `eslint.config.js` は `no-undef` と
`react/jsx-no-undef` を本命に据えており、`eslint-plugin-react-hooks` は
**一度も入っていなかった**（`node_modules` に無い）。#122 までの121回はこのルールを
一度も回していないので、フックの依存の穴は**構造的に検出されないまま積み上がっていた**。
入れて回したら26件出て、そのうち1件が本物だった。

**リポジトリは汚さずに回せる。** スクラッチパッドに `eslint@9` と
`eslint-plugin-react-hooks` を入れ、`app-*.js` の写しをそこへコピーして
`--no-config-lookup` で当てるだけ（`package.json` も `eslint.config.js` も触らない）。
**ファイルの外から lint する形は `File ignored because outside of base path` で弾かれる**ので、
写しを作る側に回す。

**26件のうち25件は「直すと壊れる/直す必要がない」だった。** トリアージの決め手は
「**その依存が推移的に覆われているか**」で、たとえば `app-admin.js:462` の
`useEffect([periods])` は `selPid` を欠くが、`selPid` が変わるのはユーザーが有効な期間を
選んだときだけなので再実行が要らない。本物だった1件だけが逆向き——
**依存が足りないのではなく、依存が毎回変わる**形だった。`exhaustive-deps` は
これを "could make the dependencies change on every render" という別の文言で出すので、
**"missing dependency" の行だけを追うと本命を見落とす**。

**修正の前に「メモ化が効いたとき古い値が出ないか」を確かめる順番が要る。** 今回の6つは
毎回作り直すことで結果的に正しかったので、依存配列の正しさは**誰も検証していなかった**。
先に閉じ込め関係（`getHeatVal`→`getStoredTime`→`_getSub`→`subsByKey`）を辿って
覆われていることを確認し、そのうえで修正し、**3種類のデータ変更で再計算が起きること**を
実測した。この順を逆にすると、速くなった代わりに黙って古い数字を出す。

**申し送り（次回の観点）**: **`eslint-plugin-react-hooks` を常設するかを決める**（ユーザー判断）。
今回はスクラッチパッド経由で回したので、**次回も同じ手間をかけないと同じ検出ができない**。
`eslint.config.js` に足すなら `exhaustive-deps` は `warn`（既に25件の許容可能な警告が出るため
`error` にはできない）で、CI の 0 errors は維持される。足さないなら、この観点は
**バグチェック側の手順として書き残す必要がある**（道具が無いと再現できない検出だから）。
あわせて、今回 O(スタッフ数) と切り捨てた `staffAliases||{}` の形が
**他のコンポーネントにも無いか**を `|| {}` / `|| []` を依存配列に渡している箇所で洗う。

### 総括

**🔴 はゼロ。🟡 を1件検出し、その場で修正・実ブラウザでの前後実測・非回帰確認・コミットまで完了した。
BACKLOG への起票は無い。**
#121 の申し送り（`makeSettings` の既定値の履歴）は**8項目すべてクリーン**で閉じた。
代わりに観点を変え、**`react-hooks/exhaustive-deps` をこのコードベースで初めて回した**ところ、
シフト作成タブの重いメモ化6つが**一度も効いていなかった**ことが分かった。打鍵1文字ごとに
ヒートマップとポジションの二部マッチングを丸ごとやり直しており、スタッフ31名の店舗では
14.3ms/打鍵。3つの派生値の参照を安定させるだけで0になり、描画結果は
ポジション不足20件以上を含む4スナップショットでバイト一致した。
