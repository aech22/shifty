...
## Shifty バグチェックレポート（2026-09-10 自動実行 #118）

> 着手時の HEAD は `2d25d2f`。観点は **#117 の申し送りをそのまま実行**した——
> 「今度は『否定』を当たる。`触らない`・`〜しない`・`起こらない`・`対象外` を機械的に集め、
> **その否定の根拠が今も成り立つか**を1件ずつ当たる」。コード5ファイルと `functions/index.js` の
> コメントから否定の断言を **157件** 集め、根拠が他所の仕様に依存するものを優先して当たった。
>
> **申し送りの予想は当たった。ただし当たり方が予想と違った。** #117 は「否定の断言は、その根拠が
> 別の場所の仕様で覆るとバグそのものになる」と書いていた。今回見つかった3件のうち、その形は1件だけで、
> **残る2件は根拠が今も正しいのに、その否定が本来の対象より広く効いていた**——
> 「Aだから除外する」と書かれた1行が、A以外のものまで黙って除外していた。
> **正しい理由が、間違った範囲を守っていた。**

### 修正済み

- **🟡 店舗間の重複チェックが、他店舗側の片側セルを休み希望と一緒に落としていた**
  （[app-admin.js](app-admin.js) の `dupErrors`・`96e4de6`）

  企業連携で複数店舗に勤務する人の二重予約を検出する `dupErrors` は、**自店舗側**の出退勤を
  「片側セル（出勤だけ・退勤だけ）なら候補時間から補完する」規則で解決する。この補完は
  **バグチェック#86 がまさにこの穴を塞ぐために入れた**もので、直上のコメントがこう書いている——
  「補完せず早期returnすると『出勤だけ入っている日は他店舗と重なっていても一度も見に行かない』になる」。

  **他店舗側だけがその修正を受け取っていなかった。**
  ...
  直上のコメントはこの `continue` を **理由つきで否定**していた——「他店舗側で休み希望マークが
  付いたセルは勤務ではないので重複エラーにしない。`effShiftStart/End` が `""` を返し
  `null` になるため直後の `continue` で除外される」。**この理由は今も正しい。**
  正しいまま、**同じ1行が片側セルまで落としていた**。休み希望と片側セルはどちらも
  「片方が null」になるので、`||` では区別できない。

  実測（配信物の関数をそのまま Node で実行。自店舗＝田中 2026-09-15 18:00〜23:00）:

  | ケース | 修正前 | 修正後 | 期待 |
  |---|---|---|---|
  | 他店舗 17:00〜22:00（両方入力・対照） | エラーあり | エラーあり | あり |
  | **他店舗 退勤22:00のみ（片側セル）** | **なし** | **あり** | **あり** |
  | 他店舗 休み希望マーク | なし | なし | なし |
  | 他店舗 メモのみ（勤務なし） | なし | なし | なし |
  | 他店舗 09:00〜15:00（重ならない） | なし | なし | なし |

  **変化したのは片側セルの1ケースだけ**で、コメントが守りたかった休み希望の除外はそのまま残る。
  実害は「実際に重なっているのに重複エラーが出ないまま二重予約になる」——しかも
  **エラーが出ないので管理者に気づく手がかりが無い**。

  修正は他店舗側を `effShiftRangeMin`（app-utils.js）へ通した。この関数は自店舗側と同じ規則
  （片側セルは `oneSidedFillBounds` で補完・休み希望とメモのみは `null`）を1つの式で持っており、
  **同じ問いに入口を2つ持たない**形になる。補完境界は自店舗の候補時間から採る——`companyData` は
  他店舗の `settings` を読んでおらず（`shopAbbrs`・`subs`・`staffAliases` の3つだけ）それしか無いが、
  **落として見ないより近い**。重複エラーは表示だけで保存を止めないので、見落としより過検出のほうが安全な向きである。

  **ユニットテスト2件を追加し、ドリフト検出テストが修正前の app-admin.js で実際に落ちることを確認済み。**

### 要確認（未修正）

- **🟡 スタッフが「休み」にする2つの入口で、「締」の追加出勤の扱いが正反対** → **BACKLOG化済み**

  同じスタッフが同じ日を「休み」にする操作に入口が2つあり、結果が食い違う。実測:

  | 入口 | status | 追加出勤(締) | 純勤務 |
  |---|---|---|---|
  | ホーム画面で再提出（`carryAdminShiftFields`） | **work** | 23:00〜25:00（残る） | **120分** |
  | 提出状況一覧のセル編集（`SmModal`） | **holiday** | **（消える）** | **0分** |

  **両方がコメントで理由つきに否定している**。app-utils.js（2026-08-25・`a082d5d`）は
  「『締』の追加出勤も**残す**——スタッフの休み希望とは別軸の出勤という明示的な不変条件を持つため」、
  app-staff.js（2026-08-06・`3914d615`・#57）は「フラグを残したまま status を holiday にするのは
  `carryAdminShiftFields` の不変条件に反する」→だから**消す**。
  **後者が19日先に書かれ、前者の決定が届いていない。** その決定は「**再提出**で」と明示的に
  スコープされており、セル編集は対象外だった。

  **条件B（仕様判断）**: 前者に揃えると、スタッフが「休み」を選んだのにセルが出勤のまま再描画される
  （編集が効かなかったように見える）。UXとして許容するかの判断が要る。

- **🟡 完全削除したスタッフの `period.keepAttrs` が残り、同名の新人が前任者の属性を継承する** → **BACKLOG化済み**

  属性の期間指定（`keepAttrs`・2026-09-08 実装）は**書く・改名で移す・読む**の3つはあるが、
  **消す経路がどこにも無い**。`settingsWithoutStaff` は `settings` の7マップしか掃除せず periods に触らない。
  実測（`summer`＝週40h / `parttime`＝週28h）:
  ...
  新人の週上限が28hではなく40hで判定され、**出すはずの上限超過エラーが出ない**。
  これは**#79 の①と同じ形**（そのときは `settingsWithoutStaff` を入れて閉じた）で、
  **2026-09-08 に足したフィールドがその掃除の対象に入っていない**。

  **条件B（過去の記録を変えてよいかの判断）**: 完全削除した人でも終了した期間の写し
  （`snapshot.staffList`）には名前が残るため、`keepAttrs` を消すと過去のシフト表の表示が変わりうる。

- **🟢 配信版数が直近コミットを含んでいない**。`index.html` の `?v=` と `app-core.js` の `build:` は
  6箇所とも `20260908-0d5aba8` で相互に一致しているが、今回 app-admin.js を変更した。
  **`/release-to-main` の手順63 がバンプするので申し送りのみ**（#115〜#117 から継続）。
- **🟢 過去レポート本文の引用は直していない**（CLAUDE.md 末尾の記録ブロック内）。**意図的**。
  その日に測った値の記録であり、後から書き換えると記録が偽になる。
- **🟢 `oneSidedFillBounds` の式が app-admin.js にもう1つある**（`HEAT_LUNCH_END_MIN`/`HEAT_DINNER_START_MIN`）。
  コメント自身が「同じ式の3つ目の写し」と認めている。**今回の実測で両者は同じ値を返すことを確認済み**
  （`{lunchEnd:900,dinnerStart:1020}`）。依頼のないリファクタリングはしない方針のため申し送りのみ。
- **🟢 属性変更の既定が、終了済み期間ゼロのとき最新3期間すべてを旧属性のままにする**
  （app-admin.js の `openAttrDialog`・`Math.max(1,defaultKeepCount(...))`）。コメントが
  「0 の選択肢は無いので最新期間（＝1）へ丸める」と明記しており**意図的**。ユーザーは他の選択肢を選べる。
- **🟢 略称バリデーションのコメントは、トースト文言については依然として偽**（app-admin.js の CompanyTab）。#114 から持ち越し。
- **🟢「残す」を選んだときのポップアップの括弧書きが実態とずれる**（app-admin.js の StaffTab）。#114 から持ち越し・変化なし。
- **🟢 プラン上限モーダルの「店舗」文言に到達経路が無い**（app-admin.js の `UpgradeModal`）。#114 から持ち越し・変化なし。
- **変化なし（BACKLOG化済み・条件A/Bでループの権限外）**: 二重課金の根治／特商法表記（🔴 2件）／
  ...
  非表示スタッフを「未提出」に数えるか（#113 起票）。
- **引き受け済みのトレードオフ（再検出しても直さない）**: `subs/$subId/.write` は認証済みなら通り、
  提出を触れるのを本人に絞っているのは UI（`canTouch`）のみ。2026-08-31 決定1。

**今回 BACKLOG へ起こしたのは 🟡 2件**（`b6f64be`）。

### 検証したこと

`npm test` **251件パス**（+2・うちドリフト検出テストは修正前の app-admin.js では落ちることを確認済み）／
`npx eslint app-*.js` **0 errors 96 warnings**（増減なし）／`node --check functions/index.js` OK。

**否定の断言の棚卸し**（申し送りの本題）: 6ファイルのコメントから
`触らない`・`〜しない`・`起こらない`・`ことがない`・`対象外`・`できない`・`通らない` 等を機械的に集めて **157件**。
根拠が他所の仕様に依存するものを優先して当たり、以下は**根拠が今も成り立つ**ことを実測で確認した:

| 否定の断言 | 出どころ | 検証 | 判定 |
|---|---|---|---|
| `isStaffHiddenNow` は期間ごとの表示判定には使わない | app-utils.js | 呼び出しは StaffTab のバッジ1箇所のみ | ✅ |
| `visibleStaffList` を `isUnregisteredSubName` に通さない | app-utils.js | 呼び出し4箇所すべて生の名簿を渡している | ✅ |
| 凍結しないスタッフ設定マップは `staffHidden` だけ | app-utils.js | 7マップ中6が凍結対象・例外1・重複0（テストで担保済み） | ✅ |
| `keepAttrs` を持たない期間は1バイトも変わらない | app-utils.js | `applyKeepAttrs` が同一参照を返す | ✅ |
| 置換で別のキーと衝突しない（`cookieSafeKey`） | app-utils.js | `genSecureId` の実測文字集合に `.` は無く `=` はある | ✅ |
| 絞り込みでヒットが減ることはない | app-admin.js | 条件が `\|\|` で追加されるだけ＝単調に広がる | ✅ |
| 他店舗側の延長は加算できない（`companyData` が読んでいないため） | app-admin.js | `companyData` は `shopAbbrs`・`subs`・`staffAliases` の3つだけ読む | ✅ |
| ヒートマップの `getHeatShift` は `getBreaksFor` を経由しない | app-utils.js | `getHeatShift` の本体は `getOT` のみ呼ぶ | ✅ |
| 主シフトが抑制されると休憩控除も延長加算も適用されない | app-utils.js | 両フィールドとも抑制で `effShiftRangeMin` が `null` | ✅ |
| **休み希望マークのセルは重複エラーにしない** | app-admin.js | 理由は正しいが**片側セルまで落ちていた** | ❌ |
...
**フォーム部品58件の `fontSize<16` 0件**（#117 が埋めた波括弧対応版の走査を文書から抽出して実行）／
`functions/index.js` の `.delete()` 誤用 **0件**／`secrets` 7箇所。

**Firebase・Stripe には読み書きとも一切アクセスしていない。実ブラウザ検証は行っていない**
（修正は `dupErrors` の他店舗ブロック1箇所に閉じており、配信物の関数をそのまま Node で実行して
5ケースの真偽表を採った）。
**未検証**: 実ブラウザでの重複エラーの赤セル表示／企業連携2店舗での実機E2E／iOS 実機／
PDF の実物／本番データ／Stripe 実データ。

### 測り方の記録

**「根拠が偽になった否定」を探しに行って、見つかったのは「根拠は正しいが範囲が広すぎる否定」だった。**
#117 の申し送りは、否定の危険を「その根拠が別の場所の仕様で覆るとバグになる」と定式化していた。
今回の3件のうちその形だったのは「締」の1件だけで、実害の大きかった `dupErrors` は違う形をしていた——
コメントは「休み希望マークだから除外する」と言い、**それは本当だった**。
ただし除外を実行している `if(os===null||oe===null)continue` は、休み希望と片側セルの
**どちらも「片方が null」になる**ことを区別できない。

**危ないのは、否定が偽であることではなく、否定が「なぜ除外するか」しか書いていないことだった。**
「何を除外**しない**か」が書かれていないと、次に読む人は範囲を検算しない——理由が正しいので納得してしまう。
#117 が見つけた `keepStaff` の「触らない（旧名が入ることがないため）」は理由のほうが偽になった例で、
今回の `dupErrors` は理由が真のまま範囲がずれた例である。**同じ「否定」でも壊れ方が2通りある。**

**同じ関数の中に正解が書いてあった。** `dupErrors` は自店舗側で片側セルを補完しており、その25行上の
コメントが「補完せず早期returnすると一度も見に行かないことになる」と、今回の実害を**そのままの言葉で**
書いている。#86 は自分が立っている行を正しく直し、同じ関数の20行下にある同じ形を見ていない。
これは #117 が `b1f8604` について書いた「**『この箇所を直す』は『この種類を潰す』ではない**」の再演で、
2回続けて同じ形が出た。**直したときに、同じ関数の中だけでも同じ形を探し直す**のが最低限の歯止めになる。

**「消す経路が無い」は grep で出る。** `keepAttrs` の件は、否定を読む前に
`grep keepAttrs` の結果を「書く・移す・読む・**消す**」で分類しただけで出た。
新しく足したフィールドは、書く側と読む側は必ず実装されるが、**掃除する側は既存の掃除関数に
自分を登録してもらう必要がある**——そして登録漏れは何も壊さず、同名の後任が現れるまで黙っている。
#79 で `settingsWithoutStaff` を作ったときの一覧は `STAFF_KEYED_SETTING_MAPS` としてテストで
ドリフト検出されているのに、**period 側に足したフィールドには同じ受け皿が無い**。

**申し送り（次回の観点）**: **今度は「掃除される側」を当たる。** 今回の `keepAttrs` は
「書く・改名で移す・読む」の3経路が揃っていて「消す」だけが無かった。同じ形が他にもあるはずで、
`period` と `settings` に載るフィールドを機械的に列挙し、**それぞれについて
①書く ②改名で移す ③削除で落とす ④期間の写しに焼く／焼かない の4つが揃っているか**の表を作れば拾える。
`STAFF_KEYED_SETTING_MAPS` は②③を、`PERIOD_SNAPSHOT_SETTING_KEYS` は④をテストで守っているが、
**period 直下のフィールド（`keepStaff`・`keepAttrs`・`snapshot`・`urlToken`）には同じ受け皿が無い**
——`renameStaffInPeriods` が3つを手で並べているだけで、4つ目を足したときに気づく仕掛けが無い。

### 総括

**🔴 ゼロ。🟡 は3件検出し、1件をその場で修正・検証・コミットまで完了、2件は仕様判断が要るため BACKLOG へ起こした。**
#117 の申し送り（「否定」を当たる）をそのまま実行し、コメント中の否定の断言 **157件** を棚卸しした。
**いちばん実害が大きかったのは `dupErrors` の他店舗側**で、企業連携で複数店舗に勤務する人が
他店舗で「退勤時刻だけ」入っている日は、自店舗と実際に重なっていても**重複エラーが出ないまま二重予約になる**。
直上のコメントは「休み希望マークだから除外する」と理由つきで否定しており、**その理由は今も正しい**。
正しいまま、同じ1行が片側セルまで落としていた——**#117 が想定した「根拠が偽になる否定」ではなく、
「根拠が真のまま範囲がずれた否定」**という別の壊れ方だった。
しかも自店舗側では同じ穴が #86 で既に塞がれており、**正解は同じ関数の25行上に書いてあった。**
