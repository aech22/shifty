...
**残業予定は半月ごとに見える（2026-09-26 ユーザー指示）**。按分は月単位でやるが、
「月が埋まっているか」（`laborMonthCovered`・按分の条件）と「判定を出してよいか」
（`laborMonthReady`＝それ＋月の最後の期間を開いている）を**分けてある**ので、前半を開いていても
その半月ぶんの残業予定は出る（判定だけが後半に回る）。集計表の「残業予定」行がこの期間ぶんで、
月の合計はツールチップに出る。
...
- 入力欄の value に種別名を入れているので、**表示のまま blur しても何もしない**ように
  `handleBlur` が先頭で弾く（メモとして保存されるのを防ぐ）。外すときは同じコマンドを
  もう一度入れるか、時間を入力して出勤に戻す。
...
## Shifty バグチェックレポート（2026-09-26 自動実行 #148）

> 着手時の HEAD は `6cde79d`。#147（`db3a513`）以降に**労務判定の3弾がまとめて入った**
> （app-utils.js +669行・app-admin.js +978行）ので、今回はその差分だけを読んだ。
> 5回続いた「コード無変更」がここで終わり、**新しいコードから🟡が2件**出た。
> どちらも過去に一度直した形（#107 の改名漏れ／「同じ状態を作る入口が2つあり片方だけ規則が違う」）の再発で、
> **新しいフィールドが古い決定を自動では継がない**という同じ根を持つ。

### 修正済み

- **[🟡] 改名しても `period.laborTotals` のキーが移らず、年度累計・有給残・36協定の年判定が旧名に取り残される**
  （app-utils.js の `renameStaffInPeriods`）→ `d99943d`

  凍結した労務の合計は名前がキーだが、`renameStaffInPeriods` は keepStaff・keepAttrs・snapshot しか移していなかった。
  実測（配信物の関数を Node で実行・4月と5月に田中の凍結値を持つ期間、購読の窓の外）:

  | | 修正前 | 修正後 |
  |---|---|---|
  | `changed` | false | true |
  | 改名後の年度の実働・有給消化 | 0分・0日（2期間とも「読めていない」） | 21000分・1.5日 |
  | 36協定の年判定が見る月の残業予定 | 4月・5月とも不明 | 50h・60h |

  **有給残が消化ぶん多く出て、年720h・複数月平均80h・月45h超6回の判定が過去の月を見落とす。**
  CLAUDE.md が「改名では renameStaffInPeriods がキーを移す（移さないと過去期間の指定が引けずエラーが戻る）」と
  keepAttrs について書いた規則が、同じ日に足した laborTotals に届いていなかった。ユニットテスト1件追加
  （修正前の実装で落ちることを確認）。

- **[🟡] 提出一覧モーダルの休暇プルダウンが `leaveTypes` だけを書き、グリッドの `ko`/`yu`/`ke` と食い違う**
  （app-admin.js の `SubsTab`・新設 `saveLeave`）→ `68e716e`

  グリッドは休暇を入れると `adminRest` も立てて出勤時刻を抑えるが、モーダルは `saveAdj(…,"leaveTypes",…)` で1フィールドだけ書いていた。
  実測（`SubsTab` だけを実ブラウザにマウントし `onSave` を state に差し替え＝Firebase へは1バイトも出ない）:

  | 操作 | 修正前 | 修正後 |
  |---|---|---|
  | 出勤10〜18時の日に「有給」 | **実働8:00のまま有給1日も消化**（出勤日扱い） | 実働0・休暇日（グリッドと同じ） |
  | `ko` で公休にした日に「—」 | `adminRest` が残り**プルダウンが「公休」に戻る＝外せない** | 休み扱いも外れて空欄に戻る |

  あわせて枠の色が旧フィールド `s.leaveType` を見ていて常に灰色だったのを `leaveTypeOf(s)` に直した。
  回帰スクリプト `example-subs-leave-select.js` を追加（修正前の app-admin.js では EXIT=1・4項目落ち、修正後 EXIT=0）。

### 要確認（未修正）

**新規の🔴は無し。新規の🟡も残っていない**（上の2件で閉じた）。BACKLOG への新規起票は無し。

- **[🟢] モーダルで「—」を選んでも、スタッフが提出した終日の休み（`status:"holiday"`）は外れない。**
  プルダウンの表示値は `leaveTypeOf` が提出の休みも「公休」と答えるので、外そうとしても公休に戻る。
  提出データそのものを管理者が書き換えるかは仕様判断なので触っていない（出勤に変える正規の経路はグリッドへの時刻入力）。
- **#147 以前から継続（変化なし）**: BACKLOG 化済みの項目はすべて据え置き。
- **引き受け済みのトレードオフ（再検出しても直さない）**: `subs/$subId/.write` は認証済みなら通る（2026-08-31 決定1）。

### 当てて否定した仮説

- **日別の休憩上書き `adjustedBreak` が空文字や null で保存され、`Number("")===0` として「休憩なし」に化ける → 起きない。**
  `saveAdj` は値が `""`/`null` のときキーごと消す。`getBreaksFor` に届くのは数値か未定義だけ。
- **`paidLeaveGranted` がスタッフ削除の後始末から漏れる → 漏れない。** `STAFF_KEYED_SETTING_MAPS` に登録済みで、
  `PERIOD_SNAPSHOT_SETTING_KEYS` にも入っている（CLAUDE.md の登録規則どおり）。
- **非 Premium の店舗で凍結値の書き込みが空回りする → しない。** `laborTotalsRef` が空なら `totalsSame=true` で書かない。

### 検証したこと

- `npm test` **342件パス**（#147 の283件から労務判定で +58、今回 +1）・`npx eslint app-*.js` **0 errors / 95 warnings**。
- RULES.md スキャン（app-*.js へ読み替え）: `DEV_MODE` は式のまま（app-core.js:12）／`subs`・`periods` の全体 `set()` 0件／
  `accounts` 全件読み 0件／読み込み順は index.html の `<script>` 行で utils→core→staff→admin→main／`functions/` に `.delete()` 0件。
- 実ブラウザの回帰スクリプト8本（新規1本＋提出一覧2本・労務4本・シフト作成タブ1本）がすべて EXIT=0。
- **配信版数は `20260926-0b59d7b` のままで、今回の2コミットに追随していない。** リリース時に `/release-to-main` の手順でバンプすること（本ループはバンプしない）。
- **Firebase・Stripe・本番データには一切アクセスしていない。**

**申し送り（次回の観点）**

1. **スケジュールタスクの PHASE 2〜4 の grep はいまも9箇所すべて削除済みの `app.js` を対象にしている**（4回目の申し送り）。
   スキルファイル側なのでこのループでは直していない。
2. **新しい「名前キーの値」が増えたら、改名・削除・写しの3経路へ届いているかを先に当てる。** 今回の laborTotals は
   period 側に載った名前キーで、`STAFF_KEYED_SETTING_MAPS`（settings 側）のドリフト検出が効かない場所だった。
   period 側の名前キー（keepStaff・keepAttrs・laborTotals・snapshot）を一覧化してテストで照合すると同じ漏れを機械的に拾える。
3. **同じフィールドを書く入口が2つある箇所は、書くキーの集合を突き合わせる。** 休暇はグリッド（`applyEditToSubs`）と
   モーダル（`saveLeave`）の2入口で、今回は後者だけ `adminRest` を持っていなかった。
