...
## Shifty バグチェックレポート（2026-08-05 自動実行 #55）

### 修正済み

- **[🟡] 提出状況一覧のセルで「休み」に変えた日に提出時刻が残り、グリッド・PDF・Excel・ヒートマップだけがその日を出勤扱いにしていた**（app-staff.js:512 `applyCellEdit`／コミット `3dc2c72`）
  スタッフ（および期間管理タブの管理者）は「提出状況一覧」のセルをタップして、その日の出勤/休みと時刻を直接変更できる。ここで**「休み」を選ぶと、`status:"holiday"` と一緒に、直前まで選ばれていた提出時刻 `start`/`end` がそのまま書き込まれていた**。`CellEditPanel`（:458）は `useState(s.start||"18:00")` で時刻を保持したままで、`handleStatus("holiday")` が **その時刻を引数に渡して即時適用する**（:463）ため、`applyCellEdit` の `{...old, status, start:newStart, end:newEnd}`（旧:515）が休みの日に時刻を載せてしまう。
  **この「休みの日に時刻が残る」状態は、読み側が真っ二つに割れる**。**表示・カウント系は `status` を見ない**（`getStoredTime`・app-admin.js:517 は `adminRest` しか見ずに `adjustedStart??start` を返す）ため、シフト作成グリッド（`getVal`:519）・PDF（`pdfResolve`:1254）・Excel（`expXl`:2148、#54 で `hasAdminDisp` ゲートを通すようになった）・ヒートマップ（`getEffHHMM`:684 → `heatData`:755）はすべて**出勤として表示・カウントする**。一方**統計系は `status` を見る**（`calcNetWorkMinutes`・`shiftBandInfo` は app-utils.js:123/158 で `status!=="work"` を早期return）ため、勤務時間 0分・出勤日数 0日・休みカウント1のまま。**同じ管理者画面の中で、グリッドは「9:00〜17:00 出勤」・集計欄は「休み・勤務0分」と食い違う。**
  **同じ操作の他の実装は正しい**。StaffView の日付カードのトグル（app-staff.js:336）は `start:iW?...:undefined, end:iW?...:undefined` で**明示的に消して**おり、一括反映（:138）も `start:undefined,end:undefined` を書いている。管理者側の `applyEditToSubs` に至っては `if(!sd.start&&!sd.end)sd.status=sd.origStatus;`（app-admin.js:600）と、**「スタッフ提出のstart/endが残っている場合は復元しない」とコメント付きで明記して、この状態を作らないよう避けている**。「休みの日は start/end を持たない」という不変条件は 4箇所のうち 3箇所で守られ、**SmModal のセル編集だけが破っていた**。
  **実測検証**（3経路の書き込みと5経路の読み取り条件式を逐語コピーした Node 再現）: スタッフが 9:00〜17:00 で提出済みの日を休みに変更したとき、修正前は ①日付カード → `{status:"holiday"}` / グリッド`['','']` / Excel`斜線` / ヒート`カウントせず`、②一括反映 → 同じ、③**提出状況一覧のセル → `{status:"holiday",start:"09:00",end:"17:00"}` / グリッド`['09:00','17:00']` / Excel`['9','17']` / ヒート`出勤としてカウント` / 勤務0分・出勤0日** と、③だけが全表示経路で乖離した。修正後は③が①②とバイト単位で一致することを再実行で確認した。
  **修正方針**: `newStatus==="work"` のときだけ `start`/`end` を書き、休みのときは `delete` する。**管理者フィールド（`adjustedStart`/`adjustedEnd`/`adminRest`/`extraStart`/`extraEnd`/`adjustedXxxFixed`）には触れていない** — #51 でスタッフ側から消してはいけないと決めた対象であり、管理者が明示的に入れた調整値は休みの日でも従来どおり表示される（これは StaffView の `carryAdminShiftFields` 経路と同じ挙動で、非回帰ケース⑤として確認済み）。
  **影響**: 「一度出した希望を後から休みに変える」は提出期間中に最も普通に起きる操作で、提出状況一覧のセルタップはそのための最短導線。スタッフの画面と提出一覧では休みと表示されるのに、**シフトを組む管理者のグリッドには元の時刻がそのまま出続け、ヒートマップの人数にも数えられ、最終的にExcelにも出勤として載る**。管理者は「休み希望は出ていない」と判断してその日にシフトを確定してしまい、スタッフは「休みにしたのに入っている」と受け取る。しかも同じ画面の集計欄は休みと言っているため、**どちらが正しいのか管理者にも判断できない**。

### 今回の対象と見つけ方

**前回（#54）以降のコード変更は #54 の修正コミット自身のみ**（`15bc17a` と同期コミット `78e8888`）。実質的に新規差分ゼロの回が **6回連続**。今回は #54 の総括が残した手法をそのまま適用した — **「過去に直したバグの修正箇所について、その行に到達するための条件を1つ上へ遡り、その条件自体が同じ整合性を満たしているかを確認する」**。

遡る対象に選んだのは **#51 の修正**（app-staff.js:161 `buildShift` の `carryAdminShiftFields`）。この行に到達する条件を上へ辿ると、`buildShift` は **StaffView の `submit()` からしか呼ばれない**。つまり「スタッフがシフトを保存する経路」が他にもあれば、そちらは #51 の引き継ぎロジックを一切通らない。数えてみると、**スタッフ側からシフトを書き込む経路は3つ**あった: ①StaffView の日付カード（`upd`:78→submit）、②一括反映（:138→submit）、③**SmModal の提出状況一覧セル編集（`applyCellEdit`:512 → `onEditSub` → 直接 `fbSet`）**。③は submit を経由せず、**`buildShift` も `carryAdminShiftFields` も通らない完全な別経路**だった。

③を読むと、管理者フィールドの引き継ぎ自体は `{...old}` のスプレッドで意図的に担保されており（コメントもその宣言をしている）、#51 の穴は塞がっていた。**しかし同じ行が、引き継ぎとは逆方向に「消すべきものを消していない」**。#54 の教訓は「直した行の1つ上の `if` が犯人でありうる」だったが、今回出てきたのは**「直した関数の1つ上の呼び出し元が、そもそも別に存在した」**という形だった。

決め手は #50 以来 **6回連続**で効いている**コメント読み**。今回当たったのは「整合を主張するコメント」ではなく、**逆側の「この状態を避ける」と明記したコメント**である。app-admin.js:598-599 の「**スタッフ提出のstart/endが残っている場合は本人が出勤に変えているため復元しない**」は、`status=holiday` と `start/end` が同居する状態を**作らないための明示的なガード**で、書いた人はこの状態が危険だと知っていた。**危険だと名指しされた状態が、別のファイルの別の経路で無造作に作られていた**。

### 検証したこと

- **スタッフ側の「休みにする」実装の全数え上げ**: 4箇所（StaffView 日付カード:336・一括反映:138・SmModal セル編集:512・reset:50/85）。`start`/`end` を残すのは **SmModal のみ**
- **読み取り側が `status` を見るかの全数え上げ**: 見ない=表示・カウント系4経路（`getStoredTime`:517→グリッド`getVal`:519 / PDF `pdfResolve`:1254 / Excel `expXl`:2148 / ヒートマップ `getEffHHMM`:684→`heatData`:755）、見る=統計系（`calcNetWorkMinutes` app-utils.js:123・`shiftBandInfo`:158）。この分断が症状の非対称の正体
- **書き込みの永続化経路の確認（削除がFirebaseに届くか）**: スタッフ側は `onSub`→`fbSet(shops/{sid}/subs/{id}, sub)` の**フルset**、管理者側（PeriodsTab:1835）は `saveSubs`→`diffSubForFlatWrite`（app-utils.js）が `{id}/shifts/{date}` に**シフトオブジェクトごと**書くため、いずれも `delete` したキーが実際に消える。片方だけ残る心配はない
- **管理者フィールドの非回帰**: 管理者が `adjustedStart:"11:00"`/`adjustedEnd:"19:00"` を入れた日を休みに変えるケース（⑤）で、修正前後とも調整値が保持され表示も不変。`extraStart`/`extraEnd`（締め）を持つ日を出勤に戻すケース（④）でも追加出勤が維持され `netMin=660` が不変
- **`origStatus` 復元経路（app-admin.js:600）との整合**: 管理者側は `!sd.start&&!sd.end` を復元条件にしているため、今回の修正で保証される不変条件と一致する（管理者側の追加修正は不要）
- **`applyCellEdit` の呼び出し元**: `onApply`（app-staff.js:629）の1箇所のみ。シグネチャは変更していない
...
- `subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／`global/shops` 全件読み **0件**（直キー読みのみ）／`global/templates` 参照 **0件**
- 削除操作の `deletedId` 渡し漏れ **0件**（app-admin.js:1835・:2985 とも第2引数あり、app-main.js:1477 は `remove()` 直呼び）
- `database.rules.json` の `".read": true` **0件**。締めルール（`database.rules.tightened.json`）と**バイト単位で同一**であることを diff で確認
- Cloud Functions: secrets 5箇所抜け漏れなし・`.delete()` 誤用 **0件**・`Number.isNaN` ガード5箇所・`archived/shops` 二段削除4箇所とも健在
- 読み込み順 utils→core→staff→admin→main 維持・SRI 11本・今回の差分に `fontSize<16` の新規混入 **0件**

### 要確認（未修正）

- **🟡（継続・次リリースで必須）index.html のキャッシュ版数を必ずバンプすること**。現在 `20260802-94ffc47`（index.html:211-215）。**#51・#52・#53・#54 から5回連続で引き継いでいる項目**で、develop の本番未反映コミットは**計9件**（うち fix 5件）に増えた。app-utils.js・app-staff.js・app-admin.js の実挙動が変わっているため、版数を上げずに配信するとブラウザキャッシュで旧ファイルが使われ、**今回の休み希望の取りこぼしも Excel の欠落もポジション不足の誤報もスタッフ再提出の消失も直っていない状態のまま**になる。
- **🟢（継続）SubsTab の「出勤」日数が、管理者が両セルを空欄化した日を1日として数える**（app-admin.js:2953・:3000 の2箇所）。`att` の計算は `(st&&en)||(extraStart&&extraEnd)` が偽のとき `shiftBandInfo` を通さず **`:1`（＝1日）にフォールバック**する。フォールバックの `:1` は「時刻が欠けた提出日は1日とみなす」意図と読めるので、**仕様判断が要る**（#52からの継続）。
- **🟢（継続）属性を削除しても休憩のタグ（`breakTimes[].tags`）に残る**（app-admin.js `deleteType`）。残ったタグはどのスタッフの属性とも一致しないため、その休憩は**誰にも適用されなくなる**。ポジション削除が同名カスケードを実装しているのと非対称。
- **🟢（継続）Excel出力とシフト作成タブで別名提出の解決順が違う**（app-admin.js:2129 `expXl` の `ss.find`）。`expXl` は `s.staffName===nm||aliases.includes(...)` の**配列順の先頭**を採る一方、`_getSubForPeriod`（:489）と `applyEditToSubs`（:545）は**完全一致を優先**してから別名を探す。:484 のコメントは「expXlと同じ別名解決」と宣言しているが**実際には揃っていない**。登録名と別名の両方で提出が存在する場合にだけ食い違い、そのとき**管理者がグリッドで編集した内容と Excel が印字する内容が別のsubになる**。重複subの発生条件（スタッフが未登録名で提出した後に管理者がその名前を別名登録する）が限定的なため🟢のままだが、**発生すれば #54 と同種の「配布物にだけ届かない」害**になる。
- **🟢（継続）期間管理タブの Excel 出力だけが管理者調整値を反映しない**（app-admin.js:1925 の resolver なし呼び出し）。ファイル名が `_修正前` になる仕様であり意図的な使い分けのため未修正。
- **🟢（継続）ヒートマップの時間列収集が `adminRest` を無視**（app-admin.js:980 付近）。列見出しの範囲決めのみで人数カウントは正しく除外。実害は余分な列が1本出る程度。
- **🟢（継続）店舗間シフト重複エラーが片側入力と他店舗の `adminRest` を見ない**（app-admin.js:833）。#53 で**意図的に据え置いた**。店舗跨ぎは #47 以降 対象外方針。
- **🟢（継続）`sanitizeForSet` の root 分岐が「書かない」ではなく「消す」に倒れている**（app-utils.js:505）。到達経路は今日も0件。
...
- **🟢（継続）スタッフ削除時に設定マップのキーが残る**（app-admin.js:2247 付近）。改名側（`renameKey`）は7マップ全て移し替える実装になっているのと非対称。
- **🟢（継続）同名ポジション登録の仕様**（#46・判断待ち）／**ポジション不足サマリーがセクション非区別**／**`isSpecialRedDate` のテスト不在**（BACKLOG登録済み）
- **🟢（継続・期限到来）`PURGE_OLD_PERIODS_DRY_RUN` の本有効化**（functions/index.js:470 付近）。着手予定 **2026-08-12 以降**＝あと **7日**。
- **🟢（継続）実機E2E未実施**: 「曜日別から選ぶ」UI／#46 削除操作／#47 改名修正／#51 のスタッフ再提出／#52 の Excel 締めセル／#53 の片側入力／#54 の休み提出日メモ。**今回の修正も実機E2Eは未実施**（書き込み3経路・読み取り5経路の条件式を逐語コピーした Node 再現で一致を確認したが、実際に提出状況一覧のセルをタップして休みに変え、シフト作成グリッドが空欄になることの目視確認は未実施）。
- **🟢群（継続）**: 変更マークの締切ゲート対象外（app-staff.js:166）／`globalTemplates` の命名／`VISION.md` 不在

### 総括

**6回連続で、実質コード変更ゼロの回に構造から 🟡 を1件見つけた**。#50「フラグの解釈がレイヤーをまたいで配られていなかった」→ #51「引き継ぐべきフィールドの一覧がハードコードされ後発分が載らなかった」→ #52「同じ表示を組み立てる経路が3つあり後発フィールドが1つにだけ配線されていなかった」→ #53「同じ問いに答える判定が5つあり片側入力の扱いが1つにしか実装されていなかった」→ #54「同じ表示を決める経路が3つあり、1つだけが他の2つが使わない入力でブランチを切っていた」→ #55「**同じ操作を行う経路が4つあり、1つだけが不変条件を破って書き込んでいた**」。6本目の枝も同じ根 —「**ある扱いが、それを必要とする全箇所に届いていない**」— から生えている。

今回はっきりしたのは、**「1つ上へ遡る」には2つの方向がある**ということ。#54 で使ったのは**制御フローの上**（その行に到達する `if` は正しいか）だったが、今回当たったのは**呼び出しグラフの上**（その関数を呼ばずに同じ仕事をする別経路はないか）だった。#51 は `buildShift` を直したが、`buildShift` を通らずにシフトを保存する経路が最初から別に存在しており、そこは #51 の検討範囲に一度も入っていなかった。**次回以降はこれを使う — 過去に直した関数について「この関数を経由せずに同じデータを書き込む/読み出す経路はいくつあるか」を数え、修正がその全部に効いているかを確かめる**。1関数を直しても、同じ仕事の入口が他にあれば症状は残る。

**コメント読みは6回連続で決め手になった**が、今回効いたのは**これまでと逆の種類**である。#50〜#54 で当たってきたのは「揃えた」と主張するコメント（宣言と実装のズレ）だったが、今回当たったのは「**この状態は作らない**」と明記した防御コメント（app-admin.js:598）だった。**危険な状態に名前を付けているコメントを見つけたら、その状態を作りうる箇所を全部数える**という使い方ができる。ある実装者が「避けるべき」と気づいて書き残した不変条件は、たいてい**そのファイルの中でしか守られていない**。

壊れ方は #53 に近い**誤報**だが、方向が逆で悪質である。#53 は「不足していないのに不足と出る」＝**過剰な警告**だったのに対し、今回は「休み希望を出したのに出勤として表示される」＝**出した意思が消える**。しかも #54 で指摘した「相手に渡る唯一の経路（Excel）」の問題と重なっていて、**スタッフの意思がグリッドを素通りしてExcelまで到達しない**。さらに厄介なのは、**スタッフ側の画面では正しく「休み」と表示される**こと。スタッフは自分の希望が通っていると信じ、管理者は希望が出ていないと判断する。**双方とも自分の画面では矛盾に気づけない**という点で、#54 の「気づかれないまま情報が伝わらない」害が**両側に発生する**形になっている。データの片側だけを更新して他方を残す書き込みは、こういう「誰も間違いに気づけない」状態を作りやすい。
