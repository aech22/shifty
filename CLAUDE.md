...
**受け入れ条件**:
- [ ] `existSub`（app-staff.js:158）を app-admin.js:552-559 と同じ別名フォールバック付きにする
- [ ] 再利用時に `staffName` を登録名へ正規化するかを決める（**ユーザー判断**）。正規化するなら app-main.js:1517 の `findIndex` を id 一致（または別名込み）へ合わせる
- [ ] 実機E2Eで「別名で提出 → 管理者が別名登録 → 別端末から再提出」を通し、subが1件のままで管理者調整値が引き継がれることを確認する
- [ ] Excel出力に本人の最新提出が出ることを確認する
- [ ] 既に2つのsubができている店舗があるか本番データで確認し、あれば統合方針を決める
