...
- **🟡（継続・次リリースで必須）index.html のキャッシュ版数を必ずバンプすること**。現在 `20260802-94ffc47`（index.html:211-215）。**#51〜#55 から6回連続で引き継いでいる項目**で、develop の本番未反映コミットは**計11件**（うち fix 6件）に増えた。app-utils.js・app-staff.js・app-admin.js の実挙動が変わっているため、版数を上げずに配信するとブラウザキャッシュで旧ファイルが使われ、**今回の提出件数の誤表示も、休み希望の取りこぼしも、Excelの欠落も、ポジション不足の誤報も、スタッフ再提出の消失も直っていない状態のまま**になる。
