...
## Shifty バグチェックレポート（2026-07-29 自動実行 #47）

### 修正済み

- **[🟡] スタッフを改名すると属性・ポジション・別名・退勤延長・社員番号・勤務先の設定が黙って初期値に戻っていた**（app-admin.js:170 `onRenameStaff`／コミット `399d220`）
  改名ハンドラは `staffColors` のキーだけを新しい名前へ移し替え、**同じくスタッフ名をキーに持つ他の6マップを旧名に取り残していた**（`staffAttributes` / `staffNumbers` / `staffPositions` / `staffAliases` / `staffWorkplaces` / `overtimeSettings.byStaff`）。`staffList` と `subs[].staffName` は正しく書き換わるため、**アプリ上は改名が成功したように見えるのに設定だけが切り離される**。実害は表示崩れではなく計算結果のズレで、深刻な順に: ①`staffAttributes` の消失で属性が `parttime` にフォールバックし（app-utils.js:212）、`getBreaksFor` がタグ付き休憩を属性で絞るため **`calcNetWorkMinutes` の純勤務時間そのものが変わる**うえ、`staffTypeLimits` の参照先も変わって日・週・隔週・月・カスタム期間の**超過警告が丸ごと消える**（app-admin.js:2921）②`staffPositions` の消失で `matchPositionSlots` に渡す保有ポジションが空になり**ポジション不足エラーが誤判定**になる（#46 の `delPos` 巻き添え削除と同じ実害クラス）③`staffAliases` の消失で別名提出の既存データが「未登録名」に落ちる ④退勤延長が0に戻る／Excel・PDFの社員番号が空になる／企業連携の他店舗シフト重複エラーが働かなくなる。全マップを移し替える `renameKey` ヘルパーで修正。**旧実装との出力一致をNodeで実測確認**（20項目全PASS）: 追加設定を持たないスタッフの改名では**旧実装と完全に同一の出力**、旧実装で6項目が実際に失われること、修正後は6項目すべてが新名へ移り旧名キーが残らないこと、他スタッフとスタッフ名以外のキーが不変であること、`overtimeSettings` 未定義のレガシー設定でクラッシュしないこと、レガシー数値形式と falsy 値（`""`/`[]`/`0`）も欠落しないことを検証済み。

### 今回の対象と見つけ方

前回（#46）の HEAD は `07519c9`。以降のコード変更は **`4e02c81`（app-admin.js の1行）のみ**で、内容は提出一覧の別名登録セレクトからスペーサー行を除外する修正（現 app-admin.js:2934）。**この1行は正しい修正**。他ファイルに差分なし。

差分が1行しかないため、**その1行が示す「バグの型」を手がかりに同型の未修正箇所を横断で探す**方針を取った。`4e02c81` はスタッフ名リストの扱い漏れであり、そこから「スタッフ名をキーに持つデータの整合性」という切り口を立て、`settings.*` の全キー（18種）を出現回数つきで列挙してスタッフ名キーのマップ7種を網羅特定した（`staffTypeLimits` は属性ID、`positions`/`requiredPositions` はポジション名、`dateCandidatePosTypes` は日付がキーで対象外）。そのうえでスタッフ名が変化する経路（改名・削除）を追って改名ハンドラの取りこぼしに到達した。**同ファイルの `deleteType`（属性削除時に `staffAttributes` を掃除・app-admin.js:3386）は正しくカスケードしている**のと対照的で、カスケードを書ける設計なのに改名だけが `staffColors` 追加時のまま更新されていなかったと見られる。
...
- 分割構成: 読み込み順 utils→core→staff→admin→main 維持・SRI 11本・`fontSize<16` の新規混入**0件**
- `npm test`: **128件パス**（0 fail）、`npx eslint app-*.js`: **0 errors** 98 warnings（修正前後で同数）
- 実機起動確認: localhost でマウント・ログイン画面の描画、`AdminView`/`StaffTab`/`SetTab`/`isSpacer` が `function` として評価されること、**コンソールエラー0件**を確認

### 要確認（未修正）

- **🟡（継続・リリース前バンプ必須）index.html のキャッシュバスティング版数が HEAD とズレている**。`?v=20260728-11bc41f` に対し develop の HEAD は `399d220`。#46 の修正2件と今回の1件を本番に届けるには版数バンプが必須
- **🟡（継続・要ユーザー判断）未反映の修正が計3件**（`d3c5166`・`7961063`・`399d220`）。いずれも develop のみで本番未反映。main へのマージは規約どおりユーザー確認が必要で、並行セッションの有無も確認のうえリリースすること
- **🟢（新規）スタッフ削除時に設定マップのキーが残る**（app-admin.js:2243 `del`）。通常は無害だが、**同名スタッフを後から再登録すると前任者の属性・ポジション・退勤延長を黙って引き継ぐ**。修正するなら `del` にも同じ掃除を入れる
- **🟢（新規）改名時に他店舗の `staffWorkplaces` は更新されない**。企業連携で他店舗側に登録された勤務先マップは自店舗 `settings` の外にあるため旧名のまま残る。店舗をまたぐ整合は権限モデル上も別扱いが要るため対象外とした
...
- **🟢（継続）実機E2E未実施**: 「曜日別から選ぶ」UI／#46 のポジション同名重複下での削除操作／**今回の改名修正も実機E2Eは未実施**（Nodeでの旧新出力比較20項目と起動・コンソールエラー0件の確認まで。dev テスト店舗のスタッフ名と設定の書き換えを伴うため見送り）
- **🟢群（継続）**: 変更マークの締切ゲート対象外（app-staff.js:166）／`globalTemplates` の命名／capabilityモデルの残存リスク／`VISION.md` 不在

### 総括

新規差分が実質1行しかない回だったが、**その1行が示す「スタッフ名の扱い漏れ」という型を横断調査したことで、既存コードに眠っていた🟡1件を発見できた**。スタッフの改名が `staffColors` 以外の6マップを旧名に置き去りにしており、**改名は成功したように見えるのに、属性が消えて純勤務時間と超過警告が変わり、ポジション不足エラーが誤判定になり、別名解決が切れる**という、画面上は気づきにくく集計だけが静かにズレるバグだった。#46 の `delPos` 巻き添え削除と根が同じで、**スタッフ名・ポジション名という文字列をキーに設定を複数マップへ分散保持している以上、キーが変化・消滅する操作のたびに全マップを追随させる必要がある**のに、その追随がマップ追加時に更新され損ねている。設定マップが増えるたび同じ穴が空く構造なので、次に `settings.staffXxx` を追加するときは改名・削除の両ハンドラの更新をセットで行うこと。
