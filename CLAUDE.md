...
## Shifty バグチェックレポート（2026-08-06 自動実行 #57）

### 修正済み

- **[🟡] 提出状況一覧のセルで「休み」にした日に、管理者の調整値と「締」の追加出勤が残り、表示・カウント系だけが出勤扱いのままになっていた**（app-staff.js:512 `applyCellEdit`／コミット `3914d61`）
  **#55 が直したのと同じ関数の、直しきれていなかった半分**である。#55 は「休みにした日にスタッフ提出の `start`/`end` を残さない」ようにしたが、**管理者がシフト作成タブで入れた `adjustedStart`/`adjustedEnd`、および「締」の追加出勤（`adjustedStartFixed`/`adjustedEndFixed` と `extraStart`/`extraEnd`）はそのまま残していた**。:520 のコメントは「管理者フィールド（adjustedXxx・adminRest・extraStart等）はここでは触らない（#51の引き継ぎ対象）」と**残すことを明示的に宣言していた**が、残すと `status` は `holiday` なのに `getStoredTime`（app-admin.js:514）が調整値を返すため、**#55 が消したはずの矛盾がそっくり再現する**。
  **実測（app-utils.js の実関数と各読み出し側の式を逐語コピーした Node 再現）**: 8/10 に 9:00〜22:00 で提出があり、管理者がグリッドで 10:00〜20:00 に調整した日を、提出状況一覧のセルから「休み」にすると、修正前は **A グリッド/PDF/Excel 表示=10:00〜20:00 ／ B ヒートマップ=10:00〜20:00 に計上 ／ C 休みカウント=0（＝休んでいない）** の一方で **D 勤務時間=0分 ／ E 出勤日数=0日** になった。「締」が付いた日で同じ操作をすると、ヒートマップに 23:00〜25:00 の追加出勤が残ったまま勤務時間だけ 0 分になった。修正後は 5 経路すべてが「休み（表示なし・休み値1・0分・0日）」で一致することを再実行で確認した。
  **「締」については明文化された不変条件そのものを破っていた**。`carryAdminShiftFields`（app-utils.js:186-189）は「追加出勤フラグ（adjustedXxxFixed）を引き継いだ日は status="work" に戻す。戻さないと extraStart/extraEnd だけが残り calcNetWorkMinutes/shiftBandInfo の status!=="work" 早期returnで追加出勤が0分に落ちる」と書いており、`applyEditToSubs`（app-admin.js:610-612）も同じ不変条件を維持している。**この不変条件を維持する箇所が2つ、破る箇所が1つ（applyCellEdit）**という構図で、#55 と同じ「同じ状態を作る経路が複数あり、1つだけが不変条件を破る」型だった。
  **修正方針**: `newStatus!=="work"` の分岐で `adjustedStart`/`adjustedEnd`/`adjustedStartFixed`/`adjustedEndFixed`/`extraStart`/`extraEnd` も削除する（#55 が `start`/`end` に対してやったのと同じ扱いに揃える）。**メモ（`adjustedStartNote`/`adjustedEndNote`）と休み希望（`adminRest`）は意図的に残した** — 休みの日でもメモは表示されるのが #54 で確定した仕様であり、`adminRest` は「休み」と同じ意味なので消す理由がない。非回帰再現でメモ付き・休み希望付きの日を休みにしても両者が残ること、「休み→出勤」に戻す操作が従来通り動くことを確認した。
  **影響**: 壊れ方は #55 と同じ**「休みにしたのに出勤に見える」**だが、消えるのが**管理者自身が組んだシフト**である分、実害が一段重い。管理者はシフト作成タブで時間を調整してから提出状況一覧で最終確認する動線を通るため、**そこで休みに変えた日が、PDF・Excel・配布用のシフト表には調整後の時間のまま印字される**。当人は「休みにした」と認識しているので照合されず、そのまま配布される。「締」（深夜の追加出勤）の場合は逆に、ヒートマップの深夜帯には人がいるのに勤務時間集計が 0 分になるため、**深夜帯の人員は埋まっているように見えるのに労働時間として計上されない**。

### 今回の対象と見つけ方

**前回（#56）以降のコード変更は #56 のコメント行番号の訂正のみ**（app-admin.js の 2 行、コメント文のみ）。実質的に新規差分ゼロの回が **8回連続**。今回は #56 の総括が指定した手法（「その数字は自分の入力を含んでいないか」で集計系を一巡する）から入ったが、**当たりは別ルート**だった。集計系を一巡する過程で `effShiftStart`/`effShiftEnd`（app-utils.js:110/115）のコメントに行き当たり、そこに **#50 の教訓が「読み出し側の責務」として書かれている**のを読んだ。

> 「ここを通さずに adjustedStart??start を直接読むと『セルは休みで表示され、休みカウント・ヒートマップも休み扱いなのに、勤務時間と出勤日数の集計だけが提出値のまま計上される』というズレになる」（app-utils.js:104-106）

このコメントは「**status と実効時刻が食い違う状態はズレになる**」と言っている。そこで**「status と時刻が食い違う shift を作れる書き込み経路はいくつあるか」**を数えたところ、`status:"holiday"` を書く経路は3つあり（StaffView の提出 `buildShift`、SmModal の `applyCellEdit`、`applyEditToSubs` の origStatus 復元）、うち **`applyEditToSubs` は時刻入力時に必ず `status="work"` を立て、空欄化時は `origStatus` に戻すという形で整合を保っている**のに対し、**`applyCellEdit` は時刻フィールドを残したまま status だけ holiday にしていた**。

**コメント読みは8回連続で決め手**になった。今回当たったのは #55（防御コメント）・#56（印の導入コメント）とはまた違う **「この読み方をしないとこうズレる」という失敗の型を書いたコメント**で、しかもそれが**バグのある関数ではなく、正しく直された側の関数（effShiftStart）に**書かれていた。**過去の修正が残した「なぜこうしたか」のコメントは、同じ壊れ方を再現できる他の経路を探す検索キーになる**。

### 検証したこと

- **「その日を休みにする」書き込み経路の全数え上げ**: 3経路（app-staff.js:161 `buildShift`／app-staff.js:512 `applyCellEdit`／app-admin.js:530 `applyEditToSubs`）。`applyEditToSubs` は整合維持、`applyCellEdit` は破っていた（修正済み）、`buildShift`＋`carryAdminShiftFields` は同じ状態を作りうる（下記 要確認）
- **休みにした日を「出勤として扱ってしまう」読み出し経路の全数え上げ**: 5経路（app-admin.js:514 `getStoredTime`＝グリッド／:1242 `pdfResolve`＝PDF／:2105 `hasAdminDisp`＋resolver＝Excel／:750 `heatData`＝ヒートマップ／:1093 `restCounts`＝休みカウント）。いずれも `status` を見ずに実効時刻だけで判断しており、**修正前は5経路すべてが出勤扱い、勤務時間（`calcNetWorkMinutes`）と出勤日数（`shiftBandInfo`）だけが0**だった
- **`applyCellEdit` の呼び出し元（SmModal）の全数え上げ**: 3箇所（app-staff.js:227・:248＝スタッフ画面、app-admin.js:1835＝期間管理タブ）。1関数の修正で3経路すべてに効く
- **削除キーが Firebase から実際に消えることの再確認**: スタッフ側は `onSub`→`fbSet`（app-main.js:1469）で sub 全体 set、管理者側は `saveSubs`→`diffSubForFlatWrite`（app-utils.js:578）が日付ノードごと書くため、どちらも `delete` したフィールドは残らない
- **非回帰**: メモ（`adjustedStartNote`）と休み希望（`adminRest`）が休みにしても残ること、「休み→出勤」に戻す操作が従来通り時刻を書き戻すこと、#55 の元ケース（提出時刻のみ）が従来通り消えることを再現スクリプトで確認
- **`npm test` 155件パス**（増減なし）、**`npx eslint app-*.js` 0 errors 98 warnings**（いずれも修正前と同数）
...
- 削除操作の `deletedId` 渡し漏れ **0件**（app-admin.js:1835・:2989 とも第2引数あり、app-main.js:1478 は `remove()` 直呼び）
- `database.rules.json` の `".read": true` **0件**。締めルール（`database.rules.tightened.json`）と diff で完全一致
- Cloud Functions: secrets 5箇所抜け漏れなし・`.delete()` 誤用 **0件**
- 読み込み順 utils→core→staff→admin→main 維持・今回の差分に `fontSize<16` の新規混入 **0件**

### 要確認（未修正）

- **🟡（新規・仕様判断が要る）スタッフが再提出でその日を「休み」にすると、管理者の調整値が残ったまま status だけ holiday になる**（app-staff.js:161 `buildShift` → app-utils.js:190 `carryAdminShiftFields`）。今回修正した `applyCellEdit` と**まったく同じ壊れ方**（表示・ヒートマップ・休みカウントは出勤／勤務時間・出勤日数は0）だが、**こちらは直し方が一意に決まらない**。`carryAdminShiftFields` は「締」の追加出勤フラグについてだけ `status="work"` に戻す不変条件を持っており、`adjustedStart`/`adjustedEnd` にも同じ扱いを広げれば「管理者の調整が勝つ」、逆に調整値を落とせば「スタッフの新しい休み申告が勝つ」となる。**どちらが正しいかは仕様判断**（#51 は「管理者が作り込んだものを黙って消さない」方針で引き継ぎを入れた経緯がある）。再現手順は今回のレポートの S1 と同じで、`carryAdminShiftFields({status:"holiday"}, {status:"work",adjustedStart:"10:00",adjustedEnd:"20:00"})` が `{status:"holiday",adjustedStart:"10:00",adjustedEnd:"20:00"}` を返すことで確認できる
- **🟡（次リリースで必須）index.html のキャッシュ版数を必ずバンプすること**。本番（origin/main）は `20260805-78e8888`、develop は `20260802-94ffc47`（release-to-main が main 側でのみ版数を上げる運用どおりで異常ではない）。**現時点の本番未反映は `origin/main..develop` の6コミット（うち fix 3件＝#55 の提出状況一覧セル・#56 の提出件数・#57 の調整値残り）**。版数を上げ忘れると app-staff.js・app-admin.js の実挙動が変わっているためブラウザキャッシュで旧ファイルが使われ、3件とも直っていない状態のまま配信される
- **🟢（新規）`ADMIN_SHIFT_FIELDS` の完全性テストが、登録漏れを検出できない形になっている**（tests/core.test.js:1124）。テストは期待値の配列をテストファイル内にハードコードして `deepStrictEqual` で突き合わせるだけなので、**実装（app-admin.js が shift オブジェクトに書くフィールド）から導出されていない**。実際に **`changed`（app-admin.js:1179 `toggleChanged` がトリプルクリックで書き込み Firebase に永続化する管理者フィールド）が未登録**で、スタッフが再提出すると管理者が手で付けた変更マークは消える（app-staff.js:163 が `delete nw.changed` して差分から作り直すため）。作り直す設計自体はコメントで宣言されているので実害は「管理者の手動マークが自動判定に上書きされる」に留まるが、**レジストリとテストの両方が実装から独立している**ため、次に管理者フィールドを増やしたときは #51 と同じ取りこぼしが再発しうる
- **🟢（新規）マイページの「スタッフ数」使用量バーが空白列（スペーサー）を人数に数える**（app-admin.js:3957 `<Bar used={staffList.length} .../>`）。同じ「登録スタッフ数」を答える他の3箇所（上限判定 :2274／スタッフタブ表示 :2327／上限警告 :2441）はいずれも `staffList.filter(n=>!isSpacer(n)).length`。上限の実施は正しい側の数え方をしているため機能的な実害はなく、発生条件も「Proでスペーサーを追加 → Freeへ解約」に限られる（#56 からの継続・再確認済み）
- **🟢（継続）SubsTab の「出勤」日数が、管理者が両セルを空欄化した日を1日として数える**（app-admin.js:2957・:3004）。`att` は `(st&&en)||(extraStart&&extraEnd)` が偽のとき `shiftBandInfo` を通さず `:1` にフォールバックする。**さらに今回、この判定が生値 `adjustedStart??start` を読んでおり `effShiftStart` を通していない**ことも確認した（片側だけ休み希望を入れた日は逆に0日になる）。ゲートを実効値に直すと空欄化の日が1日側に倒れるため、**フォールバック `:1` の意味を決めないと直せない**（#52からの継続）
- **🟢（継続）属性を削除しても休憩のタグ（`breakTimes[].tags`）に残る**（app-admin.js:3423 `deleteType`）。残ったタグはどのスタッフの属性とも一致しないため、その休憩は誰にも適用されなくなる
- **🟢（継続）Excel出力とシフト作成タブで別名提出の解決順が違う**（app-admin.js:2133 `expXl` の `ss.find` は配列順の先頭、`_getSubForPeriod`:489 と `applyEditToSubs`:545 は完全一致優先）
- **🟢（継続）期間管理タブの Excel 出力だけが管理者調整値を反映しない**（app-admin.js:1929 の resolver なし呼び出し）。ファイル名が `_修正前` になる意図的な使い分けのため未修正
- **🟢（継続）ヒートマップの時間列収集が `adminRest` を無視**（app-admin.js:980）。列見出しの範囲決めのみで人数カウントは正しく除外
- **🟢（継続）店舗間シフト重複エラーが片側入力と他店舗の `adminRest` を見ない**（app-admin.js:825 `dupErrors`）。店舗跨ぎは #47 以降 対象外方針
- **🟢（継続）`sanitizeForSet` の root 分岐が「書かない」ではなく「消す」に倒れている**（app-utils.js:505 付近）。到達経路は今日も0件
- **🟢（継続）`_fbGuard` の本番分岐が PostHog へパスをそのまま送る**（app-core.js:65）
- **🟢（継続）eslint ルールの抜け穴: 参照を変数へ置く形**（`const r=ref(p); r.set(v)`）。現状の使用箇所は0件
- **🟢（継続）スタッフ削除時に設定マップのキーが残る**（app-admin.js `deleteStaff` 付近。改名側 :176 `renameKey` は7マップ全て移し替える実装で非対称）
- **🟢（継続）同名ポジション登録の仕様**（#46・判断待ち）／**ポジション不足サマリーがセクション非区別**／**`isSpecialRedDate` のテスト不在**（BACKLOG登録済み）
- **🟢（継続・期限到来）`PURGE_OLD_PERIODS_DRY_RUN` の本有効化**（functions/index.js:470 付近）。着手予定 **2026-08-12 以降**＝あと **6日**
- **🟢（継続）実機E2E未実施**: 「曜日別から選ぶ」UI／#46 削除操作／#47 改名修正／#51 のスタッフ再提出／#52 の Excel 締めセル／#53 の片側入力／#54 の休み提出日メモ／#55・#57 の提出状況一覧セル。**今回の修正も実機E2Eは未実施**（5経路の読み出し式を逐語コピーした Node 再現で一致を確認したが、実際にグリッドで時刻を調整してから提出状況一覧で休みにし、PDF・Excel に時間が残らないことの目視確認は未実施）。また `applyCellEdit` は SmModal 内のローカル関数で export されていないため**ユニットテストが無い**（#55 も同様に追加されていない）。同じ関数で2回続けて同型のバグが出たので、切り出してテスト対象にする価値がある

### 総括

**8回連続で、実質コード変更ゼロの回に構造から 🟡 を1件見つけた**。#50〜#56 の7本に続く8本目も同じ根 —「**ある扱いが、それを必要とする全箇所に届いていない**」— から生えているが、今回は**同じ関数で2回続けて同型のバグが出た**という点が新しい。#55 も #57 も `applyCellEdit` の「休みにする」分岐で、#55 は `start`/`end` を消し、#57 は `adjustedStart`/`adjustedEnd`/`extraStart`/`extraEnd` を消した。**1回目の修正が「消すべき値の一覧」を作らずに、目についた2つのフィールドだけを消して終わっていた**のが原因である。

ここから得られる次回の手法は明確で、**フィールドを消す・引き継ぐ・除外するといった修正をしたときは、「そのフィールドと同じ役割を持つ他のフィールド」を必ず数える**こと。今回の場合、「その日の実効出退勤を決めるフィールド」は `start`/`end`/`adjustedStart`/`adjustedEnd`/`extraStart`/`extraEnd` の6つで、`getStoredTime`（app-admin.js:514）と `effShiftStart`/`effShiftEnd`（app-utils.js:110/115）と `calcNetWorkMinutes`（:137）を読めば全量が1画面で分かった。**#51 が `ADMIN_SHIFT_FIELDS` という一覧を作ってレジストリ化したのは正しい対処**だったが、そのレジストリは「再提出で引き継ぐもの」の一覧であって「休みにしたら消すもの」の一覧ではなく、**役割の違う2つの一覧が必要だったのに1つしか作られていなかった**。

**コメントの読み方も一段進んだ**。#55 は防御コメント（「この状態は作らない」）、#56 は印の導入コメント（「この印で除外する」）が決め手だったが、今回決め手になったのは **正しく直された側の関数に書かれた「この読み方をしないとこうズレる」という失敗の型の記述**（app-utils.js:104-106）だった。バグは `effShiftStart` にはなく、**`effShiftStart` のコメントが書いている失敗の型を別の場所で再現できないかを探して見つかった**。つまり**過去の修正コメントは、直った箇所の説明ではなく、同じ壊れ方を探すための検索クエリとして読める**。次回はこの使い方を明示的に採る — 過去レポートで直した関数のコメントを順に読み、そこに書かれた「ズレの形」ごとに「他にこの形を作れる経路は？」を数える。

害の質は #55 と同型（「休みにしたのに出勤に見える」＝意思の消失）だが、**消えるのが管理者自身の作業結果である分だけ重い**。管理者は「調整する → 一覧で確認して休みに直す」という順で作業するため、**壊れるのは必ず作業の最後の一手**であり、その直後に PDF・Excel を出して配布する。#56 が「作業が進むほど問題が見えなくなる」誤り方だったのに対し、今回は「**最後に直したはずのものだけが直っていない**」という誤り方で、確認の順序上いちばん気づかれにくい位置にある。
