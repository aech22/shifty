...
## Shifty バグチェックレポート（2026-09-11 自動実行 #121）

> 着手時の HEAD は `dd125a3`（#116〜#120 の本番リリース直後）。観点は **#120 の申し送り**どおり、
> 「形式を変えたコミット」を拾い、そのキーの読み手が旧形式を扱えているかを1つずつ確かめた。

### 修正済み（🟢）

- **🟢 名前なしの組み込み属性（派遣・その他）が、休憩タグの選択肢から落ちていた**
  （app-utils.js の `getAttrOptions`・`d41194e`）

  2026-06-16〜06-28 の `makeSettings` は `dispatch`/`other` を `{daily,weekly}` だけで保存した（`5f34510`）。
  `8501732` で名前が付き、`83107c8` で既定が社員・バイトのみになったので、**名前なしで残るのはこの12日間に作られた店舗だけ**。
  スタッフタブの属性選択と設定タブの制限一覧は `STAFF_TYPE_LABELS` で「派遣」「その他」を補うのに、
  `getAttrOptions` は同じ補完を社員・バイトにしか当てていなかった。#120 と同じく、**同じキーを引く読み手のうち片方だけが旧形式を知らない**形である。
  実害は2つある。休憩タグに派遣・その他を選べないことと、既に付いたタグが生のID `dispatch` のまま表示されることである。
  勤務時間の計算は変わらないので🟢とした。修正では、名前が空の**組み込み属性にだけ**既定名を補う。
  名前の無いカスタム属性は、ID を見せないという `83107c8` の意図どおり従来と同じく出さない。

  **実ブラウザ検証**（`CandTab` だけをマウント・375px・Firebase へは1バイトも出していない）は5項目すべてパスした。
  既存タグは「派遣」と表示される。選択肢は 社員・バイト・派遣・その他 の4つが並ぶ。「その他」を押すと `tags:["dispatch","other"]` で保存される。
  `pageerror`・`console.error` はどちらも0件だった。**修正前の配信物では、既存タグが `dispatch` と表示され、選択肢は社員・バイトの2つだけになる。**
  ユニットテストを1件追加し（260件）、修正前の実装で落ちることを確認した。

  **本番に 6/16〜6/28 作成の店舗があるかは確認していない**（本番データには触れていない）。

### 要確認（未修正）

- **🟢 配信版数が直近コミットを含んでいない**。6箇所とも `20260911-49f1d26` で一致しているが、今回 app-utils.js を変えた。
  `/release-to-main` の手順63がバンプするので申し送りのみとする。
- **🟢 曜日キー7の意味変更の余波**。キー7は初版から「祝日（全部）」を意味したが、`c34ed24`（7/11）で「連休中・単日」になり、キー8（最終日）が増えた。
  スタッフ画面の候補解決（`gc`）は 8→7 のフォールバックを持つので正しい。一方 `matchingPositionDayTypes` はキー7を `holSat` にしか対応させない。
  そのため旧店舗で日付別候補がキー7の一覧と完全一致すると、その日が最終日でも区分が `holSat` と推論される。
  日付別候補が手で置かれ、しかも一覧が一致したときだけ起きる推論の端なので、記録のみとする。
- **🟢 `keepStaff` の旧形式（文字列配列・2026-08-24 の同日内だけ）**。`mergeKeepStaff` は文字列を扱えることを確認した。
  `_renameKeepStaff` と app-admin.js:2626 の読み手は**今回は中身を見ていない**（未確認）。
- **#120 から継続（変化なし）**: `PLAN_LIMITS` の既定値10／SubsTab が現在の `staffTypeLimits` で判定する件／
  休憩タグ・`staffWorkplaces` の残骸／`oneSidedFillBounds` の式の二重化／属性変更の既定の件／略称バリデーションのコメント／
  「残す」ポップアップの括弧書き／プラン上限モーダルの「店舗」文言。
- **変化なし（BACKLOG化済み・条件A/Bでループの権限外）**: 二重課金の根治／特商法表記（🔴 2件）／解約通知／解約時のプラン判定／
  企業連携の解除と `verifyShopOwner` の移行猶予／別名提出の重複の根（#81）／PDF の実物確認／
  `purgeOldPeriods` の本有効化（**今日 2026-09-11 から着手可**）／非表示スタッフを「未提出」に数えるか（#113）／
...
- `npm test` は **260件パス**（+1件）。
- `npx eslint app-*.js` は **0 errors・94 warnings**。
- `node --check functions/index.js` は OK（functions は未変更）。
- **RULES.md のスキャン項目はすべてクリア**。
  - `DEV_MODE` は式のまま（app-core.js:12）。
  - `subs` の全体 `set()` は0件。`filter(s=>s.id!==…)` はすべて `deletedId` 付きか店舗一覧の操作。
  - `accounts` の全件読みは0件。
...
| 形式変更（コミット） | 読み手 | 判定 |
|---|---|---|
| 祝日 `hol` を `holSat`/`holSun` に分割（`1cdcd6b`） | `getBreakList`・`requiredPositionsFor` | ✅（#120 で修正済み） |
| 曜日キー7の意味変更・キー8の追加（`c34ed24`） | スタッフ画面の `gc`／`matchingPositionDayTypes` | ✅／🟢（上記） |
| `staffTypeLimits` の名前なしから名前ありへ（`8501732`）・組み込みの縮小（`83107c8`） | 上限判定6箇所は `typeof==="object"` で守られている。表示3箇所は既定名で補う。**`getAttrOptions` だけ補わない** | **❌ 本件（修正済み）** |
| `overtimeSettings.byStaff` の数値から `{lunch,dinner}` へ（`ba1d08a`） | `getOT`・SetTab の入力欄 | ✅ 両方とも数値を扱う |
| `staffHidden` の `true` から範囲へ（`3787403`） | `staffHiddenRanges` が唯一の読み手 | ✅ |
| `keepStaff` の文字列から `{name,index}` へ（`2d33e1e`） | `mergeKeepStaff` | ✅（他2箇所は未確認） |
| `staffPositions` | 初版（`7c087cb`）から `{lunch,dinner}` | 形式変更なし |
| sub の `status`・`periodUnit` | 値の履歴に変更なし | 形式変更なし |

なお #120 の申し送りにあった「`staffTypeLimits` の文字列からオブジェクトへの変更」は実在しなかった。初版から値はオブジェクトで、変わったのは**名前の有無**と**組み込みIDの集合**である。

**Firebase・Stripe には読み書きとも一切アクセスしていない。**
**未検証**: iOS 実機、本番に名前なしの組み込み属性を持つ店舗があるか、`keepStaff` の他2箇所の読み手。

### 測り方の記録

**効いたのは、組み込みIDの一覧（`BUILTIN_TYPES`）と既定値（`makeSettings`）の食い違いを見たこと。**
組み込みは4つなのに既定は2つしかない。この差が「過去には4つとも既定で保存されていた」ことを示しており、
`git log -S 'dispatch:{name'` で名前が付いた日と外れた日が1日で引けた。
そこから、名前なしで保存された12日間の形を読み手ごとに当てた。

**実ブラウザでは、測り方の誤りを1件踏んだ。** `h.clickExact("その他")` は行を絞らないと
上の「休憩を追加」フォームにある同名ボタンに当たる。そのため「押したのに保存されない」と出た。
比較用に「バイト」を押しても保存されなかったので、アプリではなく測り方と分かった。
スキル1.6節の罠6（部分一致・最初の1つを押す）の、**完全一致でも同名ボタンが複数ある**という変形である。
対策は同じで、押す行を絞ってから押す。

**申し送り（次回の観点）**: **今度は「既定値（`makeSettings`）の履歴」を当たる。** 今回の穴は、
既定値が変わると**その期間に作られた店舗だけが別の形のデータを持つ**ことから生まれた。
`git log -p -- app-core.js` で `makeSettings` の変遷を引き、既定値から消えたキー・値の形が変わったキーを列挙する。
そのうえで、**その形を持つ古い店舗を今の読み手が扱えるか**を確かめる。候補は `password:DEFAULT_PW`（廃止済み）・
`breakTimes.hol`・`templates` などである。

### 総括

**🔴・🟡はゼロ。🟢を1件検出し、その場で修正・実ブラウザ検証・コミットまで完了した。BACKLOG への起票は無い。**
#120 の申し送りどおり形式変更を8件棚卸しした。見つかったのは、**2026-06-16〜06-28 に作られた店舗だけが名前なしの
「派遣」「その他」を持ち、休憩タグの選択肢からだけ落ちていた**という穴である。他の読み手はすべて既定名で補っており、
1つの関数だけが補完を2つの属性にしか当てていなかった。
