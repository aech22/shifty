...
## Shifty バグチェックレポート（2026-08-31 自動実行 #101）

> 着手時の HEAD は `5974eb9`（リリース直後）。#100 以降の実コード変更は **`685c219` の1件だけ**なので、
> 静的スキャンだけでは1周が空になる回だった。#100 の「次回の手」③——**まだ実ブラウザで通していない
> 企業連携タブとマイページ**——を先に消化したところ、そこで2件見つかった。

### 修正済み

- **🟡 マイページの「使用状況」が空白列（スペーサー）をスタッフとして数えていた**（[app-admin.js:4580](app-admin.js)）→ `2d7dc94`

  `staffList.length` を生で使っていたため、キッチン/ホールの区切りである空白列まで1名として数えていた。
  上限判定（`StaffTab`:2442）と Free の残数バナーは `filter(n=>!isSpacer(n))` で数えるので、
  **同じ店舗を見ている2つの画面が違う数を出していた**。上限判定と同じ式に揃えた。
  Free での実害: 実スタッフ18名＋空白列2なら、スタッフタブ「18/20名」に対しマイページは
  **「20 / 20」を赤（上限到達色）** で出す（実際はまだ2名追加できる）。

- **🟢 マイページの「期間数」の単位が「名」だった**（同コミット）。Pro・Premium で「期間数 **2名** / 無制限」と出ていた。`unit` prop を追加。

- **🟢 スタッフ削除ポップアップのコメント2箇所が、`685c219` で廃止した「累積」の読み方のまま残っていた**
  （[app-admin.js:2361](app-admin.js)・[:2660](app-admin.js)）→ `ed70a29`。#100 で CLAUDE.md の古い記述に足をすくわれかけたのと同じ形。

**実測（A/B・実ブラウザ・配信物をディスクから返して起動・`firebaseDB.ref` を差し替えて書き込みを全て横取り＝dev には1バイトも出していない）**
標準テスト店舗＝実スタッフ4名＋空白列1・期間2件:

| | 修正前 | 修正後 |
|---|---|---|
| `?plan=premium` | スタッフ数 **5名** / 無制限　期間数 **2名** / 無制限 | スタッフ数 **4名** / 無制限　期間数 **2件** / 無制限 |
| `?plan=free` | スタッフ数 **5 / 20** | スタッフ数 **4 / 20** |

同じ画面のスタッフタブは修正前から「4/20名」で、**そちらが正しい側だった**。pageerror・console.error は修正前後・両プランとも0件。

### 要確認（未修正）

- **🟡 削除ポップアップの「既定の選択」が、これから配る期間に名前を残す**（[app-admin.js:2522](app-admin.js)）→ **BACKLOG化済み**

  `685c219` は「**選んだときの解釈**」を時系列へ直したが、「**何も選ばずに削除したときの既定**」は
  `setDelKeepCount(1)` ＝ 先頭の選択肢＝**いちばん新しい期間「まで残す」**のまま。時系列の解釈では
  「その期間とそれより古い期間すべて」なので、**最新3期間すべてに `keepStaff` が入る**。

  **実測**（375px・実ブラウザ・書き込みは全て横取り。「田中」の削除ボタンを押しただけ）: ラジオは3つで、
  **開いた時点の選択は index 0＝「2026年8月前半 まで残す」＝いちばん新しい期間**。このまま確定すると
  これから配る期間のシフト表に列が残る——**本番で報告された症状と見え方は同じ**。

  **ループで直さなかった理由（条件B）**: `685c219` 以前も既定は「最新1期間に残す」で**新しく壊れたわけではない**。
  既定を「残さない」に倒すか否かは、機能の当初の動機（配り終えたシフト表から名前が黙って消えない）と正面からぶつかるため**ユーザー判断**。

- 変化なし・**BACKLOG化済み**: #68 の「編集から削除への昇格」（`submitterUid` の上書き）／片側セルのヒートマップ／
  スタッフ削除の後始末／別名提出の重複（#81）／二重課金・特商法（🔴）／解約時のプラン判定／企業連携の解除／解約がアプリに伝わらない。

### `685c219` の検証 — 異常なし

`retainedPeriodIds` は `choices.slice(keepCount-1)`＝**選んだ期間とそれより古い側**を返し、0・非数・範囲外は空配列。
`retained.upto` の max→min 変更は「再オープン時に同じ選択肢を復元する」用途と整合する（choices は新しい順なので
min index ＝ 残っている中でいちばん新しい期間 ＝ 選んだ位置）。ポップアップ文言も実測で確認:
「2026年7月後半 まで残す ／ **2026年8月前半 からは消えます**」。

### 検証したこと

- **管理者8タブすべてを実ブラウザで巡回**（1400×950・`?plan=premium`・オーナー復帰済み）。全タブで
  **pageerror・console.error ともに0件**、`fontSize<16px` の input **0件**。**#100 の宿題③はこれで閉じた。**
- RULES.md: `DEV_MODE` は式のまま／`subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／読み込み順維持／SRI **11本**／input 追加 **0件**。
- 配信版数 `20260829-685c219`（6箇所一致・旧版数の残り0）。**今回の修正は版数より後**なので次のリリースでバンプが要る（`/release-to-main` 手順63）。
- プラン境界: Premium機能を `isPro` で守っている箇所 **0件**。`pointerEvents:"none"` のゲートはテンプレート2箇所のみで、
  **#98 のハンドラ側判定（`proOnlyTemplate`）が3経路すべてに残存**。
- CF: `secrets:` **6箇所**／`.delete()` 誤用 **0件**／`node --check` OK（functions/ に差分なし）。
- `private/adminKey` の上書き懸念を確認 → ルールが `!data.exists() || owner` なので**claim済み店舗の鍵は他人に書き換えられない**（誤検知だった）。
- `npm test` **199件パス**／`npx eslint app-*.js` **0 errors 95 warnings**（増減なし）。
- **devのデータは1バイトも変えていない**。
- **未検証**: Excel・PDF の実物確認（**9回連続**・ユーザー操作が要る）／iOS Safari 実機／本番データ／企業連携タブのログイン後の中身。

### 総括

**直したのは「選んだとき」で、既定は据え置かれていた。** 修正が届いた範囲を、症状の範囲と取り違えないこと。

**今回の2件は、コードを読んでいる限り出てこなかった。** `staffList.length` は単体では何も間違って見えない。
**同じ数を2箇所が別の式で数えているとき、机上では両方とも正しく見える。** 実ブラウザで「5名」と「4」を並べて初めて分かった。

**次回の手**: ①Excel・PDF の実物確認（9回連続・ユーザー操作が要る）②#101 の既定の判断（案A〜C）が出たら `del()` の1行と
一緒にスタッフ削除まわりを締める ③企業連携タブの**ログイン後**の中身はまだ通していない（dev への書き込みが要る）。
...
>
> **⚠️ 2026-08-27（バグチェック#99）— 案Aの射程はスタッフ画面にも及ぶ（新しい入口）**
> 同じ「認証済みなら誰でも subs を編集できる」ルールに、**閲覧専用の管理者端末とは別の入口**がある。
> スタッフ画面の提出状況一覧（`SmModal`）は、**他人の提出も編集できる**:
> - セルをタップして開く編集パネル（`handleCellClick`・app-staff.js:538）に `myName` の絞り込みが無い
> - 名前の「✎ 修正」（`onEditByName`・app-staff.js:567）も同様で、**他人の提出を自分の入力欄に読み込んで上書き提出できる**
>
> 削除だけは `myName` で自分の行に限っており（`2c1c9c7` で「名前未入力のときに絞り込みが外れる」欠陥を修正済み）、
> ルール側でも「提出した端末かオーナー」に限られる。
> **案Aを選ぶ（＝ルールで編集を締める）なら、スタッフ画面の「✎ 修正」も同時に扱う必要がある**——
> こちらは全行に出す明確な意図のあるUIなので、「他人の提出を直せる」ことを仕様として残すか否かの判断が別に要る。
>
> **🔺 2026-08-28（バグチェック#100）訂正＋新事実 — 「削除は二重に守られている」は誤りだった。編集から削除へ昇格できる**
> 上の #99 の記述にある「**削除は二重に守られていて、編集だけが素通し**」は**実測で否定された**。
> 実際には**編集できる者は、2タップで削除もできるようになる**。
>
> `submitterUid`（app-staff.js:206）は**提出のたびに無条件で現在の端末の匿名uidへ書き換わる**。
> `existSub` は名前＋期間で既存subを探して**再利用**するので、**他人の名前で提出すると新規subは作られず、
> その人のsubの `submitterUid` が自分のuidになる**。ルールは削除を「`submitterUid` の持ち主かオーナー」に
> 限っているので、**削除権ごと移る**。
>
> **実測**（375px・実ブラウザ・`firebaseDB.ref` をフックして書き込みを全て横取り＝devには1バイトも出していない）:
>
> | 手順 | 実測値 |
> |---|---|
> | ①名前未入力で提出状況一覧を開く | 削除ボタン **0個**（`2c1c9c7` の修正どおり） |
> | ②他人の行の「✎ 修正」を押す | 一覧が閉じ、その人の提出が自分の入力欄に入る（名前も自動で「田中」になる） |
> | ③そのまま提出する | `{subId}/submitterUid` = **自分のuid** を書き込む（既存subを再利用） |
> | ④一覧を開き直す | 削除ボタン **1個** |
> | ⑤削除を押す | `remove shops/{sid}/subs/{subId}` を発行 |
>
> **「✎ 修正」は必須ではない。最小再現は「他人の名前を打ち込んで提出する」だけ**で、③④が同じ結果になることを
> 別スクリプトで確認した。**スタッフ名は提出状況一覧に「未提出（3名）／佐藤／鈴木／高橋」と表示されている**ので、
> 名前を知らない訪問者でも読める。
>
> **ルール側も止めない**: `submitterUid` の `.validate` は
> `newData.val() === auth.uid || owners…` ＝ **自分のuidを書くことを明示的に許可**している。
> つまり乗っ取りの書き込みは正規の書き込みとして通り、通った結果として `.write` の削除条件を満たす。
> 標準テスト店舗の田中のsubは元々 `submitterUid` が **null**（＝オーナーしか消せない）だったものが、
> 2タップで「訪問者だけが消せる」状態に変わる。**`8952f8f` の「提出者とオーナーの2者のみ削除可」と
> `2c1c9c7` のUI修正は、この経路では両方とも成立しない。**
>
> **これは案A〜Cの選択に加えて、もう1つ決めることが要るという意味**: 案B（文言を実態に合わせる）や
> 案C（現状維持）を選んでも、**この昇格経路は残る**。ルールで編集を締める案Aを選べば副作用として塞がるが、
> 案A自体は「閲覧専用の管理者端末」の話であってスタッフ提出は対象外なので、**明示的に決める必要がある**。

**決めること（#100 追加）: 提出のたびに `submitterUid` を上書きしてよいか**

正当な「同一人物が機種変更して出し直す」と、なりすましは**区別できない**。スタッフ側に本人性の概念を
置かないのが設計原則1（登録を求めない）で、上書き自体もコード内に「別端末から出し直した場合は
最後に提出した端末が提出者になる」と明記された**意図的な仕様**。したがって選択が要る。

- **案ア: 最初の提出者を固定する**（`submitterUid` が既にあるときは上書きしない）。乗っ取りは止まるが、
  **機種変更した本人は自分の提出を削除できなくなる**（オーナーに頼む）
- **案イ: 提出の削除をオーナー限定に戻す**（`submitterUid` による削除を廃止）。`8952f8f` の判断の巻き戻しになる
- **案ウ: 現状維持**（スタッフURLを持つ者どうしは互いの提出を消せる、と割り切る）

**受け入れ条件**:
- [ ] 案A〜Cのいずれかを選ぶ（**ユーザー判断**）
- [ ] 案Aを選ぶ場合、スタッフ画面の提出状況一覧（セル編集・「✎ 修正」）を他人の提出に対して許すかも同時に決める
- [ ] **`submitterUid` の上書きについて案ア〜ウのいずれかを選ぶ（#100・ユーザー判断）**
- [ ] 決めた案を実装し、閲覧専用端末で「できること／できないこと」が画面の説明と一致することを確認する
- [ ] **実装後、#100 の①〜⑤の手順を再実行して④の削除ボタンが 0 個になることを実測する**（手順は
      `shifty-e2e-verify` 1.5節＋1.7節。修正前は 1 個になるので、素通りしないテストであることも同時に示せる）

**影響範囲**: app-admin.js（`ownerReadOnly` の分岐・バナー文言）、**app-staff.js（`submitterUid` の付与:206・`onEditByName`:567・`handleCellClick`:538）**、database.rules.json（案Aで書き込みも締めるなら／案アなら `submitterUid` の validate に「既存値があれば変更不可」を足す）
...
案Aを妨げる理由は現存しない。**#100 で「削除は二重に守られている」という #99 の記述も否定された**（上記）。
...
> **⚠️ 2026-08-31（バグチェック#101）追記 — 既定の選択が「最新の期間まで残す」になっている（仕様判断が要る）**
> `685c219` は「8月後半 まで残す」を選んだときに 9月前半にも名前が残る取り違えを直したが、
> **何も選ばずに削除したときの既定** は `del()`（[app-admin.js:2522](app-admin.js) `setDelKeepCount(1)`）が
> 1＝先頭の選択肢＝**いちばん新しい期間「まで残す」**を選んだ状態のままである。時系列の解釈では
> 「その期間とそれより古い期間すべてに残す」＝**最新3期間すべてに `keepStaff` が入る**。
>
> **実測**（375px・実ブラウザ・書き込みは全て横取り＝dev には出していない。標準テスト店舗で「田中」の削除を押しただけ）:
> ポップアップのラジオは3つ（2期間＋「どの期間にも残さない」）で、**開いた時点で選択されているのは index 0＝
> 「2026年8月前半 まで残す」**（＝いちばん新しい期間）。この状態で「削除する」を押すと、**これから配る期間の
> シフト表にその人の列が残る**。本番で報告された症状（退職者が新しい期間に出続ける）と、見え方は同じになる。
>
> `685c219` 以前も既定は「最新1期間に残す」だったので**新しく壊れたわけではない**（増えたのは古い側2期間だけ）。
> ただし「削除を押した人が既定のまま確定したとき、名前は残るべきか消えるべきか」は**プロダクトの判断**であり、
> ループでは決められない（**条件B**）。
> - **案A: 既定を「どの期間にも残さない」（`setDelKeepCount(0)`）にする** — 削除は削除、残すのは明示操作にする。
>   ただし「配り終えた過去のシフト表から名前が黙って消える」ことを防ぐという機能の当初の動機とは逆向きになる
> - **案B: 既定を「いちばん新しい “終了済み” の期間まで残す」にする** — 配布済みには残し、これから配る期間からは消える。
>   削除の実際の動機（その期間までしか入らない）に最も近いが、終了済み期間が無いときの既定を別に決める必要がある
> - **案C: 現状維持**（既定は最新まで残す。ラベルに期間名が出ているので読めば分かる）
...
- [ ] 3: **削除ポップアップの既定の選択について案A〜Cのいずれかを選ぶ（#101・ユーザー判断）**。決めたら `del()`（app-admin.js:2522）の `setDelKeepCount(1)` を変更し、375px の実ブラウザで「削除を押しただけの状態」で最新期間に `keepStaff` が入らない（案A・案B）ことを実測する
