...
## Shifty バグチェックレポート（2026-09-25 自動実行 #146）

> 着手時の HEAD は `04aa03f`。**#143 の HEAD（`9d2579b`）以降、`app-*.js`・`functions/`・
> `index.html`・`database.rules.json` は1バイトも変わっていない**（4回連続でコードが同一）。
> 基準値も #145 と完全に同一だったので、**#145 の申し送り②**——「配信物側の死んだ guard・
> `.catch(()=>{})` を当てる」——に全部を充てた。
>
> 結果は**二段になった**。死んだ guard そのものは見つからなかったが、**`.catch` を
> 「良性の既定値に丸めている」形**で3件、別経路で2件が出た。**「条件が成立しえない分岐」を
> 探すより「失敗したときに何に丸めているか」を見るほうが当たった。**

### 修正済み

- **[🟢] 片側セル補完の境界が同じ式の2つの写しで計算され、片方が `HEAT_BAND_SPLIT_MIN` を直書きの `1020` で持っている**
  （app-utils.js:156 `oneSidedFillBounds` ／ app-admin.js:862-870）→ `362c037`

  勤務時間・期間集計・週/月上限判定は `oneSidedFillBounds`（帯境界＝`HEAT_BAND_SPLIT_MIN`）を通るが、
  ヒートマップは同じ式の写しを持ち**帯境界を数値リテラル `1020` で直書きしている**。候補の集合と
  フォールバック（900 / 1020）は同一で、**現状の値は一致＝ユーザーに見える食い違いは無い**（10ケース実測）。
  問題は**帯境界を変えたときに集計側だけが追随する**こと——ヒートマップは17:00固定のまま残り、
  「ヒートマップと勤務時間集計が同じ日を別扱いする」＝**#82 で一度直した食い違いが再発する**。
  写しを消すのは配信物のリファクタなので、**#144・#145 と同じくドリフトの検出に留めた**
  （AST で式を切り出して実行し両者を照合／式中の数値が `HEAT_BAND_SPLIT_MIN` か 900 以外でないことを確認）。

  **対照で裏を取った**（写しを改変し `SHIFTY_ADMIN_SRC` で向ける。配信物は無変更）。無改変=282件パス、
  注入した7形はすべて検出: 帯境界 1020→1080 ／ `<=`→`<` ／ `>=`→`>` ／ 日付別候補を落とす ／
  全体候補を落とす ／ 既定値 900→930 ／ `max`↔`min`。

  **自分のテストの穴を対照が1つ暴いた**（記録のため）。最初の版は `<=`→`<` を**見逃した**。
  ケース set に**帯境界ちょうど（退勤17:00・出勤17:00）の候補が1件も無かった**ため——
  境界に掛からない候補しか無いと両方の写しが同じフォールバックへ落ちて差が消える。
  **境界の semantics を pin するには境界上の入力が要る。**

### 要確認（未修正）

**新規の 🔴 は無し。🟡 は3件で、いずれも条件B（仕様判断）に該当するため BACKLOG 化した。**

- **[🟡] 企業ログインのセッションで「コードで追加」した店舗が、リロードすると一覧から消える** → **BACKLOG化済み**
  書き込み先は `accounts/${authUser.uid}/shops/${code}`（app-admin.js:70）だが、企業セッション
  （`uid="company_"+companyId`）の Phase1 は `companies/{companyId}/pub/shops` を読んで **`return` する**
  （app-main.js:270-290）ので `accounts/{uid}/shops` に**到達しない**。Cookie フォールバックも
  `if(authUser){...return;}` の内側で届かない。**同じ操作のUIが2つあり**（ドロップダウン＝消える側／
  企業連携タブ＝`linkStoreToCompany` で永続する側）、設定タブは**消える側を案内している**（:4562）。
  **根は 2026-09-16 に直したものと同じ**——「連携の実体は2箇所にある。解除は必ず両方を消す」の
  **追加側が残っていた**。app-admin.js:70 の `fbSet` には `.catch` も無く、成功トーストが書き込み結果に
  関係なく出る。
- **[🟡] 読みの失敗を「問題なし」に丸めている3箇所** → **BACKLOG化済み**
  店舗間シフト重複の判定（app-admin.js:436-438）は `.catch(()=>null)` の `null` を「その店舗にデータが無い」と
  同じに扱い、`dupErrors` が**重複0件**を返す（管理者は「二重予約は無い」と読んでシフトを配る）。
  店舗略称の重複チェック（:4209）は `.catch(()=>[s.id,[]])` で「略称を持たない」ことになり
  **同じ略称を2店舗に登録できる**。`claimOwnership`（app-main.js:605-628）は読み失敗を「キー無し」に丸めて
  新しい `private/adminKey` を**書く**ため、**他のオーナー端末の既存キーが無効になる**。
  いずれもネットワーク起因（読むパスは `.read: auth != null`）で、**起きたときに気づく手がかりが無い**。
- **[🟡] `cancelPlanChange` が、取り消しに失敗しても「取り消しました」と表示する** → **既存BACKLOGタスクへ追記済み**
  `subscriptionSchedules.release` の catch が**どんな例外も「既に released/completed」とみなし**
  （functions/index.js:358-370）、:373 が `scheduledPlan` を無条件に消して `200 {ok:true, released:false}` を返す。
  **`released:false` は返っているのにクライアントが読んでいない**（app-admin.js:5160-5161 は `r.ok` だけを見る）。
  ユーザーは成功表示を見るが**Stripe 側のスケジュールは生きたまま期間終了時に降格が実行され**、
  ボタンは `scheduledPlan` があるときだけ描画されるので**やり直せず**、`subscription_schedule.*` は
  購読イベントに無いので**自然に直らない**。無条件に消すのは 2026-09-05 の意図的な決定だが、
  **その決定が想定していたのは「既に消えていた」ケースだけ**。
- **#145 以前から継続（変化なし）**: BACKLOG 化済みの既存項目はすべて変化なし（コードが同一のため）。
- **[🟢] #143 の「1時間ぶんの列幅の式が2本ある」**（app-admin.js:1248 の `22` と :1211 の
  `HEAT_NAT_HOURW=24`）と **`fvEdge=false` の死んだ分岐**（:1324）は変化なし。
  **今回の🟢は同じ「式が2本ある」型の3例目**で、この形は定常的な負債になっている。
- **[🟢] 外しても挙動が変わらない恒真 guard が5件**（上の網羅で判明・**直していない**）:
  `app-utils.js:191` の `if(we>ws)`（直前の `effShiftRangeMin` が `endMin>startMin` を保証。
  時刻の全組み合わせ 94,896 ケースで偽になったのは0件）／`:669`・`:679` の `!!fixedKey&&`
  （`CELL_COMMANDS` 由来で必ず `"締"`）／`:157`・`:164` の `WeakMap` チェック／`:664` の `!m[1]`
  （正規表現が1文字以上を捕まえるため到達しない）／`:156` の `if(!settings)`
  （配信物からの呼び出しは `:175` の `settings&&` の内側だけ＝**app-admin.js は
  `oneSidedFillBounds` を呼んでいない**ことの裏づけでもある）。どれも防御として無害。
- **[🟢] `app-staff.js:18` の `urlLocked` prop は宣言されて一度も読まれていない**（`authChecked` と同じ型）。
- **引き受け済みのトレードオフ（再検出しても直さない）**: `subs/$subId/.write` は認証済みなら通る（2026-08-31 決定1）。

### 当てて否定した仮説

- **#145 申し送り②の本題「条件が成立しえない死んだ guard が配信物に無いか」→ 無い（網羅で否定）**。
  `app-utils.js` の判定点（`if` / 三項 / `&&` / `||`）**494箇所を計装**し、実テストスイートと
  全エクスポートを網羅するコーパスで走らせた: **到達しない判定点は0件**（片側だけ回る54件は
  「証明できた恒真」5件を除きコーパスの網羅不足）。あわせて定数リテラルの判定・外側と字句的に同じ判定・
  直前の `if(!X) return` で反転済みの `if(X)` を AST で走査し、**app-utils.js / app-core.js /
  app-staff.js の3ファイルとも0件**。**この観点は閉じたと考えてよい。**

- **`periods` の差分書き込みが Firebase の往復で収束しないのではないか → 否定**。#142 が踏んだ形を
  `set()` のセマンティクス（空コンテナはノードごと消える・配列は穴があると数値キーのオブジェクトで返る）を
  再現して6ケース測り、**全ケース収束**（写しつき・写しを毎回作り直す・keepStaff を空へ・疎な配列）。
- **スタッフ名キーの設定マップの8件目（`overtimeSettings.byStaff`）が3つの surface のどれかで
  取り残されていないか → 否定**。`renameStaffInSettings`・`settingsWithoutStaff`・`_hasStaffKey` の
  **3つすべてが明示的に処理している**（`STAFF_KEYED_SETTING_MAPS` は7件なので8件目は各所で個別に書く必要がある）。
- **凍結（写し）がキーを取りこぼしていないか → 否定**。`PERIOD_SNAPSHOT_SETTING_KEYS` の
  **13キーすべてが写しに焼かれ、終了済み期間で13/13が凍結され現在値の漏れは0**。免除キー
  （`staffHidden`）は写しに入らず現在値のまま解決される。
- **`visibleStaffList` の `period` 渡し忘れ／`isUnregisteredSubName` にフィルタ済み名簿を渡す呼び出しが
  無いか → 否定**。`expXl` の2つの呼び出し元もどちらも生の名簿を渡すので「非表示の人の提出が未登録名に
  化けて末尾に列として復活する」は起きない。部門別PDFの `realStaff`・`hallStaff` も
  **フィルタ済みの `staffList` から導かれている**ので「全体PDFでは消えるのに部門別PDFでは出る」も起きない。
- **app-admin.js の写しが持つ型ガードの非対称は到達するか → 到達しない**。`oneSidedFillBounds` は
  `typeof t!=="string"` を持つが写しは持たず、候補の `start`/`end` が数値だと `t.split is not a function` で
  **タブが描画不能になる**（実測）。ただし候補を書く3経路（`addG`/`addW`/`addD`）はすべて `<select>` の
  文字列値なので**UIからは数値が入らない**。手で壊したデータでしか起きないため🟢に留めた。

### 検証したこと

- `npm test` **282件パス**（#145 の280件 +2・追加は `tests/` のみ）・`npx eslint app-*.js`
  **0 errors / 95 warnings**（同数）・`node --check functions/index.js` 通過。
- RULES.md スキャン（**#145 申し送り①の修正版 grep を使った**）: `DEV_MODE` は式のまま（app-core.js:12）／
  `subs`・`periods` の全体 `set()` 0件／`accounts` 全件読み 0件／SRI 11件／**読み込み順
  utils→core→staff→admin→main を実際に出力して確認**／`global/shops` の全件読み 0件（ヒットは全てコメント）／
  `database.rules.json` の `".read"` に `true` 0件／`functions/index.js` に `.delete()` 0件。
- **配信版数は追随済み**（`20260923-bb394a3`）。**今回の変更は `tests/`・`BACKLOG.md`・`.cursorrules` だけで、
  配信物は1バイトも変えていない**（git で確認）。
- **実ブラウザの回帰スクリプトは今回も回していない**（#143 以降配信物が無変更のため。＝未実施）。
- **Firebase・Stripe・本番データには一切アクセスしていない。** 実測はすべて配信物の関数を Node で実行
  （`app-utils.js` は require、app-admin.js の式は AST で切り出して実行）。

**申し送り（次回の観点）**

1. **#145 申し送り①（スキル自身の走査が壊れている）は、実はもっと重い。** スケジュールタスクのファイル
   （`~/.claude/scheduled-tasks/shifty-bug-check/SKILL.md`）の PHASE 2〜4 の grep は**9箇所すべてが
   `app.js` を対象にしている**が、`app.js` は**2026-07-06 の5分割（`f02cc80`）で削除されており存在しない**。
   実行すると警告と0件が返るだけで**「正常」と見分けがつかない**＝Firebase書き込みパターン・プラン制限・
   RULES違反の機械チェックは**このループでは1つも効いていない**（毎回 `app-*.js` に読み替えて手で当てている）。
   スキルファイル側なのでこのループでは直していない。
2. **「死んだ guard」は網羅して無いと確認したので、この観点は閉じてよい**（上記）。当たったのは
   隣の形——**`.catch` が失敗を良性の既定値に丸めている**（`null`→「データ無し」・`[]`→「略称無し」・
   読み失敗→「キー無し」）——で3件出た。次回は**残りの `.catch` と `||` のフォールバック**を
   同じ観点で当てるとよい。判定は「この既定値は『無い』を意味するのか『分からない』を意味するのか」。
   なお `attrIdExists`（app-utils.js:1080）の `BUILTIN_TYPES` 早期 return は、既定の
   `makeSettings` が `staffTypeLimits` に employee/parttime しか作らないのに `dispatch`/`other` にも
   true を返す（＝上限判定が走らないまま当たる）。**到達する入力は示せなかった**——属性ピッカーが
   派遣を提示するかは app-admin.js 側で決まるため。次回そこから追うとよい。
3. **「同じ式が2本ある」型が3例目**。次回は `app-admin.js` が `app-utils.js` の**エクスポート済み関数を
   使わずに同じ計算を書いている箇所**を網羅的に洗うとよい（`oneSidedFillBounds` は export されているのに
   写しがある、という形が既に1件ある）。
