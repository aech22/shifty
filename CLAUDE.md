...
## Shifty バグチェックレポート（2026-09-02 自動実行 #104）

> 着手時の HEAD は `0e23ecd`。**#103 以降に入った実コードの変更はゼロ**（#103 自身の `bc7bf2e` と
> ドキュメント2件だけ）。同じ配信物にもう一度同じスキャンを当てても新しいものは出ないので、
> 機械スキャンは「基準線が動いていないこと」の確認だけに使い、**時間はコード読解へ回した**。
> 見つけたのは #102・#103 と同じ形の負債——**同じ規則を2箇所に手で書いたら、片方だけが規則を守っていなかった**。

### 修正済み

- **🟡 期間に名前を残して削除した人が、スタッフ一覧とシフト表で別の位置に出る**（[app-admin.js:2577](app-admin.js)）→ `abcc668`

  スタッフ一覧の行の並び（`displayRows`）が、削除済み・表示中の人を **`index` の昇順**で
  `splice` していた。ところが各 `index` は「**その人を削除した瞬間の一覧**」での位置であって、
  元の一覧での位置ではない。昇順に入れると座標系がズレる。

  **同じデータを描くもう一方（`mergeKeepStaff`・app-utils.js:832）は、まさにこの理由で
  「keepStaff の後ろから入れる」と決めていて、コメントに反例まで書いてあった**——
  「index昇順だと C が空白列を跨いでキッチン側へ移る」。その制約が `displayRows` 側に
  写っていなかった。グリッド・ヒートマップ・Excel・PDF はすべて `resolvePeriodMaster` →
  `mergeKeepStaff` を通るので、**食い違っていたのはスタッフ一覧ただ1箇所**。

  **実測（375px・実ブラウザ・`StaffTab` だけをマウント＝Firebase へは1バイトも出していない）**

  | 条件 | | スタッフ一覧の行 | シフト表の列 |
  |---|---|---|---|
  | `["A","B","_spacer","C"]` から B→C の順に削除 | 修正前 | `A,B,C,_spacer` | `A,B,_spacer,C` |
  | 同上 | 修正後 | `A,B,_spacer,C` | 同じ |
  | `["A","B","C"]`（空白列なし）から B→C の順に削除 | 修正前 | `A,C,B` | `A,B,C` |
  | 同上 | 修正後 | `A,B,C` | 同じ |

  **空白列（スペーサー）はシフト作成タブのキッチン/ホールの境界**（`spIdx`）なので、
  跨ぐと「所属セクションが違って見える」。空白列が無くても、残す人が2人以上いれば
  単純に前後が入れ替わる。

  直し方は**並び順の決定を `mergeKeepStaff` 自身に委ねる**形にした（同じ関数を通す以上、
  二度と乖離できない）。材料には**最も古い選択肢の `keepStaff`** を使う——
  `retainedPeriodIds` は `slice(k-1)` なのでどの選択位置でも最古の期間を必ず含み、
  そこに retained 全員が削除順のまま並んでいる。

  **非回帰**: 残す人が0人・1人のケースは修正前後で**1文字も変わらない**（実ブラウザで確認）。
  実スタッフ行に渡る `data-staff-idx` も `staffList` の index のままで、
  **ドラッグ・編集・削除の意味は変えていない**。pageerror・console.error ともに0件。

### 調べて「バグではない」と確定したもの

- **ドラッグ並べ替えが壊れる**という疑いを立てたが、**反証した**。`displayRows` は
  描画にしか使われず、並べ替えのヒットテストは `data-staff-idx`（＝`staffList` の index）で
  行われる。削除済み・表示中の行にはこの属性が付かないので、そもそも掴めない。
  したがって上の不具合は**表示だけ**で、保存されるデータには波及しない（実測で属性値も照合済み）。
- **スタッフ再提出で `source:"grid"` が Firebase に残り続ける**（＝管理者の下書きが
  スタッフの提出として数えられない）と疑ったが、**反証した**。`diffSubForFlatWrite`
  （app-utils.js:707-710）は**新subに無いキーを `null` で明示的に消す**ので、
  `source` は差分書き込みでも確実に落ちる。
- **`ckStaffKey` の Cookie 名に不正な文字が残る**と疑ったが、**問題なし**。`genSecureId` の
  文字集合のうち Cookie 名の区切り文字は `=` だけで、これは `cookieSafeKey` が落としている
  （`@` `?` は RFC 上の separator だが実ブラウザは受け付ける）。`getCookie` の正規表現エスケープも
  `! @ % & _ ~ -` を含めて取りこぼしがない。
- **凍結名簿の消費側がバラバラ**という疑いも**反証した**。`ShiftEditTab` は `staffList` を
  解決後の値で上書きしてから `realStaff`・`spIdx`・`hallStaff`・`buildPdfCols`・`expXl` に流し、
  期間管理タブの Excel ボタン（:2095）も同じ `resolvePeriodMaster` を通る。
  **独自に名簿を組み立てていたのは `StaffTab` だけだった。**

### 要確認（未修正・#103 から変化なし）

- **🟢 `database.rules.tightened.json` が現行ルールとバイト同一の残骸** → **BACKLOG化済み**。
  今回も `cmp` でバイト単位まで同一であることを再確認した。消すか残すかは**条件B（判断）**。
- **🟢 片側セルの日は「→15:15（+15分）」の実効退勤バッジが出ない**（app-admin.js:3533付近）。
- **🟢 期限切れの後始末が、より古い期間にしか残っていない人の設定を消しうる**（app-admin.js:2533付近）。
  2026-08-31 決定4・案Aの範囲設定そのままなので**仕様判断**（条件B）。
- **🟢 `purgeInactiveShops` が、もう存在しない `inviteCodes` を毎日掃除している**（functions/index.js:808）。
  招待コード方式は `8384467` で廃止されルールからも消えたが、CF 側の期限切れ削除だけ残っている。
  読み取り1回ぶんの無駄で実害はない。**今回新規に気づいた分で、BACKLOG化はしない**
  （次に `functions/` を触る回に一緒に落とせばよい）。
...
- **配信版数が app-*.js の変更に追いついていない**: `?v=`（index.html 5箇所）と `build:`（app-core.js）は
  `20260831-c340d5c` のままで、以後 `6613c78`・`bc7bf2e`・`abcc668` が入っている。
  `/release-to-main` の手順63が標準工程としてバンプするため申し送りのみ（#103 から継続）。
...
- **RULES.md のスキャン項目は全項目クリア**: `DEV_MODE` は式のまま／`subs` の `set()` 全体上書き **0件**／
  ...
  SRI **11本**／**フォーム部品の `fontSize<16` 0件**（56件走査）。
- **CSS変数**: 未定義参照 **0行**／ダーク定義の二重管理（`[data-theme]` と `prefers-color-scheme`）**差分なし**／
  SVG属性内 `var()` は既知の `ShiftyIcon` 1件のみ。
- **Firebaseルール**: `database.rules.json` と `tightened` が**バイト同一**。ルール全体をダンプして
  `archived` / `email_otps` / `companyCodes` に読み書きルールが無い（＝既定で拒否）ことを確認。
- **削除の永続化**: `saveSubs(` の全呼び出しを再確認。第2引数の無い `app-admin.js:208` は
...
- **devのデータは1バイトも変えていない**（実ブラウザ検証は `StaffTab` の単体マウントのみ。
  `firebaseDB` は `null` のまま。`onSave`・`savePeriods`・`onSaveSettings` は全てスタブ）。
- **修正前の配信物に対しては同じスクリプトが落ちる**ことを確認済み（`git show HEAD:app-admin.js` を
  別ディレクトリへ書き出し `SHIFTY_ROOT` 相当のルートとして渡した）＝**素通りするテストではない**。
- **未検証**: Excel・PDF の実物確認（**12回連続**・ユーザー操作が要る）／iOS 実機／本番データ／
  CF の本番反映後のログ／今回の修正の **iOS Safari（WebKit）での確認**
  （並び順はレイアウトではなく配列操作の結果なのでエンジン差は生じないと判断し Chromium のみで測った）。

### 測り方の記録

対照を取るのに `SHIFTY_ROOT` 環境変数ではなく **`process.argv[2]` でルートを渡した**。
`shifty-e2e-verify` 1.6節に「zsh で `env $V` が語分割されず全条件が同じ値を返す」という罠が
記録されており（`6935506`）、**同じ罠なら踏み方を変えるのではなく、踏みようのない渡し方に変える**
のが正しい。結果として修正前 `FAIL` / 修正後 `allPass` がはっきり分かれた。

### 総括

**規則をコメントに書いても、その規則を守る責任は書いた場所にしか宿らない。**
`mergeKeepStaff` は「index昇順で入れてはいけない、C が空白列を跨ぐから」という
**反例つきの警告**を7行も持っていた。それでも 70行下の `displayRows` は昇順で入れていた。
警告は**読まれなかったのではなく、そこに無かった**のだ。

**#102 は「回避策は、それが書かれた1箇所しか直さない」、#103 は「一括修正は、対象リストに
載ったファイルしか直さない」、今回は「規則は、それが書かれた関数の中しか縛らない」。**
3回続けて同じ構造——**知識が1箇所に閉じ込められ、同じ仕事をするもう1箇所がそれを知らない**。

だから今回は「昇順を降順に直す」ではなく、**並び順の決定を `mergeKeepStaff` に委ねた**。
規則をコメントで共有するのをやめて、関数の呼び出しで共有する。
**同じ関数を通すものは、乖離しようがない。**
