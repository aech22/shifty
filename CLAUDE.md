...
## Shifty バグチェックレポート（2026-09-07 自動実行 #115）

> 着手時の HEAD は `b1d019d`。観点は **#114 の申し送りをそのまま実行**した——
> 「**『〜する必要がない』『〜は済んでいる』『〜だけを見ればよい』と断言しているコメント**を
> 機械的に集め、その断言が今も成り立つかを1つずつ当たる」。
> 断言を集めたところ、**断言そのものより、断言が指し示す『場所』のほうが先に壊れていた。**
...
- **🟡 コメント内の行番号参照27件が、ほぼ全て別のコードを指していた**
  ...
  **シンボル名＋行番号**で書く。集めると27件あり、**実体と一致していたのは2件だけ**だった。

  実測（引用された番号 → 実際の定義行）:

  | シンボル | コメントの引用 | 実際 | ずれ | 引用先に実在したもの |
  |---|---|---|---|---|
  | `registerAlias` | :2969 | 3560 | **591行** | StaffTab の「編集」ボタン |
  | `gridStaff` | :1060 | 1133 | 73行 | `allCands.forEach(...)` |
  | `getWeekMin` | :1026 | 1096 | 70行 | `positionErrorEntries` |
  | `workShiftByStaffDate` | :490-500 | 545 | 55行 | `visibleStaffList` の呼び出し |
  | `getStoredTime` | :514 | 567 | 53行 | `requestAnimationFrame(...)` |
  | `subsByKey` | :485 | 535 | 50行 | 別のコメント |
  | `heatData` | :762 / :821 | 812 | 50行 | `fieldRest(...)` |
  | `dupErrors` | :861 | 890 | 29行 | `stM:pStM,enM:pEnM,` |
  | `cellBgFor` | :1194 | 1223 | 29行 | `});` |
  | `addAlias` | :2388 | 2399 | 11行 | `useState("")` |

  **`registerAlias` の591行が示すとおり、ずれは『少し前後する』では済まない。**
  引用先には毎回まったく無関係なコードが実在するので、**参照をたどった側は
  「行が消えた」ではなく「別の何かが正解らしい」と受け取る。** 静かに間違える形である。

  **番号を振り直す直し方は採らなかった。** 27件が数週間で壊れた事実が、
  番号を人手で保つ運用が成り立たないことをそのまま示している。
  **参照から番号を落とし、シンボル名と説明だけを残した**（`grep` できる側を正本にする）。
  番号しか書かれていなかった箇所（`（:132 店舗 / :1966 期間 …）` 等）は、
  指していたものの名前に置き換えた。
...
- **🟢→🟡 「スタッフ名がキーになる設定マップは7つ」という注記が `staffHidden` を数え落としていた**
  ...
  **2026-09-06 に `staffHidden` が足された時点で古くなっていた**。
  実際は 7（定数）＋ `overtimeSettings.byStaff` ＝ **8** で、注記は 7 と言っている。
  `settings.staffHidden[名前]`（app-utils.js:986）もスタッフ名をキーに持つため、
  「禁止文字を通すと set() が同期例外を投げる」という説明としても数が足りない。

  **挙動は正しい**（禁止文字の判定は入口の `firebaseKeyForbiddenChars` が担い、
  書き写しの側は判定に使われていない）。直したのは注記だけである。
  CLAUDE.md 自身が「**一覧を別の場所へ書き写さないこと**——tests/core.test.js が
  ドリフトを検出する（#108）」と書いているとおり、**数を直すのではなく列挙をやめて
  定数を指す形にした**。テストが守れるのはコードだけで、コメントの書き写しは守れない。

### 要確認（未修正）

- **🟢 略称バリデーションのコメントは、トースト文言については依然として偽**
  （[app-admin.js:3766](app-admin.js)）。#114 から持ち越し。判定は `CELL_COMMANDS` 駆動でも
  エラー文言は直書き。**今日の実測では一致している**（レジストリのキーは `h/k/x/y/締`、
  `isRestCommand` が `y/ｙ/休`、文言は `h・k・x・y・休・締`）ので実害は無い。
- **🟢「残す」を選んだときのポップアップの括弧書きが実態とずれる**（[app-admin.js:2822](app-admin.js)）。#114 から持ち越し・変化なし。
- **🟢 プラン上限モーダルの「店舗」文言に到達経路が無い**（[app-admin.js:4954](app-admin.js)）。#114 から持ち越し・変化なし。
- **🟢 配信版数が直近4コミットを含んでいない**。`index.html` の `?v=` と `app-core.js` の
  `build:` は6箇所とも `20260906-d3006ae` で相互に一致しているが、その後 `084d7cc`・`621b315`・
  今回の2件が app-*.js に入っている。**`/release-to-main` の手順63 がバンプするので申し送りのみ**。
...
**断言しているコメントを集め、断言ごとに反証を試みた。** 成り立っていたものは次のとおり。

- **「Firebase書き込みの唯一の入口」**（app-core.js:66）→ `.set(` / `.update(` を全ファイルで数えると
  Firebase 参照に対する呼び出しは `fbSet`・`fbUpd` の**2行だけ**（他は `Map.set`）。
  eslint の `no-restricted-syntax` も `ref/child/push(...).set/update` を error にしている。
  **ref を変数に取って書く抜け道**（lint が捕まえられない形）も探したが、`firebaseDB.ref()` を
  変数へ代入している2箇所は購読とクエリで、どちらも書き込んでいない。**成立**
- **「プランの購入・変更の唯一の入口」**（app-admin.js:4594）→ `CF_BASE` への `fetch` は4本で、
  Checkout/changePlan は `requestPlanAction` の中だけにある。呼び出し元は MyPageTab と
  UpgradeModal の2つで、**どちらも `requestPlanAction` を通る**。**成立**
- **「提出データを操作できるかの唯一の判定」**（app-staff.js:558）→ `canTouch` はセルクリック・
  名前クリック（✎ 修正）・削除ボタン・カーソル表示の**全経路**で通っている。**成立**
- **「タブ配列・決済失敗バナー・レンダリングの3箇所は必ずこの1つを共用する」**（app-admin.js:17）→
  `hideMypage` の参照は :168（タブ配列）・:196（バナー）・:224（レンダリング）の**ちょうど3箇所**。**成立**
- **「別名解決は resolveSubByAlias に一本化」**（app-admin.js:556）→ 呼び出しは
  グリッド・週集計・提出一覧・Excel・スタッフ再提出・Cookie復元の**6経路**（#105/#106/#112 の記録と一致）。**成立**
- **「このマージ1箇所で全経路に効く」**（app-staff.js:553）→ `SmModal` の呼び出し元は
  StaffView 2つ・PeriodsTab 1つの**3つ**で、`period` はコンポーネント内で解決している。**成立**
- **「実際に落としているのは visibleStaffList の1箇所」**（app-admin.js:2438）→ 呼び出しは
  app-admin.js:490 と :2135 の2箇所だが、**絞り込みの規則そのものは app-utils.js の1関数**にある。**成立**
- **「呼び出し元2箇所のどちらも名簿を渡す」**（app-admin.js:2134）→ `expXl` の呼び出し元は
  シフト作成タブ（`rosterStaffList`）と期間タブ（`resolvePeriodMaster` の staffList）で、
  **どちらも非表示スタッフを含む名簿**。**成立**
- **「レジストリに登録するだけで反映される（個別編集は不要）」**（app-admin.js:308）→ `GridLegend` は
  `CELL_COLOR_LEGEND` と `CELL_COMMANDS` を `map` で描いており、固定行は「時間+任意の文字」1件のみ。**成立**
...
**実ブラウザ検証は行っていない。** 今回の変更は**コメントだけ**で、`// 以降を除去した差分が
4ファイルとも空**であることを確認済み＝画面に現れる差が原理的に存在しないため、
ブラウザで確かめられることが無い。**未検証**: iOS 実機／PDF の実物／本番データ／Stripe 実データ。

### 測り方の記録

**「断言が正しいか」より先に「断言が指す先が正しいか」を見るべきだった。**
今回集めた断言（唯一の入口・1箇所で完結・全経路を通る）は**9件すべてが今も成立していた**。
一方で、その断言を補強するために添えられた**行番号のほうは27件中25件が壊れていた**。
**主張は寿命が長く、位置は寿命が短い。** 同じコメントの中で、寿命の違うものが同居していた。

**ずれ幅は「そのコメントが最後に触られてから、上に何行入ったか」を測っている。**
`registerAlias` の591行は、そのコメントを書いた日から今日までに app-admin.js の
その位置より上へ591行が積まれたということで、**コメントの鮮度を数値で読める**。
文面を読んで違和感を探すより速く、判定に主観が入らない（#114 の「文とコードのどちらが新しいか」を
git で比べる手と同じ形で、こちらは git を引かずに済む）。

**書き写した一覧は、定数の側にテストがあっても守られない。** `STAFF_KEYED_SETTING_MAPS` には
#108 でドリフト検出テストが入っているが、それが見張るのは**コードの書き写し**だけで、
**コメントの書き写しは対象外**だった。だから `staffHidden` の追加が2箇所の注記を素通りした。
「正本を1つに決める」を守るなら、**コメントで列挙することも書き写しに数える**必要がある。

**申し送り（次回の観点）**: **今回と同じ手を、コードではなくドキュメント側へ向ける。**
`CLAUDE.md`・`BACKLOG.md`・`RULES.md`・`VISION.md` は本文中で
`app-admin.js:3425` のような**ファイル:行**を大量に引用している。今回コードの中では
27件中25件が壊れていた以上、**同じ形式で書かれたドキュメント側の引用も同程度に壊れているはず**である。
`/tmp/refcheck2.js` と同じ要領で `.md` の引用を機械的に解決し、
引用先に名指しされたシンボルが実在するかを1件ずつ当たれば、同じ形が拾える。
**ドキュメントのほうが読者が多い**（次のループ・ユーザー・他モデルが最初に読む）ぶん、実害は大きい。

### 総括

**🔴 ゼロ。🟡 は2件検出し、どちらもその場で修正・検証・コミットまで完了した。**
#114 の申し送り（断言しているコメントを集めて当たる）をそのまま実行したところ、
**断言そのもの9件は全て今も成立していた一方で、断言に添えられた行番号27件のうち25件が
別のコードを指していた**。最悪は591行のずれで、引用先には無関係なボタンが実在していた。
壊れていたのは主張ではなく**主張の宛先**で、直し方も「番号を振り直す」ではなく
「**寿命の短いほうを捨てる**」だった。番号が27件そろって壊れた事実自体が、
人手で保つ運用は成り立たないという証拠になっている。
...
**影響範囲**: app-staff.js（`SmModal`）、呼び出し元3箇所（app-staff.js:264・:287／app-admin.js:1966）の props
...
> （app-staff.js:292/:301）で `resolveAlias` 済みだが、**Cookieから復元する経路（:48・:59）は
> それを通らない**。:156 のコメントは「name は :292/:281 で resolveAlias 済み＝常に登録名」と
...
> :167（`staffName` を `resolveAlias` に通し、**:156 の不変条件を構成で保証**する）。
