...
## Shifty バグチェックレポート（2026-09-24 自動実行 #145）

> 着手時の HEAD は `c5df7a8`。**#143 の HEAD（`9d2579b`）以降、`app-*.js`・`functions/`・
> `index.html`・`database.rules.json` は1バイトも変わっていない**（3回連続でコードが同一）。
> 同じ走査は同じ答えしか返さないので、基準値だけ機械で採り直し、残りは **#144 の申し送り**
> （「テストがあること」と「そのテストが実装を読んでいること」を取り違えていないか）に充てた。
>
> **申し送りへの答えは二段になった**。一次判定（「そのテストは配信物を readFileSync するか、
> require した関数を動かすか」）は **278件中278件が該当＝該当なし**。しかし**この判定は
> #144 が見つけた欠陥そのものを素通りさせる**——あのテストは `u.ADMIN_SHIFT_FIELDS` を
> 使うので「実装を動かす」側に入る。判定を一段鋭くしたところで1件出た。

### 修正済み

- **[🟡] 「禁止された書き方が残っていない」と主張する2つのテストが、実在する再発形を見逃していた**
  （tests/core.test.js:2000・:2580）→ `fef6176`

  どちらも `assert.deepStrictEqual(<正規表現の結果>, [])` の形で、**正規表現が再発形に
  当たらなければ永久に通る**。空振りを検出する仕掛け（走査が機能していることの担保）を
  持たない assert は、全7件中この2件だけだった（残り5件は `assert.ok(m)`・`assert.ok(i>0)`・
  `assert.ok(limitLines.length>0)` のような guard を持つか、`u.*` の実配列から計算している）。

  **対照で裏を取った**（app-admin.js の写しに禁止された書き方を注入し、差し替え口
  `SHIFTY_ADMIN_SRC` で向ける。配信物は1バイトも変更していない）:

  | 注入した再発形 | 旧（正規表現） | 新（AST） |
  |---|---|---|
  | `Object.values(staffAliases).flat()` | 検出 | 検出 |
  | **`Object.values(settings.staffAliases\|\|{}).flat()`** | **見逃す** | 検出 |
  | **`Object.values(settings?.staffAliases\|\|{}).flat()`** | **見逃す** | 検出 |
  | **改行して `.flat()` を次行へ** | **見逃す** | 検出 |
  | **中間変数に入れてから展開** | **見逃す** | 検出 |
  | 一覧をそのままコピペ | 検出 | 検出 |
  | **一覧をシングルクォートで書き写し** | **見逃す** | 検出 |
  | **一覧を行分け＋コメント付きで書き写し** | **見逃す** | 検出 |
  | 無関係な `Object.values({}).flat()` | 検出しない（正） | 検出しない（正） |
  | `[...STAFF_KEYED_SETTING_MAPS,"staffMemo"]`（正当な拡張） | 検出しない（正） | 検出しない（正） |

  見逃していた4形のうち **`settings.staffAliases` と オプショナルチェーンは app-admin.js に
  実在する書き方**（:622・:1935・:2243・:2356）で、想像上の形ではない。

  **修正**: 両方を `@babel/core` の `parseSync` による AST 走査へ移した（#144 と同じ扱い）。
  別名の自前展開は「`Object.values(X).flat()` かつ X が別名マップ」を AST で判定し、
  中間変数への代入も1段だけ辿る。一覧の書き写しは「要素が全て `staffXxx` の文字列リテラルで
  2つ以上の配列リテラル」で判定するのでクォートの種類・改行・コメントに依存しない。
  あわせて**両方に「0件が測定失敗でないこと」の担保**を入れた（`Object.values` の出現数・
  配列リテラルの出現数がゼロなら走査が壊れているとして落とす。実測 18/1件・376/31件）。
  共通の AST パース補助 `_parseAppFile` を置き、直上に「この系統のテストを正規表現で書かないこと」
  を理由つきで明記した。**実装（`app-*.js`）は1バイトも変えていない。**

  **対照スクリプト自身の故障も1件あった**（報告のため記載）。最初に組んだ対照は
  「改行して `.flat()` を次行へ」を「新版も見逃す」と出したが、原因は `printf '%s'` が
  引数中の `\n` を展開せず、**リテラルの `\n` を注入していた**こと。本物の改行で測り直すと
  検出する。AST は改行に非依存なので当然で、**対照の側が壊れていた**。

### 要確認（未修正）

**新規の 🔴🟡 は無し。** コードが同一のため #143・#144 からの持ち越しはいずれも変化なし。

- **#144 から継続（変化なし）**: BACKLOG 化済みの既存項目はすべて変化なし。
  `settings`/`staff`/`templates` の全体 `set()`／確定済み期間のタブ間の食い違い／
...
- **`app-utils.js` にブラウザAPI依存が混入していないか → 否定**。`window`/`document`/`location`/
  `localStorage` 等の出現は**コメント2箇所だけ**（:538・:818）。この不変条件は
  「npm test が落ちる」ことでしか守られておらず、**テストが呼ばない関数の中に混入しても落ちない**
  ので当てたが、実際には守られていた。
- **`fixedShiftCommandFor` は本当にテストからしか呼ばれないか → 肯定**（CLAUDE.md の記述どおり）。
  配信物からの呼び出しは0件、`tests/core.test.js` の2テストのみ。判定の根拠として読まないこと。
- **`PERIOD_SNAPSHOT_EXEMPT_STAFF_MAPS` は `staffHidden` だけか → 肯定**（app-utils.js:957）。
- **他ファイルとの一致を主張する残り4テストも空振りではないか → 否定**。`PLAN_RANK_UI`（:1804）は
  `assert.ok(m)`、`dupErrors`（:2779）は `assert.ok(i>0)`、`staffHidden`（:2614）は
  `assert.ok(limitLines.length>0)` で、いずれも**前提が崩れたら落ちる**ように書かれていた。

### 検証したこと

- `npm test` **280件パス**（#144 と同数。テストの本数は増やさず中身を強くした）・
  `npx eslint app-*.js` **0 errors / 95 warnings**（同数）・`node --check functions/index.js` 通過。
- RULES.md スキャン: `DEV_MODE` は式のまま（app-core.js:12）／`subs`・`periods` の全体 `set()` **0件**／
  `accounts` 全件読み 0件／SRI 11件／読み込み順 utils→core→staff→admin→main／
  `global/shops` の全件読み 0件（直キー読みのみ）／`database.rules.json` に `".read": true` 0件／
  `functions/index.js` に `.delete()` 0件。
- **配信版数は追随済み**（`20260923-bb394a3`。今回の変更は `tests/` のみ＝配信物ではない）。
- **実ブラウザの回帰スクリプトは今回も回していない**。#143 が7本 `allPass` を確認した時点から
  配信物が1バイトも変わっていないため（＝未実施であることを明記する）。
- **Firebase・Stripe・本番データには一切アクセスしていない。**

**申し送り（次回の観点）**

1. **このスキル自身の走査が3つ壊れている**（今回は実物を目で確かめて裏を取ったが、次回以降のために記録する）。
   `grep -o 'src="app-[a-z]*\.js"' index.html` は `?v=` が付くため**常に0行**を返す（読み込み順の検査が
   事実上効いていない）。`grep -n 'ref("global/shops")'` と `grep -n '".read": true'` も書式が実態と違い
   常に0件。**3つとも「正常」と同じ見た目の0件を返す**ので、レポートに「0件＝正常」と書けてしまう。
   走査そのものはスキルファイル側の話なのでこのループでは直していない（スコープ外は報告に留める）。
   正しくは `grep -o 'src="app-[a-z]*\.js[^"]*"'`／`grep -n 'global/shops'`／`grep -n '"\.read"'`。
2. 今回で「一覧 vs 実装」「テストが実装を読んでいるか」「その assert は空振りしないか」の3層が揃った。
   次は**配信物側の同じ形**——`.catch(()=>{})` や早期 return のうち、条件が成立しえない死んだ guard が
   無いか——を当てるとよい。判定は「その分岐に到達する入力が存在するか」で、純粋関数については
   `app-utils.js` を読んで機械的に採れる。
3. #143 申し送りの**ヒートマップ寸法定数**の境界水準を `example-heat-colw-fixed.js` に足す件は
   **今回も未着手**（実ブラウザが要り、コード無変更の回では回す理由が無かった）。
