...
## Shifty バグチェックレポート（2026-09-10 自動実行 #117）

> 着手時の HEAD は `b1f8604`。観点は **#116 の申し送りをそのまま実行**した——
> 「今度は『数』と『一覧』を当たる。`(\d+)(件|箇所|本|つ|…)` を機械的に集め、その数を実際に数え直す」。
> 数量の断言は**513件**あり、大半はその日に測った記録だった。**生きた断言に絞ると1件だけ壊れていて、
> その1件を検算しに行ったら、数ではなく検算の道具のほうが壊れていた。**
> あわせて #116 が .md に対してやった位置の検査を**コード内コメントへ向けた**ところ、
> b1f8604 が直したブロックの11行下に取りこぼしがあった。

### 修正済み

- **🟡 改名が `period.keepStaff` を移さず、同じ人がシフト表で2列に割れる**
  （[app-utils.js](app-utils.js) の `renameStaffInPeriods`・`8546bfb`）

  `renameStaffInPeriods` は写し（`snapshot`）と `keepAttrs` のキーを移し替えるのに
  **`keepStaff` だけ触っていなかった**。直上のコメントがその理由を明記していた——
  「keepStaff は触らない（削除済みの行に改名の導線が無く、**旧名が入ることがない**ため）」。

  **この前提は、同じコードベースの別の場所が明記している挙動で崩れる。** StaffTab の削除処理の
  コメントは「取り消し不能とは書かない——**同名で追加し直せば設定マップもsubsも復帰し**、
  失われるのは並び順だけである」と言っている。追加し直した行は現役スタッフなので、
  編集ボタンから普通に改名できる。そのとき `keepStaff` だけが旧名で残る。

  実測（配信物の関数をそのまま Node で実行。削除して残す → 同名で追加し直す → 改名）:

  | | 修正前 | 修正後 |
  |---|---|---|
  | `period.keepStaff` | `[{name:"田中",index:1}]` | `[{name:"田中太郎",index:1}]` |
  | シフト作成グリッドの列 | 佐藤 / **田中** / **田中太郎** / 鈴木 | 佐藤 / 田中太郎 / 鈴木 |
  | 同一人物の列数 | **2** | 1 |

  `mergeKeepStaff` は旧名を「名簿に居ない別人」とみなして足すため、**同じ人の列が2本並ぶ**。
  片方は空列で、Excel・PDF も同じ解決名簿を使うので同じ形で出る。

  これは #105〜#107 が3回続けて踏み、コード中のコメントが繰り返し警告している
  **「同じ不変条件に入口が2つあり、片方だけが知っている」**形そのものである。今回違ったのは、
  入口が2つあることを**コメントが明示的に否定していた**点で、否定の根拠（旧名は入らない）が
  他所の仕様によって偽になっていた。**「触らない」と書いた理由は、書いた時点では正しかった。**

  修正は `_renameKeepStaff` を足して `keepAttrs` と同じ扱いに揃えた。配列・数値キーobject
  （Firebase往復の形）・文字列要素のいずれでも移す。該当者が居なければ `null` を返して
  参照ごと据え置く（無駄な書き込みをしない）。改名先が既に居れば重複させない。
  **ユニットテスト4件を追加し、うち3件が修正前の `app-utils.js` では落ちることを確認済み。**

- **🟡 iOSズームの走査が偽陰性を出しうる書き方だった＋件数の断言が古かった**
  （[CLAUDE.md](CLAUDE.md)・`233c606`）

  申し送りどおり「現在はフォーム部品**56件**を全ファイル走査して0件」を検算したら**58件**だった
  （56→57→58 とずれていた）。ここまでは予想どおりの「数の腐り」である。
  **予想していなかったのは、検算に使った走査自体が壊れていたこと。**

  タグの終端を `indexOf(">")` で決めると、`onChange={e=>…}` の**矢印の `>` に当たって**
  タグを途中で切り、その先の `style` を読まないまま「0件」と答える。

  | 走査対象 | 素朴版 | 波括弧対応版 |
  |---|---|---|
  | 現在の配信物 | 0件 | **0件**（58件中） |
  | `bc7bf2e~1`（`fontSize:14` が実在した版） | **0件（偽陰性）** | **1件**（app-main.js:1491） |

  **現在の配信物が0件であること自体は正しく、結論は変わらない。** 変わったのは、その0件が
  偽陰性でないと言えるようになったこと。RULES.md の「input の fontSize を16px未満にしない」を
  守っているかの判定は、この走査ひとつに乗っている——そして守れなかったとき何が起きるかは、
  **同じ段落が「約2ヶ月残った」と記録している**。

  件数は書くのをやめ（UIを足すたびに増えるのでコード変更なしに偽になる）、**走査手順そのもの**を
  CLAUDE.md に埋めた。埋めたコマンドは**文書から抽出してそのまま実行し**、両版で上表の結果が
  出ることを確認している。

- **🟡 コード内コメントの行番号参照4件が別のコードを指していた**
  （[app-admin.js](app-admin.js)・[tests/core.test.js](tests/core.test.js)・`b3219c8`）

  #116 は `.md` の `ファイル:行` を潰し、`b1f8604` は app-admin.js の JSXコメント1ブロックを
  潰したが、**コード内コメントを全ファイル走査していなかった**。

  | 参照元 | 指していた先 | 実際の定義 | 引用先に実在したもの |
  |---|---|---|---|
  | app-admin.js | `savePeriods` app-main.js:1132 | 1155 | `revertAdminWrite` |
  | app-admin.js | `expXl(:2087)` | 2117 | `// フォールバック（iOS Safari 12以下等）` |
  | app-admin.js | `buildPdfCols(:1337)` | 1357 | `const staffAliasesPdf=…` |
  | tests/core.test.js | `_getSubForPeriod` app-admin.js:563 | 560 | `_getWorkShift`（**別の関数**） |

  最後の1件は3行しかずれていないのに**別の関数を名指しできてしまう**（`_getSubForPeriod` と
  `_getWorkShift` は3行違いで隣り合う）。ずれ幅が小さいほど、間違いだと気づきにくい。

  **1件目は `b1f8604` が直したブロックの11行下にある。** 前回のセッションは自分が立っている
  コメントブロックを直し、すぐ下の同じ形を見ていない。**「この箇所を直す」と「この種類を潰す」は別の作業。**
  直し方は #115・#116 と同じで、番号を振り直さず grep できるシンボル名だけに置き換えた。
  置換後の再走査でコード内コメントの行番号つき参照は**0件**。

### 要確認（未修正）

- **🟢 配信版数が直近コミットを含んでいない**。`index.html` の `?v=` と `app-core.js` の
  `build:` は6箇所とも `20260908-0d5aba8` で相互に一致しているが、今回 app-utils.js・app-admin.js を
  変更した。**`/release-to-main` の手順63 がバンプするので申し送りのみ**（#115・#116 から継続）。
- **🟢 #115・#116 レポート本文の引用は直していない**（CLAUDE.md 末尾の記録ブロック内）。
  **意図的**。その日に測った値の記録であり、後から書き換えると記録が偽になる。
- **🟢 略称バリデーションのコメントは、トースト文言については依然として偽**（app-admin.js の CompanyTab）。
  #114 から持ち越し。今日の実測でも文言とレジストリは一致しており実害なし。
...
**今回 BACKLOG へ起こした項目は無い。** 検出した🟡3件はその場で閉じた。

### 検証したこと

`npm test` **249件パス**（+4・追加分の3件は修正前の app-utils.js では落ちることを確認済み）／
`npx eslint app-*.js` **0 errors 96 warnings**（増減なし）／`node --check functions/index.js` OK。

**数量の断言の検算**（申し送りの本題）: 5文書から `(\d+)(件|箇所|本|つ|経路|マップ|エンドポイント…)` を
機械的に集めて**513件**。大半は測定日つきの記録なので、**現在のコードについて言っている断言**に絞って
実測と突き合わせた。壊れていたのは**フォーム部品の件数1件だけ**で、残りは全て一致した:

| 断言 | 出どころ | 実測 | 判定 |
|---|---|---|---|
| `SmModal` の呼び出し元は3つ（StaffView 2・PeriodsTab 1） | app-staff.js | 3 | ✅ |
| `resolveSubByAlias` は6経路 | CLAUDE.md | 6（staff 2 + admin 4） | ✅ |
| `visibleStaffList` を列に使う呼び出し元2箇所 | app-admin.js | 2 | ✅ |
| `verifyShopOwner` を通る課金系4エンドポイント | BACKLOG.md | 4 | ✅ |
| `database.rules.json` の最上位キー6つ | CLAUDE.md | 6 | ✅ |
| `STAFF_KEYED_SETTING_MAPS` は7件（+`overtimeSettings.byStaff` で8） | CLAUDE.md | 7 | ✅ |
| `hideMypage` を共用する3箇所 | app-admin.js | 3 | ✅ |
| `kind:"fixed"` は現状1件のみ登録 | app-admin.js | 1（`締`） | ✅ |
| **フォーム部品56件** | CLAUDE.md | **58** | ❌ |

**存在の検査**（#116 の手をコードへ向けた）: コメント中の識別子形トークン**307件**のうち、
配信物のどこにも無いものは16件で、**全て偽陽性**だった（`adjustedXxx` 等のプレースホルダ、
`WKWebView`・`indexedDB`・`reCAPTCHA`・`StrictMode`・`TypeError` のような外部の名前）。
**コード側では「消えた名前を指し続ける」形は出なかった**——#116 が .md で1件見つけたのと対照的で、
コメントは隣のコードと一緒に消えるぶん腐りにくい。

**RULES.md のスキャン項目は全項目クリア**: `DEV_MODE` は式のまま（app-core.js）／
`subs` の `set()` 全体上書き **0件**／`accounts`・`global/shops` の全件読み **0件**／
読み込み順（utils→core→staff→admin→main）維持／SRI **11本**／
**フォーム部品58件の `fontSize<16` 0件（波括弧対応版で実測）**／
`functions/index.js` の `.delete()` 誤用 **0件**／`secrets` 7箇所。

**Firebase・Stripe には読み書きとも一切アクセスしていない。実ブラウザ検証は行っていない**
（`keepStaff` の修正は純粋関数1つに閉じており、配信物の関数をそのまま Node で実行して
グリッドの列が2本→1本になることを確認した）。
**未検証**: iOS 実機／PDF の実物／本番データ／Stripe 実データ／実ブラウザでの改名操作。

### 測り方の記録

**「数が古い」を追いかけたら、数を数える道具のほうが壊れていた。** 申し送りは
「N件という断言は要素が1つ増えた瞬間に静かに偽になる」と予想し、それは当たった（56→58）。
だが検算に使った素朴な走査は、`fontSize:14` が実在する版に当てても **0件と答えた**。
**間違った数のほうが、間違った測り方より安全だった**——数のずれは読めば気づけるが、
測り方の偽陰性は「0件」という**正しく見える答え**を返す。

**だから今回は、数を直さずに数を消した。** 直した数はまた腐る。代わりに
**走査手順そのものを文書に置き、文書から抽出してそのまま実行できることを確認した**。
検査器が正しいことの証明には**既知の違反を含む過去の版**を使った（`bc7bf2e~1`）。
「0件」と言う検査器は、**1件出るはずの入力で1件出すことを見せない限り信用できない。**

**位置の参照は、ずれ幅が小さいほど危ない。** 今回の4件のうち最悪は23行ずれだが、
一番たちが悪いのは**3行ずれ**だった——`_getSubForPeriod` と `_getWorkShift` は隣り合う行にあり、
3行ずれた参照は**実在する別の関数を正しそうに名指しする**。#116 は1141行のずれを見つけて
「引用先に無関係な店舗切り替えボタンが実在した」と書いたが、大きくずれた参照は読めば分かる。
**小さくずれた参照は読んでも分からない。**

**「この箇所を直す」は「この種類を潰す」ではない。** `b1f8604` は自分が見ていたコメントブロックを
正しく直し、**11行下の同じ形**を残した。#116 が .md でやった走査をコードへ広げるだけで出た。
直したときに、その直し方を**全ファイルに機械的に当て直す**ところまでを1つの作業と数える必要がある。

**申し送り（次回の観点）**: **今度は「否定」を当たる。** 今回いちばん実害のあった
`keepStaff` のバグは、コメントが **「触らない（〜が無いため）」と理由つきで否定していた**箇所にあった。
数や位置の断言は腐っても「古い」だけだが、**否定の断言は、その根拠が別の場所の仕様で覆ると
バグそのものになる**（しかも「意図的にそうしてある」と読めるので、次に来た人が直さない）。
コード・文書から `触らない`・`〜しない`・`必要ない`・`起こらない`・`ことがない`・`対象外` を
機械的に集め、**その否定の根拠が今も成り立つか**を1件ずつ当たれば同じ形が拾えるはずである。
**否定は、肯定と違って「試せば分かる」形をしていない**——書かれた理由を読んで、
それを覆す仕様が他所に無いかを探すしかない。

### 総括

**🔴 ゼロ。🟡 は3件検出し、すべてその場で修正・検証・コミットまで完了した。**
#116 の申し送り（「数」を当たる）をそのまま実行したところ、生きた数量の断言9件のうち
**壊れていたのは1件**（フォーム部品 56→58）だった。**予想していなかったのは、その1件を
検算しに行った先で、検算の道具が偽陰性を返したこと**——素朴な走査は、`fontSize:14` が
実在する過去の版に当てても「0件」と答える。RULES.md のiOSズーム規則を守れているかの判定は
この走査ひとつに乗っており、守れなかったときに何が起きるかは同じ段落が「約2ヶ月残った」と
記録している。**実害がいちばん大きかったのは数の話ではなく**、改名が `period.keepStaff` を
移さないため**同じ人がシフト作成グリッド・Excel・PDF で2列に割れる**バグで、
これは「削除して残す → 同名で追加し直す → 改名」で再現する。
コメントはその箇所を「触らない」と**理由つきで否定していた**が、
その理由は同じコードベースの別の場所が明記している挙動によって偽になっていた。
