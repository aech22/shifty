...
## Shifty バグチェックレポート（2026-09-21 自動実行 #139）

> 着手時の HEAD は `8f5147f`。#138 以降に入ったのは**ドキュメントのコミット3件だけ**で app-*.js は
> 1バイトも変わっていなかったため、新しいコードを当たる代わりに **#138 の申し送りをそのまま実行した**
> ——「確定済み期間について、シフト作成タブと提出一覧が同じ属性・退勤延長・従業員番号を見ているか」。
> 答えは**見ていない**（🟡・BACKLOG化）。測っている途中で、**同じ提出一覧タブの中でも行と詳細モーダルが
> 違う属性を見ている**ことが分かり、こちらは判断が要らなかったので修正した。

### 修正済み

- **[🟡] 提出一覧で、行の超過判定と詳細モーダルの勤務時間が違う属性の休憩を引く**（app-admin.js の `SubsTab`・`9a36285`）
  `keepAttrs`（属性の期間指定）を持つ期間で、**行の超過判定だけが「その期間の属性」で休憩を引き**、
  詳細モーダルの勤務計・週間勤務時間・月計・各日の勤務時間・合計は**現在の属性**で引いていた。

  **実測**（375px・実ブラウザ・`SubsTab` だけをマウント＝**Firebase へは1バイトも出していない**）:

  | | 修正前 | 修正後 |
  |---|---|---|
  | 行の「1日超過」バッジ | 出る（＝行は 8:00 と判定） | 出る |
  | モーダルの勤務計／合計／週間勤務時間 | **7:00** | **8:00** |

  **バッジは「7時間を超えた」と言っているのに、同じモーダルの数字は 7:00 ちょうど**という、
  1つの画面の中で完結する矛盾になっていた。行に `applyKeepAttrs` を当てたときのコメントは
  「休憩の属性タグも同じ属性で引く必要がある」と明記しており、**モーダルだけがその決定から漏れていた**。
  `applyKeepAttrs` は `keepAttrs` を持たない期間では**同じ参照を返す**ので、指定の無い期間の挙動は変わらない。
  回帰テストを `.claude/skills/shifty-e2e-verify/scripts/example-subs-keepattrs.js` に保全（`66ddf54`）。
  **修正前の配信物（`8f5147f`）に向けると EXIT=1** ＝素通りするテストではない。

### 要確認（未修正）

- **[🟡] 確定済み期間で、シフト作成タブと提出一覧が同じ人・同じ日に違う勤務時間を出す**
  → **BACKLOG化済み**（条件B＝仕様判断）。期間の確定（写し）は**シフト作成タブの中だけ**で効き、
  提出一覧は `resolvePeriodMaster` を通らないので終了した期間でも現在の設定で計算する。

  | | シフト作成タブ（凍結） | 提出一覧（現在値） |
  |---|---|---|
  | **純勤務** | **8:30** | **7:00** |
  | 退勤延長 / 休憩 / 週上限 / 従業員番号 | +30分 / 60分 / 28h / A-01 | +0分 / 120分 / 40h / B-99 |

  配布した Excel・PDF は凍結側から出るため、**食い違うのは提出一覧のほう**。写しは「確定」ボタンを
  押さなくても期間が生きている間に自動で撮られるので、**シフトを作った期間はほぼ全て該当する**。
  2026-08-21 は「凍結はシフト作成タブの中だけ」と合意し、2026-09-08 の `keepAttrs` は提出一覧の
  同じ問題を**直した**——**逆向きの答えが2つ入っている**のでコードからは決められない。

- **#138 から継続（変化なし）**: 変更マークと「変更あり」バッジの食い違い（🟡・BACKLOG化済み）／
  属性削除で休憩のタグが残る（🟢）／`moveStaffHiddenBoundaries` の同一開始日（🟢・意図的）／
  `dup`・`posErr` がPDFに無い理由のコメント（🟢）／部門別PDFの未登録名（🟢）／休み提出の斜線と黄色（🟢）／
  「締」が無効な店舗のメモ（🟢）／「締」入りの略称（🟢）／店舗切替の `plan`（🟢）／
  `purgeInactiveShops` の招待コード後始末（🟢）／企業ログインの試行制限（🟡・BACKLOG化済み）／
  `recentPeriodIds`（🟢）／`ShiftEditTab` の他店舗 `subs` 全件読み（🟢）／退勤が出勤より前（🟡・BACKLOG化済み）。
...
- **詳細モーダルの `det.staffName` が別名のまま → 否定**（`setDet({...sub,staffName:resolvedName})`・app-admin.js:3856 が解決済みの名前で開く）。
- **`getOT`（退勤延長）も `keepAttrs` の影響を受ける → 否定**（`overtimeSettings.byStaff` はスタッフ名キー）。
  同じ理由で出勤日数（`shiftBandInfo`）も影響を受けない。
- **`period` を渡し忘れた呼び出しが残っている → 否定**。`visibleStaffList`(2)・`isUnregisteredSubName`(4)・
  `mergeKeepStaff`(3)・`applyKeepAttrs`(2) の**全呼び出しが `period` を渡している**。
- **`isPro` で Premium 機能を出している → 否定**（0件）。**CF の `secrets` 抜け → 否定**（7つの `runWith` すべて宣言あり・`.delete()` 誤用0件）。
...
- RULES.md スキャン: `DEV_MODE` は式のまま／`subs` の全体 `set()` 0件／`accounts` 全件読み 0件／
  SRI 11/11（12件目は `rel="canonical"`）／読み込み順 utils→core→staff→admin→main／
  フォーム部品 **58件・`fontSize<16` は0件**。
- **配信版数は `20260920-40a88de` のままで、今日の app-admin.js 変更を指していない**。
  リリース時に `/release-to-main` の手順63でバンプすること（バンプしないとキャッシュに旧JSが残る）。
- **Firebase・Stripe には一切アクセスしていない**。

**申し送り（次回の観点）**: 「`period` を渡し忘れた呼び出し」は上記のとおり走査済みで空振りになる。
次に測るとよいのは**同じ数字を出す式が2本ある場所**——`SubsTab` は行と詳細モーダルで週・月の集計を
別々に書いており（行の `_min`/`_windowVio` とモーダルの `_dayMin`）、**2本が同じ日に同じ分数を返すかは
未測定**。とくに**行の「1日超過」だけが `sub.shifts[d]` を直に読み、週・月は `_shiftAt` 経由で
別名解決した1シフトを引く**非対称があり、同一人物・同一期間に sub が2つある状態（#81 の根）では
**別の sub を読みうる**。
