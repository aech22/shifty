...
## Shifty バグチェックレポート（2026-08-19 自動実行 #84）

### 修正済み

- **🟡 詳細モーダルの週間・月間勤務時間が、別名で提出されたぶんの属性・退勤延長を引けていなかった**（[app-admin.js:3102](app-admin.js)）→ `98ec60a`
  属性別制限や休憩タグの元になる `staffAttributes` と、退勤延長の `overtimeSettings.byStaff` は、どちらも**登録名をキーに持つ設定**である（`onRenameStaff`・:196/:201 が登録名のリネームに合わせてキーを移し替えていることが根拠）。ところが提出一覧の詳細モーダル（:3102）は、集計対象の `wSS` に**別名で提出されたsubが混ざる**にもかかわらず、`getBreaksFor`/`getOT` を `s.staffName` / `os.staffName`＝**subが持っている名前そのまま**で引いていた。別名は当然この設定マップに載っていないので、属性は既定の `parttime`、延長は0分に落ちる。
  **同じ集計を正しく書いている箇所が、同じファイルに3つあった**——`getPeriodMin`(:1020)・`getWeekMin`(:1032)・行の上限判定の `_min`(:3044) は、いずれも「**シフトは別名フォールバックで引き当てるが、属性・延長は解決済みの登録名で引く**」形で揃っている（`_getSubForPeriod`・`_getWorkShift`・`_shiftAt` がフォールバックを担い、引数の名前は登録名のまま渡す）。**規約から外れていたのはモーダルの1箇所だけ**で、設計判断は要らなかった。
  **配信物から `:3044` と `:3102` の両ブロックを切り出して同一データで実行**して断定した。登録名「田中」＋別名「たなか」、ディナー延長+60分、前期間ぶんを別名で提出——という条件で、**同じ人の同じ週を、行は 45:00、モーダルは 43:00 と計算した**（別名ぶん2日の延長120分が落ちていた）。**週上限44時間を設定した店舗では、行に「週超過」バッジが出ているのに、確認のため詳細を開くと44時間を下回って見える**。修正後は両者とも 45:00 で一致する。

- **🟡 詳細モーダルの月計が、同一日を二重計上していた**（[app-admin.js:3106](app-admin.js)）→ `bc95af4`
  上の1件目を直す過程で、**同じ行にもう1つ独立した欠陥**が見つかった。`moTot` は `wSS`（同一人物の登録名ぶん＋別名ぶんのsub）を**subごとに走査して足す**形で、**日付での重複排除が無かった**。一方、行の月計（:3044 `monthMap`）は `_min`→`_shiftAt` を通すので `name|date` につき1シフトしか採らず、**モーダル自身の週バーも**日付ごとに1件しか採っていなかった。つまり**同じモーダルの中で、週バーの合計と月計が食い違う**。
  重複が実際に起こる条件は既知で、**BACKLOG に起票済みの「別名で提出した人が別端末から再提出すると、同じ期間に2つのsubができる」がそのまま該当する**（期間の重なりも別経路で同じ状態を作る）。
  **実測**: 実勤務3日・1440分に対し、行の月計とモーダルの週バーが 24:00 を出す一方で、**モーダルの月計だけが 48:00（ちょうど2.00倍）**だった。
  週・月の集計を `_dayMin`（日付ごとに `_shiftAt` で1シフトだけ引く）に**一本化**した。#83 が `_windowVio` で採ったのと同じ手で、以後どちらか片方だけが直る形にならない。非回帰7ケース（別名なし・設定なし・属性タグ付き休憩・休み・前月除外・管理者調整値・週バー）を配信物の実評価で確認し全パス。

- **🟡 ヒートマップの時間軸が、別名提出者の退勤延長を見落として短く描かれていた**（[app-admin.js:997](app-admin.js)）→ `ff151df`
  同じレンズを全ファイルに広げて出た3件目。`heatHours`（軸の上端を決める式）だけが `getOT(sub.staffName,…)` と**subの名前そのまま**で引いており、**出勤人数を数える `heatData`(:789) は登録名（`staffList` 由来の `name`）で引いていた**。同じ延長設定を2つのキーで見ていたことになる。
  **実測**: 別名提出・退勤22:00・ディナー延長+120分のとき、**`heatData` は24時まで出勤として計上するのに、軸は22時までしか伸びない**。計上済みの2時間ぶんの行が軸から欠けるため、**その時間帯に人がいることが画面に出ない**（数字が違うのではなく、行ごと消える）。修正後は両者とも24時で一致。別名を使っていない店舗では不変。

### 要確認（未修正）

- **🔴（継続・プロセス）このループから実機E2Eが7回続けて実行できない**（環境要因・コード変更なし）
  今回も `preview_list`（稼働0件）と `preview_start` を実際に実行し、**「Dev servers can't be started from unattended sessions」と明示的に拒否された**。Bashでのサーバー起動は規約で禁止のため代替なし。**#78〜#83 に続き今回も1件も実行できなかった。ユーザーが対話セッションで `/bug-check` を手動実行すれば解消する。**
- **🟡 #77 の宿題（Free 上限の実機検証）は静的監査完了・実測のみ未達**（上記🔴が理由）。**残るは実機で21人目・2件目を押す1回だけ**
- **🟢（新規・申し送り）今回の修正で `_dayMin`(:3106) という式が1つ増えた**。`_min`(:3044) と**同じ計算を別スコープで書いた形**で、現時点では両者とも `_shiftAt`＋登録名で一致している。**次回はこの2つが乖離していないかを見ること**（#83 の申し送り「新しく増えた式が次回の少数派になっていないか」と同じ点検。なお #83 が増やした `_windowVio`・`_allWork` は今回確認したところ**単一実装のままで少数派になっていない**）
- **🟢 `effShiftStart`/`effShiftEnd` の未採用が app-admin.js になお3箇所残る**（`:994` ヒートマップの時間軸／`:3033` 提出一覧の出勤日数／`:3091` 詳細モーダルの出勤日数）。**#82・#83 と同じ理由で今回も触らなかった**——直し方が BACKLOG の判断待ち項目（片側セルだけの日を何時間と数えるか）に依存する
...
- **🟡 別名提出者が別端末から再提出すると同一期間に2つのsubができExcelが古い方を出す**（#81）→ BACKLOG化済み（**今回、この重複が引き起こす3つ目の観測可能な症状＝月計の二重計上を特定し、表示側は上記2件目で閉じた**旨を追記済み）
...
- 以下は #46〜#83 からの継続（変化なし）: 未claim店舗の取り込み／`findActiveSubscription` の `|| live[0]`／`planExpiry` のUTC計算／**片側セルだけの日が0分**／`purgeOldPeriods` のUTC日付／`generateInviteCode` のデッドコード／ほか

### 今回の対象と見つけ方

**#83 以降のコミットは0件。12回続けて「差分ゼロ」の回である。**

**(1) 申し送りが指した方向を、少しずらして当てた。** #83 は「**同じ形・同じキーのインデックスを全部並べて、重複時の解決規則を突き合わせる**」を推奨していた。今回まずそれをやったが、`subsByKey`(:485)・`workShiftByStaffDate`(:496)・`shiftByStaffDate`(:2999)・`companyData` の `workMap`(:407) の4つは**規則が揃っており新しい違反は出なかった**（#83 の修正が効いている）。そこで**「同じキーで引いているか」ではなく「引くときのキーの作り方が揃っているか」**へレンズをずらした——`calcNetWorkMinutes` の11箇所の呼び出しを並べ、**属性・延長を引く名前が登録名か生のsub名か**を突き合わせたところ、3箇所が生の名前だった。**インデックスの棚卸しは尽きても、引数の棚卸しはまだ残っていた。**

**(2) 「同じ操作を正しく書いている箇所が他にあるか」が7回連続で決め手だった。** 1件目は同一ファイル内に**正しい形が3つ**（:1020・:1032・:3044）あり、外れていたのは1箇所だけ。3件目は**同じヒートマップの中**で `heatData` が正しく `heatHours` が間違っていた。どれも多数派へ揃えるだけで済み、設計判断が要らなかった。

**(3) 最初の1件を直す作業が、2件目を露出させた。** `moTot` の名前キーを直すために式を読み込んだ結果、**同じ式が日付の重複排除をしていない**ことに気づいた。1件目（誤った属性・延長）と2件目（二重計上）は**根が別**で、片方だけ直しても数字は合わない。**「直す対象を読む」ことが独立した検出手段になる**という点は、grep 主体の探索では出てこない。

**(4) 見つけた順ではなく、影響の形で3件を分けてコミットした。** 1件目＝数字が少なく出る／2件目＝数字が倍になる／3件目＝行ごと消える。**同じ「別名の取り違え」という根から出ていても、症状も直し方も独立している**ので1コミット1論点に割った。

**(5) 配信物をNodeで実行する手法が7回目の定着。今回は初めて「2つのブロックを同時に切り出して突き合わせた」。** #83 は1ブロックを抽出して単体で評価したが、今回は `:3044`（行）と `:3102`（モーダル）を**同じ抽出器で切り出し、同一の入力で同時に走らせて差を出した**。「画面Aと画面Bが食い違う」という主張は、**両方を同じデータで実際に走らせない限り推測にとどまる**。修正後も同じスクリプトを再実行するだけで一致を確認できた。

### 検証したこと

- `DEV_MODE`（app-core.js:12・式のまま）／`DEV_PLAN_OVERRIDE`（:125）正常
- `subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／`global/shops` の全件読み **0件**（直キー読みのみ）／CF `.delete()` **0件**／CF secrets **6宣言**
...
- **名前キーのレンズ（今回の主レンズ）**: `getOT` 12箇所・`getBreaksFor` 10箇所の**第3引数（名前）を全部並べて**、登録名（`name`/`n`/`resolvedName`/`det.staffName`）か生のsub名かを分類。修正後は**生のsub名で引いている箇所が0件**であることを再確認
- **同型インデックスの再棚卸し**: `subsByKey`(:485)・`workShiftByStaffDate`(:496)・`shiftByStaffDate`(:2999)・`workMap`(:407) の重複解決規則を再確認し、**#83 の修正後は乖離なし**（`workMap` だけ「両方揃った方を優先」だが、理由がコメントに明記された意図的な規則）
- **#83 が増やした式の追跡**: `_windowVio`・`_allWork`・`_workDatesOf` の呼び出し元を数え、**いずれも単一実装のままで少数派になっていない**ことを確認
- **修正の検証（配信物の実評価）**: 行×モーダルの週・月一致／二重計上の倍率／ヒートマップ軸の上端／非回帰7ケース
- **未検証（今回は手段が無い）**: ブラウザでの実描画すべて／**修正後の詳細モーダルの実表示**／**ヒートマップ軸が実際に伸びること**／**Free 上限に達した店舗での実ブロック**／実機 iOS Safari

### 総括

**測れない回が7回続き、それでも 🟡 が3件出て3件とも直った。** 3件とも根は1つ——**`sub.staffName` には別名がそのまま入るのに、属性・退勤延長の設定は登録名をキーに持つ**——で、#81 が「別名レンズ」として9箇所を閉じた際に、**設定マップを引く経路までは追い切れていなかった**ことになる。

**実害の順に並べると2件目が最も重い。** 月計が**ちょうど2倍**になるのは、労務管理の数字としてそのまま誤りであり、しかも**同じ画面の週バーの合計と一致しない**ので、管理者はどちらを信じるべきか判断できない。1件目は「行の警告と詳細の数字が逆を向く」形で、**上限超過の確認作業そのものを無効にする**。3件目は数字ではなく**行が消える**ので、気づく手がかりが画面上に残らない。

**「レンズを尽くした」と「レンズをずらす」は違う、と分かった回だった。** #83 の推奨（同型インデックスの棚卸し）は正しく、実際にやってみたら**新しい違反は0件だった**——つまり前回の修正が効いていた。そこで打ち止めにせず、**同じ対象を別の軸（引数の作り方）で切り直したら3件出た**。次も同じことが起きうる。

**次回の手を1つに絞る。** 実測が復活したなら**#77 の宿題（実機で21人目・2件目を押す1回）をそのまま実行する**。使えないままなら、推奨は**「同じ設定マップを引いている箇所を、マップごとに全部並べてキーの作り方を突き合わせる」**——今回は `staffAttributes`・`overtimeSettings.byStaff` の2つで3件出た。**まだ `staffColors`・`staffNumbers`・`staffPositions`・`staffWorkplaces`・`staffAliases` の5つを同じやり方で見ていない**（`staffColors`(:2127) と PDF の `cols`(:1488) は未登録名の扱いが絡むため、BACKLOG の仕様判断待ち項目と切り分けて見ること）。あわせて、**今回増えた `_dayMin` が `_min` と乖離していないか**を必ず見ること。
