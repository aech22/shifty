...
## Shifty バグチェックレポート（2026-08-21 自動実行 #88）

### 修正済み

- **🟡 シフト作成タブのExcelだけが「書き出し店舗名」（`xlShopName`）を無視していた**（[app-admin.js:1616](app-admin.js)）→ `e4f8df2`
  設定タブ「Excel書き出し設定」の店舗名は、説明文が **「設定した名前はExcel出力時のファイル名・シート内店舗名に反映されます」**（`:3823`）と**無条件で**約束している。ところが Excel を出す経路は2つしかなく、**シフト作成タブの「Excel出力」だけが `settings.xlShopName` を読まず、店舗の登録名（`shopName`）を使っていた**。期間タブの Excel（`:2020`）は既に `m.settings.xlShopName||shopName` を使っている＝**2箇所のうち1箇所だけがズレていた**。
  **何が起きるか**: `shopName` は `expXl` の中で2つの出力を同時に決める——**シート2行目の店舗名セル**（`:2193`・太字14pt）と、**ダウンロードファイル名**（`:2288`）。実測では設定「鷄えん 東通り店」・登録名「東通り」に対し、期間タブが `鷄えん 東通り店2026年8月後半_修正前.xlsx`（セルも「鷄えん 東通り店」）を出す一方、シフト作成タブは `東通り2026年8月後半.xlsx`（セルも「東通り」）を出した。
  **ズレていた側が、実際に配る側だった**: 期間タブの Excel は `resolver` を渡さないため **`_修正前`**（提出そのまま）で、シフト作成タブの Excel が**管理者の調整を反映した完成版＝店舗に配るシート**である。つまり設定が効いていなかったのは**成果物の方**で、効いていたのは下書きの方だった。
  **仕様判断が不要だった理由**: 設定のキー名が `xlShopName`（xl = Excel）で、UIも「Excel書き出し設定」ブロックの中にあり、説明文がファイル名とシート内店舗名の**両方**を名指ししている。もう一方の呼び出し元が既にその通りに実装している。**「登録名を使うのが正しい」と読める材料がどこにも無い**ため、揃える方向は一意に決まった。
  **PDFは変更していない**（`:1402`・`:1487`・`:1550` は `shopName||"店舗"` のまま）。設定名が `xlShopName` で説明文も「Excel出力時」と範囲を明示しており、PDFが登録名を使うのは仕様の範囲内と判断した。`_修正前` サフィックスの有無も2つのボタンの意図的な区別なので残した。
  **検証（配信物の式をNodeで実行）**: シート内店舗名セル式とファイル名式をそのまま取り出し、`xlShopName` の3状態で修正前後を突き合わせた。**設定ありのときだけ挙動が変わって両ボタンの店舗名が一致し、空文字（クリア済み）・未設定では修正前と完全に同一＝非回帰**であることを実測。

### 要確認（未修正）

- **🔴（継続・プロセス）このループから実機E2Eが11回続けて実行できない**（環境要因・コード変更なし）
  今回も `preview_list`（稼働0件）と `preview_start` を実際に実行し、**「Dev servers can't be started from unattended sessions」と明示的に拒否された**。Bashでのサーバー起動は規約で禁止のため代替なし。**#78〜#87 に続き今回も1件も実行できなかった。ユーザーが対話セッションで `/bug-check` を手動実行すれば解消する。**
- **🟡（新規）期間の確定（写し）が `periods` ノードを肥大化させ、起動時のDL量が期間数に比例して増える** → **BACKLOG化済み**（設計判断待ち）
  `1c5272b` の `period.snapshot` は staffList ＋ 凍結対象settings 15キーの**丸ごとのコピー**で、`periods` は起動時に**全件・無条件で購読される**（[app-main.js:385](app-main.js)。subs のような期間窓が無い）。**実測: 1期間あたり 57,620 bytes（期間レコード本体は約250 bytes ＝約230倍）、うち `dateCandidates`+`dateCandidatePosTypes` だけで 89%。** 期間36件なら約2.0MBが毎回の起動時にDLされる。書き込み側も `savePeriods` が periods 全体を `set` するため、**設定を1つ変えてタブを開くたびに全期間の写しを再アップロードする**。
  **ループで直さなかった理由**: 「日付範囲ぶんだけ写す」が自然に見えるが**挙動を変える**——ヒートマップ補完境界（`:786`・`:1024`）は `Object.values(settings.dateCandidates).flat()` ＝**期間外を含む全日付**から最小/最大を取っている。凍結の粒度という設計判断になるため案A〜Dを添えて起票した。
- **🟢（新規）`period.lockedAt` は書かれるが、どこからも読まれない**（[app-admin.js:1578](app-admin.js)）
  「この期間を確定」ボタンだけが `lockedAt:new Date().toISOString()` を書き、「確定を解除」が消す。**`app-*.js`・`functions/index.js`・`database.rules.json` のいずれにも読み出しは無い**（grep 0件）。しかも**自動凍結の経路（写し更新の useEffect・`:476`）は `lockedAt` を書かない**ため、「手で確定した期間にだけ日時が残り、最終日を過ぎて自動で凍結した期間には残らない」という非対称になっている。確定/非確定の判定は `snapshot.staffList` の有無だけで行われており動作には影響しない。今回は🟢のため修正していない（「いつ確定したか」を将来UIに出す意図があるなら自動凍結側にも書くべきで、そこは仕様判断になる）。
- **🟢（新規）`periodSnapshotEqual` だけが配列/オブジェクトの表現差を吸収せず、`resolvePeriodMaster` とで防御が非対称**（[app-utils.js:697](app-utils.js)）
  `resolvePeriodMaster` は Firebase が写しの `staffList` をオブジェクト化して返す場合に備えて `Object.values` で受け直す（同名のユニットテストもある）。一方 `periodSnapshotEqual`（`_normSnap`）は**配列とオブジェクトを別物として `JSON.stringify` 比較する**。**Nodeで `sanitizeForSet`＋RTDBの往復規則を再現して実測**したところ、`staffList` に穴（`null`/`undefined`）があると読み戻しが `{"0":…,"2":…}` になり、`periodSnapshotEqual` が**永久に false ＝ 毎レンダーで再書き込み（書き込みループ）**になった。密な配列・空配列では正しく1回で収束することも確認済み。
  **ただし現行の経路では到達しない**: staff の購読ハンドラが `.filter(s=>s&&typeof s==="string")` で**読み込み時に必ず密な文字列配列へ落としている**（[app-main.js:405-407](app-main.js)）ため、`buildPeriodSnapshot` に穴あき配列が渡ることがない。**実害を示せていないので🟢に留める**が、「片方だけがオブジェクト化を想定している」非対称は残る。
- **🟢（新規）確定済みバッジが「閲覧のみ」端末に出ない**（[app-admin.js:1571](app-admin.js)）
  バッジとボタン2つがまとめて `!ownerReadOnly` の内側にある。一方 `resolvePeriodMaster` は `ownerReadOnly` を見ないので、**閲覧専用端末にも凍結された名簿・設定はそのまま適用される**。バッジは操作ではなく状態表示なので、「なぜこの期間だけ現在のスタッフ一覧と違うのか」が閲覧専用端末からは分からない。ボタンを隠すのは正しい（`periods` の書き込みはルールで拒否される）。
- **🟢（#87からの申し送り・確認済み）#87 で直した提出一覧の絞り込みは、重複表示を生まない**
  該当式（`:3068`）は `subs.filter(s=>…&&(!fn||s.staffName.includes(fn)||dispName(s).includes(fn))&&…)` ＝ **1つの `filter` に `||` を足した形**なので、生の別名と表示名の両方に部分一致する sub でも評価は1回・行も1回しか出ない。#87 が申し送った懸念は成立しない（変わるのは「件数が増えうる」ことだけで、それは修正の意図そのもの）。
- **🟢 `index.html` の `?v=` と `build:` が現在の配信物を指していない**（5本のscriptタグと `app-core.js:2` の6箇所は相互に一致しているが、いずれも `20260814-53f610f` のまま。**その版数以降 app-*.js は5ファイル・216行変更されている**）。申し送りのみ——`/release-to-main` の手順63がバンプする
- **🟡 #77 の宿題（Free 上限の実機検証）は静的監査完了・実測のみ未達**（上記🔴が理由。**残るは実機で21人目・2件目を押す1回だけ**）
- **🟢 Free のスタッフ上限が2箇所に別々の値（`??10` と `PLAN_LIMITS.free`=20）で書かれている**（#87）→ 本番では到達しない（`setPlan` の入口が値を絞る）ため変化なし
- **🟢 `parseTime` の受理範囲（0〜30時）と `gto()` の生成範囲（0:00〜27:00）の食い違い**（#87）／**`database.rules.tightened.json` が `database.rules.json` と完全同一**（#87）／**企業連携の他店舗データ取得だけが3ヶ月窓の外**（#87）→ いずれも変化なし
- **🟡 店舗間シフト重複の判定が退勤延長を見ていない**（#86）／**`dupErrors` だけが片側セルの日を対象外にしている**（#86）／**`x` セルが店舗間重複の判定から除外されていない**（#85）→ すべて BACKLOG化済み（仕様判断待ち・変化なし）
- **🟢 `effShiftStart`/`effShiftEnd` の未採用が app-admin.js になお3箇所**（`:1009` ヒートマップの時間軸／`:3055` 提出一覧の出勤日数／`:3113` 詳細モーダルの出勤日数）。**#82〜#87 と同じ理由で今回も触らなかった**——直し方が BACKLOG の判断待ち項目（片側セルだけの日を何時間と数えるか）に依存する
- **🟢 自由メモの先頭数字が時刻に食われる**（#86）／**略称lookupは大小文字を区別するが登録時の予約語チェックは区別しない**（#86）／**`addAllFromWeekday` と `handleSaveAll` のトーストが件数を多めに言う**（#81）→ 変化なし
- **🟡 スタッフURLを開いた誰でも他人の提出を削除できる**（#70）／**スタッフ削除の後始末がリネームと正反対**・**未登録名が画面に出ずExcel/PDFにだけ出る**（#79）／**期間の重複にバリデーションが無い**（#83）／**解約イベントだけが metadata でプラン判定**（functions/index.js:319）→ すべて BACKLOG化済み
- **🟡 コントラスト5件**（#72・#75・#76・#77）／**タップ領域44px未満**（#74）→ BACKLOG化済み・デザイン判断待ち（**今回も実測不能のため変化なし**）
- 以下は #46〜#87 からの継続（変化なし）: 未claim店舗の取り込み／`findActiveSubscription` の `|| live[0]`／`planExpiry` のUTC計算／**片側セルだけの日が0分**／`purgeOldPeriods` のUTC日付／`generateInviteCode` のデッドコード／ほか

### 今回の対象と見つけ方

**16回ぶりに「差分あり」の回だった。** #87 以降に `c889660`（別名の正規化）と `1c5272b`（期間の確定＝終了した期間のマスタ凍結）の2つの実装コミットが入っており、**#72〜#87 の15回が対象にしてきた「既存コードの棚卸し」ではなく、新しく入ったコードそのものを見る回**になった。

**(1) 新機能は「その機能が自分で宣言した目的」を物差しにした。** `1c5272b` のコミットメッセージには **「期間管理タブのExcelボタンも同じ凍結を通す（同じ期間のExcelが出す場所で変わらないように）」**と書いてある。つまり作者自身が「2つのExcelボタンは同じものを出すべきだ」という基準を明文化していた。そこで**その基準で2つの呼び出し元を1引数ずつ突き合わせた**ところ、名簿（`staffList`）と設定（`settings`）は揃えられているのに、**5番目の引数＝店舗名だけが揃っていなかった**。新機能のレビューでは「実装が正しいか」より先に**「その実装が掲げた基準を、実装自身が満たしているか」**を見るのが速い。

**(2) 決め手は、設定画面の説明文をコードの仕様書として使ったことだった。** 2つの呼び出し元が食い違っているとき、どちらが正しいかはコードだけでは決まらない（#86 の「多数派に揃える」も2対1でないと効かない。今回は1対1）。**`:3823` の「設定した名前はExcel出力時のファイル名・シート内店舗名に反映されます」という画面上の文言が、そのまま受け入れ条件になった**——`expXl` を読むと `shopName` はまさにファイル名（`:2288`）とシート内店舗名（`:2193`）の2つだけを決めており、**説明文と実装が1対1で対応していた**。片方がその約束を破っていた、という形に落ちたので仕様判断が要らなかった。

**(3) 「どちらが主要導線か」を確かめたことで優先度が上がった。** 最初は「2つのExcelの見た目が違うだけ」に見えたが、`resolver` の有無を追うと**期間タブ側は `_修正前`（提出そのまま）、シフト作成タブ側が調整済みの完成版**だと分かった。**設定が効いていなかったのは、実際に店舗へ配るシートの方だった。** 同じ「不一致」でも、壊れている側がどちらかで実害がまったく変わる。

**(4) 新機能のリスクは「動くか」より「保存形式」に出た。** `1c5272b` のロジック自体は素直で、`resolvePeriodMaster` の凍結・解除・写しの更新のいずれにも誤りを見つけられなかった（セキュリティルールが `snapshot` を受け付けるか＝`periods/$periodId` の `.validate` が `hasChildren(['id'])` だけであることも確認済み）。**問題は「写しを period の中に丸ごと置く」という保存形式のコストの方**で、Nodeで実測して初めて 1期間57KB・89%が `dateCandidates` という数字が出た。**新機能を見るときは、ロジックと保存形式を別々に当てる。**

**(5) 「直せそうに見えたが挙動が変わる」ことを確かめて、BACKLOGへ回した。** 89%を占める `dateCandidates` を期間の日付範囲ぶんに削るのは一見安全だが、**ヒートマップの補完境界が `Object.values(...).flat()` で期間外の日付も含めて最小/最大を取っている**ことを確認して、挙動が変わると判明した。**「削っても大丈夫」を検証せずに実装しなかったのが今回の分かれ目**で、これが仕様判断（凍結の粒度）としてBACKLOGへ回る理由になった。

### 検証したこと

- `DEV_MODE`（app-core.js:12・式のまま）／`DEV_PLAN_OVERRIDE`（:125）正常
- `subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／`global/shops` の全件読み **0件**／`global/templates` **0件**／CF `.delete()` **0件**／`database.rules.json` の `".read": true` **0件**
- 読み込み順維持（utils→core→staff→admin→main）・SRI **11本**
- `npm test` **174件パス**／`npx eslint app-*.js` **0 errors 98 warnings**（修正前後で増減なし）／`node --check functions/index.js` OK
- **新機能 `1c5272b` の監査（今回の主対象）**: ①props名のシャドウイング（`staffList:staffListProp`）が漏れていないか＝生のpropsを参照しているのは写し更新の useEffect と「確定」ボタンの2箇所のみで、本文は全て解決後の値（grep で全5箇所を確認）②`ShiftEditTab` に settings の保存経路が無い＝凍結した settings がライブの設定へ書き戻る事故が起きないこと（props に `saveSettings` 無し）③`periods/$periodId` の `.validate` が `hasChildren(['id'])` だけ＝`snapshot` の追加がルールで拒否されないこと ④写し更新が `ownerReadOnly` で止まること
- **写しの往復収束の再実測**（`sanitizeForSet` ＋ RTDBの「null値は保存されない・疎な配列はオブジェクト化」を再現）: 密な配列・空配列は**1回で収束**／穴あき配列は**収束せず書き込みループ**（上記🟢・現行経路では到達不能）
- **写しのサイズ実測**: スタッフ25名・日付別候補730日・休憩4区分の想定で 1期間 57,620 bytes、`dateCandidates` 系が 51,146 bytes（89%）、期間12/24/36件で 675KB/1.3MB/2.0MB
- **`c889660`（別名の正規化）の監査**: `existSub` の別名フォールバックが `source:"grid"` の下書きを「前回の提出」と誤認しないこと（`isFirstSubmission` が両経路に等しく効く・`:174`）、`sub.id` が `existSub.id` を再利用するため `onSub` の `findIndex(s=>s.id===sub.id)`（app-main.js:1517）が正規化後も旧エントリに当たること
- **修正の検証（配信物の式をNodeで実行）**: シート内店舗名セル式・ファイル名式を `xlShopName` の3状態（あり／空文字／未設定）で修正前後比較。**設定ありのときだけ変化し両ボタンが一致、空文字・未設定は完全同一**
- **#87 の申し送りの消化**: 提出一覧の絞り込み（`:3068`）が単一の `filter` ＋ `||` であり重複行を生まないことを確認
- **未検証（今回は手段が無い）**: ブラウザでの実描画すべて／**期間確定機能の実機E2E**（確定済みバッジの表示・確定/解除の往復・確定済み期間でのセル編集＝`1c5272b` の残課題そのもの）／**修正後のExcelを実際に開いてシート内店舗名とファイル名を目視**／**Free 上限に達した店舗での実ブロック**／実機 iOS Safari

### 総括

**測れない回が11回続いたが、今回は「新しく入ったコードを読む」対象があったため、棚卸しのレンズを回さずに 🟡 を1件見つけて直せた。** #72〜#87 の15回は差分ゼロで、毎回「既存コードの中から同じ形の非対称を探す」レンズを発明し続ける回だった。今回は `c889660` と `1c5272b` という実装コミットがあり、**新機能が自分で宣言した基準（「同じ期間のExcelが出す場所によって中身が変わらないように」）をそのまま検査項目として使えた。**

**見つかった不整合は、新機能が作ったものではなく、新機能が「揃えようとして揃え残した」ものだった。** `1c5272b` は2つのExcelボタンの名簿と設定を揃えたが、店舗名は元から食い違っていて、そのまま残った。**「揃える」というコミットは、揃え漏れが最も見つかりやすい場所でもある**——作者が全部の引数を見比べた直後なのに残っているものは、その視野の外にあったものだからで、実際 `xlShopName` は凍結の対象キーではない＝作者が見ていたリスト（`PERIOD_SNAPSHOT_SETTING_KEYS`）に載っていないキーだった。

**新機能のロジックには誤りを見つけられず、コストの方に問題が出た。** `resolvePeriodMaster` の凍結判定・解除・写しの更新の収束は、Nodeで往復を再現しても正しく動いた。代わりに実測で出たのは **1期間あたり57KBの写しが `periods` に載り、起動のたびに全件DLされる**という保存形式の話で、これは「データ保存上限②」で subs を3ヶ月に絞った努力と逆を向いている。**ロジックの正しさとデータの置き場所は別々に検査する必要がある**というのが今回の一番の収穫で、`npm test` も eslint もこの種の問題には一切反応しない（#72 のコントラストが目視レビューで落ちるのと同じ構造）。

**別名系列は今回、根の方に手が入った。** #81・#84・#85・#86・#87 と5回続けて別の場所に症状が出ていた「`sub.staffName` に別名が残る」問題は、`c889660` で**登録名へ正規化する**方向にユーザー判断が下り、`existSub` の別名フォールバックと `onSub` の id 引きが入った。監査した範囲では誤りは見つからなかったが、**これは実機E2E（別端末＝Cookieなしからの再提出）を通していない修正**で、しかもスタッフ提出パスは #51・#56・#58 の回帰がすべて起きた場所である。**11回止まっている実機確認の中で、最も優先度が高いのはここ。**

**次回の手を1つに絞る。** 実測が復活したなら、**#77 の宿題より先に `c889660` と `1c5272b` の実機E2Eを通す**（どちらも「Nodeでは実測したがブラウザでは一度も動かしていない」状態で develop に乗っており、片方はスタッフ提出パス、もう片方は期間データの構造を変える）。使えないままなら、推奨は**「新しく入ったコミットが、そのコミットメッセージで宣言した基準を自分で満たしているかを1項目ずつ照合する」**——今回はそれが唯一の当たりだった。あわせて `1c5272b` の残りの引数の食い違いとして、**PDF出力（`:1402`/`:1487`/`:1550`）が `shopName` を使い、Excelが `xlShopName` を使うようになった**ため、**同じ期間のExcelとPDFで店舗名が違う**状態になっている点を見ること（設定名と説明文の範囲からは仕様どおりだが、ユーザーから見て説明がつくかは別）。

**CF側の宿題（#68 の2つ目）は今回も持ち越し。**
...
- [ ] 実機E2Eで「別名で提出 → 管理者が別名登録 → 別端末から再提出」を通し、subが1件のままで管理者調整値が引き継がれることを確認する（**未実施**。バグチェック#78〜#87 の10回連続で `preview_start` が unattended session に拒否されているため。**対話セッションでの手動確認が要る**）
- [ ] Excel出力に本人の最新提出が出ることを確認する（**未実施**・上と同じ理由）
...
**残課題（実機E2E未実施）**: バグチェック#78〜#87 の10回連続で `preview_start` が unattended session に拒否されており、ブラウザでの確認ができていない。**対話セッションで次の3点を通す必要がある**——①終了済み期間で「確定済み」バッジが出て、スタッフを削除しても列が残ること ②「確定を解除」で現在値に戻ること ③確定済み期間でもセルの編集と保存ができること。
