...
## Shifty バグチェックレポート（2026-08-18 自動実行 #81）

### 修正済み

- **🟡 スタッフ削除の確認が、別名で出された提出を数え落としていた**（[app-admin.js:2332](app-admin.js)）→ `3a5b830`
  #79 が追加した確認文は `提出済みのシフト${sc}件は削除されません（提出一覧とExcelには残ります）。` と、**残るものの件数を自分から名指しして約束する**文だった。その `sc` が `subs.filter(s=>s.staffName===n&&…)` ＝**登録名の完全一致だけ**で数えていた。
  **`sub.staffName` には別名がそのまま入っている。** `registerAlias`（`:2969`）は `staffAliases` に登録するだけで `staffName` を書き換えないため、別名で出された提出は別名を名前に持ったまま残る。これは事故ではなく**別名機能の設計そのもの**（未登録名の提出を後から本人に紐付けるのが目的）なので、別名を使っている店舗では常にこの形になる。
  **配信物の述語をNodeで実行して実測**: 3件すべてが別名ぶんのとき **`sc` は 0** になり、**警告文が丸ごと消えて `「田中」を削除しますか？` だけになる**。その裏で提出一覧には「田中」の行が3つ並び（`:3010` が `resolveAlias` 済みの名前で表示する）、Excel も 3件を田中の列に出す。混在時は「1件」と出るが実際は2件だった。
  **断定できた根拠は、同じ述語を他の4箇所が既に別名込みで書いていたこと**——`app-staff.js:519`（未提出リスト）・`app-admin.js:2172`（Excel）・`:3069`（週間勤務時間）・`:552-559`（グリッド編集）。**5箇所のうちここだけが名前一致だった。**
  修正後、配信物から `del` を切り出して5ケース実行し、全提出が別名ぶん＝3件、混在＝2件、別名なし・grid由来・スペーサーの3つは非回帰であることを確認した。

### 要確認（未修正）

- **🟡 別名で提出した人が別端末から再提出すると、同一期間に2つのsubができ、Excelが古い方を出す** → **BACKLOG化済み**
  上の修正と根は同じ（`sub.staffName` に別名が残る）だが、症状も直し方も別。スタッフ画面の既存sub検索（[app-staff.js:158](app-staff.js)）が**登録名の完全一致だけ**で探すため、Cookieの無い端末から名前を打ち直すと `resolveAlias` が登録名を返し、**既存subが見つからず2つ目のsubが作られる**。
  **実測**: 提出一覧に「田中」の行が2つ並び、**Excel は古い方（退勤17:00）を出力して本人の最新提出（18:00）を黙って落とした**。管理者が旧subに入れた `adjustedStart` も引き継がれない。
  **さらに画面とExcelが別のsubを見る**: グリッド（`_getSubForPeriod`・`:501-510`）は**完全一致を優先**してから別名へ落ちるが、`expXl` の `find`（`:2172`）は**完全一致を優先せず配列順**で最初に条件へ合った要素を返す。`ss` は並べ替えていない＝Firebaseのキー順なので、**グリッドは18:00を表示しExcelは17:00を出す**。
  **正解はコードベース内にある**（`:552-559` が同じ問題を別名フォールバックで既に解決済み）が、**ループでは直さなかった**: ①`staffName` を正規化するかの判断が要り、正規化するなら `app-main.js:1517` の `findIndex` も同時に直さないと画面上だけ重複が増える ②**スタッフ提出パスは #51・#56・#58 の回帰が全て発生した場所**で、非破壊確認に実機E2Eが要る。
- **🔴（継続・プロセス）このループから実機E2Eが4回続けて実行できない**（環境要因・コード変更なし）
  稼働中サーバー0件を実行して確認。Bashでのサーバー起動は規約で禁止のため代替なし。**#72〜#77 の6回が柱にしてきたブラウザ実測が、#78・#79・#80 に続き今回も1件も実行できなかった。ユーザーが対話セッションで `/bug-check` を手動実行すれば解消する。**
- **🟡 #77 の宿題（Free 上限の実機検証）は静的監査完了・実測のみ未達**（上記🔴が理由）。**残るは実機で21人目・2件目を押す1回だけ**
- **🟢 `index.html` の `?v=` と `build:` が現在の配信物を指していない**（`20260814-53f610f` のまま）。申し送りのみ——`/release-to-main` の手順63がバンプする
- **🟢 `addAllFromWeekday`（`:2599`）と `handleSaveAll`（`:692`）のトーストが件数を多めに言う**。前者は実際に追加された `added` ではなく `wcands.length`×`selDates.length` を、後者は `isRestCommand` でスキップした分も含む `entries.length` を報告する。**データは正しく数字だけが実態より大きい**ため🟢。
...
- 以下は #46〜#80 からの継続（変化なし）: 未claim店舗の取り込み／`findActiveSubscription` の `|| live[0]`／`planExpiry` のUTC計算／片側セルだけの日が0分／`purgeOldPeriods` のUTC日付／`generateInviteCode` のデッドコード／ほか

### 今回の対象と見つけ方

**#80 以降のコミットは0件。9回続けて「差分ゼロ」の回である。**

**(1) 申し送りどおり「トーストのレンズ」を回したが、当たりはトーストではなく確認文の側だった。** `tt(` を97箇所すべて直前の処理と突き合わせた結果、**トースト固有の🔴🟡はゼロ**で、出たのは件数を多めに言う🟢が2件だけ。ただしこの一周の途中で「ユーザーに数字を約束している文」という共通点に気づき、そこから確認文の `sc` に到達した。**レンズは外れたが、回す過程で別の当たりが出た。**

**(2) 「同じ操作を正しく書いている箇所が同一ファイルにあるか」が4回連続で決め手だった。** 今回は**5箇所中4箇所が既に別名込みで数えていた**。**この判定法は今回、修正すべきかだけでなく「どう直すか」まで与えた**（多数派の式に揃えるだけで設計判断が一切要らなかった）。

**(3) 一度は誤検出を掴み、reachability を確認して取り下げた。** 当初「`addAlias`（`:2265`）が他スタッフに登録済みの別名を重複登録できる」と判断しかけたが、**別名の入力UIは自由入力ではなく `unregisteredNames` のチップだけ**で、そのリストは `!allAliases.includes(n)` で絞られているため**重複は作れない**。`addAlias` 内の2つのガードは到達不能な防御コードだった。**「コード上そう書ける」と「UIから到達できる」は別物で、後者を確認するまで報告しない。**

**(4) 2件目は「正解がコードベース内にある」のに、あえて直さずBACKLOGへ回した。** **#80 までの4回は「判断が要るか」で仕分けていたが、今回は「検証手段があるか」で仕分けた。**

**(5) 配信物をNodeで実行する手法が4回目の定着。** 述語の突き合わせ4本に加え、**修正後の `del` をファイルから行範囲で切り出して `eval` し5ケースを走らせた**。ブラウザ無しで「確認文の実際の文字列」を読めている。

### 検証したこと

- `DEV_MODE`（app-core.js:12・式のまま）／`DEV_PLAN_OVERRIDE`（:125）正常
- `subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／`global/shops` 全件読み **0件**／CF `.delete()` **0件**／CF secrets 抜けなし（6宣言）
...
- **トーストのレンズ**: `tt(` **97箇所**を直前の処理と突き合わせて1周（🟢2件のみ）
- **別名レンズの完全性**: 名前で提出を引く**9箇所**を列挙し、**7箇所が別名対応済み・1箇所を今回修正・1箇所（+ローカルstateの1箇所）をBACKLOG化**。レンズは閉じた
- **修正の検証（配信物の実評価）**: 修正後の `del` を5ケース実行
- **誤検出の棄却**: `addAlias` の重複登録がUIから到達不能であることを入力経路まで辿って確認
- **未検証（今回は手段が無い）**: ブラウザでの実描画すべて／**Free 上限に達した店舗での実ブロック**／今回変更した確認文の実表示／実機 iOS Safari

### 総括

**測れない回が4回続き、それでも 🟡 が2件出た。** どちらも根は1つ——**`sub.staffName` に別名が入ったまま残る**——で、#79・#80 が触った「破壊的操作の確認文」の延長線上にあった。

**「同じ操作の別実装を探す」が4回連続で機能し、今回は直し方まで与えた。** 5箇所中4箇所が正しく1箇所だけ違う、という形は**仕様書もユーザー判断も要らずに白黒がつく**うえ、多数派の式をそのまま採用すればよい。

**今回の収穫は、仕分けの基準を1つ増やしたこと。** **「正解が分かっていても、検証手段が無いなら触らない」**を適用した。スタッフ提出パスは過去3回（#51・#56・#58）の回帰の発生源で、実測できない回に触る場所ではない。

**別名レンズはここで閉じた**（9箇所を列挙し尽くした）。**次回の手を1つに絞る。** 実測が復活したなら**#77 の宿題をそのまま実行する**。使えないままなら、推奨は**「同じ意味の値を2つの式で出している箇所」を横断で洗う**こと——今回の当たりも #56 の件数の食い違いもすべてこの形である。`staffName` の9箇所は閉じたので、**次は日付・期間・時刻で引いている述語**を同じ方法で列挙する。
