...
## Shifty バグチェックレポート（2026-09-20 自動実行 #138）

> 着手時の HEAD は `27926bd`。#137 以降に入った app-*.js の変更は `40a88de`（変更マークの締切ゲート）の
> 1件だけだったので、いちばん新しいコードを最初に当たった。#137 の申し送り（Excel と PDF の**列の集合**）は
> 2件とも測って**否定**し、代わりに同じ「1つの事実を描く surface が複数ある」形を `40a88de` が
> 触ったばかりの場所で見つけた。

### 修正済み

- **[🟢] 締切ゲートのコメントが偽の不変条件を主張していた**（app-utils.js の `deadlineGatePassed`・`9c1f5d1`）
  `40a88de` のコメントは「提出一覧の変更ありバッジと緑セルの**両方がここを通る**」と書いており、
  2つの surface が一致すると読める。実測すると一致するのは**1度の提出の瞬間だけ**（下記🟡）。
  挙動は変えず、実測で分かった境界をコメントに書いた。**偽の不変条件は、次に読む人が「ここは揃っている」と
  信じて別の場所を探す原因になる**ので🟢だが放置しなかった（#103 の「全箇所解消済み」が2ヶ月偽だった件と同じ形）。

### 要確認（未修正）

- **[🟡] 変更マーク（緑セル）と提出一覧の「変更あり」バッジが、締切日の編集とセル編集の2経路でまた食い違う**
  → **BACKLOG化済み**（条件B＝仕様判断）。判定式は `deadlineGatePassed` に一本化されたが、**答えを出す時点が違う**。
  バッジは**読み取り時**に毎回計算し、緑セルは**提出した瞬間**の判定を `sub.shifts[日付].changed` に焼いて凍結する。

  **① 締切日を編集すると追随しない**（実測6ケース中4ケースで食い違う）。とくに **9/10→9/15 の延長**で
  「緑セルあり・一覧バッジなし」＝`40a88de` が直した症状そのものに戻る。締切の延長は
  「未提出者がいるので延ばす」という普通の運用で、`PEF`（app-admin.js:2158）からいつでも変えられる。

  **② スタッフが提出状況一覧のセル編集で変えると緑にならない**。`SmModal` の `applyCellEdit` は
  `updatedAt`/`isUpdated` を進めるのでバッジは出るが、**`changed` を立てる側の処理を持たない**。
  管理者は「締切後に誰かが変えた」と分かるのに、**どの日が変わったかがシフト表のどこにも出ない**。

  **ループで直さなかった理由**: 素直な直し方（緑セルも読み取り時ゲートにする）を当てると、
  **`changed` を共有している管理者の手動マーク（`toggleChanged`）まで締切内で消える**。
  手動か自動かを区別する印が `changed:true` に無く、先に「その区別を持つか」を決める必要がある。

- **#137 から継続（変化なし）**: 属性削除で休憩のタグが「誰にも当たらない行」として残る（🟢）／
  同じ開始日の期間が2つあると `moveStaffHiddenBoundaries` が境界を動かさない（🟢・意図的）／
  `dup`・`posErr` が画面限定である理由がPDFのコメントに無い（🟢）／部門別PDFで未登録名の列が落ちる理由のコメントが無い（🟢）／
  休み提出の日に斜線と黄色が両方乗る（🟢）／「締」が無効な店舗でもメモから「締」が消える（🟢）／「締」入りの略称（🟢）／
  店舗切替で `plan` が同期リセットされない（🟢）／`purgeInactiveShops` の招待コード後始末（🟢）／
  企業ログインの試行制限（🟡 → BACKLOG化済み）／`recentPeriodIds` がテスト専用（🟢）／
  `ShiftEditTab` の他店舗 `subs` 全件読み（🟢）／退勤を出勤より前に入れると集計が0になる（🟡 → BACKLOG化済み）。
- **変化なし（BACKLOG化済み）**: Cloud Functions の本番反映（3件）／二重課金の根治／特商法表記（🔴）／解約時のプラン判定／
  `verifyShopOwner` の移行猶予／別名提出の重複の根（#81）／PDFの実物確認／`purgeOldPeriods` の本有効化／
  非表示スタッフの未提出カウント／「締」の休みの入口2つ／完全削除したスタッフの `keepAttrs`。
- **引き受け済みのトレードオフ（再検出しても直さない）**: `subs/$subId/.write` は認証済みなら通る。2026-08-31 決定1。

### 当てて否定した仮説

- **#137 の申し送り①「部門フィルタを掛けた Excel と PDF の列」→ 前提が成り立たない**。`expXl` は
  **dept という引数を持たない**（画面の `deptFilter` は Excel に一切効かない）。PDF の dept は出力モーダルで
  別に選ぶもので画面のフィルタとは連動しない＝「掛けた状態」という状況自体が作れない。
- **#137 の申し送り②「`showSpacerDate`（35名以上）の境界」→ 否定（揃う）**。`dept="all"` の列は
  Excel・PDF とも `[...visibleStaffList(roster,settings,period), ...未登録名]` で、名簿・settings・期間・
  別名表・ソート規則（`localeCompare("ja")`）がすべて同じ入口から来る。判定式も両者とも「isSpacer でない列が34超」。
- **`delete nw.changed` が Firebase に届かない → 否定**（`diffSubForFlatWrite` は日オブジェクトごと書く）。
- **管理者の期間管理タブでのセル編集が「スタッフの再提出」になる → 否定**（app-admin.js:2009 は `isUpdated` を立てない・#59 で対処済み）。
- **スタッフ名キーの設定マップの登録漏れ → 否定**（`STAFF_KEYED_SETTING_MAPS` 7件＋`overtimeSettings.byStaff` で網羅）。
- **PDF の週間勤務時間だけ属性の既定値が違う → 否定**（組み込み属性は表示名しか持たず上限を持たないので同結果）。

### 検証したこと

- `npm test` **270件パス**・`npx eslint app-*.js` **0 errors / 95 warnings**（いずれも増減なし）。
  `node --check functions/index.js` 通過。今回の変更はコメントのみで挙動を変えていない。
- RULES.md スキャン: `DEV_MODE` は式のまま／`subs` の全体 `set()` 0件／`accounts` 全件読み 0件／SRI 11/11／
  読み込み順 utils→core→staff→admin→main／フォーム部品 **58件・`fontSize<16` は0件**／
  Cloud Functions の `.delete()` 誤用 0件・`secrets` の抜け漏れなし。
- 配信版数は **`20260920-40a88de`** で app-*.js の最後の変更を正しく指している（#137 の申し送りはリリース時のバンプで解消済み）。
- **Firebase・Stripe には一切アクセスしていない**（実測はすべて配信物の関数を Node で読み込んで実行）。

**申し送り（次回の観点）**: 今回効いたのは「**判定式を一本化しても、答えを出す時点（読み取り時／書き込み時）が
違えば surface はまた割れる**」という形。同じ形は他にもある——期間の写し（`snapshot`）は**書き込み時に凍結する側**で、
`resolvePeriodMaster` を通らない surface（提出一覧 `SubsTab` は `keepAttrs` だけ個別に当てている）は現在値で読む。
**確定済み期間について、シフト作成タブと提出一覧が同じ属性・退勤延長・従業員番号を見ているかはまだ誰も測っていない**。
`staffAttributes` 以外の凍結対象キーで突き合わせるとよい。
