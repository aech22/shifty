...
## 🟢 スタッフ削除時の「名前をどの期間まで残すか」の残作業（取り消し導線・配布物の実物確認）

**目的**: 2026-08-24 にスタッフ削除のポップアップと `period.keepStaff` を実装した（`95a3504` 機能本体・`2d33e1e` 位置保持・`c0f2fbd` 文言修正）。**機能は動いていて実ブラウザで検証済み**だが、ループ内で閉じられなかったものが3つ残る。

**残っているもの**:

1. **「残す」を後から取り消す導線が無い**（`keepStaff` を消すUIが無い）。ポップアップで誤って「残す」を選んでも、取り消すには**同じ名前でスタッフを登録し直してもう一度削除する**しかない。逆向き（「残さない」を選んだあとに残したくなった）も同じ。実害は「不要な列が1つ残る」だけで、データは失われない。**どこに置くかが判断どころ**——期間タブの各期間カード（そこに「確定/確定解除」がある）に並べるのが自然だが、削除済みスタッフのためだけにUIを1つ増やす価値があるかは運用次第。**案A: 期間タブの期間カードに「シフト表に残しているスタッフ」欄を出して個別に外せるようにする／案B: スタッフ一覧の下に「削除済みで表に残している人」の一覧を出す／案C: 現状維持（誤選択は登録し直して削除でやり直せる）**
2. **Excel・PDF のファイルを実際に開いての列確認が未実施**。グリッドの列が正しい位置に出ることは実ブラウザの `th` 実測で確認済みで、`expXl`/`buildPdfCols` は同じ解決後 `staffList` を受け取るためコード上は同じになるはず。ただし**ファイルを開いて見てはいない**。#81 から続く「Excel実物の確認」と同じ性質の宿題で、**まとめて1回やれば両方閉じる**。
3. **「確定済み（選ばなくても残ります）」ラベルの表示が未確認**。写し(`snapshot`)を持つ期間でのみ出る分岐で、ロジックはユニットテストで担保しているが実ブラウザで踏んでいない。

**受け入れ条件**:
- [ ] 1 の案A〜Cを選ぶ（**ユーザー判断**）。案A・案Bなら実装する
- [ ] 2: 「残す」を選んで削除したあとの Excel と PDF を実際に開き、**その人の列が元の位置に**（末尾ではなく）出ることを確認する。あわせて「残さない」を選んだ場合に**提出があれば末尾に未登録名として出る**ことも確認する
- [ ] 3: 写しを持つ期間でポップアップを開き、ラベルが出ることを確認する（`shifty-e2e-verify` 1.6節のハーネスで `snapshot` 入りの期間を渡せば足りる）

**影響範囲**: app-admin.js（`StaffTab` のポップアップ・案Aなら `PeriodsTab`）、app-utils.js（`mergeKeepStaff` は変更不要の見込み）
**備考**: バグチェック#93 と同じ日にユーザー依頼で実装した機能の申し送り・**1は条件B（UIをどこに置くかの仕様判断）、2は条件A（ファイルを開いて目視するのはユーザーの操作）に該当**。3はループでも閉じられるので次回の実装ループで拾ってよい。**機能自体は未完成ではない**——実ブラウザで「ポップアップが最新3期間のみを出す」「選んだ期間にだけ `keepStaff` が入る」「グリッドの列が元の位置のまま残る」「残さないと指定した期間では消える」をすべて確認済み（pageerror・console error 0件）。なお `keepStaff` は `periods` ノードに載るため、上の🟡「期間の確定（写し）が `periods` ノードを肥大化させる」と同じノードを太らせるが、**1人あたり数十バイト**なので写し（1期間57KB）に比べれば無視できる。
