...
## Shifty バグチェックレポート（2026-08-23 自動実行 #92）

### 修正済み

- **🟡 スタッフを編集中に別の行を削除すると、保存が「別人」の名前を書き換える**（[app-admin.js:2317](app-admin.js)）→ `f7a780f`
  `StaffTab` は「編集中」「別名パネル」「ポジションパネル」の対象を **`staffList` の index** で保持していた。パネルを開いたまま**別の行を削除する・並べ替える・他端末が staffList を変える**と、同じ index が別人を指すようになる。**編集欄は開いたまま別人の行へ移り、中身は前の人の名前のまま**なので、そこで保存すると `onRenameStaff` が**その別人**を書き換える。
  **被害の広さ**: リネームは名前1つの付け替えでは済まない。`onRenameStaff`（app-admin.js:186）は名前をキーに持つ**設定マップ7つ**（色・属性・従業員番号・ポジション・別名・勤務先・退勤延長）と **subs の `staffName`** をまとめて移す。つまり誤爆すると、無関係なスタッフの設定一式が別名義へ移動する。
  **実測（配信物の `StaffTab` を実ブラウザで単体マウント。Firebaseへは1バイトも書かない）**: `[田中,佐藤,鈴木,高橋,渡辺]` で**高橋**を編集モードにし、そのまま**田中**を削除すると、`"高橋"` が入った編集欄が**渡辺の行**へ移った（画面には**高橋が2つ並ぶ** — 通常表示の高橋と、編集欄の中の「高橋」）。ここで `高橋 太郎` に直して保存すると **`onRenameStaff("渡辺","高橋 太郎")`**。**修正後は同じ操作で編集欄が高橋に追従し `onRenameStaff("高橋","高橋 太郎")` になる。** いずれも pageerror 0件。
  **無変更で保存した場合だけは既存のガードが拾う**: `staffList.includes(trimmed)` の重複チェックに当たって「▲ 既に登録されている名前です」で止まる。**止まらないのは「実際に新しい名前へ変えたとき」＝本来の使い方をしたときだけ**、という形になっていた。
  **修正**: 対象を index ではなく**スタッフ名**で持つ（`editKey`/`aliasKey`/`posKey`）。名前は `add`・`confirmEdit` が重複を禁止し、空白列も `"__spacer__"+genToken()` で一意なのでキーにできる。削除された人のパネルは行ごと消え、並べ替えではパネルが本人に追従する。
  **非回帰（実測）**: ①通常のリネーム（一覧を触らずに編集→保存）は従来どおり `onRenameStaff("鈴木","鈴木 一郎")`。②別名パネルを開いたまま上の行を削除しても、パネルは**高橋に付いたまま**で高橋の別名「たかはし」を表示し続ける。③dev標準テスト店舗の実データで管理者画面→スタッフタブを開き、編集・別名・ポジションの3パネルが**開閉とも正常**・コンソールエラー0件。

### 要確認（未修正）

- **🟢 休憩タグの編集も同じ「indexで持っている」形**（[app-admin.js:2600](app-admin.js) `editTagKey` = `"dayType_index"`）。上の件と同じ構造だが、休憩リストを変えるのも編集するのも**同じパネルの中にいる同じ操作者**で、他端末からの変更で並びが動く余地が実質ない。今回は触っていない。
- **🟢 `selDates` を render 中に破壊的ソートしている**（[app-admin.js:2829](app-admin.js) `selDates.sort().map(...)`）。state の配列を setState を通さず並べ替えている。表示は日付順が正しいので**見た目の実害はない**が、`selDates[0]`（:2705 / :2838 の「登録済み候補」表示）が「最初に選んだ日」ではなく「最も早い日」になる。
- **🟢 スタッフ用の `PLAN_LIMITS` フォールバックだけが Free の実値と食い違う**（[app-admin.js:2357](app-admin.js) の `PLAN_LIMITS[plan]?.staff??10`）。#91 から継続・変化なし。`PLAN_LIMITS.free.staff` は **20** なのにフォールバックは **10**。`plan` state の初期値が `"free"` なので現状このフォールバックには到達せず**実害はない**が、Firebase が想定外のプラン名を返した瞬間にスタッフ上限が黙って半分になる。**到達経路が現存しない＝「原因だった証拠」を示せない**ため今回も触っていない。
- **🟢 期間の日付に検証がない**（[app-admin.js:1901](app-admin.js) `create` / [:2052](app-admin.js) `PEF`）→ **BACKLOG化済み**（既存の「期間どうしの日付の重なりを許すか」へ追記）。終了日<開始日で「0日間」の期間が作れ、作成フォームの日数プレビューは `startDate<=endDate` のときしか出ないため**警告も出ない**。編集で日付を空にすると `recentPeriodIds` の窓から外れ、**アクティブ期間でない限りその期間の提出が購読対象から落ちる**。判断の余地がない検証漏れだが、直す場所が重なり判定と同じ関数なので同時に入れるのが自然。
- **🟢 CFの `linkStoreToCompany` が shopId の文字種を検証していない**（#90 から継続・変化なし）。CF側は `async` ハンドラなので rejected promise になり固まらない。**修正には CF のデプロイ（ユーザー操作）が要る**ため申し送り。
- **🟢 `name|date` 複合キーの分解が2箇所で食い違う**（[app-admin.js:1641](app-admin.js) `indexOf` / [:3073](app-admin.js) `lastIndexOf`）。#90 から継続・変化なし。
- **🟢 `effShiftStart`/`effShiftEnd` の未採用が app-admin.js になお3箇所**（`:1031` / `:3120` / `:3178`）。**#82〜#91 と同じ理由で今回も触っていない**——直し方が BACKLOG の判断待ち項目（片側セルだけの日を何時間と数えるか）に依存する。
- **🟢 配信版数（`?v=` と `build:`）は6箇所とも `20260822-e9d98a4` で一致しているが、今回 app-admin.js が変わった**ため次のリリースでバンプが要る。申し送りのみ（`/release-to-main` の手順63が標準工程として持つ。BACKLOG化はしない）。
- 以下は変化なし・すべて BACKLOG化済み: **コントラスト5件**（#72・#75・#76・#77・#91でベースライン取得済み）／**タップ領域44px未満**（#74）／**休憩の適用条件が画面の説明文と食い違う**（#89）／**期間の確定（写し）が `periods` を肥大化させる**（#88）／**別名で提出した人の再提出でsubが2つできる**（#81・コード修正済み・Excel実物の確認と本番データ確認が残）／**スタッフURLを開いた誰でも他人の提出を削除できる**（#70）／**スタッフ削除の後始末がリネームと正反対**・**未登録名が画面に出ずExcel/PDFにだけ出る**（#79）／**`x` セルが店舗間重複の判定から除外されていない**（#85）／**店舗間重複の判定が退勤延長と片側セルを見ていない**（#86）／**解約イベントだけが metadata でプラン判定**（#68）／**企業連携の解除が店舗をオーナー0人に戻す**（#65）／**Pro→Premium の二重課金の根治**・**特商法表記**（BACKLOG 🔴）
- 以下は #46〜#91 からの継続（変化なし）: 未claim店舗の取り込み／`findActiveSubscription` の `|| live[0]`／`planExpiry` のUTC計算／`purgeOldPeriods` のUTC日付／`generateInviteCode` のデッドコード／ほか

### 今回の対象と見つけ方

**#91 が名指しで残したレンズ（「保険・タイムアウト・リトライが書かれているのに救えない経路」）から入ったが、そこは空振りだった。** `.catch(()=>{})` の8箇所と `try{}catch{}` で `console.warn` だけの初期化4箇所を全部辿ったが、いずれも「握り潰してよい理由がコメントに書かれている」か「ルールで拒否されることが分かっている書き込み」で、救うべきものを落としている経路は無かった。`readShop` のガード（#91）と `firebaseKeyForbiddenChars` の入口3経路（#89・#90）も再確認して非回帰。

**(1) レンズを「保険」から「状態の同一性」へ回したら出た。** #89〜#91 は3回続けて「文字列がFirebaseのパスになる」経路だった。今回もその延長で `ref()` を追ったが、そこは既に塞がっている。そこで**「UIが何を指しているつもりか」と「実際に何を指すか」がズレる場所**を探した。React の state に**リストのindexを保存している箇所**を全数（`editIdx`/`aliasIdx`/`posIdx`/`dragIdx`/`dragOverIdx`/`editTagKey`）洗い出し、**そのリストが自分以外の理由で変わりうるか**を1つずつ見た。`staffList` は削除・並べ替え・他端末の変更・Firebaseの購読echoで動く。他は動かないか、動く前に操作が終わる。

**(2) 「見た目が壊れるだけ」と「別人を書き換える」の分かれ目は、値がどこから来るかだった。** 別名パネル・ポジションパネルは、開いたまま行がズレても**表示内容が `n`（その行の人）から読み直される**ので、画面は常に自己整合する（別人のパネルが開いて見えるだけ）。ところが編集欄だけは**`editName` という別のstateに前の人の名前を持っている**。だから行がズレた瞬間に「行＝渡辺／中身＝高橋」という**画面自身が矛盾した状態**になり、保存でその矛盾がデータへ流れ込む。**同じ構造のズレでも、参照を持ち越すか値を持ち越すかで、UIの乱れで済むかデータ破壊になるかが変わる。**

**(3) 既存のガードが「本来の使い方のときだけ」効かない形になっていた。** 何も直さず保存すると重複名チェックに当たって止まる。止まらないのは新しい名前へ実際に変えたときだけ。**「一見ガードがある」ことが、この経路を今まで見えにくくしていた**と思う（重複エラーが出た人は「変な挙動だった」で終わり、実害の出る経路は静かに通る）。

**(4) 検証は「アプリ全体を起動する」ではなく「その部品だけ実ブラウザでマウントする」でやった。** #91 で確立した `route.fulfill` で配信物を供給する手口の応用で、`window.StaffTab` を自前の親コンポーネントから直接 render し、`onSave`/`onRenameStaff` をローカルのスパイに差し替えた。**実ブラウザ・実React・実コンポーネントのまま、Firebaseへの書き込みだけが起きない。** そのおかげで dev の標準テスト店舗を汚さずに「スタッフを削除する」操作を何度でも試せた。**破壊的な操作を含む再現には、環境を用意するのではなく副作用の出口だけ差し替えるほうが速くて安全。**
...
- 読み込み順維持（utils→core→staff→admin→main）・SRI **11本**／新規追加した `input` **0件**（既存の `fontSize:16` を1箇所そのまま維持）
...
- **旧 `editIdx`/`aliasIdx`/`posIdx` の残存参照 0件**（改名後に全ファイル grep で確認）
- **実ブラウザでの実測3本**（devサーバー無し・`route.fulfill` で配信物を供給。いずれも pageerror・console error **0件**）: ①修正前の再現（別人がリネームされる）②修正後の同一手順（本人がリネームされる）＋通常リネームの非回帰＋別名パネルの追従 ③dev標準テスト店舗の実データで管理者画面→スタッフタブの3パネル開閉。**②③はFirebaseへ書き込みを行っていない**（②はスパイ、③は開閉のみ）
- **未検証**: iOS Safari 実機（WebKitでの再測定は今回未実施・Chromiumのみ）／並べ替え（ドラッグ）中にパネルが追従することの実操作確認（コード上は名前キーなので追従するが、ポインタ操作での実測はしていない）／他端末から `staffList` が変わるケースの実測（今回は同一ページ内の削除で再現した）／Excel・PDF ファイルを実際に開いての中身確認／本番データの確認（ユーザー操作）

### 総括

**#91 が残したレンズは空振りで、代わりに「UIが指しているつもりの対象と、実際に指す対象がズレる」という別の軸で1件出た。**

**今回の分かれ目は「Firebaseのパスを追うのをやめた」こと。** #89・#90・#91 が3回続けて同じ根（文字列→Firebaseキー）だったので、4件目もそこにあると思って探し始めた。実際には書き込み側も読み取り側も既に塞がっており、**同じ鉱脈を4回目も掘るのは効率が悪い**と判断してレンズを変えたのが当たりだった。**同じ種類のバグが3回続いたら、それは「もう掘り尽くした」の合図でもある。**

**「参照で持つか、値で持つか」がデータ破壊の境目になっていた。** 別名・ポジションのパネルは行がズレても表示が自己整合するので、実害はUIの混乱で止まる。編集欄だけが `editName` という**独立したstateに前の対象の値を持ち越して**いたために、画面自身が矛盾したまま保存できてしまった。**「開いたまま対象が変わりうるUI」を疑うときは、開いている間に何を持ち越しているかを見るとよい。**

**ガードの存在が発見を遅らせることがある。** 無変更保存は重複名チェックで止まるので、この経路を偶然踏んだ人は「よく分からないエラーが出た」で終わる。実際に壊れるのは正しく使ったときだけで、**エラーが出る側の経路が、エラーの出ない側の経路を隠していた**。#91 の「保険があるのに効いていない」と裏返しの形（今回は「保険が効く経路だけが目立ち、効かない経路が見えない」）。

**次回の手。** 実測手段が2種類（アプリ全体起動／部品だけマウント）そろったので、据え置きを続けて片付けるのが効率的。優先順は ①**タップ領域44px の実測**（#74・#91 でも未実施。手順はコントラストと同じで `getBoundingClientRect` に替えるだけ）②**WebKit での再測定**（ユーザーの主戦場は iOS Safari だが #91・#92 とも Chromium のみ）③**スタッフ20名超の上限UI**（今回の「部品だけマウント」なら **devデータを汚さずに測れる**——`staffList` を25名にしたpropsで `StaffTab` を単体マウントすればよい。#91 が「作り方を決めてから」と保留した障害はもう無い）。
コード側の新しいレンズを1つ挙げるなら、**「操作の開始時に控えた値と、確定時に読む値が別物になっている場所」を数える**。今回の `editName`（開始時の控え）と `staffList[i]`（確定時の読み）がそうだったように、**開始と確定の間に第三者（他端末・他の操作・購読echo）が挟まれるUIは Shifty に他にもある**（シフト作成グリッドの `localEdits`/`heatEdits` はキーが `name|date|field` で名前を含むため同種のズレが起きうる。スタッフ画面の `sd` も提出ボタンを押すまで保持される）。
