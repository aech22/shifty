...
## Shifty バグチェックレポート（2026-08-05 自動実行 #56）

### 修正済み

- **[🟡] 期間管理タブの「提出：N件」が、管理者自身のシフト作成タブ入力（`source:"grid"`）を提出として数えていた**（app-admin.js:1921 → 修正後 :1925 `PeriodsTab`／コミット `c49ab14`）
  管理者がシフト作成タブで**まだ提出していないスタッフのセルに時刻・メモ・休み希望(`y`)を入力すると、その場で `source:"grid"` の sub が新規作成される**（`applyEditToSubs` の `idx===-1` 分岐・app-admin.js:554／:563）。これは「スタッフURL経由の提出」ではなく管理者の下書きなので、提出件数に数えてはいけない。コード側もそれを**明示的に宣言している** — :561-562 に「**シフト作成タブから直接新規作成したsubはsource:"grid"を付与する。提出一覧(SubsTab)はスタッフURL経由の提出のみを表示するため、この印で除外する**」とコメントがある。
  **この不変条件は「その期間に何件提出されたか」を答える4経路のうち3つで守られていた**: スタッフ画面の提出状況バッジ（app-staff.js:430）・提出状況一覧の「提出済み N名」（app-staff.js:505）・提出一覧タブの行（app-admin.js:2928 `SubsTab`）はいずれも `s.source!=="grid"` を条件に含む。**期間管理タブの期間カードだけが `subs.filter(s=>s.periodId===p.id).length` と素通しだった**。
  **同じカードの中で矛盾する**のが症状として最悪の形だった。期間カードは `onClick` でそのまま提出状況ビュー（`viewPeriodId` → `SmModal`・app-admin.js:1835）を開く。つまり**管理者がカードで「提出：5件」を見て、そのカードをタップすると、開いた画面が「提出済み 3名」「未提出（2名）：高橋、伊藤」と答える**。同一タブ・同一クリックで数字が食い違う。
  **実測検証**（4経路の条件式を逐語コピーした Node 再現）: 登録5名・スタッフURLからの提出3名・管理者がグリッドで2名分のセルに入力した状態で、修正前は **A バッジ=3 / B SmModal=3 / C 提出一覧=3 / D 期間カード=5** と D だけが乖離した。修正後は D=3 で A/B/C と一致することを再実行で確認した。
  **修正方針**: 該当行の条件に `&&s.source!=="grid"` を足しただけ。**Excel出力（`expXl`・:1973 の `subs.filter(s=>s.periodId===p.id)`）は意図的に据え置いた** — Excelは管理者がグリッドで組んだシフトを配布するためのものなので、grid由来のsubも出力対象であるのが正しい。同じ `subs.filter(s=>s.periodId===p.id)` という式でも、問いが「提出は何件か」なのか「出力すべきシフトは何件か」なのかで正解が違う。
  **影響**: この件数は管理者が「全員出したか」を判断するために見る数字で、締切後に提出を締めてシフトを組み始める判断に直結する。管理者がグリッドで下書きを始めると**自分の入力が提出件数を押し上げる**ため、**入力すればするほど「全員提出済み」に近づいて見える**。「20名中20件」を見て未提出者への催促をやめると、実際には出していないスタッフの希望が反映されないままシフトが確定する。しかも下書きを始めるのは締切後＝催促の最後のタイミングであり、**一番間違えてはいけない瞬間にだけ数字がずれる**。

### 今回の対象と見つけ方

**前回（#55）以降のコード変更は #55 の修正コミット自身のみ**（`3dc2c72` と同期コミット `cea8b80`）。実質的に新規差分ゼロの回が **7回連続**。今回は #55 の総括が指定した手法をそのまま適用した — **「過去に直した関数について『この関数を経由せずに同じデータを書き込む/読み出す経路はいくつあるか』を数え、修正がその全部に効いているかを確かめる」**。

出発点は #55 で直した `applyCellEdit`（app-staff.js:512）が属する `SmModal` にした。`SmModal` の呼び出し元を数えると**3箇所**あり、うち2つは StaffView（提出前:248／提出後:227）だが、**3つ目は app-admin.js:1835 の PeriodsTab** だった。つまり `SmModal` は**スタッフ画面のコンポーネントでありながら管理者の期間管理タブでも同じ画面として使われている**。そこで「この2画面は同じ問いに同じ答えを返すか」を照合したところ、`SmModal` 自身は `source!=="grid"` を除外しているのに、**それを呼び出している側の期間カードが除外していなかった**。

決め手は #50 以来 **7回連続**で効いている**コメント読み**。今回当たったのは #55 と同じ「防御コメント」型 — app-admin.js:561-562 の「提出一覧はスタッフURL経由の提出のみを表示するため、この印で除外する」は、`source:"grid"` という印の**存在理由そのものを宣言している**。**印を導入したコメントを見つけたら、その印を見るべき箇所を全部数える**という使い方になる。`source:"grid"` の全出現は5箇所（作成2・除外3）で、**除外すべきなのに数えていなかった4箇所目**が grep 一発で出た。

### 検証したこと

- **「その期間の提出件数」を答える経路の全数え上げ**: 5経路（app-staff.js:430 バッジ／app-staff.js:505 SmModal／app-admin.js:2928 SubsTab／app-admin.js:1921 期間カード／app-admin.js:1973 expXl）。`source:"grid"` を除外するのは修正前で3経路、**除外すべきなのにしていなかったのが :1921**、**除外しないのが正しいのが :1973（Excel）**
- **`source:"grid"` の全出現の突き合わせ**: 生成2箇所（:554 休み希望のみのsub／:563 時刻・メモのsub）と除外3箇所。生成側はどちらも `idx===-1`（そのスタッフのsubが登録名でも別名でも見つからない）のときだけ通る＝**既存の提出に `source:"grid"` が後付けされることはない**ことを確認した
- **`SmModal` の呼び出し元の全数え上げ**: 3箇所（app-staff.js:227・:248・app-admin.js:1835）。管理者側からも同じコンポーネントが開くため、修正しなければ**同一タブ内で矛盾する**ことを確認した
- **`削除` 経路の非回帰**: 期間カードから開く SmModal の `onDeleteSub`（:1835）は `saveSubs(a, subId)` と第2引数を渡しており、grid subを削除しても Firebase から消える。今回の修正は表示条件のみで書き込み経路には触れていない
- **`diffSubForFlatWrite` が削除キーを実際に消すことの再確認**（#55 の修正が依存している前提）: app-utils.js:578 は `out[id+"/shifts/"+date] = shifts[date]` と**日付ノードごと**書くため、`update()` でその日付の下は丸ごと置換される＝`delete` したキーは消える。#55 の記述どおりであることをコードで確認した
- **プラン上限まわりの誤報チェック（新規）**: app-admin.js:1851 の「期間追加はProプランで利用できます」は `PLAN_LIMITS.free.periods` をハードコードしているが、**:1849 で `plan==="free"` のブロック内に閉じている**ため誤報にならない（Pro/Premiumには表示されない）。誤検知として除外した
...
- `subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／`global/shops` 全件読み **0件**（直キー読みのみ）
...
- 読み込み順 utils→core→staff→admin→main 維持・今回の差分に `fontSize<16` の新規混入 **0件**（変更は表示条件1式とコメントのみ）

### 要確認（未修正）

- **🟡（次リリースで必須・#51〜#54 分は解消済み）index.html のキャッシュ版数を必ずバンプすること**。**#51〜#54 を含むリリース `92dc08a` が 2026-08-05 に main へ入り、本番の版数は `20260805-78e8888` にバンプ済み**であることを `git show origin/main:index.html` で確認した（#51〜#55 の各レポートが「6回連続で引き継いでいる」と書いていた滞留は、ここで解消している）。develop 側の index.html が `20260802-94ffc47` のままなのは、release-to-main が main 側でのみ版数を上げる運用どおりで異常ではない。**現時点の本番未反映は `origin/main..develop` の4コミット（うち fix 2件＝#55 の提出状況一覧セル・#56 の提出件数）**。次のリリースで版数を上げ忘れると、app-staff.js・app-admin.js の実挙動が変わっているためブラウザキャッシュで旧ファイルが使われ、**今回の提出件数の誤表示も #55 の休み希望の取りこぼしも直っていない状態のまま**配信される。
- **🟢（新規）マイページの「スタッフ数」使用量バーが空白列（スペーサー）を人数に数える**（app-admin.js:3957 `<Bar used={staffList.length} .../>`）。同じ「登録スタッフ数」を答える他の3箇所 — 上限判定（:2274）・スタッフタブの「N/20名」表示（:2327）・上限警告（:2441）— はいずれも `staffList.filter(n=>!isSpacer(n)).length` でスペーサーを除いている。**バーだけが素の `staffList.length`**。空白列の追加は `isPro` ゲート（:2440）なのでFreeプランでは通常発生しないが、**Proでスペーサーを追加した店舗が解約してFreeに落ちると**、スタッフタブが「20/20名」と言う横でマイページが「21 / 20」を**赤字（`isOver`）で**表示する。しかもその表示は「Proにアップグレード（500円/月）」ボタンの真上にある。上限の**実施**（:2274）は正しい側の数え方をしているため機能的な実害はなく、発生条件も解約経路に限られるため🟢とした。
- **🟢（継続）SubsTab の「出勤」日数が、管理者が両セルを空欄化した日を1日として数える**（app-admin.js:2957・:3004 の2箇所）。`att` の計算は `(st&&en)||(extraStart&&extraEnd)` が偽のとき `shiftBandInfo` を通さず **`:1`（＝1日）にフォールバック**する。フォールバックの `:1` は「時刻が欠けた提出日は1日とみなす」意図と読めるので、**仕様判断が要る**（#52からの継続）。
- **🟢（継続）属性を削除しても休憩のタグ（`breakTimes[].tags`）に残る**（app-admin.js:3423 `deleteType`）。残ったタグはどのスタッフの属性とも一致しないため、その休憩は**誰にも適用されなくなる**。ポジション削除が同名カスケードを実装しているのと非対称。
- **🟢（継続）Excel出力とシフト作成タブで別名提出の解決順が違う**（app-admin.js:2133 `expXl` の `ss.find`）。`expXl` は `s.staffName===nm||aliases.includes(...)` の**配列順の先頭**を採る一方、`_getSubForPeriod`（:489）と `applyEditToSubs`（:545）は**完全一致を優先**してから別名を探す。:486 のコメントは「expXlと同じ別名解決」と宣言しているが**実際には揃っていない**。登録名と別名の両方で提出が存在する場合にだけ食い違い、そのとき**管理者がグリッドで編集した内容と Excel が印字する内容が別のsubになる**。
- **🟢（継続）期間管理タブの Excel 出力だけが管理者調整値を反映しない**（app-admin.js:1929 の resolver なし呼び出し）。ファイル名が `_修正前` になる仕様であり意図的な使い分けのため未修正。
- **🟢（継続）ヒートマップの時間列収集が `adminRest` を無視**（app-admin.js:980）。列見出しの範囲決めのみで人数カウントは正しく除外。実害は余分な列が1本出る程度。
- **🟢（継続）店舗間シフト重複エラーが片側入力と他店舗の `adminRest` を見ない**（app-admin.js:825 `dupErrors`）。#53 で**意図的に据え置いた**。店舗跨ぎは #47 以降 対象外方針。
...
- **🟢（継続）スタッフ削除時に設定マップのキーが残る**（app-admin.js `deleteStaff` 付近。改名側は :176 `renameKey`）。改名側（`renameKey`）は7マップ全て移し替える実装になっているのと非対称。
...
- **🟢（継続）実機E2E未実施**: 「曜日別から選ぶ」UI／#46 削除操作／#47 改名修正／#51 のスタッフ再提出／#52 の Excel 締めセル／#53 の片側入力／#54 の休み提出日メモ／#55 の提出状況一覧セル。**今回の修正も実機E2Eは未実施**（5経路の条件式を逐語コピーした Node 再現で一致を確認したが、実際にグリッドで未提出スタッフのセルに入力してから期間管理タブの件数が増えないことの目視確認は未実施）。

### 総括

**7回連続で、実質コード変更ゼロの回に構造から 🟡 を1件見つけた**。#50「フラグの解釈がレイヤーをまたいで配られていなかった」→ #51「引き継ぐべきフィールドの一覧がハードコードされ後発分が載らなかった」→ #52「同じ表示を組み立てる経路が3つあり後発フィールドが1つにだけ配線されていなかった」→ #53「同じ問いに答える判定が5つあり片側入力の扱いが1つにしか実装されていなかった」→ #54「同じ表示を決める経路が3つあり、1つだけが他の2つが使わない入力でブランチを切っていた」→ #55「同じ操作を行う経路が4つあり、1つだけが不変条件を破って書き込んでいた」→ #56「**同じ問いに答える経路が5つあり、1つだけが除外印を見ていなかった**」。7本目も同じ根 —「**ある扱いが、それを必要とする全箇所に届いていない**」— から生えている。

今回はっきりしたのは、**同じ式が正しい箇所と間違っている箇所の両方に出る**ということ。`subs.filter(s=>s.periodId===p.id)` は期間カード（:1925）では**バグ**で expXl（:1973）では**正しい**。式が同じでも**問いが違う**からで、前者は「何件提出されたか」、後者は「何件出力するか」を訊いている。だから「同じ式を全部揃える」という直し方は誤りで、**箇所ごとに何を訊いているかを言葉にしてから揃える対象を決める**必要があった。#52〜#55 は「揃っていないから揃える」で済んだが、今回は**揃えてはいけない箇所が混ざっていた**のが違いである。**次回以降はこれを使う — 同一の式が複数箇所に出ていたら、まず各箇所の「問い」を1文で書き出し、問いが同じもの同士だけを突き合わせる**。問いを書かずに式だけを見比べると、正しい箇所まで巻き込んで壊す。

**コメント読みは7回連続で決め手になった**。#50〜#54 は「揃えた」と主張するコメント（宣言と実装のズレ）、#55 は「この状態は作らない」という防御コメント、今回は**「この印を付ける／この印で除外する」という印の導入コメント**だった。3種類とも共通するのは、**実装者がその場で気づいた不変条件を、その場にしか書き残していない**という点である。印・フラグ・サフィックスのような「後で誰かが見る前提のマーカー」を導入するコメントを見つけたら、**そのマーカーを見るべき箇所を grep で全部数える**のが最短で、今回はそれで4箇所目が一発で出た。

壊れ方は #53 と同じ**誤報**だが、方向が逆である。#53 は「不足していないのに不足と出る」＝**過剰な警告**、#55 は「休みにしたのに出勤に見える」＝**意思の消失**だったのに対し、今回は「**まだ出ていないのに出ていることになる**」＝**欠けているものが見えなくなる**。しかも管理者が下書きを進めるほど数字が「揃って」いくため、**作業が進むほど問題が見えなくなる**方向に誤る。#54 以来の「気づかれないまま情報が伝わらない」系の害だが、今回は**誰も気づけないどころか、進捗しているという逆の確信を与える**点で質が悪い。管理者が自分で作ったデータを、自分の進捗を測る指標に混ぜてしまう構造は、他の集計（出勤日数・勤務時間・ヒートマップ）にも同じ形で潜みうるので、次回は「**その数字は自分の入力を含んでいないか**」という問いで集計系を一巡すると良い。
