...
## Shifty バグチェックレポート（2026-09-04 自動実行 #109・同日2回目）

> 着手時の HEAD は `202f8b9`（同日朝の #108）。**作業ツリーに、自分のものでない未コミットの
> 変更が5ファイル分あった**（mtime 19:31〜19:32）。実行中のセッションは無く、編集元は
> cwd がホームのセッションで、**そのため本リポジトリの自動コミットフックが発火していなかった**。
> 今回はまずこれを保全することから始め、そのうえで #108 の主題（正本を作ったら既存の入口を
> 全部そこへ寄せる）を、その未コミット変更が新しく作った正本に対して当てた。

### 修正済み

- **🟡 期限付き削除で名前を残した期間で、本人の提出だけが「未登録」扱いになっていた**（[app-staff.js:242](app-staff.js) ほか）→ `74da454`

  `keepStaff` で「この期間まで残す」と指定した人は `staffList` から消えて `period.keepStaff` に
  だけ載る。シフト作成グリッド・Excel・PDF は `resolvePeriodMaster` 経由で**マージ済み名簿**を
  受け取るのに、提出一覧の「別名を登録」バッジ・スタッフタブの未登録名・スタッフ画面の名前
  サジェストの3経路は生の `staffList` を見ていた。判定を `isUnregisteredSubName`（app-utils.js:890）
  へ一本化する内容で、**この修正自体は別セッションの作業**。内容を確認したうえでコミットした。

  **実ブラウザでのA/B実測**（SubsTab だけをマウントし `onSave` をスパイに差し替え＝Firebase へは
  1バイトも出していない。`staffList=["田中","鈴木"]`・9月前半の `keepStaff` に佐藤）:

  | 行 | 修正前（`202f8b9`） | 修正後 |
  |---|---|---|
  | 佐藤（残すと指定した人） | 「別名を登録」が**出る** | **出ない** |
  | 山田（本当に未登録） | 出る | 出る（非回帰） |
  | 田中（登録済み） | 出ない | 出ない |

  pageerror・console.error ともに両方 0件。

  **保全のしかたにも理由がある**。この Stop フックは `app-*.js` の5ファイルしか `git add` しない。
  放っておくと `eslint.config.js`（新関数のグローバル宣言）と `tests/core.test.js` が取り残された
  **部分コミットが push される**。実測（HEAD の worktree に作業ツリーの app-*.js だけを置いて計測）で
  **`no-undef` 2件**——`isUnregisteredSubName is not defined`——になり、CI の lint が落ちる状態だった。
  5ファイルを揃えて1コミットにしてある。

- **🟢→予防 正本ができた直後に、Excel と PDF の2入口が直書きのまま残っていた**（[app-admin.js:1355](app-admin.js)）→ `04c4049`

  上の `74da454` は「提出された名前が名簿にも別名にも無いか」の正本を作り、**3経路**を通した。
  ところが同じ判定をしている**残り2経路**——Excel（`expXl`）と PDF（`buildPdfCols`）の列構成——は
  `!staffList.includes(n)&&!isSpacer(n)&&!allAliases.includes(n)` の直書きのまま残っていた。

  **現時点の挙動は変わらない**。どちらも `resolvePeriodMaster` 経由のマージ済み名簿を受け取るためで、
  差分テストでも**マージ済み名簿20,000ケースで不一致0件**（空文字の名前だけ新実装が列に足さない＝改善）。
  つまりこれもバグではなく**バグの作り方**で、**#108 とまったく同じ形**である——
  正本を作る作業と、既存の呼び出し元を全部その正本へ寄せる作業は別物で、前者だけやると
  「正本があるのに従っていない箇所」が生まれる。#108 は #107 が作った正本で同じことが起き、
  今回は**その次の日に、次の正本で、また起きた**。

  4入口すべてを正本へ寄せ、直書きが復活したら落ちるテストを追加した。**修正前のコードに向けると
  実際に落ち、残っていた2箇所の式（`Object.values(staffAliasesPdf).flat()` /
  `Object.values(staffAliases).flat()`）を名指しする**ことを確認済み。

### 要確認（未修正）

- **🟢（新規）期限付き削除で名前を残した人が「未提出」の一覧にだけ数えられない**（[app-staff.js:541](app-staff.js)）→ **BACKLOG化済み**

  今回で名簿の解決は5経路が揃ったが、**6つ目**が提出状況一覧の「未提出」（`notSubmitted`）で、
  ここだけが生の `staffList` を見ている。**実測**（SmModal だけをマウント。`keepStaff` に佐藤・
  提出は田中のみ）: 表示は**「未提出（1名）」＝鈴木のみ**で、**佐藤は画面に一度も出ない**。
  佐藤はその期間のシフト表に列があり、提出画面では自分の名前をサジェストから選べるのに、である。
  管理者は「全員出した」と読んで締め切れてしまう。

  数えるべきか（＝退職者を催促するか）は**仕様判断（条件B）**なので直さず起票した。ただし
  今回の決定「残した人はその期間の名簿の一員」とは食い違っている点は記しておく。

- **🟢 配信版数が app-*.js に追いついていない**（#103 から継続）。`?v=` と `build:` は
  `20260831-c340d5c` のままで、以後 app-*.js のファイル変更が**18件**入っている。
  `/release-to-main` の手順63が標準工程としてバンプするため申し送りのみ。
- **変化なし（#108 から継続・BACKLOG化済み）**: プラン変更予約を取り消せない（🟡）／
  秘密鍵ログ修正の本番反映／二重課金の根治／特商法表記（🔴 2件）／解約通知／解約時のプラン判定／
  企業連携の解除／別名提出の重複（#81・根は未解決）／スタッフ削除の残り確認2件／
  `database.rules.tightened.json` の残骸（今回も `cmp` でバイト同一を再確認）。
- **変化なし（#108 から継続・BACKLOG化しない🟢）**: `resolveShopMeta` の JST 未適用（安全側・dry-run中）／
...
- `npm test` **223件パス**（+2。うち1件は今回追加したドリフト検出で、修正前のコードでは落ちることを確認済み）／
  `npx eslint app-*.js` **0 errors 95 warnings**（警告は増減なし）／`node --check functions/index.js` OK／
  `secrets` 指定6箇所／`.delete()` の誤用 **0件**／`database.rules.json` と `tightened` は `cmp` でバイト同一。
- **実ブラウザ実測2本**（1.6節のマウントハーネス・375px）: SubsTab の「別名を登録」A/B（上表）／
  SmModal の「未提出」。どちらも pageerror・console.error 0件。
- **差分テスト1本**: 旧述語と `isUnregisteredSubName` を**マージ済み名簿20,000ケース**で比較し不一致0件。
- **Firebase には1バイトも書いていない**（`app-main.js` を読まないハーネスのみ使用）。
- **未検証**: Excel・PDF の実物確認（**17回連続**・ユーザー操作が要る）／iOS 実機／本番データ／
  プラン変更予約の Stripe 実データ／**今回保全した修正のうちスタッフ画面のサジェスト経路**
  （SubsTab と SmModal は実ブラウザで踏んだが、StaffView は踏んでいない）。

### 測り方の記録

**今回いちばん危なかったのは、測定ではなく「何もしないこと」だった。** 作業ツリーの未コミット変更は、
自分が1文字も触らなくても、**このセッションが終わった瞬間にフックが部分コミットして push する**。
バグチェックは読み取り中心の作業なので「触らなければ安全」と考えがちだが、**フックがある環境では
"終わる"こと自体が書き込み**になる。着手時に `git status` を見て差分の中身と mtime を確認し、
フックの `git add` 対象を読んでから動いたので、部分コミットの前に揃えられた。

### 総括

**#107 が正本を作り、#108 がその繋ぎ忘れを塞ぎ、#109 は次の正本で同じ繋ぎ忘れを見つけた。**
3日連続で同じ形が出たということは、これは個々の不注意ではなく**手順の穴**だと考えたほうがよい——
「共通関数を作る」と「既存の呼び出し元を全部そこへ寄せる」は別の作業で、前者だけで満足しやすい。
今回はドリフト検出テストを2種類（一覧の直書き禁止・入口の本数固定）置いたので、
**次に誰かが直書きを足したら機械が落とす**。それでも6つ目の入口（未提出の一覧）は
テストでは拾えなかった——**判定式が違う**（名簿への所属 vs 未登録名）ため、
同じ不変条件を守っているのに文字列としては似ていないからである。
**正本を作るときは「同じ式を探す」ではなく「同じ問いに答えている箇所を探す」**必要がある。
