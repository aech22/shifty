...
## Shifty バグチェックレポート（2026-09-16 自動実行 #130）

> 着手時の HEAD は `05e1f22`。#129 のあとに**コードの変更が入った**（`2766643` 企業連携タブからの
> 他店舗ログイン・`c20a092` 解除が連携の両側を消すように変更・`05e1f22` 配信版数のバンプ）ので、
> この新しい企業連携まわりを重点的に見た。**コードのバグは見つからなかった**——立てた仮説は
> 実測で2つとも否定された。見つけて直したのは CLAUDE.md の記述漏れ1件（🟢）。

### 修正済み

- **[🟢] Cloud Functions の表から `changePlan`・`cancelPlanChange` が落ちていた**（CLAUDE.md:387）
  `exports` は17本あるのに、アーキテクチャ節の表は15本しか載せていなかった。落ちていた2本は
  **どちらも課金のプラン変更系**で、BACKLOG 節の中でしか言及が無い。この表は全セッションが
  「課金はどう動くか」を引くときの正本なので、載っていないと「Checkout と Portal しか無い」と読める。
  **二重課金の根治（2026-08-11・案C）は、まさに `createCheckoutSession` を使わせないために
  `changePlan` を新設した変更**だったため、表だけ見て `createCheckoutSession` に手を入れる誤誘導が起きうる。
  あわせて `createCheckoutSession` の行に「有効な契約がある店舗は409で拒否する」を補った。
  追記した3行はいずれも実装を読んで確認している（409＋`code:"already_subscribed"` ／
  `subscriptions.update` と `end_behavior:"release"` ／ `subscriptionSchedules.release` と
  `scheduledPlan` の無条件クリア）。

### 要確認（未修正）

- **新規の🔴🟡はゼロ。** 今回コードは1バイトも変えていない。
- **#129 から継続（変化なし）**: 管理者が退勤を出勤より前の時刻で入力すると集計だけ黙って0になる（🟡・BACKLOG化済み）。
- **#127・#128 から継続**: 解約済み契約の請求書支払いで有料プランに戻る／`subscription.updated` が
  順序を疑わず書く（🟡・同じBACKLOGタスクに集約・Stripe設定の確認とCFデプロイ待ち）。
- **#124 から継続**: `fixedShiftCommandFor`・`recentPeriodIds` がテスト専用／`fmtH4` の残骸／
  `subsWindowCutoff` の月末繰り上がり／`staffAliases` 欠落時のメモ作り直し2件。
  **「配信版数が追随していない」は解消**——`05e1f22` が `?v=`（5箇所）と `build:`（1箇所）を
  `20260916-c20a092` へ揃えた。同コミットは版数の文字列以外を変えていないことを差分で確認済み。
...
- **[🟢] 今回の新規（実害は小・申し送り）**: 連携店舗の一覧合流によって、シフト作成タブが
  **他店舗の `subs` をまるごと読む対象が増えた**（app-admin.js:403 の `companyData` 取得）。
  この取得は `shops/{他店舗}/subs` の**全件読み**で、期間の絞り込みが効かない（コメントにもそう書いてある）。
  自店舗ぶんは「データ保存上限②」で直近3ヶ月の期間別購読に絞り込んだのに、**他店舗ぶんはその外側にある**。
  合流前は `allLinkedShops` ＝ accounts の店舗だけだったので、企業の作成者が
  シフト作成タブを開くと、**いまは企業に載っている全店舗ぶんの全期間の提出データを毎回落とす**。
  読み取り自体は `subs .read: auth != null` なので拒否されず（＝黙って空になる形の壊れ方はしない）、
  正しさの問題ではない。店舗数と蓄積量に比例するコストの話なので、企業の連携店舗が増えたときに効いてくる。
- **引き受け済みのトレードオフ（再検出しても直さない）**: `subs/$subId/.write` は認証済みなら通る。2026-08-31 決定1。

### 立てて、実測で否定した仮説2つ

どちらも「新しく入った企業連携の合流が、既存のガードを無効化していないか」という筋で立てた。

1. **「連携店舗の一覧合流が『最後の店舗は解除できません』のガードを骨抜きにする」→ 否定**。
   `unlinkShopFromAuth` のガードと CompanyTab の解除ボタンの表示条件は、どちらも
   `allLinkedShops`（＝accounts ∪ companies）を数える。一方 Phase1 は `accounts/{uid}/shops` **しか**
   読まない（app-main.js:293）ので、「最後の accounts 店舗」を解除するとリロード後に
   `toUnbound()` へ落ちる、と読んだ。
   **実測**（`stub-firebase.js` ハーネス・Firebaseへは1バイトも出していない。accounts は S1 のみ、
   企業には S1・S2・S3 が載っている状態で S1 を解除）:

   | | 実測 |
   |---|---|
   | 解除は通るか | **通る**（ボタンも出る） |
   | `accounts/{uid}/shops` | `{"S1":true}` → **null** |
   | リロード後の画面 | **「店舗を選択してください / B店 → / C店 →」** |

   **ログイン画面ではなく店舗選択が出る**＝合流した企業店舗は `unbound` の選択画面にも供給されるので、
   そこから入り直せる。締め出しは起きない。`pageerror`・`console.error` ともに0件。

2. **「店舗メニューの『解除』が企業側まで消すのは想定外では」→ 想定どおり**。
   `unlinkShopFromAuth` が `onUnlinkShop`（店舗メニュー）と `onUnlinkStoreFromCompany`（企業連携タブ）の
   両方に渡っているため、店舗メニューの解除でも `unlinkStoreFromCompany` CF が走る。ただし
   **片側だけ消すと合流でリロード時に戻ってくる**ので、両側を消すのが正しい（CLAUDE.md にその旨あり）。
   企業アカウントを持たない利用者では `_resolveCompanyId` が空を返し **CFを呼ばない**ことも、
   ハーネスの `no-company` シナリオで確認済み（呼ばれたCF: `[]`）。
...
- RULES.md のスキャン項目はすべてクリア（`DEV_MODE` は式のまま・`subs` 全体 `set()` 0件・`accounts` 全件読み0件・
  SRI 11本・`.delete()` 0件・`secrets:` 7件・読み込み順正常）。fontSize 走査: **フォーム部品58件・16未満0件**。
  CSS変数の未定義参照 0件・ダーク定義の二重管理に差分なし。
- **`example-company-unlink.js`（`8c39687` で保全されたハーネス）を HEAD に当てて 37項目すべてパス・fail 0**。
  5シナリオ（作成者／企業アカウント無し／企業ログインセッション／CFが拒否／店舗追加）が、
  操作直後とリロード後で一覧・`accounts`・`companies` の3つとも食い違わないことを確認した。
- **Cloud Functions 13本の入力検証を機械的に棚卸しした**（`shopId`・`companyId` を受ける全エンドポイント）。
  `claimCompanyShop` は `isValidShopId`＋`isValidCompanyId`＋`isDemoShop`＋`assertCompanyMember` を
  すべて通しており、#125・#126 で見つけたパス正規化のすり抜けと同じ形は**新しい関数には持ち込まれていない**。
  `linkStoreToCompany`・`unlinkStoreFromCompany`・`createCompany` に `isValidShopId` が無いのは #129 と同じ
  （すり抜けは成立しない・多重防御の提案どまり）。
- `companies/{companyId}/grants` にクライアント用のルールが**無い**こと（＝CF専用パス）を
  `database.rules.json` で確認。`.write:false` と `pub` の読み条件も読んだ。
- `unlinkStoreFromCompany` の「最後のオーナーは外さない」ガードが、`grants` 由来のuidを
  `revoke` に含めるようになっても正しく働くことをコードで追った（企業uidと付与uidしか
  オーナーが居ない店舗では拒否し、元からのオーナーが居れば通す。`claimCompanyShop` は
  既にオーナーのuidを `grants` に記録しないので、元からのオーナーが巻き添えにならない）。
- **Firebase・Stripe には一切アクセスしていない。** 本番・dev のデータは読んでも書いてもいない。

- **`allLinkedShops` の読み手を全数当てた**（`grep` で10箇所）。意味が「accounts の店舗」から
  「accounts ∪ companies」へ変わったので、読み手ごとに影響を見た。
  `createCompany`（app-main.js:1057）は**企業を作る前に呼ばれる**＝合流がまだ走っていないので従来どおり。
  `unbound` の店舗選択（:1437）と `onSwitchToShop`（:1700）は合流の恩恵を受ける側。
  影響が変わったのは上の🟢1件（`ShiftEditTab` の他店舗読み）だけだった。

**申し送り（次回の観点）**: 今回いちばん効いたのは「**新しく足した合流・派生が、既存のガードや
既存の絞り込みの前提を黙って変えていないか**」という見方だった。仮説1（ガードの骨抜き）は白、
🟢の1件（絞り込みの外側にある全件読みの対象が増えた）は当たり。
**同じ形がもう1つある**——`ShiftEditTab` の他店舗 `subs` 取得は `selPid` を依存に持たない代わりに
全期間を読む設計で、`onLoadPastSubs`（過去参照ボタン）とは無関係に走る。
自店舗の3ヶ月窓と他店舗の全件読みが**同じ画面に同居している**ので、
どちらかに揃えるかどうかを次に見るとよい（揃える／揃えないはコストと機能の判断なので条件Bに当たりうる）。
