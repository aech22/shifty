...
## Shifty バグチェックレポート（2026-09-01 自動実行 #103）

> 着手時の HEAD は `6935506`。**#102 と同じ日の実行**で、間に入った実コード変更は #102 自身の修正
> （`6613c78` getOT の帯判定・`87f66ee` 秘密鍵のログ）だけ。**追試したところ両方とも崩れていなかった。**
> 今回見つかったのは、#102 と同じ形——**一括是正が「見なかったファイル」に残っていた1件**だった。

### 修正済み

- **🟡 ログイン画面の店舗コード入力が iOS Safari でズームを誘発していた**（[app-main.js:1496](app-main.js)）→ `bc7bf2e`

  `fontSize:14` のため、フォーカスすると iOS Safari がページを自動ズームする（16px未満が条件）。
  **iOS はズームを自動で戻さない**ので、ユーザーは拡大されたビューポートに取り残される。
  出る場所が悪い——**未バインド端末が最初に見る画面**（`unbound` のログイン画面）で、
  新規の管理者が「店舗コードを貼り付け」に触れた瞬間に起きる。VISION の
  「モバイルファースト」「スタッフ・管理者ともスマートフォン前提」に正面から反する。

  **同じ画面の他の入力5つは全部 16px だった**——メール・パスワード・パスワード確認・
  企業コード・企業パスワード。**これ1つだけが取り残されていた。**

  **原因は「掃除がこのファイルを開いていない」**。iOS ズーム一括是正 `b7c084d`（2026-07-08 21:06）が
  触ったのは app-staff.js（フォーム部品4件）と app-admin.js（46件）だけで、
  **app-main.js（6件）を一度も見ていない**。app-main.js は `f02cc80`（2026-07-06 03:31）の
  5ファイル分割で**その2日前に生まれたばかり**で、掃除の対象リストに載らなかった。
  そのまま CLAUDE.md に「2026-07-06 に全箇所解消済み」と書かれ、**約2ヶ月**残った。

  **実測（playwright・375×812・カードの幾何をそのまま再現・1変種1ドキュメント）**

  | | font | iOSズーム | カード幅 | 横スクロール | 見切れ |
  |---|---|---|---|---|---|
  | 修正前 Chromium | 14 | **する** | 335 | なし | なし |
  | 修正後 Chromium | 16 | しない | 335 | なし | なし |
  | 修正前 WebKit | 14 | **する** | 335 | なし | なし |
  | 修正後 WebKit | 16 | しない | 351.2 | なし | なし |

  WebKit（＝iOS Safari のエンジン）だけカードが 16.2px 広がり、外周の余白が 20px → 約12px に詰まる
  （input の固有幅が font サイズに比例するため）。**見切れも横スクロールも発生しない**ので許容した。
  `minWidth:0` で防げるかを実測したが**両エンジンとも1pxも変わらなかった**ので入れていない
  （効かない保険を残さない）。

  フォーム部品を全ファイル機械走査して **56件中0件**（タグ内の `fontSize` を検査）。

- **🟢 CLAUDE.md の「iOS ズームは 2026-07-06 に全箇所解消済み」が事実と違った**（CLAUDE.md:544）→ 本コミット

  上記のとおり **app-main.js は一度も走査されていない**のに「全箇所」と書かれていた。
  CLAUDE.md は毎セッションのコンテキストに載るので、**この記述を信じたセッションは
  「もう対策済み」として新規 input を追加しうる**。#102 の `857041e`（締めルールの記述が
  切替前のまま）とまったく同じ形の負債で、経緯（どの掃除が何を見なかったか）ごと残した。

### 調べて「バグではない」と確定したもの

- **ヒートマップの延長時間帯に列が描画されない**という仮説を立てたが、**反証した**（[app-admin.js:1082](app-admin.js)）。
  `heatHours` は延長ぶんとして**終端の時刻しか** `hrs` に足さない（両側セルのみ・片側セルは足しもしない）一方、
  `countHeat`（:859）は延長後の `pEnM` まで出勤として数えるため、「数えているのに列が無い」時間帯が
  生まれるはずだと考えた。**実際には生まれない**——`hrs` は :1094 で `min`〜`max` の**連続範囲**に
  畳まれてから返るので、集合に穴が空いても列は欠けない。
  実ブラウザでも確認（`ShiftEditTab` 単体マウント・ランチ候補 09:00-15:00／ディナー 17:00-23:00・
  田中のランチ延長 +120分＝16時台はどちらの候補にも属さない条件）: **描画された列は 9〜23 の連続**で
  16時台も出る。pageerror・console.error ともに0件。

### 要確認（未修正・#102 からの持ち越しで変化なし）

- **🟢 `database.rules.tightened.json` が現行ルールとバイト同一の残骸** → **BACKLOG化済み**。
  今回も `cmp` で**バイト単位まで同一**であることを再確認した（片方だけ直された形跡は無い）。
  `firebase.json:7` が参照するのは `database.rules.json` の1本だけ。消すか残すかは**条件B（判断）**。
- **🟢 片側セルの日は「→15:15（+15分）」の実効退勤バッジが出ない**（[app-admin.js:3533](app-admin.js)）。
  分の計算は正しく、実害は「なぜ合計が増えたか画面から辿れない」だけ。
...
- **変化なし・BACKLOG化済み**: 秘密鍵ログ修正の本番反映／二重課金の根治／特商法表記（🔴 2件）／
  解約通知／解約時のプラン判定／企業連携の解除／別名提出の重複（#81）／スタッフ削除の残り確認2件。

### 申し送り（BACKLOG化しない）

- **配信版数が app-*.js の変更に追いついていない**: `?v=`（index.html 5箇所）と `build:`（app-core.js）は
  `20260831-c340d5c` のままだが、以後 `6613c78`（app-utils.js）・`bc7bf2e`（app-main.js）が入っている。
  **`/release-to-main` の手順63が標準工程としてバンプする**ため、ここは申し送りのみ（二重管理にしない）。

### 検証したこと

- **RULES.md のスキャン項目は全項目クリア**: `DEV_MODE` は式のまま（`location.hostname !== "shiftyshifty.app"`）／
  `subs` の `set()` 全体上書き **0件**／`accounts`・`shops`・`global/shops` の全件読み **0件**／
  読み込み順（utils→core→staff→admin→main）維持／SRI **11本**／
  **フォーム部品の `fontSize<16` 0件**（56件走査・今回の修正で解消）。
- **CSS変数**: 未定義参照 **0行**／ダーク定義の二重管理（`[data-theme]` と `prefers-color-scheme`）**差分なし**／
  SVG属性内 `var()` は既知の `ShiftyIcon` 1件のみ（Chromium・WebKit とも解決されることは #71 で実測済み）。
- **Firebaseルール**: `database.rules.json` と `tightened` が**バイト同一**／`firebase.json` の参照は1本だけ。
- **Cloud Functions**: 秘密鍵のログは**2箇所とも**バイト長のみ（`loaded(<n>b)` / `EMPTY`）＝ `87f66ee` が
  正しく効いている／`substring(0`・`substr(` の残り **0件**／`secrets:` を読む関数はすべて
  `runWith` で宣言済み（STRIPE×4・SMTP×2・SURVEY×1）／`.delete()` 誤用 **0件**／`node --check` OK。
- **削除の永続化**: `saveSubs(` の全呼び出しを確認。第2引数の無い `app-admin.js:208` は
  **削除ではなくリネーム**（`subs.map` でidを保持）なので `update()` 意味論で正しい＝
  #102 の「Firebaseから消えないパターン0件」は今回も成立。
- `npm test` **205件パス**（増減なし）／`npx eslint app-*.js` **0 errors 95 warnings**（増減なし）／
  `node --check functions/index.js` OK。
- **devのデータは1バイトも変えていない**（実ブラウザ検証は `ShiftyEditTab` の単体マウントと、
  ログイン画面の幾何を再現した独立ページのみ。`firebaseDB` は `null` のまま）。
- **未検証**: Excel・PDF の実物確認（**11回連続**・ユーザー操作が要る）／iOS 実機での実際のズーム挙動
  （WebKit エンジンでの `font-size` 実測までは取ったが、実機の拡大動作そのものは踏んでいない）／
  本番データ／CF の本番反映後のログ。

### 測り方を間違えた記録

`minWidth:0` の要否を測るとき、**3つの変種を1つのHTMLに並べて**同時に測った。
ところが変種(b)（`minWidth` なし・16px）がページを横に広げ、**その影響が同じドキュメントに
同居する変種(a)(c)のカード幅まで動かした**——WebKit でカード幅が 342.1 / 364.1 / 364.1 と、
本来independentなはずの数字が変種間で連動した。1変種1ドキュメントに分け直したら
335 / 351.2 / 351.2 になり、**(b)と(c)が完全に同一＝`minWidth:0` は効いていない**と確定した。

#102 の「複数条件が同じ値を返したら疑う」の**裏返し**で、今回は
**「独立なはずの条件が連動して動いたら疑う」**。どちらも根は同じ——
**測定対象どうしを同じ器に入れると、器を通して干渉する。** 条件を変えて測るときは、
条件の数だけ器を分ける。

### 総括

**掃除は、掃除する人が開いたファイルしか綺麗にしない。**
`b7c084d` は iOS ズームの一括是正として正しく仕事をした——**見た2ファイルの中では**。
そして見なかった app-main.js は、**その2日前に生まれたばかりだった**。
分割（`f02cc80`）と掃除（`b7c084d`）は48時間しか離れていないのに、
掃除の対象リストは分割前の世界のままだった。

**#102 は「回避策は、それが書かれた1箇所しか直さない」だった。今回は
「一括修正は、対象リストに載ったファイルしか直さない」。** 同じ穴の別の口で、
どちらも**「直した」と記録された瞬間に見えなくなる**のが厄介なところだ。
CLAUDE.md には2ヶ月間「全箇所解消済み」と書いてあった。
**「全」と書くときは、全を数えたのか、見た範囲を全と呼んだのかを区別する。**
今回フォーム部品を56件と数えたのは、次に同じ嘘をつかないためでもある。
