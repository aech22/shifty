...
## Shifty バグチェックレポート（2026-09-05 自動実行 #111）

> 着手時の HEAD は `bc70932`。作業ツリーは**クリーン**。今日は #110 に加えて BACKLOG 4件の修正と
> リリース（`bfa184b`・版数 `20260905-ee7736e`）が入っており、**本番へ出たばかりのコードを対象にした
> 最初のチェック**になる。観点は「今日書かれた所」と、#110 が申し送った
> 「**有効期限を持つデータを1つずつ確かめる**」の2つ。

### 修正済み

**なし。🔴・🟡 ともに検出ゼロ**（コードは1行も変更していない）。

### 要確認（未修正・いずれも🟢）

- **🟢 `verifyShopOwner` にだけ「移行猶予」が残り、コメントが期限切れの記述になっている**
  （[functions/index.js:102](functions/index.js)）→ **BACKLOG化済み**（「企業連携の解除が…」へ追記）

  `if (owners && !owners[decoded.uid])` は **`owners` が空（未claim）の店舗では誰でも `ok:true`** になる。
  コメントはこれを「移行猶予として許可する」と説明するが、**その猶予は 2026-07-28（`dbdd9d9`）に
  `database.rules.json` 側で終了済み**。**ルールだけ締められ Cloud Functions は猶予のまま**という非対称で、
  読んだ人は現行の意図的な仕様と受け取る。課金系4エンドポイントが全てこの判定を通るため、
  **新設の `cancelPlanChange` も同じ穴を自動的に引き継いだ**。
  実害は小さい（※以下は実測ではなく既存記録とコード上の推論）: 未claim は本番でデモのみで
  `isDemoShop` が先に403／未claim店舗は Stripe customer を持たず `changePlan`・`cancelPlanChange` は409で停止／
  owners を空へ戻す経路は `d6c826a` で閉鎖済み。素通りしうるのは `createCheckoutSession`（＝#67 の形）のみ。

- **🟢 `purgeOldPeriodsCutoff` が UTC で日付を切っている**（[functions/index.js:913](functions/index.js)）
  → **BACKLOG化済み**（「データ保存上限④-b」へ追記）。`timeZone("Asia/Tokyo")` なのに `toJstDateStr` を
  通しておらず、**JST 00:00〜09:00 の発火でカットオフが1日古くなる**（実測: JST 00:30 → `2023-09-04`／
  期待 `2023-09-05`）。**ずれる向きは「1日多く残す」で消しすぎではなく**、9/11 以降の本有効化を止める
  理由にはならない。同じUTC/JST問題は 2026-08-25 に `planExpiry` で直したが**この関数には届いていなかった**。

- **変化なし（#110 から継続・BACKLOG化済み）**: 二重課金の根治／特商法表記（🔴 2件）／解約通知／
  解約時のプラン判定／企業連携の解除／別名提出の重複（#81）／PDF の実物確認／Stripe秘密鍵ログの本番反映。
- **変化なし（BACKLOG化しない🟢）**: `resolveShopMeta` の JST 未適用／自分の別名へ改名／
  ヒートマップの時間目盛り／片側セルの実効退勤バッジ／期限切れの後始末の範囲。
...
- **`cancelPlanChange` は兄弟関数と構造が揃っている**。CORS・`isDemoShop`・`verifyShopOwner`・
  `findActiveSubscription` の順序が `changePlan` と同一。解放後に書く `plan` は**解放前に取得した契約の
  現行price由来**＝降格前のプラン（正しい）。Stripe にスケジュールが無くても DB の予約表示は必ず消す。
- **「取り消してもバナーが消えないのでは」という疑いは外れた**。コミットの実測は `MyPageTab` 単体マウント＋
  `fetch` スパイで実アプリの state を踏んでいないため疑ったが、`scheduledPlan`・`scheduledPlanDate` は
  [app-main.js:518-522](app-main.js) で**リアルタイム購読済み**（null 書き込みも通る）。配線は正しい。
- **`SmModal` の「未提出」修正は3つの呼び出し元すべてに効く**。BACKLOG は「2ファイル3箇所」と
  書いていたが、実装は `SmModal` **内部**（[app-staff.js:546](app-staff.js)）で `mergeKeepStaff` を通しており、
  `StaffView` の2つと `PeriodsTab`（app-admin.js:1961）が自動的に同じ名簿を見る。取り残しは無い。
- **`database.rules.tightened.json` の削除は配線を壊していない**。`firebase.json` は
  `database.rules.json` の1本だけを参照し実在する。残る `tightened` の出現は**全て説明文**でコード・設定は0件。

**2029年の祝日を独立に検算した（テストの循環を避けるため）**。`756d209` の追加分は**同じセッションが
書いた参照カレンダー**で検証されており、テストが通ること自体は正しさの証拠にならない。規則から
**別に参照カレンダーを組み直して照合**した結果、**テーブル19件／参照19件／欠落なし／余分なし**で
曜日も全件一致。**覆う最終日は `20291123`** でコミットの宣言（2029-11-23 で尽きる）と一致する。

**「同じ問いに2箇所が答えている」箇所の機械照合**: プラン判定は `isPro`/`isPremium`/`isPaid` の定義4箇所が
同形で、**Premium を締め出す `pro` 単独判定は0件**（逆方向も0件）／**クライアントの書き込み18パスを
ルールと突き合わせ**、拒否されるのは既知の `lastActivity`（no-op）のみ／**課金系4エンドポイント全てに
`isDemoShop` ガードあり**（新設分も漏れなし）。

**RULES.md のスキャン項目は全項目クリア**: `DEV_MODE` は式のまま／`subs` の `set()` 全体上書き **0件**／
全件読み **0件**／読み込み順維持／SRI **11本**／フォーム部品の `fontSize<16` **0件**／
`.delete()` 誤用 **0件**／`secrets` 7箇所。**配信版数のズレは今回なし**（#103 からの申し送りが解消）。

`npm test` **226件パス**／`npx eslint app-*.js` **0 errors 96 warnings**／`node --check functions/index.js` OK。
**Firebase・Stripe には読み書きとも一切アクセスしていない**。

**未検証**: PDF の実物（jsPDF がラスタ化するため列テキストが残らない）／iOS 実機／本番データ（claim 状況・
Stripe 実データ・Cloud Logging）／**`cancelPlanChange` の Stripe 実挙動**——予約解除後に `scheduledPlan` が
実際に消えることと「降格予約→取り消し→期間終了をまたいでも降格しない」は実購入テスト待ちで、
今回確認できたのは**コードの配線とクライアント側の状態反映まで**。

### 測り方の記録

**今回の収穫は「テストが通ることを正しさの証拠にしなかった」こと。** 2029年の祝日は `npm test` が緑だが、
そのテストの参照カレンダーを書いたのは**追加したのと同じセッション**である。共有された思い違いがあれば
両方が同じ方向に間違い、テストは何も言わない。**一致に意味があるのは独立に作った場合だけ**である。

**外れた仮説も収穫だった。** バナーの購読を疑ったのは、コミットの実測が単体マウント＋スパイで
**実アプリの state を一度も通っていない**と読めたからである。外れたが、
**「その実測は何を踏んでいないか」を見る**読み方自体は正しい。

**当たった2件はどちらも「コードではなくコメントと暦が古い」形だった。** #110 が言った
「いつまで正しいかを持っているデータ」は、テーブルや定数だけでなく**コメントにも宿る**。
次は「日付・期限・"猶予"・"暫定"・"当面" を含む記述」を機械的に洗い出し、期限切れを確かめたい。

### 総括

**🔴・🟡 ともにゼロ。** 出たばかりのコードに欠陥が無いことを確かめる回として使った。
新設の `cancelPlanChange` は構造・デモガード・購読配線に漏れが無く、`SmModal` の修正は
BACKLOG が「3箇所」と書いた所をコンポーネント内部の1箇所で正しく解いている。

**#107〜#109 の「正本の繋ぎ忘れ」は2回連続で出ていない**（今回はプラン判定・書き込みパス・
デモ判定の3クラスタを機械照合し、いずれも揃っていた）。見つかった🟢2件は、どちらも
**「一度直した問題が、同じ形の別の場所に残っている」**もので（UTC/JST は `planExpiry` で直したが
`purgeOldPeriodsCutoff` に届かず、未claim猶予はルールで終わらせたが `verifyShopOwner` に届いていない）、
**修正が「報告された場所」で止まり、同じ問いに答えている残りへ広がらない**という共通の形を持つ。
片方はコメントが実態と食い違っているぶん、むしろ発見しにくくなっている。
