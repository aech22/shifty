...
## Shifty バグチェックレポート（2026-08-21 自動実行 #87）

### 修正済み

- **🟡 提出一覧の氏名絞り込み・並べ替えが生の別名を見ていて、画面に出ている名前でその行を引けなかった**（[app-admin.js:3022](app-admin.js)）→ `34b7997`
  提出一覧の氏名列は `resolveAlias` で解決した**登録名**を表示する（`:3051` の `resolvedName`）。しかし同じリストの**絞り込み**（`s.staffName.includes(fn)`）と**並べ替え**（`a[sf]`）だけが**生の `sub.staffName`** を見ていた。`sub.staffName` には別名がそのまま残る（`registerAlias`・`:3011` は `staffAliases` に登録するだけで `staffName` を書き換えない）ため、両者が食い違う。
  **何が起きるか**: 別名「たなか」で提出した田中さんの行は、画面には**「田中」と表示されている**。その状態で氏名欄に「田中」と打つと、**その行だけが消え、件数も1件減る**。管理者から見ると「田中さんは未提出」に見える。並べ替えも同じ根で、氏名列で並べ替えると**表示していない文字列（別名）を基準に**並ぶ——実測では「あべ < 田中 < 佐藤 < 山田」という、その列を読む限り説明のつかない順序になっていた。
  **多数派が既に正解だった（仕様判断が不要だった理由）**: 同じファイルで subs を名前で引く箇所は `_getSub`(:568)・スタッフ数カウント(:2345)・`_shiftAt`(:3008)・`_workDatesOf`(:3009)・詳細モーダルの週月計(:3114)・`expXl`(:2185)、および app-staff.js の `SmModal`(:519) と、**すべて別名を込みで照合している**。**生の名前だけで照合しているのは `:3022` の1箇所だけ**だった。しかも同じ行の直下（`:3051`）が解決済みの名前を表示しているので、「表示している名前で引けるべき」以外の解釈が無い。
  **修正**: 絞り込みは**生の名前との一致を残したまま**表示名との一致を追加した（＝ヒットが減ることはない。別名「たなか」で引く従来の使い方もそのまま通る）。並べ替えは**氏名列のときだけ**表示名を使う（提出日時の並べ替えには触れていない）。
  **検証（配信物の述語をNodeで実行）**: `resolveAlias`・`subLastActionTime` を実際に読み込み、修正前後の式を同一データで走らせて突き合わせた。**絞り込みは6クエリ中「田中」の1件だけが 0件→1件 に変わり、他5件（別名・登録名・無関係な名前・空）はすべて同一**。氏名列の並べ替えは「あべ < 田中 < 佐藤 < 山田」→「あべ < 佐藤 < 山田 < 田中」＝表示名の昇順になった。**提出日時の並べ替えは修正前後で完全一致**（非回帰）。

### 要確認（未修正）

- **🔴（継続・プロセス）このループから実機E2Eが10回続けて実行できない**（環境要因・コード変更なし）
  今回も `preview_list`（稼働0件）と `preview_start` を実際に実行し、**「Dev servers can't be started from unattended sessions」と明示的に拒否された**。Bashでのサーバー起動は規約で禁止のため代替なし。**#78〜#86 に続き今回も1件も実行できなかった。ユーザーが対話セッションで `/bug-check` を手動実行すれば解消する。**
- **🟡 #77 の宿題（Free 上限の実機検証）は静的監査完了・実測のみ未達**（上記🔴が理由）。今回あらためて**期間の作成経路が2つとも `checkPeriodLimit` を通ること**（`:1860` 手動入力・`:1927` プリセット）と、**スタッフ追加が `lim` でブロックされスペーサーを数に含めないこと**（`:2326`）をコード上で再確認した。**残るは実機で21人目・2件目を押す1回だけ。**
- **🟢（新規）Free のスタッフ上限が2箇所に別々の値で書かれている**（[app-admin.js:2306](app-admin.js)）
  `const lim=PLAN_LIMITS[plan]?.staff??10;` のフォールバックが **10** だが、`PLAN_LIMITS.free.staff` は **20**。同じファイルの `UpgradeModal`(:4074) は `PLAN_LIMITS[plan]||PLAN_LIMITS.free` ＝ **20** にフォールバックするので、**同じ「プラン不明時」に2つの画面が違う上限を主張する**（片方は10人で止め、もう片方は「上限20人」と表示する）。他の2箇所（期間 `??1`・店舗 `??Infinity`）は free の値と一致しており、**ズレているのは `staff` の1箇所だけ**。
  **本番では到達しない**ことを確認した——`setPlan` の呼び出しは [app-main.js:465](app-main.js) の1箇所だけで、`["free","pro","premium"].includes(val)` で絞ってから入れており、初期値も `"free"`（:74）。到達するのは **DEV_MODE で `?plan=` に未知の文字列を渡したときだけ**（`DEV_PLAN_OVERRIDE`・app-core.js:125 は値を検証しない）。今回は🟢のため修正していない。
- **🟢（新規）`parseTime` の受理範囲（0〜30時）と候補時刻リスト `gto()` の生成範囲（0:00〜27:00）が食い違う**（[app-admin.js:479-481](app-admin.js) / [app-utils.js:87](app-utils.js)）
  #86 が次回候補に挙げた組を実際に並べた結果。シフト作成グリッドのセルは `28`〜`30` を 28:00〜30:00 として受理するが、**その時刻はドロップダウン（`TO`）に存在しない**ため、詳細モーダルの調整用 select で選び直せず、選択状態も再現できない。ただし「パーサは寛容・ピッカーは厳格」は入力補助として妥当な非対称でもあり、**壊れる経路（誤集計・例外）は見つからなかった**（`calcNetWorkMinutes`・`shiftBandInfo` は分換算のみで上限を持たない）。実害を示せていないので🟢に留める。
- **🟢（新規）`database.rules.tightened.json` が `database.rules.json` と完全に同一になっている**（`diff` で差分0）
  2026-07-28 の締めルール切替で本体へ反映済みのため、**現在は同じ内容の2ファイルを手で同期し続ける状態**になっている。次に片方だけ直すと黙って乖離する。BACKLOG の🟢「招待コード方式の残骸を消すか復活させるか」が**両ファイルの `inviteCodes` ルール削除を受け入れ条件に含んでいる**ので、その決着時に一緒に片付けるのが自然。単独のタスクは起票していない（二重管理になるため）。
- **🟢（新規・申し送り）企業連携の他店舗データ取得だけが3ヶ月窓の外にある**（[app-admin.js:392](app-admin.js)）
  `companyData` は連携店舗ごとに `shops/{id}/subs` を**期間で絞らずまるごと**読む。自店舗の subs は「データ保存上限②」で直近3ヶ月＋アクティブ期間に絞る部分購読になっているのに、**他店舗ぶんだけが全件読み**で、店舗数×蓄積量に比例して増え続ける。コード上のコメント（:422-425）は「期間ドロップダウンの切替ごとに再取得しない」ためにこの形を選んだと説明しており、**窓を掛けない点については触れていない**。ただし `purgeOldPeriods` が本有効化されれば36ヶ月で頭打ちになるため、上限のない増加ではない。今回は変更せず記録のみ。
- **🟡 店舗間シフト重複の判定が退勤延長（残業）の時間を見ていない**（#86）→ BACKLOG化済み（仕様判断待ち・変化なし）
- **🟡 `dupErrors` だけが「片方のセルしか埋まっていない日」を判定対象から外している**（#86）→ BACKLOG化済み（仕様判断待ち・変化なし）
- **🟡 `x`（ヘルプ・カウント外）のセルが店舗間重複の判定から除外されていない**（#85）→ BACKLOG化済み（仕様判断待ち・変化なし）
- **🟢（#86からの申し送り・確認済み）今回1つ厳しくした略称バリデーションは、既存データを持つ店舗に非対称を作っていない**
  #86 の宿題として実際に読み直した。`addAbbr`(:3210) の新しい `/^[\d.:]/` は**新規登録だけを止める**もので、`removeAbbr` にも既存値の読み出し（`abbrsOf`・`abbrToShop`）にも検査は入っていない。したがって「先頭が数字の略称を登録済みの店舗が、その略称を消せない／他の略称を追加できない」といった詰みは発生しない。**#86 が申し送った「登録済みで機能しない値がデータに残る」ことだけは変わらず残る**（掃除も警告も実装されていない）。
- **🟢 `index.html` の `?v=` と `build:` が現在の配信物を指していない**（5本のscriptタグと `app-core.js:2` の6箇所は相互に一致しているが、いずれも `20260814-53f610f` のまま。**その版数以降 app-*.js は20コミット変更されている**）。申し送りのみ——`/release-to-main` の手順63がバンプする
- **🟢 `effShiftStart`/`effShiftEnd` の未採用が app-admin.js になお3箇所残る**（`:1005` ヒートマップの時間軸／`:3051` 提出一覧の出勤日数／`:3109` 詳細モーダルの出勤日数）。**#82〜#86 と同じ理由で今回も触らなかった**——直し方が BACKLOG の判断待ち項目（片側セルだけの日を何時間と数えるか）に依存する
- **🟢 自由メモの先頭数字が時刻に食われる**（#86）／**略称lookupは大小文字を区別するが登録時の予約語チェックは区別しない**（#86）→ どちらも変化なし
- **🟢 `addAllFromWeekday`（`:2607`）と `handleSaveAll`（`:698`）のトーストが件数を多めに言う**（#81から継続・変化なし）
- **🟡 別名提出者が別端末から再提出すると同一期間に2つのsubができExcelが古い方を出す**（#81）→ BACKLOG化済み
- **🟢 期間の重複にバリデーションが無い**（#83）→ BACKLOG化済み（仕様判断待ち）
- **🟡 コントラスト5件**（#72・#75・#76・#77）／**タップ領域44px未満**（#74）→ BACKLOG化済み・デザイン判断待ち（**今回も実測不能のため変化なし**）
- **🟡 スタッフURLを開いた誰でも他人の提出を削除できる**（#70）／**スタッフ削除の後始末がリネームと正反対**・**未登録名が画面に出ずExcel/PDFにだけ出る**（#79）／**解約イベントだけが metadata でプラン判定**（functions/index.js:319）→ すべて BACKLOG化済み
- 以下は #46〜#86 からの継続（変化なし）: 未claim店舗の取り込み／`findActiveSubscription` の `|| live[0]`／`planExpiry` のUTC計算／**片側セルだけの日が0分**／`purgeOldPeriods` のUTC日付／`generateInviteCode` のデッドコード／ほか

### 今回の対象と見つけ方

**#86 以降のコミットは0件。15回続けて「差分ゼロ」の回である。**

**(1) 申し送られたレンズ（同じ制約が2箇所に別々の書き方で存在する組を全部並べる）を、名指しされた候補から順に当てた。** #86 は3つの候補を挙げていた。結果は**3つとも🟢止まり**だった——`parseTime`(0〜30) vs `gto`(0:00〜27:00) は食い違うが壊れる経路が示せず、`addAbbr` の4文字制限 vs PDF の `vfontSize`(5文字基準) は `base*5/len` で連続的に縮むだけで境界を持たず、`database.rules.json` の `.validate`（`staffName` 50・`name` 100・`lastActivity` 40・`adminKey` 16〜64）はクライアント側の `maxLength` と**全件一致**していた（スペーサーが `__spacer__` 始まりの文字列＝50文字制限に収まることも確認）。

**(2) レンズは当たらなかったが、レンズの形は当たった。** 候補を潰す過程で `PLAN_LIMITS` を全参照した際に、**同じ「プラン不明時の上限」が `??10` と `||PLAN_LIMITS.free`(=20) の2通りに書かれている**のを見つけた。これは #86 が探せと言った形そのもの——ただし本番では到達しない（`setPlan` の入口が1箇所で値を絞っている）ため🟢に落ちた。**探し方は正しく、当たりが軽かった。**

**(3) 決め手は、レンズを「制約」から「名前」へ持ち替えたことだった。** #84・#85 が別名まわりで連続して当てていたので、**subs を名前で引く箇所を1つ残らず列挙し直した**（8箇所）。7箇所が別名込みで照合しているのに `:3022` だけが生の `staffName` を見ており、**しかもその直下（`:3051`）が解決済みの名前を表示していた**。1つの式の中で「表示する名前」と「引く名前」が別物になっていて、多数派も直下の表示コードも既に正解を示していたので、**仕様判断を挟まずに閉じられた**。

**(4) 修正の形を「足すだけ」に寄せた。** 絞り込みを表示名一致に**置き換える**と、別名そのもの（「たなか」）で引く従来の使い方が消える。そこで生の名前との一致を残したまま表示名との一致を **or で足した**。この形なら定義上ヒットが減らないので、「今まで引けていたものが引けなくなる」回帰が原理的に起きない。実測でも変化したのは壊れていた1クエリだけだった。

**(5) 並べ替えは同じ式の中にあり、同じ根だったので同じコミットに入れた。** 氏名列を「表示していない文字列」で並べ替えているのは、絞り込みが表示名で引けないのと同一の欠陥である。別コミットに割ると1つの論点が2つに見えるため分けなかった。提出日時の並べ替えには手を入れず、非回帰を実測で確認した。

### 検証したこと

- `DEV_MODE`（app-core.js:12・式のまま）／`DEV_PLAN_OVERRIDE`（:125）正常
- `subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／`global/shops` の全件読み **0件**（直キー読みのみ）／CF `.delete()` **0件**
- 読み込み順維持（utils→core→staff→admin→main）・SRI **11本**／`database.rules.json` の `".read": true` **0件**
- `npm test` **168件パス**／`npx eslint app-*.js` **0 errors 98 warnings**（修正前後で増減なし）／`node --check functions/index.js` OK
- **名前で subs を引く全経路の棚卸し（今回の主レンズ）**: `_getSub`(:568)・スタッフ数カウント(:2345)・`_shiftAt`(:3008)・`_workDatesOf`(:3009)・詳細モーダルの週月計(:3114)・`expXl`(:2185)・`SmModal`(app-staff.js:519)・提出一覧の絞り込み/並べ替え(:3022) の**8経路**を分類。**生の名前で照合しているのは :3022 の1箇所のみ**（→修正）。`SmModal` には氏名の絞り込み・並べ替え自体が存在しないことも確認
- **制約の二重定義の棚卸し（#86の申し送りを実行）**: `parseTime` vs `gto`／`addAbbr` の4文字 vs `vfontSize` の5文字／`database.rules.json` の `.validate` vs クライアントの `maxLength`／`PLAN_LIMITS` の4つのフォールバック。**ズレは3件見つかったが、いずれも実害を示せず🟢**
- **`det.staffName` の書き換えが Firebase へ漏れないこと**: 詳細モーダルは `setDet({...sub,staffName:resolvedName})`(:3089) で**登録名に差し替えたコピー**を持つ。この `det` が保存経路に乗ると別名提出の `staffName` を黙って書き換えることになるため追跡したが、`saveAdj`(:3024) は `subs.map` を id で回して `shifts` だけを触り、`setDet`(:3027) はローカルstateのみ。**書き戻し経路なし**
- **Free 上限の静的再確認**: 期間作成の2経路（`:1860`・`:1927`）が両方 `checkPeriodLimit` を通ること、スタッフ追加(`:2326`)がスペーサーを除いて数えること
- **修正の検証（配信物の述語をNodeで実行）**: `resolveAlias`・`subLastActionTime` を実際に読み込み、修正前後の絞り込み式・並べ替え式を同一データで比較（絞り込み6クエリ／氏名列の並べ替え／提出日時の並べ替え）
- **未検証（今回は手段が無い）**: ブラウザでの実描画すべて／**修正後に提出一覧で実際に絞り込み・並べ替えを操作したときの表示**／**先頭数字の略称を既に登録している本番店舗があるか**（#86の申し送り）／**Free 上限に達した店舗での実ブロック**／実機 iOS Safari

### 総括

**測れない回が10回続き、それでも 🟡 が1件出て直った。** 根は #81・#84・#85 と同じ「`sub.staffName` に別名がそのまま残る」だが、**今回の現れ方はこれまでで一番直接的だった**——集計値がずれるのでも、判定が発火しないのでもなく、**画面に「田中」と書いてある行が、「田中」で絞り込むと消える**。#85・#86 が「黙って出ない」形だったのに対し、今回は**打てば分かる**形で、しかも件数まで一緒に減る。

**別名系列は4回連続で別の場所に出ている。** #81 が提出データの取り違え、#84 が属性・退勤延長の引き漏らし、#85 が店舗間重複の不発、#86 が略称の登録境界、そして今回が一覧の絞り込み。**根は1つ（`registerAlias` が `staffName` を書き換えない）で、症状だけが毎回違う場所に出ている。** BACKLOG の「別名で提出した人が別端末から再提出すると…」が**その根そのものを扱う唯一のタスク**で、そこには「`staffName` を登録名へ正規化するか」という仕様判断が置かれたまま止まっている。**この判断が決まるまで、派生は今後も別の場所に出続けると考えたほうがよい。**

**申し送りのレンズは今回も直接は当たらなかったが、3回連続で「軸をずらすきっかけ」としては働いた。** #86 が名指しした3候補はすべて🟢で終わり、その作業の中で `PLAN_LIMITS` の二重定義（これも🟢）に触れ、そこから「同じものが2通りに書かれている」の対象を制約から**名前**へ持ち替えたところで当たった。#84→#85→#86→#87 と4回続けて同じ形になっている。

**次回の手を1つに絞る。** 実測が復活したなら**#77 の宿題（実機で21人目・2件目を押す1回）をそのまま実行する**。使えないままなら、推奨は**「画面に表示している値と、その画面が内部で使っている値が別物になっている組を全部並べる」**——今回の1件は「表示は解決済みの名前・照合は生の名前」だった。同じ形の候補: **提出一覧の「出勤」列**（`:3051` は生の `adjustedStart??start` で判定しながら `shiftBandInfo` には生のshiftを渡す＝1つの式に2つの実効時刻が同居している。#82で確認済みだが未修正）／**詳細モーダルのヘッダー名**（解決済みを表示）と**その下の表**（`det.shifts` は元のsubのまま）／**ヒートマップの時間軸**（`:1005` は生の時刻で `hrs` を作るが、セルの中身は `effShiftStart/End` 基準）。**あわせて、今回いじった提出一覧の絞り込みが、`fn` に別名と登録名の両方が部分一致する店舗で重複表示を生まないか**（`filter` なので1行は1回しか出ないが、期待件数の説明が変わる）を見ること。
