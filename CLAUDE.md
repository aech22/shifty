...
## Shifty バグチェックレポート（2026-08-11 自動実行 #68）

### 修正済み

- **🟡 有料プラン中のプラン変更が、契約が無いときに無言で「新規契約」の決済ページへ遷移する**（app-admin.js:3953 `requestPlanAction` / :4004 `MyPageTab.changePlan`）
  「有料プラン表示だが Stripe の契約が無い」店舗（手動でプランを付与した本番の自家用12店舗が該当。契約が失効した店舗でも起こりうる）でマイページの「Proプランに変更」を押すと、`changePlan` が 409 `no_subscription` を返し、クライアントは `createCheckoutSession` へフォールバックして**新規の月額契約の決済ページへ遷移する**。ところが直前に出る確認文は「Premiumの機能は今の期間の終わりまでご利用いただけます」「日割りの返金はありません」という**プラン変更の説明**で、新しい契約が始まることも金額（500円/月）も自動更新であることも告げていない。フォールバックしたことを `newContract` として返し、そのときだけ金額・自動更新・切替時点を示す確認を取り直すようにした（コミット `e1c0377`）。契約があって `changePlan` が成功する通常経路と、Free からの購入（`UpgradeModal` 側に定期購入の説明が出ている）は従来どおり。

### 今回の対象と見つけ方

#66 以降コードの追加は無く（`c895548` 以降 develop に新規コミット0件）、**#67 が自分で残した宿題を実行する回**になった。#67 の総括は次の2手を指定していた——①新しいモードやゲートを足したら「Firebase直書き／Cloud Functions／外部遷移」の**3つの出口すべて**にガードがあるかを表にする（特に `ownerReadOnly` はCFを呼べるのか、確かめた記録が無い）②本番でだけ再現する条件をdevとの差分として持つ。

この2つを実行した結果、**3つ目の出口＝外部遷移（`window.location.href` によるStripeへの遷移）が、ガードの有無ではなく「遷移の直前に何を約束したか」という別の軸で壊れていた**。①②はガードの有無で数えられるが、③だけは「遷移すること自体は正しい。ただし利用者が同意した内容と違う」という形の壊れ方をする。#67 が「出口を数える」と書いたときに想定していたのは塞ぐ／塞がないの二値だったが、外部遷移は塞ぐべき経路ではないので、二値で数えると素通りする。

### 検証したこと

- **修正前の挙動を localhost で実測（Claude Browser・標準テスト店舗 `eb6AfsQv4JAht+cX*xP7fuDa`・`?plan=premium`）**: `fetch` を横取りして `/changePlan` に 409 `{code:"no_subscription"}` を返させたところ、`/changePlan` → `/createCheckoutSession` の順に飛び、返した決済URL（`about:blank#FAKE_CHECKOUT` に差し替え）へ**実際にページ遷移した**（タブがアプリを離れたことで確認）。Stripe は一切呼んでいない。
- **確認文の実物を採取**: 1つ目の確認文は「Proプランに変更します。／・Premiumの機能は今の期間の終わりまでご利用いただけます／・その翌日からProに切り替わります／・日割りの返金はありません」。**この文面のまま新規契約の決済ページへ送られていた**のが今回の問題。
- **修正後を実測**: 同じ経路で2つ目の確認文「この店舗には有効な契約が見つかりませんでした。／続けるとProプランの『新規のお申し込み』になります。／・Proプラン 500円/月（税込・1店舗あたり）／・毎月自動で更新される定期課金です／・お支払いが完了した時点でProプランに切り替わります／決済ページへ移動しますか？」が出ること、**キャンセルすると遷移しない**（`location.href` 不変）ことを確認。
- **非回帰**: `changePlan` が 200（期間終了時ダウングレード）を返す通常経路では確認は1回だけで、追加の確認は出ず、遷移も起きないことを実測。コンソールエラー **0件**。`npm test` **168件パス**、`npx eslint app-*.js` **0 errors 98 warnings**、`node --check functions/index.js` OK。
- **#67 の宿題に対する答え（否定的な結果として記録）**: **閲覧専用端末（`ownerReadOnly=true`）は課金系Cloud Functionsを呼べない。** `createCheckoutSession`・`changePlan`・`createPortalSession` はいずれも `verifyShopOwner`（functions/index.js:89）を通り、`owners` が存在して自uidが含まれない端末は 403 で弾かれる。`ownerReadOnly=true` は「ownersが存在し自分がそこに居ない」ことと同義なので、この経路は閉じている。企業系Callableも管理コード（`shopId.adminKey`）の提示が必要で同様。**ただし出口①だけは閉じていない**（下記🟡）。
- **未検証（正直に記載）**: 下記🟡の `resolveShopMeta` の非対称は**コードを読んで確認しただけ**で、実際に食い違いが起きるかは Stripe がスケジュールのフェーズmetadataを契約へ適用するかに依存し、**Stripeの状態を再現できないため未検証**。今回の修正はクライアントのみで、**Cloud Functions・本番への反映はしていない**（#67 の🔴が依然として未リリースであることも変わっていない）。
...
- Cloud Functions: `runWith` の secrets **6箇所**・`.delete()` 誤用 **0件**
- index.html の読み込み順 utils→core→staff→admin→main 維持・SRI **11本**維持・`?v=20260811-83a7bf5`（**本番と同値のまま。#67 の🔴リリース時にバンプが必要**）
- `input`/`select`/`textarea` の `fontSize<16` **0件**

### 要確認（未修正）

- **🟡（新規）解約イベントだけが「契約の実際のprice」ではなく metadata でプランを判定している**（functions/index.js:319 `resolveShopMeta` / :557）→ **BACKLOG化済み**。同関数は冒頭で `if (md && md.shopId) return {shopId: md.shopId, plan: md.plan || null}` と早期returnする。**イベント本体が Subscription そのものである `customer.subscription.deleted` / `.updated` は必ずこの行で返るため、同じ関数が :336 で行っている「プランは metadata ではなく契約の実際のprice から解決する」という処置がこの2イベントには届いていない**。:336 のコメント自身が「metadata を信じると降格を巻き戻す」と書いており、`99d8cd5` はそれを直した修正だったが、**適用されたのは invoice 経路（subscriptionを retrieve する分岐）だけだった**。実害の形: `customer.subscription.deleted` は受け取った plan と DB の現行プランが食い違うと「別契約の解約」とみなして**ダウングレードを丸ごとスキップする**（:563）ため、一度でも食い違うと**解約しても店舗が有料プランのまま残り、以後イベントは二度と来ないので自動回復しない**。なおこのスキップ判定の前提だった「1店舗2契約」は、`createCheckoutSession` が有効契約のある店舗を409で拒否するようになった時点で構造的に起こらなくなっており、**ガード自体の要否も含めて見直す価値がある**。
- **🟡（新規）「閲覧専用」バナーが出ている端末でも、提出データの書き換え・削除は実際に通る**（app-admin.js:164 のバナー / database.rules.json:52 / app-main.js:1538）→ **BACKLOG化済み（仕様判断）**。`subs` の書き込みルールは `auth != null` だけで owner を要求していない。これはスタッフがURLから提出できる必要があるため**ルール側では分離できない**（スタッフはオーナーではない）。結果として、管理コードを持たない端末は設定・期間・スタッフの変更こそ拒否されるが、提出一覧・シフト作成タブのセル編集と提出の削除は通る。バナーの文言（「閲覧のみ」）と実際の権限が食い違う。クライアント側で止める案には、`ownerReadOnly` が Firebase 初期化前・匿名認証の遅延でも true になる（app-main.js:541）ため**正当なオーナーが一時的に編集不能になる**副作用があり、文言を実態に合わせる案と併せてユーザー判断が要る。
- **🟢（新規）マイページに `ownerReadOnly` が渡っていない**（app-admin.js:209 では `SetTab` にのみ渡す / :3976 `MyPageTab` の引数に無し）。閲覧専用端末にもプラン変更ボタンと請求管理ボタンが表示される。押しても上記のとおりCF側で403になるためデータ上の実害はないが、押せないボタンを出している。
- **🟢（新規・記述の訂正）BACKLOG の「自家用12店舗はプラン変更ボタンを押しても何も起きない」は誤りだった** → BACKLOG側に訂正済み。実際には決済ページへ遷移し、完了すると Premium 表示の店舗が Pro の実課金に切り替わる。**「押しても何も起きない」を前提に『対応不要』と判断していたら、判断の前提自体が間違っていた**ことになる。#66→#67 と続けてきた「ドキュメントの記述が実在するか照合する」スキャンを、識別子だけでなく**挙動の記述**にも広げたことで見つかった。
- 以下は #46〜#67 からの継続（今回も状態変化なし）: **#67 の🔴（デモURLの課金・オーナー権限の穴が develop 止まりで本番未反映）**／未claim店舗を誰でも自分の企業に取り込める根の未決着／`findActiveSubscription` の `|| live[0]` フォールバック／期間終了時ダウングレードの `planExpiry` 順序依存／`subscription_schedule.*` の4分岐が購読設定上まだ走らない／`planExpiry` のUTC基準計算／決済失敗フラグが別契約の請求成功で解除される／片側セルだけ入力された日の勤務時間が0分／`purgeOldPeriods` のカットオフがUTC日付／`getBreaksFor` が残業前の時刻で休憩適用を判定／`generateInviteCode` と2 stateのデッドコード／`urlLocked` が StaffView で未使用／`src` 引数を持つ6関数の可変性が未使用／`staffSectionOn` だけが `isCountExcluded` を見ていない／SubsTab 詳細モーダルの「月計」が別名subを重複加算しうる／`allAbbrs` の先読みが `saveMetaField` 直後の値を一時的に巻き戻す／`sub.shopId` が死にフィールド／店舗メニューの店舗削除が `global/shops/{id}` を消さない／`companyCodes/{code}` 生成の TOCTOU／略称の「先勝ち」／期間の手動作成に「終了日 ≥ 開始日」検証なし／提出状況ビューのセル編集がスタッフ提出値を書き換える／Excel出力の analytics が管理者のグリッド入力を提出として数える／マイページの「スタッフ数」バーがスペーサーを人数に数える／属性削除後も `breakTimes[].tags` に残る／Excel出力とシフト作成タブで別名解決順が違う／`heatHours` が `adminRest` を無視／`_fbGuard` の本番分岐が PostHog へパスを送る／スタッフ削除時に設定マップのキーが残る

### 総括

**「同意を取ったこと」と「実際に起きること」がずれていた。** 今回の🟡は、ガードの抜けでもロジックの誤りでもない。`changePlan` が使えないときに `createCheckoutSession` へ落とす分岐は、契約が失効した利用者を救うための**意図された設計**であり、それ自体は正しい。壊れていたのは、その分岐に入る前に表示した確認文が「プランの変更」の説明のままだったことだけである。コードとしてはどこも間違っていないのに、利用者から見ると「解約や返金の話をされて OK を押したら、知らない金額の決済ページに立っていた」という結果になる。

**#67 が指定した「出口を数える」という手は正しかったが、数え方に穴があった。** 出口①（Firebase直書き）と②（Cloud Functions）は「塞いだか／塞いでいないか」の二値で数えられる。しかし出口③（外部遷移）は**塞ぐべきではない出口**であり、二値で数えると常に「ガード不要」と分類されて検査対象から落ちる。実際 #67 は出口③を列挙していながら、そこを調べずに終わっている。**外部遷移の出口は「塞いだか」ではなく「遷移の直前に利用者へ何を約束したか」で検査する**——これが今回得た検査軸である。今回の修正でこの軸は満たしたが、同じ軸で見るべき遷移はもう1つある（`openPortal` によるStripeポータルへの遷移。こちらは遷移先が「請求管理」で説明と一致しており問題はない）。

**もう1つ、今回は「記録が判断の前提になっているとき、記録の誤りは判断の誤りになる」という形の再発を見た。** BACKLOG に書かれていた「押しても何も起きないUXになる」は、実測すれば5分で誤りと分かる記述だったが、そこから「ボタンの出し分けが要るか要判断」という**軽い扱い**が導かれていた。実際には無言で課金導線へ送っていたのだから、優先度の判定そのものが誤っていたことになる。#66 が「ドキュメント記載の識別子が実在するか」を照合し、#67 がそれを実施したが、**照合対象は識別子（関数名・state名）だけで、挙動の記述は照合していなかった**。

ここから次回の手が2つ出る。

**1つ目。ドキュメントの照合を「識別子が在るか」から「書いてある挙動が本当か」へ広げる。** 対象は限定してよい——**BACKLOG と CLAUDE.md のうち、『実害はない』『何も起きない』『問題ない』と実害を否定している記述**だけを集め、1つずつ実測で裏を取る。実害を否定する記述は、それ自体が「対応しない」という判断の根拠になっているため、誤っていたときのコストが最も大きい。今回の1件はまさにこの型だった。同型の記述は現在の BACKLOG に複数ある（「データは壊れない」「実害なし」「実害は軽微」等）。

**2つ目。今回できなかったCloud Functions側の検査を、次回は Stripe の実データで閉じる。** 上の🟡（`resolveShopMeta` の非対称）は**コードの非対称は確実だが、発火するかは Stripe の挙動次第**という宙ぶらりんの状態で残っている。これは BACKLOG に既にある「実購入での全遷移検証」（Pro購入 → Premiumへアップグレード → Proへダウングレード → 解約）と**同じ1回の操作で確認できる**。ダウングレードが期間終了で発火した直後に、Stripe上の subscription の `metadata.plan` を1回読むだけで、フェーズmetadataが適用されるのかどうかが確定し、この項目は「実害あり」か「多重防御として正しく効いている」かのどちらかに閉じられる。**コードだけでは決着しない項目を、決着する1回の操作に紐付けて持ち越す**のが、宙ぶらりんを次回も繰り返さない唯一の方法である。
