...
**色も斜線も使わず、セルに種別名を出す。** 公休＝「公休」／有給＝「有給」／慶弔＝「慶弔」を
**出勤・退勤の両方のセル**に出す（片方だけだと半日の休み希望と見分けがつかない）。
- コマンドは `ko`＝公休・`yu`＝有給・`ke`＝慶弔。`y` を出勤・退勤の両方に入れても公休になる。
- **種別名を出すセルには斜線を引かない**（文字と重なって読めなくなる）。スタッフ提出の休みは
  種別名を出さないので従来どおり斜線のまま。
