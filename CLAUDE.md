...
## Shifty バグチェックレポート（2026-08-20 自動実行 #86）

### 修正済み

- **🟡 先頭が数字の店舗略称が登録できてしまい、他店舗ヘルプ判定も店舗間シフト重複判定も無言で効かなくなっていた**（[app-admin.js:3212](app-admin.js)）→ `3a9b818`
  `addAbbr`（CompanyTab）の予約語チェックは `/^[\d.:]+$/`＝**数字だけの略称**を弾く。しかし実際にセルを解釈する `extractNote`（app-utils.js:480）のパース境界は `^([\d.:]+)(.*)$` で、**先頭の数字・`.`・`:` を時刻部として貪欲に取る**。両者の境界が食い違っているため、「2号」「2号店」「2F」のような**先頭だけが数字の略称は登録を通ってしまう**。
  **通ったあと何が起きるか**: 管理者がセルに 9時＋「2号」の意で `92号` と入力すると、`numeric="92"`（`parseTime` が2桁92を弾いて**時刻が消える**）・`note="号"` になる。`abbrToShop` は完全一致lookupなので `"号"` では引けず、`getHelpInfo` が `null` を返す。結果、**ヘルプ帯のヒートマップ除外も、店舗間シフト重複の赤セル・エラーパネルも一度も発火しない**。時刻を付けず `2号` だけ入れた場合はさらに悪く、`numeric="2"`→`parseTime`→**02:00 の出勤として保存される**。
  **パーサ側では直せないことを確認した**。`92号` を「9＋2号」と「92＋号」のどちらに読むかは原理的に決められない。したがって直せるのは登録側だけで、**検査をパーサの境界に一致させる**のが唯一の整合解になる。既存の検査が「数字のみは時刻と読まれるから不可」という**同じ理由**で置かれていた（トースト文言に明記）ため、仕様判断は不要だった。
  **修正**: `/^[\d.:]+$/`（全部が数字）→ `/^[\d.:]/`（先頭が数字・記号）に変更。トーストも「数字のみ」→「数字や記号（. :）で始まる」に合わせた。
  **検証（配信物の実評価）**: `app-utils.js` を Node で読み込み、21パターンの略称について「登録できるか」と「`9`+略称 の入力で `note` が略称と完全一致するか」を突き合わせた。**拒否に変わったのは機能しない4件（2号・2号店・2F・9時）だけ**で、機能する略称（三・東・西口・号2・A1・：東〈全角コロン〉・ｷｯﾁﾝ）は全件許可のまま。修正後に許可される略称のうち `note` が一致しないものは**0件**。
  **未移行（申し送り）**: 既に先頭数字の略称を登録済みの店舗があれば、その値はデータに残り機能しないまま。今回の修正は新規登録を止めるだけで、既存値の掃除・警告表示は含まない。

### 要確認（未修正）

- **🔴（継続・プロセス）このループから実機E2Eが9回続けて実行できない**（環境要因・コード変更なし）
  今回も `preview_list`（稼働0件）と `preview_start` を実際に実行し、**「Dev servers can't be started from unattended sessions」と明示的に拒否された**。Bashでのサーバー起動は規約で禁止のため代替なし。**#78〜#85 に続き今回も1件も実行できなかった。ユーザーが対話セッションで `/bug-check` を手動実行すれば解消する。**
- **🟡 #77 の宿題（Free 上限の実機検証）は静的監査完了・実測のみ未達**（上記🔴が理由）。**残るは実機で21人目・2件目を押す1回だけ**
- **🟡（新規）店舗間シフト重複の判定が、退勤延長（残業）の時間を見ていない** → BACKLOG化済み（仕様判断待ち）
  `dupErrors`(:855) は自店舗の勤務帯を生の退勤時刻で取り `getOT` を加算しないが、`heatData`(:797) は「延長中の時間帯も出勤扱いにする」と明示して加算する。そのため**延長ぶんだけが他店舗と重なるケースは検出されない**（自店舗 10:00〜17:00＋延長60分／他店舗 17:30〜 → 17:00〜18:00 が二重だが無検出）。延長は `calcNetWorkMinutes` にも入る＝勤務時間として数えている時間なので、「数えているのに二重予約では見ない」形になっている。
  **今回は直さなかった。多数派に揃えるだけでは決まらないため。** 同じ `realStaff` を回す3計算のうち OT を足すのは `heatData` の1つだけで、`positionErrors` も足さない＝**多数派は「足さない」側**。#84・#85 でやった「少数派を多数派へ揃える」を機械的に当てると `heatData` の方を削ることになり、そちらには明示的な設計意図がコメントで書かれている。加えて**他店舗側の延長は現状読めない**（`companyData` は `shopAbbrs`・`subs`・`staffAliases` しか読まない）ため、案によっては取得を1パス増やす判断も伴う。
- **🟡（新規）`dupErrors` だけが「片方のセルしか埋まっていない日」を判定対象から外している** → BACKLOG化済み（仕様判断待ち）
  `:854` の `if(s===null||e===null)return;`。`heatData`(:788-789) と `positionErrors`(:898-900) は片側セルを `HEAT_LUNCH_END_MIN`／`HEAT_DINNER_START_MIN` で補完して出勤扱いにするのに、**`dupErrors` だけは補完せず抜ける**ため、片側しか埋まっていない日は店舗間重複を一度も見に行かない。既出の「片側セルだけの日を何時間と数えるか」（#57から継続）と同じ根だが、**その項目でまだ列挙されていなかった箇所**なので追記した。
- **🟡 `x`（ヘルプ・カウント外）のセルが店舗間重複の判定から除外されていない**（#85）→ BACKLOG化済み（仕様判断待ち・変化なし）
- **🟢（新規・申し送り）自由メモにも同じ「先頭の数字が時刻に食われる」性質がある**が、こちらは直せない
  `9研修` が `numeric="9"`・`note="研修"` になるのと同じ仕組みで、`92名体制` のような**先頭が数字のメモ**は `numeric="92"` に食われて時刻が消える。ただしメモは登録・照合の工程を持たない自由入力で、`92` を「9＋2名体制」と読む解釈も同様に成り立つため、**略称と違って検査で止める場所がない**。仕様として受け入れるしかない性質として記録に残す（実害は入力し直しで解消する）。
- **🟢（新規・申し送り）略称のlookupは大文字小文字を区別するが、登録時の予約語チェックは区別しない**
  `extractNote`(:499-501) はサフィックスが `CELL_COMMANDS` に一致したときだけ小文字化し、それ以外は**打った通りの文字列**を `note` にする。一方 `addAbbr` の予約語チェックは `toLowerCase()` 比較。そのため略称 `AB` を登録した店舗では `9AB` と打つ必要があり `9ab` では引けない。exact-match は `:540` のコメントで意図として明記されている挙動なので誤りではないが、非対称ではある。
- **🟢（申し送り・確認済み）期間ドロップダウンの切替で `localEdits`／`heatEdits` は正しくクリアされている**（`:1542`）
  「期間Aで編集 → 期間Bへ切替 → 保存を押す」で期間Aの日付が期間Bのsubへ書き込まれる、という経路を疑って追ったが、**`onChange` が両バッファを消しているため成立しない**。ただし `:437` の自動補正（`periods` が非同期到着 or 期間削除で `selPid` が無効になったときの `setSelPid(periods[0].id)`）だけは**クリアを伴わない**。到達するにはシフト作成タブを開いたまま期間が消える必要があり、タブ切替でこのコンポーネントはアンマウントされるため実質到達不能。**今回は変更していない**（申し送りのみ）
- **🟢 `effShiftStart`/`effShiftEnd` の未採用が app-admin.js になお3箇所残る**（`:1005` ヒートマップの時間軸／`:3051` 提出一覧の出勤日数／`:3109` 詳細モーダルの出勤日数）。**#82〜#85 と同じ理由で今回も触らなかった**——直し方が BACKLOG の判断待ち項目（片側セルだけの日を何時間と数えるか）に依存する
- **🟢 `index.html` の `?v=` と `build:` が現在の配信物を指していない**（`20260814-53f610f` のまま。**今回さらに app-admin.js を1回変更したので差は広がった**）。申し送りのみ——`/release-to-main` の手順63がバンプする
...
- 以下は #46〜#85 からの継続（変化なし）: 未claim店舗の取り込み／`findActiveSubscription` の `|| live[0]`／`planExpiry` のUTC計算／**片側セルだけの日が0分**／`purgeOldPeriods` のUTC日付／`generateInviteCode` のデッドコード／ほか

### 今回の対象と見つけ方

**#85 以降のコミットは0件。14回続けて「差分ゼロ」の回である。**

**(1) 申し送りの指示を実行し、他店舗から読む3経路を並べた。** #85 は「`companyData`（略称の突き合わせ・`getHelpInfo` の略称→店舗解決）と `shopMeta`（CompanyTab）は同じ『他店舗から読む』経路なのに重複判定しか追っていない」と名指しで残していた。実際に3経路（`companyData`・`shopMeta`・`allAbbrs` 先読み）を並べたところ、**キーの引き方という軸では3つとも白だった**——`shopMeta` は他店舗の `staff` を読んでその店舗自身の名前空間の中だけで使い、`staffWorkplaces` の書き込みも同じ名前空間で閉じている。#85 で1つ増えた `otherAliases` も、参照側が1箇所しかなく少数派になっていない。

**(2) 空振りの過程で、略称という「文字列そのもの」に軸が移った。** `abbrToShop` の突き合わせを追ううちに、比較しているのが**登録時に検査を通った文字列**と**セルのパーサが切り出した文字列**の2つで、**それぞれ別の場所で別の規則が定義されている**ことに気づいた。#85 が「レンズを尽くすとレンズをずらすは違う」と書いた形が今回も再現した——指示された場所は空振りで、そこを見る過程で軸がずれて出た。

**(3) 決め手は「同じ理由で置かれた検査が、境界だけ足りていない」ことだった。** `addAbbr` の数字チェックは、トースト文言（「数字のみの略称は使用できません」）が示すとおり**「数字は時刻として読まれるから」**という理由で置かれている。パーサの実際の境界は `^([\d.:]+)` ＝ 先頭の連続した数字・記号。**理由は正しく、境界の定義だけが `+$`（全部）と `[` （先頭）でズレていた。** 意図が明文で残っていたので仕様判断が不要になり、「意図に実装を合わせる」だけで閉じた。

**(4) 「パーサ側では直せない」ことを先に確定してから、登録側を直した。** `92号` を「9＋2号」と「92＋号」のどちらに読むかは情報として区別できない。ここを確かめずに `extractNote` に手を入れていたら、既存の `9研修`（数字＋任意メモ）の解釈まで壊していた。**直せる場所が1つしかないと分かってから触った。**

**(5) 修正の影響範囲を、意見ではなく実測で確定した。** バリデーションの変更は「今まで登録できたものが登録できなくなる」変更なので、**新旧2つの述語と `extractNote` を同時に走らせ、21パターンについて「登録可否がどう変わるか」と「そもそも機能するのか」を1枚の表にした**。拒否に変わった4件が**全て機能しないもの**であること、許可のまま残った略称が**全て機能する**ことを確認してからコミットした。全角コロンの `：東` が影響を受けない（`[\d.:]` は半角のみ）ことも同じ表で確認できた。

### 検証したこと

- `DEV_MODE`（app-core.js:12・式のまま）／`DEV_PLAN_OVERRIDE`（:125）正常
- `subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／`global/shops` の全件読み **0件**（直キー読みのみ）／`global/templates` **0件**／CF `.delete()` **0件**
...
- **文字列の境界のレンズ（今回の主レンズ）**: ユーザーが登録した文字列を後からパーサが切り出して突き合わせる経路について、**登録側の検査規則とパーサの境界規則を並べた**。`addAbbr`(:3212) の `/^[\d.:]+$/` と `extractNote`(:480) の `^([\d.:]+)` が不一致（→修正）。予約語チェック（`CELL_COMMANDS` レジストリ駆動）と長さ制限（4文字）は一致
- **他店舗から読む3経路の棚卸し（#85の申し送りを実行）**: `companyData`(:385)・`shopMeta`(:3160)・`allAbbrs` 先読み(:3196) について、読んだ値をどの名前空間のキーで引くかを分類。**3経路とも白**。#85 で追加した `otherAliases` の参照元が1箇所のみで少数派化していないことも確認
- **`localEdits`/`heatEdits` のライフサイクル**: `setSelPid` の呼び出し元2箇所（`:437` 自動補正・`:1542` ドロップダウン）を洗い出し、ドロップダウン側がクリアを伴うことを確認（→期間跨ぎの誤書き込みは成立しない）
- **修正の検証（配信物の実評価）**: 21パターンの略称で新旧バリデーションと `extractNote` の結果を突き合わせ。拒否に変わった4件は全て `note` 不一致（機能しない）、許可のまま残った略称は全て `note` 一致（機能する）
- **未検証（今回は手段が無い）**: ブラウザでの実描画すべて／**修正後に企業連携タブで実際にトーストが出ること**／**先頭数字の略称を既に登録している本番店舗があるか**／**Free 上限に達した店舗での実ブロック**／実機 iOS Safari

### 総括

**測れない回が9回続き、それでも 🟡 が1件出て直った。** ただし今回の根は #81・#84・#85 の「別名」系列とは別で、**「同じルールが2箇所に別々の書き方で存在し、境界だけズレていた」**という形だった。#85 までの3回はインデックスのキーの作り方——つまり**データの引き方**——を見ていたが、今回はデータですらなく**文字列の切り方の取り決め**だった。

**実害は #85 とよく似た「黙って出ない」形で、しかも重なっている。** 先頭が数字の略称を登録した店舗では、ヘルプ帯のヒートマップ除外も店舗間シフト重複の赤セルも一度も出ない——**#85 で直したのと同じ機能が、別の入口からもう一度死んでいた**。#85 は「別名で提出している人」で死に、今回は「先頭が数字の略称を付けた店舗」で死ぬ。どちらも画面には何の痕跡も残らず、機能自体は「動いている」ように見える。**この機能は2回続けて、別々の理由で無言の不発を起こしていたことになる。**

**申し送りは今回も空振りだったが、実行したこと自体は無駄ではなかった。** #85 が名指しした3経路は全て白で、「まだ見ていない場所」を見ても何も出なかった。しかしその作業の中で `abbrToShop` の突き合わせが視界に入り、**軸が「どのキーで引くか」から「そのキーはどう作られるか」へずれた**ところで出た。#84→#85→#86 と3回連続で同じ形になっている——**申し送りは正解の場所を当てるためではなく、軸をずらすきっかけとして機能している。**

**次回の手を1つに絞る。** 実測が復活したなら**#77 の宿題（実機で21人目・2件目を押す1回）をそのまま実行する**。使えないままなら、推奨は**「同じ制約が2箇所以上に別々の書き方で書かれている組を全部並べる」**——今回の1件は「理由は正しく、境界の定義だけが違っていた」形だった。候補: `parseTime`(app-admin.js:478) の受理範囲（0〜30時）と `TO`/`gto`(app-utils.js) の生成範囲（0:00〜27:00）が食い違わないか／`addAbbr` の4文字制限と、Excel・PDFの列幅・縦書き縮小（`vfontSize` の5文字基準）が想定する名前長／`database.rules.json` の `.validate`（`staffName` 50文字・`name` 100文字）とクライアント側の入力制限。**あわせて、今回1つ厳しくした略称バリデーションが、既存データを持つ店舗で「登録済みなのに追加できない」等の非対称を生んでいないか**を見ること。
