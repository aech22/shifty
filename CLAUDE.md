...
## Shifty バグチェックレポート（2026-09-16 自動実行 #129）

> 着手時の HEAD は `41f202d`。**#126 以降コードの変更は無い**（docs のみ）。#128 の申し送り
> （クライアント側の購読順序）を先に当て、白だったので Webhook から離れて未監査の領域へ移った。

### 修正済み

なし。🟡を1件検出したが、直し方の選択にユーザー判断が要るためコードは変えていない。

### 要確認（未修正）

- **[🟡] 管理者が退勤を出勤より前の時刻で入力すると、シフト表には正しく見えるのに勤務時間・ヒートマップ・上限判定が黙って0になる**（app-utils.js `effShiftRangeMin` / app-admin.js `applyEditToSubs`・`saveAdj`）→ **BACKLOG化済み（新規タスク）**。
  深夜営業の店舗で 22:00〜翌2:00 を、退勤セルに **「2」** と入れたときに起きる。正しい入力は24時超え表記の「26」で、候補時間の選択肢（`gto()` は 0:00〜27:00）もその表記だが、**シフト作成タブのセルは自由入力**でそれを伝える導線が無い。
  原因は `effShiftRangeMin` の `return e>s?{...}:null;`。退勤 ≤ 出勤 の日を null にし、`calcNetWorkMinutes`・`shiftBandInfo`・`getBreaksFor` がそろって「勤務時間なし」として扱う。**捨てたことを伝える経路がどこにも無い。**
  **実測**（配信物の関数を Node で実行。`parseTime` は app-admin.js のソースから切り出して実行）:

  | 管理者が入力した退勤 | 実効レンジ | 純勤務 | 出勤数 | 「両側入力」と判定 |
  |---|---|---|---|---|
  | 22:00 → **02:00** | **null** | **0:00** | **0** | **true** |
  | 22:00 → 26:00 | あり | 4:00 | 0.5 | true |

  `expXl` は保存値をそのまま印字するので**Excel には「22 / 2」と正しく出る**＝配るシフト表は正しく集計だけが間違う、という気づきにくい壊れ方。**候補時間の3経路と休憩の入力欄は同じ条件を弾いている**のに、管理者の編集経路2つだけが素通りする点が決め手だった。
  **直さなかった理由**: 弾く／24時間足す／表示だけ警告、のどれを採るかで結果が変わる（条件B。セルは1つずつ確定するので単純に弾くと編集の途中経過まで弾かれる）。加えてセル編集フローは #51・#56・#58 の回帰が起きた場所で実機E2Eが前提（条件D）。
- **#128 の申し送りは白**: `startSubscriptions` は 361行で `off()` を済ませてから 377行でリセットするので、`billingSchedule` に前店舗の値が残る経路は無い。ただし `plan`・`planExpiry`・`paymentFailed` は同期リセットされず、購読が返るまでの1往復だけ前店舗のプランが残る（自己修復・🟢）。
- **[🟢] 今回の新規**: 企業連携3本（`createCompany`・`linkStoreToCompany`・`unlinkStoreFromCompany`）に `isValidShopId` が無い（**すり抜けは成立しない**ことを確認済み。多重防御の提案どまり）／他店舗略称に「締」を含む文字列を登録できる（予約語判定が完全一致・`extractNote` の `hasFixed` は部分一致）／略称の重複チェックに先読み待ちの隙。
- **#127・#128 から継続**: 解約済み契約の請求書支払いで有料プランに戻る／`subscription.updated` が順序を疑わず書く（🟡・同じBACKLOGタスクに集約・Stripe設定の確認とCFデプロイ待ち）。
- **#124 から継続（変化なし）**: `fixedShiftCommandFor`・`recentPeriodIds` がテスト専用／`fmtH4` の残骸／`subsWindowCutoff` の月末繰り上がり／`staffAliases` 欠落時のメモ作り直し2件／**配信版数 `20260911-fe54a15` が `e8c2980` に追随していない**（リリース時にバンプ）。
- **変化なし（BACKLOG化済み）**: 二重課金の根治／特商法表記（🔴）／解約通知／解約時のプラン判定／`verifyShopOwner` の移行猶予（#125・#126 のCFデプロイもここ）／別名提出の重複の根（#81）／PDFの実物確認／`purgeOldPeriods` の本有効化／非表示スタッフの未提出カウント（#113）／「締」の休みの入口2つ（#118）／完全削除したスタッフの `keepAttrs`（#118）。
...
- RULES.md スキャン項目はすべてクリア（`DEV_MODE` は式のまま・`subs` 全体 `set()` 0件・`accounts` 全件読み0件・SRI 11本・`.delete()` 0件・`secrets:` 7件・読み込み順正常）。fontSize 走査: フォーム部品58件・16未満0件。
- **`matchPositionSlots` を総当たりと突き合わせて白を確認**: 全割当を探索する実装を別に書き、枠1〜6・出勤者0〜6・ポジション4種を乱数で **4000ケース** 回して `matchedCount` を比較 → **不一致0件**。不足数の合計が「枠数 − マッチ数」と一致することも確認。
- ユニットテストが1度も触れない公開関数を機械的に洗い出した（89件中10件。実体のある関数は `buildSuggestList` のみ）。
- スタッフ非表示の範囲・`resolvePeriodMaster`・`applyKeepAttrs`・`database.rules.json`・`e8c2980` の差分を読み、**問題を見つけなかった**。
- **Firebase・Stripe には一切アクセスしていない。** 実ブラウザでの確認も行っていない。

**申し送り（次回の観点）**: 今回の🟡は「他の入力欄は弾いている条件を、管理者の編集経路だけ素通りさせる」形だった。同じ形が他にもないかを当てるとよい——`applyEditToSubs` と `saveAdj` は候補時間（`settings.candidates`）を一切参照していない。スタッフは候補からしか選べないのに管理者は候補外の任意時刻を入れられる点が、意図か取りこぼしかを先に確かめるとよい。
