...
## Shifty バグチェックレポート（2026-09-02 自動実行 #105）

> 着手時の HEAD は `6305a58`。**#104 以降に入った実コードの変更はゼロ**（#104 自身の `abcc668` と
> ドキュメント1件だけ）。同じ配信物なので機械スキャンは「基準線が動いていないこと」の確認に留め、
> **時間はコード読解へ回した**（#104 と同じ方針）。
> 見つけたのは #102〜#104 とまったく同じ形——**同じ規則を2箇所に書いたら、片方だけが規則を守っていなかった**。
> ただし今回は一段悪い。**守っていない側について、コードのコメントが「もう片方と同じだ」と名指しで
> 主張していた**（app-admin.js:556「Excel出力（expXl）と同じ別名解決」）。読んだ人は確認しない。

### 修正済み

- **🟡 同じ期間で、画面（シフト作成グリッド・PDF）とExcelが別の提出データを見ることがある**（[app-admin.js:2290](app-admin.js)）→ `e2fe16a`

  別名で提出した人は `sub.staffName` に別名が残る（`registerAlias` は `staffAliases` に登録するだけで
  `staffName` を書き換えない）。そのため同一期間に「登録名のsub」と「別名のsub」が併存しうる。
  どちらを採るかの規則は**3箇所が「完全一致を優先し、無ければ別名」で揃っていた**——
  `_getSubForPeriod`（グリッド）・`_getWorkShift`（週集計）・`_shiftAt`（提出一覧）。
  **`expXl` だけが `ss.find(登録名一致 || 別名一致)`** ＝ 配列の先頭に近い方を採っていた。
  `ss` は `subs.filter(...)` で並べ替えていないので、並びは **Firebase のキー順（提出時刻順ではない）**。
  つまり**どちらが出力されるかは事実上決まっていない**。

  **実測**（配信物のソース行を切り出してそのまま Node で実行。ルートは `process.argv[2]` で渡す）

  | subs の並び | グリッド・PDF | Excel | |
  |---|---|---|---|
  | 別名sub `A`(退勤17:00) が先 | `B` 18:00 | **`A` 17:00** | 修正前・**食い違う** |
  | 登録名sub `B`(退勤18:00) が先 | `B` 18:00 | `B` 18:00 | 修正前・たまたま一致 |
  | 別名subが先 | `B` 18:00 | `B` 18:00 | **修正後** |
  | 登録名subが先 | `B` 18:00 | `B` 18:00 | 修正後 |

  **管理者は、画面で確認した内容と違うExcelを店舗に配ることになる。**
  そして食い違ったときに気づく手がかりが無い（#84 で月計の二重計上を直したぶん、
  重複の存在に気づく経路はむしろ1つ減っている）。

  直し方は **#104 と同じ形**にした。順序を直すのではなく、**解決規則を `resolveSubByAlias`
  （[app-utils.js:548](app-utils.js)）という1つの純粋関数に出し、4経路すべてをそこへ通した**。
  `expXl` は毎セル `ss.find` を回していたのを、名前→sub の逆引き Map（重複時は最初の1件＝
  `subsByKey` と同じ規則）を1度だけ作って引く形に変えた（日付×スタッフ×subs の全走査も消えた）。

  **非回帰**: 重複が無い通常時（登録名subだけ／別名subだけ）は修正前後で**同じsubを返す**（実測済み）。
  `_getSubForPeriod`・`_getWorkShift`・`_shiftAt` の3つは元から完全一致優先なので**挙動は1バイトも変わらない**。
  ユニットテスト4件追加（**205 → 209件パス**）。`ShiftEditTab` を実ブラウザにマウントした
  ハーネス（`shifty-e2e-verify` 1.6節）で `allPass=true`・**pageerror / console.error ともに0件**。

  **→ BACKLOG #81 の「画面とExcelが別のsubを見る」だけを閉じた**（`e2fe16a` を同タスクへ追記）。
  **根（subが2つできること自体）は #81 のまま残る**が、重複が残っている店舗でも
  Excelがグリッドと違うものを出すことはもう無い。

### 調べて「バグではない」と確定したもの

- **ヒートマップが持つ「片側セル補完の境界」の写しがズレている**と疑ったが、**反証した**。
  `app-admin.js:819` のコメント自身が「`oneSidedFillBounds` と同じ式の3つ目の写し」と認めているので
  本命だと思ったが、両者を突き合わせると**フィルタ条件・境界値（1020分）・フォールバック（900/1020）まで
  一致**していた。`timeToMin` が壊れた文字列に `NaN` を返す（app-utils 側は `null`）差はあるが、
  直後の `m<=1020` が `NaN` を落とすため結果は同じ。
- **ヒートマップと店舗間重複判定で退勤延長の帯判定が食い違う**と疑ったが、**反証した**。
  ヒートマップは補完後の退勤を明示して `getOT` に渡し、`dupErrors` は生のシフトを渡して `getOT` 内部の
  `effShiftRangeMin(settings)` に補完させる。**入口は違うが同じ `oneSidedFillBounds` を通るので答えは同じ**。
- **同じ形の `find` が他にもある**と疑い、`app-*.js` の `.find(` かつ `||` を全件洗い出したが、
  **該当は expXl の1件だけだった**。`app-staff.js:168` の `existSub` は
  `完全一致 || 別名一致` の**「式の連鎖」**（`c889660`）で、1つの述語の中の OR ではない＝完全一致が必ず優先される。
  残りはすべて `find(...) || フォールバック値` の形で、優先順位の問題を持たない。
- **PDF も同じ穴を持つ**と疑ったが、**反証した**。`buildPdfCols` は列の名前だけを組み立て、
  セルの値は `getEffHHMM`／`_getSub` 経由＝グリッドと同じ経路。今回の修正で自動的に揃っている。

### 要確認（未修正・#104 から変化なし）

- **🟢 `database.rules.tightened.json` が現行ルールとバイト同一の残骸** → **BACKLOG化済み**。
  今回も `cmp` でバイト単位まで同一であることを再確認した。消すか残すかは**条件B（判断）**。
- **🟢 片側セルの日は「→15:15（+15分）」の実効退勤バッジが出ない**（app-admin.js:3534付近）。
  勤務時間には延長が乗るのにバッジの条件が生の `end` を要求するため。表示のみ。
...
- **🟢 `purgeInactiveShops` が、もう存在しない `inviteCodes` を毎日掃除している**（functions/index.js:808）。
  読み取り1回ぶんの無駄で実害はない。次に `functions/` を触る回に一緒に落とせばよい（#104 から継続・BACKLOG化しない）。
...
- **変化なし・BACKLOG化済み**: 秘密鍵ログ修正の本番反映／二重課金の根治／特商法表記（🔴 2件）／
  解約通知／解約時のプラン判定／企業連携の解除／**別名提出の重複（#81・根は未解決）**／スタッフ削除の残り確認2件。

### 申し送り（BACKLOG化しない）

- **配信版数が app-*.js の変更に追いついていない**: `?v=`（index.html 5箇所）と `build:`（app-core.js）は
  `20260831-c340d5c` のままで、以後 `6613c78`・`bc7bf2e`・`abcc668`・**`e2fe16a`** が入っている。
  `/release-to-main` の手順63が標準工程としてバンプするため申し送りのみ（#103・#104 から継続）。
...
- **RULES.md のスキャン項目は全項目クリア**: `DEV_MODE` は式のまま／`subs` の `set()` 全体上書き **0件**／
  ...
  SRI **11本**／**フォーム部品の `fontSize<16` 0件**。
- **CSS変数**: 未定義参照 **0行**。
- `npm test` **209件パス**（+4・いずれも新規の `resolveSubByAlias` を直接固定するもの）／
  `npx eslint app-*.js` **0 errors 95 warnings**（増減なし）／`node --check functions/index.js` OK。
- **修正前の配信物に対しては同じ実測スクリプトが「食い違う」と出る**ことを確認済み
  （`e2fe16a` を入れる前の作業ツリーで実行）＝**素通りするテストではない**。
- **devのデータは1バイトも変えていない**（実ブラウザ検証は `ShiftEditTab` の単体マウントのみ。
  `firebaseDB` は `null` のまま）。
- **未検証**: Excel・PDF の実物確認（**13回連続**・ユーザー操作が要る。**今回の修正はまさにExcelの中身を
  変えうる修正なので、次に実物を開く機会にこの期間も一緒に見ておくとよい**）／iOS 実機／本番データ／
  重複subが実際に残っている店舗があるか（#81 の未実施項目）。

### 測り方の記録

比較対象を**「同じデータを2つの経路に流す」**形に組んだ。どちらの経路も**配信物のソース行を
文字列として切り出してそのまま実行**しており、こちらで書き直した「等価なはずのコード」は使っていない。
`expXl` 側の1行には `,sh=sub?.shifts?.[ds]` が同居しているので `ds` も渡す、という具合に、
**切り出した行を改変せずに動かすことを優先した**（改変した時点で「配信物を測った」と言えなくなる）。

途中、共有関数の切り出しが `for` ループの閉じ括弧で止まって `SyntaxError` になった。
**測定手段の故障であってコードの異常ではない**ので、閉じ括弧の判定を「桁0の `}`」へ直して測り直した
（#97 の「全件が同じ異常値を見たら、まず測定手段の故障を疑う」と同じ扱い）。

### 総括

**#104 の結論は正しかったが、まだ足りていなかった。**
#104 は「規則は、それが書かれた関数の中しか縛らない」と書いた。今回はその先がある——
**規則を守っていない側が、「自分は守っている」と書いていた。**

`app-admin.js:556` のコメントは `_getSubForPeriod` を「Excel出力（expXl）と同じ別名解決」と説明していた。
書いた人はそのつもりだったのだろうし、読んだ人もそう信じる。だが `expXl` は完全一致を優先しておらず、
**コメントは事実ではなく意図を書いていた**。#104 の `mergeKeepStaff` は「反例つきの正しい警告」が
届かなかった事例だったが、今回は**間違った保証**が積極的に確認を止めていた。前者より悪い。

**「AとBは同じだ」とコメントに書きたくなったら、それは同じ関数にすべきだという合図**として扱う。
今回も直し方は #104 と同じにした——`resolveSubByAlias` を1つ作り、4経路すべてを通す。
**同じ関数を通すものは、同じだと書く必要すらない。**
