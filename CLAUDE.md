...
## Shifty バグチェックレポート（2026-09-03 自動実行 #106）

> 着手時の HEAD は `f04f2d5`。**#105 以降に入った実コードの変更は #105 自身の `e2fe16a` だけ**で、
> あとはドキュメント3件。#104・#105 と同じ状況なので、機械スキャンは「基準線が動いていないこと」の
> 確認に留め、**時間はコード読解へ回した**。
> 見つけたのは **#105 の続き**だった。#105 は「同じ規則を2箇所に書いたら片方が守っていなかった」を
> `resolveSubByAlias` への一本化で閉じたが、**通し忘れた5本目の経路が残っていた**。
> しかも残っていたのは、4経路の中で唯一「スタッフが自分で操作する」経路だった。

### 修正済み

- **🟡 別名が複数登録されている人の再提出が、画面と別のsubを上書きしていた**（[app-staff.js:169](app-staff.js)）→ `8897a25`

  `registerAlias` は `staffAliases` に登録するだけで `sub.staffName` を書き換えないため、
  同じ人の提出が複数の別名subに分かれて併存しうる（BACKLOG #81 の根）。

  #105 は「完全一致を別名より優先する」規則を `resolveSubByAlias`（app-utils.js:548）に集約し、
  グリッド・週集計・提出一覧・Excel の**4経路**を通した。だが**スタッフ再提出の既存sub検索
  （`existSub`）だけがこの関数を通っていなかった**。自前で `完全一致 || 別名includes` と
  書いており、**完全一致の優先はできていた**——#105 はここを見て「式の連鎖なので問題なし」と
  判定しており、その判定自体は正しい。**取りこぼしたのは一段深いところで、
  「別名が複数登録されているとき、どの別名を先に見るか」**だった。

  `subs.find(s=>aliasesOfName.includes(s.staffName))` は配列の先頭に近い方を返す＝
  **Firebase のキー順（提出時刻順ではない）**。画面側は別名の**登録順**で引く。両者は揃わない。

  **実測**（`shifty-e2e-verify` 1.6節のハーネスで `StaffView` だけを実ブラウザにマウントし、
  `onSub` をスパイに差し替え。**Firebase へは1バイトも出していない**）。
  別名 `["たなか","タナカ"]` が登録され両方にsubがあり、`subs` の並びは タナカ が先の期間で、
  Cookieの無い端末から「田中」と入力して再提出する:

  | | 再提出が上書きするsub | 管理者メモ「研修」の引き継ぎ |
  |---|---|---|
  | 修正前 | **`B_タナカ`** | **失われる（null）** |
  | 修正後 | `A_たなか` | 引き継がれる |
  | グリッド・Excel が読むsub | `A_たなか`（修正前後とも） | |

  つまり修正前は、**スタッフの提出が、管理者の画面に一度も表示されないsubへ入る**。
  そのうえ `A` に入れた管理者調整値・メモは取り残される。
  「研修」メモは `A` にしか無いので、**メモが引き継がれたこと自体が「正しいsubを選んだ」ことの
  独立した裏付け**になっている。

  直し方は #105 と同じにした——順序を直すのではなく、`resolveSubByAlias` へ通す
  （呼び出しは**4経路 → 5経路**）。

  **非回帰**: 配信物のソース行を切り出した Node 実測で、通常時**7ケース**（別名1つ／
  登録名subと別名subの併存＝#105の形／登録名subのみ／該当なし／別名subが別期間のみ／
  `staffAliases` 未設定／`settings` が `undefined`）すべてで**修正前後が同じsubを返す**。
  実ブラウザでも **pageerror / console.error ともに0件**。
  **同じ検証スクリプトを修正前の worktree（`f04f2d5`）に向けると落ちる**＝素通りするテストではない。

  あわせて**コメントの陳腐化も直した**。ここには「Excel(expXl:2185のfind)が古い方を出し」と
  書かれていたが、**その `find` は #105 で廃止済み**で、`app-admin.js:2185` は今や無関係な行を指す。
  #105 の教訓（誤った保証が確認を止める）がそのまま当てはまるので、根拠ごと書き直した。

### 調べて「バグではない」と確定したもの

- **提出一覧の詳細モーダルが別名を解決していない**と疑ったが、**反証した**。
  行側（app-admin.js:3468）は `resolvedName` を使うのに、モーダル側（:3525・:3530・:3534・:3547）は
  `det.staffName` を素で `getOT` / `getBreaksFor` に渡しており、別名subを開くと退勤延長・休憩が
  引かれない——という筋を立てたが、**`setDet({...sub,staffName:resolvedName})`（:3505）で
  詰める時点で既に登録名へ寄せてあった**。
- **スタッフ向けの提出状況一覧が管理者の下書き（`source:"grid"`）を「提出済み」に数える**と疑ったが、
  **反証した**。`app-staff.js:533` が SubsTab と同じ `s.source!=="grid"` で除外しており、
  未提出リスト（:537）もその除外後の集合を見ている。
- **ヒートマップが別名subを二重に数える**と疑ったが、**反証した**。`heatData` は `realStaff` を
  回して `_getSub`（別名解決済み・1件）を引くので重複しない。
- **`canTouch` の名前照合が非対称**と疑ったが、**反証した**。`resolveAlias` を両辺に掛けている
  （app-staff.js:547）。名前未入力の空文字がどの行にも一致しないことも意図どおり。
- **`expXl` の `subByName` が `ss` より前に定義されていて TDZ で落ちる**と疑ったが、**反証した**。
  `ss` は :2124、`subByName` は :2131 で、順序は正しい。

### 要確認（未修正・#105 から実質変化なし）

- **🟢 ヒートマップの時間目盛り（`heatHours`・app-admin.js:1065）だけが素の `subs` を走査している**（新規・BACKLOG化しない）。
  `heatData` が `realStaff`＋`_getSub` で数えるのに対し、目盛りの列を決めるここは期間内の全subを見るため、
  **重複subや登録外の名前の時間帯にも列ができうる**。出るのは常に0の列で、数字は狂わない＝表示のみ。
  次に同じ関数を触る回に揃えればよい。
...
- **配信版数が app-*.js の変更に追いついていない**: `?v=`（index.html 5箇所）と `build:`（app-core.js）は
  `20260831-c340d5c` のままで、以後 `6613c78`・`bc7bf2e`・`abcc668`・`e2fe16a`・**`8897a25`** が入っている。
  `/release-to-main` の手順63が標準工程としてバンプするため申し送りのみ（#103〜#105 から継続）。
...
- **CSS変数**: 未定義参照 **0行**。
- `npm test` **209件パス**（増減なし）／`npx eslint app-*.js` **0 errors 95 warnings**（増減なし）／
  `node --check functions/index.js` OK／Cloud Functions の `secrets` 指定は6箇所とも過不足なし
  （`SMTP_USER` と `SMTP_PASS` の対も維持）／`.delete()` の誤用 **0件**。
- **devのデータは1バイトも変えていない**（実ブラウザ検証は `StaffView` の単体マウントのみ。
  `onSub` はスパイ、`app-main.js` を読み込まないので `firebaseDB` は `null` のまま）。
- **未検証**: Excel・PDF の実物確認（**14回連続**・ユーザー操作が要る）／iOS 実機／本番データ／
  **重複subが実際に残っている店舗があるか**（#81 の未実施項目。今回の修正は「重複が既にある場合に
  どちらを掴むか」を揃えるもので、重複そのものは減らさない）。

### 測り方の記録

今回は**2回、測定手段のほうが壊れた**。どちらもコードの異常ではないので、直して測り直した
（#97・#105 と同じ扱い）。

1. **`input[type="text"]` でスタッフ名の入力欄を掴めなかった。** 配信物の `<input>` には
   `type` 属性が書かれていない。DOM の**プロパティ** `el.type` は既定値の `"text"` を返すが、
   CSS の属性セレクタは**属性が無ければ一致しない**。プロパティで確認して属性で掴もうとしたのが誤り。
   素の `#root input` に変えて解決した。
2. **確認ダイアログの「提出する」を押せず、同じボタンを6回押し続けた。** 部分一致で探していたため、
   一覧に残っている**「シフトを提出する」に毎回先に当たっていた**（前者は後者の部分文字列）。
   ダイアログが開いている間は完全一致で掴むようにして解決した。

どちらも「押したつもりで実は押していない」形の故障で、**`submittedCount` を出力に含めていなければ
『修正後も0件＝壊れている』と読み違えていた**。**検証スクリプトには必ず「そもそも操作が届いたか」を
示す値を持たせる**こと。

### 総括

**#105 は正しかったが、数え落としがあった。**

#105 は「規則を1つの関数に出し、**4経路すべて**を通した」と書いた。実際には**5本目があった**。
そして残ったのは、4本の中で唯一**スタッフ自身が操作する経路**だった——管理者側の4経路
（グリッド・週集計・提出一覧・Excel）はどれも `app-admin.js` にあり、**同じファイルを読んでいれば
目に入る**。`app-staff.js` の1本だけが別のファイルにあった。

**「すべての経路を通した」と書くときは、grep した結果を根拠にする。**
今回の検出も、推測ではなく `staffName` を直接比較している箇所を全ファイルで洗い出したことから始まった。

もう1つ。#105 が `existSub` を見て「式の連鎖だから完全一致が優先される＝問題なし」と判定したのは
**正しい**。正しいのに、バグは残っていた。**「その観点では正しい」と「正しい」は違う。**
規則が2段（完全一致 vs 別名／別名どうしの順序）あるとき、1段目の確認は2段目の保証にならない。
**同じ関数を通していれば、段が何段あっても揃う**——それが一本化の値打ちで、
#105 の結論はここでも変わらない。
