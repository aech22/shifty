...
## Shifty バグチェックレポート（2026-09-12 自動実行 #123・21時の回）

> 着手時の HEAD は `18b2323`（#122 のレポート反映直後）。**同日2回目の回**で、スケジュールが
> 09:00 と 21:00 の1日2回のため重複起動ではない。#122 が `react-hooks/exhaustive-deps` という
> 「道具を増やす」観点で当たったので、今回は**別の道具**＝ AST（`@babel/parser`）で構造的な
> 不整合を5種類まとめて当てた。**コードは1バイトも変えていない。**
...
| 走査 | 何を見るか | 結果 |
|---|---|---|
| ① 引数不足 | 定義の必須引数より少ない呼び出し | 21件ヒット・**全件が意図的な省略** |
| ② 引数過剰 | 定義の引数個数を超える呼び出し（＝シグネチャ縮小の取り残し） | **0件** |
| ③ オブジェクトの重複キー | 後勝ちで前の指定が黙って消える | **0件** |
| ④ JSX の重複属性 | 同上（`style` や `onClick` の二重指定） | **0件** |
| ⑤ フックの呼び出し位置 | 分岐・ループ・try・非コンポーネント関数の内側 | **0件** |

①の21件の内訳は `enterShop` 4件（省略時は `[shopObj]` に落ちる）・`subsWindowCutoff` 3件
（本体が「省略時は今日・既定3ヶ月」を明示処理）・`SC` 11件（`font||{}` で受け、11件とも `val=null` の
空セル）・`ph` 2件・`_refreshCompanyLinkedShops` 1件（連携追加時は削除IDが無いのが正しい）。
**引数を落としても壊れない設計の箇所ばかりで、ドリフトは1件も無かった。**
②〜⑤は `functions/index.js` にも当てて、いずれも0件。

### 要確認（未修正・すべて🟢）

- **🟢 `subsWindowCutoff` の3ヶ月前が、月末の基準日で1〜3日ぶん繰り上がる**（app-utils.js:78）。
  `base.setMonth(base.getMonth()-3)` が存在しない日付を翌月へ繰り上げる。**実測**:
  基準日 2026-05-31 → cutoff **2026-03-03**（素直には 2026-02-28）／2026-12-31 → **2026-10-01**（同 2026-09-30）／
  2026-05-28 と 2026-06-01 は一致。**ずれる向きは「窓が短くなる」側**で、その1〜3日に開始する期間が
  自動購読から外れる。ただし `hasOlderPeriods` が**同じ関数を使う**ので「過去参照」ボタンが必ず出るうえ、
  アクティブ期間は常に購読される＝**読めなくなる期間は無い**ため修正しない。
- **🟢 `settings.staffAliases` が無い店舗で、別名を使う軽いメモ2つが毎回作り直される**
  （app-admin.js:2474 ／ app-staff.js:250）。**#122 が申し送った2件と完全に同一で新規ゼロ**。
  計算量が O(その期間のsubs) と O(スタッフ数) と桁が違うという #122 の判断を引き継ぎ修正しない。
- **🟢 配信版数が app-admin.js の変更に追随していない**。6箇所とも `20260911-fe54a15` で相互には
  一致しているが、その後 #122 の `e8c2980` が app-admin.js を変えている。**リリース時にバンプしないと
  打鍵の応答性の修正がキャッシュに阻まれて届かない**。`/release-to-main` 手順63の担当なのでBACKLOG化はしない。
- **#121・#122 から継続（変化なし）**: 曜日キー7の意味変更の余波／`keepStaff` の旧形式の読み手2箇所が未確認／
  `PLAN_LIMITS` の既定値10／SubsTab が現在の `staffTypeLimits` で判定する件／休憩タグ・`staffWorkplaces` の残骸／
  `oneSidedFillBounds` の式の二重化／属性変更の既定の件／略称バリデーションのコメント／「残す」ポップアップの
  括弧書き／プラン上限モーダルの「店舗」文言／`makeSettings` の `templates:[]` が読まれない件／
  `syncAxis` の echo 記録が反射の来ない場合に残り続ける件（未検証の推測のまま）。
- **変化なし（BACKLOG化済み・条件A/Bでループの権限外）**: 二重課金の根治／特商法表記（🔴 2件）／解約通知／
  解約時のプラン判定／企業連携の解除と `verifyShopOwner` の移行猶予／別名提出の重複の根（#81）／PDF の実物確認／
  `purgeOldPeriods` の本有効化（2026-09-11 から着手可）／非表示スタッフを「未提出」に数えるか（#113）／
...
- `npm test` は **260件パス**（増減なし＝今回コードを変えていない）。
- `npx eslint app-*.js` は **0 errors・94 warnings**（増減なし）。`node --check functions/index.js` は OK。
- **RULES.md のスキャン項目はすべてクリア**。`DEV_MODE`・`DEV_PLAN_OVERRIDE` は式のまま（app-core.js:12）。
  `subs` の全体 `set()` 0件・`accounts` の全件読み0件。`filter(s=>s.id!==…)` は7件で、提出の削除2件は
  削除IDを渡し、1件は `.remove()` の直呼び、残り4件は店舗一覧の操作。読み込み順は utils→core→staff→admin→main、
  SRI 11本。未定義のCSS変数0件・ダーク定義の二重管理に差分なし。**フォーム部品58件・`fontSize<16` は0件**。
  SVG属性の `var()` は `app-staff.js:9` の1件＝既知。`.delete()` 誤用0件・`secrets:` 7件。
- プラン判定は `isPro=pro||premium` と `isPremium=premium` の使い分けが保たれており、シフト作成タブの編集・
  ポジションエラー・PDF はすべて `isPremium` 側。
- **Firebase・Stripe には読み書きとも一切アクセスしていない**（静的解析と Node での純粋関数実行のみ）。

### 測り方の記録

**最初に書いた走査は、0件と言いながら app-admin.js の74%を読んでいなかった。** 文字列とコメントを
空白で潰してから括弧を数える素朴な方式にしたところ、正規表現リテラルの開始判定が **JSX の閉じタグ
`</div>` の `/` に食いつき**、そこから次の `/` までを丸ごと消していた。非空白文字の残存率は
app-admin.js が **26.0%**・app-staff.js が 45.2%で、最長の消失区間378文字の中身は
文字列でもコメントでもない**ただのコード**だった。

**この失敗は「0件」という出力からは絶対に見えない。** 捕まえられたのは**わざと壊した写しに当てる対照**を
置いたからで、`app-admin.js:2178` の `visibleStaffList(staffList,settings,p)` から `p` を落とした写しを
検出できなければ走査が壊れている、という形にした。最初の版はこの対照を**検出できなかった**
（その過程でスプレッド `...fn(` をメソッド呼び出しの除外に巻き込む2つ目の欠陥も出た）。

**直し方はヒューリスティックの上塗りではなく道具の交換だった。** `node_modules` に
`@babel/parser` と `@babel/traverse` が**すでに入っている**（`@babel/eslint-parser` の依存）ので、
リポジトリに何も足さずに実 AST へ移せる。移したあとは対照が正しく検出し（22件 vs 21件で差分は
ちょうど仕込んだ1件）、本体は0件になった。走査の全文は Obsidian のバグチェックログ #123 にある。

**#122 の申し送り「`eslint-plugin-react-hooks` を常設するか」への部分的な答えでもある。**
`exhaustive-deps` の完全な再現には依然プラグインが要るが、**今回の5走査は依存追加なしで再現できる**。

**申し送り（次回の観点）**: **走査を書いたら必ず「わざと壊した写し」に当ててから本体に当てる**。
この手順が無ければ今回「5種類ぜんぶ0件でクリーン」という**嘘の報告**をそのまま出していた。
あわせて**素朴な文字列マスクで JS を読む走査はこのコードベースでは書かない**（JSX がある限り同じ形で壊れる）。
まだ当てていない構造的な観点として「`useState` の setter だけ呼ばれて値が読まれない state」
（CLAUDE.md が `authChecked` で1件記録済み）と「定義されているが一度も呼ばれない関数」が
AST でそのまま取れるので、次回はそこから始めるとよい。
