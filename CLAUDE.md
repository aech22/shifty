...
## Shifty バグチェックレポート（2026-09-04 自動実行 #108）

> 着手時の HEAD は `a582a15`。**#107 以降に入った実コードの変更はゼロ**（#107 自身の
> `3976e7c`・`0d6042d` と、そのドキュメント反映だけ）。#104 から5回連続で同じ状況なので、
> 機械スキャンは「基準線が動いていないこと」の確認に留め、**時間はコード読解と実測へ回した**。
> 今回は #105〜#107 が3回続けて掘った別名・改名の継ぎ目を**意図的に避け**、
> 締（追加出勤）・ポジション照合・書き込みパス・課金の4本を新しく当たった。

### 修正済み

- **🟢→予防 スタッフ設定マップの一覧が、改名と削除で二重管理になっていた**（[app-admin.js:2548](app-admin.js)）→ `909aa36`

  #107 は改名の規則を `renameStaffInSettings` に出し、対象マップの正本として
  **`STAFF_KEYED_SETTING_MAPS`**（app-utils.js:919）を新設した。ところが**同じ7マップを
  相手にするもう一方の入口**——削除の後始末 `settingsWithoutStaff`——は、その一覧を
  **直書きしたまま残っていた**。

  ```js
  ["staffColors","staffAttributes","staffNumbers","staffPositions","staffAliases","staffWorkplaces"].forEach(...)
  ```

  **現時点の挙動は変わらない**。両者がバイト単位で同一であることを機械的に確認した:

  | | 内容 |
  |---|---|
  | `STAFF_KEYED_SETTING_MAPS` | `["staffColors","staffAttributes","staffNumbers","staffPositions","staffAliases","staffWorkplaces"]` |
  | app-admin.js:2548 の直書き | 同上（`identical: true`） |

  つまりこれは**まだバグではなく、バグの作り方**である。新しいスタッフ名キーのマップを
  1つ足したとき、改名は正本を見るので追随するのに削除は直書きを見るので追随しない——
  **「改名では移すのに、削除では消し残す」**という形で黙って食い違う。
  これは #105・#106・#107 が3回続けて踏んだ「同じ不変条件に入口が2つあり、
  片方だけが知っている」と**まったく同じ構造**で、しかも今回は
  **#107 の修正自身が正本を作った直後に、既存の入口を繋ぎ忘れていた**。

  直書きをやめて正本を参照する形に変え、ドリフト検出のテストを2件足した:

  1. app-admin.js にマップ一覧の直書きが残っていないこと
  2. `STAFF_KEYED_SETTING_MAPS` が全て `PERIOD_SNAPSHOT_SETTING_KEYS` に含まれること
     （含まれないマップができると、そのマップだけ写しの側で改名が届かず **#107 が再発する**）

  **1は修正前のコードに向けると実際に落ちる**ことを確認済み（`git stash` で戻して実行 →
  `fail 1`・エラーメッセージに直書きの中身が出る）＝素通りするテストではない。
  2 は現時点で全て含まれており、7マップとも写しの側で改名が届くことの裏付けにもなっている。

### 調べて「バグではない」と確定したもの（今回の主な収穫）

いずれも**疑って、測って、反証した**もの。

- **期間タブとシフト作成タブの Excel が別物を出すのでは**と疑ったが、**反証した**。
  シフト作成タブ（:1651）は `adjResolver` を渡し、期間タブ（:2070）は渡さない。
  だが `expXl` はファイル名を `${resolver?"":"_修正前"}.xlsx` で分けており（:2346）、
  **「調整前」と「調整後」を出し分ける意図的な設計**だった。両者とも `resolvePeriodMaster`
  の結果を使う点も揃っている（ShiftEditTab は props を `periodMaster` で影付けしている）。
- **`matchPositionSlots`（ポジション不足の二部マッチング）が最大マッチを外すのでは**と疑ったが、
  **反証した**。総当たりの最大マッチングと突き合わせる差分テストを書き、
  **ランダム4,000ケースで不一致0件**。増補路（augmenting path）が要るケースも含めて正しい。
- **書き込みパスの差分計算がデータを落とすのでは**と疑ったが、**反証した**。
  `diffSubForFlatWrite` → `applyFlatSubWrite` の往復を、管理者フィールド（`adjustedStart`・
  `adminRest`・`extraStart/extraEnd`・メモ）入りのランダムなsubで検査し、
  **20,000往復で内容の不一致0件**。差分を当てた結果が新しいsubと完全に一致する。
- **ヒートマップの時間目盛り（`heatHours`・:1051）が素の `subs` を走査している件**（#106 からの🟢）を
  **改めて検証して、🟢のままでよいことを確認した**。収集元は候補時間・素のsubs・`heatEdits`・
  固定コマンドの4つで、最後に **min〜max の連続レンジへ均される**（:1069）。
  素のsubsは解決後のsubsの上位集合なので**行が欠けることは無く、増えるだけ**＝常に0の列が出るだけ。
- **`resolveShopMeta` の price 優先（`c51b62e`）が Subscription 以外のイベントを壊すのでは**と
  疑ったが、**反証した**。`planOfSubscription` は `items.data[0].price` を見るため、
  Checkout Session や Invoice では `null` を返して `metadata.plan` へフォールバックする。
- **`purgeOldPeriodsCutoff`（functions/index.js:853）が UTC で日付を切っている**のは事実だが、
  **実害は無い**。同ファイルには JST 補正の `toJstDateStr`（:409）があるのにこちらは通していない。
  ただしズレは最大1日で、**向きは「削除を1日遅らせる」＝安全側**。しかも `PURGE_OLD_PERIODS_DRY_RUN`
  が true でまだ実削除していない。次に `functions/` を触る回に揃えればよい（🟢）。

### 要確認（未修正）

- **🟡（新規）プラン変更の予約（ダウングレード）を、ユーザーが自分で取り消せない** → **BACKLOG化済み**

  Premium → Pro のダウングレードは Subscription Schedule による「予約」になり、マイページに
  青い「プラン変更の予約中」バナーが出る（app-admin.js:4614）。ところが**その予約を取り消す
  導線がアプリ内にどこにも無い**。

  原因はコード上で確定できる。プラン変更ボタンの選択肢は
  `["pro","premium"].filter(p=>p!==plan&&p!==bs.scheduledPlan)`（:4509）で作られ、
  セクション全体が `changeOptions.length>0` でしか描画されない（:4628）。
  **有料プランは2つしかない**ので、予約中は現在プランと予約プランで2つとも除外され、必ず空になる。

  **実測**（同じ式をそのまま Node で評価）:

  | 状態 | changeOptions | 「プランを変更」セクション |
  |---|---|---|
  | Premium利用中・Proへの降格を予約済み | `[]` | **表示されない** |
  | Pro利用中・Premiumを予約済み | `[]` | **表示されない** |
  | 予約なし・Premium利用中（対照） | `["pro"]` | 表示される |
  | 予約なし・Pro利用中（対照） | `["premium"]` | 表示される |

  **逃げ道も無い**のが効いている。解約バナー（:4604）は「解約を取り消す場合は下の『請求管理』から」と
  案内しているが、**変更予約バナーには同等の案内が無い**。しかも 2026-08-11 の実測で
  **カスタマーポータルには「プラン変更」のUIが存在しない**ことが分かっている。
  結果として、ユーザーは予約を入れた瞬間に**降格が確定し、切替後に再アップグレードして
  差額を払う以外に戻す手段が無い**。

  **降格そのものは正しく動く**（予約・切替・解約の各Webhookは実装済み）。壊れているのは
  「入れた予約から降りられない」という一方通行性だけなので 🔴 ではなく 🟡 とした。
  取り消しを許すかは**条件B（仕様判断）**、確認は**条件A（Stripe実データ）**。

- **🟢 `resolveShopMeta` の JST 未適用**（functions/index.js:853）。上記のとおり安全側・dry-run中。
- **🟢（#107 から継続）自分の別名へ改名すると、その別名が自分自身に残る**（app-admin.js:2443 付近）。表示のみ。
- **🟢（#106 から継続）ヒートマップの時間目盛りだけが素の `subs` を走査**。今回改めて検証し表示のみと確定。
- **🟢 `database.rules.tightened.json` が現行ルールとバイト同一の残骸** → **BACKLOG化済み**。今回も `cmp` で同一を再確認。
- **🟢 片側セルの日は「→15:15（+15分）」の実効退勤バッジが出ない**（app-admin.js:3534付近）。表示のみ。
- **🟢 期限切れの後始末が、より古い期間にしか残っていない人の設定を消しうる**（app-admin.js:2547付近）。2026-08-31 決定4・案Aの範囲設定そのままなので**仕様判断（条件B）**。
- **🟢（#104 から継続）`purgeInactiveShops` が、もう存在しない `inviteCodes` を毎日掃除している**（functions/index.js:808）。読み取り1回ぶんの無駄。
...
- **配信版数が app-*.js の変更に追いついていない**: `?v=`（index.html 5箇所）と `build:`（app-core.js）は
  ...
  `3976e7c`・`0d6042d`・**`909aa36`** が入っている。`/release-to-main` の手順63が標準工程として
  バンプするため申し送りのみ（#103〜#107 から継続）。
...
- **CSS変数**: 未定義参照 **0行**。
- `npm test` **221件パス**（+2。どちらもドリフト検出。1件目は修正前のコードでは落ちることを確認済み）／
  `npx eslint app-*.js` **0 errors 95 warnings**（警告は増減なし）／
  `node --check functions/index.js` OK／Cloud Functions の `secrets` 指定は6箇所とも過不足なし／
  `.delete()` の誤用 **0件**。
- **差分テスト2本**（今回の主な実測）: `matchPositionSlots` 対 総当たり最大マッチング **4,000ケース不一致0**／
  `diffSubForFlatWrite`＋`applyFlatSubWrite` の往復 **20,000ケース内容不一致0**。
- **Firebase には1バイトも書いていない**（実ブラウザ検証は行わず、配信物の関数を Node で実行しただけ）。
- **未検証**: Excel・PDF の実物確認（**16回連続**・ユーザー操作が要る）／iOS 実機／本番データ／
  **プラン変更予約の Stripe 実データでの挙動**（今回の🟡は式の評価とUIコードから確定したもので、
  実際に予約を作って画面を見てはいない）。

### 測り方の記録

今回も**測定手段のほうが先に壊れた**（#97・#105・#106・#107 と同じ扱いで、直して測り直した）。

差分書き込みの往復テストが、初回は **8,000件中2,022件が「不一致」**と出た。
数が多すぎるのが手掛かりで、**本当に2割が壊れているならアプリはとうに動いていない**。
中身を見ると、比較に使った `JSON.stringify` が**キーの順序まで見ていた**だけだった——
差分適用で新しい日付を足すと `{02,03,01}`、期待値は `{01,02,03}` で、
内容は同じなのに文字列が違う。キーを再帰的にソートしてから比べる形に直したところ
**20,000件で不一致0件**になった。

**「異常値の割合が大きすぎるときは、まず測定手段を疑う」**——#97 で
「全15店舗が未claim」と出たときと同じ判断で拾えた。もし最初の2,022件をそのまま
「書き込みパスにバグがある」と報告していたら、存在しない不具合の調査に次の回を丸ごと使わせていた。

### 総括

**今回いちばん確からしい成果は、バグを1つ見つけたことではなく「バグの作り方」を1つ塞いだこと。**

#105 は「規則を1関数に出して全経路を通す」で閉じ、#106 は「その経路の数え落とし」、
#107 は「そもそも1つの規則だと気づく必要があった」だった。今回はその続きで、
**#107 が正本を作った、まさにその瞬間に、既存の入口を1つ繋ぎ忘れていた**。
正本を作る作業と、既存の呼び出し元を全部その正本へ寄せる作業は別物で、
前者だけやると「正本があるのに従っていない箇所」が生まれる——
これは最初から二重管理だったときより**むしろ危ない**。正本があるぶん、
次の人は「ここを直せば全部に効く」と信じてしまうからだ。

だから今回は直すだけでなく、**直った状態が壊れたら落ちるテスト**を置いた。
`STAFF_KEYED_SETTING_MAPS` が `PERIOD_SNAPSHOT_SETTING_KEYS` の部分集合であることの
検査も同じ意図で、**#107 の再発条件そのものを機械が見張る**形にしてある。

一方で、**「別名・改名の継ぎ目をもう一度掘る」ことは意図的にやめた**。3回続けて同じ場所から
出たということは、その周辺の密度が高いのは確かだが、同時に**そこばかり見ていた**という
ことでもある。今回新しく当たった4本（締の追加出勤・ポジション照合・書き込みパス・課金）のうち
3本は差分テストで**白であることを積極的に示せた**——これは「まだ見ていない」とは違う。
残る1本（課金）から、**3回の別名調査では絶対に出てこなかった種類の問題**が出た。
