...
## Shifty バグチェックレポート（2026-08-22 自動実行 #89）

### 修正済み

- **🟡 スタッフ名にFirebase禁止文字を許していて、設定の保存が黙って失われる**（[app-admin.js:2364](app-admin.js) 追加 / [:2356](app-admin.js) 改名）→ `1fe705a`
  スタッフ名は `staffColors` / `staffAttributes` / `staffNumbers` / `staffPositions` / `staffAliases` / `staffWorkplaces` / `overtimeSettings.byStaff` の**7つの設定マップでFirebaseの「キー」になる**。ところが名前の入口（追加・改名）はどちらも**空文字と重複しか見ていなかった**。
  **何が起きるか**: Firebaseはキーに 半角ピリオド・シャープ・ドル記号・スラッシュ・角括弧と制御文字を許さない（SDK内部の `INVALID_KEY_REGEX_` を実測で確認）。「田中.太郎」のような名前を登録したあと**名前色・属性・社員番号・ポジション・退勤延長・別名・勤務先のどれか1つを設定した瞬間**、`saveSettings` の `set()` が**同期例外**を投げる。そして [app-main.js:1127](app-main.js) の `fbW` は `fbSet(...).catch(...)` の形なので、**`.catch` を付ける前に投げられる＝書き込み失敗のログすら出ない**。一方 `setSettings` と `localStorage` は fbW より前に実行済みなので、**画面上は保存されたように見えて、他端末・再読込には一切反映されない**。
  **改名はさらに悪い**: `onRenameStaff`（[app-admin.js:182](app-admin.js)）は `saveStaff` → `saveSubs` → `saveSettings` の順に呼ぶため、**staffList と subs だけが新しい名前になり、7マップは旧キーのまま残る**。これは同じ関数のコメント（`:187`）が「漏れると属性・ポジション等が黙って初期値に戻る」と警告している状態そのもので、それが**黙って**起きる。
  **正解はコードベース内にあった**: `genSecureId`（[app-utils.js:419](app-utils.js)）は「Firebase禁止文字（. $ # [ ] /）を除外した記号のみ使用」と明記して同じ集合を避けている。**IDを生成する側は知っていて、名前を受け取る側だけが知らなかった**ので、仕様判断なしに入口をそちらへ揃えられた。カスタム属性のIDも `"custom_"+genSecureId(8)`（`:3599`）で、**自由入力をキーにしていないのはこの1箇所だけ**だった。
  **修正**: `firebaseKeyForbiddenChars(name)` を app-utils.js に追加し、`add()` と `confirmEdit()` で弾く（「▲ 名前に使えない文字があります（.）」）。**半角スペース・ハイフン・全角記号は有効なキーなので弾かない**（「田中 太郎」「Anne-Marie」「Ｍ．ケン」は通る）。プラン上限のアップセルより前に置いて、上限に達している店舗でも理由が正しく出るようにした。
  **既存の悪い名前は袋小路にならない**: そこから正常な名前へ改名するときは `renameKey` が `m[oldName]===undefined` で早期returnするため禁止キーが生まれず、保存が通る。
  **実測（配信物の述語＋firebase-admin の実SDK検証で修正前後を突き合わせ）**: 修正前は「田中.太郎」で `set()` が同期throw・**警告ログなし**・ローカルだけ更新。修正後は入口で止まり書き込みなし。「田中 太郎」は**修正前後で完全に同一**（非回帰）。ユニットテスト3件追加（`npm test` 174→**177件パス**）。

### 要確認（未修正）

- **🟡（新規）休憩の適用条件が、画面の説明文と食い違う（7時間半のシフトから休憩が1分も引かれない日がある）** → **BACKLOG化済み**（仕様判断待ち）
  休憩時間設定の画面は **「休憩は出退勤時間が実際に休憩時間帯と重なるスタッフにのみ適用されます」**（[app-admin.js:2922](app-admin.js)）と書いているが、`getBreaksFor`（[app-utils.js:264-271](app-utils.js)）は**重なりでは足りず、休憩を丸ごとまたぐ**（出勤 < 休憩開始 かつ 退勤 > 休憩終了）ときだけ適用する。
  **実測（配信物をNodeで実行・平日/休憩12:00〜13:00）**: 09:00〜20:00 は休憩1件適用で純勤務10:00。**12:30〜20:00（30分重なる）は適用0件で純勤務 7:30**、09:00〜12:30 も0件、端が一致する 09:00〜13:00・12:00〜20:00 も0件。またぎ判定は 2026-07-10 の**意図的な**設計変更（「ランチのみ・ディナーのみのシフトに適用しない」とコメントに理由あり）だが、その意図が想定していたのは**休憩の端に接するだけの短いシフト**であって、「休憩の途中から出勤する長時間シフト」ではない。どちらを正とするかは運用判断のため起票した（案A: 説明文を実装へ寄せる／案B: 重なり分だけ引く／案C: 端一致だけ除外する）。
- **🟢（新規）CLAUDE.md の `dayTypeOf` の説明が古かった** → 本レポートと同じコミットで修正
  「祝日をholSat/holSunに分割した5分類（必要ポジション設定タブ用・**breakTimes等には非影響**）」と書かれていたが、実際は `getBreakList`（[app-utils.js:146](app-utils.js)）が `positionDayTypeFor` を通して**同じ5区分を共有している**（app-utils.js:314 のコメントが正しい）。「非影響」と読むと、日区分に触るときに休憩時間の非回帰確認を省いてしまう。
- **🟡 #77 の宿題（Free 上限の実機検証）は静的監査完了・実測のみ未達**（下記🔴が理由。**残るは実機で21人目・2件目を押す1回だけ**）
- **🔴（継続・プロセス）実機E2Eが今回も実行できない**（環境要因・コード変更なし）
  `preview_list`（稼働0件）と `preview_start` を実際に実行し、**「Dev servers can't be started from unattended sessions」と明示的に拒否された**。**#88 で判明したとおり、この拒否は「無人の間だけ」効く**——今回は本当に無人（ユーザーからのメッセージが1件も無いスケジュール起点のターン）なので拒否は正常な動作であり、**次にユーザーが会話を始めたターンで再試行すれば通る**。#88 の実機E2E（期間確定・別名正規化）は 2026-08-22 にその方法で完走済み。
- **🟢（#88からの申し送り・再確認）同じ期間のExcelとPDFで店舗名が違う**（Excel=`xlShopName`／PDF=`shopName`・[app-admin.js:1402](app-admin.js)/`:1487`/`:1550`）。設定名が `xlShopName`・説明文も「Excel出力時」と範囲を明示しているため仕様どおりだが、ユーザーから見て説明がつくかは別。今回も変化なし
- **🟡（#88起票）期間の確定（写し）が `periods` を肥大化させる**（1期間57KB・36件で約2.0MBが毎回の起動時にDL）→ BACKLOG化済み（案A〜Dの設計判断待ち）
- **🟢（#88からの申し送り・確認済み）`period.lockedAt` は書かれるが読まれない**／**`periodSnapshotEqual` の配列/オブジェクト非対称**（現行経路では到達不能）／**確定済みバッジが閲覧のみ端末に出ない** → いずれも変化なし
- **🟢 `index.html` の `?v=` と `build:` は現在の配信物を指している**（6箇所とも `20260822-e9d98a4`。ただし**今回の `1fe705a` で app-utils.js・app-admin.js が変わった**ため、次のリリースでバンプが要る）。申し送りのみ——`/release-to-main` の手順63がバンプする
- **🟡 店舗間シフト重複の判定が退勤延長を見ていない**（#86）／**`dupErrors` だけが片側セルの日を対象外にしている**（#86）／**`x` セルが店舗間重複の判定から除外されていない**（#85）→ すべて BACKLOG化済み（仕様判断待ち・変化なし）
- **🟢 `effShiftStart`/`effShiftEnd` の未採用が app-admin.js になお3箇所**（`:1009` ヒートマップの時間軸／`:3055` 提出一覧の出勤日数／`:3113` 詳細モーダルの出勤日数）。**#82〜#88 と同じ理由で今回も触らなかった**——直し方が BACKLOG の判断待ち項目（片側セルだけの日を何時間と数えるか）に依存する
...
- 以下は #46〜#88 からの継続（変化なし）: 未claim店舗の取り込み／`findActiveSubscription` の `|| live[0]`／`planExpiry` のUTC計算／**片側セルだけの日が0分**／`purgeOldPeriods` のUTC日付／`generateInviteCode` のデッドコード／ほか

### 今回の対象と見つけ方

**#88 以降にコードの差分は無かった**（入ったのは `6456cff` のリリース版数バンプと記録のみ）。そこで #88 が当たりを出した **「画面の説明文をコードの仕様書として使う」レンズを、1つの設定ではなくアプリ全体へ広げた**——`app-admin.js` / `app-staff.js` から「〜します／〜されます／〜できます」で終わる説明文を機械的に抜き出し（70件強）、1つずつ実装と突き合わせた。

**(1) このレンズ自体は当たったが、当たりは1件だけだった。** 候補時間の優先順位（「祝日は平日・土日より優先適用されます」→ `gc`・app-staff.js:91 が 日付別>祝日>曜日別>全体 で実装どおり）、退勤延長の帯判定、ポジションの重複追加、企業連携のヘルプ除外——大半は説明文どおりだった。**唯一ずれていたのが休憩の「重なる」**で、実装は「またぐ」だった（上記🟡）。ただしこれは仕様判断が要るためBACKLOG行きで、**このレンズだけでは今回の修正には届かなかった**。

**(2) 修正に届いたのは、レンズを変えて「自由入力がFirebaseのキーになる経路」を全部数えたときだった。** 説明文の照合中に `staffTypeLimits` のカスタム属性IDが `"custom_"+genSecureId(8)` で生成されているのに気づき、**「では他に、ユーザーが打った文字列がそのままキーになっている場所は無いか」**を機械的に洗った（計算プロパティ `[expr]:` を全ファイルから抽出し、日付・生成ID・固定文字列を除外して残りを1つずつ追跡）。残ったのは**スタッフ名だけ**で、それが7つの設定マップでキーになっていた。**「1箇所だけ守られていない」ではなく「1箇所だけ守られている」から逆に辿ったのが今回の分かれ目。**

**(3) 決め手は、禁止文字の集合をコードベース自身が既に書いていたこと。** `genSecureId` のコメントに **「Firebase禁止文字（. $ # [ ] /）を除外した記号のみ使用」** とあり、SDKの `INVALID_KEY_REGEX_` を実測した集合と一致した。**どの文字を弾くかが仕様判断にならなかった**ので、そのまま実装できた（#88 が設定画面の説明文を仕様書として使ったのと同じ構造で、今回は「もう一方の実装のコメント」が仕様書だった）。

**(4) 一番の危険は「例外が投げられること」ではなく「そのログすら出ないこと」だった。** `fbW` は `fbSet(path,val).catch(e=>console.warn("書き込み失敗:",path,e))` と書いてあり、**書き込み失敗を拾う意図が明示されている**。ところが `fbSet` は**同期に**投げるので `.catch` は取り付けられる前に飛ばされる。つまり**「失敗を観測するために置かれたコードが、この失敗だけは観測できない」**。実SDKで再現するまでこの非対称は見えなかった。

**(5) 実SDKを持ち込んだのが検証の質を決めた。** `functions/node_modules` の firebase-admin は同じ `@firebase/database` のキー検証を持つため、**推測ではなく本物の検証器**で「どの文字が落ちるか」「同期に投げるか」を確かめられた。`sanitizeForSet` は `undefined` しか見ていない＝**この経路には既存のガードが1つも無い**ことも同時に確定した。

### 検証したこと

- `DEV_MODE`（app-core.js:12・式のまま）／`DEV_PLAN_OVERRIDE`（:125）正常
- `subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／`global/shops` の全件読み **0件**／`global/templates` **0件**／`database.rules.json` の `".read": true` **0件**
- 読み込み順維持（utils→core→staff→admin→main）・SRI **11本**
- `npm test` **177件パス**（新規3件）／`npx eslint app-*.js` **0 errors 98 warnings**（修正前後で増減なし）／`node --check functions/index.js` OK
- **Firebaseのキー検証を実SDKで実測**: `@firebase/database` の `INVALID_KEY_REGEX_`（角括弧・ピリオド・シャープ・ドル記号・スラッシュ・U+0000〜U+001F・U+007F）を確認し、firebase-admin の `ref().set()` に「田中.太郎」「A/B」「山田#2」「S$1」「X[1]」を渡して**全て同期throw**すること、「正常な名前」は受理されることを実測
- **修正の検証（配信物の述語＋実SDK）**: `fbW`／`fbSet`／`saveSettings` の順序をそのまま再現し、修正前＝「画面は保存済み・localStorage更新・Firebase未達・警告ログなし」、修正後＝「入口で停止・書き込みなし」。正常な名前は修正前後で完全同一
- **休憩の適用条件を実測**: 12:00〜13:00 の休憩に対し 09:00〜20:00＝1件適用（純勤務10:00）、12:30〜20:00＝0件（7:30）、09:00〜12:30＝0件（3:30）、端一致の2ケース＝0件
- **「自由入力→Firebaseキー」経路の全数確認**: 計算プロパティを全ファイルから抽出して追跡。**キーになるのはスタッフ名のみ**で、ポジション名・テンプレート名・企業名・店舗略称・期間ラベル・別名はすべて**値**、属性IDは生成、日付・shopId・subId・urlToken も生成または日付
- **期間確定（`1c5272b`）の凍結カバレッジの照合**: `ShiftEditTab` が読む `settings` のキーを機械的に抽出し `PERIOD_SNAPSHOT_SETTING_KEYS`（15キー）と突き合わせ。**凍結対象外は `xlShopName` の1つだけで、これは意図的**（`resolvePeriodMaster` のコメントが名指ししている）＝**#88 の xlShopName 以外に凍結漏れは無い**
- **未検証（今回は手段が無い）**: ブラウザでの実描画すべて／**今回の修正の実機確認**（禁止文字を入れたときのトースト表示・正常な名前の非回帰）／**Free 上限に達した店舗での実ブロック**／実機 iOS Safari

### 総括

**差分ゼロの回だったが、#88 のレンズ（画面の説明文＝仕様書）をアプリ全体へ広げた副産物として、これまで一度も見ていなかった経路——「ユーザーが打った文字列がFirebaseのキーになる」——にたどり着いて 🟡 を1件直せた。**

**見つかったのは「守り忘れ」ではなく「守っている場所が1つだけあった」ことから逆算した穴だった。** カスタム属性のIDは `genSecureId` で生成され、その関数のコメントには Firebase禁止文字が列挙してある。**同じリスクを知っているコードが同じファイルにあるのに、スタッフ名の入口だけがそれを通っていなかった。** #79 の確認ダイアログ、#77 の `disabled`、#88 の `xlShopName` と同じで、**このコードベースで見つかる不整合は、正解がコードベース内の別の場所に既に書かれていることが多い。**

**今回の失敗の仕方は、これまでで最も静かだった。** 書き込みは失敗し、失敗を拾うために書かれた `.catch` は同期throwを取れず、`setSettings` と `localStorage` だけが先に成功する。**コンソールにも `npm test` にも eslint にも一切出ず、画面には「保存された」と見える。** 改名の場合は staffList と subs だけが新名で確定し、7つの設定マップが旧キーに取り残される——コード自身が「漏れると属性・ポジション等が黙って初期値に戻る」と警告している、まさにその状態になる。**「壊れたときに何が観測できるか」を先に問うと、この種のバグは早く見つかる。**

**説明文レンズは、当たりの密度は低いが外れ方が有益だった。** 70件強の約束のうち、実装とずれていたのは休憩の1件だけ。ただし**その1件は仕様判断で、ループでは閉じられない**。#88 が1回で当てたのは新機能があったからで、**既存コードに同じレンズを回すと「ほとんど合っている」ことの確認になる**——それ自体は価値があるが、修正には別のレンズが要る。今回はそれが「自由入力の行き先を数える」だった。

**次回の手。** 実測が復活していれば、**まず今回の修正の実機確認**（禁止文字を含む名前を追加しようとしてトーストが出ること・「田中 太郎」が従来どおり追加できること）と **#77 の Free 上限**を、この順で1ターンに収める（どちらもスタッフタブで完結し、ページ遷移が要らない）。使えないままなら、推奨は**「値として扱っているはずのものが、どこかでキー・ID・ファイル名になっていないか」を経路ごとに数える**——今回のレンズをスタッフ名以外へ広げる形で、具体的には**Excel/PDFのファイル名に入る文字列**（`xlShopName`・期間ラベル。OS が禁じるスラッシュ・コロン等を含めるとダウンロードが壊れうる）と、**localStorage のキーに入る文字列**が未検査のまま残っている。
