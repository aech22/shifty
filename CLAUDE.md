...
## Shifty バグチェックレポート（2026-09-26 自動実行 #149）

> 着手時の HEAD は `0cd469e`。#148（`2d1ca8d`）以降に入った `af643d2`・`36bc492`・`7cc132c` の差分だけを読んだ
> （労務判定の途中表示・属性の並びと改称・セルの文字消しで休暇を外す・該当日のラベル表示）。
> **新しいコードから🟡が1件**出て、修正した。

### 修正済み

- **[🟡] 公休を文字消しで外すと、スタッフ提出のある日だけ反対側に提出時刻が戻り片側だけの勤務になる**
  （app-admin.js の `applyEditToSubs`）→ `bf075a3`

  `ko` は終日なので、文字消しでも両側の種別を外す。しかし空欄の上書きを書くのは打ったセルだけだった。
  実測（`ShiftEditTab` だけを実ブラウザにマウントし `onSave` を state に差し替え＝Firebase へは1バイトも出ない。
  田中の 7/2 に提出 09:00〜17:00 がある状態で `ko` を入れてから外す）:

  | 外し方 | 修正前の表示（出勤／退勤） | 修正後 |
  |---|---|---|
  | 出勤セルの「公休」を消す | 空欄／**17** | 空欄／空欄 |
  | 退勤セルの「公休」を消す | **9**／空欄 | 空欄／空欄 |
  | `ko` をもう一度入れる（対照） | 9／17 | 9／17（変化なし） |

  修正前は**誰も入力していない片側だけの勤務**ができ、片側セルの補完で勤務時間として数えられる。
  回帰スクリプト `example-cell-clear.js` の既存項目 `ko1_bothBlank` が「両方空欄」を期待していたが、
  **提出の無い日しか測っていなかった**ので通っていた。`ko3_staffDayBothBlank` を追加し、
  修正前の配信物（HEAD を `git archive` で展開し `SHIFTY_ROOT` に指定）では EXIT=1・この1項目だけが落ちることを確認した。

### 要確認（未修正）

**新規の🔴🟡は無し**（上の1件で閉じた）。BACKLOG への新規起票は無し。

- **[🟢] 公休を外す2つの入口で、外した後の中身が違う。** `ko` の再入力は両側をスタッフ提出値へ戻し、
  文字消しは両側を空欄の上書きにする。どちらも両側で揃っているので片側勤務は生まれない。
  「消す操作＝空欄」「同じコマンド＝元に戻す」という既存の規則（yu/ke も同じ）どおりなので、仕様として据え置く。
- **#148 以前から継続（変化なし）**: BACKLOG 化済みの項目はすべて据え置き。
- **引き受け済みのトレードオフ（再検出しても直さない）**: `subs/$subId/.write` は認証済みなら通る（2026-08-31 決定1）。

### 当てて否定した仮説

- **「要確認」から実数表示へ変えたことで、旧キー（`pending`・`unknown`・`laborMonthReady`）を読む箇所が取り残されている → 残っていない。**
  app-*.js 全体の grep で0件。`st.key==="skip"` の分岐が1つ残るが、`weekRestStateOf` がもう返さないだけで害は無い。
- **7日揃わない週が総括で ×休なし（要修正）に化ける → 化けない。** `weekRestStateOf` は揃わない週を `partial` にし、
  総括は `key==="none"` だけを見る。
- **途中の月の残業予定が `period.laborTotals` に凍結される → されない。** 書き込みは `laborIsLastOfMonth&&laborMonthCovered` の両方が要り、
  年度の数え直し（`liveMonthOtFor`）も月の全日が読めるときしか値を返さない。
- **「バイト」→「パート・アルバイト」の改称が一部の画面に届かない → 届いている。** 組み込み属性の名前を
  `staffTypeLimits[id].name` から読む箇所は0件で、全画面が `getAttrOptions`／`STAFF_TYPE_LABELS` を通る。

### 検証したこと

- `npm test` **344件パス**・`npx eslint app-*.js` **0 errors / 95 warnings**。
  修正の初稿はキーを文字列連結で作っていたため、シフト日へ書くキーのドリフト検出テストが落ちた。キーを直接書く形に直して通した。
- `DEV_MODE` は式のまま（app-core.js:12）。今回の変更に `set()`・フォーム部品の追加は無い。
- 実ブラウザの回帰スクリプト10本（`example-cell-clear` と労務5本・シフト作成タブ・提出一覧の休暇・属性の並び・過去データボタン）がすべて EXIT=0。
- **配信版数は `20260926-7cc132c` のままで、`bf075a3` に追随していない。** リリース時に `/release-to-main` の手順でバンプすること（本ループはバンプしない）。
- **Firebase・Stripe・本番データには一切アクセスしていない。**

**申し送り（次回の観点）**

1. **スケジュールタスクの PHASE 2〜4 の grep はいまも削除済みの `app.js` を対象にしている**（5回目の申し送り）。
2. **回帰スクリプトの前提データを疑う。** 今回の `ko1_bothBlank` は「提出の無い日」だけで組まれていたため、
   提出値へのフォールバックが起きる形を一度も踏まずに通っていた。セル操作の回帰は「提出あり」「提出なし」の両方で組むこと。
