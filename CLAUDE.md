...
## Shifty バグチェックレポート（2026-08-07 自動実行 #59）

### 修正済み

- **[🟡] 管理者が期間管理タブの「提出状況」ビューでセルを編集すると、そのスタッフの提出が「再提出」として記録され、締切を守ったスタッフに「更新（＝締切後の変更あり）」バッジが立っていた**（app-admin.js:1835 `onEditSub`／コミット `1589071`）
  **管理者の操作が、スタッフの提出メタとして記録されていた**。`isUpdated`/`updatedAt` は「スタッフ本人が再提出した」ことを表すフィールドで、`subHasRealUpdate`（app-utils.js:632・**締切日ゲート付き**）と `subLastActionTime`（:619）が読む。管理者の提出状況ビューの `onEditSub` だけが、セル編集のたびに**管理者自身の編集時刻**でこの2つを立てていた。
  **実測（app-utils.js の実関数 `subHasRealUpdate`・`subLastActionTime` を require した Node 再現）**: 締切 8/5 の期間で、スタッフ「山田」が 8/1 に提出しその後一切触っていない状態で、管理者が 8/7（＝締切後）に 8/10 のセルを編集すると、修正前は **`isUpdated:true` ／ `updatedAt`＝管理者の編集時刻 ／ `subHasRealUpdate(締切8/5)`＝`true`（「更新: 8/7 11:00」バッジが点灯）／ 並べ替えキーが 8/7 に繰り上がる**。修正後は **`isUpdated` なし ／ バッジ `false` ／ 並べ替えキーは提出時刻 8/1 のまま** になることを確認した。
  **壊れ方は2経路**。①提出一覧（app-admin.js:2984）に琥珀色の「更新: M/D HH:MM」が出る。このバッジは締切日ゲート付き＝「締切後に変更した」という意味に絞って設計されているため、**締切を守ったスタッフが締切破りとして表示される**。②`subLastActionTime` が並べ替えキー（:2927）なので、そのスタッフが「今さっき再提出した人」として一覧の先頭に繰り上がる。
  **修正方針**: 管理者の編集では `isUpdated`/`updatedAt` を**立てない**（`{...sub,shifts}` をそのまま保存する）。既にスタッフの本物の再提出で立っている値は引き継ぐ（消すと本物の再提出記録が失われるため）。保存自体は `saveSubs` の参照比較＋`diffSubForFlatWrite` が `shifts/{日付}` の差分を検出するので、これらのフィールドに依存していない。

### 今回の対象と見つけ方

前回（#58）以降の差分は #58 の修正（`a2e38cb` app-staff.js:158）と CLAUDE.md 同期のみ。今回は #58 の総括が指定した2つの手法をそのまま採った。

**1つ目「まだ起点にしていないコメントから読む」**。#58 が挙げた未使用の起点3本を順に読んだ。①`ADMIN_SHIFT_FIELDS`（app-utils.js:177-183「ここに載っていないフィールドは黙って消える」）→ 未登録なのは `changed` だけだが、app-staff.js:169 が `delete nw.changed` と**明示的に**消して作り直しており意図的。空振り。②`applyEditToSubs` の「締」トグル（app-admin.js:604-616）→ 「締」を出勤/退勤セルに単独・両方・時間付きで入れて消す4パターンを手で追い、`origStatus` の退避と復元がすべての順序で成立することを確認。空振り。**当たったのは3本目**の `diffSubForFlatWrite`（app-utils.js:565-571「sub全体をset/updateすると他端末の変更を巻き戻す」）で、ここから「subs を書く経路は何本あるか」を数え直したところ、`saveSubs`（差分書き込み）と `onSub`（sub全体 set）の2系統に割れていることが分かり、`onSub` の呼び出し元をたどって **app-admin.js:1835 の管理者用 SmModal** に行き着いた。

**2つ目「印が付いたデータを読む箇所を全部数える」を、書き手側に反転して適用した**。#58 は「印を読む側の数え上げ」だったが、今回は逆に**同じ意味を持つフィールドを書く側**を数えた。管理者が1日分のセルを編集する経路は3本ある — `applyEditToSubs`（シフト作成タブ）・`saveAdj`（提出一覧の詳細モーダル・app-admin.js:2930）・SmModal 経由（:1835）。**前2本は `isUpdated`/`updatedAt` を立てず、3本目だけが立てていた**。この「3本のうち1本だけが違う」が決め手になった。

### 検証したこと

- **提出メタ（`submittedAt`/`isUpdated`/`updatedAt`）を書く全経路の数え上げ**: `submittedAt` の書き手は3箇所（app-admin.js:554・:563＝いずれも `source:"grid"` 付きのグリッド新規作成／app-staff.js:192＝`isFirstSubmission` 判定済み）でいずれも正しい。`isUpdated`/`updatedAt` の書き手は3箇所（app-staff.js:193 submit＝正しい／app-staff.js:236・257 スタッフ側 SmModal＝編集者がスタッフ本人なので正しい／**app-admin.js:1835＝今回のバグ・修正済み**）
- **管理者のセル編集3経路の対称性**: `applyEditToSubs`（:530-618）・`saveAdj`（:2930-2934）はいずれも提出メタを触らず `shifts` のみ更新。修正後の :1835 がこれに揃った
- **修正の非回帰（Node再現）**: ①スタッフ本人が締切後に再提出した場合は従来どおり `subHasRealUpdate`＝`true` ②締切前のスタッフ再提出は従来どおり `false` ③既に `isUpdated:true` が立っている sub を管理者が編集しても値は保持される（スプレッドで引き継ぐため）④`saveSubs` の差分検出は `{...sub,shifts}` が新しいオブジェクト参照であるため従来どおり発火する
- **「締」トグルの不変条件の再確認（当たりなし）**: 休み提出日に「締」を①出勤セル単独 ②出勤＋退勤の両セル ③出勤セルに「締」＋退勤セルに時間 の3通りで入れて外す手順を追い、`origStatus` の退避（app-admin.js:583・610）と復元（:599-602）がどの順序でも成立し、`extraStart`/`extraEnd` が最後に必ず落ちることを確認
- **`ADMIN_SHIFT_FIELDS` の未登録フィールドの棚卸し（当たりなし）**: `changed`（管理者のトリプルクリックで永続化・app-admin.js:1187 `toggleChanged`）が未登録だが、app-staff.js:169 が再提出時に意図的に削除・再計算しており、管理者が「緑マークを消して確認済みにする」主用途では再提出後も消えたままになる（＝壊れない）
...
- 読み込み順 utils→core→staff→admin→main 維持・今回の差分に `fontSize<16` の新規混入 **0件**（今回の変更はJSXのpropsとコメントのみ）

### 要確認（未修正）

- **🟡（新規・再現手段がなく未修正）スタッフの提出書き込み（`onSub`）だけが、リスナーの巻き戻し保護と差分書き込みの両方を通っていない**（app-main.js:1461-1472）。`pendingSubWritesRef` は app-main.js:52-60 と :399-402 のコメントが「これがないと…後続の編集がその欠落状態のsubを丸ごと書き直すことで永久にデータが消える」と明記している保護機構だが、**登録しているのは `saveSubs`（:1146）1箇所のみ**。`onSub` は ①`pendingSubWritesRef` に登録しない ②`subsMapRef.current` を更新しない ③`fbSet` で **sub全体を set** する（`diffSubForFlatWrite` を通さない）④`setSubs(a)` が関数型更新でなく古いクロージャの配列を基準にする、の4点で `saveSubs` と非対称。理屈の上では「提出直後に別のechoで state が巻き戻り、続けて同じsubを編集すると直前の編集がサーバー上から消える」経路が成立するが、**Firebase の楽観的ローカル反映があるため成立条件を机上で確定できず、Node では再現できない**。推測での修正はしない方針に従い未修正とする。**着手するなら実機（2端末・提出状況一覧での連続セル編集）での再現が先**
- **🟡（次リリースで必須）index.html のキャッシュ版数を必ずバンプすること**。本番（origin/main）は `20260805-78e8888`、develop は `20260802-94ffc47`。**現時点の本番未反映は `origin/main..develop` の10コミット（うち fix 5件＝#55・#56・#57・#58・#59）**。版数を上げ忘れると app-staff.js・app-admin.js の実挙動が変わっているためブラウザキャッシュで旧ファイルが使われ、5件とも直っていない状態のまま配信される
...
- **🟢（新規）管理者が提出状況ビューでセルを編集すると、スタッフ提出値（`start`/`end`）そのものが書き換わる**（app-staff.js:530 `applyCellEdit`）。管理者の他の2経路は `adjustedStart`/`adjustedEnd` に書いて提出値を保護しているのに対し、SmModal はスタッフ本人の編集画面と共用のため生の提出値を直接更新する。「提出値に戻す」（`saveAdj` の「提出値」選択）では復元できない。**今回の修正の対象外**（提出メタの問題とは別で、SmModal の二重用途に踏み込む設計判断が要る）
...
- **🟢（新規）期間削除時に `subs` を全件 `once("value")` で読んでいる**（app-main.js:1096）。削除対象期間の subs を購読窓（直近3ヶ月）の外まで含めて消すための意図的な一括読みで、期間削除時のみ・1回限り。RULES.md が禁じているのは `accounts` 全件読みで違反ではないが、`orderByChild("periodId").equalTo()` を削除期間ごとに回せば読み量を絞れる
...
- **🟢（継続）実機E2E未実施**: #46 削除操作／#47 改名修正／#51 のスタッフ再提出／#52 の Excel 締めセル／#53 の片側入力／#54 の休み提出日メモ／#55・#57 の提出状況一覧セル／#58 のグリッド下書き→初回提出／**#59 の「管理者が提出状況ビューでセル編集→提出一覧の更新バッジが出ない」も実機E2Eは未実施**（`subHasRealUpdate`/`subLastActionTime` を require した Node 再現で確認したが、実際に管理者画面から編集して一覧のバッジ・並び順を目視確認してはいない）。SmModal の `applyCellEdit`・`onEditSub` は export されていないため**ユニットテストが無い**。#55・#57・#58 に続き、**export されていない関数で4回続けてバグが出ている**

### 総括

**10回連続で、実質コード変更ゼロの回に構造から 🟡 を1件見つけた**。#50〜#58 の9本と同じ根 —「**ある扱いが、それを必要とする全箇所に届いていない**」— の変種だが、今回は**向きが逆**だった。これまでの9本はすべて「**必要な扱いが1箇所に届いていない**」（除外を忘れた・引き継ぎを忘れた・抑制を忘れた）で、探し方は「正しくやっている箇所を数えて、抜けを探す」だった。今回見つかったのは「**不要な扱いが1箇所だけ余分に付いている**」で、3本ある管理者のセル編集経路のうち2本が正しく提出メタを立てず、1本だけが立てていた。

ここから次回の手法が出る。**「N箇所のうち1箇所だけ違う」を見つけたとき、多数派が正しいと決めてかからないこと**。#50〜#58 は多数派が正しく少数派が抜けだったため「抜けを埋める」で直った。今回は少数派が余分だったので、**埋めるのではなく取り除く**のが正解だった。数え上げで割れ目を見つけたら、次に問うのは「どちらが多いか」ではなく「**そのフィールドは誰の行為を表すのか**」である。`isUpdated` は「スタッフが再提出した」という**行為者付きの主張**なので、管理者が書いた時点で嘘になる。フィールド名に行為者が埋まっている（`submittedAt`・`isUpdated`・`source`）ものは、書き手の立場と突き合わせるだけで判定できる。

**コメントを検索クエリとして読む手法は10回目も有効**で、#58 の「まだ起点にしていないコメントから読む」という指示もそのまま効いた（未使用の3本を順に当たり、3本目で当たった）。次回の未使用の起点候補は、`pendingSubWritesRef` の宣言コメント（app-main.js:52-60「これがないと永久にデータが消える」＝**今回の🟡要確認の出どころで、まだ汲み尽くしていない**）、`sanitizeForSet`（app-utils.js:513-521「同期例外なので .catch() では捕捉できない」）、`restAppliedRef` の二重適用ガード（app-admin.js:620-622「Enterキー確定はhandleBlurを直接呼んだ後にネイティブblurも発火する」）あたり。特に3本目は「同じイベントが2回発火する」という**トグル系全般に効く形**なので、`toggleChanged`（:1179・トリプルクリック）が同じ二重発火に晒されていないかを次回の起点にできる。

害の質は #58（ノイズが信号を埋める）とも #55〜#57（意思が消える）とも違い、今回は**濡れ衣**である。「更新: 8/7 11:00」バッジは締切日ゲート付きで設計されており、管理者にとっては「このスタッフは締切を過ぎてから出し直した」という人事的な意味を持つ。それが**管理者自身がそのスタッフのセルを直した瞬間に立つ**。管理者は自分の編集操作とバッジ点灯を結び付けにくい（編集は提出状況ビュー、バッジは提出一覧タブと画面が違う）ため、**誤った情報がスタッフ本人の評価として使われるまで誰も気付かない**という誤り方になる。
