...
- **未検証**: 今回の🟡の修正は**実ブラウザで踏んでいない**。判定はすべて純粋関数
  （`isReservedShopAbbr`）へ出してユニットテストで固定したが、企業連携タブで実際に
  「西締」を入力してトーストが出るところは確認していない（`CompanyTab` は firebaseDB から
  他店舗の `shopAbbrs` を直接読むため、単体マウントの配線が重い）。変更したのは
  `addAbbr` の1行（判定式の差し替え）とトーストの文言だけ。
