...
## Shifty バグチェックレポート（2026-08-15 自動実行 #75）

### 修正済み

なし。**今回は修正が0件で、それが正しい結論である**（理由は「総括」参照）。

### 要確認（未修正）

- **🟡（新規）土日祝の日付色が固定の色リテラルで、ダークにだけ沈む**（app-admin.js:246・app-admin.js:1653）→ **BACKLOG化済み・デザイン判断待ち**
  375px・ダークのシフト作成タブで、日付列の **土曜 `#1976d2` = 3.18:1**・**日曜/祝日 `#e53935` = 3.46:1**（16px・非太字なので AA は 4.5:1）。**Chromium と WebKit 26.5 で同値**。土曜の青は**ライトでは 4.60:1 で AA を満たしており、ダークでのみ落ちる**＝「白背景を前提に選ばれた色が暗い背景にそのまま置かれている」形。該当2箇所は同じ式 `(day===0||isHol)?"#e53935":day===6?"#1976d2":"var(--c-text)"` で、**平日だけがテーマ変数に追随し、土日祝の2色だけが固定リテラル**という非対称になっている。ただし直すには「ダーク用の青／赤を決める」必要があり色の選択が入るため、既存の🟡「色のコントラストを決め切る」に**決めること3**として追記した
- **🟡 固定の淡い色の帯にダークで沈む文字色が直書きされている**（#72から継続）→ BACKLOG化済み・**デザイン判断待ち**。今回**新しい実例**を実測: 設定タブのポジションチップ（「調理長」「フロア」および削除の `×`）が `--c-danger` 直書き on 合成後 `rgb(51,44,60)` = **2.79:1**。3バナーだけの問題ではないことが確認できたのでBACKLOGへ追記した
- **🟡 アクセント塗り＋白文字が 2.85:1 で AA 未達**（#72から継続）→ BACKLOG化済み・**デザイン判断待ち**。ダーク79件の**過半がこれ**（タブバー8タブ・主要ボタン・「変更あり」バッジ等）
- **🟡 管理者画面のタップ可能要素がほぼ全て 44×44px 未満**（#74から継続）→ BACKLOG化済み・デザイン判断待ち
- **🟢 `index.html` の `?v=` と `build:` が今回の窓の変更を指していない**（`20260814-53f610f` のまま・以降 app-admin.js/app-core.js/index.html に変更あり）。**申し送りのみ**——`/release-to-main` の手順63が標準工程としてバンプするため BACKLOG 化しない
- **🟢 WebKit のシフト作成グリッドで空セルの input が scrollW 36 > clientW 34**（2px）。**中身が空なので何も切れておらず**、Chromium では出ない。閾値1pxのノイズと判定して修正しない
...
- 以下は #46〜#74 からの継続（状態変化なし）: 未claim店舗を誰でも自分の企業に取り込める根／`findActiveSubscription` の `|| live[0]`／`subscription_schedule.*` の4分岐が未購読／`planExpiry` のUTC基準計算／決済失敗フラグの誤解除／片側セルだけの日が0分／`purgeOldPeriods` のUTC日付／`getBreaksFor` の休憩判定／`generateInviteCode` のデッドコード／`urlLocked` が StaffView で未使用／`staffSectionOn` と `isCountExcluded`／SubsTab「月計」の別名重複／`sub.shopId` が死にフィールド／店舗削除が `global/shops/{id}` を消さない／`companyCodes` の TOCTOU／略称の先勝ち／期間作成の日付検証なし／提出状況ビューのセル編集／Excel analytics の数え方／スタッフ数バーのスペーサー／`breakTimes[].tags` の残留／別名解決順の不一致／`heatHours` が `adminRest` を無視／スタッフ削除時のキー残留／`onSub` のローカルstate更新キーの不一致

### 今回の対象と見つけ方

**#74 以降のコミットは0件**（`a915cc7` が #74 の記録で、そこからコードは1行も動いていない）。**検査すべき新しい差分は存在せず**、中身は #74 が指名した宿題そのものになった。#73・#74 と同じ形の回で、これで3回続けて「差分ゼロ・宿題実行」の回である。

**(1) 宿題は「375px × ダークで #72 のコントラスト検査と #73 のはみ出し検査を再実行する」だった**。#72 のコントラスト実測は 1400px 幅・ライト主体、#73 のはみ出し検査と #74 のタップ検査はどちらもライトのみで、**375px × ダークという組は一度も測られていなかった**。管理者8タブ＋スタッフ画面の9画面を、Chromium と WebKit 26.5 の両方で回した。

**(2) 測る前に「本当にその画面を測っているか」を確認したら、計測自体が壊れていた**。巡回スクリプトのセレクタを `button:has-text('スタッフ')` と書いたところ、これは**タブバーの「スタッフ」ではなく画面切替の「スタッフ画面」ボタンに一致**していた。つまりスタッフタブのつもりでスタッフ画面を測り、以降のタブは「見つからない」で飛ばされていた。**この状態でも overflow 検査は「9画面すべてページ幅OK・0件」と出す**——測っていない画面が「異常なし」に化ける典型で、#73 の教訓（測れなかった画面を黙らない）がそのまま当てはまった。`button:text-is(...)` の完全一致へ変え、さらに**各タブで `data-theme`・`body` の実背景色・閲覧のみバナーの有無・タップ要素数を毎回出力する事前確認スクリプト**を書いて、9画面すべてで `theme=dark` / `bodyBg=rgb(15,23,42)` / `claim=ok` を確認し、**タップ要素数が #74 の表（20/43/25/15/139/11/15/66）と完全一致することを突き合わせてから**本計測に入った。

**(3) コントラスト検査でも同じ穴を踏みかけた**。巡回スペックの `pre`（「管理者画面」へ入る前置き操作）は **`--fresh` を付けたときしか実行されない**仕様で、`--fresh` を持たないコントラスト検査では前置きが走らず、**スタッフ画面1枚だけを測って「ダーク12件」と報告するところだった**。前置きを `audit:false` の通常ステップに書き換えて9画面に到達させたところ、件数は**ダーク12件 → 79件**に変わった。

**(4) 結果は「ダーク79件・ライト113件」だが、意味があるのは『ダークにだけ出る6件』だけだった**。両テーマに出る違反は元の配色の詰めの問題で、**片方にだけ出る違反が「色がテーマに追随していない」の第一候補**になる。6件の内訳は、3件がアクセント塗り（既知の🟡・BACKLOG決めること2）、**残る3件が土曜の日付色**だった。

**(5) 「小さい違反」ではなく「非対称」を根拠にした**。土曜の青は**ライト 4.60:1（AA可）→ ダーク 3.18:1（AA不可）**で、片方でだけ落ちる。さらに該当2箇所は同じ1行の中で**平日だけ `var(--c-text)` を使い、土日祝の2色だけ固定リテラル**という非対称を持つ。この2つが揃って初めて「意図した配色」ではなく「ダークを想定していない色がそのまま残っている」と言える。

**(6) それでも修正はせずBACKLOGへ回した**。直すには「ダーク用の青／赤」を新しく決める必要があり、`--c-accent`/`--c-danger` は index.html に「ブランド色なのでテーマ非依存」と明記して :root だけで定義されているため、**曜日色に対応する変数が存在しない**。さらに同じ意味に**4色**が使われている（スタッフ画面 土=`#3B82F6`・日=`#FF4757`／管理者画面 土=`#1976d2`・日=`#e53935`）ので、どれを正とするかも同時に決まってしまう。**色の選択が入る＝ユーザー判断**という #72 以来の線引きに従った。

### 検証したこと

- `DEV_MODE`（app-core.js:12・式のまま）／`DEV_PLAN_OVERRIDE`（:125）正常
- `subs` の `set()` 全体上書き **0件**／`accounts` 全件読み **0件**／`global/shops` の全件読み **0件**／CF `.delete()` 誤用 **0件**／`purgeInactiveShops` の安全装置（`Number.isNaN` スキップ・`archived/shops` 二段削除）維持
...
- **375px × ダークの実測（標準テスト店舗・`?plan=premium`・管理者8タブ＋スタッフ画面の9画面）**:
  - **はみ出し検査: Chromium 0件・WebKit 26.5 も 0件**（空セル input の2px 1件を除く）。9画面すべてで `documentElement.scrollWidth == 375`。**#73 の `flexWrap` 修正がダークでも非回帰**であることを確認
  - **コントラスト検査: ダーク79件・ライト113件・ダークにだけ出る違反6件**。WebKit でも**ダーク79件で完全一致**
  - **「色指定がなく既定の黒になっていた」ケースは 0件**＝ #72 の `3490707` がダーク375pxでも非回帰
  - 静的走査（明るい固定背景＋色指定なし＝巡回に出ないモーダル・エラー状態の取りこぼし検出）も **0件**
- **数値の検算**: 土曜 `#1976d2` on 白 = 4.60:1 / on `#1E293B` = 3.18:1、日曜 `#e53935` on 白 = 4.23:1 / on `#1E293B` = 3.46:1 を手計算で再現し、計測値と一致することを確認
- リポジトリは**無変更**（計測物はすべてスクラッチパッドに置き、`git status` が着手前と同一であることを確認）
- **未検証**: 実機のiOS Safari（WebKitエンジンでの確認に留まる）／375px 以外の狭い幅（320px 等）／ダークでのタップ領域実測（#74 のライト実測から寸法は変わらない見込みだが測っていない）／モーダル類（UpgradeModal・TermsModal・CellEditPanel）は巡回に出ないため**ダークで未計測**（静的走査では0件だが、走査は「固定の明るい背景＋色指定なし」しか見ない）／🟡の実機E2E（スタッフURLでの他人の提出削除）は今回も未実施

### 総括

**今回の主な収穫は、検出したバグではなく「測定が壊れていたことを2回とも出荷前に捕まえた」ことである。** セレクタが別のボタンに一致して8タブが飛ばされていた件と、前置き操作が `--fresh` 限定で実行されず1画面しか測っていなかった件——どちらも**検査ツールは正常終了し、それらしい件数を返していた**。前者は「9画面すべてページ幅OK・0件」、後者は「ダーク12件」という、**そのまま報告に書けてしまう体裁の整った数字**を出す。#73 が「未計測を黙ると『他は0件』が全画面の結論に化ける」と書いた通りのことが、今度は自分の側で起きかけた。

**だから今回入れた事前確認が本体だった。** 各画面で `data-theme` と実際の `body` 背景色、閲覧のみバナーの有無、タップ要素数の4つを毎回出力し、**タップ要素数を #74 の表と突き合わせて同一画面であることを確認してから**計測に入った。同じ数字（20/43/25/15/139/11/15/66）が出たことで初めて「#74 と同じ9画面を、今度はダークで測っている」と言える。**前回の実測値が今回の測定の正しさを検証する装置になった**——これはログを数字つきで残していたから成立した使い方で、記録の副次的な用途として覚えておく価値がある。

**「ダークにだけ出る違反」という切り口は、3回目にして最も切れ味があった。** #72 のコントラスト実測は118件、今回のダークは79件で、**件数はどちらも多すぎて単体では何も指さない**（#74 のタップ領域309件と同じ問題）。しかし79件を「両テーマに出る」と「ダークにだけ出る」に割ると後者は6件に落ち、さらにその6件から既知のアクセント塗り3件を引くと**3件しか残らない**。#74 では第2の軸が「取り返しがつくか」だったが、今回は**「片方のテーマでだけ壊れるか」**だった。閾値検査を有効にする第2の軸は毎回違うが、**「常態から外れているものを探す」という形は共通している**。

**土曜の青を指摘の中心に据えたのは、それが唯一『非対称』を2つ持っていたからである。** ①ライトでは AA を満たしダークでだけ落ちる ②同じ1行の中で平日だけがテーマ変数を使い土日祝だけが固定リテラル。日曜の赤（3.46:1）の方が悪いように見えるが、赤はライトでも 4.23:1 で未達なので「ダークを想定していない」証拠にはならず、**単に元の色が濃すぎるという別の話**になる。**より悪い数値ではなく、より説明のつく数値を選んだ**。

**それでも直さなかった判断は #72・#74 と同じ線である。** 「既定の黒に落ちている」（#72）や「他の削除には confirm があるのにここだけ無い」（#74）は、**直し方に色や寸法の選択が入らない**から loop が直せた。今回は `--c-sat`/`--c-sun` に**何色を入れるか**を決めないと直せず、しかも同じ意味に4色が散らばっているので統一の是非まで巻き込む。3回連続で同じ判定基準が機能しており、**この線引きはもう安定したと見てよい**。

**次回の手を1つに絞る。** 「描画結果を測る」軸は #72〜#75 の4回で、ライト1400px・ライト375px・ダーク375px を埋め、**残っているのは「巡回に出てこない画面」だけ**になった。UpgradeModal・TermsModal・CellEditPanel・SmModal・各種エラー/空状態は、**4回とも一度も計測されていない**（静的走査は「明るい固定背景＋色指定なし」しか見ないので代わりにならない）。しかもモーダルは**固定の明るい背景を持ちやすい＝ダークで沈む条件が揃った場所**で、決めること1がまさにその形である。**次はモーダル・空状態・エラー状態を開いた上で375px×ダークのコントラストとはみ出しを測る**。開くのに必要な操作（アップグレード導線・規約ボタン・セル編集・提出0件の期間）はすべてUI操作で到達でき、**コードと実機だけで閉じられ、ユーザー判断を要しない**。
...
**影響範囲**: index.html（CSS変数を増やす場合）、app-admin.js（3バナー・タブ・バッジ・ボタン・日付列の曜日色 :246/:1653・ポジションチップ）、app-staff.js（出勤/休みの選択状態・CellEditPanel・曜日色）、app-core.js（`AB` 等のスタイル定数）
**備考**: バグチェック#72（2026-08-13）で検出・**条件B（仕様＝デザイン判断）に該当**。グローバル指示「UIデザインの基準」により、検出結果を一括修正で先行させずユーザーの合意を得てから直す。既定の黒に依存していた24件（最悪1.18:1・シフト作成タブ）は判断不要の実装バグだったため同日 `3490707` で修正済み。**決めること3は バグチェック#75（2026-08-15）で追加**——#72 のコントラスト実測は1400px幅・ライト主体だったため、375px×ダークで初めて出た。#75 の実測では「色指定がなく既定の黒」は**0件**で `3490707` の非回帰を確認済み。
