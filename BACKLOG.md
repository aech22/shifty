...
**✅ 2026-08-31 に本番反映まで完了**（ルール → クライアント → データの順）。対象は引き継ぎの「12店舗」ではなく
**11店舗**だった——本番の `accounts` を読んで「有料プランだが Stripe 契約なし」で機械的に導出した結果で、
`鷄えん東通り`（`shop_1780453329813`）だけは **Stripe契約を持つ実課金店舗**なのでフラグを書いていない。
デモと「新しい店舗」×2 はプラン未設定なので対象外（Freeのアップグレード導線を残す）。

書き込みは監査時のリストを固定せず**書く直前に同じ規則で再導出**し、Stripe契約のある店舗が1件でも
混ざったら中止する安全弁を通した。結果 **11/11件成功・対象外への誤書き込み0件**。

**本番での実測**（配信版数 `20260831-27f8a76`・実ブラウザ・読み取りのみ）:

| 店舗 | タブ | 判定 |
|---|---|---|
| 京月梅田（`billingExempt=true`） | 期間・スタッフ・候補・提出一覧・シフト作成・企業連携・設定 | **マイページなし** |
| 鷄えん東通り（実課金・フラグ無し） | 上記＋**マイページ** | 従来どおり（解約導線が生きている） |
