...
## 🔴 終了した期間のシフトを「確定」させ、その後のマスタ変更を反映しない（シフト作成タブ）

**目的**: シフト作成タブは staffList・スタッフ属性・ポジション・従業員番号・退勤延長などを**すべて現在値**で参照している。そのため**スタッフを1人削除すると、過去に配り終えたシフト表からその人の列が消える**（属性・ポジション・退勤延長を変えれば過去期間の集計も動く）。画面と配布済みのExcel/PDFが食い違い、あとから「先月のシフトを確認する」ができない。終了した期間はその時点のマスタで固定する。

**決定済み（2026-08-21 ユーザー判断）**:
- **確定済み期間でも、シフト作成タブでのセル編集は今までどおり可能**（凍結するのはマスタ側であって、シフトそのものではない）
- **凍結する範囲**: スタッフ一覧／スタッフ属性・属性別制限／ポジション・必要ポジション／従業員番号／退勤延長／名前色・別名／勤務先店舗／休憩時間／候補時間（曜日別・日付別）

**設計上の急所（実装前に必ず読むこと）**: 「最終日を超えたら自動確定」は、**写しを撮れるのがアプリの動いている瞬間だけ**という原理的な制約を持つ。最終日の翌日にスタッフを削除して3日後にアプリを開くと、その時点で撮った写しには既にその人がいない。したがって **「期間が生きている間（`today <= endDate`）はシフト作成タブを開くたびに写しを更新し続け、最終日を超えたら更新を止める」** 形にする。確定の瞬間に撮るのではなく、確定までずっと最新化しておく。

**受け入れ条件**:
- [ ] `period.snapshot`（`{staffList, settings:{...凍結対象キー}}`）と `period.lockedAt` を期間オブジェクトに持たせる
- [ ] `today <= endDate` の期間は現在値で動作し、シフト作成タブを開いたときに写しを更新する（**内容に変化があったときだけ書く**／`ownerReadOnly` 端末は書かない）
- [ ] `today > endDate` かつ写しがある期間は、staffList と凍結対象 settings を**写しから読む**
- [ ] 写しが無い過去期間（既存データ）は従来どおり現在値で動く（遡って写しを作らない＝今日の値で過去を固定してしまわないため）
- [ ] 凍結は**シフト作成タブの中だけ**に効かせる。スタッフタブ・設定タブ・候補タブ・提出一覧は常に現在値で動く（`ShiftEditTab` 内で解決する。`AdminView` の props を差し替えない）
- [ ] 凍結の効果が Excel・PDF 出力にも及ぶことを確認する（`expXl`・`buildPdfCols` が同じ `staffList`/`settings` を受け取る経路であることの確認を含む）
- [ ] 「確定を解除」ボタン（オーナー限定・押すと現在値に戻る）を付ける — 無いと押し間違い・仕様誤解からの復旧手段が無くなる
- [ ] 確定中であることが画面で分かる（期間ドロップダウン横のバッジ等）
- [ ] 凍結判定・写しの差分検出を `app-utils.js` の純粋関数に切り出し、`tests/core.test.js` にユニットテストを追加する
- [ ] `npm test` 全パス・`npx eslint app-*.js` 0 errors
- [ ] `?v=` と `build:` のバンプはリリース工程（`/release-to-main` 手順63）で行う

**影響範囲**: `app-admin.js`（`ShiftEditTab`: `realStaff`:457／`hallStaff`:459／`gridStaff`:1076、`settings.*` を読む全箇所、`getOT`/`getBreaksFor` へ渡す settings、`expXl`・`buildPdfCols` の呼び出し）、`app-main.js`（`savePeriods` 経由の写し書き込み）、`app-utils.js`（純粋関数）、`tests/core.test.js`。**Firebase セキュリティルールの変更は不要**（`periods` は既にオーナー限定書き込み・`.validate` は `id` の存在のみを要求するため、フィールド追加で拒否されない）。

**備考**: 2026-08-21 にユーザーから直接依頼・同日に判断2点（セル編集可・凍結範囲）を確定。優先度🔴は「過去のシフト表が黙って変わる＝配布済みの記録が失われる」ため（VISION 設計原則5「データを勝手に失わない」に触れる）。データ量の目安: 20名店舗で写し1件あたり数KB程度、期間は月2件想定なので `periods` の肥大は許容範囲だが、`purgeOldPeriods`（36ヶ月超削除）が写しごと消すことを確認しておくこと。
