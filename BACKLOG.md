...
**受け入れ条件**:
- [ ] **疎通確認**: `charming-euphoria` の詳細ページからテストイベント（`invoice.payment_succeeded` 等）を送信し、直後の `stripeWebhook` ログで **200** を確認する。**今日デプロイした4件が実際に動く状態かは、まだ一度も positive に確認できていない**（配信0のため）
- [ ] **残骸データの整理**: 2026/07/09 に解約・返金済みの2契約に対応する `accounts/{shopId}` が、いまも `plan:"premium"` のまま残っている。解約当時のコードは店舗を特定できずプランを落とせなかった（`4acd089` で修正済みだが過去のイベントには遡及しない）
      - 推奨: Stripe から当時の `customer.subscription.deleted` を**手動再送**する。残骸の解消と「修正が本当に効くか」の検証を兼ねられる
      - 代替: `accounts/{shopId}/plan` を直接 `free` に戻す（検証は兼ねられない）
- [ ] 対象は `accounts` 13件のうち `stripeCustomerId` を持つ2件（残り11件は `planExpiry:2100-06-29` の自家用シードで対象外）
**影響範囲**: 本番Firebase の `accounts/{shopId}`（データのみ・コード変更なし）、Stripeダッシュボード操作
**備考**: 2026-08-11 のCF本番デプロイ後の確認で検出・**条件A（ユーザーにしか実行できない操作）に該当**。本番データの変更を伴うためユーザーの指示を得てから実行する。
