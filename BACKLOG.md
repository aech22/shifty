...
- [ ] **疎通確認（未達・手段が無い）**: 本番モードには**テストイベントを送る手段が存在しない**ことを2026-08-11に確認した。①エンドポイントの「…」メニューは「無効化／シークレットキーを更新／削除」のみでテスト送信が無い ②ワークベンチの Shell は「本番環境では読み取り専用」で `stripe trigger` が使えない ③7/09のイベント再送は「Events older than 30 days have limited data」に阻まれる。**したがって、デプロイした4件が実データで動くことはまだ一度も確認できていない**
      - 案A（費用0）: 次の実課金の発生時に確認する。ただし現在課金活動がなく時期が読めない
      - 案B（推奨・費用は返金可能）: テスト店舗でアプリから Pro(¥500) を実購入 → `plan` が `pro` になるのを確認 → カスタマーポータルから解約 → `free` に戻るのを確認 → 返金。署名検証・`checkout.session.completed`・`customer.subscription.deleted`・`subscription_data.metadata` による店舗特定までを一度に検証できる
      - **補足**: 18件の400は「関数が起動し・リクエストを受け・最後まで実行された」ことの証拠でもあるため、疎通そのものは動いている。未確認なのは「本番の署名シークレットが正しいか」と「署名通過後の業務ロジック」の2点に絞られている
