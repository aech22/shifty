  ...
  return(
    <div style={{background:"var(--c-bg)",minHeight:"calc(100vh - 44px)"}}>
        ...
        <div style={{maxWidth:900,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
                    ...
                    <div style={{position:"absolute",top:"calc(100% + 6px)",left:0,background:"var(--c-card)",borderRadius:12,boxShadow:"0 8px 24px rgba(0,0,0,.2)",zIndex:200,minWidth:180,overflow:"hidden"}}>
                      {shops.map(sh=>(
                        <div key={sh.id} style={{display:"flex",alignItems:"center",background:sh.id===currentShopId?"rgba(248,112,54,.12)":"var(--c-card)",borderBottom:"1px solid var(--c-border)"}}>
                          <div onClick={()=>{setCurrentShopId(sh.id);setShopMenuOpen(false);}} style={{flex:1,padding:"11px 16px",cursor:"pointer",fontSize:14,fontWeight:sh.id===currentShopId?700:400,color:sh.id===currentShopId?"var(--c-accent-text)":"var(--c-text)",display:"flex",alignItems:"center",gap:8}}>
                            {sh.id===currentShopId&&<span style={{fontSize:10}}>✓</span>}{sh.name}
                          </div>
                          <button onClick={e=>{e.stopPropagation();const isLast=shops.length<=1;const msg=isLast?`ログアウトしますか？（この端末の店舗セッションを終了します）`:`「${sh.name}」からログアウトしますか？（他の店舗のセッションは維持されます）`;if(!window.confirm(msg))return;setShopMenuOpen(false);if(logoutShop)logoutShop(sh.id);else logout();}} style={{padding:"6px 10px",margin:"0 8px",background:"rgba(255,71,87,.08)",border:"1px solid rgba(255,71,87,.2)",borderRadius:4,color:"var(--c-danger-text)",fontSize:11,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>ログアウト</button>
                        </div>
                      ))}
                      <div style={{borderTop:"1px solid var(--c-border)",padding:"8px 10px",display:"flex",gap:6}}>
                        ...
                        }} style={{flex:1,padding:"7px",background:"var(--c-accent-solid)",border:"none",borderRadius:8,fontSize:12,fontWeight:700,color:"white",cursor:"pointer"}}>＋ 新規</button>
                      </div>
                      {/* 店舗コードで追加パネル */}
                      {shopCodeMode&&<div style={{borderTop:"1px solid var(--c-border)",padding:"10px"}}>
                        ...
                        <div style={{display:"flex",gap:6}}>
                          ...
                          <button onClick={addShopByCode} style={{padding:"7px 10px",background:"var(--c-accent-solid)",border:"none",borderRadius:8,fontSize:12,fontWeight:700,color:"white",cursor:"pointer"}}>追加</button>
                        </div>
                        {shopCodeError&&<div style={{fontSize:11,color:shopCodeError==="確認中..."?"var(--c-warn-text)":"var(--c-danger)",marginTop:4}}>{shopCodeError}</div>}
                      </div>}
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            {[["periods","期間"],["staff","スタッフ"],["candidates","候補"],["submissions","提出一覧"],["edit","シフト作成"],["company","企業連携"],["mypage","マイページ"],["settings","設定"]].map(([id,l])=>(
              <button key={id} onClick={()=>setTab(id)} style={{padding:"7px 13px",background:tab===id?"var(--c-accent-solid)":"var(--c-input)",border:`1px solid ${tab===id?"var(--c-accent)":"var(--c-border)"}`,borderRadius:8,color:tab===id?"white":"var(--c-text2)",fontSize:12,fontWeight:600,cursor:"pointer"}}>{l}</button>
            ))}
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"20px 14px 60px"}}>
        {DEMO_MODE&&<div style={{background:"var(--c-note-bg)",border:"1px solid var(--c-note-border)",borderRadius:8,padding:"14px 16px",marginBottom:16,display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
          <div style={{flex:1,minWidth:200}}>
            <div style={{fontSize:13,fontWeight:700,color:"var(--c-note-text)",marginBottom:2}}>これはデモです（サンプル店舗・全機能が使えるPremium表示）</div>
            <div style={{fontSize:12,color:"var(--c-note-text)"}}>自由に触って試せます。入力内容は保存されず、ページを再読み込みすると元に戻ります。</div>
          </div>
          {/* ?start=1 を付けるのはハッシュだけを外すと同一ドキュメント遷移になり再読み込みされないため。
              GA4でデモからの「無料で始める」到達を計測できる副次効果もある */}
          <a href={window.location.pathname+"?start=1"} style={{padding:"10px 18px",background:"var(--c-accent-solid)",color:"#fff",borderRadius:8,fontSize:14,fontWeight:700,textDecoration:"none",whiteSpace:"nowrap"}}>無料で始める</a>
        </div>}
        {ownerReadOnly&&<div style={{background:"var(--c-warn-bg)",border:"1px solid var(--c-warn-border)",borderRadius:8,padding:"12px 16px",marginBottom:16,display:"flex",alignItems:"center",gap:10}}>
          <div style={{flex:1}}>
            <div style={{fontSize:13,fontWeight:700,color:"var(--c-warn-text)",marginBottom:2}}>この端末は管理者として登録されていません（閲覧のみ）</div>
            <div style={{fontSize:12,color:"var(--c-warn-text)"}}>設定・シフトの変更を行うには、登録済みの端末の「設定タブ → 管理コード」を「店舗名ボタン → コードで追加」に入力してください。</div>
          </div>
        </div>}
        {paymentFailed&&<div style={{background:"var(--c-danger-bg)",border:"1px solid var(--c-danger-border)",borderRadius:8,padding:"12px 16px",marginBottom:16,display:"flex",alignItems:"center",gap:10}}>
          <div style={{flex:1}}>
            <div style={{fontSize:13,fontWeight:700,color:"var(--c-danger-text)",marginBottom:2}}>決済に失敗しました</div>
    ...
    <div ref={scrollRef} onScroll={onScroll} style={{overflowX:"auto",...(maxH?{overflowY:"auto",maxHeight:maxH}:{}),border:BD,borderRadius:8,flex:rowH?undefined:1,minWidth:rowH?undefined:200}}>
      ...
      <table style={{borderCollapse:"collapse",minWidth:"max-content"}}>
        ...
        <tbody>{dates.map(date=>{
          ...
          const dc=(day===0||isHol)?"var(--c-sun)":day===6?"var(--c-sat)":"var(--c-text)";
          return(<tr key={date} style={rowH?{height:rowH}:{}}>
            ...
            {heatHours.map((hr,hi)=>{const n=countHeat(section,date,hr);return(
              <td key={hi} style={{minWidth:22,padding:"2px 1px",textAlign:"center",fontSize:11,borderLeft:BD,borderBottom:BD,background:hBg(n,maxC),color:n===0?"var(--c-text4)":"var(--c-heat-ink)",fontWeight:n>0?600:400,verticalAlign:"middle"}}>{n||""}</td>
            );})}
          </tr>);
        })}</tbody>
      </table>
          ...
          <tbody>{rows.map(row=>{
            ...
            return(<tr key={row.id} style={{background:bg}}>
              ...
              {mapGridCols(name=>{const min=row.getMin(name);const vio=row._violateFn?row._violateFn(name,min):false;const cellBg=vio?"rgba(255,71,87,.15)":bg;return(
                <td key={name} style={{width:colW,minWidth:colW,maxWidth:colW,boxSizing:"border-box",padding:"3px 2px",borderLeft:BD,borderBottom:BD,textAlign:"center",fontSize:11,background:cellBg,fontWeight:(row._bold||vio)&&min>0?700:400,color:min>0?(vio?"var(--c-danger-text)":(row._color||"var(--c-text2)")):"var(--c-text4)"}}>{min>0?fmtH4(min):""}</td>
              );},spacerCell)}
            </tr>);
          })}</tbody>
        </table>
...
function ShiftEditTab({subs,periods,staffList:staffListProp,onSave,tt,settings:settingsProp,plan,shopId,shopName,onUpgrade,allLinkedShops=[],onLoadPastSubs,pastSubsLoaded=false,savePeriods,ownerReadOnly=false}){
  ...
  const nameColor=name=>((settings.staffColors||{})[name]==="red"?"var(--c-sun)":"var(--c-text)");
  ...
  // 濃さはテーマごとの --c-heat-scale で調整する（ダークは重ねすぎると文字が読めなくなる）
  const hBg=(n,mx)=>n===0?"transparent":`rgba(248,112,54,calc(${(0.15+(n/mx)*0.75).toFixed(3)} * var(--c-heat-scale)))`;
  ...
  const periodRows=[
    {id:firstHalf?.id||"nofirst",label:firstHalf?.label||`${mo2}月前半`,getMin:name=>firstHalf?getPeriodMin(firstHalf.id,name):0,
      _bold:firstHalf?.id===selPid,_color:firstHalf?.id===selPid?"var(--c-accent-text)":undefined,_bg:firstHalf?.id===selPid?"rgba(248,112,54,0.10)":undefined},
    {id:secondHalf?.id||"nosecond",label:secondHalf?.label||`${mo2}月後半`,getMin:name=>secondHalf?getPeriodMin(secondHalf.id,name):0,
      _bold:secondHalf?.id===selPid,_color:secondHalf?.id===selPid?"var(--c-accent-text)":undefined,_bg:secondHalf?.id===selPid?"rgba(248,112,54,0.10)":undefined},
    {id:"total",label:"月計",getMin:name=>sameMoPeriods.reduce((a,p)=>a+getPeriodMin(p.id,name),0),_bold:true,_color:"var(--c-accent-text)",
    ...
    {id:"monthly_limit",label:"月上限",getMin:name=>{const t=(settings.staffAttributes||{})[name]||"parttime";const tls={employee:{name:"社員"},parttime:{name:"バイト"},...(settings.staffTypeLimits||{})};const l=tls[t];return(l&&typeof l==="object"&&l.monthly)?l.monthly*60:0;},_color:"var(--c-sat)",_bg:"rgba(96,165,250,0.07)"}
  ];
    ...
    <div ref={outerRef} style={{padding:"12px 8px"}}>
      ...
      <div style={{marginBottom:10,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
        ...
        {period&&isPeriodEnded(period,todayStr)&&!ownerReadOnly&&savePeriods&&(periodLocked
          ?<React.Fragment>
            <span title="この期間はスタッフ・属性・退勤延長などを終了時点の内容で固定しています（シフトの編集は可能）"
              style={{padding:"3px 8px",background:"rgba(248,112,54,.15)",border:"1px solid rgba(248,112,54,.45)",borderRadius:4,color:"var(--c-accent-text)",fontSize:11,fontWeight:700,whiteSpace:"nowrap"}}>確定済み</span>
        ...
        {isPremium&&<button onClick={handleSaveAll}
          style={{padding:"6px 14px",background:"var(--c-accent-solid)",border:"none",borderRadius:8,color:"white",fontSize:13,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap"}}>
          保存
        </button>}
      </div>
      ...
      {Object.keys(dupErrors).length>0&&(
        <div style={{background:"rgba(255,71,87,.08)",border:"1px solid rgba(255,71,87,.3)",borderRadius:8,padding:"8px 12px",marginBottom:10}}>
          <div style={{fontSize:12,fontWeight:700,color:"var(--c-danger-text)",marginBottom:4}}>⚠ 出勤がだぶついています（他店舗と時間重複）</div>
      ...
      {!period?<div style={{color:"var(--c-text3)"}}>期間を選択してください</div>:(
        <div style={useBreakout?{marginLeft:-(containerLeft+8),width:"100vw",paddingLeft:8,paddingRight:8,boxSizing:"border-box",display:hasPanel?"flex":"block",justifyContent:fitsCentered?"center":"flex-start",alignItems:"flex-start",gap:4}:{}}>
            ...
            <table style={{borderCollapse:"collapse",width:fitAll?"100%":"unset",minWidth:fitAll?"unset":"max-content"}}>
              ...
              <tbody ref={gridBodyRef}>
                {dates.map(date=>{
                  ...
                  const dc=(isSun||isHol||isSpecRed)?"var(--c-sun)":isSat?"var(--c-sat)":"var(--c-text)";
          ...
          {(positionErrorEntries.kitchen.length>0||positionErrorEntries.hall.length>0||positionErrorEntries.all.length>0)&&(
            <div style={{background:"var(--c-danger-bg)",border:"1px solid var(--c-danger-border)",borderRadius:8,padding:"8px 12px",marginBottom:10}}>
              <div style={{fontSize:12,fontWeight:700,color:"var(--c-danger-text)",marginBottom:4}}>⚠ ポジションが不足しています</div>
              <div style={{fontSize:12,color:"var(--c-text2)",lineHeight:1.7}}>
          ...
          {weeks.length>0&&<SummaryTable
            ...
            }),{id:"weekly_limit",label:"週上限",getMin:name=>{const t=(settings.staffAttributes||{})[name]||"parttime";const tls={employee:{name:"社員"},parttime:{name:"バイト"},...(settings.staffTypeLimits||{})};const l=tls[t];return(l&&typeof l==="object"&&l.weekly)?l.weekly*60:0;},_color:"var(--c-sat)",_bg:"rgba(96,165,250,0.07)"}]}
          />}
  ...
  return(
    <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
        <AT>期間管理</AT>
        <button onClick={()=>{setShow(v=>!v);setUsePreset(true);setPresetDeadline("");setForm({label:"",startDate:"",endDate:"",deadlineDate:""}); }} style={{padding:"9px 16px",background:"var(--c-accent-solid)",border:"none",borderRadius:8,color:"white",fontSize:13,fontWeight:700,cursor:"pointer"}}>＋ 新しい期間を作成</button>
      </div>
      {plan==="free"&&<div style={{fontSize:12,color:"var(--c-text3)",marginBottom:10,background:"var(--c-card)",border:"1px solid var(--c-border)",borderRadius:8,padding:"7px 10px"}}>
        ...
        {periods.length>=PLAN_LIMITS.free.periods&&<span style={{marginLeft:8,color:"var(--c-warn-text)",fontSize:11}}>期間追加はProプランで利用できます</span>}
      </div>}
      {show&&<AC title="新しい期間を作成">
        {/* プリセット使用 / 手動入力 の切り替え */}
        <div style={{display:"flex",gap:8,marginBottom:16}}>
          <button onClick={()=>{setUsePreset(true);setPresetDeadline("");}} style={{flex:1,padding:"9px 0",border:`2px solid ${usePreset?"var(--c-accent)":"var(--c-border2)"}`,borderRadius:8,background:usePreset?"rgba(248,112,54,.15)":"rgba(0,0,0,.03)",color:usePreset?"var(--c-accent-text)":"var(--c-text3)",fontSize:13,fontWeight:700,cursor:"pointer"}}>プリセットから選ぶ</button>
              ...
              :<>
                <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12}}>
                  <div>
                    ...
                    {p.deadlineDate&&<div style={{fontSize:12,marginTop:3,color:ip?"var(--c-sun)":"var(--c-text4)"}}>締切：{p.deadlineDate.replace(/-/g,"/")} {ip?"（済み）":""}</div>}
                ...
                {/* URLシェア */}
                <div style={{marginTop:10,padding:"8px 12px",background:"var(--c-input)",borderRadius:8,display:"flex",alignItems:"center",gap:8}}>
                  <span style={{fontSize:11,color:"var(--c-text4)",flexShrink:0}}>URL</span>
                  <span style={{fontSize:11,color:"var(--c-text2)",flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{pUrl}</span>
    ...
    <div>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:"var(--c-card)",borderRadius:12,padding:"18px 18px 14px",maxWidth:440,width:"100%",maxHeight:"85vh",overflowY:"auto",boxShadow:"0 8px 32px var(--c-shadow)"}}>
            ...
            <div style={{display:"flex",gap:8,justifyContent:"flex-end",flexWrap:"wrap"}}>
              ...
              {isRetained&&<button onClick={undoDelete} style={{...AGray,color:"var(--c-accent-text)",borderColor:"var(--c-accent)"}}>削除を取り消す</button>}
      ...
      <AC title="スタッフ一覧">
        {!isPro&&<div style={{fontSize:12,color:"var(--c-text3)",marginBottom:10,background:"var(--c-card)",border:"1px solid var(--c-border)",borderRadius:8,padding:"7px 10px"}}>
          ...
          {!isPro&&<span style={{marginLeft:8,color:"var(--c-warn-text)",fontSize:11}}>並べ替え・名前色変更はProプラン（500円/月）で利用できます</span>}
        </div>}
        ...
        {displayRows.map(row=>row.kind==="retained"?(
          ...
          {isSpacer(n)
            ...
            {editKey===n
              ...
              :<>
                ...
                {isPro&&<button onClick={()=>{setAliasKey(aliasKey===n?null:n);}} style={{padding:"6px 10px",background:aliasKey===n?"rgba(248,112,54,.15)":"rgba(248,112,54,.06)",border:`1px solid ${aliasKey===n?"var(--c-accent)":"rgba(248,112,54,.3)"}`,borderRadius:4,color:"var(--c-accent-text)",fontSize:12,cursor:"pointer",minWidth:64,textAlign:"center"}}>
                  別名{(staffAliases[n]||[]).length>0?` (${(staffAliases[n]||[]).length})`:""}
                </button>}
                {isPremium&&<button onClick={()=>{setPosKey(posKey===n?null:n);}} style={{padding:"6px 8px",background:posKey===n?"rgba(59,130,246,.15)":"rgba(59,130,246,.06)",border:`1px solid ${posKey===n?"#3B82F6":"rgba(59,130,246,.3)"}`,borderRadius:4,color:"var(--c-sat)",fontSize:12,cursor:"pointer",width:118,boxSizing:"border-box",flexShrink:0,whiteSpace:"nowrap",textAlign:"center"}}>
                ...
                <button onClick={()=>startEdit(n)} style={{padding:"6px 10px",background:"rgba(59,130,246,.08)",border:"1px solid rgba(59,130,246,.25)",borderRadius:4,color:"var(--c-sat)",fontSize:12,cursor:"pointer"}}>編集</button>
                <button onClick={()=>del(i)} style={AD}>削除</button>
              </>
            }
          </div>}
          ...
          {isPro&&aliasKey===n&&(
            <div style={{marginTop:4,padding:"12px 14px",background:"rgba(248,112,54,.04)",border:"1px solid rgba(248,112,54,.2)",borderRadius:8,position:"sticky",left:0,maxWidth:"calc(100vw - 76px)",boxSizing:"border-box"}}>
              <div style={{fontSize:12,fontWeight:700,color:"var(--c-accent-text)",marginBottom:8}}>別名（スタッフが入力できる名前）</div>
              {/* 登録済み別名 */}
              {(staffAliases[n]||[]).length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:10}}>
                {(staffAliases[n]||[]).map((alias,ai)=>(
                  <div key={ai} style={{display:"flex",alignItems:"center",gap:4,background:"rgba(248,112,54,.1)",border:"1px solid rgba(248,112,54,.25)",borderRadius:12,padding:"3px 10px 3px 12px",fontSize:13,color:"var(--c-accent-text)",fontWeight:600}}>
                    {alias}
                    <button onClick={()=>delAlias(n,alias)} style={{background:"none",border:"none",color:"var(--c-accent-text)",cursor:"pointer",padding:"0 0 0 4px",fontSize:14,lineHeight:1}}>×</button>
                  </div>
                ))}
              </div>}
              {/* 最新期間の未登録名から選ぶ */}
          ...
          {isPremium&&posKey===n&&(
            <div style={{marginTop:4,padding:"12px 14px",background:"rgba(59,130,246,.04)",border:"1px solid rgba(59,130,246,.2)",borderRadius:8,position:"sticky",left:0,maxWidth:"calc(100vw - 76px)",boxSizing:"border-box"}}>
              <div style={{fontSize:12,fontWeight:700,color:"var(--c-sat)",marginBottom:8}}>ポジション（設定タブで登録したポジションから選択）</div>
                  ...
                  return(
                    <div key={meal} style={{marginBottom:10}}>
                      ...
                      <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:6}}>
                        ...
                        {cur.map((p,pi)=>(
                          <div key={pi} style={{display:"flex",alignItems:"center",gap:4,background:"rgba(59,130,246,.1)",border:"1px solid rgba(59,130,246,.25)",borderRadius:12,padding:"3px 10px 3px 12px",fontSize:13,color:"var(--c-sat)",fontWeight:600}}>
                            {p}<button onClick={()=>savePositions(n,meal,cur.filter((_,ci)=>ci!==pi))} style={{background:"none",border:"none",color:"var(--c-sat)",cursor:"pointer",padding:"0 0 0 4px",fontSize:14,lineHeight:1}}>×</button>
                          </div>
                        ))}
                      </div>
                      <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
        ...
        {staffList.filter(n=>!isSpacer(n)).length>=lim&&<div style={{marginTop:10,fontSize:12,color:"var(--c-warn-text)",textAlign:"center"}}>▲ 上限に達しています。アップグレードするとさらに追加できます。</div>}
      </AC>
    </div>
  );
}
    ...
    <div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:16,flexWrap:"wrap"}}>
        {[["global","全体"],["weekday","曜日別"],["date","日付別"],["template","テンプレ"],...(plan==="premium"?[["break","休憩"]]:[])] .map(([id,l])=>(
          <button key={id} onClick={()=>setMode(id)} style={{padding:"8px 14px",background:mode===id?"var(--c-accent-solid)":"var(--c-border)",border:`1px solid ${mode===id?"var(--c-accent-solid)":"var(--c-border)"}`,borderRadius:8,color:mode===id?"white":"var(--c-text)",fontSize:13,fontWeight:600,cursor:"pointer"}}>{l}</button>
        ))}
      </div>
      ...
      {mode==="weekday"&&<AC title="曜日別候補（全体より優先）">
          ...
          {WDAY_OPTS.map(d=>{
            ...
            return(<button key={d} onClick={()=>setSelDows(prev=>prev.includes(d)?prev.filter(x=>x!==d):[...prev,d])}
              style={{padding:"7px 14px",borderRadius:12,fontSize:13,fontWeight:700,border:"1px solid",cursor:"pointer",
                background:sel?(isSat?"#1565C0":isSun?"#C81E2B":"var(--c-accent-solid)"):"var(--c-input)",
                borderColor:sel?"transparent":isSat?"rgba(147,197,253,.3)":isSun?"rgba(252,165,165,.3)":"var(--c-border2)",
                color:sel?"white":isSat?"var(--c-sat)":isSun?"var(--c-danger-text)":"var(--c-text2)"}}>
              {wdLabel(d)}
            </button>);
          })}
        </div>
        ...
        <div style={{marginBottom:16,padding:"12px",background:"var(--c-input2)",borderRadius:8}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            ...
            }} style={{padding:"6px 12px",background:"rgba(255,71,87,.15)",border:"1px solid rgba(255,71,87,.3)",borderRadius:8,color:"var(--c-danger-text)",fontSize:12,fontWeight:700,cursor:"pointer"}}>× 休業日に設定</button>
          </div>
        </div>
                ...
                {cands.length>0&&<div style={{padding:"6px 8px"}}>
                  {cands.map((c,i)=>(
                    <div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",
                      ...
                      {c.closed
                        ?<span style={{fontSize:13,color:"var(--c-danger-text)",fontWeight:600}}>× 休業日</span>
      ...
      {mode==="date"&&<AC title="日付別候補（最優先）">
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
            {selDates.sort().map(dt=>(
              <div key={dt} style={{display:"flex",alignItems:"center",gap:4,background:"rgba(248,112,54,.15)",border:"1px solid rgba(248,112,54,.3)",borderRadius:8,padding:"4px 8px"}}>
                <span style={{fontSize:12,color:"var(--c-accent-text)",fontWeight:600}}>{dt.replace(/-/g,"/")}</span>
                <button onClick={()=>setSelDates(prev=>prev.filter(d=>d!==dt))} style={{background:"none",border:"none",color:"var(--c-accent-text)",cursor:"pointer",fontSize:14,lineHeight:1,padding:0}}>×</button>
              </div>
            ))}
          </div>
        </div>}
        ...
        {(()=>{
          ...
          return(<div style={{marginTop:14}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:5}}>{dispDates.map(dt=>{const sel=selDates.includes(dt);return(<button key={dt} onClick={()=>setSelDates(prev=>(prev.length===1&&prev[0]===dt)?[]:[dt])} style={{padding:"4px 9px",borderRadius:4,background:sel?"var(--c-accent-solid)":"var(--c-border)",border:`1px solid ${sel?"var(--c-accent)":"var(--c-border2)"}`,color:"var(--c-text)",fontSize:11,fontWeight:600,cursor:"pointer"}}>{dt.replace(/-/g,"/")}（{((settings.dateCandidates||{})[dt]||[]).length}件）</button>);})}</div>
          </div>);
        })()}
      </AC>}

      {mode==="template"&&<AC title="曜日別候補テンプレート">
        {plan==="free"&&<div style={{background:"var(--c-warn-bg)",border:"1px solid var(--c-warn-border)",borderRadius:8,padding:"10px 14px",marginBottom:12,fontSize:13,color:"var(--c-warn-text)"}}>テンプレート機能はProプラン（500円/月）で利用できます</div>}
                ...
                {brks.map((b,i)=>{
                  ...
                  return(<div key={i} style={{padding:"8px 12px",borderBottom:i<brks.length-1?"1px solid var(--c-border)":"none"}}>
                    ...
                    <div style={{display:"flex",gap:5,flexWrap:"wrap",marginTop:5}}>
                      {tags.length>0?tags.map(tg=>(<span key={tg} style={{fontSize:11,padding:"2px 8px",borderRadius:12,background:"rgba(248,112,54,.12)",color:"var(--c-accent-text)",fontWeight:600}}>{attrName(tg)}</span>))
...
function SubsTab({subs,periods,staffList,onSave,tt,settings={},onSaveSettings,plan="free",onLoadPastSubs,pastSubsLoaded=false}){
  ...
  return(<div>
    ...
    <div style={{marginBottom:12,fontSize:13,color:"var(--c-text3)"}}>件数：<strong style={{color:"var(--c-accent-text)",fontSize:16}}>{fil.length}</strong></div>
            ...
            :fil.map(sub=>{const resolvedName=resolveAlias(sub.staffName,staffAliases);const ds=Object.keys(sub.shifts||{}).sort(),wkDays=ds.filter(d=>sub.shifts[d]&&sub.shifts[d].status==="work");const att=wkDays.reduce((acc,d)=>{const sh=sub.shifts[d];return acc+(shiftBandInfo(sh,settings).attendance||1);},0);const attLabel=`${att}日`;const at=new Date(sub.submittedAt).toLocaleString("ja-JP",{month:"numeric",day:"numeric",hour:"2-digit",minute:"2-digit"});const subPeriod=periods.find(p=>p.id===sub.periodId);const hasRealUpdate=subHasRealUpdate(sub,subPeriod?.deadlineDate);
              ...
              <td style={{padding:"10px 14px",borderBottom:"1px solid rgba(0,0,0,.03)",color:"var(--c-text)",fontWeight:600,...(hasVio?{boxShadow:"inset 2px 0 0 #FF4757"}:{})}}>
                <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                  ...
                  {hasRealUpdate&&<span style={{fontSize:10,background:"var(--c-accent-solid)",color:"#fff",padding:"2px 7px",borderRadius:4,fontWeight:700}}>変更あり</span>}
                  {/* 超過は異常だが「今すぐ操作する」項目ではないので、要対応バッジとは別の見え方にする */}
                  {hasVio&&<span style={{fontSize:11,color:"var(--c-danger-text)",fontWeight:700,whiteSpace:"nowrap"}}>{dailyVio?"1日超過":""}{dailyVio&&weeklyVio?" / ":""}{weeklyVio?"週超過":""}</span>}
                  {isPro&&isUnregistered(sub.staffName)&&(
                    linkTarget?.subName===sub.staffName
                      ...
                      :<button onClick={()=>setLinkTarget({subName:sub.staffName})}
                        style={{fontSize:10,background:"var(--c-accent-solid)",color:"#fff",border:"none",padding:"2px 8px",borderRadius:4,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap"}}>
                        別名を登録
                      </button>
                  )}
                </div>
              ...
              <td style={{padding:"10px 14px",borderBottom:"1px solid rgba(0,0,0,.03)",color:"var(--c-text3)",whiteSpace:"nowrap"}}>
                  {at}
                  {hasRealUpdate&&<><br/><span style={{fontSize:10,color:"var(--c-warn-text)",fontWeight:700}}>更新: {new Date(sub.updatedAt).toLocaleString("ja-JP",{month:"numeric",day:"numeric",hour:"2-digit",minute:"2-digit"})}</span></>}
                </td>
              <td style={{padding:"10px 14px",borderBottom:"1px solid rgba(0,0,0,.03)"}}><div><span style={{background:"rgba(248,112,54,.15)",color:"var(--c-accent-text)",border:"1px solid rgba(248,112,54,.3)",padding:"2px 8px",borderRadius:4,fontSize:12,fontWeight:600}}>{attLabel}</span></div></td>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            ...
            <tbody>{Object.keys(det.shifts||{}).sort().map(ds=>{const d=pd(ds),s=det.shifts[ds],iw=s&&s.status==="work";const detOT2=isPremium&&iw?getOT(det.staffName,settings,s):0;const nm=iw?calcNetWorkMinutes(s,getBreaksFor(settings,ds,det.staffName,s),detOT2,settings):0;const effEnd=isPremium&&iw&&detOT2>0&&(s.adjustedEnd??s.end)?`→${(()=>{const en=s.adjustedEnd??s.end;const[h,m]=en.split(":").map(Number);const tot=h*60+m+detOT2;return`${Math.floor(tot/60)}:${String(tot%60).padStart(2,"0")}`;})()}`:null;return(<tr key={ds}>
              ...
              <td style={{padding:"9px 12px",borderBottom:"1px solid var(--c-border)"}}>
                {iw?<div>{isPremium&&<div style={{color:"var(--c-text4)",fontSize:11}}>{s.start}</div>}{isPremium?<select value={s.adjustedStart||""} onChange={e=>saveAdj(det.id,ds,"adjustedStart",e.target.value||"")} style={{fontSize:16,padding:"3px 5px",background:"var(--c-input)",border:`1px solid ${s.adjustedStart?"#3B82F6":"var(--c-border)"}`,borderRadius:4,color:s.adjustedStart?"var(--c-sat)":"var(--c-text)",cursor:"pointer",marginTop:2,maxWidth:72}}><option value="">提出値</option>{TO.map(t=><option key={t} value={t}>{t}</option>)}</select>:<span style={{fontSize:13,color:"var(--c-text2)"}}>{s.start||"-"}</span>}</div>:"-"}
              </td>
              <td style={{padding:"9px 12px",borderBottom:"1px solid var(--c-border)"}}>
                {iw?<div>{isPremium&&<div style={{color:"var(--c-text4)",fontSize:11}}>{s.end}</div>}{isPremium?<><select value={s.adjustedEnd||""} onChange={e=>saveAdj(det.id,ds,"adjustedEnd",e.target.value||"")} style={{fontSize:16,padding:"3px 5px",background:"var(--c-input)",border:`1px solid ${s.adjustedEnd?"#3B82F6":"var(--c-border)"}`,borderRadius:4,color:s.adjustedEnd?"var(--c-sat)":"var(--c-text)",cursor:"pointer",marginTop:2,maxWidth:72}}><option value="">提出値</option>{TO.map(t=><option key={t} value={t}>{t}</option>)}</select>{effEnd&&<div style={{fontSize:10,color:"var(--c-ok)",marginTop:2,fontWeight:600}}>{effEnd}（+{detOT2}分）</div>}</>:<span style={{fontSize:13,color:"var(--c-text2)"}}>{s.end||"-"}</span>}</div>:"-"}
              </td>
      ...
      <div key={shop.id} style={{background:"var(--c-input)",borderRadius:8,border:`1px solid ${isCurrent?"rgba(248,112,54,.4)":"var(--c-border2)"}`,marginBottom:8,overflow:"hidden"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 12px",cursor:"pointer"}} onClick={()=>toggleExpand(shop.id)}>
          <div style={{display:"flex",alignItems:"center",gap:8,minWidth:0}}>
            ...
            {isCurrent&&<span style={{fontSize:10,background:"var(--c-accent-solid)",color:"white",padding:"2px 6px",borderRadius:4,fontWeight:700,flexShrink:0}}>表示中</span>}
          ...
          <div style={{display:"flex",gap:6,flexShrink:0}} onClick={e=>e.stopPropagation()}>
            {!isCurrent&&onSwitchToShop&&(
              <button onClick={()=>{onSwitchToShop(shop.id);tt(`✓ 「${shop.name}」に切り替えました`);}}
                style={{padding:"5px 10px",background:"rgba(248,112,54,.1)",border:"1px solid rgba(248,112,54,.3)",borderRadius:8,color:"var(--c-accent-text)",fontSize:12,fontWeight:600,cursor:"pointer"}}>
                ログイン
              </button>
            )}
                      ...
                      {others.map(os=>{
                        ...
                        return(<button key={os.id} onClick={()=>toggleWorkplace(shop.id,nm,os.id)}
                          style={{padding:"4px 10px",borderRadius:12,border:`1px solid ${sel?"var(--c-accent)":"var(--c-border2)"}`,
                            background:sel?"rgba(248,112,54,.12)":"var(--c-bg)",color:sel?"var(--c-accent-text)":"var(--c-text3)",
      ...
      {companyInfo?(
        <div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,background:"var(--c-input)",border:"1px solid var(--c-border2)",borderRadius:8,padding:"10px 14px",marginBottom:12}}>
            <span style={{flex:1,fontFamily:"monospace",fontSize:15,color:"var(--c-accent-text)",letterSpacing:"0.1em",fontWeight:700}}>{companyInfo.code}</span>
            <button onClick={()=>{
            ...
            }} style={{padding:"6px 12px",background:"var(--c-accent-solid)",border:"none",borderRadius:8,color:"white",fontSize:12,fontWeight:700,cursor:"pointer",flexShrink:0}}>コピー</button>
          </div>
          {/* パスワード変更 */}
          {coPwEdit?(
        ...
        authUser.isAnonymous?(
          ...
          <div>
            ...
            {coErr&&<div style={{color:"var(--c-danger-text)",fontSize:12,marginBottom:8}}>{coErr}</div>}
            {coCreated?(
              <div style={{background:"rgba(34,197,94,.1)",border:"1px solid rgba(34,197,94,.3)",borderRadius:8,padding:"12px 14px"}}>
                ...
                <div style={{fontFamily:"monospace",fontSize:16,color:"var(--c-ok)",fontWeight:700,letterSpacing:"0.1em"}}>{coCreated.code}</div>
              </div>
            ):(
              <button disabled={coBusy} onClick={async()=>{
                setCoErr("");
    ...
    {listShops.length>0&&<AC title="連携店舗">
      ...
      {companyInfo&&(
        coAddOpen?(
          ...
          <button onClick={()=>setCoAddOpen(true)} style={{width:"100%",padding:"10px",background:"rgba(248,112,54,.12)",border:"1px solid rgba(248,112,54,.3)",borderRadius:8,color:"var(--c-accent-text)",fontSize:13,fontWeight:700,cursor:"pointer",marginBottom:12}}>＋ 管理コードで追加</button>
        )
      )}
...
function SetTab({settings,onSave,subs,saveSubs,tt,syncStatus,plan="free",shopId,staffList=[],
  ...
  const providerRow=(pid,icon,label)=>{
    ...
    return(
      <div key={pid} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 0",borderBottom:"1px solid var(--c-border2)"}}>
        ...
        {linked
          ?<span style={{fontSize:11,fontWeight:600,color:"var(--c-ok)",background:"rgba(34,197,94,.12)",padding:"3px 10px",borderRadius:12,whiteSpace:"nowrap",flexShrink:0}}>連携済み</span>
  ...
  return(<div>
    ...
    {shopId&&<AC title="この端末の管理コード">
      {ownerReadOnly?(
        <div style={{fontSize:12,color:"var(--c-warn-text)",lineHeight:1.6}}>この端末は管理者登録されていないため、正しい管理コードを表示できません。既に管理者登録済みの端末（設定変更ができる端末）でこのコードを確認してください。</div>
      ):(<>
        ...
        {typeEntries.map(([type,limRaw])=>{
          ...
          return(<div key={type} style={{marginBottom:8,padding:"10px 12px",background:"var(--c-input)",border:"1px solid var(--c-border)",borderRadius:8}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
              ...
              {!isBuiltin&&<button onClick={()=>deleteType(type)} style={{padding:"4px 10px",background:"var(--c-danger-bg)",border:"1px solid var(--c-danger-border)",borderRadius:4,color:"var(--c-danger-text)",fontSize:12,cursor:"pointer"}}>削除</button>}
            </div>
        ...
        {pendingNewType&&<div style={{marginBottom:8,padding:"10px 12px",background:"var(--c-input)",border:"1px solid var(--c-accent)",borderRadius:8}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
            <input autoFocus value={pendingNewType.name} placeholder="属性名を入力"
              ...
              style={{...AI,flex:1,fontSize:16,fontWeight:700,padding:"4px 8px"}}/>
            <button onClick={confirmAddType} style={{padding:"4px 10px",background:"rgba(248,112,54,.15)",border:"1px solid var(--c-accent)",borderRadius:4,color:"var(--c-accent-text)",fontSize:12,cursor:"pointer"}}>追加</button>
          ...
          return(
            <div key={sec} style={{flex:"1 1 220px",minWidth:220}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:8}}>
                ...
                {list.map(p=>(
                  <div key={p} style={{display:"flex",alignItems:"center",gap:4,background:"rgba(248,112,54,.1)",border:"1px solid rgba(248,112,54,.25)",borderRadius:12,padding:"3px 10px 3px 12px",fontSize:13,color:"var(--c-accent-text)",fontWeight:600}}>
                    {p}<button onClick={()=>delPos(p)} style={{background:"none",border:"none",color:"var(--c-accent-text)",cursor:"pointer",padding:"0 0 0 4px",fontSize:14,lineHeight:1}}>×</button>
                  </div>
                ))}
              </div>
              <div style={{display:"flex",gap:6}}>
    ...
    {plan==="premium"&&<AC title="必要ポジション設定">
      ...
      <div style={{display:"flex",gap:6,marginBottom:8,flexWrap:"wrap"}}>
        {POSITION_DAY_TYPES.map(([id,label])=>(
          <button key={id} onClick={()=>setReqDayType(id)} style={{padding:"6px 12px",background:reqDayType===id?"var(--c-accent-solid)":"var(--c-input)",border:`1px solid ${reqDayType===id?"var(--c-accent)":"var(--c-border2)"}`,borderRadius:8,color:reqDayType===id?"white":"var(--c-text2)",fontSize:12,fontWeight:600,cursor:"pointer"}}>{label}</button>
        ))}
      </div>
      <div style={{display:"flex",gap:6,marginBottom:14}}>
        {[["lunch","ランチ"],["dinner","ディナー"]].map(([id,label])=>(
          <button key={id} onClick={()=>setReqMeal(id)} style={{padding:"6px 12px",background:reqMeal===id?"var(--c-accent-solid)":"var(--c-input)",border:`1px solid ${reqMeal===id?"var(--c-accent)":"var(--c-border2)"}`,borderRadius:8,color:reqMeal===id?"#fff":"var(--c-text2)",fontSize:12,fontWeight:600,cursor:"pointer"}}>{label}</button>
        ))}
      </div>
      {(()=>{
        const rp=settings.requiredPositions||{};
              ...
              return(
                <div key={sec} style={{flex:"1 1 220px",minWidth:220}}>
                  ...
                  <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:8,minHeight:32,alignContent:"flex-start"}}>
                    ...
                    {slots.map((p,i)=>(
                      <div key={i} style={{display:"flex",alignItems:"center",gap:4,background:"var(--c-danger-bg)",border:"1px solid var(--c-danger-border)",borderRadius:12,padding:"3px 10px 3px 12px",fontSize:13,color:"var(--c-danger-text)",fontWeight:600}}>
                        {p}<button onClick={()=>setCur(sec,slots.filter((_,ci)=>ci!==i))} style={{background:"none",border:"none",color:"var(--c-danger-text)",cursor:"pointer",padding:"0 0 0 4px",fontSize:14,lineHeight:1}}>×</button>
                      </div>
                    ))}
                  </div>
                  {options.length===0
        ...
        {[["2week","2週間（前半／後半）"],["1month","1ヶ月"]].map(([val,label])=>{
          ...
          return(<button key={val} onClick={()=>onSave({...settings,periodUnit:val})}
            style={{flex:1,padding:"10px 8px",borderRadius:8,border:`2px solid ${sel?"var(--c-accent)":"var(--c-border)"}`,
              background:sel?"rgba(248,112,54,.1)":"var(--c-input)",color:sel?"var(--c-accent-text)":"var(--c-text2)",
        ...
        {[["auto","↺ 自動（システム設定）",null],["light","ライト","light"],["dark","ダーク","dark"]].map(([key,label,val])=>{
          ...
          return(<button key={key} onClick={()=>changeTheme(val)}
            style={{flex:1,padding:"10px 8px",borderRadius:8,border:`1px solid ${sel?"var(--c-accent)":"var(--c-border2)"}`,
              background:sel?"var(--c-accent-solid)":"var(--c-input)",color:sel?"#fff":"var(--c-text2)",
      ...
      {acctLoading
        ...
        :acctEmailMode
          ?<div>
            ...
            {acctError&&<div style={{fontSize:12,color:"var(--c-danger-text)",marginBottom:10,background:"rgba(239,68,68,.08)",padding:"8px 10px",borderRadius:8}}>{acctError}</div>}
            ...
            }} style={{width:"100%",padding:"11px",background:"var(--c-accent-solid)",border:"none",borderRadius:8,color:"white",fontSize:14,fontWeight:700,cursor:"pointer",marginBottom:8,opacity:acctLoading?.5:1}}>
              {acctEmailMode==="login"?"ログイン":"アカウント作成"}
            </button>
            {acctEmailMode==="login"
              ?<div style={{textAlign:"center",fontSize:12,color:"var(--c-text4)"}}>アカウントがない場合は<button onClick={()=>{setAcctEmailMode("register");setAcctError("");}} style={{background:"none",border:"none",color:"var(--c-accent-text)",fontSize:12,cursor:"pointer",textDecoration:"underline"}}>新規登録</button></div>
              :<div style={{textAlign:"center",fontSize:12,color:"var(--c-text4)"}}>既にアカウントがある場合は<button onClick={()=>{setAcctEmailMode("login");setAcctError("");}} style={{background:"none",border:"none",color:"var(--c-accent-text)",fontSize:12,cursor:"pointer",textDecoration:"underline"}}>ログイン</button></div>
            }
          </div>
          :<div style={{display:"flex",flexDirection:"column",gap:8}}>
            ...
            {acctError&&<div style={{fontSize:12,color:"var(--c-danger-text)",background:"rgba(239,68,68,.08)",padding:"8px 10px",borderRadius:8}}>{acctError}</div>}
          </div>
      }
    </AC>}
    ...
    {authUser&&<AC title="アカウント連携">
      ...
      {linkError&&<div style={{marginTop:10,fontSize:12,color:"var(--c-danger-text)"}}>{linkError}</div>}
    </AC>}
...
function MyPageTab({plan="free",planExpiry,billingSchedule=null,staffList=[],periods=[],shopId,tt,onUpgrade}){
    ...
    return(
      <div style={{marginBottom:14}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
          ...
          <span style={{fontSize:13,fontWeight:700,color:isOver?"var(--c-danger-text)":max===Infinity?"var(--c-ok)":"var(--c-text)"}}>
  ...
  return(
    <div>
      ...
      <AC title="現在のプラン">
        <div style={{display:"flex",alignItems:"center",gap:16,padding:"8px 0 16px"}}>
          <div style={{flex:1}}>
            ...
            {!isPaid&&<div style={{fontSize:12,color:"var(--c-text3)",marginTop:3}}>無料プランをご利用中です</div>}
          </div>
        </div>
        ...
        {isPaid&&bs.cancelAtPeriodEnd&&(
          <div style={{background:"rgba(255,71,87,.08)",border:"1px solid rgba(255,71,87,.3)",borderRadius:8,padding:"10px 13px",marginBottom:12}}>
            <div style={{fontSize:13,fontWeight:700,color:"var(--c-danger-text)",marginBottom:2}}>解約済み</div>
        ...
        {isPaid&&!bs.cancelAtPeriodEnd&&bs.scheduledPlan&&bs.scheduledPlan!==plan&&(
          <div style={{background:"rgba(96,165,250,.08)",border:"1px solid rgba(96,165,250,.35)",borderRadius:8,padding:"10px 13px",marginBottom:12}}>
            <div style={{fontSize:13,fontWeight:700,color:"var(--c-sat)",marginBottom:2}}>プラン変更の予約中</div>
            <div style={{fontSize:12,color:"var(--c-text2)",lineHeight:1.6}}>
                ...
                return(
                  <button key={p} onClick={()=>changePlan(p)} disabled={!!changing}
                    style={{flex:"1 1 160px",padding:"11px 14px",borderRadius:8,cursor:changing?"default":"pointer",fontSize:14,fontWeight:700,
                      ...
                      background:isUp?"var(--c-accent-solid)":"var(--c-input)",color:isUp?"white":"var(--c-text2)",opacity:changing?.6:1}}>
        ...
        {plan==="free"&&<div style={{marginTop:4}}>
          ...
          <button onClick={()=>onUpgrade&&onUpgrade({type:"edit",plan})}
            style={{width:"100%",padding:"13px",background:"var(--c-accent-solid)",border:"none",borderRadius:8,color:"white",fontSize:15,fontWeight:700,cursor:"pointer",marginBottom:8}}>
                ...
                <tr key={feat}>
                  ...
                  {vals.map((v,i)=>(
                    <td key={i} style={{padding:"9px 6px",textAlign:"center",borderBottom:"1px solid var(--c-border)",
                      ...
                      color:v==="✓"?"var(--c-ok)":v==="✕"?"var(--c-text3)":"var(--c-text)",fontWeight:600}}>
                      {v}
                    </td>
                  ))}
                </tr>
      ...
      {(!isPaid||DEMO_MODE)&&<AC title="請求・サブスクリプション管理">
        <div style={{fontSize:13,color:"var(--c-text3)",textAlign:"center",padding:"8px 0"}}>
          {DEMO_MODE
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.65)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:20,animation:"fI .2s"}} onClick={onClose}>
      <div style={{background:"var(--c-card)",border:"1px solid var(--c-border)",borderRadius:12,width:"100%",maxWidth:400,padding:"28px 24px",animation:"sI .2s",boxShadow:"0 8px 40px rgba(0,0,0,.3)"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{background:"rgba(248,112,54,.08)",border:"1px solid rgba(248,112,54,.25)",borderRadius:12,padding:"14px 16px",marginBottom:16}}>
          <div style={{fontSize:12,fontWeight:700,color:"var(--c-accent-text)",marginBottom:8}}>プラン比較</div>
        ...
        <div style={{fontSize:11,color:"var(--c-text3)",lineHeight:1.7,marginBottom:14}}>
          ...
          <span style={{display:"block",marginTop:6}}>
            <a href="/terms.html" target="_blank" rel="noopener" style={{color:"var(--c-accent-text)",textDecoration:"none"}}>利用規約</a>
            <span style={{margin:"0 6px",color:"var(--c-text4)"}}>·</span>
            <a href="/privacy.html" target="_blank" rel="noopener" style={{color:"var(--c-accent-text)",textDecoration:"none"}}>プライバシーポリシー</a>
          </span>
        </div>
        {error&&<div style={{color:"var(--c-danger-text)",fontSize:12,textAlign:"center",marginBottom:10,background:"rgba(255,71,87,.1)",padding:"8px",borderRadius:8}}>{error}</div>}
        {done&&<div style={{color:"var(--c-ok)",fontSize:13,fontWeight:700,textAlign:"center",marginBottom:10,background:"rgba(34,197,94,.12)",padding:"10px",borderRadius:8}}>✓ {done}</div>}
        {/* 変更が完了したら購入ボタンは出さない（同じ操作を二度実行させない） */}
        {done?null:isEditType?(
          <button onClick={()=>checkout("premium")} disabled={!!loading} style={{width:"100%",padding:"13px",background:loading==="premium"?"var(--c-text3)":"var(--c-accent-solid)",border:"none",borderRadius:8,color:"white",fontSize:15,fontWeight:700,cursor:loading?"not-allowed":"pointer",marginBottom:12}}>
          ...
          <button onClick={()=>checkout("pro")} disabled={!!loading} style={{width:"100%",padding:"13px",background:loading==="pro"?"var(--c-text3)":"var(--c-accent-solid)",border:"none",borderRadius:8,color:"white",fontSize:15,fontWeight:700,cursor:loading?"not-allowed":"pointer",marginBottom:12}}>
        ...
        <button onClick={onClose} style={{width:"100%",padding:"11px",background:done?"var(--c-accent-solid)":"var(--c-input)",border:done?"none":"1px solid var(--c-border)",borderRadius:8,color:done?"white":"var(--c-text3)",fontSize:done?15:13,fontWeight:done?700:400,cursor:"pointer"}}>{done?"閉じる":"今はしない"}</button>
      </div>
    </div>
  );
}
...
function CL({items,onDel}){return items.map((c,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:c.closed?"rgba(255,71,87,.08)":"rgba(255,255,255,.05)",border:`1px solid ${c.closed?"rgba(255,71,87,.2)":"var(--c-border)"}`,borderRadius:8,marginBottom:6}}>
  {c.closed
    ?<span style={{flex:1,fontSize:14,color:"var(--c-danger-text)",fontWeight:600}}>× 休業日</span>
