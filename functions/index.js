  ...
  .https.onRequest(async (req, res) => {
    ...
    try {
      const session = await stripe.checkout.sessions.create({
        ...
        success_url: successUrl || "https://aech22.github.io/shifty/?payment=success",
        cancel_url:  cancelUrl  || "https://aech22.github.io/shifty/?payment=cancel",
        locale: "ja",
      });
  ...
  .https.onRequest(async (req, res) => {
    ...
    try {
      const session = await stripe.billingPortal.sessions.create({
        customer: customerId,
        return_url: returnUrl || "https://aech22.github.io/shifty/",
      });
      res.status(200).json({ url: session.url });
    } catch (e) {
